...
BATCH_ROOT = Path("/Users/duhokim/NebulaMind/NebulaMind/.hermes/handoffs/galaxy-evolution/mastermind/aas-autopilot/dr-research-lane-9-20260714")
PROMPT_DIR = BATCH_ROOT / "prompts"
PACKET_DIR = BATCH_ROOT / "packets"
STATE_PATH = PACKET_DIR / "DR_RESEARCH_BATCH_9_STATE.json"
SUMMARY_PATH = PACKET_DIR / "DR_RESEARCH_BATCH_9_FINAL_SUMMARY_VERIFIED.md"
HOLD_SUMMARY_PATH = PACKET_DIR / "DR_RESEARCH_BATCH_9_HOLD_SUMMARY.md"
LEDGER_PATH = Path("ledger/RUN_LEDGER.jsonl")
JOURNAL_PYTHON = "/Users/duhokim/NebulaMind/NebulaMind/backend/.venv/bin/python"
HWAO_TARGET = "hwao-pilot-gemini-resume:0.0"
...
def initial_state(specs):
    return {
        "batch_id": "DR_RESEARCH_BATCH_9_REFERENCE_20260714",
        "created_utc": utcnow(),
        "updated_utc": utcnow(),
        "advisory_only": True,
        "reference_only": True,
...
def current_prompt_identity(page, prompt):
    ...
    if query_text != expected_query or title_without_product != prompt_title:
        return None
    return {
...
def journal_entry(etype, note, files):
    ...
    args = [JOURNAL_PYTHON, "-B", "broker/journal.py", str(LEDGER_PATH), "tori", etype, note] + [str(Path(path)) for path in files]
...
def stage_and_submit(spec, state, browser):
    ...
    holder = f"tori-goru-dr9-{paper_id}-submit"
...
def wait_for_plan(identity, spec, state, browser):
    ...
    holder = f"tori-goru-dr9-{paper_id}-plan"
...
def start_research(identity, spec, state, browser):
    paper_id = spec["paper_id"]; path = identity["conversation_path"]
    holder = f"tori-goru-dr9-{paper_id}-start"
    client = UDSClient(SOCK); target_lease = None; account_lease = None
    try:
        target_lease = acquire_target(client, holder, "write", ttl=1200)
        page = exact_page(browser, path)
...
def poll_terminal(identity, spec, state, browser):
    paper_id = spec["paper_id"]; path = identity["conversation_path"]
    holder = f"tori-goru-dr9-{paper_id}-monitor"
    client = UDSClient(SOCK); lease = None
    try:
...
def save_packet(snapshot, result_sha, identity, spec, state):
    ...
    lines.extend(["", "## Reference-only safety receipt", "", "- advisory_only: true", "- No `.tex` edit or auto-apply is authorized or performed by this lane.", "- No DB, API, wiki, trust, autopilot-lane, deploy, git, publish, cron, billing, account, credential, or secret mutation is authorized or performed.", ""])
    ...
    note = (f"REFERENCE-ONLY {spec['paper_id']} Deep Research packet saved/verified before deletion; advisory_only=true; "
            ...
            "no .tex/DB/autopilot-lane/auto-apply/deploy/git/publish/cron/account/secret mutation.")
    entry = journal_entry(f"dr9_{spec['paper_id']}_reference_packet_saved_verified", note, [packet_path, metadata_path, spec["prompt_path"]])
...
def delete_exact_own(identity, spec, state, browser):
    ...
    if not saved or saved.get("entry_sha256") != save_sha or saved.get("type") != f"dr9_{paper_id}_reference_packet_saved_verified":
    ...
    holder = f"tori-goru-dr9-{paper_id}-delete"
    client = UDSClient(SOCK); lease = None
    try:
        ...
        entry = journal_entry(f"dr9_{paper_id}_exact_own_deleted", note, [deletion_path, packet_path, metadata_path])
...
def recover_new_chat(identity, spec, browser):
    ...
    if record["path"] != own_path:
        return False
    holder = f"tori-goru-dr9-{spec['paper_id']}-recover"
    client = UDSClient(SOCK); lease = None
    try:
...
def write_failure(spec, state, exc, identity=None):
    ...
    entry = None
    with suppress(Exception):
        entry = journal_entry(f"dr9_{spec['paper_id']}_failed_or_stopped", f"REFERENCE-ONLY {spec['paper_id']} failed/stopped; error={type(exc).__name__}: {str(exc)[:500]}; failure_sha256={failure_sha}; no auto-apply or protected mutation.", [failure_path, spec["prompt_path"]])
    ...
    report_hwao(f"DR9 {spec['paper_id']} FAILED/STOPPED reference-only: {type(exc).__name__}: {str(exc)[:500]}. failure={failure_path} sha={failure_sha}. No protected mutation. Next paper proceeds only if broker/target rails permit.")
...
def write_summary(state, final=True):
    ...
    lines = [
        ...
        "## Hard boundary", "", "No `.tex`, DB, autopilot-lane, auto-apply, deploy, git, publish, cron, billing, account, credential, or secret mutation was authorized or performed by this runner.", "",
    ...
    lines.extend(["## Downstream gate", "", "These packets are advisory references only. Hwao/local validators must verify identifiers, source roles, wording, and manuscript fit before any separately approved candidate-copy edit. Nothing is auto-applied.", ""])
    ...
    save_state(state)
    entry_type = "dr9_reference_batch_final_summary" if final else "dr9_reference_batch_hold_summary"
...
def main():
    ...
    report_hwao(f"DR9 batch runner ACTIVE starting paper_{args.start:02d}; reference-only packets under {PACKET_DIR}; exact target {TARGET_ID}; one run at a time; no protected mutation.")
