...
BATCH_ROOT = Path("/Users/duhokim/NebulaMind/NebulaMind/.hermes/handoffs/galaxy-evolution/mastermind/aas-autopilot/dr-revised-20260714/round2")
PROMPT_DIR = BATCH_ROOT / "dr-reresearch-prompts"
PACKET_DIR = BATCH_ROOT / "dr-reresearch-packets"
STATE_PATH = PACKET_DIR / "ROUND2_DR_RERESEARCH_STATE.json"
SUMMARY_PATH = PACKET_DIR / "ROUND2_DR_RERESEARCH_FINAL_SUMMARY.md"
HOLD_SUMMARY_PATH = PACKET_DIR / "ROUND2_DR_RERESEARCH_HOLD_SUMMARY.md"
...
def sidebar_title_normalized(text):
    """Normalize Gemini's optional spaces around zero-width math separators."""
    return re.sub(r"\s*\u200b\s*", "\u200b", normalized(text))
...
def stage_and_submit(spec, state, browser):
    ...
    holder = f"goru-dr-reresearch-r2-{paper_id}-submit"
    ...
    try:
        ...
        if not (active.count() == 1 and active.is_visible()):
            # A prior preflight can leave the tools menu open even though it
            # performed no account action.  Prefer the semantic checkbox role
            # and open the menu only when that exact control is not visible.
            deep = page.get_by_role("menuitemcheckbox", name="Deep research", exact=True)
            ...
            if len(visible) != 1:
                ...
                tools.click()
                try:
                    deep.wait_for(state="visible", timeout=5000)
                except Exception:
                    pass
                visible = [deep.nth(i) for i in range(deep.count()) if deep.nth(i).is_visible()]
            if len(visible) != 1:
...
def wait_for_plan(identity, spec, state, browser):
    ...
    holder = f"goru-dr-reresearch-r2-{paper_id}-plan"
...
def start_research(identity, spec, state, browser):
    paper_id = spec["paper_id"]; path = identity["conversation_path"]
    holder = f"goru-dr-reresearch-r2-{paper_id}-start"
    client = UDSClient(SOCK); target_lease = None; account_lease = None
    try:
        target_lease = acquire_target(client, holder, "write", ttl=1200)
        page = exact_page(browser, path)
...
def poll_terminal(identity, spec, state, browser):
    paper_id = spec["paper_id"]; path = identity["conversation_path"]
    holder = f"goru-dr-reresearch-r2-{paper_id}-monitor"
    client = UDSClient(SOCK); lease = None
    try:
...
def save_packet(snapshot, result_sha, identity, spec, state):
    ...
    if raw_start not in packet_text:
        raise PaperFailure("saved packet result start marker absent")
    # The verbatim prompt can itself embed a complete prior packet, including
    # both wrapper headings.  The current packet's wrapper is the final one.
    result_and_suffix = packet_text.rsplit(raw_start, 1)[1]
    # The verbatim re-research prompt embeds the prior round's full packet,
    # including its own "Captured source anchors" heading.  Verify against the
    # final wrapper delimiter after the new result rather than requiring that
    # heading to occur only once in the entire saved packet.
    if raw_end not in result_and_suffix:
        raise PaperFailure("saved packet result end marker absent")
    saved_raw = result_and_suffix.rsplit(raw_end, 1)[0]
    ...
    entry = journal_entry(f"dr_reresearch_r2_{spec['paper_id']}_reference_packet_saved_verified", note, [packet_path, metadata_path, spec["prompt_path"]])
...
def delete_exact_own(identity, spec, state, browser):
    ...
    if not saved or saved.get("entry_sha256") != save_sha or saved.get("type") != f"dr_reresearch_r2_{paper_id}_reference_packet_saved_verified":
    ...
    holder = f"goru-dr-reresearch-r2-{paper_id}-delete"
    client = UDSClient(SOCK); lease = None
    try:
        ...
        pre_links = page.locator(f'a[href="{path}"]')
        pre_visible = [pre_links.nth(i) for i in range(pre_links.count()) if pre_links.nth(i).is_visible()]
        exact_row_identity = None
        if len(pre_visible) == 1:
            pre_title = normalized(pre_visible[0].inner_text())
            if pre_title and (pre_title.startswith(captured_title) or captured_title.startswith(pre_title)):
                exact_row_identity = {
                    "evidence": "exact_route_plus_unique_history_row_plus_captured_title_prefix",
                    "row_title_normalized_sha256": sha_bytes(pre_title.encode()),
                }
        if not prompt_identity and not result_identity and not exact_row_identity:
        ...
        if len(visible) == 1:
            ...
            if (
                not options_label.startswith(prefix)
                or sidebar_title_normalized(options_label[len(prefix):])
                != sidebar_title_normalized(current_title)
            ):
        ...
        evidence = {
            ...
            "exact_history_row_identity_evidence": exact_row_identity,
        ...
        entry = journal_entry(f"dr_reresearch_r2_{paper_id}_exact_own_deleted", note, [deletion_path, packet_path, metadata_path])
...
def recover_new_chat(identity, spec, browser):
    ...
    if record["path"] != own_path:
        return False
    holder = f"goru-dr-reresearch-r2-{spec['paper_id']}-recover"
    client = UDSClient(SOCK); lease = None
    try:
...
def write_failure(spec, state, exc, identity=None):
    ...
    entry = None
    with suppress(Exception):
        entry = journal_entry(f"dr_reresearch_r2_{spec['paper_id']}_failed_or_stopped", f"REFERENCE-ONLY round-2 re-research {spec['paper_id']} failed/stopped; error={type(exc).__name__}: {str(exc)[:500]}; failure_sha256={failure_sha}; no auto-apply or protected mutation.", [failure_path, spec["prompt_path"]])
...
def restore_submitted_owned_route(identity, spec, state, browser):
    ...
    holder = f"goru-dr-reresearch-r2-{spec['paper_id']}-restore"
...
def write_summary(state, final=True):
    ...
    save_state(state)
    entry_type = "dr_reresearch_r2_reference_batch_final_summary" if final else "dr_reresearch_r2_reference_batch_hold_summary"
