...
PAPER_NUMBER = 4
GATE_PATH = Path("receipts/TORI_RELAY_DUHO_FRESH_GENTLE_PAPER04_20260715T015802Z.md").resolve()
GATE_SHA256 = "747494ac23d6fef6a4781e917ad3aefbee49e6747c2aa24fc064f5f9e103cbba"
PRECHECK_V1_PATH = r.PACKET_DIR / "paper_04_round1_review_dr_packet.gentle_precheck.json"
PRECHECK_V1_SHA256 = "340222e4206a961b7deb36b74142ab1feffe2608c393fac766583c1cb43be270"
PRECHECK_PATH = r.PACKET_DIR / "paper_04_round1_review_dr_packet.gentle_precheck_v2.json"
...
def freeze_for_redirect(client, holder, phase, actual_url):
    ...
    if (parsed.netloc == "www.google.com" and parsed.path.startswith("/sorry")) or parsed.netloc == "accounts.google.com":
        r.freeze_for_challenge(client, holder, "paper_04", phase)


def verify_gate_state(state, spec):
    if hashlib.sha256(GATE_PATH.read_bytes()).hexdigest() != GATE_SHA256:
        raise RuntimeError("fresh paper_04 gate hash mismatch")
    ...
    if paper.get("status") != "failed":
        raise RuntimeError(f"paper_04 expected failed state, got {paper.get('status')}")
    identity = paper.get("identity") or {}
    if identity.get("conversation_id") != "e6d8c8cdaaa88040" or identity.get("conversation_path") != "/app/e6d8c8cdaaa88040":
        raise RuntimeError("paper_04 exact-owned identity mismatch")
    if identity.get("prompt_sha256") != spec["prompt_sha256"]:
        raise RuntimeError("paper_04 prompt identity mismatch")
    failure = Path(paper["failure_path"])
    if not failure.is_file() or r.sha_file(failure) != paper.get("failure_sha256"):
        raise RuntimeError("paper_04 first immutable failure receipt mismatch")
    if not PRECHECK_V1_PATH.is_file() or r.sha_file(PRECHECK_V1_PATH) != PRECHECK_V1_SHA256:
        raise RuntimeError("paper_04 first gentle precheck receipt mismatch")
    precheck_v1 = json.loads(PRECHECK_V1_PATH.read_text())
    if not (
        precheck_v1.get("classification") == "unaccepted_or_ambiguous_hold"
        and precheck_v1.get("message_count") == 1
        and precheck_v1.get("prompt_identity", {}).get("evidence") == "exact_route_plus_visible_user_query_plus_document_title"
        and precheck_v1.get("start_controls") == [{"disabled": False, "label": "Start research"}]
        and precheck_v1.get("challenge") is False
        and precheck_v1.get("research") is False
        and precheck_v1.get("stop") is False
        and precheck_v1.get("failure") is False
        and precheck_v1.get("fresh_start_actions_allowed") == 0
    ):
        raise RuntimeError("paper_04 first gentle precheck is not the expected zero-submit false-negative")
    for output in (spec["packet_path"], spec["metadata_path"], spec["deletion_path"], PRECHECK_PATH):
        if Path(output).exists():
            raise RuntimeError(f"paper_04 fresh output already exists: {output}")
    ...
    if not ok:
        raise RuntimeError(f"ledger invalid before paper_04: {message}")
    ...
    if spacing_seconds < 20 * 60:
        raise RuntimeError(f"gentle spacing not met: {spacing_seconds:.1f} seconds")
    return paper, identity, message, last, spacing_seconds


def restore_exact_paper04(browser, identity):
    desired = identity["conversation_path"]
    record = r.target_record()
    if not record:
        raise RuntimeError("exact target record absent before paper_04 restore")
    if record["path"] == desired:
        return
    allowed_current = {"/app/bd0b18ee06967bbe", "/app"}
    if record["path"] not in allowed_current:
        raise RuntimeError(f"unexpected current route before paper_04 restore: {record['path']}")
    holder = "goru-paper04-gentle-restore"
    ...
    try:
        ...
        if r.page_challenge(page):
            r.freeze_for_challenge(client, holder, "paper_04", "before exact-owned restore")
        r.check_action(client, lease, "paper_04 navigate from held paper_03/new-chat route to exact-owned route", record["path"], page)
        ...
        if r.page_challenge(page):
            r.freeze_for_challenge(client, holder, "paper_04", "after exact-owned restore")
        parsed = urlparse(page.url)
        if parsed.scheme != "https" or parsed.netloc != "gemini.google.com" or parsed.path != desired:
            r.check_action(client, lease, "paper_04 fail closed unexpected restored destination", desired, page)
            raise r.TargetDrift(f"paper_04 restored destination mismatch: {parsed.netloc}{parsed.path}")
        r.check_action(client, lease, "paper_04 verify restored exact-owned route", desired, page)
    finally:
...
def classify_current(page, spec, identity):
    ...
    client = r.UDSClient(r.SOCK)
    lease = r.acquire_target(client, "goru-paper04-gentle-classify", "read", ttl=240, max_wait=20)
    try:
        freeze_for_redirect(client, "goru-paper04-gentle-classify", "classification redirect", page.url)
        if r.page_challenge(page):
            r.freeze_for_challenge(client, "goru-paper04-gentle-classify", "paper_04", "classification")
        r.check_action(client, lease, "paper_04 read-only exact-owned classification", path, page)
...
def main():
    ...
    paper, identity, ledger_before, last_account, spacing_seconds = verify_gate_state(state, spec)

    try:
        with sync_playwright() as pw:
            browser = pw.chromium.connect_over_cdp(r.BASE)
            restore_exact_paper04(browser, identity)
            ...
            precheck = {
                "status": "PAPER04_GENTLE_PRECHECK",
                "paper_id": "paper_04",
                ...
                "previous_zero_submit_precheck_path": str(PRECHECK_V1_PATH),
                "previous_zero_submit_precheck_sha256": PRECHECK_V1_SHA256,
                "classification_repair": "Gemini's restored plan exposes one assistant message-content plus the separately verified user-query; one or two message-content nodes are accepted only with exact route, exact prompt identity, exactly one enabled Start, and zero active/terminal/failure/challenge signals.",
            ...
            if classification == "unaccepted_or_ambiguous_hold":
                ...
                report_hwao(f"PAPER_04 GENTLE BACKOFF before submit: exact route was ambiguous/unaccepted; no prompt Send and no Start action performed. Hold receipt {PRECHECK_PATH} sha {precheck_sha}. Yui remains held.")
                print(json.dumps({"status": "PAPER04_GENTLE_BACKOFF_NO_SUBMIT", "precheck": str(PRECHECK_PATH), "precheck_sha256": precheck_sha, "ledger_verify": ledger_before}, sort_keys=True))
                browser.close()
                return 2
            ...
            if classification == "unchanged_plan_one_fresh_start_allowed":
                ...
                report_hwao(f"PAPER_04 GENTLE SUBMIT SETTLED: one broker-serialized Start action accepted; account lease {current.get('research_start_account_lease_id')}; UTC {current.get('research_start_utc')}. No prompt resend. Yui may be considered for Hwao's separate clearance; no paper_05 action is authorized.")
            else:
                ...
                report_hwao(f"PAPER_04 GENTLE SUBMIT ALREADY SETTLED: pre-gate Start was delayed-accepted ({classification}); no new prompt or Start action. Goru is continuing read-only result custody. Hwao may separately decide Yui clearance; no paper_05 action is authorized.")
            ...
            try:
                ...
                report_hwao(f"PAPER_04 saved/verified but exact-own cleanup held: {type(cleanup_exc).__name__}: {str(cleanup_exc)[:400]}. No retry; no paper_05. Packet remains safe.")
            browser.close()

        ok, ledger_after = r.ledger.verify(r.LEDGER_PATH)
        if not ok:
            raise RuntimeError(f"ledger invalid after paper_04: {ledger_after}")
        ...
        result = {
            "status": "GORU_GENTLE_PAPER04_COMPLETE" if cleanup_status == "exact_owned_deleted" else "GORU_GENTLE_PAPER04_SAVED_VERIFIED_CLEANUP_HOLD",
            "paper_id": "paper_04",
            ...
            "next_action": "WAIT; no paper_05 action is authorized",
        }
        ...
        report_hwao(f"PAPER_04 CHALLENGE STOP/FROZEN: {exc}. Never interact. Wake Duho. No retry and no paper_05; Yui remains held.")
        raise
    except r.PaperFailure as exc:
        with __import__("contextlib").suppress(Exception):
            r.write_failure(spec, state, exc, identity)
        report_hwao(f"PAPER_04 GENTLE FIRST UNACCEPTED/SOFT-THROTTLE BACKOFF: {exc}. No retry; no paper_05. Yui remains held pending Hwao.")
        print(json.dumps({"status": "PAPER04_GENTLE_BACKOFF_NO_RETRY", "error": str(exc), "precheck": str(PRECHECK_PATH)}, sort_keys=True))
        return 2
