...
PROMPT_PATH = AREA_DIR / "area_review_01_kennicutt_evans_2012_DR_PROMPT.md"
RAW_PACKET_PATH = AREA_DIR / "area_review_01_kennicutt_evans_2012_DR_RAW_PACKET.md"
RAW_METADATA_PATH = AREA_DIR / "area_review_01_kennicutt_evans_2012_DR_PACKET.raw.metadata.json"
CANONICAL_PACKET_PATH = AREA_DIR / "area_review_01_kennicutt_evans_2012_DR_PACKET.md"
STATE_PATH = AREA_DIR / "area_review_01_kennicutt_evans_2012_DR_STATE.json"
FAILURE_PATH = AREA_DIR / "area_review_01_kennicutt_evans_2012_DR_FAILURE.json"
RUN_ID = "REVIEW_BASE_01_KENNICUTT_EVANS_2012"
MARKER = "REVIEW_BASE_01_DR_COMPLETE_REFERENCE_ONLY"
FIRST_NOT_BEFORE = "2026-07-15T12:13:29Z"
SPACING_MINUTES = 30
...
def update_manifest(status, **fields):
    ...
    item = next(row for row in manifest["queue"] if row["id"] == 1)
    item.update(fields)
...
def spec():
    ...
    return {
        "paper": 1,
        "paper_id": "review_base_01",
        "shortname": "kennicutt_evans_2012_review_source_base",
...
def initial_state(current_spec):
    ...
    return {
        ...
        "queue_item": 1,
        "review": "Kennicutt & Evans 2012 — Star Formation in the Milky Way and Nearby Galaxies",
        "review_identity": {
            "doi": "10.1146/annurev-astro-081811-125610",
            "arxiv": "1204.3552",
            "ads_bibcode": "2012ARA&A..50..531K",
...
def wait_for_safe_gap(state):
    ...
    while True:
        ...
        if broker.get("frozen"):
            raise r.GlobalChallengeStop("broker frozen before Review Base item 01 dispatch")
        ...
        update_manifest("WAITING_ITEM_01_STANDARD_GAP" if remaining > 0 or live_account else "ITEM_01_ACCOUNT_GATE_READY", account_gate_not_before=state["account_gate_not_before"])
...
def prepare_new_chat(browser, state):
    ...
    if record["path"] == "/app":
        ...
        return
    holder = "goru-review-base-01-new-chat"
    ...
    try:
        ...
        if r.page_challenge(page):
            r.freeze_for_challenge(client, holder, "review_base_01", "pre-dispatch current-route classification")
        r.check_action(client, lease, "review_base_01 navigate to new chat", record["path"], page)
        page.goto("https://gemini.google.com/app", wait_until="domcontentloaded", timeout=30000)
        page.wait_for_timeout(1200)
        if r.page_challenge(page):
            r.freeze_for_challenge(client, holder, "review_base_01", "post-navigation new-chat classification")
        if not r.target_matches("/app", page):
            raise r.TargetDrift("Review Base item 01 new-chat route failed exact verification")
...
def classify_after_start_timeout(identity, prompt, browser):
    ...
    holder = "goru-review-base-01-start-settlement"
    client = r.UDSClient(r.SOCK)
    lease = None
    try:
        ...
        if r.page_challenge(page):
            r.freeze_for_challenge(client, holder, "review_base_01", "post-Start timeout settlement")
...
def output_quality(result):
    lower = result.lower()
    counts = {prefix: len(set(re.findall(rf"\[REV01-{prefix}\d{{2}}\]", result))) for prefix in ("E", "D", "N", "U")}
    primary_count = len(set(re.findall(r"\[REV01-P\d{3}\]", result)))
    ...
    checks = {
        ...
        "review_doi_present": "10.1146/annurev-astro-081811-125610" in result,
        "review_arxiv_present": "1204.3552" in result,
        "review_ads_present": "2012ARA&A..50..531K" in result,
...
def save_packet(snapshot, result_sha, identity, current_spec, state):
    ...
    lines = [
        "# Review Base 01 raw Deep Research packet — Kennicutt & Evans 2012",
        "",
        "advisory_only: true",
        "canonical_packet_released: false",
    ...
    if r.sha_bytes(raw.encode()) != result_sha:
        raise r.PaperFailure("Review Base item 01 raw-result custody mismatch")
    entry = r.journal_entry(
        "review_base_01_raw_packet_saved",
        f"Review Base 01 Kennicutt & Evans 2012 raw packet saved; conversation={identity['conversation_id']}; raw_sha={r.sha_file(RAW_PACKET_PATH)}; output_shape_pass={quality['pass']}; canonical_released=false; no wiki/DB write; no deletion.",
    ...
    update_manifest("ITEM_01_RAW_LANDED_LOCAL_VERIFICATION_PENDING", conversation_id=identity["conversation_id"], submit_utc=identity["submit_utc"], research_start_utc=state["paper"].get("research_start_utc"), raw_packet_path=str(RAW_PACKET_PATH), raw_packet_sha256=r.sha_file(RAW_PACKET_PATH), output_shape_pass=quality["pass"])
    return metadata
...
def write_failure(state, exc, identity=None):
    ...
    update_manifest("QUEUE_HOLD_ITEM_01", failure_path=str(FAILURE_PATH), error=f"{type(exc).__name__}: {exc}")
    with suppress(Exception):
        r.journal_entry("review_base_01_queue_hold", f"Review Base queue HOLD on item 01: {type(exc).__name__}: {exc}; retry=false; challenge={failure['global_challenge_stop']}", [QUEUE_PATH, MANIFEST_PATH, PROMPT_PATH, FAILURE_PATH])
    r.report_hwao(f"REVIEW-BASE HARD HOLD item 01: {type(exc).__name__}: {exc}; no retry; queue stopped; challenge={failure['global_challenge_stop']}.")
