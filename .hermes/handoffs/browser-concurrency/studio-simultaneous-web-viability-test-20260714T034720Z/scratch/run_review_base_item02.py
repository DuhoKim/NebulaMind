...
PROMPT_PATH = AREA_DIR / "area_review_02_madau_dickinson_2014_DR_PROMPT.md"
RAW_PACKET_PATH = AREA_DIR / "area_review_02_madau_dickinson_2014_DR_RAW_PACKET.md"
RAW_METADATA_PATH = AREA_DIR / "area_review_02_madau_dickinson_2014_DR_PACKET.raw.metadata.json"
CANONICAL_PACKET_PATH = AREA_DIR / "area_review_02_madau_dickinson_2014_DR_PACKET.md"
STATE_PATH = AREA_DIR / "area_review_02_madau_dickinson_2014_DR_STATE.json"
FAILURE_PATH = AREA_DIR / "area_review_02_madau_dickinson_2014_DR_FAILURE.json"
RUN_ID = "REVIEW_BASE_02_MADAU_DICKINSON_2014"
MARKER = "REVIEW_BASE_02_DR_COMPLETE_REFERENCE_ONLY"
FIRST_NOT_BEFORE = "2026-07-15T12:48:51.425950Z"
SPACING_MINUTES = 31
...
def update_manifest(status, **fields):
    ...
    item = next(row for row in manifest["queue"] if row["id"] == 2)
    item.update(fields)
...
def spec():
    ...
    return {
        ...
        "paper": 1,
        "paper_id": "review_base_02",
        "shortname": "madau_dickinson_2014_review_source_base",
...
def initial_state(current_spec):
    ...
    return {
        ...
        "queue_item": 2,
        "review": "Madau & Dickinson 2014 — Cosmic Star-Formation History",
        "review_identity": {
            "doi": "10.1146/annurev-astro-081811-125615",
            "arxiv": "1403.0007",
            "ads_bibcode": "2014ARA&A..52..415M",
...
def wait_for_safe_gap(state):
    ...
    while True:
        ...
        if broker.get("frozen"):
            raise r.GlobalChallengeStop("broker frozen before Review Base item 02 dispatch")
        ...
        update_manifest("WAITING_ITEM_02_STANDARD_GAP" if remaining > 0 or live_account else "ITEM_02_ACCOUNT_GATE_READY", account_gate_not_before=state["account_gate_not_before"])
...
def prepare_new_chat(browser, state):
    ...
    if record["path"] == "/app":
        ...
        return
    holder = "goru-review-base-02-new-chat"
    ...
    try:
        ...
        if r.page_challenge(page):
            r.freeze_for_challenge(client, holder, "review_base_02", "pre-dispatch current-route classification")
        r.check_action(client, lease, "review_base_02 navigate to new chat", record["path"], page)
        page.goto("https://gemini.google.com/app", wait_until="domcontentloaded", timeout=30000)
        page.wait_for_timeout(1200)
        if r.page_challenge(page):
            r.freeze_for_challenge(client, holder, "review_base_02", "post-navigation new-chat classification")
        if not r.target_matches("/app", page):
            raise r.TargetDrift("Review Base item 02 new-chat route failed exact verification")
...
def classify_after_start_timeout(identity, prompt, browser):
    ...
    holder = "goru-review-base-02-start-settlement"
    client = r.UDSClient(r.SOCK)
    lease = None
    try:
        ...
        if r.page_challenge(page):
            r.freeze_for_challenge(client, holder, "review_base_02", "post-Start timeout settlement")
...
def output_quality(result):
    lower = result.lower()
    counts = {prefix: len(set(re.findall(rf"\[REV02-{prefix}\d{{2}}\]", result))) for prefix in ("E", "D", "N", "U")}
    primary_count = len(set(re.findall(r"\[REV02-P\d{3}\]", result)))
    ...
    checks = {
        ...
        "review_doi_present": "10.1146/annurev-astro-081811-125615" in result,
        "review_arxiv_present": "1403.0007" in result,
        "review_ads_present": "2014ARA&A..52..415M" in result,
...
def save_packet(snapshot, result_sha, identity, current_spec, state):
    ...
    if r.sha_bytes(raw.encode()) != result_sha:
        raise r.PaperFailure("Review Base item 02 raw-result custody mismatch")
    entry = r.journal_entry(
        "review_base_02_raw_packet_saved",
        f"Review Base 02 Madau & Dickinson 2014 raw packet saved; conversation={identity['conversation_id']}; raw_sha={r.sha_file(RAW_PACKET_PATH)}; output_shape_pass={quality['pass']}; canonical_released=false; no wiki/DB write; no deletion.",
    ...
    update_manifest("ITEM_02_RAW_LANDED_LOCAL_VERIFICATION_PENDING", conversation_id=identity["conversation_id"], submit_utc=identity["submit_utc"], research_start_utc=state["paper"].get("research_start_utc"), raw_packet_path=str(RAW_PACKET_PATH), raw_packet_sha256=r.sha_file(RAW_PACKET_PATH), output_shape_pass=quality["pass"])
    return metadata
...
def write_failure(state, exc, identity=None):
    ...
    update_manifest("QUEUE_HOLD_ITEM_02", failure_path=str(FAILURE_PATH), error=f"{type(exc).__name__}: {exc}")
    with suppress(Exception):
        r.journal_entry("review_base_02_queue_hold", f"Review Base queue HOLD on item 01: {type(exc).__name__}: {exc}; retry=false; challenge={failure['global_challenge_stop']}", [QUEUE_PATH, MANIFEST_PATH, PROMPT_PATH, FAILURE_PATH])
