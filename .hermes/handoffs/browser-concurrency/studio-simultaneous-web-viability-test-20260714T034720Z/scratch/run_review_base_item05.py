...
PROMPT_PATH = AREA_DIR / "area_review_05_maiolino_mannucci_2019_DR_PROMPT.md"
RAW_PACKET_PATH = AREA_DIR / "area_review_05_maiolino_mannucci_2019_DR_RAW_PACKET.md"
RAW_METADATA_PATH = AREA_DIR / "area_review_05_maiolino_mannucci_2019_DR_PACKET.raw.metadata.json"
CANONICAL_PACKET_PATH = AREA_DIR / "area_review_05_maiolino_mannucci_2019_DR_PACKET.md"
STATE_PATH = AREA_DIR / "area_review_05_maiolino_mannucci_2019_DR_STATE.json"
FAILURE_PATH = AREA_DIR / "area_review_05_maiolino_mannucci_2019_DR_FAILURE.json"
RUN_ID = "REVIEW_BASE_05_MAIOLINO_MANNUCCI_2019"
MARKER = "REVIEW_BASE_05_DR_COMPLETE_REFERENCE_ONLY"
FIRST_NOT_BEFORE = "2026-07-15T15:19:21Z"
SPACING_MINUTES = 35
...
def update_manifest(status, **fields):
    ...
    item = next(row for row in manifest["queue"] if row["id"] == 5)
    item.update(fields)
...
def spec():
    ...
    return {
        ...
        # remains item 05 in queue_item/paper_id and artifact names.
        "paper": 1,
        "paper_id": "review_base_05",
        "shortname": "maiolino_mannucci_2019_review_source_base",
...
def wait_for_safe_gap(state):
    ...
    while True:
        ...
        if broker.get("frozen"):
            raise r.GlobalChallengeStop("broker frozen before Review Base item 05 dispatch")
        ...
        update_manifest("WAITING_ITEM_05_STANDARD_GAP" if remaining > 0 or live_account else "ITEM_05_ACCOUNT_GATE_READY", account_gate_not_before=state["account_gate_not_before"])
...
def prepare_new_chat(browser, state):
    ...
    try:
        ...
        if r.page_challenge(page):
            r.freeze_for_challenge(client, holder, "review_base_05", "pre-dispatch current-route classification")
        r.check_action(client, lease, "review_base_05 navigate to new chat", record["path"], page)
        page.goto("https://gemini.google.com/app", wait_until="domcontentloaded", timeout=30000)
        page.wait_for_timeout(1200)
        if r.page_challenge(page):
            r.freeze_for_challenge(client, holder, "review_base_05", "post-navigation new-chat classification")
        if not r.target_matches("/app", page):
            raise r.TargetDrift("Review Base item 05 new-chat route failed exact verification")
...
def classify_after_start_timeout(identity, prompt, browser):
    ...
    try:
        ...
        if r.page_challenge(page):
            r.freeze_for_challenge(client, holder, "review_base_05", "post-Start timeout settlement")
...
def output_quality(result):
    lower = result.lower()
    counts = {prefix: len(set(re.findall(rf"\[REV05-{prefix}\d{{2}}\]", result))) for prefix in ("E", "D", "N", "U")}
    primary_count = len(set(re.findall(r"\[REV05-P\d{3}\]", result)))
...
def save_packet(snapshot, result_sha, identity, current_spec, state):
    ...
    if r.sha_bytes(raw.encode()) != result_sha:
        raise r.PaperFailure("Review Base item 05 raw-result custody mismatch")
    entry = r.journal_entry(
        "review_base_05_raw_packet_saved",
        f"Review Base 05 Maiolino & Mannucci 2019 raw packet saved; conversation={identity['conversation_id']}; raw_sha={r.sha_file(RAW_PACKET_PATH)}; output_shape_pass={quality['pass']}; canonical_released=false; no wiki/DB write; no deletion.",
    ...
    update_manifest("ITEM_05_RAW_LANDED_LOCAL_VERIFICATION_PENDING", conversation_id=identity["conversation_id"], submit_utc=identity["submit_utc"], research_start_utc=state["paper"].get("research_start_utc"), raw_packet_path=str(RAW_PACKET_PATH), raw_packet_sha256=r.sha_file(RAW_PACKET_PATH), output_shape_pass=quality["pass"])
    return metadata
...
def write_failure(state, exc, identity=None):
    ...
    update_manifest("QUEUE_HOLD_ITEM_05", failure_path=str(FAILURE_PATH), error=f"{type(exc).__name__}: {exc}")
    with suppress(Exception):
        r.journal_entry("review_base_05_queue_hold", f"Review Base queue HOLD on item 05: {type(exc).__name__}: {exc}; retry=false; challenge={failure['global_challenge_stop']}", [QUEUE_PATH, MANIFEST_PATH, PROMPT_PATH, FAILURE_PATH])
    r.report_hwao(f"REVIEW-BASE HARD HOLD item 05: {type(exc).__name__}: {exc}; no retry; queue stopped; challenge={failure['global_challenge_stop']}.")
...
def stage_and_submit_with_route_recovery(current_spec, state, browser):
    try:
        ...
        page = r.exact_page(browser, record["path"])
        if r.page_challenge(page):
            raise r.GlobalChallengeStop("Review Base 05 challenge after prompt submit")
        prompt = PROMPT_PATH.read_text().rstrip("\n")
        evidence = r.current_prompt_identity(page, prompt)
        if not evidence:
            raise r.TargetDrift("Review Base 05 navigation timeout without exact visible prompt identity")
        message, count, latest = q.latest_account_grant()
        if message != f"OK ({count} entries)" or not latest or latest.get("holder") != "goru-dr-reresearch-r2-review_base_05-submit":
            raise r.TargetDrift(f"Review Base 05 submit custody mismatch after navigation timeout: {latest}")
        return {
