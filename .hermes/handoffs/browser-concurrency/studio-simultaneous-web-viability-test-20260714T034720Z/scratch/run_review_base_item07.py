...
PROMPT_PATH = AREA_DIR / "area_review_07_peroux_howk_2020_DR_PROMPT.md"
RAW_PACKET_PATH = AREA_DIR / "area_review_07_peroux_howk_2020_DR_RAW_PACKET.md"
RAW_METADATA_PATH = AREA_DIR / "area_review_07_peroux_howk_2020_DR_PACKET.raw.metadata.json"
CANONICAL_PACKET_PATH = AREA_DIR / "area_review_07_peroux_howk_2020_DR_PACKET.md"
STATE_PATH = AREA_DIR / "area_review_07_peroux_howk_2020_DR_STATE.json"
FAILURE_PATH = AREA_DIR / "area_review_07_peroux_howk_2020_DR_FAILURE.json"
RUN_ID = "REVIEW_BASE_07_PEROUX_HOWK_2020"
MARKER = "REVIEW_BASE_07_DR_COMPLETE_REFERENCE_ONLY"
FIRST_NOT_BEFORE = "2026-07-15T20:18:24Z"
SPACING_MINUTES = 29
...
def update_manifest(status, **fields):
    ...
    item = next(row for row in manifest["queue"] if row["id"] == 7)
    item.update(fields)
...
def spec():
    ...
    return {
        ...
        # remains item 07 in queue_item/paper_id and artifact names.
        "paper": 1,
        "paper_id": "review_base_07",
        "shortname": "peroux_howk_2020_review_source_base",
...
def initial_state(current_spec):
    ...
    return {
        ...
        "queue_item": 7,
        "review": "Péroux & Howk 2020 — The Cosmic Baryon and Metal Cycles",
        "review_identity": {
            "doi": "10.1146/annurev-astro-021820-120014",
            "arxiv": "2011.01935",
            "ads_bibcode": "2020ARA&A..58..363P",
...
def wait_for_safe_gap(state):
    ...
    while True:
        ...
        if broker.get("frozen"):
            raise r.GlobalChallengeStop("broker frozen before Review Base item 07 dispatch")
        ...
        update_manifest("WAITING_ITEM_07_STANDARD_GAP" if remaining > 0 or live_account else "ITEM_07_ACCOUNT_GATE_READY", account_gate_not_before=state["account_gate_not_before"])
...
def prepare_new_chat(browser, state):
    ...
    if record["path"] == "/app":
        ...
        return
    holder = "goru-review-base-07-new-chat"
    ...
    try:
        ...
        if r.page_challenge(page):
            r.freeze_for_challenge(client, holder, "review_base_07", "pre-dispatch current-route classification")
        r.check_action(client, lease, "review_base_07 navigate to new chat", record["path"], page)
        page.goto("https://gemini.google.com/app", wait_until="domcontentloaded", timeout=30000)
        page.wait_for_timeout(1200)
        if r.page_challenge(page):
            r.freeze_for_challenge(client, holder, "review_base_07", "post-navigation new-chat classification")
        if not r.target_matches("/app", page):
            raise r.TargetDrift("Review Base item 07 new-chat route failed exact verification")
...
def classify_after_start_timeout(identity, prompt, browser):
    ...
    try:
        ...
        if r.page_challenge(page):
            r.freeze_for_challenge(client, holder, "review_base_07", "post-Start timeout settlement")
...
def output_quality(result):
    lower = result.lower()
    counts = {prefix: len(set(re.findall(rf"\[REV07-{prefix}\d{{2}}\]", result))) for prefix in ("E", "D", "N", "U")}
    primary_count = len(set(re.findall(r"\[REV07-P\d{3}\]", result)))
...
def save_packet(snapshot, result_sha, identity, current_spec, state):
    ...
    lines = [
        "# Review Base 07 raw Deep Research packet — Péroux & Howk 2020",
        "",
        "advisory_only: true",
        "canonical_packet_released: false",
    ...
    if r.sha_bytes(raw.encode()) != result_sha:
        raise r.PaperFailure("Review Base item 07 raw-result custody mismatch")
    entry = r.journal_entry(
        "review_base_07_raw_packet_saved",
        f"Review Base 07 Péroux & Howk 2020 raw packet saved; conversation={identity['conversation_id']}; raw_sha={r.sha_file(RAW_PACKET_PATH)}; output_shape_pass={quality['pass']}; canonical_released=false; no wiki/DB write; no deletion.",
    ...
    update_manifest("ITEM_07_RAW_LANDED_LOCAL_VERIFICATION_PENDING", conversation_id=identity["conversation_id"], submit_utc=identity["submit_utc"], research_start_utc=state["paper"].get("research_start_utc"), raw_packet_path=str(RAW_PACKET_PATH), raw_packet_sha256=r.sha_file(RAW_PACKET_PATH), output_shape_pass=quality["pass"])
    return metadata
...
def write_failure(state, exc, identity=None):
    ...
    update_manifest("QUEUE_HOLD_ITEM_07", failure_path=str(FAILURE_PATH), error=f"{type(exc).__name__}: {exc}")
    with suppress(Exception):
        r.journal_entry("review_base_07_queue_hold", f"Review Base queue HOLD on item 07: {type(exc).__name__}: {exc}; retry=false; challenge={failure['global_challenge_stop']}", [QUEUE_PATH, MANIFEST_PATH, PROMPT_PATH, FAILURE_PATH])
    r.report_hwao(f"REVIEW-BASE HARD HOLD item 07: {type(exc).__name__}: {exc}; no retry; queue stopped; challenge={failure['global_challenge_stop']}.")
...
def stage_and_submit_with_route_recovery(current_spec, state, browser):
    try:
        ...
        if not record or record["path"] not in {"/app"} and not record["path"].startswith("/app/"):
            raise r.TargetDrift(f"Review Base 07 navigation timeout on unexpected route: {record}") from exc
        page = r.exact_page(browser, record["path"])
        if r.page_challenge(page):
            raise r.GlobalChallengeStop("Review Base 07 challenge after prompt submit")
        prompt = PROMPT_PATH.read_text().rstrip("\n")
        recovered_from_history = False
        if record["path"] == "/app":
            holder = "goru-dr-reresearch-r2-review_base_07-route-recovery"
            client = r.UDSClient(r.SOCK)
            lease = None
            try:
                lease = r.acquire_target(client, holder, "write", ttl=300)
                r.check_action(client, lease, "review_base_07 inspect exact accepted history route without resend", "/app", page)
                ...
                matches = []
                needle = "Queue item 07 of 20 Core review: Péroux"
                for i in range(min(links.count(), 150)):
                    link = links.nth(i)
                    if link.is_visible() and needle in (link.inner_text() or ""):
                        matches.append(link)
                if len(matches) != 1:
                    raise r.TargetDrift(f"Review Base 07 accepted history route count {len(matches)} after submit timeout")
                href = matches[0].get_attribute("href")
                path = urlparse(href).path if href else ""
                if not path.startswith("/app/"):
                    raise r.TargetDrift(f"Review Base 07 invalid accepted history href: {href}")
                page.goto(f"https://gemini.google.com{path}", wait_until="domcontentloaded", timeout=30000)
                page.wait_for_timeout(1500)
                if not r.target_matches(path, page):
                    raise r.TargetDrift(f"Review Base 07 exact accepted history navigation failed: {page.url}")
            ...
            if not record or not record["path"].startswith("/app/"):
                raise r.TargetDrift(f"Review Base 07 route absent after history recovery: {record}")
        evidence = r.current_prompt_identity(page, prompt)
        if not evidence:
            raise r.TargetDrift("Review Base 07 navigation timeout without exact visible prompt identity")
        message, count, latest = q.latest_account_grant()
        if message != f"OK ({count} entries)" or not latest or latest.get("holder") != "goru-dr-reresearch-r2-review_base_07-submit":
            raise r.TargetDrift(f"Review Base 07 submit custody mismatch after navigation timeout: {latest}")
        return {
