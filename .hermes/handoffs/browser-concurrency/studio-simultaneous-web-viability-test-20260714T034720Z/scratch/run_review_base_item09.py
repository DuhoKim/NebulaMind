...
PROMPT_PATH = AREA_DIR / "area_review_09_forster_schreiber_wuyts_2020_DR_PROMPT.md"
RAW_PACKET_PATH = AREA_DIR / "area_review_09_forster_schreiber_wuyts_2020_DR_RAW_PACKET.md"
RAW_METADATA_PATH = AREA_DIR / "area_review_09_forster_schreiber_wuyts_2020_DR_PACKET.raw.metadata.json"
CANONICAL_PACKET_PATH = AREA_DIR / "area_review_09_forster_schreiber_wuyts_2020_DR_PACKET.md"
STATE_PATH = AREA_DIR / "area_review_09_forster_schreiber_wuyts_2020_DR_STATE.json"
FAILURE_PATH = AREA_DIR / "area_review_09_forster_schreiber_wuyts_2020_DR_FAILURE.json"
RUN_ID = "REVIEW_BASE_09_FORSTER_SCHREIBER_WUYTS_2020"
MARKER = "REVIEW_BASE_09_DR_COMPLETE_REFERENCE_ONLY"
FIRST_NOT_BEFORE = "2026-07-16T01:22:47Z"
SPACING_MINUTES = 26
...
def update_manifest(status, **fields):
    ...
    item = next(row for row in manifest["queue"] if row["id"] == 9)
    item.update(fields)
...
def spec():
    ...
    return {
        ...
        # remains item 09 in queue_item/paper_id and artifact names.
        "paper": 1,
        "paper_id": "review_base_09",
        "shortname": "forster_schreiber_wuyts_2020_review_source_base",
...
def initial_state(current_spec):
    ...
    return {
        ...
        "queue_item": 9,
        "review": "Förster Schreiber & Wuyts 2020 — Star-Forming Galaxies at Cosmic Noon",
        "review_identity": {
            "doi": "10.1146/annurev-astro-032620-021910",
            "arxiv": "2010.10171",
            "ads_bibcode": "2020ARA&A..58..661F",
...
def wait_for_safe_gap(state):
    ...
    while True:
        ...
        if broker.get("frozen"):
            raise r.GlobalChallengeStop("broker frozen before Review Base item 09 dispatch")
        ...
        update_manifest("WAITING_ITEM_09_STANDARD_GAP" if remaining > 0 or live_account else "ITEM_09_ACCOUNT_GATE_READY", account_gate_not_before=state["account_gate_not_before"])
...
def prepare_new_chat(browser, state):
    ...
    if record["path"] == "/app":
        ...
        return
    holder = "goru-review-base-09-new-chat"
    ...
    try:
        ...
        if r.page_challenge(page):
            r.freeze_for_challenge(client, holder, "review_base_09", "pre-dispatch current-route classification")
        r.check_action(client, lease, "review_base_09 navigate to new chat", record["path"], page)
        page.goto("https://gemini.google.com/app", wait_until="domcontentloaded", timeout=30000)
        page.wait_for_timeout(1200)
        if r.page_challenge(page):
            r.freeze_for_challenge(client, holder, "review_base_09", "post-navigation new-chat classification")
        if not r.target_matches("/app", page):
            raise r.TargetDrift("Review Base item 09 new-chat route failed exact verification")
...
def classify_after_start_timeout(identity, prompt, browser):
    ...
    try:
        ...
        if r.page_challenge(page):
            r.freeze_for_challenge(client, holder, "review_base_09", "post-Start timeout settlement")
...
def output_quality(result):
    lower = result.lower()
    counts = {prefix: len(set(re.findall(rf"\[REV09-{prefix}\d{{2}}\]", result))) for prefix in ("E", "D", "N", "U")}
    primary_count = len(set(re.findall(r"\[REV09-P\d{3}\]", result)))
...
def save_packet(snapshot, result_sha, identity, current_spec, state):
    ...
    lines = [
        "# Review Base 09 raw Deep Research packet — Förster Schreiber & Wuyts 2020",
        "",
        "advisory_only: true",
        "canonical_packet_released: false",
    ...
    if r.sha_bytes(raw.encode()) != result_sha:
        raise r.PaperFailure("Review Base item 09 raw-result custody mismatch")
    entry = r.journal_entry(
        "review_base_09_raw_packet_saved",
        f"Review Base 09 Förster Schreiber & Wuyts 2020 raw packet saved; conversation={identity['conversation_id']}; raw_sha={r.sha_file(RAW_PACKET_PATH)}; output_shape_pass={quality['pass']}; canonical_released=false; no wiki/DB write; no deletion.",
    ...
    update_manifest("ITEM_09_RAW_LANDED_LOCAL_VERIFICATION_PENDING", conversation_id=identity["conversation_id"], submit_utc=identity["submit_utc"], research_start_utc=state["paper"].get("research_start_utc"), raw_packet_path=str(RAW_PACKET_PATH), raw_packet_sha256=r.sha_file(RAW_PACKET_PATH), output_shape_pass=quality["pass"])
    return metadata
...
def write_failure(state, exc, identity=None):
    ...
    update_manifest("QUEUE_HOLD_ITEM_09", failure_path=str(FAILURE_PATH), error=f"{type(exc).__name__}: {exc}")
    with suppress(Exception):
        r.journal_entry("review_base_09_queue_hold", f"Review Base queue HOLD on item 09: {type(exc).__name__}: {exc}; retry=false; challenge={failure['global_challenge_stop']}", [QUEUE_PATH, MANIFEST_PATH, PROMPT_PATH, FAILURE_PATH])
    r.report_hwao(f"REVIEW-BASE HARD HOLD item 09: {type(exc).__name__}: {exc}; no retry; queue stopped; challenge={failure['global_challenge_stop']}.")
...
def stage_and_submit_with_route_recovery(current_spec, state, browser):
    try:
        ...
        if not record or record["path"] not in {"/app"} and not record["path"].startswith("/app/"):
            raise r.TargetDrift(f"Review Base 09 navigation timeout on unexpected route: {record}") from exc
        page = r.exact_page(browser, record["path"])
        if r.page_challenge(page):
            raise r.GlobalChallengeStop("Review Base 09 challenge after prompt submit")
        prompt = PROMPT_PATH.read_text().rstrip("\n")
        recovered_from_history = False
        if record["path"] == "/app":
            holder = "goru-dr-reresearch-r2-review_base_09-route-recovery"
            client = r.UDSClient(r.SOCK)
            lease = None
            try:
                lease = r.acquire_target(client, holder, "write", ttl=300)
                r.check_action(client, lease, "review_base_09 inspect exact accepted history route without resend", "/app", page)
                ...
                matches = []
                needle = "Queue item 09 of 20 Core review: Förster Schreiber"
                for i in range(min(links.count(), 150)):
                    link = links.nth(i)
                    if link.is_visible() and needle in (link.inner_text() or ""):
                        matches.append(link)
                if len(matches) != 1:
                    raise r.TargetDrift(f"Review Base 09 accepted history route count {len(matches)} after submit timeout")
                href = matches[0].get_attribute("href")
                path = urlparse(href).path if href else ""
                if not path.startswith("/app/"):
                    raise r.TargetDrift(f"Review Base 09 invalid accepted history href: {href}")
                page.goto(f"https://gemini.google.com{path}", wait_until="domcontentloaded", timeout=30000)
                page.wait_for_timeout(1500)
                if not r.target_matches(path, page):
                    raise r.TargetDrift(f"Review Base 09 exact accepted history navigation failed: {page.url}")
            ...
            if not record or not record["path"].startswith("/app/"):
                raise r.TargetDrift(f"Review Base 09 route absent after history recovery: {record}")
        evidence = r.current_prompt_identity(page, prompt)
        if not evidence:
            raise r.TargetDrift("Review Base 09 navigation timeout without exact visible prompt identity")
        message, count, latest = q.latest_account_grant()
        if message != f"OK ({count} entries)" or not latest or latest.get("holder") != "goru-dr-reresearch-r2-review_base_09-submit":
            raise r.TargetDrift(f"Review Base 09 submit custody mismatch after navigation timeout: {latest}")
        return {
