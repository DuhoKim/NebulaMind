...
PROMPT_PATH = AREA_DIR / "area_review_10_conroy_2013_DR_PROMPT.md"
RAW_PACKET_PATH = AREA_DIR / "area_review_10_conroy_2013_DR_RAW_PACKET.md"
RAW_METADATA_PATH = AREA_DIR / "area_review_10_conroy_2013_DR_PACKET.raw.metadata.json"
CANONICAL_PACKET_PATH = AREA_DIR / "area_review_10_conroy_2013_DR_PACKET.md"
STATE_PATH = AREA_DIR / "area_review_10_conroy_2013_DR_STATE.json"
FAILURE_PATH = AREA_DIR / "area_review_10_conroy_2013_DR_FAILURE.json"
RUN_ID = "REVIEW_BASE_10_CONROY_2013"
MARKER = "REVIEW_BASE_10_DR_COMPLETE_REFERENCE_ONLY"
FIRST_NOT_BEFORE = "2026-07-16T02:03:16Z"
SPACING_MINUTES = 33
...
def update_manifest(status, **fields):
    ...
    item = next(row for row in manifest["queue"] if row["id"] == 10)
    item.update(fields)
...
def spec():
    ...
    return {
        ...
        # remains item 10 in queue_item/paper_id and artifact names.
        "paper": 1,
        "paper_id": "review_base_10",
        "shortname": "conroy_2013_review_source_base",
...
def initial_state(current_spec):
    ...
    return {
        ...
        "queue_item": 10,
        "review": "Conroy 2013 — Modeling the Panchromatic Spectral Energy Distributions of Galaxies",
        "review_identity": {
...
def wait_for_safe_gap(state):
    ...
    while True:
        ...
        if broker.get("frozen"):
            raise r.GlobalChallengeStop("broker frozen before Review Base item 10 dispatch")
        ...
        update_manifest("WAITING_ITEM_10_STANDARD_GAP" if remaining > 0 or live_account else "ITEM_10_ACCOUNT_GATE_READY", account_gate_not_before=state["account_gate_not_before"])
...
def prepare_new_chat(browser, state):
    ...
    if record["path"] == "/app":
        ...
        return
    holder = "goru-review-base-10-new-chat"
    ...
    try:
        ...
        if not r.target_matches("/app", page):
            raise r.TargetDrift("Review Base item 10 new-chat route failed exact verification")
...
def save_packet(snapshot, result_sha, identity, current_spec, state):
    ...
    if r.sha_bytes(raw.encode()) != result_sha:
        raise r.PaperFailure("Review Base item 10 raw-result custody mismatch")
    ...
    update_manifest("ITEM_10_RAW_LANDED_LOCAL_VERIFICATION_PENDING", conversation_id=identity["conversation_id"], submit_utc=identity["submit_utc"], research_start_utc=state["paper"].get("research_start_utc"), raw_packet_path=str(RAW_PACKET_PATH), raw_packet_sha256=r.sha_file(RAW_PACKET_PATH), output_shape_pass=quality["pass"])
    return metadata
