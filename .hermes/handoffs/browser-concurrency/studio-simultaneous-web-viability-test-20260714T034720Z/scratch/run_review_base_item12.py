...
PROMPT_PATH = AREA_DIR / "area_review_12_blanton_moustakas_2009_DR_PROMPT.md"
RAW_PACKET_PATH = AREA_DIR / "area_review_12_blanton_moustakas_2009_DR_RAW_PACKET.md"
RAW_METADATA_PATH = AREA_DIR / "area_review_12_blanton_moustakas_2009_DR_PACKET.raw.metadata.json"
CANONICAL_PACKET_PATH = AREA_DIR / "area_review_12_blanton_moustakas_2009_DR_PACKET.md"
STATE_PATH = AREA_DIR / "area_review_12_blanton_moustakas_2009_DR_STATE.json"
FAILURE_PATH = AREA_DIR / "area_review_12_blanton_moustakas_2009_DR_FAILURE.json"
RUN_ID = "REVIEW_BASE_12_BLANTON_MOUSTAKAS_2009"
MARKER = "REVIEW_BASE_12_DR_COMPLETE_REFERENCE_ONLY"
FIRST_NOT_BEFORE = "2026-07-16T03:25:20.280344Z"
SPACING_MINUTES = 36
...
def update_manifest(status, **fields):
    ...
    item = next(row for row in manifest["queue"] if row["id"] == 12)
    item.update(fields)
...
def spec():
    ...
    return {
        ...
        # remains item 12 in queue_item/paper_id and artifact names.
        "paper": 1,
        "paper_id": "review_base_12",
        "shortname": "blanton_moustakas_2009_review_source_base",
...
def initial_state(current_spec):
    ...
    return {
        ...
        "queue_item": 12,
        "review": "Blanton & Moustakas 2009 — Physical Properties and Environments of Nearby Galaxies",
        "review_identity": {
            "doi": "10.1146/annurev-astro-082708-101734",
            "arxiv": "0908.3017",
            "ads_bibcode": "2009ARA&A..47..159B",
...
def wait_for_safe_gap(state):
    ...
    while True:
        ...
        if broker.get("frozen"):
            raise r.GlobalChallengeStop("broker frozen before Review Base item 12 dispatch")
        ...
        update_manifest("WAITING_ITEM_12_STANDARD_GAP" if remaining > 0 or live_account else "ITEM_12_ACCOUNT_GATE_READY", account_gate_not_before=state["account_gate_not_before"])
...
def prepare_new_chat(browser, state):
    ...
    if record["path"] == "/app":
        ...
        return
    holder = "goru-review-base-12-new-chat"
    ...
    try:
        ...
        if not r.target_matches("/app", page):
            raise r.TargetDrift("Review Base item 12 new-chat route failed exact verification")
...
def save_packet(snapshot, result_sha, identity, current_spec, state):
    ...
    if r.sha_bytes(raw.encode()) != result_sha:
        raise r.PaperFailure("Review Base item 12 raw-result custody mismatch")
    ...
    update_manifest("ITEM_12_RAW_LANDED_LOCAL_VERIFICATION_PENDING", conversation_id=identity["conversation_id"], submit_utc=identity["submit_utc"], research_start_utc=state["paper"].get("research_start_utc"), raw_packet_path=str(RAW_PACKET_PATH), raw_packet_sha256=r.sha_file(RAW_PACKET_PATH), output_shape_pass=quality["pass"])
    return metadata
