...
PROMPT_PATH = AREA_DIR / "area_review_18_freeman_bland_hawthorn_2002_DR_PROMPT.md"
RAW_PACKET_PATH = AREA_DIR / "area_review_18_freeman_bland_hawthorn_2002_DR_RAW_PACKET.md"
RAW_METADATA_PATH = AREA_DIR / "area_review_18_freeman_bland_hawthorn_2002_DR_PACKET.raw.metadata.json"
CANONICAL_PACKET_PATH = AREA_DIR / "area_review_18_freeman_bland_hawthorn_2002_DR_PACKET.md"
STATE_PATH = AREA_DIR / "area_review_18_freeman_bland_hawthorn_2002_DR_STATE.json"
FAILURE_PATH = AREA_DIR / "area_review_18_freeman_bland_hawthorn_2002_DR_FAILURE.json"
RUN_ID = "REVIEW_BASE_18_FREEMAN_BLAND_HAWTHORN_2002"
MARKER = "REVIEW_BASE_18_DR_COMPLETE_REFERENCE_ONLY"
FIRST_NOT_BEFORE = "2026-07-16T06:55:56.507441Z"
SPACING_MINUTES = 38
...
def update_manifest(status, **fields):
    ...
    item = next(row for row in manifest["queue"] if row["id"] == 18)
    item.update(fields)
...
def spec():
    ...
    return {
        ...
        # remains item 18 in queue_item/paper_id and artifact names.
        "paper": 1,
        "paper_id": "review_base_18",
        "shortname": "freeman_bland_hawthorn_2002_review_source_base",
...
def initial_state(current_spec):
    ...
    return {
        ...
        "queue_item": 18,
        "review": "Freeman & Bland-Hawthorn 2002 — The New Galaxy: Signatures of Its Formation",
        "review_identity": {
...
def wait_for_safe_gap(state):
    ...
    while True:
        ...
        if broker.get("frozen"):
            raise r.GlobalChallengeStop("broker frozen before Review Base item 18 dispatch")
        ...
        update_manifest("WAITING_ITEM_18_STANDARD_GAP" if remaining > 0 or live_account else "ITEM_18_ACCOUNT_GATE_READY", account_gate_not_before=state["account_gate_not_before"])
...
def prepare_new_chat(browser, state):
    ...
    if record["path"] == "/app":
        ...
        return
    holder = "goru-review-base-18-new-chat"
    ...
    try:
        ...
        if not r.target_matches("/app", page):
            raise r.TargetDrift("Review Base item 18 new-chat route failed exact verification")
...
def output_quality(result):
    lower = result.lower()
    counts = {prefix: len(set(re.findall(rf"\[REV18-{prefix}\d{{2}}\]", result))) for prefix in ("E", "D", "N", "U")}
    primary_count = len(set(re.findall(r"\[REV18-P\d{3}\]", result)))
    ...
    checks = {
        ...
        "review_doi_present": "10.1146/annurev.astro.40.060401.093840" in result,
        "review_arxiv_present": "astro-ph/0208106" in result,
        "review_ads_present": "2002ARA&A..40..487F" in result,
...
def save_packet(snapshot, result_sha, identity, current_spec, state):
    ...
    lines = [
        "# Review Base 18 raw Deep Research packet — Freeman & Bland-Hawthorn 2002",
        "",
        "advisory_only: true",
        "canonical_packet_released: false",
    ...
    if r.sha_bytes(raw.encode()) != result_sha:
        raise r.PaperFailure("Review Base item 18 raw-result custody mismatch")
    entry = r.journal_entry(
        "review_base_18_raw_packet_saved",
        f"Review Base 18 Freeman & Bland-Hawthorn 2002 raw packet saved; conversation={identity['conversation_id']}; raw_sha={r.sha_file(RAW_PACKET_PATH)}; output_shape_pass={quality['pass']}; canonical_released=false; no wiki/DB write; no deletion.",
    ...
    update_manifest("ITEM_18_RAW_LANDED_LOCAL_VERIFICATION_PENDING", conversation_id=identity["conversation_id"], submit_utc=identity["submit_utc"], research_start_utc=state["paper"].get("research_start_utc"), raw_packet_path=str(RAW_PACKET_PATH), raw_packet_sha256=r.sha_file(RAW_PACKET_PATH), output_shape_pass=quality["pass"])
    return metadata
