...
PROMPT_PATH = AREA_DIR / "area_review_20_renzini_2006_DR_PROMPT.md"
RAW_PACKET_PATH = AREA_DIR / "area_review_20_renzini_2006_DR_RAW_PACKET.md"
RAW_METADATA_PATH = AREA_DIR / "area_review_20_renzini_2006_DR_PACKET.raw.metadata.json"
CANONICAL_PACKET_PATH = AREA_DIR / "area_review_20_renzini_2006_DR_PACKET.md"
STATE_PATH = AREA_DIR / "area_review_20_renzini_2006_DR_STATE.json"
FAILURE_PATH = AREA_DIR / "area_review_20_renzini_2006_DR_FAILURE.json"
RUN_ID = "REVIEW_BASE_20_RENZINI_2006"
MARKER = "REVIEW_BASE_20_DR_COMPLETE_REFERENCE_ONLY"
FIRST_NOT_BEFORE = "2026-07-16T08:00:37.712976Z"
SPACING_MINUTES = 25
...
def update_manifest(status, **fields):
    ...
    item = next(row for row in manifest["queue"] if row["id"] == 20)
    item.update(fields)
...
def spec():
    ...
    return {
        ...
        # remains item 20 in queue_item/paper_id and artifact names.
        "paper": 1,
        "paper_id": "review_base_20",
        "shortname": "renzini_2006_review_source_base",
...
def initial_state(current_spec):
    ...
    return {
        ...
        "queue_item": 20,
        "review": "Renzini 2006 — Stellar Population Diagnostics of Elliptical Galaxy Formation",
        "review_identity": {
...
def classify_after_start_timeout(identity, prompt, browser):
    ...
    holder = "goru-review-base-20-start-settlement"
    client = r.UDSClient(r.SOCK)
    lease = None
    try:
        ...
        if r.page_challenge(page):
            r.freeze_for_challenge(client, holder, "review_base_20", "post-Start timeout settlement")
...
def output_quality(result):
    lower = result.lower()
    counts = {prefix: len(set(re.findall(rf"\[REV20-{prefix}\d{{2}}\]", result))) for prefix in ("E", "D", "N", "U")}
    primary_count = len(set(re.findall(r"\[REV20-P\d{3}\]", result)))
    ...
    checks = {
        ...
        "review_doi_present": "10.1146/annurev.astro.44.051905.092450" in result,
        "review_arxiv_present": "astro-ph/0603479" in result,
        "review_ads_present": "2006ARA&A..44..141R" in result,
...
def save_packet(snapshot, result_sha, identity, current_spec, state):
    ...
    if r.sha_bytes(raw.encode()) != result_sha:
        raise r.PaperFailure("Review Base item 20 raw-result custody mismatch")
    entry = r.journal_entry(
        "review_base_20_raw_packet_saved",
        f"Review Base 20 Renzini 2006 raw packet saved; conversation={identity['conversation_id']}; raw_sha={r.sha_file(RAW_PACKET_PATH)}; output_shape_pass={quality['pass']}; canonical_released=false; no wiki/DB write; no deletion.",
    ...
    update_manifest("ITEM_20_RAW_LANDED_LOCAL_VERIFICATION_PENDING", conversation_id=identity["conversation_id"], submit_utc=identity["submit_utc"], research_start_utc=state["paper"].get("research_start_utc"), raw_packet_path=str(RAW_PACKET_PATH), raw_packet_sha256=r.sha_file(RAW_PACKET_PATH), output_shape_pass=quality["pass"])
    return metadata
