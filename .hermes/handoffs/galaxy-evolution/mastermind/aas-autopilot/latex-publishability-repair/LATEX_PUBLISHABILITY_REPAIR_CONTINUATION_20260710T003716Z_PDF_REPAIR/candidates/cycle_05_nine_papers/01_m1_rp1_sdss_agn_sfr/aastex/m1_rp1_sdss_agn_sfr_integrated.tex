...
\caption{BPT line-ratio diagram for the cached SDSS DR17 optical emission-line subset used in the RP-1 analysis. The figure shows the standard classification plane defined by H$\alpha$, H$\beta$, [O~III]$\lambda5007$, and [N~II]$\lambda6584$ and identifies the optical selection boundary separating star-forming, composite, and AGN-like objects.}
\label{fig:bpt}
\end{figure*}
...
\caption{Matched-pair catalog-sSFR offsets for broad BPT optical AGN hosts minus nearest star-forming controls in stellar-mass--redshift space. The figure summarizes the offset distribution used to estimate the median optical AGN-control difference and shows how the inferred separation changes with matching choice and line-selection threshold.}
\label{fig:offsets}
\end{figure*}
