...
BPT-selected optical AGN candidates number 4,440 of 60,000 emission-line galaxies ($0.074 \pm 0.001$). The median specific star-formation rate for this sample is $\log(\mathrm{sSFR}/\mathrm{yr}^{-1}) = -11.53$, compared to $-10.14$ for the full denominator. This optical sample defines a follow-up denominator for resolved escape/recycling work, but SDSS alone cannot measure outflow velocity or fate. Figure~\ref{fig:topic} shows the target-selection baseline.
