...
You are an external read-only reviewer. Do not write files. Do not run shell commands. Do not use network/web. Do not access secrets. Review only the local manuscript/context files named below and return a concise but specific critique.

## Scope
Critique these three active Galaxy Evolution AAS pilot revision drafts, which have not yet received an external CLI critique in this lane:
