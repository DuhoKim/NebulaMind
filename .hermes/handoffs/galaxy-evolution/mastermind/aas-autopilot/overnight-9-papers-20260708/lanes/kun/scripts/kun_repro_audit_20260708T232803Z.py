...
GORU_BPT_MC_SUMMARY = OVERNIGHT / "lanes/goru/artifacts/goru_bpt_flux_error_mc_20260708T232006Z.json"
GORU_BPT_MC_INVENTORY = OVERNIGHT / "lanes/goru/tables/goru_bpt_flux_error_mc_inventory_20260708T232006Z.csv"
        ...
        if inv.exists():
            with inv.open(newline="", encoding="utf-8", errors="replace") as handle:
                ...
                for row in reader:
                    ...
                    if p is not None:
                        sha_val = row.get("sha256") or None
                        if isinstance(sha_val, str) and sha_val.upper().startswith("SELF_"):
                            sha_val = None
                        add_expected(expected, p, source=inv.name, sha_value=sha_val, bytes_value=row.get("bytes") or None, kind="inventory_row")
                        added += 1
...
def goru_bpt_mc_checks() -> Dict[str, Any]:
    data = safe_json(GORU_BPT_MC_SUMMARY)
    key_results = data.get("key_results", {}) if isinstance(data, dict) and json_ok(data) else {}
    row_counts = data.get("row_counts", {}) if isinstance(data, dict) and json_ok(data) else {}
    inv_rows: List[Dict[str, Any]] = []
    inv_mismatches: List[Dict[str, Any]] = []
    if GORU_BPT_MC_INVENTORY.exists():
        with GORU_BPT_MC_INVENTORY.open(newline="", encoding="utf-8", errors="replace") as handle:
            for row in csv.DictReader(handle):
                p = normalize_path(row.get("path"), base=GORU_BPT_MC_INVENTORY.parent)
                try:
                    expected_bytes: Optional[int] = int(row.get("bytes") or 0)
                except Exception:
                    expected_bytes = None
                expected_sha = row.get("sha256") or None
                if isinstance(expected_sha, str) and expected_sha.upper().startswith("SELF_"):
                    expected_sha = None
                exists = bool(p and p.exists())
                actual_bytes = p.stat().st_size if p and p.exists() else None
                actual_sha = sha256(p) if p and p.exists() else None
                rec = {
                    "artifact_name": row.get("artifact_name"),
                    "path": str(p) if p else row.get("path"),
                    "relative_path": rel(p) if p else row.get("path"),
                    "exists": exists,
                    "expected_bytes": expected_bytes,
                    "actual_bytes": actual_bytes,
                    "expected_sha256": expected_sha,
                    "actual_sha256": actual_sha,
                    "matches_bytes": actual_bytes == expected_bytes if expected_bytes is not None else None,
                    "matches_sha256": actual_sha == expected_sha if expected_sha else None,
                }
                inv_rows.append(rec)
                if not exists or rec["matches_bytes"] is False or rec["matches_sha256"] is False:
                    inv_mismatches.append(rec)
    rp1_84 = key_results.get("rp1_matched_pair_stability_threshold_0p84", {}) if isinstance(key_results, dict) else {}
    rp1_95 = key_results.get("rp1_matched_pair_stability_threshold_0p95", {}) if isinstance(key_results, dict) else {}
    return {
        "summary_json_path": str(GORU_BPT_MC_SUMMARY),
        "inventory_csv_path": str(GORU_BPT_MC_INVENTORY),
        "summary_json_ok": json_ok(data),
        "inventory_exists": GORU_BPT_MC_INVENTORY.exists(),
        "inventory_row_count": len(inv_rows),
        "inventory_mismatch_count": len(inv_mismatches),
        "inventory_mismatches": inv_mismatches,
        "source_rows": key_results.get("source_rows") if isinstance(key_results, dict) else None,
        "recomputed_bpt_mismatches_vs_cached_label": key_results.get("recomputed_bpt_mismatches_vs_cached_label") if isinstance(key_results, dict) else None,
        "monte_carlo_draws_per_row": key_results.get("monte_carlo_draws_per_row") if isinstance(key_results, dict) else None,
        "deterministic_bpt_counts": key_results.get("deterministic_bpt_counts") if isinstance(key_results, dict) else None,
        "stable_original_label_rows_p_ge_0p84": key_results.get("stable_original_label_rows_p_ge_0p84") if isinstance(key_results, dict) else None,
        "rp1_pairs_retained_p084": rp1_84.get("retained_pairs") if isinstance(rp1_84, dict) else None,
        "rp1_median_delta_p084": rp1_84.get("median_delta_log_sSFR_agn_minus_control") if isinstance(rp1_84, dict) else None,
        "rp1_pairs_retained_p095": rp1_95.get("retained_pairs") if isinstance(rp1_95, dict) else None,
        "rp1_median_delta_p095": rp1_95.get("median_delta_log_sSFR_agn_minus_control") if isinstance(rp1_95, dict) else None,
...
def render_markdown(out: Dict[str, Any]) -> str:
    ...
    lines.append(f"- Goru BPT flux-error MC packet: summary_ok={gb['summary_json_ok']}, inventory rows={gb['inventory_row_count']}, inventory mismatches={gb['inventory_mismatch_count']}, source rows={gb['source_rows']}, recomputed cached-label mismatches={gb['recomputed_bpt_mismatches_vs_cached_label']}, retained RP-1 pairs p>=0.84={gb['rp1_pairs_retained_p084']} median delta={gb['rp1_median_delta_p084']}.")
    ...
    lines.append(f"- Latest Goru MC row-count packet: transition={gb['row_counts'].get('transition_rows')}, stability={gb['row_counts'].get('stability_rows')}, matched-pair sensitivity={gb['row_counts'].get('matched_pair_sensitivity_rows')}, paper metrics={gb['row_counts'].get('paper_metrics_rows')}, S/N summary={gb['row_counts'].get('sn_summary_rows')}.")
    lines.append("")
    ...
    if c["expected_artifact_mismatch_count"]:
        ...
        for mismatch in out["expected_artifacts"].get("mismatches", [])[:5]:
            lines.append(f"  - Non-blocking expected-artifact mismatch: `{mismatch.get('relative_path')}` sources={mismatch.get('sources')} expected_bytes={mismatch.get('expected_bytes')} actual_bytes={mismatch.get('actual_bytes')} expected_sha={mismatch.get('expected_sha256')} actual_sha={mismatch.get('actual_sha256')}")
...
def main() -> None:
    ...
    if not goru_mc.get("summary_json_ok"):
        blockers.append("Latest Goru BPT flux-error MC summary JSON failed parse or is missing")
    if goru_mc.get("inventory_mismatch_count"):
        blockers.append(f"Latest Goru BPT flux-error MC inventory has {goru_mc.get('inventory_mismatch_count')} missing/hash/byte mismatch row(s)")
    if goru_mc.get("source_rows") != 60000:
        blockers.append(f"Latest Goru BPT MC source rows={goru_mc.get('source_rows')}, expected 60000")
    if goru_mc.get("recomputed_bpt_mismatches_vs_cached_label") != 0:
        blockers.append(f"Latest Goru BPT MC recomputed label mismatches={goru_mc.get('recomputed_bpt_mismatches_vs_cached_label')}, expected 0")
    if goru_mc.get("rp1_pairs_retained_p084") != 5368:
        blockers.append(f"Latest Goru BPT MC p>=0.84 retained pairs={goru_mc.get('rp1_pairs_retained_p084')}, expected 5368")
    ...
    summary_counts = {
        ...
        "goru_bpt_mc_inventory_rows": goru_mc.get("inventory_row_count"),
        "goru_bpt_mc_inventory_mismatches": goru_mc.get("inventory_mismatch_count"),
    ...
    out: Dict[str, Any] = {
        ...
        "goru_bpt_flux_error_mc": goru_mc,
