...
\section{Matched-control result: optical AGN and catalog sSFR}\label{sec:rp1-result}
...
\caption{Matched-pair catalog-sSFR offsets for broad BPT optical AGN hosts minus nearest star-forming controls in stellar-mass--redshift space. The large negative offset, $\Delta\log(\mathrm{sSFR}/\mathrm{yr}^{-1})=-1.309$ dex, is robust within the optical emission-line subset but remains selection- and subclass-dependent.}
\label{fig:offsets}
\end{figure*}
