...
\tablehead{\colhead{$\log(M_\star/M_\odot)$ bin} & \colhead{$z$ bin} & \colhead{$N$} & \colhead{Low-sSFR fraction} & \colhead{Broad optical BPT fraction}}
\startdata
...
\enddata
\tablecomments{Fractions are rounded to three decimals for compact display; the retained JSON stores the full precision. This table is an observed optical target vector, not a simulation comparison.}
\end{deluxetable*}
