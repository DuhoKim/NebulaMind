...
\tablehead{\colhead{Atlas entry} & \colhead{Run ID} & \colhead{Artifact key}}
\startdata
Relative neighbor-count baseline & \shortstack[l]{\texttt{SDSS\_REMAINING\_TOPIC\_PILOTS}\\\texttt{20260708T125828Z}} & \texttt{m1\_rp2} \\
Maintenance-heating denominator & \shortstack[l]{\texttt{SDSS\_REMAINING\_TOPIC\_PILOTS}\\\texttt{20260708T125828Z}} & \texttt{m1\_rp3} \\
Resolved-kinematics follow-up denominator & \shortstack[l]{\texttt{SDSS\_REMAINING\_TOPIC\_PILOTS}\\\texttt{20260708T125828Z}} & \texttt{m2\_p1} \\
Radio-jet environment baseline & \shortstack[l]{\texttt{SDSS\_REMAINING\_TOPIC\_PILOTS}\\\texttt{20260708T125828Z}} & \texttt{m2\_p2} \\
Stellar-mass selection diagnostic & \shortstack[l]{\texttt{SDSS\_REMAINING\_TOPIC\_PILOTS}\\\texttt{20260708T125828Z}} & \texttt{m2\_p3} \\
Tracer-threshold census & \shortstack[l]{\texttt{SDSS\_REMAINING\_TOPIC\_PILOTS}\\\texttt{20260708T125828Z}} & \texttt{m3\_p1} \\
Low-sSFR optical denominator & \shortstack[l]{\texttt{SDSS\_REMAINING\_TOPIC\_PILOTS}\\\texttt{20260708T125828Z}} & \texttt{m3\_p2} \\
Simulation target vector & \shortstack[l]{\texttt{SDSS\_REMAINING\_TOPIC\_PILOTS}\\\texttt{20260708T125828Z}} & \texttt{m3\_p3} \\
\enddata
\tablecomments{All eight artifacts are under the tracked run recorded in the custody file and use the shared 60,000-row SDSS analysis sample. The table is an index only; the exact artifact paths and SHA-256 hashes are enumerated in \texttt{provenance/REAL\_DATA\_SOURCE\_CUSTODY.json}. The flagship matched-control result is separately traced there as well, together with the approximate row counts for the tracked inputs.}
\end{deluxetable*}
...
The rounded cell extrema in Table~\ref{tab:simulation-vector} span 0.001 to 0.856 for low-sSFR fraction and 0.001 to 0.610 for broad optical BPT-selected fraction. The follow-up ingredients are simulations \citep{eagle2015} passed through the exact optical S/N and fiber-aperture selection function used here, including the sequential 60,000-row \texttt{specObjID} cache cap, then through the SDSS, MaNGA, ALMA, X-ray, and radio selection functions, together with aperture models and noise models. Without those matched selection steps, any simulation comparison is not a valid test. This entry remains an optical baseline only; the missing observables listed in Table~\ref{tab:atlas-followup} are required before any physical inference.
