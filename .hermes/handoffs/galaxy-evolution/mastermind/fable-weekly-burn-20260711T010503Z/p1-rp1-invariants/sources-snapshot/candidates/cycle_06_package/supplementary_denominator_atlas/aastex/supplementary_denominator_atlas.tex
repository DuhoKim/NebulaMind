...
\tablehead{\colhead{Topic} & \colhead{Observed baseline} & \colhead{Artifact} & \colhead{Section}}
\startdata
Environment & low-sSFR vs.\ 10th-neighbor rank (60,000 total; 15,000 per quartile) & \texttt{m1\_rp2} & Sec.~\ref{sec:neighbor-count} \\
Maintenance heating & broad optical BPT-selected hosts in massive low-sSFR galaxies (9,298 massive; 5,695 low-sSFR) & \texttt{m1\_rp3} & Sec.~\ref{sec:maintenance-heating} \\
Outflow kinematics & high-excitation broad optical BPT-selected subset (4,440/60,000) & \texttt{m2\_p1} & Sec.~\ref{sec:outflow-kinematics} \\
Env.\ jets & neighbor-rank-stratified broad optical BPT-selected fraction in massive hosts & \texttt{m2\_p2} & Sec.~\ref{sec:radio-jet-env} \\
Mass bin & low-sSFR and broad optical BPT-selected incidence by $M_\star$ bin (15 cells with $n\geq50$) & \texttt{m2\_p3} & Sec.~\ref{sec:mass-bin} \\
Tracer census & tracer prevalence in 60k sample (0.136 to 0.418) & \texttt{m3\_p1} & Sec.~\ref{sec:tracer-census} \\
Gas depletion & gas-depletion low-sSFR baseline; H$\alpha$ proxy (6,729 galaxies) & \texttt{m3\_p2} & Sec.~\ref{sec:gas-depletion} \\
Simulation vector & mass-redshift target vector (15 cells with $n\geq50$) & \texttt{m3\_p3} & Sec.~\ref{sec:simulation-vector} \\
\enddata
...
\subsection{Relative neighbor-count baseline: SDSS 10th-neighbor index for low-sSFR incidence}\label{sec:neighbor-count}
...
\subsection{Maintenance-heating denominator: broad optical BPT-selected hosts in massive SDSS galaxies}\label{sec:maintenance-heating}
...
\subsection{High-excitation broad optical BPT-selected baseline: resolved kinematics follow-up}\label{sec:outflow-kinematics}
...
\subsection{Radio-jet environment baseline: broad optical BPT-selected fraction vs. 10th-neighbor index in massive hosts}\label{sec:radio-jet-env}
...
\subsection{Stellar-mass selection diagnostic: low-sSFR and broad optical BPT-selected incidence}\label{sec:mass-bin}
...
\subsection{Tracer-threshold census for multiphase follow-up}\label{sec:tracer-census}
