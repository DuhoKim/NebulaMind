# Hard-burn brief H12 — Deep synthesis: maintenance heating + radio-jet environment (m1_rp3 + m2_p2, fully offline)
...
## Ownership
- Your ONLY write area: `<root>/h12-heating-radio-jet-synthesis/` — create it. Never write anywhere else. Do not modify T0.md, `briefs/`, other `h*` subdirs, the prior burn root, or any repo/runner/live file. Do not create STOP/HOLD files.
...
## ACK
First action: write `<own>/H12_ACK.md` containing exactly the line `FABLE_HARD_BURN_H12_ACK_20260711T035354Z` plus your UTC start timestamp.
...
- Topic artifact A: `<root>/h5-supplement-value-verification/sources-snapshot/m1_rp3_maintenance_heating/analysis_results.json` — `06291f82c3fbe0f7fe84f7249568882ca4fa44972bcc25a55e367ef1fdcc7e6e`
- Topic artifact B: `<root>/h5-supplement-value-verification/sources-snapshot/m2_p2_radio_jet_environment/analysis_results.json` — `4e1ff701bb5b98af4945d5adad2e543e00005e1ab3907e8fae7d15e70c93e351`
...
1. Artifact anatomy for BOTH artifacts: parse every field; restate each measurement design in plain language (sample, denominators, statistic, uncertainties); enumerate every numeric with units and role.
...
3. JOINT synthesis — the lane's center of mass: do the two artifacts support a single radio-mode maintenance story? Explicit cross-checks: shared or overlapping samples/denominators; quantities one artifact implies about the other; whether prose links the topics and whether that link is number-backed. Produce a consistency table (agrees / independent / in tension, with values and line numbers) and a tension list.
4. Confounders per topic (e.g., radio-AGN selection, environment metric, stacking assumptions) — each "addressable with current artifact (how)" or "requires new run (GATED)".
5. Falsifiable predictions + next analyses, dependency-ordered; runner/network/DB actions GATED.

## Deliverables (all in `<own>/`)
1. Headline: `MAINTENANCE_HEATING_RADIO_JET_SYNTHESIS.md` — top line marker `FABLE_HARD_BURN_H12_SYNTHESIS_20260711T035354Z`. Substantial: both claim inventories + joint consistency table. Helper scripts in `<own>/tools/`.
2. `H12_RECEIPT.md` — custody table (pinned vs recomputed sha256 for every input used), every produced file with bytes+sha256, poll log, safety attestation (no writes outside own subdir, snapshots and originals untouched, no banned action), status COMPLETE or PARTIAL, ending with the exact line `FABLE_HARD_BURN_H12_DONE_20260711T035354Z`.
3. 0-byte done marker `<own>/FABLE_HARD_BURN_H12_DONE_20260711T035354Z` — write it when you finish or stop for any reason.
