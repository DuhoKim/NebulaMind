# Hard-burn brief H14 — Deep synthesis: multiphase census + gas depletion efficiency (m3_p1 + m3_p2, fully offline)
...
## Ownership
- Your ONLY write area: `<root>/h14-census-depletion-synthesis/` — create it. Never write anywhere else. Do not modify T0.md, `briefs/`, other `h*` subdirs, the prior burn root, or any repo/runner/live file. Do not create STOP/HOLD files.
...
## ACK
First action: write `<own>/H14_ACK.md` containing exactly the line `FABLE_HARD_BURN_H14_ACK_20260711T035354Z` plus your UTC start timestamp.
...
- Topic artifact A: `<root>/h5-supplement-value-verification/sources-snapshot/m3_p1_multiphase_census/analysis_results.json` — `e711563011102657b6d5cab279c1b2ab7ed087dfc734e50932ad4edfe90d0683`
- Topic artifact B: `<root>/h5-supplement-value-verification/sources-snapshot/m3_p2_gas_depletion_efficiency/analysis_results.json` — `42965b6f359c23b56098f5f9845561f4a4a2ba81e1e00df09fbae4acf3bcc2d9`
...
1. Artifact anatomy for BOTH artifacts: parse every field; restate each measurement design in plain language (sample, denominators, statistic, uncertainties); enumerate every numeric with units and role. Explicit unit audit (phase fractions sum sensibly; efficiencies/timescales dimensionally coherent).
...
3. JOINT synthesis — the lane's center of mass: are the census phase fractions and the depletion/efficiency values mutually consistent when combined? Cross-checks: census totals vs depletion denominators; implied depletion timescales from combining the two (show arithmetic, offline only); whether prose combines them anywhere and whether that combination is number-backed. Consistency table (agrees / independent / in tension) + tension list.
4. Confounders per topic (phase tracer completeness, aperture, conversion factors e.g. alpha_CO-like assumptions as stated in the artifacts, sample overlap) — each "addressable with current artifact (how)" or "requires new run (GATED)".
5. Falsifiable predictions + next analyses, dependency-ordered; runner/network/DB actions GATED.

## Deliverables (all in `<own>/`)
1. Headline: `MULTIPHASE_CENSUS_DEPLETION_SYNTHESIS.md` — top line marker `FABLE_HARD_BURN_H14_SYNTHESIS_20260711T035354Z`. Substantial: both claim inventories + joint census↔depletion consistency analysis. Helper scripts in `<own>/tools/`.
2. `H14_RECEIPT.md` — custody table (pinned vs recomputed sha256 for every input used), every produced file with bytes+sha256, poll log, safety attestation (no writes outside own subdir, snapshots and originals untouched, no banned action), status COMPLETE or PARTIAL, ending with the exact line `FABLE_HARD_BURN_H14_DONE_20260711T035354Z`.
3. 0-byte done marker `<own>/FABLE_HARD_BURN_H14_DONE_20260711T035354Z` — write it when you finish or stop for any reason.
