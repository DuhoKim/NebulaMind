...
Rev1 invalidation acknowledged: rev1 write-scope missed packet-root `_tmp_run_all/`, reported by Tori as 8.6 MB. Rev1 receipts are preserved byte-identically:
...
Rev2 correction performed:
- patched only `tests/run_all.sh` so `TMPDIR` is `receipts/_tmp_run_all`
- added `trap 'rm -rf "$TMPDIR"' EXIT`
- removed only the generated packet-root `_tmp_run_all/`
- reran the full harness
- verified no `_tmp_run_all`, receipt temp, bytecode cache file, or other temp output remains outside `receipts/`

## Commands and Exit Codes

- `bash -n tests/run_all.sh` -> exit 0
- `rm -rf _tmp_run_all` -> exit 0
- `tests/run_all.sh` -> exit 0 after rev2 receipt-scoped temp/trap repair
...
Rev2 final audit found no packet-root `_tmp_run_all/`, no lingering `receipts/_tmp_run_all/`, and no temp or bytecode output outside `receipts/`.
