...
CAPTURE_RUNNER = PACKET / "capture" / "run_capture_v2.mjs"
sys.path.insert(0, str(PACKET / "validator"))
from validator_v2 import validate_document  # noqa: E402
...
def test_t14_exact_mechanical_residue_and_t15_determinism(tmp_path: Path) -> None:
    ...
    assert capture["schema"] == "NM_GEMINI_RENDERED_DOM_V2"
    assert capture["chip_map_status"] == "OK"
    assert capture["capture_flags"] == []
    assert first_path.read_bytes() == second_path.read_bytes()
    assert capture == second_capture

    spec = json.loads((PACKET / "validator" / "contract_spec_v2.json").read_text())
    ...
    c4 = by_clause_code[("C4", "UNCITED_CELL_CLAIM")]
    ...
    assert c7["near_duplicate_indices"] == [[14, 29]]
