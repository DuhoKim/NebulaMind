# C1 — Canary: simulation calibration provenance and out-of-sample validation ledger

Run ID: `JOINT_C1` · REQ: `REQ_JOINT_C1_20260711T100139Z` · Sole run of packet
`gemini-web-joint-burn-recovery-20260711T100139Z` (launch only after `JOINT_ARMED_*` per DIRECTION §3).
Provenance and non-overlap: none of the 15 valid rampage reports covers calibration provenance —
base R6/R6b covered forward-model pipelines/selection effects; the provenance topic was authored as
extension R15 but NEVER launched (extension closed at verification hard stop). This is a fresh
authoring with a new REQ ID and marker, not a retry of any VOID conversation.

Tori: submit ONLY the text BETWEEN the sentinel lines (sentinels and code fence excluded), into the
canary's own new conversation tab per DIRECTION §6. No steering; single neutral "continue" ONLY on
visible mid-generation truncation (packet-authorized, logged); otherwise no follow-ups of any kind.

-----BEGIN PASTE REQ_JOINT_C1_20260711T100139Z-----
```markdown
...
```
-----END PASTE REQ_JOINT_C1_20260711T100139Z-----
