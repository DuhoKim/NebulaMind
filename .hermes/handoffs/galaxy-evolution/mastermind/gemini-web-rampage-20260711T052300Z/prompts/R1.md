# R1 — r2 six-card M3 contract run (existing H2 contract, reused verbatim)

Run ID: `RAMPAGE_R1` · REQ: `REQ_M3_RT_20260711T091128Z` (r2) · Wave 1
Provenance: paste text below is VERBATIM the H2 r2 candidate (hard-burn
`fable-weekly-hard-burn-20260711T035354Z/h2-gemini-req-contract/GEMINI_SIDECAR_REQ_CONTRACT_PACKET.md`
§A minus the OPERATOR-SIDE adjudication-reference block, per H2 step R2), as already embedded in
`pilot-gemini-resume-20260711T050514Z/WEB_OPERATOR_PACKET.md` §5. Adjudication rules stay in H2 §B
(operator side, never pasted).

**Operator spot-verification before pasting (any mismatch ⇒ RUN_VOID, do not submit):**
the `**Revision:** r2` line, the six-card mapping table, contract items C1–C8, and the C8 marker
string `GEMINI_WEB_M3_RT_OUTPUT_DONE_REQ_M3_RT_20260711T091128Z` must all be present and identical
to H2 §A. Record in `meta.md`: `paste_source: rampage prompts/R1.md (verbatim from H2 §A minus OPERATOR-SIDE block)`.

**Completion marker for this run (inside Gemini's body): `GEMINI_WEB_M3_RT_OUTPUT_DONE_REQ_M3_RT_20260711T091128Z`** (C8 below).
