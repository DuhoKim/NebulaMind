...
The same galaxy can look different depending on which phase is being measured. Ionized gas, molecular gas, neutral gas, hot halo gas, and stars do not respond to feedback in the same way or on the same timescale. Cold star-forming gas is often near $10^2$ to $10^4$ K, while the circumgalactic medium and group atmospheres can reach $10^5$ to $10^7$ K. That is why a single label such as "quenched" can hide important differences between temporary suppression, long-lived shutdown, and rapid gas removal. Structural properties such as size, bulge fraction, bar strength, and merger disturbance add another layer because they can either trigger central inflow or reflect earlier dynamical change.
...
**Primary observables.** Median $\Delta\log\mathrm{sSFR}$, the matched-pair distribution width, the fraction of pairs that remain offset after reweighting, and the stability of the result across AGN tracers (BPT, WISE, radio, X-ray) and control definitions.
...
**Decision criterion.** The result favors permanent removal if the median escaping fraction is above $50\%$ in the matched sample and the same gas does not reappear in later CGM measurements. The result favors recycling-limited regulation if most of the gas remains below escape speed or returns in a later halo measurement. Mixed behavior should be treated as phase-dependent regulation, not as a binary outcome.
...
**Decision criterion.** A robust environmental dependence is present if coupling efficiency shifts systematically with density or group membership after correcting for radio-power calibration, jet age, morphology, and aperture choice, and if the mechanical coupling efficiency parameter $\eta$ differs by more than $3\sigma$ between dense environments (clusters/groups) and matched field controls. If the response is flat once controls are applied, environment is a secondary effect.
