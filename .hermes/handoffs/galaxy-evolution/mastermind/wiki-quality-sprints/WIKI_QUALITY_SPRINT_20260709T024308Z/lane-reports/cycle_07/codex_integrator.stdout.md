...
Galaxy evolution is controlled by a small set of physical reservoirs and observables. Halo mass and assembly history set the gravitational environment. Typical galaxy halos span roughly $10^{11}$ to $10^{13}\,M_\odot$ for ordinary systems, while group and cluster halos extend to $\sim10^{14}$ to $10^{15}\,M_\odot$. Stellar mass and star formation rate measure how much baryonic material has already been converted into stars and how fast the current conversion is proceeding. Gas fractions, metallicity, dust, and molecular and atomic gas tracers show how much fuel remains and how processed that fuel has become.
...
Consequently, AGN feedback is best treated as a localized mechanism that can regulate some hosts and some phases, rather than as a guaranteed universal driver of host-galaxy quenching.
...
Not every outflow empties a galaxy. Some gas can leave the star-forming disk but later return, and some feedback regulates turbulence without depleting the reservoir. In specific cases, such as M51, observations show localized compressive or positive feedback rather than host-wide quenching. <!--cite:28060-->
...
Chemical enrichment provides a long-timescale record of these processes. Metallicity scaling relations encode how much gas has been processed into stars, how much enriched material has been lost, and how much low-metallicity gas has diluted the interstellar medium. At high redshift, the same observables become harder to interpret because dust, AGN light, stellar-population assumptions, and spectroscopic completeness can all bias inferred masses and star-formation histories.
...
**Hypothesis / objective.** Optical AGN hosts selected via the Baldwin-Phillips-Terlevich (BPT) line-ratio diagram, using $[\mathrm{O\,III}]/\mathrm{H}\beta$ versus $[\mathrm{N\,II}]/\mathrm{H}\alpha$, have lower catalog sSFR than matched star-forming controls. The offset may still be partly driven by selection bias, aperture limits, dust, or denominator construction rather than by physical feedback.

**Primary observables.** Median $\Delta\log\mathrm{sSFR}$, the matched-pair residual distribution, the fraction of pairs that remain offset after reweighting, and the stability of the result across AGN tracers (BPT, WISE, radio, X-ray) and control definitions. Where available, compare these with H$\alpha$ equivalent width and the 4000 Å break index ($\mathrm{D}_n(4000)$), which help separate current star formation from older stellar populations so the offset is not judged from a single SFR estimator.
...
**Decision criterion.** Permanent removal is favored if the median inferred escaping fraction is $> 50\%$ across the matched active sample and the CGM at large impact parameters ($R \gt 100\ \text{kpc}$) shows elevated metal column densities without comparable returning or infalling components. Recycling-limited regulation is favored if $> 70\%$ of the outflowing mass has $v_{\mathrm{out}} \lt v_{\mathrm{esc}}$ and background sightlines reveal returning or infalling gas kinematics that are consistent with the host rotation.
...
**Decision criterion.** A robust environmental dependence is present if the coupling efficiency parameter $\eta$ is significantly higher in group or cluster cores compared to matched field environments by a factor of $\ge 2$, with a statistical significance of $\ge 3\sigma$ after controlling for jet morphology, radio-power calibration, and aperture effects. If the difference is $\lt 1\sigma$ or non-systematic, the null hypothesis that coupling is independent of environment is accepted.
