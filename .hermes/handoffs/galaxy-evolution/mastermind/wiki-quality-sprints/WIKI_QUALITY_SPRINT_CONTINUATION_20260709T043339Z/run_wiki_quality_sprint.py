...
MAX_CYCLES = int(os.environ.get("WIKI_SPRINT_MAX_CYCLES", "64"))
...
SOURCE_FILES = {
    ...
    "galaxy_wiki_current_draft": REPO / ".hermes" / "handoffs" / "galaxy-evolution" / "mastermind" / "wiki-quality-sprints" / "WIKI_QUALITY_SPRINT_20260709T024308Z" / "candidates" / "cycle_08" / "galaxy-evolution-wiki-candidate.md",
    "research_topics_current": REPO / ".hermes" / "handoffs" / "galaxy-evolution" / "mastermind" / "wiki-quality-sprints" / "WIKI_QUALITY_SPRINT_20260709T024308Z" / "candidates" / "cycle_08" / "research-topics-candidate.md",
...
def contract_sanitize(text: str) -> str:
    """Remove known wiki-contract tokens from local candidates only."""
    replacements = {
        "\\sim": "roughly ",
        ...
        "&lt;": "<",
        "&amp;": "&",
        "universal quenching": "one-size-fits-all quenching",
        "Universal quenching": "One-size-fits-all quenching",
    }
...
""".strip()
    ...
    return contract_sanitize(wiki_md), contract_sanitize(topics_md)
...
def run_cycle(cycle: int, previous_wiki: Path, previous_topics: Path) -> Path:
    ...
    if wiki_block and len(wiki_block) > 2000:
        write_text(current_wiki, contract_sanitize(wiki_block) + "\n")
    if topics_block and len(topics_block) > 1000:
        write_text(current_topics, contract_sanitize(topics_block) + "\n")
...
def main() -> int:
    start = utcnow()
    end = dt.datetime.fromisoformat(TARGET_END_UTC.replace("Z", "+00:00"))
    if end <= start + dt.timedelta(minutes=10):
