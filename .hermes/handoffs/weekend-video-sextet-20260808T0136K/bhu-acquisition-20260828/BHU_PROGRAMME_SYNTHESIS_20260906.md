...
| 3 | **Does the entropy chain of entry 21 derive its temperature, or import it?** | 4 — "this object is really a black hole" rests on it | 4 — eight printed equations (Roupas 2022 eqs. 17–26, SOURCE lines 161–240 of `2203.13295_clean.txt`), kit executed (`r3f_controls.py`, 10/10) | If T0 is derived, the corpus has its first construction-fixed thermodynamic magnitude; if imported, instance seven of the pattern | R3F (`R3F_ROUPAS_ENTROPY_PREREG_DRAFT_20260905.md`, DRAFT 3) |
| 4 | **Is Smoller–Temple's present shock position bounded once the start epoch's printed floor is used?** | 3 | 5 — the PDF settled the formula tonight (`smoller_temple_2000_shockwave_astroph9812063.pdf` p.58, eq. 7.31 with ln(1/R*), eq. 7.29 floor; recorded in `R3G_SMOLLER_TEMPLE_SHOCK_MAGNITUDE_PREREG_DRAFT_20260905.md` §0); C2 of `r3g_controls.py` reproduces 0.019 | A bounded magnitude on the exact-GR branch would be the pattern's first partial breaker (condition 1 met; 2–5 to test) | R3G (`R3G_SMOLLER_TEMPLE_SHOCK_MAGNITUDE_PREREG_DRAFT_20260905.md`, DRAFT 3a) |
...
the only claims that have died (1, 7, 44) are the ones where a number was fixed — by the authors themselves for 1 and 44
(`W_EXPLICIT`), and by a borrowed nuclear-physics bar for 7 (`W_BORROWED`, `WARRANT_TABLE_20260903.md`).
...
- **Warrant by assertion.** Five of eight directional claims are `W_DIRECTION_ASSUMED` (entries 6, 23, 25, 27, 56; 24, 26 and 54
  are `W_DIRECTION_DERIVED` after reconciliation — `WARRANT_TABLE_20260903.md`), and the founding identification's successors (entries 23–27) rest on a scale that is
...
## Source-referee corrections (V2, 2026-09-06 01:01 KST)
A blind codex source referee (`BHU_PROGRAMME_SYNTHESIS_SOURCE_REFEREE_codex_20260906.md`, ACCESS_SHA verified) checked 42 sourced
