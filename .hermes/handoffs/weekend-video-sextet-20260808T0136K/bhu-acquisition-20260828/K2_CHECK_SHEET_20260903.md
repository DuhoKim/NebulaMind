...
`K2_RESULT_20260903.md`. Sources under `bhu-reading-20260823/sources/`. The route-2 executable receipt added by the
support audit is `K2_route2_tori_repair.py` with output `K2_route2_tori_repair.out` and regression test `test_K2_route2_agy.py`.

## A. Inputs (every one pinned)
| # | input | value / form | pin |
|---|---|---|---|
| 1 | interior metric | ds² = −dτ² + a²[dχ² + S_k(χ)²dΩ²] with S_k = sin χ (k=+1) or χ (k=0) — the angular gauge used in every step below; the sources print the equivalent r-gauge a²[dr²/(1−kr²) + r²dΩ²] with r = S_k(χ); dust ρ ∝ a⁻³ | entry 22 `2606.25023_clean.txt` L316–320; entry 4 `knutsen_2009_gravcosmol15_273_clean.txt` L43–66, L1093–1103; entry 56 `gaztanaga_mass_mnras_clean.txt` L141 |
| 2 | exterior metric | ds² = −F dT² + F⁻¹dR² + R²dΩ², F = 1 − 2GM/R − ΛR²/3 | entry 4 `knutsen_2009_gravcosmol15_273_clean.txt` L238–283 |
| 3 | smooth (Darmois–Israel) condition | induced metric and extrinsic curvature K_ab continuous; a shell ⇔ [K_ab] ≠ 0 | entry 4 L516–525, L603–642, L695–700; entry 22 L289–297 |
| 4 | null junction (Barrabès–Israel) | transverse curvature K_ab = e^μ_a e^ν_b ∇_μ N_ν; jump ⇒ surface stress | entry 5 `khakshournia_2010_note_pathria_arxiv1412.0105_clean.txt` L49–55, L137–179 |
...
| 6 | Pathria's boundary | r_b = 1, i.e. the maximum-expansion sphere χ = π/2 of the closed model | entry 1 L67–71, L394–409; entry 4 L1063–1072, L1093–1119 |
...
2. **B1 (comoving χ = χ*):** interior angular curvature K⁻_θθ = R C_k with C_k ≡ √(1 − k S_k²); exterior K⁺_θθ = R √(Ṙ² + F).
   ...
   ⇒ **S_ab = 0: smooth.** k=0: expands forever (entry 56's cell). k=+1: recollapses in the stated range (entry 1 L394–417;
   entry 22 Proposition 2 L740–748). *(Control C1 prints the matching identity; recollapse is not inferred from C_k² < 1.)*
...
5. **B3 (general timelike χ*(τ)):** [K_θθ] = 0 ⇔ M equals the Misner–Sharp dust mass inside the boundary; Birkhoff makes M constant
   ...
   third seat verified this equation; the support-audit receipt prints the mass derivative and the equator second derivative at
   `K2_route2_tori_repair.out` L4–5. A shelled non-comoving boundary has trajectory-dependent S_ab, reported, not classed).
6. **Extra row:** the comoving surface χ* = π/2 followed in time: smooth, M = C/2, exterior side T = const with F(R_b) = −ȧ², so
   ...
   F > 0 this forces the static limiting case; `K2_route2_tori_repair.out` L11).
7. **Deletion probe (C4):** removing the energy-condition test flips exactly the two B2 k=+1 cells and nothing else
   (`K2_route2_tori_repair.out` L12; route-1 exact failure set in `K2_claude_RESULT.md` §Control outcomes).
...
**Second route (17:30 KST; executable receipt repaired 2026-09-04):** the agy report's independent Darmois/Misner–Sharp derivation
reproduces every class and control. Its original companion script was a no-output stub; the support audit preserved that file and
added an actual symbolic receipt plus a passing regression test — `K2_ROUTE2_RECONCILIATION_20260903.md`.
