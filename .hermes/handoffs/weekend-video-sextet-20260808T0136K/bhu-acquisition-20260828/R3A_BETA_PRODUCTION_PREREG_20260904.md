# R3-A — FROZEN PRE-REGISTRATION: is entry 59's particle-production coefficient β derived, or tuned?

**Tori, 2026-09-04 19:56 KST. Version 1. FROZEN pending the fresh referee gate. ORDERED by Duho ("run 1 and 2", relayed by Blanc 2026-09-04 19:52 KST — first of two, sequential.)**
...
Classes 3 and 4 are distinct and must not be merged: a seat separates them by quoting the exact text where the
author either targets a specific observable value (fitted) or leaves the parameter arbitrary (free).

## 4. Controls, each with an exact named code

- **C1 — source identity.** The pinned text is PDF-extracted and **does not contain clean strings**: the gate
  verified, and Tori re-verified byte-exactly, that L126 stores `K = β(κ \x02˜ )2 ,` — the epsilon is a raw
  `\x02` control byte — and that L128 contains the ligature `ﬃ` in "coeﬃcient". A seat matching on clean
  strings will fail. **C1 therefore requires the seat to print the Python `repr()` of lines 87, 126 and 128
  and to assert a match after normalisation** (strip control bytes, expand ligatures). Exact assertion:
  `C1_SOURCE_IDENTITY=PASS`.
- **C2 — dependence probe, with a STALL GUARD.** Show which reported observables move with `β` and which do not.
  The gate warned this design can stall: symbolic mapping through the paper's solver chain may hang a symbolic
  engine indefinitely, burning the seat-day without tripping the two-attempt rule. **Every symbolic operation
  therefore runs under a hard wall-clock cap of 120 seconds.** On timeout the seat prints
  `DEPENDENCE_SYMBOLIC_TIMEOUT` and falls back to numerical parameter variation, which answers the same
  question. A stall is a reportable outcome, never a silent hang. If none
...
## 8. Harness custody — pinned, per the BS-4 lesson

Duho's 19:52 relay: *"a named script is not a receipt until someone runs it, and the harness that runs it must be
pinned — Hwao's BS-4 was REJECTED today on exactly that."* Taken as a standing requirement. **Every seat must EXECUTE live commands to verify its python version and python3 binary SHA, printing the actual
execution output at the top of its record, and the result file records it.** The gate was right that a
pre-written block with a hardcoded hash is decoration — a seat would simply echo it. The values below are given
**for comparison only**; a seat that prints them without having run the commands fails C5:

```
python      3.9.6 (CPython), /Library/Developer/CommandLineTools/usr/bin/python3
sympy       1.14.0
numpy       1.26.4
platform    Darwin 25.6.0 arm64
python3 sha256  b8763cf250e607a778bb4603cecb5b90338814d0a3dfcba0d57b1de242f610e9  (/usr/bin/python3)
```

A seat whose printed harness differs from this block has its output flagged in the reconciliation, not silently merged.
Exact assertion: `C5_HARNESS_PINNED=PASS`, added to §4's control set, passing **only** on live execution output.

## 9. Gate record (draft → V1 frozen)

`R3_DRAFT_GATE_20260904_agy.md` returned `GATE_A=PREREG_SOUND_WITH_REPAIRS` with one repair, **applied verbatim**: C3
told the seat to "confirm the derivation", which the gate correctly called non-mechanical — a seat could satisfy it
with a sentence. It now requires the seat to **print the exact text and line numbers**. The gate also confirmed this
does not re-run K3 ("auditing the particle production rate is physically distinct from K3's audit of the spin-density
closure") and judged the study worth its one seat-day.

The gate also answered the stall question with a real risk this document now guards against, and confirmed the line
tracing, the circularity bound, the no-re-run of K3, and that no sentence asserts the paper is wrong.

R3A_PREREG_V2_FROZEN
