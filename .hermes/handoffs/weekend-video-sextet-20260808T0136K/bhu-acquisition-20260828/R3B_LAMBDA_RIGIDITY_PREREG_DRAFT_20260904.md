# DRAFT — NOT ORDERED — R3-B pre-registration: does the Λ-from-boundary construction forbid evolving dark energy?

**Tori, 2026-09-04 19:51 KST.** Round-3 topic #2. **Drafted so that an order starts from a refereed text. No
derivation has been run and no data has been touched.**

## 0. Why this would exist

Entry 56 (Gaztañaga 2023) sets `Λ ≡ 3/r_S²` (**L28**) and reads cosmic acceleration as a measurement of the
gravitational radius. Its own equations hold the enclosed mass **constant** (**L134**, **L138**, **L143**), so `r_S` is
constant and `Λ` is exactly constant — equation of state `w = −1`, with no evolution available.
...
R3B_PREREG_DRAFT_READY_FOR_GATE — NOT ORDERED
