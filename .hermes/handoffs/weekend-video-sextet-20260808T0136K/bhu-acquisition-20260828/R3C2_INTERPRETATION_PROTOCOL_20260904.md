...
It reads **two tallies from one pass**: the reproduction tally (`REPRO_EXACT` / `REPRO_FAILED` / the non-arithmetic
...
- **One or more claims filed `REPRO_EXACT` with `rests_on = DERIVED_ONLY`** — a number that follows from the paper's
...
- **`REPRO_EXACT` claims exist but every one has `rests_on` in {`USES_CHOSEN`, `USES_FITTED`, `USES_IMPORTED`,
