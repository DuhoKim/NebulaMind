...
**Tori, 2026-09-05. Version 26 — INTEGRATED CANDIDATE, UNADOPTED (V25 + D1 wording B + the stronger D7 + ownership batching; NONE of the three is approved — this file is a SEPARATE candidate, not the living draft, which stays V25); the signed design of record remains V23 (`R3C2_V23_DESIGN_OF_RECORD_55b466fa.md`, frozen 2026-09-06 00:00:22Z) until Duho signs V24 (see §10; §10.18 — the narrow scope correction after the 10:17 run abort; nothing else changed — Duho's ruling "hide the comparison, keep the taxonomy"). OPTION (c) ADOPTED — Duho's ruling "Q-R3C2 c", 2026-09-05 14:08 KST: one pass,
...
> fails a ledger that carries one. The seat's tool is `r3c2_ledger_tools.py`, sha256 `9e916b81e296f890ea88977c6b469255f8d135a33615c6fe4e8c25e005b188fb` (the staged tool of `r3c2_staged_d1d7/`, which takes that name at adoption; `validate` takes the candidate file as its third argument), pinned at
...
- **C1 — denominator.** Claims **included**, claims **excluded** (with the exclusion ledger of §3), and the attempts
  ...
  `ledger_b<k>.json` and `SEAT_REPORT_b<k>.md`, sealed by the custodian in batch order before the next batch is dispatched; the lane's
  pinned `join` (pure function, no renaming) produces the seat's one candidate file, one exclusion file and one ledger, over which
  `census` and `validate` run. The files named below are those joined files.** <!--SEAT-REDACT-->*(This control previously referenced a class §3 abolished — a gate finding; the
...
- **C1B — batch coverage.** `C1B_BATCH_COVERAGE=PASS` iff every manifest text is owned by exactly one batch, every owned text's bytes
  ...
  tally. `C1B_BATCH_COVERAGE=PASS|FAIL|NOT_RUN`.
...
## 10.20 V26 — INTEGRATED CANDIDATE, UNADOPTED: V25 + the three recommended method texts, for the principal's choice (2026-09-06 22:12 KST)

**Status.** Not the living draft (V25, `ab6352d3…`, unchanged), not signed, not gated, not run. Prepared under standing preparation
authority (Blanc 22:11: integration work is not blocked by the coordinator's earlier anti-churn pause on numbered versions, which was
his own measure and never a limit of the principal's) so that, once the principal chooses, the version carrying his choices already
exists in reviewable form; if he chooses an alternative, the corresponding change below is applied instead. Sources: the candidate
clause file, the batch preparation file, the staged kit and the decision brief; their digests are in the brief.

**Exact change list versus V25.**
1. §1 — the excluded-kinds sentence names the sixth kind (`AUTHOR_SPECIFIED_INPUT`), matching §3 and the tool.
2. §2 — the IMPORTED rule is D1 wording B: evidence at the claiming paper's naming sentence, value machine-matched as a numeric token at
   the external line of a manifest text whose bytes verify, first line carrying symbol and numeral, claim bound to its claiming file
   through the candidate file. *Alternative A:* quote the source's own line instead (the validate rule must be re-staged first).
   *Alternative "change":* inherited origins (`IMPORTED_CHOSEN`…) — a taxonomy expansion, not prepared.
3. C3 — one sentence: how `ORIG_CITATION` is satisfied for an import; the seat tool pinned in §3 is the staged tool (`9e916b81e296f890…`),
   whose `validate` takes the candidate file.
4. C6 — the stronger D7: source-only enumeration of all 89 texts first, census-gated first-write seal, seed-required selection, handout
   of identifiers with file and line only, re-derivations sealed before release, comparison over the union of keys with omissions in
   either direction fatal and disputes under the 10% rule, bounded PASS. *Alternative "weaker":* revert this control to V25's text.
5. C1, C1B, §9 — ownership batching: sessions under one packet, all texts furnished, ownership partitioned, global identifiers, sealed
   batches, pinned join, `C1B_BATCH_COVERAGE` and `JOIN` controls; the lane tool `r3c2_batch_tools_STAGED.py` (`1c1251931acc4666…`) is
   pinned lane-side at adoption. *Alternative "line-balanced":* a different pure function of the same manifest for the partition;
   *alternative "defer":* revert C1/C1B/§9 to V25's text and the run cannot start.

**Costs.** Reading: 12 sessions × 2 seats, plus 12 for the auditor; batch 1 confirms capacity before batch 2. Audit: a third full
enumeration plus assigned re-derivations; runtime unmeasured. Custody: the dispatch record must inventory and probe each session,
dispatch the auditor in a fresh context, keep selection custodian-only, and record release events against both seals.

**What this version does not do.** It adopts nothing; its packet is built by the pinned builder into a SEPARATE candidate packet
directory (`r3c2_seat_packet_v26_candidate/`), the V25 seat packet untouched; C0 and the two-seat gate may run on it as preparation;
the approval procedure runs on it only after the principal's choices are recorded here in his words.
