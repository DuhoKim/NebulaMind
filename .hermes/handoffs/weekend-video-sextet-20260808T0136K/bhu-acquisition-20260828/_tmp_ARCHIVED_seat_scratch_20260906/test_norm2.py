...
k_min = 1e-6
k_max = 20.0
# More points to resolve k_max up to 20
r_grid = np.linspace(1e-5, L, 50000)

k_int = np.geomspace(k_min, k_max, 50000)
...
idx_norm = np.searchsorted(k_out, 0.1)
ratio = P_B[idx_norm] / (As * (k_out[idx_norm] / KP)**(ns - 1))
print(f"Norm ratio at k=0.1: {ratio}")
