...
for i,(name,A,B,aud,exp,must,comp) in enumerate(CONS):
    ...
    def cmp_view(k):
        ...
        (an identity, not an ordering) and is the ONLY field allowed to differ in that variant — every finding, code, reason, verdict, merged byte
        and ledger digest must still be equal."""
        r=runs[k]
        if r[0]=="MERGE_FAIL" or "reversed-rederivation-order" not in k: return r
    ...
    whole_ok=all((runs[k]==base) if "reversed-rederivation-order" not in k else (cmp_view(k)==base_v) for k in keys)
    ...
    diff=[k for k in keys if ((runs[k]!=base) if "reversed-rederivation-order" not in k else (cmp_view(k)!=base_v))]
...
print(f"runs per construction: 2 seat orders x seeds {SEEDS} + reversed arrival + reversed rederivation order; ONLY={ONLY!r}")
