...
GI="graph integrity"
SEARCH_DIFF="origin_search differs"
SS={"query":["x","choose","adopt","fit"],"files":["p.txt","q.txt"],"matches":["p.txt:6","q.txt:1"]}
def _srev(s,f): d=dict(s); d[f]=list(reversed(d[f])); return d
SS_FILES_REV=_srev(SS,"files"); SS_MATCHES_REV=_srev(SS,"matches")
SS_CHANGED=dict(SS,matches=["p.txt:6","q.txt:2"])      # an entry CHANGED — a real evidence difference
SS_ADDED=dict(SS,matches=SS["matches"]+["r.txt:9"])     # an entry ADDED
SS_DELETED=dict(SS,matches=SS["matches"][:1])           # an entry DELETED
...
CONS=[
 ...
 ("search-inventory order (codex V37 N1, MOI source g): both seats ORIG_SILENT with the same search; auditor supplies the same inventory", [SIL(SS)],[SIL(SS)], {"i1":dict(full(SIL(SS)),origin_search=SS)}, 0, "", None),
 ("search-inventory order, auditor lists the same files in REVERSE (codex V37 N1: this flipped C6 PASS->FAIL)", [SIL(SS)],[SIL(SS)], {"i1":dict(full(SIL(SS)),origin_search=SS_FILES_REV)}, 0, "", None),
 ("search-inventory order, auditor lists the same matches in REVERSE", [SIL(SS)],[SIL(SS)], {"i1":dict(full(SIL(SS)),origin_search=SS_MATCHES_REV)}, 0, "", None),
 ("GUARD (Blanc 09:07): a CHANGED search entry must still FAIL after canonicalisation — normalisation must not erase an evidence difference", [SIL(SS)],[SIL(SS)], {"i1":dict(full(SIL(SS)),origin_search=SS_CHANGED)}, 1, SEARCH_DIFF, None),
 ("GUARD: an ADDED search entry must still FAIL after canonicalisation", [SIL(SS)],[SIL(SS)], {"i1":dict(full(SIL(SS)),origin_search=SS_ADDED)}, 1, SEARCH_DIFF, None),
 ("GUARD: a DELETED search entry must still FAIL after canonicalisation", [SIL(SS)],[SIL(SS)], {"i1":dict(full(SIL(SS)),origin_search=SS_DELETED)}, 1, SEARCH_DIFF, None),
...
for i,(name,A,B,aud,exp,must,comp) in enumerate(CONS):
    ...
    def _revsearch(v):   # source (g): the auditor's own inventory order — the permutation that flipped C6
        if not (isinstance(v,dict) and isinstance(v.get("origin_search"),dict)): return v
        s2=dict(v["origin_search"])
        for f in ("files","matches"):
            if isinstance(s2.get(f),list): s2[f]=list(reversed(s2[f]))
        return dict(v,origin_search=s2)
    aud_revsearch={k:_revsearch(v) for k,v in aud.items()}
    ...
    runs["AB/seed0/reversed-auditor-search-order"]=outcome(i,a,b,aud_revsearch,name,SEEDS[0],"ABaudsearch")
    keys=list(runs); base=runs[keys[0]]
    def AUD_BYTES_VARY(k): return ("reversed-rederivation-order" in k) or ("reversed-auditor-dependency-order" in k) or ("reversed-auditor-search-order" in k)
    def cmp_view(k):
...
print(f"runs per construction: 2 seat orders x seeds {SEEDS} + reversed arrival + reversed rederivation order + reversed dependency order (both seat orders) + reversed auditor dependency order + reversed auditor search-inventory order; ONLY={ONLY!r}")
