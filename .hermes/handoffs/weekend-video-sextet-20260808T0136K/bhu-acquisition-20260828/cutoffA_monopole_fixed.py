#!/usr/bin/env python3
"""Reading B with the weighted monopole removed and support chi_S=A."""
...
def wtilde(s):
    """Transform of the overlap window of two radius-A/2 balls (support A)."""
    z = s * BALL_RADIUS
    f = np.ones_like(z)
    nz = z != 0
...
def reading_b_table(kmin):
    ...
    p_dim_min = p_dim.min()
    p_tiny = p_dim[0]
    p_ks = np.interp(KSEC, klo, p_dim)
    tiny_ratio = abs(p_tiny) / abs(p_ks)
...
def main():
    ...
    print("Monopole-subtracted Reading B calculation (fixed support)")
    print(f"chi_section/support = {A:.1f} Mpc; ball_radius = {BALL_RADIUS:.1f} Mpc; k_section = {KSEC:.9g} 1/Mpc")
    ...
    vals, minima, pminima, ratios = [], [], [], []
    for km in kmins:
        k, pk, norm, c, minimum, k_at_min, pminimum, ratio = reading_b_table(km)
        cl = camb_cls(k, pk)
        s = s_half(cl)
        vals.append(s)
    ...
    print(f"Sanity |P_B(k_tiny=1e-7)|/|P_B(k_S={KSEC:.9g})| = {ratios[-1]:.8e} (< 1e-3: {'PASS' if ratios[-1] < 1e-3 else 'FAIL'})")
    ...
    print(f"Comparison (smallest-k_min case): LCDM 34,924 | Reading A 6,897 | Reading B fixed {vals[-1]:.0f} | observed ~1,150")
