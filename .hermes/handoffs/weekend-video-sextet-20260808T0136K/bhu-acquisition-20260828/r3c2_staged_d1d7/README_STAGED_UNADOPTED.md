# r3c2_staged_d1d7 — STAGED, UNADOPTED tooling for the D1 / D7 / batch candidates (V35, 2026-09-06 20:38 KST)

Nothing here is installed. Since V26 the tools here are the SAME bytes as the pinned tools (`r3c2_ledger_tools.py` sha256 0c11e019b0a438de…, and the lane, batch, manifest and builder tools),
...
- `r3c2_spi_exhibition.py` — SEAT-PERMUTATION INVARIANCE exhibition: every reviewer construction under both seat orders; merged bytes and the whole audit outcome must be equal. `_v32_bytecopy/`, `_v33_bytecopy/` and `_v34_bytecopy/` — pinned byte copies of the V32, V33 and V34 lane and seat tools against which the exhibition must FAIL (the fail-first records: seat-order search loss at V32; unsorted merge serialisation at V33; per-input mixing composing a cyclic primary view at V34). Merge now preserves both complete seat graphs; compute classifies from them; the exhibition also runs compute on every merged ledger. Every construction asserts its expected PASS/FAIL and permutation equality of merged bytes and the complete artefact.
