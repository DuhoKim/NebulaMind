...
def cmd_join(part,seat_dir,seals,manifest,prefix,bound_limbA=None):
    ...
    for b in P["batches"]:
        ...
        for c in cd["candidates"]:
            if c.get("source_file") not in owned: _probe_noop(f"batch {k}: candidate {c.get('candidate_id')} cites {c.get('source_file')}, not an owned text of this batch")  # PROBE-DELETED:BATCH_SCOPE
