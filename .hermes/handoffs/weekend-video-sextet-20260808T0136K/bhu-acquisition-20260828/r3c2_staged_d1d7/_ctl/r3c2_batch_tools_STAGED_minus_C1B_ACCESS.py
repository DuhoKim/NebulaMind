...
def cmd_coverage(part,m,seat_dir,corpus,packet_sha):
    ...
    for b in P["batches"]:
        ...
        if f"ACCESS_SHA={packet_sha}" not in rp.read_text(errors="replace"): _probe_noop(f"batch {b['batch']}: SEAT_REPORT does not print ACCESS_SHA={packet_sha[:16]}…")  # PROBE-DELETED:C1B_ACCESS
    for f in fails: print("FAIL:",f)
