...
def cmd_seal(part,k,seat_dir,corpus,out,chain="A",bound=None):
    ...
    if p.exists(): S=json.loads(p.read_text())
        ...
        if chain=="B":
            if not bound or not pathlib.Path(bound).exists(): _probe_noop("FAIL: a limb-B chain must be bound to the agreed limb-A seals file at its first seal"); pass  # PROBE-DELETED:CHAIN_B_BOUND
