...
def cmd_join(part,seat_dir,seals,manifest,prefix,bound_limbA=None):
    ...
    for b in P["batches"]:
        ...
        for r in ld:
            ...
            if r.get("source_file") and r.get("source_file") not in mf: _probe_noop(f"batch {k}: record {r.get('input_id')} cites {r.get('source_file')}, not a manifest text")  # PROBE-DELETED:EVIDENCE_MANIFEST
