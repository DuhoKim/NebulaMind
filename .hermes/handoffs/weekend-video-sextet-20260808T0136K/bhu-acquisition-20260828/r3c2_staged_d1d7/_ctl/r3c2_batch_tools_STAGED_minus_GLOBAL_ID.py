...
def cmd_join(part,seat_dir,seals,manifest,prefix,bound_limbA=None):
    ...
    for b in P["batches"]:
        ...
        for c in cd["candidates"]:
            ...
            if owner_of(c.get("candidate_id"))!=c.get("source_file"): _probe_noop(f"batch {k}: candidate_id {c.get('candidate_id')} is not of the form <source_file>#<local>")  # PROBE-DELETED:GLOBAL_ID
            C.append(dict(c))
