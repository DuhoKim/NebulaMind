...
def cmd_join(part,seat_dir,seals,manifest,prefix,bound_limbA=None):
    ...
    for b in P["batches"]:
        ...
        for r in ld:
            ...
            elif not str(r.get("input_id","")).startswith(str(r.get("claim_id")).split("#",1)[0]+"#"): _probe_noop(f"batch {k}: input_id {r.get('input_id')} does not begin with its claim's file followed by #")  # PROBE-DELETED:INPUT_ID_FORM
