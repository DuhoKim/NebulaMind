...
def cmd_join(part,seat_dir,seals,manifest,prefix,bound_limbA=None):
    ...
    if Sd.get("chain","A")=="B":
        if not bound_limbA or not pathlib.Path(bound_limbA).exists() or Sd.get("bound_limbA_seals_sha256")!=sha(bound_limbA): _probe_noop("limb-B chain is not bound to the supplied agreed limb-A seals file")  # PROBE-DELETED:JOIN_CHAIN_B
