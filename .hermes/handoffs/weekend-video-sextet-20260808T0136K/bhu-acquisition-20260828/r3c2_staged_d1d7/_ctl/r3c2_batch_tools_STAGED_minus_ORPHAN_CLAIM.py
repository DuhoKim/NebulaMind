...
def cmd_join(part,seat_dir,seals,manifest,prefix,bound_limbA=None):
    ...
    for b in P["batches"]:
        ...
        for r in ld:
            if r.get("claim_id") not in inc_ids: _probe_noop(f"batch {k}: ledger record {r.get('input_id')} names claim {r.get('claim_id')}, not an included candidate of this batch")  # PROBE-DELETED:ORPHAN_CLAIM
