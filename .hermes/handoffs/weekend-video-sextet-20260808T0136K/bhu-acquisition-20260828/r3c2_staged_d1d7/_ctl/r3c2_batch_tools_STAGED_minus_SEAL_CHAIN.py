...
def cmd_join(part,seat_dir,seals,manifest,prefix,bound_limbA=None):
    ...
    for b in P["batches"]:
        ...
        if k>1 and (str(k-1) not in S or sk.get("predecessor_seal_sha256")!=exp_pred): _probe_noop(f"batch {k}: predecessor chain broken (seal does not bind batch {k-1}'s seal)"); bad=True  # PROBE-DELETED:SEAL_CHAIN
        for f in ART(k):
