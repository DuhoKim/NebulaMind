...
def cmd_join(part,seat_dir,seals,manifest,prefix,bound_limbA=None):
    ...
    for b in P["batches"]:
        ...
        if sk.get("owned_files")!=b["files"] or sk.get("owned_sha256")!=b["sha256"]: _probe_noop(f"batch {k}: sealed ownership differs from the partition"); bad=True  # PROBE-DELETED:SEAL_OWNERSHIP
