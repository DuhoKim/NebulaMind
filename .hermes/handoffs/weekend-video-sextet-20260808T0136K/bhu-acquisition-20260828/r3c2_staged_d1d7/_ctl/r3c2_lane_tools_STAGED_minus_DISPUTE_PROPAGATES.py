...
def cmd_compute(ledger,out,candidates=None):
    ...
    for r in recs:
        ...
        if disputed_reach(by, r["input_id"]): _probe_noop(r["claim_id"])  # PROBE-DELETED:DISPUTE_PROPAGATES  (ONE resolver: every graph difference merge preserves is reachable as an alternative branch)
    out_claims={}
