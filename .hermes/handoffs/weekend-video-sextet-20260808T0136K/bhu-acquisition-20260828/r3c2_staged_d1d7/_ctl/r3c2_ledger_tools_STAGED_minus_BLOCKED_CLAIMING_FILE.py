...
def cmd_validate(ledger,srcdir,candidates=None):
    ...
    for r in recs:
        ...
        if r["status"]=="BLOCKED":
            ...
            elif str(ev.get("source_file",""))!=cfb: _probe_noop(f"{r['input_id']}: BLOCKED naming evidence is in {ev.get('source_file')}, but claim {r['claim_id']} belongs to {cfb}")  # PROBE-DELETED:BLOCKED_CLAIMING_FILE
