...
def cmd_audit_compare(seal1, ac, ax, sc, sx, sl, sel, seal2, red, out):
    ...
    for k in sorted(set(a_all)|set(s_all)):
        ...
        if s is not None and a is None:
            ...
            if a.get("included"): row["result"]="OMISSION"; row["direction"]="audit_included_absent_from_sealed"; _probe_noop(f"COMPLETENESS audit_included_absent_from_sealed: {list(k)}")  # PROBE-DELETED:C6_BOTH_SEATS_MISSED
