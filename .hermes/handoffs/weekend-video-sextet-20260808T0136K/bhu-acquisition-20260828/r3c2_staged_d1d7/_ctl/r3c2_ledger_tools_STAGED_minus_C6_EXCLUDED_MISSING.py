...
def cmd_audit_compare(seal1, ac, ax, sc, sx, sl, sel, seal2, red, out):
    ...
    for k in sorted(set(a_all)|set(s_all)):
        ...
        if s is not None and a is None:
            ...
            else: row["result"]="OMISSION"; row["direction"]="audit_excluded_absent_from_sealed"; _probe_noop(f"COMPLETENESS audit_excluded_absent_from_sealed: {list(k)} (a passage the seats never enumerated is incompleteness whatever the auditor's disposition)")  # PROBE-DELETED:C6_EXCLUDED_MISSING
        else:
