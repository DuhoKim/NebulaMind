...
def cmd_audit_compare(seal1, ac, ax, sc, sx, sl, sel, seal2, red, out):
    ...
    for cid0,r0 in sorted(RD.items(),key=lambda kv:str(kv[0])):   # V36 canon
        for iid0,ar0 in sorted((r0.get("inputs") or {}).items(),key=lambda kv:str(kv[0])):
            ...
            if sr0 is not None and str(sr0.get("claim_id"))!=str(cid0): identity_conflict[iid0]=f"reconstructed under claim {cid0}, but the sealed record belongs to claim {sr0.get('claim_id')}"; _probe_noop(f"C6_IDENTITY: {iid0} {identity_conflict[iid0]}")  # PROBE-DELETED:C6_IDENTITY
