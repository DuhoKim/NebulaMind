...
def cmd_audit_compare(seal1, ac, ax, sc, sx, sl, sel, seal2, red, out):
    ...
    for k in sorted(set(a_all)|set(s_all)):
        ...
        if s is not None and a is None:
            if s.get("included"): row["result"]="OMISSION"; row["direction"]="sealed_included_absent_from_audit_enumeration"; _probe_noop(f"COMPLETENESS sealed_included_absent_from_audit_enumeration: {list(k)}")  # PROBE-DELETED:C6_SEALED_MISSING
