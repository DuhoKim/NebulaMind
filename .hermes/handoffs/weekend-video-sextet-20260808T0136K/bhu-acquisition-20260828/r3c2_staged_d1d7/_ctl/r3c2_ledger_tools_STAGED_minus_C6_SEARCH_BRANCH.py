...
def cmd_audit_compare(seal1, ac, ax, sc, sx, sl, sel, seal2, red, out):
    ...
    def compare_record(iid, ar, sr):
        ...
        if aev.get("reason_code")=="ORIG_SILENT":
            ...
            if canon_search(ar.get("origin_search"))!=canon_search(expected_search): _probe_noop("origin_search differs (structural comparison against the matched branch's search)")  # PROBE-DELETED:C6_SEARCH_BRANCH
        return diffs, branch
