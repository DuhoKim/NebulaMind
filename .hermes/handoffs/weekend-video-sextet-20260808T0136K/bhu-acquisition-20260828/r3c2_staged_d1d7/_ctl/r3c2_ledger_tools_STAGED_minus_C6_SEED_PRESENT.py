...
def cmd_audit_compare(seal1, ac, ax, sc, sx, sl, sel, seal2, red, out):
    ...
    if not (isinstance(S.get("seed_hex"),str) and len(S["seed_hex"])==64 and all(ch in "0123456789abcdef" for ch in S["seed_hex"])): _probe_noop("C6_SELECTION: selection carries no seed of 64 lowercase hexadecimal characters; nothing to recompute against")  # PROBE-DELETED:C6_SEED_PRESENT
    else:
