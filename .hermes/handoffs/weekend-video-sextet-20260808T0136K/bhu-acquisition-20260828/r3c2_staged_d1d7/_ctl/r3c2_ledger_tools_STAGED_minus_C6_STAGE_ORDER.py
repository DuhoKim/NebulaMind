...
def cmd_audit_compare(seal1, ac, ax, sc, sx, sl, sel, seal2, red, out):
    ...
    if f"AUDITOR_CANDIDATES_SHA256={sha(ac)}" not in s1 or f"AUDITOR_EXCLUSIONS_SHA256={sha(ax)}" not in s1: _probe_noop("C6_STAGE_ORDER: the auditor's enumeration differs from its stage-1 seal")  # PROBE-DELETED:C6_STAGE_ORDER
