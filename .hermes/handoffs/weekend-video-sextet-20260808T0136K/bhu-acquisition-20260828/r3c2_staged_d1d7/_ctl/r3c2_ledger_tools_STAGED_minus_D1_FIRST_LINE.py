    ...
    for r in recs:
        ...
        if r["status"]=="PRINTED" and rc=="ORIG_CITATION":
            ...
            if vl is None: fails.append(f"{r['input_id']}: cannot read external value line {r['source_file']}:{r['source_line']}")
                ...
                elif first!=int(r["source_line"]): _probe_noop(f"{r['input_id']}: external value line {r['source_line']} is not the first line of {r['source_file']} carrying both {r['symbol']} and {r.get('value')} (that is line {first})")  # PROBE-DELETED:D1_FIRST_LINE
            continue
        if r["status"]=="PRINTED":
