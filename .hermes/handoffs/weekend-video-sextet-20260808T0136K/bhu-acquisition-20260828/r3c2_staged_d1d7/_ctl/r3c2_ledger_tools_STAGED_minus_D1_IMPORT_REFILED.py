    ...
    for r in recs:
        ...
        if r["status"]=="PRINTED":
            ...
            if r["source_file"]!=cf0 and not (r["origin"]=="IMPORTED" and rc=="ORIG_CITATION"):
                _probe_noop(f"{r['input_id']}: value line is in {r['source_file']} but claim {r['claim_id']} belongs to {cf0}: such a record must be IMPORTED with ORIG_CITATION (submitted {r['origin']}/{rc})")  # PROBE-DELETED:D1_IMPORT_REFILED
                continue
