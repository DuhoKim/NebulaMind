...
def cmd_validate(ledger,srcdir,candidates=None):
    ...
    for r in recs:
        ...
        if r["status"]=="PRINTED" and rc=="ORIG_CITATION" and r["source_file"]==claim_file.get(r["claim_id"]):
            ...
            elif not token_in(r.get("value"), vl): _probe_noop(f"{r['input_id']}: value {r.get('value')} is not a numeric token at {r['source_file']}:{r['source_line']}")  # PROBE-DELETED:D1_LOCAL_VALUE_TOKEN
            continue
