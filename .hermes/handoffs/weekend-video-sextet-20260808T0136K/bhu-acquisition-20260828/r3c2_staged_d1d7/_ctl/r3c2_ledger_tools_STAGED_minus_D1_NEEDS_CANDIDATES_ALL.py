...
def cmd_validate(ledger,srcdir,candidates=None):
    ...
    for r in recs:
        ...
        if r["status"]=="PRINTED":
            if not candidates: _probe_noop(f"{r['input_id']}: PRINTED record needs the candidate file to bind claim {r['claim_id']} to its claiming paper"); continue  # PROBE-DELETED:D1_NEEDS_CANDIDATES_ALL
