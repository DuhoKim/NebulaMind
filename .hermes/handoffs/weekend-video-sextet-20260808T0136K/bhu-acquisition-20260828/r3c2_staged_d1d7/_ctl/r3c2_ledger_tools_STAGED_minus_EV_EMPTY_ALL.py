...
def cmd_validate(ledger,srcdir,candidates=None):
    ...
    for r in recs:
        ...
        if rc!="ORIG_SILENT":
            ...
            if not str(ev.get("verbatim","")).strip(): _probe_noop(f"{r['input_id']}: empty quotation (every non-ORIG_SILENT record quotes its evidence line)"); continue  # PROBE-DELETED:EV_EMPTY_ALL
