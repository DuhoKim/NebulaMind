...
def cmd_validate(ledger,srcdir,candidates=None):
    ...
    for r in recs:
        ...
        if r["status"]=="PRINTED":
            ...
            if not token_in(r.get("value"), line): _probe_noop(f"{r['input_id']}: value {r.get('value')} is not a numeric token at {r['source_file']}:{r['source_line']}")  # PROBE-DELETED:PRINTED_VALUE_LINE
