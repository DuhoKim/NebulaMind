...
def cmd_validate(ledger,srcdir,candidates=None):
    ...
    for r in recs:
        ...
        if r["status"]=="STANDARD":
            ...
            elif r.get("source_file") and r.get("source_file")!=cfs: _probe_noop(f"{r['input_id']}: STANDARD value line is in {r.get('source_file')}, outside the claiming file {cfs}: a value outside the claiming paper follows the named-source rule (PRINTED/IMPORTED with ORIG_CITATION), even when it is on the closed list")  # PROBE-DELETED:STD_CLAIMING_FILE
