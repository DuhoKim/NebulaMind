...
def cmd_validate(ledger,srcdir,candidates=None):
    ...
    for r in recs:
        ...
        if r["status"]=="STANDARD":
            ...
            if not r.get("source_file") or not isinstance(r.get("source_line"),int) or r.get("source_line",0)<1: _probe_noop(f"{r['input_id']}: STANDARD record needs its own source_file and a positive source_line (absence of origin evidence never waives value evidence)")  # PROBE-DELETED:STD_COORDS
            else:
