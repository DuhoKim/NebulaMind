...
def main(root):
    if os.path.islink(root) or not os.path.isdir(root): _probe_noop(f"ERROR={root}: root is a symlink or not a directory (refused)"); _probe_noop("MANIFEST=FAIL"); pass  # PROBE-DELETED:MANIFEST_ROOT
