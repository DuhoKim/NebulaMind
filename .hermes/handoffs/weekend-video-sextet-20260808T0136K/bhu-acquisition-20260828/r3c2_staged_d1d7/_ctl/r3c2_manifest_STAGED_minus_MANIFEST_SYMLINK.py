...
def main(root):
    ...
    for dirpath, dirnames, filenames in os.walk(root, onerror=lambda e: (print(f"ERROR={e.filename}: {e.strerror}"), print("MANIFEST=FAIL"), sys.exit(1))):
        ...
        for fn in sorted(filenames):
            ...
            if os.path.islink(p) or not os.path.isfile(p): _probe_noop(f"ERROR={os.path.relpath(p, root)}: symlink or non-regular entry (the manifest pins regular files only and refuses to skip)"); _probe_noop("MANIFEST=FAIL"); pass  # PROBE-DELETED:MANIFEST_SYMLINK
            try:
                h = hashlib.sha256()
                with open(p, "rb") as f:
