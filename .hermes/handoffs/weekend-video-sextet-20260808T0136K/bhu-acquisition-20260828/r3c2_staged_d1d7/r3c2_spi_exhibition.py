...
def w(name,obj): p=W/name; p.write_text(json.dumps(obj,indent=1,sort_keys=True)); return str(p)
...
# constructions: (name, seatA records, seatB records, auditor inputs for c1 (dict input_id -> full record or None for absent))
CONS=[
 ("same-origin, different complete searches (codex V32 F1); auditor = B", [SIL(S1)],[SIL(S2)], {"i1":dict(full(SIL(S2)),origin_search=S2)}),
 ("same-origin, different searches; auditor = A", [SIL(S1)],[SIL(S2)], {"i1":dict(full(SIL(S1)),origin_search=S1)}),
 ("ORIG_SILENT alternative vs CHOSEN (codex V31 F1); auditor = the silent branch", [CH()],[SIL(S1)], {"i1":dict(full(SIL(S1)),origin_search=S1)}),
 ("complete alternative branch: CHOSEN vs FITTED; auditor = FITTED", [CH()],[FIT()], {"i1":full(FIT())}),
 ("hybrid direction 1: primary origin+evidence with the alternative's parents", [DER(["i2"]),STD("i2"),STD("i3")],[DER(["i3"]),STD("i2"),STD("i3")], {"i1":full(dict(DER(["i3"]),origin_evidence=DER(["i2"])["origin_evidence"])),"i2":full(STD("i2")),"i3":full(STD("i3"))}),
 ("hybrid direction 2: alternative origin+evidence with the primary's parents", [DER(["i2"]),STD("i2"),STD("i3")],[dict(CH(),derived_from=["i3"]),STD("i2"),STD("i3")], {"i1":dict(full(CH()),derived_from=["i2"]),"i2":full(STD("i2")),"i3":full(STD("i3"))}),
 ("agreeing root sets, differing evidence quotation; auditor = fabricated third", [CH()],[CH(ev_verbatim="we choose a = 2 (sic)")], {"i1":full(CH(ev_verbatim="fabricated"))}),
 ("wrong-owner dependency: i9 belongs to c2 but reconstructed under cZ", [DER(["i9"]),rec("c2","i9")],[DER(["i9"]),rec("c2","i9")], {"i1":full(DER(["i9"]))}),
 ("transitive: dependency in another claim disputed, auditor matches its alternative", [DER(["i9"]),rec("c2","i9",origin="CHOSEN",ev_reason_code="ORIG_CHOICE_STATED",ev_source_file="paperB.txt",ev_source_line=5,ev_verbatim="we choose a = 2")],[DER(["i9"]),rec("c2","i9",origin="FITTED",ev_reason_code="ORIG_FIT_STATED",ev_source_file="paperB.txt",ev_source_line=5,ev_verbatim="we choose a = 2")], {"i1":full(DER(["i9"]))}),
]
...
def audit(tag, merged, extra_inputs, wrong_owner=False):
    ...
    for k in ("sealed_ledger_sha256",): art.pop(k,None)
    return rc, fails, art
results=[]; allok=True
for i,(name,A,B,aud) in enumerate(CONS):
    ...
    ok=same_bytes and same_outcome; allok&=ok; results.append((name,ok,same_bytes,same_outcome))
    v=[(o[1][0] if o[0]!="MERGE_FAIL" else "MERGE_FAIL") for o in outs]
    print(("ok  " if ok else "BAD ")+f"{name}: merged-bytes-equal={same_bytes} outcome-equal={same_outcome} verdict(AB,BA)={v}")
