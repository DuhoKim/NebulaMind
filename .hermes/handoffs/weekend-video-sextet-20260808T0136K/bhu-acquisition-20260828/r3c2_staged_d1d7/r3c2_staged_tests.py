...
def probe(name, tool, marker, *args, token, want_rc=0):
    ...
    def neut(l): return l.replace("fails.append(","_probe_noop(").replace("diffs.append(","_probe_noop(").replace("out_claims.update(","_probe_noop(").replace("disputed_claims.add(","_probe_noop(").replace("; sys.exit(1)","; pass").replace("; return 1","; pass").replace("PROBE:","PROBE-DELETED:")
...
rc,out,_=full_case("origin",AC,[A4],SC,[X4],lambda ids: {**rd_ok(ids),"c1":{"outcome":"REPRO_WITHIN_STATED_PRECISION","printed_value":"4","reproduced_value":"4","inputs":{"i1":dict(I1,origin="CHOSEN")}}}); check("D7 neg: re-classified origin differs from the sealed ledger",rc,out,1,["AUDIT c1: MISMATCH (input i1: origin CHOSEN vs sealed IMPORTED; root_origins differ: audit ['CHOSEN'] vs sealed ['IMPORTED'])"])
...
rc,out,_=full_case("fabev",AC,[A4],SC,[X4],lambda ids: {**rd_ok(ids),"c1":{"outcome":"REPRO_WITHIN_STATED_PRECISION","printed_value":"4","reproduced_value":"4","inputs":{"i1":dict(I1,origin_evidence=dict(I1["origin_evidence"],verbatim="fabricated"))}}}); check("gate-3 F1: a complete reconstruction whose evidence quotation differs is MISMATCH",rc,out,1,["AUDIT c1: MISMATCH (input i1: origin_evidence.verbatim fabricated vs sealed We adopt a = 2 from paperB (2020))"])
probe("gate-3 F1 probe: deleting the evidence comparison lets the fabricated quotation PASS",SEAT,"PROBE:C6_EVIDENCE","audit","compare",*_,W/"fabev_p.json",token="C6_AUDIT_SAMPLE=PASS")
...
rc,out,_=full_case("noborrow",AC,[A4],SC,[X4],lambda ids: {**rd_ok(ids),"c1":{"outcome":"REPRO_WITHIN_STATED_PRECISION","printed_value":"4","reproduced_value":"4","inputs":{"i1":dict(I1d)}}},sl_=sl3); check("gate-3 F1: a reconstruction that omits a claim's dependency (i2) is MISMATCH — the dependency is never borrowed from the sealed side",rc,out,1,["AUDIT c1: MISMATCH (input i2: not reconstructed; dependency not reconstructed by the auditor or cyclic: i1: derived_from i2 not in ledger)"])
