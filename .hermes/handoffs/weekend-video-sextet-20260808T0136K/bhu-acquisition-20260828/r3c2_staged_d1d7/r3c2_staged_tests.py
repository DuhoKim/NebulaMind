...
rc,out=run(SEAT,"validate",W/"d1_pos.json",S); check("D1 neg: validate without the candidate file cannot bind the claiming paper",rc,out,1,["needs the candidate file to bind claim paperA.txt#1"])
probe("D1 probe: deleting the candidate-file requirement turns that negative into PASS",SEAT,"PROBE:D1_NEEDS_CANDIDATES","validate",W/"d1_pos.json",S,token="C3_NO_SUBSTITUTION=PASS")
...
rc,out=run(SEAT,"validate",w("d1_n_verb.json",{"records":[rec(ev_source_line=2)]}),S,CANDS); check("D1 neg: verbatim not at the citing sentence",rc,out,1,["verbatim not found at citing sentence paperA.txt:2"])
probe("D1 probe: deleting the citing-sentence check turns that negative into PASS",SEAT,"PROBE:D1_VERBATIM_SITE","validate",W/"d1_n_verb.json",S,CANDS,token="C3_NO_SUBSTITUTION=PASS")
rc,out=run(SEAT,"validate",w("d1_n_empty.json",{"records":[rec(ev_verbatim="")]}),S,CANDS); check("D1 neg: empty quotation (codex)",rc,out,1,["empty verbatim quotation"])
probe("D1 probe: deleting the empty-quotation check turns that negative into PASS",SEAT,"PROBE:D1_EMPTY_VERBATIM","validate",W/"d1_n_empty.json",S,CANDS,token="C3_NO_SUBSTITUTION=PASS")
...
def rd_ok(ids, outc=None):
    return {cid:({"outcome":"REPRO_WITHIN_STATED_PRECISION","printed_value":"4","reproduced_value":"4","inputs":{"i1":"IMPORTED"}} if cid=="c1" else {"outcome":(outc or {}).get(cid,{"c2":"REPRO_NOT_EVALUABLE","c3":"REPRO_NO_DERIVATION_STATED"}.get(cid,"REPRO_NOT_EVALUABLE")),"inputs":{}}) for cid in ids}
...
def full_case(tag, aud_cands, aud_x, sealed_cands, sealed_x, rdfn=rd_ok):
    ...
    s1_,sel_,s2_,rd_,*_=stage(tag,ac_,ax_,sc_,rdfn); rc,out=run(SEAT,"audit","compare",s1_,ac_,ax_,sc_,sx_,sl,sel_,s2_,rd_,W/f"{tag}_C6.json"); return rc,out,(s1_,ac_,ax_,sc_,sx_,sl,sel_,s2_,rd_)
...
rc,out,_=full_case("mismatch",AC,[A4],SC,[X4],lambda ids: {**rd_ok(ids),"c1":{"outcome":"REPRO_FAILED","printed_value":"4","reproduced_value":"5","inputs":{"i1":"IMPORTED"}}}); check("D7 neg: re-derived outcome differs from the sealed record",rc,out,1,["AUDIT c1: MISMATCH (outcome REPRO_FAILED vs sealed REPRO_WITHIN_STATED_PRECISION; printed/reproduced values differ)"])
rc,out,_=full_case("origin",AC,[A4],SC,[X4],lambda ids: {**rd_ok(ids),"c1":{"outcome":"REPRO_WITHIN_STATED_PRECISION","printed_value":"4","reproduced_value":"4","inputs":{"i1":"CHOSEN"}}}); check("D7 neg: re-classified origin differs from the sealed ledger",rc,out,1,["AUDIT c1: MISMATCH (origin i1: CHOSEN vs sealed IMPORTED)"])
