...
C2 takes the seat's transcription of eq. 7.31 as JSON {coef_lo, coef_hi, T0, h0, Rstar, log_power, printed_value} and prints
the reproduced value beside the printed .019 — PASS only within two significant figures. SETTLED FROM THE PDF (page 58,
2026-09-06): eq. 7.31 is (2.62..2.65e-7) T0^4/(h0^2 H0^2) * ln(1/R*), the logarithm UNSQUARED; the extracted text's stray "2"s are
the squared h0 and H0. Eq. 7.29 prints a floor R* >= 2.2/4000 = 6.75e-4 and p.59 the admissible interval 2.7/4000 <= R* <= 1."""
...
ANCHORS = [(104, "parameters in the problem other than the experimentally determined values of the Hubble constant"),
           ...
           (231, "at which we start the shock"), (287, "For example, at T0 = 2.7"), (3372, "2.2/4000")]
def c1(src): c1_identity(src, ANCHORS, "C1_SOURCE_IDENTITY")
def evaluate(t):
    lnq = math.log(1.0/t["Rstar"])**t.get("log_power", 1)
...
def neg():
    t = {"coef_lo": 2.62e-7, "coef_hi": 2.65e-7, "T0": 2.736, "h0": 0.55, "Rstar": 2.7/4000, "printed_value": 0.019, "log_power": 2}
    lo, hi = evaluate(t); ok = round(lo, 2) != 0.019 and round(hi, 2) != 0.019
    chk("NEG planted ln^2(1/R*) for ln(1/R*): (S2) must fail", ok, f"planted value [{lo:.4g},{hi:.4g}] vs 0.019")
