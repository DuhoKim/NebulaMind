#!/usr/bin/env python3
"""Run per-card ASR and compare it with exact gated V7 narration."""
...
def norm(s):
    ...
    s=re.sub(r'\b1\.5\b','one point five',s)
    s=re.sub(r'\b1\.97\b','one point nine seven',s)
    s=re.sub(r'\b0\.04\b','zero point zero four',s)
    s=re.sub(r'\b2\.08\b','two point zero eight',s)
    s=re.sub(r'\b0\.07\b','zero point zero seven',s)
...
records=[]
for c in T['cards']:
    cid=c['card_id']; audio=BUILD/f'audio/cards/card-{cid}.wav'; out=BUILD/f'audio/cards/card-{cid}-asr.json'
    if not out.exists():
        p=subprocess.run([PY,str(BUILD/'transcribe.py'),str(audio),'--out',str(out)],capture_output=True,text=True,timeout=300)
        if p.returncode: raise RuntimeError(f'ASR {cid} failed: {p.stderr[-500:]}')
    tr=json.loads(out.read_text()).get('text','')
    ne,nt=norm(c['narration']),norm(tr); ratio=difflib.SequenceMatcher(None,ne,nt).ratio()
...
report={'status':'PASS' if all(r['status']=='PASS' for r in records) else 'HOLD','model':'whisper-1','scope':'decoded pre-encode master split at card boundaries','threshold':.93,'records':records}
(BUILD/'audio/raw_master_asr.json').write_text(json.dumps(report,indent=2,ensure_ascii=False)+'\n')
