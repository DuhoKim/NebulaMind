...
def instruction_policy(sentence: str) -> dict[str, str]:
    normalized = sentence.replace("–", "-").replace("—", "-")
    if "arXiv:1910.10819" in normalized:
        instructions = (
            BASE_INSTRUCTIONS
            + " Pronounce arXiv as archive. Read the identifier 1910.10819 digit-perfectly as "
            + "nineteen ten point one zero eight one nine; do not insert an extra 1."
        )
        version = "arxiv-identifier-retake-v1"
    elif "Saadeh bound" in normalized:
        instructions = BASE_INSTRUCTIONS + " Pronounce Saadeh carefully as SAH-deh, with two distinct syllables."
        version = "saadeh-pronunciation-retake-v1"
    elif "inflation-era rate" in normalized:
        instructions = (
            BASE_INSTRUCTIONS
            + " In the phrase inflation-era rate, pause slightly after inflation, then say ERA as EAR-uh, "
            + "not error. Enunciate the final D in pinned: PINNED, not pin."
        )
        version = "inflation-era-pinned-retake-v2"
    elif "galaxy spin-up" in normalized:
        instructions = BASE_INSTRUCTIONS + " Say galaxy spin-up without a possessive sound after galaxy."
        version = "galaxy-spin-up-retake-v1"
    elif "would need about 25 quadrillion" in normalized:
        instructions = BASE_INSTRUCTIONS + " Enunciate the word need clearly with a final D; do not say meet."
        version = "need-final-d-retake-v1"
    elif "Brown-Bethe" in normalized:
        instructions = (
            BASE_INSTRUCTIONS
...
def main() -> int:
    ...
    for panel in frozen["panels"]:
        ...
        for sentence_index, sentence in enumerate(pipeline.split_sentences(panel["narration"]), 1):
            ...
            policy = instruction_policy(sentence)
            if raw.exists():
    ...
    subprocess.run(
        [
            ...
            str(raw_master),
            "-af",
            f"loudnorm=I=-16:LRA=7:TP=-2.3,aresample={RATE},apad,atrim=end_sample={total_samples}",
            "-ar",
            str(RATE),
            "-ac",
            str(CHANNELS),
