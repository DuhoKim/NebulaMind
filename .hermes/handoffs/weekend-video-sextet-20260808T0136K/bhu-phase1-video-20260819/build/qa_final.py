...
FORBIDDEN_PHRASES = ["is falsified", "we proved", "theory is dead", "smolin"]
...
def normalize_words(text: str) -> list[str]:
    ...
    normalized = normalized.replace("brown lee row", "brown lee rho")
    normalized = normalized.replace("brown li and rowe", "brown lee and rho")
    normalized = normalized.replace("brown, li, and rowe", "brown, lee, and rho")
    normalized = normalized.replace("brownlee row", "brown lee rho")
    normalized = normalized.replace("brown beta", "brown bethe")
    normalized = normalized.replace("arkiv", "arxiv")
    normalized = normalized.replace("archive", "arxiv")
    normalized = normalized.replace("ar kiv", "arxiv")
    normalized = normalized.replace("sade", "saadeh")
    normalized = normalized.replace("backdoor", "back door")
    normalized = normalized.replace("spinup", "spin up")
    normalized = normalized.replace("1910 10819", "1910.10819")
    normalized = normalized.replace("1022", "10 to the power 22")
    normalized = normalized.replace("1030", "10 to the power 30")
    normalized = normalized.replace("inflation error rate", "inflation era rate")
    ...
    normalized = normalized.replace("ar kiv", "arxiv")
    normalized = normalized.replace("1910 10819", "1910.10819")
    normalized = re.sub(r"\bnebula mined\b", "nebulamind", normalized)
    ...
    normalized = re.sub(r"\bore\b", "or", normalized)
    ...
    normalized = re.sub(r"\bpercent\s+plus or minus\b", "plus or minus", normalized)
    ...
    for word, digit in (("one", "1"), ("two", "2"), ("three", "3"), ("four", "4"), ("five", "5"), ("eight", "8")):
...
def main() -> int:
    ...
    forbidden_affirmations = FORBIDDEN_PHRASES
    ...
    asr_report = {
        ...
        "normalization_policy": {
            "allowed": [
                ...
                "observed homophonic ASR spelling of the explicitly quoted logical word or",
                "astronomical identifier token spacing and re-measure word-boundary forms",
                "NebulaMind proper-name token boundary and mined homophone",
                "redundant percent token around a plus-or-minus expression and spoken 8",
                "arXiv archive/ARKIV phonetic rendering and Saadeh/Sadeh phonetic spelling",
                "closed/open compound tokenization such as back door/backdoor",
                "hyphen/punctuation tokenization in arXiv:1910.10819 and spin-up",
                "spoken exponent notation rendered as concatenated 1022/1030 tokens",
                "the twice-retaken, phrase-specific inflation era/error ASR homophone",
            ],
            "forbidden": [
                "negation removal",
    ...
    forbidden_corpus = "\n".join(
        [record["effective_transcript"] for record in asr_records]
        + srt_payloads
        + vtt_payloads
        + [text for card in card_audit["cards"] for text in card["emitted_text"]]
    ).lower()
    forbidden_sweep_hits = [phrase for phrase in FORBIDDEN_PHRASES if phrase in forbidden_corpus]
    forbidden_sweep_status = "PASS_FORBIDDEN_SWEEP" if not forbidden_sweep_hits else "HOLD_FORBIDDEN_SWEEP"
    ...
    overall_status = (
        "PASS_LOCAL_RENDER_QA_READY_FOR_MIRU_REVIEW"
        ...
        and forbidden_sweep_status == "PASS_FORBIDDEN_SWEEP"
    ...
    report = {
        ...
        "forbidden_sweep_status": forbidden_sweep_status,
        "forbidden_sweep_phrases": FORBIDDEN_PHRASES,
        "forbidden_sweep_hits": forbidden_sweep_hits,
