...
class FrozenInputTests(unittest.TestCase):
    def test_load_frozen_inputs_reconstructs_script_and_passed_gate(self):
        ...
        self.assertEqual(
            ...
            "befcce2b46d9da37721a1dc78344ccbe5a7925643a53fde6141ab5b95390ae70",
        )
    ...
    def test_sentence_split_preserves_exact_text_and_quoted_period_boundary(self):
        ...
        self.assertIn("This is Duho's personal side-interest, not a NebulaMind research programme.", sentences)
        self.assertIn("The paper's dark-energy route fails its own numbers twice.", sentences)


class CardRendererTests(unittest.TestCase):
    def test_rendered_cards_use_exact_closed_world_text_and_honest_ladder_geometry(self):
        import render_cards
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(geometry["panel_03"]["horn_2_label"], "HORN 2: < 10⁻¹⁸ × DARK ENERGY")
            self.assertEqual(geometry["panel_08"]["required_universe_tiles"], 12000)
            self.assertFalse(geometry["panel_03"]["unlabeled_log_spacing_used"])
...
class AudioTimelineTests(unittest.TestCase):
    def test_timeline_reconstructs_exact_narration_on_fixed_card_grid(self):
        ...
        self.assertEqual(timeline["master_duration_seconds"], 322.0)
        self.assertEqual(timeline["master_sample_count"], 322 * 48000)
...
class AssemblyContractTests(unittest.TestCase):
    def test_stream_validator_requires_video_audio_and_default_english_subtitle(self):
        ...
        probe = {
            "streams": [
                {"codec_type": "video", "codec_name": "h264", "width": 1920, "height": 1080, "r_frame_rate": "30/1", "nb_frames": "9660"},
            ...
            "format": {"duration": "322.000000"},
        }
        assemble.validate_media_contract(probe, 322 * 30, 322.0)
...
class AsrDiffTests(unittest.TestCase):
    def test_asr_normalization_allows_numeric_form_but_never_erases_negation(self):
        ...
        expected = "The theory is not refuted with 36 trillion galaxies and 0.74 standard deviations."
        spoken = "The theory is not refuted with thirty six trillion galaxies and zero point seven four standard deviations."
        self.assertEqual(qa_final.normalize_words(expected), qa_final.normalize_words(spoken))
        altered = "The theory is refuted with thirty six trillion galaxies and zero point seven four standard deviations."
    ...
    def test_phase1_phrase_specific_asr_forms_are_declared_without_erasing_other_errors(self):
        import qa_final

        self.assertEqual(
            qa_final.normalize_words("The inflation-era rate has no pinned source."),
            qa_final.normalize_words("The inflation error rate has no pinned source."),
        )
        self.assertNotEqual(
            qa_final.normalize_words("The era is explicit."),
            qa_final.normalize_words("The error is explicit."),
        )
