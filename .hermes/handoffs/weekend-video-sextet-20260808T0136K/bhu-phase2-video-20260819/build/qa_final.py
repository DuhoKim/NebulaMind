#!/usr/bin/env python3
"""Full final-MP4 ASR, visual, caption, and media QA for the Phase-2 render."""
...
CONTRACT_PHRASES = {
    "01": [
        "Under the published homogeneous-bounce reading, the strongest defensible ceiling leaves the best sky signal roughly 10,000 to 100,000 times below even counting every observable galaxy.",
        "The inheritance route now exists on the record as a ceiling, and it says the route stays closed: nothing observable.",
    ],
    "05": [
        "1 honest caveat: the erratum's content remains unread behind a paywall, so our recomputations stand in its place, and the strict chain awaits external theorist review before any publication claim.",
    ],
    "07": [
        "If all parent angular momentum reached the bounce, rotation would overshoot the causal limit by a factor of 6.6 times 10 to the power 26.",
        "Self-consistency leaves at most about 1 part in 10 to the power 27, with the treatment branches spanning roughly 1 order of magnitude.",
        "Under Reading 2, rotating collapse never reaches this homogeneous solution at all; what happens instead remains underived.",
    ],
    "10": [
        "We did not prove BHU cosmology false, and we did not test the wider family.",
        "Asked through this published chain, the axis question answers: nothing observable.",
        "The strongest inheritance route now exists on the record as a ceiling, and the ceiling says the route stays closed.",
    ],
}
...
NUMBER_PHRASES = [
    ...
    ("one hundred and thousand", "100000"),
    ("ten thousand", "10000"),
...
FORBIDDEN_PHRASES = ["bhu is false", "bhu is impossible", "we proved the idea wrong", "smolin refuted"]
COSMETIC_WORDS = {"a", "an", "the", "s"}


def normalize_words(text: str) -> list[str]:
    normalized = unicodedata.normalize("NFKD", text).lower().replace("ł", "l")
    ...
    normalized = re.sub(r"\bfermion\s+intrinsic\s+spin\b", "fermions intrinsic spin", normalized)
    normalized = re.sub(r"\bnuclear\s+synthesis\b", "nucleosynthesis", normalized)
...
def judge_mismatches(panel_id: str, expected: str, differences: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for difference in differences:
        ...
        expected_tokens = difference["expected"]
        transcript_tokens = difference["transcript"]
        touches_contract = any(index in protected for index in range(i1, max(i1 + 1, i2)))
        proper_name_phonetic = expected_tokens == ["poplawski"] and transcript_tokens in (["poplovsky"], ["popovsky"])
        cosmetic = not touches_contract and (set(expected_tokens + transcript_tokens).issubset(COSMETIC_WORDS) or proper_name_phonetic)
        if proper_name_phonetic:
            reason = "observed phonetic ASR rendering of the paper-chain proper name; no claim, number, caveat, Reading clause, or verdict word changed"
        elif cosmetic:
            reason = "minor function-word ASR variance outside protected phrases"
        else:
            reason = "number, exact caveat/Reading/verdict phrase, or substantive narration word"
...
def multipart(audio: Path) -> tuple[bytes, str]:
    boundary = "----HermesBHUPhase2" + secrets.token_hex(12)
...
def decoded_frame_rms(source: Path, decoded: Path) -> float:
    left = Image.open(source).convert("RGB").crop((80, 35, 1840, 245))
    right = Image.open(decoded).convert("RGB").crop((80, 35, 1840, 245))
...
def main() -> int:
    ...
    for panel, card in zip(frozen["panels"], timeline["cards"]):
        ...
        asr_start = max(card["start_seconds"], card["speech_start_seconds"] - 0.08)
        asr_end = min(card["end_seconds"], card["speech_end_seconds"] + 0.15)
        run(["ffmpeg", "-y", "-hide_banner", "-loglevel", "error", "-i", str(decoded_audio), "-ss", f"{asr_start:.6f}", "-to", f"{asr_end:.6f}", "-ar", "48000", "-ac", "1", "-c:a", "pcm_s16le", str(segment)])
        ...
        # Exact protected phrases must survive as contiguous normalized word sequences.
        phrase_checks = []
    ...
    asr_report = {
        ...
        "normalization_policy": ["case/punctuation", "curly/straight apostrophes", "hyphen tokenization", "UK/US programme spelling", "digit/spoken-number forms", "NebulaMind token boundary", "Popławski diacritic", "fermions possessive/plural homophone before intrinsic spin", "observed nucleosynthesis/nuclear-synthesis ASR word boundary"],
    ...
    markdown = [
        "# ASR QA — BHU Phase 2 results video", "",
        f"Status: `{asr_status}`", f"Final MP4 SHA-256: `{candidate_hash}`", f"Gateway ASR model: `{ASR_MODEL}`", "",
        "Every panel below was transcribed from audio decoded from the exact final MP4. Word errors are computed after the declared representational normalization only. Any number, exact caveat sentence, Reading-1 clause, verdict, negation, or substantive narration change is contract-bearing; contract-bearing residuals are not accepted.", "",
    ...
    for record in records:
        ...
        markdown.append("")
    markdown.extend(["## Final judgment", "", f"Cosmetic residual mismatches: {json.dumps(all_cosmetic_residuals, ensure_ascii=False) if all_cosmetic_residuals else 'none' }", "", f"Contract-bearing residual mismatches: {json.dumps(all_contract_residuals, ensure_ascii=False) if all_contract_residuals else 'none — acceptable for freeze'}", ""])
    pipeline.ASR_QA.write_text("\n".join(markdown), encoding="utf-8")

    # Visual and caption checks against decoded final media.
    frame_records = []
    contact_images = []
    for panel, card, audit in zip(frozen["panels"], timeline["cards"], card_audit["cards"]):
        frame = pipeline.BUILD / f"_tmp_frame-{panel['id']}.png"
        run(["ffmpeg", "-y", "-hide_banner", "-loglevel", "error", "-ss", f"{card['start_seconds'] + 1.0:.6f}", "-i", str(candidate), "-frames:v", "1", str(frame)])
        rms = decoded_frame_rms(pipeline.BUILD / audit["path"], frame)
        exact_heading = audit["heading"] == panel["assertion_heading"] == panel["viewer_text_closed_world"][0]
        frame_records.append({"panel_id": panel["id"], "heading_exact": exact_heading, "heading_crop_rms": rms, "decoded_pixels_match": rms < 20.0, "status": "PASS" if exact_heading and rms < 20.0 else "HOLD"})
        contact_images.append(Image.open(frame).convert("RGB").copy())
        frame.unlink(missing_ok=True)
    sheet = Image.new("RGB", (1920, 810), (7, 13, 25))
    ...
    contact = pipeline.BUILD / "qa/decoded-contact-sheet.png"
    sheet.save(contact, format="PNG", optimize=False, compress_level=9)
    heading_status = "PASS_EXACT_ASSERTION_HEADINGS_AND_DECODED_PIXELS" if all(record["status"] == "PASS" for record in frame_records) else "HOLD_DECODED_FRAME_QA"
    ...
    geometry_status = "PASS_LABELED_MAGNITUDE_GEOMETRY_AND_PLANCK_MARKERS" if (
        geometry["panel_03"]["linear_ladder_rungs"] == 730 and geometry["panel_03"]["planck_caveat_markers"] == 2 and
        geometry["panel_04"]["linear_intervals"] == 6 and geometry["panel_07"]["order_steps"] == 27 and
        geometry["panel_09"]["bbn_order_steps"] == 45 and not geometry["panel_09"]["unlabeled_log_compression"]
    ...
    overall = "PASS_LOCAL_RENDER_QA_READY_FOR_KIMI_REVIEW" if all([
        ...
        equation_status.startswith("PASS"), geometry_status.startswith("PASS"), decode.returncode == 0,
    ...
    print(json.dumps({key: report[key] for key in ("status", "candidate_sha256", "duration_seconds", "asr_status", "contract_bearing_residual_count", "cosmetic_residual_count", "heading_status", "caption_status", "equation_status", "geometry_status")}, ensure_ascii=False))
