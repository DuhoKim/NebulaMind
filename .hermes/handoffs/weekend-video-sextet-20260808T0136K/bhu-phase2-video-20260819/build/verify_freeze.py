#!/usr/bin/env python3
"""Verify candidate, narration WAV, QA, and gated-input hashes from FREEZE.json."""
...
def main() -> int:
    ...
    if manifest["status"] != "FROZEN_LOCAL_ONLY_READY_FOR_KIMI_RENDER_GATE":
    ...
    if pipeline.DONE.read_text(encoding="utf-8").splitlines()[0] != "GPT3_P2V_COMPLETE":
        raise RuntimeError("GPT3_DONE.md completion token mismatch")
    print(json.dumps({"status": "PASS_FROZEN_PROVENANCE_VERIFIED", "candidate_sha256": manifest["candidate"]["sha256"], "narration_wavs": len(manifest["narration_wavs"]), "gated_inputs": len(manifest["gated_inputs"])}))
    return 0
