...
23–27 ASSUMED (packet 01:00: whether ruling A(a) reaches the warrant axis); prospects 21/47 ROUTE_CONNECTED, 9/16
...
**Warrant (2026-09-03, corpus-wide warrant audit; `bhu-acquisition-20260828/WARRANT_TABLE_20260903.md`):** `W_DIRECTION_ASSUMED` (PACKET — joined to the 23–27 question; the sign is the causal premise restated) — borrowed inputs: finite-cloud BHU background from Part I (SOURCE lines 75–79); gravitational-instability results [32–34] (SOURCE lines 987–1010); measured cutoff/anomaly literature [12,35,39–43] (SOURCE lines 1010, 1513–1516) Tier untouched pending Duho.
**Phase (b) map (2026-09-02; ruling delegated by Duho in direct chat — "i'm not the expert on this area so please ask to as various as models possible and decide on your own" — four-model blind poll codex/agy/kimi/claude-seat all `VOTE_A`; Tori's ruling `bhu-acquisition-20260828/INTERP_RULING_23_27_20260902.md`):** The 60° causal-horizon cutoff was tested like-for-like on the masked Planck SMICA map (pre-registered, controls passed, blind-doubled; `bhu-acquisition-20260828/PROGRAM_A_FREEDOM_MAP_20260902.md` §§1–2, 7, 9; `PHASEB_RESULT_RECONCILIATION_20260902.md`). **What the papers fix:** the angular scale, non-circularly — χ_§ from Ω_Λ measured by SNe/BAO, not from the correlation under test; 57.4°, rounded by the paper to 60±3 (the large-angle deficit itself was known before these papers, so this is a non-circular derivation, not an out-of-sample forecast). **What the papers do not fix:** the amplitude — **NOT CALIBRATABLE from the papers** (supersedes "NOT yet calibrated" in the RQ-C map): no perturbation prescription (READING_C ×3 + CLASS_REFUTED), and the author states that no initial-conditions model exists (2003.11544 L466, in context about observer-to-observer differences in the causal boundary). Under lane-built completions — exploratory, not calibration; non-exhaustive; amplitudes not attributable to the theory — every completion moved the observed-sky percentile upward in the predicted direction, from 0.15–0.2% (ΛCDM) to at best 2.2–2.8% (Reading A, 2π/χ_§), roughly 15×, and none lifts the observed sky above the ~3% level. Standing objection to the direction itself, carried open: absence of causal contact does not by itself imply zero correlation (common initial conditions can correlate disconnected regions). Open route — CLOSED 2026-09-02 (Program (C), ordered by Duho, `bhu-acquisition-20260828/PROGRAM_C_FLUX_RESULT_20260902.md`, four seats blind-agreed + numeric control): the paper's own Φ(χ>χ_§)=0 flux condition, imposed on the perturbed solution, constrains only the sky monopole when applied to our causal region (C_ℓ for ℓ ≥ 1 and S₁/₂ exactly unchanged) and admits no continuous power spectrum when applied to every observer; it is not the missing amplitude prescription. **Tier UNCHANGED (QUALITATIVE-DIRECTIONAL):** direction and scale a priori, amplitude unscorable; any threshold would be the lane's, recorded in the Program (A) write-up, not here.
**RQ-C map (BHU Lane 2, 2026-08-31; Duho ruling q3 = annotate, keep tier):** The causal-horizon power-spectrum cutoff is a PREDICTED (out-of-sample), Planck-consistent, **scale-level CMB falsifier CANDIDATE** -- l_cut = 3 / theta = 60 deg, fixed from background H0/Omega_Lambda (r_S=2GM), no free parameter, published BEFORE the CMB analysis; consistent with Planck's low quadrupole + the >60 deg S_1/2 large-angle deficit (both seats agree). NOT yet calibrated: the papers supply no cutoff amplitude / C_l threshold (the model owns that missing proof). **Tier UNCHANGED (QUALITATIVE-DIRECTIONAL).** See `RQ_C_RECONCILIATION_20260831.md`.
...
**Warrant (2026-09-03, corpus-wide warrant audit; `bhu-acquisition-20260828/WARRANT_TABLE_20260903.md`):** `W_DIRECTION_ASSUMED` (PACKET — joined to the 23–27 question; the sign is the causal premise restated) — borrowed inputs: Ωm, ΩΛ, and H0 background normalization (SOURCE lines 210–224); observed CMB homogeneity scale ΘH = 66 ± 9° (SOURCE lines 302–303) Tier untouched pending Duho.
