# Brief: blind independent computation of Phase 5b P5 (seat: gpt1)
...
## ADDITIONAL for P5 (this supersedes the P2b version above where they conflict)

Compute the bound across the FULL opacity range, tau from 0 to saturation (>= 20), not one
value. The exterior may be transparent OR opaque within the authorised physics; your answer
must cover both. Decide for yourself, and justify:
 - whether the exterior's own thermal emission is Doppler-shifted on crossing, and what that
   implies for the angular pattern at high opacity;
 - whether the anisotropy survives, weakens, or vanishes as the exterior becomes opaque, and
   whether any limit exists;
 - the normaliser for the offset (state which radius and why).
Report the dipole coefficient and bound as functions of tau, and say plainly whether opacity
can hide the offset.
