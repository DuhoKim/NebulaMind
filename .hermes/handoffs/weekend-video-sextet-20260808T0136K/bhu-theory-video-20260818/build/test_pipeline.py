...
class FrozenInputTests(unittest.TestCase):
    def test_load_frozen_inputs_reconstructs_script_and_passed_gate(self):
        ...
        self.assertEqual(
            ...
            "fdfa1cccffa27b77730ac43e01bfbd8e925103cc4a0428632871866ca72db788",
        )
    ...
    def test_sentence_split_preserves_exact_text_and_quoted_period_boundary(self):
        ...
        self.assertIn("Could a spinning parent black hole leave our universe turning?", sentences)
        self.assertIn("This does not kill the idea.", sentences)
...
class CardRendererTests(unittest.TestCase):
    def test_rendered_cards_use_exact_closed_world_text_and_honest_ladder_geometry(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(geometry["tenfold_tick_count"], 6)
            self.assertEqual(geometry["explicit_tenfold_step_blocks"], 4)
            self.assertEqual(geometry["needed_tick_power"], -2)
            self.assertEqual(geometry["allowed_between_tick_powers"], [-6, -7])
            self.assertFalse(geometry["unlabeled_log_spacing_used"])
...
class AudioTimelineTests(unittest.TestCase):
    def test_timeline_reconstructs_exact_narration_on_fixed_card_grid(self):
        ...
        self.assertEqual(timeline["master_duration_seconds"], 309.0)
        self.assertEqual(timeline["master_sample_count"], 309 * 48000)
...
class AssemblyContractTests(unittest.TestCase):
    def test_stream_validator_requires_video_audio_and_default_english_subtitle(self):
        ...
        probe = {
            "streams": [
                {"codec_type": "video", "codec_name": "h264", "width": 1920, "height": 1080, "r_frame_rate": "30/1", "nb_frames": "9270"},
