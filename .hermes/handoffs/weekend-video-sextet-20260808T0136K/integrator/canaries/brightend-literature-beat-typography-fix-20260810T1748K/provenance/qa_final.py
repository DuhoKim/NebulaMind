...
def normalize(text: str) -> str:
    text = text.lower().replace("(y-int)", "y intercept").replace("y-int", "y intercept").replace("0.7", "zero point seven").replace("∼", " approximately ").replace("λcdm", "lambda c d m").replace("lambda cdm", "lambda c d m").replace("7-10", "seven to ten").replace("7 to 10", "seven to ten")
...
def transcribe_opening(video: Path, records: list[dict]) -> dict:
    ...
    if any(record["id"] == "i05q" for record in opening):
        ...
        semantic_terms = ["jwst", "high redshift", "stellar masses", "massive galaxy candidates", "unresolved", "observations", "lambda c d m", "neither claim", "explanation", "finding"]
...
def main() -> int:
    ...
    for record in timeline["records"]:
        size = record.get("params", {}).get("caption_font_size", 18 if record["section"] == "literature" and len(record["text"]) > 190 else 24 if record["section"] == "literature" else 31)
        fnt = renderer.font(size, bold=record["section"] != "literature", serif=record["section"] == "literature", serif_symbol=record.get("params", {}).get("serif_font") == "STIX Two Math")
