    ...
    for index in range(frames):
        ...
        if previous is not None:
            ...
            if diff < 0.08:
                ...
                current = 0.0
        previous = array
    return {"sample_rate_fps": rate, "sampled_frames": frames, "mean_absolute_frame_difference": float(np.mean(diffs)), "max_absolute_frame_difference": float(np.max(diffs)), "longest_near_unchanged_seconds": longest, "threshold": 0.08}
...
def transcribe_opening(video: Path, records: list[dict]) -> dict:
    ...
    report = {"status": "PASS" if similarity >= 0.86 else "HOLD", "provider_route": "Hermes managed OpenAI audio gateway", "model": "whisper-1", "start_seconds": start, "end_seconds": end, "expected": expected, "transcript": transcript, "similarity": similarity, "audio_sha256": sha256(wav)}
