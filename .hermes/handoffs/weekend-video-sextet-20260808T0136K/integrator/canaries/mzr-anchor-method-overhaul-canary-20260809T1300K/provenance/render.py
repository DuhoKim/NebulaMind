...
class Renderer:
    ...
    def chrome(self,draw,t,section):
        ...
        scan_left=max(xs[0],scan_start); scan_right=min(xs[-1],scan_start+180)
    ...
    def icon(self,draw,kind,x,y,color,q=1.0,scale=1.0):
        ...
        if kind=="archive":
            ...
            draw.line((x-100,y+55,x-100,y-60,x+110,y+55),fill=c,width=3)
            pts=[]
            for i in range(40):
                xx=x-90+i*5; yy=y+30-55*math.sin(i/39*1.25)
                pts.append((xx,yy))
            draw.line(pts,fill=c,width=7)
        elif kind=="plane":
    ...
    def peak_curve(self,draw,r,p,t):
        P=r["params"]; self.title(draw,P["heading"])
        x0,y0,x1,y1=300,760,1570,260; draw.line((x0,y0,x0,y1,x1,y1),fill=MUTED,width=4)
        center_text(draw,P["x_label"],((x0+x1)/2,815),25,MUTED,bold=True); center_text(draw,P["y_label"],(180,510),25,MUTED,bold=True)
        pts1=[]; pts2=[]
        for i in range(101):
            q=i/100; x=x0+q*(x1-x0); pts1.append((x,y0-120-210*q)); pts2.append((x,y0-285+95*math.sin(q*math.pi)))
        draw.line(pts1,fill=BLUE,width=8); draw.line(pts2,fill=PURPLE,width=8)
        center_text(draw,P["curve_a"],(1390,390),25,BLUE,bold=True); center_text(draw,P["curve_b"],(1320,520),25,PURPLE,bold=True)
        sx=x0+(x1-x0)*((t*.09)%1.0); draw.line((sx,y1,sx,y0),fill=AMBER,width=5)
        badge(draw,(560,195,1310,255),"CONCEPTUAL SWEEP · VALUES WITHHELD",AMBER,24)
        rounded(draw,(1610,305,1865,710),22,PANEL2,CYAN,3); wrapped(draw,P["outcome"],(1640,345,1835,680),25,WHITE,bold=True,max_lines=7)
    ...
    def peak_plane(self,draw,r,p,t):
        ...
        x0,y0,x1,y1=1260,735,1840,275; draw.line((x0,y0,x0,y1,x1,y1),fill=MUTED,width=4)
        ...
        badge(draw,(1300,195,1800,255),"SCHEMATIC · NO DATA POINTS",AMBER,22)
        q=(t*.22)%1.0; tx=1260+q*560; ty=735-q*360; draw.ellipse((tx-12,ty-12,tx+12,ty+12),fill=CYAN)
