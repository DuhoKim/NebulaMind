...
def main() -> int:
    ...
    gain = min(-20.3 - float(before["input_i"]), -2.3 - float(before["input_tp"]))
    master = audio / "narration_master.wav"
    subprocess.run(
        ["ffmpeg", "-hide_banner", "-loglevel", "error", "-y", "-i", str(raw_master), "-af", f"volume={gain:.3f}dB", "-c:a", "pcm_s16le", str(master)],
        check=True,
    )
