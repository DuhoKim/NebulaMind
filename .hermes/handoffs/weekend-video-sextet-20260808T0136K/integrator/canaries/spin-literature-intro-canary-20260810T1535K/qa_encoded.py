...
VIDEO = ROOT / "spin-literature-intro-canary-20260810T1535K.mp4"
PREDECESSOR = ROOT.parents[1] / "canaries/spin-method-overhaul-canary-20260809T2340K"
...
OUT = ROOT / "encoded_qa"
FRAMES = OUT / "frames"
CONTACT = ROOT / "encoded-contact-sheet-v5.jpg"
REPORT = ROOT / "encoded_qa.json"
OCR_PATH = OUT / "ocr.txt"
ASR_REPORT = OUT / "encoded-why-study-introduction-transcription-v5.json"
...
def main() -> int:
    ...
    probe = json.loads(
        run(
            [
                ...
                "-count_frames",
                "-show_entries",
                "format=duration,size,bit_rate:stream=index,codec_name,codec_type,width,height,r_frame_rate,avg_frame_rate,sample_rate,channels,bit_rate,duration,nb_frames,nb_read_frames",
                "-of",
                "json",
                str(VIDEO),
            ]
    ...
    encoded_frame_count = int(video_streams[0]["nb_read_frames"])
    encoded_video_duration = float(video_streams[0]["duration"])
    raw_frame_count = math.ceil(timeline["master_duration_seconds"] * 30)
    shortest_delta = timeline["master_duration_seconds"] - encoded_video_duration
    frame_stats = []
    for label, time_seconds, path in items:
        with Image.open(path) as image:
    ...
    opening = script["sentences"][: len(opening_ids)]
    ...
    opening_by_id = {item["id"]: item for item in opening}
    ...
    checks = {
        ...
        "raw_frame_count_matches_ceil_audio_times_fps": build["raw_frames_submitted"] == raw_frame_count,
        "encoded_frame_count_matches_independent_decode": build["encoded_video_frames"] == encoded_frame_count == raw_frame_count - 1,
        "legacy_frame_count_field_is_encoded_count": build["frame_count"] == encoded_frame_count,
        "shortest_tail_delta_is_nonnegative_under_one_frame": 0 <= shortest_delta < 1 / 30,
        ...
        "all_sentence_frames_nonblack": len(frame_stats) >= len(script["sentences"]) and all(item["mean_luma"] > 8 for item in frame_stats[: len(script["sentences"])]),
        "five_beat_introduction_is_first": [item["id"] for item in opening] == opening_ids,
        ...
        "three_literature_sentences_verbatim_and_attributed": all(
            opening_by_id[quote_id]["text"] == attribution_prefixes[quote_id] + quote
            for quote_id, quote in literature_quotes.items()
        ),
        "land_null_not_presented_as_settled": "remains disputed" in opening_by_id["i05u"]["text"].lower() and "adopts none" in opening_by_id["i05u"]["text"].lower(),
        "broad_reason_precedes_sorting_method": opening[0]["beat"] == "expectation" and opening[-1]["beat"] == "catch-and-handoff",
        "method_spine_script_is_byte_semantically_unchanged": current_method == predecessor_method,
        ...
        "scientific_unicode_font_is_stix_two_text": 'UNICODE_PATH = "/System/Library/Fonts/Supplemental/STIXTwoText.ttf"' in renderer_source,
        ...
        "encoded_why_study_intro_asr_intelligible": asr["status"] == "PASS" and asr["normalized_similarity"] >= 0.84 and all(asr["checks"].values()),
        ...
        "predecessor_mp4_preserved": sha256(PREDECESSOR / "spin-method-overhaul-canary-20260809T2340K.mp4") == "4d230cc0efca0eb68a8d027d614b6b7e500590cff06154f1514d4402a84d7078",
        "predecessor_script_v4_preserved": sha256(PREDECESSOR / "narration_script_v4.json") == "5df1d0a20e1feede746a82cd784ecd43c5cd1f21ebcc74d5418cbb87d69e90f1",
    }
    report = {
        "candidate": ROOT.name,
        ...
        "frame_count_derivation": {
            "raw_frames_submitted": raw_frame_count,
            ...
            "one_frame_seconds": 1 / 30,
            "ffmpeg_shortest": True
        },
        "loudness": loudness,
        "stability": stability,
        "motion": motion,
