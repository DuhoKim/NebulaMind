#!/usr/bin/env python3
"""Decode sentence masters, derive the real audio timeline, and assemble the narration master.

The timeline is computed from decoded PCM sample counts, not from word estimates. Inter-sentence
pauses are then solved to the signed 115 delivered WPM target. A scalar loudness gain preserves exact
...
SRT = ROOT / "spin-method-overhaul-canary-20260808T1312K.srt"
SAMPLE_RATE = 48_000
TARGET_WPM = 115.0
LEAD_SECONDS = 0.6
TAIL_SECONDS = 2.4
BASE_PAUSE_SECONDS = 0.9
SECTION_GAPS_AFTER = {"s03", "s05", "s06", "s10", "s13", "s15", "s18"}
...
def main() -> int:
    ...
    section_gaps = [i for i, (record, _, _) in enumerate(sentence_pcm[:-1]) if record["id"] in SECTION_GAPS_AFTER]
    ...
    assert sum(pause_samples) == pause_total_samples
            ...
            timeline_records.append(
                {
                    ...
                    "decoded_file": str((decoded_dir / f"{record['id']}.wav").relative_to(ROOT)),
                    "decoded_file_sha256": sha256(decoded_dir / f"{record['id']}.wav"),
