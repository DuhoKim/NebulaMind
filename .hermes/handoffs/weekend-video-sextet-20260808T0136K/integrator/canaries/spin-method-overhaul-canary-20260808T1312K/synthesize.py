#!/usr/bin/env python3
"""Synthesize the signed narration one sentence per managed OpenAI TTS call.

This file is self-contained inside the canary. It resolves the managed gateway at runtime,
requests the explicit Alloy voice at speed 1.18, writes content-addressed MP3 masters, probes every
file, and records no token or secret.
"""
...
SCRIPT_PATH = ROOT / "narration_script.json"
...
INSTRUCTIONS = (
    ...
    "Use deliberate phrasing without sounding dramatic or implying a result."
)
        ...
        if not output.exists():
            ...
            for attempt in (1, 2):
                try:
                    ...
                    last_error = None
                    break
                except Exception as exc:  # network/provider boundary; preserve exact error
                    last_error = exc
