#!/usr/bin/env python3
"""Render the introduction-corrected spin-method conference-science overhaul.

The renderer is isolated inside the versioned canary. Storyboard previews may use explicitly
provisional timing while audio is blocked; encoding requires the final v3 PCM-derived timeline.
"""
...
TIMELINE_PATH = ROOT / "audio_v3" / "timeline.json"
AUDIO_PATH = ROOT / "audio_v3" / "narration_master.wav"
STORYBOARD_PATH = ROOT / "storyboard_v3.json"
OUTPUT_PATH = ROOT / "spin-method-overhaul-canary-20260808T1959K.mp4"
RECEIPT_PATH = ROOT / "build_receipt.json"
PREVIEW_PATH = ROOT / "final-timing-contact-sheet-v3.jpg"
QA_FRAME_DIR = ROOT / "qa_frames" / "sentence-mid-v3"
...
def global_chrome(draw: ImageDraw.ImageDraw, t: float, section: str) -> None:
    ...
    stages = [
        ("QUESTION", {"motivation", "question", "two-worlds"}),
...
def draw_motivation(draw: ImageDraw.ImageDraw, sid: str, p: float, t: float) -> None:
    """Graphics-first four-move introduction: handedness, universe, sorters, question."""
    center_text(draw, "GALAXY SPIN HANDEDNESS", (W / 2, 135), 52, WHITE)
    center_text(draw, "WHY THE MIRROR TEST MATTERS", (W / 2, 183), 27, AMBER)

    spiral_q = seg(p, 0.02, 0.65) if sid == "i01" else 1.0
    draw_spiral(draw, 540, 380, 165, 1.0, mix(BG, BLUE, spiral_q), 8)
    draw_spiral(draw, 1380, 380, 165, -1.0, mix(BG, PURPLE, spiral_q), 8)
    center_text(draw, "CLOCKWISE · CW", (540, 555), 30, mix(BG, BLUE, spiral_q))
    center_text(draw, "ANTICLOCKWISE · ACW", (1380, 555), 30, mix(BG, PURPLE, spiral_q))
    badge(draw, (720, 290, 1200, 365), "TWO HANDEDNESSES", CYAN, 31)

    left_q = 0.16
    right_q = 0.16
    if sid == "i02":
        left_q = seg(p, 0.02, 0.70)
    elif sid == "i03":
        left_q, right_q = 0.55, seg(p, 0.02, 0.70)
    elif sid == "i04":
        left_q = right_q = 0.88

    left_box = (90, 630, 750, 845)
    right_box = (1170, 630, 1830, 845)
    rounded(draw, left_box, 27, PANEL, mix(GRID, BLUE, left_q), 4)
    rounded(draw, right_box, 27, PANEL, mix(GRID, PURPLE, right_q), 4)

    for i in range(15):
        x = 125 + (i * 173) % 575
        y = 660 + (i * 71) % 145
        r = 2 if i % 3 else 3
        draw.ellipse((x - r, y - r, x + r, y + r), fill=mix(BG, BLUE, left_q))
    center_text(draw, "IF GENUINE", (420, 670), 25, mix(BG, AMBER, left_q))
    center_text(draw, "IMAGE-LINKED · SKY", (420, 720), 30, mix(BG, BLUE, left_q))
    center_text(draw, "FACT ABOUT THE UNIVERSE", (420, 785), 26, mix(BG, WHITE, left_q))

    rounded(draw, (1275, 657, 1725, 730), 15, mix(BG, PURPLE, 0.10 * right_q), mix(GRID, PURPLE, right_q), 3)
    badge(draw, (1320, 670, 1470, 718), "CW", mix(GRID, PURPLE, right_q), 20)
    badge(draw, (1530, 670, 1680, 718), "ACW", mix(GRID, PURPLE, right_q), 20)
    center_text(draw, "COULD ARISE IN SORTING", (1500, 765), 25, mix(BG, AMBER, right_q))
    center_text(draw, "FACT ABOUT THE SORTERS", (1500, 812), 26, mix(BG, WHITE, right_q))

    if sid == "i04":
        q = 0.72 + 0.28 * seg(p, 0.10, 0.78)
        arrow(draw, (750, 740), (815, 740), BLUE, 1.0, 4)
        arrow(draw, (1170, 740), (1105, 740), PURPLE, 1.0, 4)
        flow_dots(draw, (755, 740), (810, 740), t, BLUE, 2)
        flow_dots(draw, (1165, 740), (1110, 740), t, PURPLE, 2)
        rounded(draw, (805, 650, 1115, 825), 26, PANEL2, mix(GRID, CYAN, q), 4)
        wrapped_text(draw, "HOW DO WE TELL THEM APART?", (835, 675, 1085, 800), 29, mix(BG, WHITE, q))
...
def render() -> None:
    if TIMING_AUTHORITY != "PCM_DERIVED_FINAL" or not AUDIO_PATH.exists():
        raise RuntimeError(
            "BLOCKED_BEFORE_SYNTHESIS: encoding requires audio_v3/timeline.json and "
            "audio_v3/narration_master.wav derived from fresh Alloy 1.18 audio"
        )
    ...
    receipt = {
        "candidate": ROOT.name,
        "revision": "v3-hwao-introduction-order",
        "renderer": str(Path(__file__).name),
        "renderer_sha256": sha256(Path(__file__)),
        "narration_script": "narration_script_v3.json",
        "narration_script_sha256": sha256(ROOT / "narration_script_v3.json"),
