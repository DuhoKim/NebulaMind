...
SCRIPT_PATH = ROOT / "narration_script_v3.json"
BUILD_RECEIPT = ROOT / "build_receipt.json"
OUT = ROOT / "encoded_qa"
FRAMES = OUT / "frames"
CONTACT = ROOT / "encoded-contact-sheet-v3.jpg"
...
def ocr_frames(items: list[tuple[str, float, Path]]) -> dict:
    ...
    forbidden_phrases = ["sn ia", "dark energy", "black hole"]
...
def main() -> int:
    ...
    semantic_terms = ["significance", "dipole", "parity", "cosmology", "grb", "sn ia", "dark energy", "quasar", "h0", "black hole", "desi", "ganalyzer"]
    semantic_hits = [term for term in semantic_terms if re.search(rf"(?<![a-z0-9]){re.escape(term)}(?![a-z0-9])", narration)]
    ...
    checks = {
        ...
        "all_27_sentence_frames_nonblack": len(frame_stats) >= 27 and all(item["mean_luma"] > 8 for item in frame_stats[:27]),
        "four_move_introduction_is_first": [item["id"] for item in opening] == ["i01", "i02", "i03", "i04"],
        "opening_required_terms_present": all(
            term in opening_text
            for term in ("two handednesses", "sky", "fact about the universe", "humans sorted", "fact about the sorters", "how do we tell")
        ),
        "opening_universe_clause_is_conditional": all(term in opening[1]["text"].lower() for term in ("if", "were", "would")),
        "opening_sorters_clause_is_conditional": "could" in opening[2]["text"].lower() and "apparent" in opening[2]["text"].lower(),
        "mirror_is_longest_section": all(mirror_interval > duration for section, duration in section_intervals.items() if section != "mirror-climax"),
        ...
        "encoded_integrated_loudness_intelligible": -22 <= float(loudness["input_i"]) <= -14,
        ...
        "predecessor_mp4_preserved": sha256(PREDECESSOR / "spin-method-overhaul-canary-20260808T1312K.mp4") == "40804f86b221bc9af3d5107b923b954b379e0734e384c33c29fc0363712d65c9",
        "predecessor_script_v2_preserved": sha256(PREDECESSOR / "narration_script_v2.json") == "3f033dd02d00767c6bb4cc1baf8b7197a78847bad076411cbaed9aab732cd416",
    }
    report = {
        "candidate": ROOT.name,
        "status": "PENDING_SEXTET_POST_ENCODED_REVIEW" if all(checks.values()) else "MACHINE_QA_HOLD",
