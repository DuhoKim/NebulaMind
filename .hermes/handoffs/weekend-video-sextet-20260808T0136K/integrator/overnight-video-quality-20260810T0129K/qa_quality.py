#!/usr/bin/env python3
"""Full encoded QA, reading-time audit, receipts, and freeze for a quality candidate."""
...
RENDERER = ROOT / "render_quality.py"
...
def loudness(path: Path) -> dict:
    p = subprocess.run(["ffmpeg", "-hide_banner", "-nostats", "-i", str(path), "-af", "loudnorm=I=-20.5:LRA=7:TP=-2.3:print_format=json", "-f", "null", "-"], check=True, capture_output=True, text=True)
...
def extract_frames(artifact_root: Path, video: Path, records: list[dict], peak_section: str) -> tuple[list[dict], Path]:
    out = artifact_root / "encoded_qa" / "frames"
    ...
    boundary=next((r for r in records if r.get("visual")=="boundary"),None)
    if boundary is not None and float(boundary.get("boundary_preroll_seconds",0.0))>0:
        t=boundary["audio_start_seconds"]-float(boundary["boundary_preroll_seconds"])/2
        target=out/f"boundary-preroll-{t:07.3f}.jpg"
        run(["ffmpeg","-hide_banner","-loglevel","error","-y","-ss",f"{t:.6f}","-i",str(video),"-frames:v","1","-q:v","2",str(target)])
        items.append({"label":"boundary-preroll","time":t,"path":target})
    ...
    contact=artifact_root/"encoded-contact-sheet.jpg"; sheet.save(contact,quality=94,subsampling=0)
    return items,contact


def ocr_frames(items: list[dict], artifact_root: Path) -> dict:
    parts=[]
    for item in items:
        p=run(["tesseract",str(item["path"]),"stdout","--psm","6"])
        parts.append(f"--- {item['label']} ---\n{p.stdout}")
    text="\n".join(parts); (artifact_root/"encoded_qa/ocr.txt").write_text(text)
...
def transcribe_intro(artifact_root: Path, video: Path, records: list[dict]) -> dict:
    start=max(0.0,records[0]["audio_start_seconds"]-0.25); end=records[3]["audio_end_seconds"]+0.35
    wav=artifact_root/"encoded_qa/encoded-introduction.wav"
    ...
    (artifact_root/"encoded_qa/encoded-introduction-transcription.json").write_text(json.dumps(report,indent=2)+"\n")
    return report
...
def main() -> int:
    ap=argparse.ArgumentParser(); ap.add_argument("candidate_dir"); ap.add_argument("--artifact-dir",required=True); args=ap.parse_args(); candidate=Path(args.candidate_dir).resolve(); artifact_root=Path(args.artifact_dir).resolve()
    if candidate==artifact_root or candidate in artifact_root.parents: raise SystemExit("artifact workspace must be outside the candidate directory")
    ...
    items,contact=extract_frames(artifact_root,video,timeline["records"],spec["peak_section"])
    ocr=ocr_frames(items,artifact_root); intro=transcribe_intro(artifact_root,video,timeline["records"]); mot=motion(video,duration); loud=loudness(video)
    ...
    reading=timeline["reading_policy"]
    profile=spec["quality_profile"]
    unjustified_long_pauses=[r["id"] for r in timeline["records"] if r["pause_after_seconds"]>4.0 and r["pause_reason"] not in {"reading_time_floor","tail_reading_time_floor","special_named_hold"}]
    measured_i=float(loud["input_i"]); measured_lra=float(loud["input_lra"]); measured_tp=float(loud["input_tp"])
    checks={
        ...
        "all_sentence_states_extracted":len([x for x in items if not x["label"].startswith("peak-") and x["label"]!="boundary-preroll"])==len(timeline["records"]),
        ...
        "loudness_within_0_15_lu_of_series_target":abs(measured_i-float(profile["integrated_lufs_target"]))<=0.15,
        "loudness_range_in_series_band":5.0<=measured_lra<=8.0,
        "true_peak_safe":measured_tp<=-2.0,
        "every_card_meets_reading_time_floor":reading["all_cards_meet_floor"] is True and reading["minimum_margin_seconds"]>=-1e-6,
        "no_unjustified_pause_over_four_seconds":not unjustified_long_pauses,
        "raw_sentence_audio_reused_without_resynthesis":timeline["raw_sentence_audio_reused_without_resynthesis"] is True,
        "boundary_preroll_present":any(x["label"]=="boundary-preroll" for x in items),
        ...
        "curve_icon_primitive_unavailable":re.search(r"elif\s+kind\s*==\s*['\"]curve['\"]\s*:",renderer_source) is None,
        "progress_rail_independent_fill_absent":"scan_start" not in renderer_source and "scan_right" not in renderer_source,
        "badge_primitive_fits_text":"fit_text_size" in renderer_source and "badge text exceeds available width" in renderer_source,
    ...
    report={"status":"PASS" if all(checks.values()) else "HOLD","candidate":candidate.name,"video":video.name,"video_sha256":sha256(video),"artifact_workspace":str(artifact_root),"probe":probe,"timeline_summary":{k:timeline[k] for k in ("sentence_count","word_count","delivered_wpm","master_duration_seconds","max_abs_audio_visual_start_delta_seconds","section_intervals_seconds")},"reading_policy":reading,"unjustified_long_pauses":unjustified_long_pauses,"loudness":loud,"motion":mot,"introduction_transcription":intro,"ocr":ocr,"caption_lines":caption_lines,"numeric_guard":numeric,"contact_sheet":str(contact),"checks":checks,"passed":sum(checks.values()),"total":len(checks)}
