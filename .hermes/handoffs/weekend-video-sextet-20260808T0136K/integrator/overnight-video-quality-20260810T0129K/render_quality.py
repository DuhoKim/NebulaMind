#!/usr/bin/env python3
"""Quality-track renderer for versioned sibling candidates."""
...
def wrapped(draw, text: str, box, size: int, color=WHITE, mono=False, bold=False, align="center", max_lines=None):
    f = font(size, mono, bold); lines = wrap_lines(draw, text, f, box[2] - box[0])
...
def fit_text_size(draw, text, max_width, size, min_size=12, mono=False, bold=True):
    fitted=size
    while fitted>min_size and draw.textlength(text,font=font(fitted,mono,bold))>max_width:
        fitted-=1
    if draw.textlength(text,font=font(fitted,mono,bold))>max_width:
        raise RuntimeError(f"badge text exceeds available width: {text}")
    return fitted


def badge(draw, box, text, color, size=24):
    # Primitive-level containment: badge text must stay inside its own pill.
    fitted=fit_text_size(draw,text,(box[2]-box[0])-36,size)
    rounded(draw, box, 15, mix(BG, color, 0.10), color, 2)
    center_text(draw, text, ((box[0]+box[2])/2,(box[1]+box[3])/2), fitted, color, bold=True)
...
class Renderer:
    def __init__(self,candidate:Path,artifact_dir:Path|None=None):
        ...
        self.artifact_root=(artifact_dir or candidate).resolve()
        self.qa_frames=self.artifact_root/"qa_frames"; self.preview=self.artifact_root/"final-timing-contact-sheet.jpg"
        numeric_guard(candidate,self.spec); self.validate()

    def validate(self):
        ids=[r["id"] for r in self.records]
        if len(ids)!=len(set(ids)): raise RuntimeError("duplicate ids")
        icon_kinds={r["id"]:r.get("params",{}).get("icon") for r in self.records if r.get("params",{}).get("icon")}
        if any(kind=="curve" for kind in icon_kinds.values()):
            raise RuntimeError(f"result-like curve icon primitive is forbidden: {icon_kinds}")
        allowed_icons={"archive","galaxy","paired_strokes","plane","anchor"}
        unknown_icons={rid:kind for rid,kind in icon_kinds.items() if kind not in allowed_icons}
        if unknown_icons: raise RuntimeError(f"unknown icon primitives: {unknown_icons}")
        ...
        d=ImageDraw.Draw(Image.new("RGB",(W,H))); f=font(31,bold=True)
        over={r["id"]:len(wrap_lines(d,r["text"],f,W-410)) for r in self.records}
    ...
    def display_state(self,t):
        """Return the visual state and whether its narration caption is active.

        A dense scientific-boundary card is revealed during the final justified
        pause before its narration. This creates a card-first beat without
        deleting or rewriting the exact spoken/subtitle boundary.
        """
        record,progress=self.active(t)
        profile=self.spec.get("quality_profile",{})
        lead=float(profile.get("boundary_preroll_seconds",0.0))
        if lead>0:
            for candidate in self.records:
                start=float(candidate["audio_start_seconds"])
                if candidate.get("visual")=="boundary" and start-lead<=t<start:
    ...
    def chrome(self,draw,t,section):
        ...
        text="METHOD DESIGN · NO MEASURED VALUE"; f=font(22,bold=True); draw.text((W-70-draw.textlength(text,font=f),31),text,font=f,fill=AMBER)
        draw.line((70,80,W-70,80),fill=GRID,width=2)
        stages=[("MOTIVATION",{"motivation","difficulty"}),("DISCRIMINANT",{"peak"}),("SOURCE",{"sample"}),("ESTIMATOR",{"estimator"}),("CONTROLS",{"controls","discipline"}),("SCIENCE",{"boundary","payoff"})]
        y=888; xs=[110,450,790,1130,1470,1810]
        draw.line((xs[0],y,xs[-1],y),fill=GRID,width=3)
        # No independent moving fill: the active dot and label are the sole position cue.
        for x,(label,sections) in zip(xs,stages):
            on=section in sections
            # Breathing motion stays anchored to the truthful stage. Unlike the
            # removed sweep, it never travels between stages or claims a second
            # position on the rail.
            pulse=0.8+0.2*math.sin(math.pi*t)
            c=mix(MUTED,CYAN,pulse) if on else MUTED
            radius=9+int(round(math.sin(math.pi*t))) if on else 8
            if on:
                glow=0.22+0.18*math.sin(math.pi*t)
                focus_left=max(20,x-140); focus_right=min(W-20,x+140)
                rounded(draw,(focus_left,y+17,focus_right,y+51),12,mix(BG,CYAN,glow))
            draw.ellipse((x-radius,y-radius,x+radius,y+radius),fill=c)
            center_text(draw,label,(x,y+34),18,c,bold=True)
    ...
    def icon(self,draw,kind,x,y,color,q=1.0,scale=1.0):
        ...
        if kind=="archive":
        ...
        elif kind=="paired_strokes":
            # Non-graph symbol: two equal-length, separated, horizontal strokes.
            # No axes, slope, relative response, intersection, or crossing is encoded.
            for yy in (-30,30):
                draw.line((x-92,y+yy,x+92,y+yy),fill=c,width=8)
                draw.ellipse((x-96,y+yy-4,x-88,y+yy+4),fill=c)
                draw.ellipse((x+88,y+yy-4,x+96,y+yy+4),fill=c)
        elif kind=="plane":
            ...
            raise RuntimeError(f"unknown icon primitive {kind}")
    ...
    def discipline(self,draw,r,p,t):
        ...
        # Keep the discipline explainer below the global title/subtitle stack.
        center_text(draw,"so the answer cannot be shaped after seeing it",(W/2,230),25,MUTED,bold=True)
    ...
    def frame(self,t):
        img=Image.new("RGB",(W,H),BG); draw=ImageDraw.Draw(img); self.background(draw,t); r,p,caption_active=self.display_state(t); self.chrome(draw,t,r["section"])
        visual=r["visual"]
        ...
        if caption_active:
            if r.get("display_citation"): draw.text((90,852),r["display_citation"],font=font(18),fill=MUTED)
            rounded(draw,(170,940,1750,1054),22,(5,12,23),GRID,2); wrapped(draw,r["text"],(205,958,1715,1038),31,WHITE,bold=True,max_lines=2)
        return img
    ...
    def preview_frames(self):
        ...
        lead=float(self.spec.get("quality_profile",{}).get("boundary_preroll_seconds",0.0))
        boundary=next((r for r in self.records if r.get("visual")=="boundary"),None)
        if boundary is not None and lead>0:
            t=boundary["audio_start_seconds"]-lead/2; p=self.qa_frames/f"boundary-preroll-{t:07.3f}.png"; self.frame(t).save(p); items.append(("boundary-preroll",t,p))
    ...
    def render(self):
        ...
        cmd=["ffmpeg","-y","-hide_banner","-loglevel","error","-f","rawvideo","-pix_fmt","rgb24","-s",f"{W}x{H}","-r",str(FPS),"-i","-","-i",str(audio),"-map","0:v:0","-map","1:a:0","-c:v","libx264","-preset","medium","-crf","20","-pix_fmt","yuv420p","-c:a","aac","-b:a","192k","-movflags","+faststart","-t",f"{self.duration:.6f}",str(self.output)]
        ...
        environment={"python":sys.version,"platform":platform.platform(),"pillow":PIL.__version__,"ffmpeg":ffmpeg_version,"font_paths":{"sans":FONT_PATH,"mono":MONO_PATH},"font_sha256":{"sans":sha256(Path(FONT_PATH)),"mono":sha256(Path(MONO_PATH))},"build_command":cmd[:-1]+[self.output.name]}
        ...
        profile=self.spec.get("quality_profile",{})
        receipt={"candidate":self.root.name,"output":self.output.name,"output_sha256":sha256(self.output),"output_bytes":self.output.stat().st_size,"renderer":"overnight quality-track sibling renderer","renderer_path":str(renderer_snapshot.relative_to(self.root)),"renderer_sha256":sha256(renderer_snapshot),"render_environment_path":str(environment_path.relative_to(self.root)),"render_environment_sha256":sha256(environment_path),"spec_sha256":sha256(self.root/"spec.json"),"timeline_sha256":sha256(self.root/"audio/timeline.json"),"audio_master_sha256":sha256(audio),"duration_seconds":self.duration,"fps":FPS,"resolution":[W,H],"source_grounded_runtime_percent":100*grounded/max(self.duration,1),"quality_profile":profile,"quality_profile_sha256":hashlib.sha256(json.dumps(profile,sort_keys=True,separators=(",",":")).encode()).hexdigest(),"video_reportable_now":False}
        (self.root/"build_receipt.json").write_text(json.dumps(receipt,indent=2)+"\n"); print(self.output); print(receipt["output_sha256"])


def main():
    ap=argparse.ArgumentParser(); ap.add_argument("candidate_dir"); ap.add_argument("--preview",action="store_true"); ap.add_argument("--render",action="store_true"); ap.add_argument("--artifact-dir"); a=ap.parse_args()
    if not a.preview and not a.render: raise SystemExit("choose --preview or --render")
    artifact_dir=Path(a.artifact_dir).resolve() if a.artifact_dir else None
    if a.preview and artifact_dir is None: raise SystemExit("preview requires --artifact-dir outside the candidate")
    r=Renderer(Path(a.candidate_dir).resolve(),artifact_dir)
