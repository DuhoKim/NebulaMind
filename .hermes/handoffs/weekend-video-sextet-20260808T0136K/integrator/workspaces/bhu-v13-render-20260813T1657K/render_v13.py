#!/usr/bin/env python3
"""Render V13 from the exact three-seat-gated, closed-world visual contract."""
...
STORY_PATH = SOURCE / "STORYBOARD_DRAFT_V13.json"
NARRATION_PATH = SOURCE / "NARRATION_DRAFT_V12.md"
TEXT_CONTRACT_PATH = SOURCE / "V13_VISUAL_TEXT_CONTRACT.json"
SPEC_PATH = SOURCE / "LANA_VISUAL_REDESIGN_SPEC.md"
PRE_RENDER_GATE_PATH = SOURCE / "V13_PRE_RENDER_GATE_RECEIPT.json"
...
OUTPUT = OUTPUT_DIR / "bhu-closing-record-v13-local-20260813T0932Z.mp4"
OUTPUT_SRT = OUTPUT_DIR / "bhu-closing-record-v13-captions-20260813T0932Z.srt"
OUTPUT_VTT = OUTPUT_DIR / "bhu-closing-record-v13-captions-20260813T0932Z.vtt"
...
class Renderer:
    def __init__(self):
        paths = {
            "STORYBOARD_DRAFT_V13.json": STORY_PATH, "NARRATION_DRAFT_V12.md": NARRATION_PATH,
            "V13_VISUAL_TEXT_CONTRACT.json": TEXT_CONTRACT_PATH, "LANA_VISUAL_REDESIGN_SPEC.md": SPEC_PATH,
            "V13_PRE_RENDER_GATE_RECEIPT.json": PRE_RENDER_GATE_PATH,
        ...
        self.pre_render_gate = json.loads(PRE_RENDER_GATE_PATH.read_text())
        if self.story["estimated_duration_seconds"] != DURATION or float(self.timeline["master_duration_seconds"]) != DURATION:
            raise RuntimeError("V13 duration contract drift")
        ...
        exact_no_terminus = "no 95.4% endpoint, arrow, tick, bracket, marker, whisker, shaded boundary, axis-aligned glyph, or scaled terminus"
        if self.story["render_contract"].get("card_05_no_terminus_prohibition") != exact_no_terminus:
            raise RuntimeError("V13 exact nine-term no-terminus prohibition drift")
        illustration_role = [{"role": "illustration_tag", "text": "ILLUSTRATION", "permitted_when": "QA judges a generated asset could be read as an observation"}]
        if self.text_contract["rules"].get("conditionally_permitted_roles") != illustration_role:
            raise RuntimeError("V13 conditional ILLUSTRATION role drift")
        if self.pre_render_gate.get("status") != "PASS_V13_PRE_RENDER_THREE_SEAT_EXACT_HASH_GATE":
            raise RuntimeError("V13 pre-render three-seat gate is not PASS")
        for name, expected in self.pre_render_gate["frozen_packet"].items():
            if sha(SOURCE / name) != expected:
                raise RuntimeError(f"V13 pre-render gate source drift: {name}")
        if any(seat.get("verdict") != "PASS" for seat in self.pre_render_gate["seats"].values()):
            raise RuntimeError("V13 pre-render seat gate is not all PASS")
    ...
    def interval_boundaries(self) -> list[int]:
        ...
        if result[0] != 0 or result[-1] != DURATION*FPS:
            raise RuntimeError("render boundaries do not cover V13")
        return result
    ...
    def render(self):
        ...
        manifest = {
            "status": "ENCODED_V13_CANDIDATE_WITH_EMBEDDED_SUBTITLE_STREAM_AWAITING_ENCODED_QA",
