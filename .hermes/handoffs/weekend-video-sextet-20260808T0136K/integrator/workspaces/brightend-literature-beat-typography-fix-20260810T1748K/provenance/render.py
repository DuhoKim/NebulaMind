...
SERIF_SYMBOL_PATH = "/System/Library/Fonts/Supplemental/STIXTwoMath.otf"
NUMERIC = re.compile(r"(?<![A-Za-z0-9_])(\d[\d,]*\.?\d*)(?![A-Za-z0-9_])")


@functools.lru_cache(maxsize=None)
def font(size: int, mono: bool = False, bold: bool = False, serif: bool = False, serif_symbol: bool = False) -> ImageFont.FreeTypeFont:
    if serif_symbol:
        return ImageFont.truetype(SERIF_SYMBOL_PATH, size)
    if serif:
...
def wrapped(draw, text: str, box, size: int, color=WHITE, mono=False, bold=False, align="center", max_lines=None, serif=False, serif_symbol=False):
    f = font(size, mono, bold, serif, serif_symbol); lines = wrap_lines(draw, text, f, box[2] - box[0])
...
class Renderer:
    ...
    def validate(self):
        ...
        hits=[]
        for term in self.spec.get("forbidden_terms",[]):
            pattern=rf"(?<![a-z0-9]){re.escape(term.lower())}(?![a-z0-9])"
            for record in self.records:
                record_corpus=(record["text"]+" "+" ".join(flatten_strings(record.get("params",{})))).lower()
                exact_attributed_quote=(record.get("section")=="literature" and "exact abstract sentence" in record.get("display_citation","").lower())
                if re.search(pattern,record_corpus) and not exact_attributed_quote:
                    hits.append(term); break
        ...
        def caption_font(record):
            ...
            size=record.get("params",{}).get("caption_font_size",18 if len(record["text"])>190 else 24)
            return font(size,serif=True,serif_symbol=record.get("params",{}).get("serif_font")=="STIX Two Math")
    ...
    def background(self,draw,t):
        ambient=round(6*(1+math.sin(t*2.1)))
        ambient_grid=(20+ambient//3,35+ambient,55+ambient)
        for x in range(0,W+1,96): draw.line((x,80,x,H),fill=ambient_grid,width=1)
        for y in range(96,H,96): draw.line((0,y,W,y),fill=ambient_grid,width=1)
        # Subtle whole-frame ambience prevents long static-card freezes after
        # removing the misleading rail sweep; it carries no result geometry.
        for i in range(120):
            x=(i*211+37)%W; y=90+((i*97+int(t*7))%(H-190)); r=2 if i%4 else 3
    ...
    def literature(self,draw,r,p,t):
        ...
        if P.get("closing"):
            ...
            flow_dots(draw,(960,555),(960,625),t,color,4)
            return
        q=seg(p,.02,.72)
        ...
        wrapped(draw,r["text"],(385,420,1535,625),P.get("quote_font_size",32),WHITE,max_lines=6,serif=True,serif_symbol=P.get("serif_font")=="STIX Two Math")
        draw.line((360,674,1560,674),fill=(45,66,88),width=2)
        flow_dots(draw,(360,674),(1560,674),t,color,6)
    ...
    def frame(self,t):
        ...
        caption_size=r.get("params",{}).get("caption_font_size",18 if visual=="literature" and len(r["text"])>190 else 24 if visual=="literature" else 31)
        rounded(draw,(170,940,1750,1054),22,(5,12,23),GRID,2); wrapped(draw,r["text"],(205,958,1715,1038),caption_size,WHITE,bold=visual!="literature",max_lines=2,serif=visual=="literature",serif_symbol=r.get("params",{}).get("serif_font")=="STIX Two Math")
        return img
