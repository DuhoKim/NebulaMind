#!/usr/bin/env python3
"""Render the why-study-corrected spin-method conference-science overhaul.

The renderer is isolated inside the versioned workspace. Encoding requires the final v4
...
AUDIO_PATH = ROOT / "audio_v4" / "narration_master.wav"
STORYBOARD_PATH = ROOT / "storyboard_v4.json"
OUTPUT_PATH = ROOT / "spin-method-overhaul-canary-20260809T2340K.mp4"
RECEIPT_PATH = ROOT / "build_receipt.json"
PREVIEW_PATH = ROOT / "final-timing-contact-sheet-v4.jpg"
QA_FRAME_DIR = ROOT / "qa_frames" / "sentence-mid-v4"
...
def caption_font_size(record: dict) -> int:
    return 27 if record["id"] == "i04" else 31
...
def caption(draw: ImageDraw.ImageDraw, t: float, record: dict) -> None:
    ...
    wrapped_text(draw, record["text"], (205, 950, W - 205, 1045), caption_font_size(record), fill)
...
def draw_motivation(draw: ImageDraw.ImageDraw, sid: str, p: float, t: float) -> None:
    ...
    if sid == "i05":
        center_text(draw, "THE QUESTION REMAINS OPEN", (W / 2, 182), 31, AMBER)
        left_q = seg(p, 0.02, 0.58)
        right_q = seg(p, 0.20, 0.76)
        rounded(draw, (105, 270, 720, 650), 30, PANEL, mix(GRID, BLUE, left_q), 4)
        center_text(draw, "CLAIMS", (412, 350), 38, mix(BG, BLUE, left_q))
        center_text(draw, "reported in the literature", (412, 410), 27, MUTED)
        badge(draw, (205, 490, 620, 560), "LONGO 2011 · OTHERS", BLUE, 25)
        rounded(draw, (1200, 270, 1815, 650), 30, PANEL, mix(GRID, PURPLE, right_q), 4)
        center_text(draw, "CHALLENGES", (1507, 350), 38, mix(BG, PURPLE, right_q))
        center_text(draw, "bias tests and null reports", (1507, 410), 27, MUTED)
        badge(draw, (1300, 490, 1715, 560), "LAND ET AL. 2008 · OTHERS", PURPLE, 25)
        arrow(draw, (730, 460), (830, 460), BLUE, left_q, 4)
        arrow(draw, (1190, 460), (1090, 460), PURPLE, right_q, 4)
        flow_dots(draw, (735, 460), (825, 460), t, BLUE, 2)
        flow_dots(draw, (1185, 460), (1095, 460), t, PURPLE, 2)
        rounded(draw, (820, 340, 1100, 585), 28, PANEL2, AMBER, 4)
        center_text(draw, "OPEN", (960, 405), 35, WHITE)
        center_text(draw, "QUESTION", (960, 465), 35, WHITE)
        center_text(draw, "?", (960, 540), 58, AMBER)
        badge(draw, (500, 715, 1420, 790), "NO CONSENSUS OR ANSWER ADOPTED", AMBER, 30)
        motivation_source(draw, "Land et al. 2008 · Longo 2011 · contested literature")
        return
...
def validate_caption_layout() -> None:
    ...
    for record in RECORDS:
        caption_font = font(caption_font_size(record))
...
def render() -> None:
    if TIMING_AUTHORITY != "PCM_DERIVED_FINAL" or not AUDIO_PATH.exists():
        raise RuntimeError(
            "BLOCKED_BEFORE_SYNTHESIS: encoding requires audio_v4/timeline.json and "
            "audio_v4/narration_master.wav derived from fresh Alloy 1.18 audio"
        )
    ...
    receipt = {
        ...
        "narration_script": "narration_script_v4.json",
        "narration_script_sha256": sha256(ROOT / "narration_script_v4.json"),
