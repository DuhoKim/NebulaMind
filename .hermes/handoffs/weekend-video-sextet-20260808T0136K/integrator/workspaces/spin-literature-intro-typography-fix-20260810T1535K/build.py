#!/usr/bin/env python3
"""Render the literature-beat-corrected spin-method conference-science overhaul.

The renderer is isolated inside the versioned workspace. Encoding requires the final v5
PCM-derived timeline and freshly synthesized Alloy 1.18 narration.
"""
...
TIMELINE_PATH = ROOT / "audio_v5" / "timeline.json"
AUDIO_PATH = ROOT / "audio_v5" / "narration_master.wav"
STORYBOARD_PATH = ROOT / "storyboard_v5.json"
OUTPUT_PATH = ROOT / "spin-literature-intro-canary-20260810T1535K.mp4"
RECEIPT_PATH = ROOT / "build_receipt.json"
PREVIEW_PATH = ROOT / "final-timing-contact-sheet-v5.jpg"
QA_FRAME_DIR = ROOT / "qa_frames" / "sentence-mid-v5"
...
UNICODE_PATH = "/System/Library/Fonts/Supplemental/STIXTwoText.ttf"

if TIMELINE_PATH.exists():
    TIMELINE = json.loads(TIMELINE_PATH.read_text())
    TIMING_AUTHORITY = "PCM_DERIVED_FINAL"
else:
    STORYBOARD = json.loads(STORYBOARD_PATH.read_text())
    TIMELINE = {
        "records": STORYBOARD["records"],
        "master_duration_seconds": STORYBOARD["provisional_preview_duration_seconds"],
    }
    TIMING_AUTHORITY = "PROVISIONAL_STORYBOARD_PREVIEW_ONLY"
RECORDS = TIMELINE["records"]
DURATION = float(TIMELINE["master_duration_seconds"])


@lru_cache(maxsize=None)
def font(size: int, mono: bool = False, unicode: bool = False) -> ImageFont.FreeTypeFont:
    path = UNICODE_PATH if unicode else (MONO_PATH if mono else FONT_PATH)
...
def fit_text_size(
    ...
    fitted = size
    while fitted > min_size and draw.textlength(label, font=font(fitted)) > max_width:
        fitted -= 1
    if draw.textlength(label, font=font(fitted)) > max_width:
        raise RuntimeError(f"badge text exceeds available width: {label}")
    return fitted


def badge(draw: ImageDraw.ImageDraw, box: tuple[int, int, int, int], label: str, color=CYAN, size=28) -> None:
    fitted = fit_text_size(draw, label, (box[2] - box[0]) - 36, size)
    rounded(draw, box, 18, mix(BG, color, 0.16), color, 2)
    center_text(draw, label, ((box[0] + box[2]) / 2, (box[1] + box[3]) / 2 + 1), fitted, color)
...
def global_chrome(draw: ImageDraw.ImageDraw, t: float, section: str) -> None:
    ...
    center_text(draw, "NEBULAMIND · GALAXY HANDEDNESS", (72, 45), 25, MUTED, anchor="lm")
    right_label = "WHY-STUDY QUESTION · NO ANSWER SELECTED" if section == "motivation" else "METHOD DESIGN · NO MEASURED VALUE"
    center_text(draw, right_label, (W - 72, 45), 23, AMBER, anchor="rm")
    draw.line((70, 80, W - 70, 80), fill=GRID, width=2)

    stages = [
        ("WHY IT MATTERS", {"motivation"}),
        ("MIRROR TEST", {"question", "two-worlds", "mirror-climax"}),
    ...
    for i, (name, memberships) in enumerate(stages):
        ...
        pulse = 0.8 + 0.2 * math.sin(math.pi * t)
        color = mix(MUTED, CYAN, pulse) if active else MUTED
        r = 9 + int(round(math.sin(math.pi * t))) if active else 6
        if active:
            glow = 0.22 + 0.18 * math.sin(math.pi * t)
            focus_left = max(20, int(x - 145))
            focus_right = min(W - 20, int(x + 145))
            rounded(draw, (focus_left, y + 17, focus_right, y + 51), 12, mix(BG, CYAN, glow))
        draw.ellipse((x - r, y - r, x + r, y + r), fill=color)
        center_text(draw, name, (x, y + 34), 19, color)


def caption_font_size(record: dict) -> int:
    return 24 if record["id"] in {"i04", "i05l", "i05s", "i05d"} else 31


def caption_uses_unicode_font(record: dict) -> bool:
    return record["id"] in {"i05l", "i05s"}
...
def caption(draw: ImageDraw.ImageDraw, t: float, record: dict) -> None:
    ...
    wrapped_text(
        ...
        record["text"],
        (205, 950, W - 205, 1045),
        caption_font_size(record),
        fill,
        unicode=caption_uses_unicode_font(record),
    )
...
def motivation_source(draw: ImageDraw.ImageDraw, label: str) -> None:
    center_text(draw, label, (W - 95, 846), 21, MUTED, anchor="rm")


def draw_motivation(draw: ImageDraw.ImageDraw, sid: str, p: float, t: float) -> None:
    """Five-beat why-study arc: expectation, origin, stakes, dispute, method seam."""
    center_text(draw, "WHY TEST GALAXY HANDEDNESS?", (W / 2, 132), 49, WHITE)

    if sid in {"i01", "i02"}:
        center_text(draw, "THE EXPECTATION: NO DIRECTION SHOULD BE SPECIAL", (W / 2, 182), 27, AMBER)
        reveal = seg(p, 0.02, 0.72) if sid == "i01" else 1.0
        sphere = (480, 245, 1440, 700)
        draw.ellipse(sphere, outline=mix(GRID, CYAN, 0.55 + 0.25 * reveal), width=4)
        draw.ellipse((480, 360, 1440, 585), outline=mix(GRID, CYAN, 0.35), width=2)
        draw.ellipse((690, 245, 1230, 700), outline=mix(GRID, CYAN, 0.35), width=2)
        draw.line((480, 472, 1440, 472), fill=mix(GRID, CYAN, 0.32), width=2)
        for index in range(16):
            angle = 2 * math.pi * index / 16 + 0.035 * t
            x = 960 + 390 * math.cos(angle)
            y = 472 + 165 * math.sin(angle)
            chirality = 1.0 if index % 2 == 0 else -1.0
            color = BLUE if chirality > 0 else PURPLE
            local = clamp(reveal * 16 - index)
            draw_spiral(draw, x, y, 34, chirality, mix(BG, color, local), 4)
        badge(draw, (640, 725, 1280, 792), "ISOTROPY EXPECTATION · CONCEPTUAL SKY", CYAN, 27)
        if sid == "i02":
            q = 0.72 + 0.28 * math.sin(t * 3.0)
            center_text(draw, "CLOCKWISE", (365, 472), 26, mix(MUTED, BLUE, q))
            center_text(draw, "ANTICLOCKWISE", (1555, 472), 26, mix(MUTED, PURPLE, q))
            center_text(draw, "BALANCED · NO PREFERRED AXIS", (960, 820), 26, WHITE)
        motivation_source(draw, "Cosmological principle · expectation under isotropy")
        return

    if sid == "i03":
        center_text(draw, "WHY BALANCE IS A PREDICTION, NOT AN ACCIDENT", (W / 2, 182), 27, AMBER)
        cx, cy = 960, 450
        q = seg(p, 0.03, 0.72)
        draw_spiral(draw, cx, cy, 115, 1.0, mix(BG, CYAN, q), 7)
        for index in range(8):
            angle = 2 * math.pi * index / 8 + 0.07 * t
            x = cx + 355 * math.cos(angle)
            y = cy + 245 * math.sin(angle)
            color = BLUE if index % 2 == 0 else PURPLE
            draw.ellipse((x - 28, y - 28, x + 28, y + 28), fill=mix(BG, color, 0.22), outline=color, width=3)
            arrow(draw, (x, y), (cx + 120 * math.cos(angle), cy + 82 * math.sin(angle)), color, q, 4)
            flow_dots(draw, (x, y), (cx + 125 * math.cos(angle), cy + 86 * math.sin(angle)), t + index, color, 2)
        badge(draw, (555, 215, 1365, 280), "SURROUNDING GRAVITY BUILDS ANGULAR MOMENTUM", CYAN, 27)
        rounded(draw, (205, 700, 755, 820), 24, PANEL, BLUE, 3)
        draw_spiral(draw, 330, 760, 52, 1.0, BLUE, 4)
        center_text(draw, "CLOCKWISE POSSIBLE", (500, 760), 26, BLUE)
        rounded(draw, (1165, 700, 1715, 820), 24, PANEL, PURPLE, 3)
        draw_spiral(draw, 1290, 760, 52, -1.0, PURPLE, 4)
        center_text(draw, "ANTICLOCKWISE POSSIBLE", (1480, 760), 26, PURPLE)
        center_text(draw, "NO BRANCH SELECTED", (960, 825), 23, MUTED)
        motivation_source(draw, "White 1984 · angular-momentum growth in protogalaxies")
        return

    if sid == "i04":
        center_text(draw, "THE STAKES ARE CONDITIONAL", (W / 2, 182), 29, AMBER)
        q = seg(p, 0.02, 0.74)
        rounded(draw, (105, 265, 695, 555), 28, PANEL, mix(GRID, BLUE, q), 4)
        badge(draw, (180, 305, 620, 370), "IF A PREFERENCE WERE GENUINE", AMBER, 25)
        draw_spiral(draw, 310, 465, 65, 1.0, BLUE, 5)
        draw_spiral(draw, 490, 465, 65, -1.0, PURPLE, 5)
        center_text(draw, "HANDEDNESS TEST", (400, 535), 25, WHITE)
        arrow(draw, (705, 410), (840, 410), CYAN, q, 5)
        rounded(draw, (830, 300, 1090, 520), 25, PANEL2, CYAN, 4)
        center_text(draw, "MIRROR", (960, 365), 29, CYAN)
        center_text(draw, "SYMMETRY", (960, 415), 29, CYAN)
        center_text(draw, "?", (960, 480), 62, AMBER)
        arrow(draw, (1100, 410), (1235, 410), CYAN, q, 5)
        rounded(draw, (1225, 265, 1815, 555), 28, PANEL, mix(GRID, PURPLE, q), 4)
        center_text(draw, "PREFERRED AXIS?", (1520, 355), 35, PURPLE)
        for angle in (0, math.pi / 4, math.pi / 2, 3 * math.pi / 4):
            dx, dy = 120 * math.cos(angle), 120 * math.sin(angle)
            draw.line((1520 - dx, 440 - dy, 1520 + dx, 440 + dy), fill=mix(GRID, PURPLE, 0.45), width=3)
        center_text(draw, "NO DIRECTION SELECTED", (1520, 535), 22, MUTED)
        rounded(draw, (270, 610, 1650, 810), 24, (10, 18, 31), GRID, 2)
        wrapped_text(
            draw,
        ...
        center_text(draw, "Longo 2011 · exact conditional abstract sentence in narration", (960, 790), 20, MUTED)
        motivation_source(draw, "Longo 2011 · arXiv:1104.2815v1 · conditional")
        return

    if sid in {"i05l", "i05s", "i05d", "i05u"}:
        center_text(draw, "WHAT OTHER STUDIES REPORTED", (W / 2, 182), 31, AMBER)
        if sid == "i05u":
            cards = [
                (115, 270, 655, "LONGO 2011", "REPORTED DIPOLE", BLUE),
                (690, 270, 1230, "SHAMIR 2012", "REPORTED ASYMMETRY", CYAN),
                (1265, 270, 1805, "LAND ET AL. 2008", "CONSISTENT WITH ISOTROPY", PURPLE),
            ]
            q = seg(p, 0.02, 0.72)
            for x0, y0, x1, author, report, color in cards:
                rounded(draw, (x0, y0, x1, 525), 26, PANEL, mix(GRID, color, q), 4)
                center_text(draw, author, ((x0 + x1) / 2, 335), 28, color)
                wrapped_text(draw, report, (x0 + 30, 390, x1 - 30, 475), 26, WHITE)
                center_text(draw, "OTHER-STUDY REPORT", ((x0 + x1) / 2, 500), 18, MUTED)
                arrow(draw, ((x0 + x1) / 2, 535), (960, 665), color, q, 4)
            rounded(draw, (500, 625, 1420, 790), 28, PANEL2, AMBER, 4)
            center_text(draw, "CLAIMED · DISPUTED · UNSETTLED", (960, 680), 34, WHITE)
            badge(draw, (665, 720, 1255, 780), "NO ANSWER SELECTED", AMBER, 28)
            motivation_source(draw, "Longo 2011 · Shamir 2012 · Land et al. 2008 · no winner")
            return

        cards = {
            "i05l": (
                "LONGO 2011 · REPORTED CLAIM",
                ...
                "arXiv:1104.2815v1 · exact abstract sentence",
                BLUE,
            ),
            "i05s": (
                "SHAMIR 2012 · REPORTED CLAIM",
                ...
                "arXiv:1207.5464v1 · exact abstract sentence",
                CYAN,
            ),
            "i05d": (
                "LAND ET AL. 2008 · REPORTED NULL",
                ...
                "arXiv:0803.3247v2 · exact abstract sentence",
                PURPLE,
            ),
        }
        author, quote, source, color = cards[sid]
        q = seg(p, 0.02, 0.72)
        rounded(draw, (245, 250, 1675, 755), 30, PANEL, mix(GRID, color, q), 4)
        badge(draw, (440, 295, 1480, 365), author, color, 29)
        rounded(draw, (340, 405, 1580, 650), 22, (10, 18, 31), GRID, 2)
        wrapped_text(draw, quote, (390, 430, 1530, 625), 32, WHITE, unicode=sid in {"i05l", "i05s"})
        center_text(draw, "ATTRIBUTED TO ANOTHER STUDY · NOT THIS VIDEO'S FINDING", (960, 700), 22, AMBER)
        motivation_source(draw, source)
        return

    center_text(draw, "THE CATCH: THE LABELS CAN CARRY A BIAS", (W / 2, 182), 29, AMBER)
    q = seg(p, 0.02, 0.74)
    rounded(draw, (95, 260, 620, 675), 30, PANEL, BLUE, 4)
    center_text(draw, "SAME IMAGE", (357, 315), 29, BLUE)
    draw_spiral(draw, 357, 465, 125, 1.0, BLUE, 7)
    center_text(draw, "conceptual spiral", (357, 625), 24, MUTED)
    arrow(draw, (630, 465), (790, 360), CYAN, q, 5)
    arrow(draw, (630, 465), (790, 575), CYAN, q, 5)
    rounded(draw, (790, 280, 1150, 435), 22, PANEL2, PURPLE, 3)
    center_text(draw, "HUMAN SORTER", (970, 330), 27, PURPLE)
    badge(draw, (865, 360, 955, 415), "CW", PURPLE, 23)
    badge(draw, (985, 360, 1075, 415), "ACW", PURPLE, 23)
    rounded(draw, (790, 500, 1150, 655), 22, PANEL2, TEAL, 3)
    center_text(draw, "ALGORITHM", (970, 550), 27, TEAL)
    badge(draw, (865, 580, 955, 635), "CW", TEAL, 23)
    badge(draw, (985, 580, 1075, 635), "ACW", TEAL, 23)
    arrow(draw, (1160, 360), (1300, 465), PURPLE, q, 5)
    arrow(draw, (1160, 575), (1300, 465), TEAL, q, 5)
    draw.line((1380, 275, 1380, 660), fill=CYAN, width=5)
    for yy in range(295, 650, 42):
        draw.line((1365, yy, 1395, yy + 16), fill=CYAN, width=3)
    draw_spiral(draw, 1585, 465, 115, -1.0, CYAN, 7)
    badge(draw, (1265, 710, 1810, 790), "HAND OFF TO THE MIRROR CONTROL", CYAN, 27)
    motivation_source(draw, "Land et al. 2008 · demonstrated handedness classification bias")
...
def validate_caption_layout() -> None:
    ...
    for record in RECORDS:
        caption_font = font(caption_font_size(record), unicode=caption_uses_unicode_font(record))
...
def render() -> None:
    if TIMING_AUTHORITY != "PCM_DERIVED_FINAL" or not AUDIO_PATH.exists():
        raise RuntimeError(
            "BLOCKED_BEFORE_SYNTHESIS: encoding requires audio_v5/timeline.json and "
            "audio_v5/narration_master.wav derived from fresh Alloy 1.18 audio"
        )
    ...
    probe = subprocess.run(
        [
            ...
            "-count_frames",
            "-show_entries",
            "format=duration,size:stream=index,codec_name,codec_type,width,height,r_frame_rate,sample_rate,channels,duration,nb_frames,nb_read_frames",
            "-of",
            "json",
            str(OUTPUT_PATH),
        ],
    ...
    encoded_frame_count = int(video_stream["nb_read_frames"])
    encoded_video_duration = float(video_stream["duration"])
    shortest_delta = DURATION - encoded_video_duration
    if frame_count != math.ceil(DURATION * FPS):
        raise RuntimeError("raw frame derivation does not match ceil(audio_duration * fps)")
    if encoded_frame_count != frame_count - 1:
        raise RuntimeError(
            f"unexpected -shortest frame relationship: raw={frame_count}, encoded={encoded_frame_count}"
        )
    if not 0 <= shortest_delta < 1 / FPS:
        raise RuntimeError(f"unexpected audio/video tail delta: {shortest_delta}")
    receipt = {
        "candidate": ROOT.name,
        "revision": "v5-duho-literature-beat-only",
        "renderer": str(Path(__file__).name),
        "renderer_sha256": sha256(Path(__file__)),
        "narration_script": "narration_script_v5.json",
        "narration_script_sha256": sha256(ROOT / "narration_script_v5.json"),
        ...
        "raw_frames_submitted": frame_count,
        "encoded_video_frames": encoded_frame_count,
        "encoded_video_duration_seconds": encoded_video_duration,
        "audio_minus_encoded_video_seconds": shortest_delta,
        "frame_count_derivation": "raw_frames_submitted = ceil(audio_master_seconds * 30); ffmpeg -shortest encodes one fewer video frame because audio ends before the final raw frame completes; delta is under one frame",
        "fps": FPS,
        "resolution": [W, H],
        ...
        "ffprobe": probe_data,
        "status": "PENDING_KUN_GUARDRAIL_REVIEW_THEN_TORI_EXACT_HASH_REGATE",
    }
