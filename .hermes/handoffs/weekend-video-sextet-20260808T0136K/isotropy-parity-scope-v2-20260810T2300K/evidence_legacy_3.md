...
| `anymask_g` | int16 |  | Bitwise mask set if the central pixel from any image satisfies each condition in gg as cataloged on the [DR10 bitmasks page](https://www.legacysurvey.org/dr10/bitmasks) |
| `anymask_r` | int16 |  | Bitwise mask set if the central pixel from any image satisfies each condition in rr as cataloged on the [DR10 bitmasks page](https://www.legacysurvey.org/dr10/bitmasks) |
| `anymask_i` | int16 |  | Bitwise mask set if the central pixel from any image satisfies each condition in ii as cataloged on the [DR10 bitmasks page](https://www.legacysurvey.org/dr10/bitmasks) |
| `anymask_z` | int16 |  | Bitwise mask set if the central pixel from any image satisfies each condition in zz as cataloged on the [DR10 bitmasks page](https://www.legacysurvey.org/dr10/bitmasks) |
| `allmask_g` | int16 |  | Bitwise mask set if the central pixel from all images satisfy each condition in gg as cataloged on the [DR10 bitmasks page](https://www.legacysurvey.org/dr10/bitmasks) |
| `allmask_r` | int16 |  | Bitwise mask set if the central pixel from all images satisfy each condition in rr as cataloged on the [DR10 bitmasks page](https://www.legacysurvey.org/dr10/bitmasks) |
| `allmask_i` | int16 |  | Bitwise mask set if the central pixel from all images satisfy each condition in ii as cataloged on the [DR10 bitmasks page](https://www.legacysurvey.org/dr10/bitmasks) |
| `allmask_z` | int16 |  | Bitwise mask set if the central pixel from all images satisfy each condition in zz as cataloged on the [DR10 bitmasks page](https://www.legacysurvey.org/dr10/bitmasks) |
| `wisemask_w1` | uint8 |  | W1 bitmask as cataloged on the [DR10 bitmasks page](https://www.legacysurvey.org/dr10/bitmasks) |
| `wisemask_w2` | uint8 |  | W2 bitmask as cataloged on the [DR10 bitmasks page](https://www.legacysurvey.org/dr10/bitmasks) |
| `psfsize_g` | float32 | arcsec | Weighted average PSF FWHM in the gg band |
| `psfsize_r` | float32 | arcsec | Weighted average PSF FWHM in the rr band |
| `psfsize_i` | float32 | arcsec | Weighted average PSF FWHM in the ii band |
| `psfsize_z` | float32 | arcsec | Weighted average PSF FWHM in the zz band |
| `psfdepth_g` | float32 | 1/nanomaggy² | For a 5σ5σ point source detection limit in gg, 5/(√psfdepth\_g)5/(psfdepth\_g) gives flux in nanomaggies and −2.5\[log10(5/(√psfdepth\_g))−9\]−2.5\[log10⁡(5/(psfdepth\_g))−9\] gives corresponding AB magnitude |
| `psfdepth_r` | float32 | 1/nanomaggy² | For a 5σ5σ point source detection limit in rr, 5/(√psfdepth\_r)5/(psfdepth\_r) gives flux in nanomaggies and −2.5\[log10(5/(√psfdepth\_r))−9\]−2.5\[log10⁡(5/(psfdepth\_r))−9\] gives corresponding AB magnitude |
| `psfdepth_i` | float32 | 1/nanomaggy² | For a 5σ5σ point source detection limit in ii, 5/(√psfdepth\_i)5/(psfdepth\_i) gives flux in nanomaggies and −2.5\[log10(5/(√psfdepth\_i))−9\]−2.5\[log10⁡(5/(psfdepth\_i))−9\] gives corresponding AB magnitude |
| `psfdepth_z` | float32 | 1/nanomaggy² | For a 5σ5σ point source detection limit in zz, 5/(√psfdepth\_z)5/(psfdepth\_z) gives flux in nanomaggies and −2.5\[log10(5/(√psfdepth\_z))−9\]−2.5\[log10⁡(5/(psfdepth\_z))−9\] gives corresponding AB magnitude |
...
| `nea_g` | float32 | arcsec² | [Noise equivalent area](https://www.legacysurvey.org/dr9/nea) in gg. |
| `nea_r` | float32 | arcsec² | [Noise equivalent area](https://www.legacysurvey.org/dr9/nea) in rr. |
| `nea_i` | float32 | arcsec² | [Noise equivalent area](https://www.legacysurvey.org/dr9/nea) in ii. |
| `nea_z` | float32 | arcsec² | [Noise equivalent area](https://www.legacysurvey.org/dr9/nea) in zz. |
| `blob_nea_g` | float32 | arcsec² | [Blob-masked noise equivalent area](https://www.legacysurvey.org/dr9/nea) in gg. |
| `blob_nea_r` | float32 | arcsec² | [Blob-masked noise equivalent area](https://www.legacysurvey.org/dr9/nea) in rr. |
| `blob_nea_i` | float32 | arcsec² | [Blob-masked noise equivalent area](https://www.legacysurvey.org/dr9/nea) in ii. |
| `blob_nea_z` | float32 | arcsec² | [Blob-masked noise equivalent area](https://www.legacysurvey.org/dr9/nea) in zz. |
...
| `wise_coadd_id` | char\[8\] |  | unWISE coadd brick name (corresponding to the, _e.g._, legacysurvey-<brick>-image-W1.fits.fz [coadd file](https://www.legacysurvey.org/dr10/files/#image-stacks-south-coadd)) for the center of each object |
| `wise_x` | float32 | pix | X position of coordinates in the brick image stack that corresponds to `wise_coadd_id` (see the [DR9 updates page](https://www.legacysurvey.org/dr9/updates/#data-model-changes) for transformations between `wise_x` and `bx`) |
| `wise_y` | float32 | pix | Y position of coordinates in the brick image stack that corresponds to `wise_coadd_id` (see the [DR9 updates page](https://www.legacysurvey.org/dr9/updates/#data-model-changes) for transformations between `wise_y` and `by`) |
