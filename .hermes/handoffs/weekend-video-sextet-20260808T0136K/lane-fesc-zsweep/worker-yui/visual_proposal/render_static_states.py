...
OUT = ROOT / ".hermes/handoffs/weekend-video-sextet-20260808T0136K/lane-fesc-zsweep/worker-yui/visual_proposal"
STATES = OUT / "states"
...
def render_states(source):
    ...
    fig, top, bottom, rail = setup_figure("Does the modelled galaxy photon budget close?", "01 / 08")
    draw_base(top, bottom, data, top_alpha=0.30, bottom_alpha=0.28)
    rail_heading(fig, "The scientific question")
    rail_lines(fig, [
        ("COMPARE", "required leakage\nvs proxy-inferred leakage", INF),
        ("ASK", "when does the model interval\nstop allowing closure?", GREEN),
        ("STATUS", "literature-anchored model\npropagation, not sky data", WARN),
    ])
    ...
    fig, top, bottom, rail = setup_figure("Closure is a statement about the Δ envelope", "03 / 08")
    ...
    rail_lines(fig, [
        ...
        ("CLOSURE ALLOWED", "while the 16–84%\ninterval still spans zero", GREEN),
    ])
    ...
    rail_heading(fig, "Keep two crossings separate")
    rail_lines(fig, [
        ("CLOSURE ENVELOPE", f"z_c = {zc:.3f}\nbootstrap 8.030–8.059", GREEN),
    ...
    fig, top, bottom, rail = setup_figure("The conditional deficit deepens across redshift", "05 / 08")
    ...
    fig, top, bottom, rail = setup_figure("Removing one named prior moves the crossing earlier", "06 / 08")
    ...
    for text in legend.get_texts():
        text.set_color(FG)
    rail_heading(fig, "No-SFRD-tail scenario")
    rail_lines(fig, [
        ("ONE CHANGE", "remove the JWST-motivated\nSFRD tail only", NO_TAIL),
        ("CROSSING", f"z_c = {zc_no:.3f}\nbootstrap 7.602–7.631", NO_TAIL),
        ("DIRECTION", "earlier crossing means\nclosure gets harder", GREEN),
    ], gap=0.13)
    paths.append(save_state(fig, "S06_no_tail_scenario.png"))

    fig, top, bottom, rail = setup_figure("The dominant untested systematic sits outside the Monte Carlo", "07 / 08")
    draw_base(top, bottom, data, top_alpha=0.36, bottom_alpha=0.55)
    rail_heading(fig, "Model boundary")
    rounded_box(fig, 0.72, 0.56, 0.225, 0.17, "#182539", edge="#46617f", radius=0.018)
    add_text(fig, 0.738, 0.70, "INSIDE THIS MODEL", 15, color=INF, weight="bold")
    add_text(fig, 0.738, 0.655, "ionizing efficiency\nIGM clumping\nSFRD priors", 15, color=FG)
    rounded_box(fig, 0.72, 0.31, 0.225, 0.18, "#2a2119", edge="#815d3e", radius=0.018)
    add_text(fig, 0.738, 0.455, "OUTSIDE THIS MODEL", 15, color=WARN, weight="bold")
    add_text(fig, 0.738, 0.408, "whether low-z proxy\ncalibrations transport\nto z > 6", 15, color=FG)
    add_text(fig, 0.725, 0.23, "NO SURVEY MEASUREMENT\nIN THIS STUDY", 15, color=GOLD, weight="bold")
