...
5. At 80 s, the instrument check is prose: “seven” and “three,” not `7/7` and `0/3`. It does not warn that these controls did not cover the recorded `src.redshift`-qualifier contamination.
...
- expose the `21` drop reasons as `19 redshift-search-axis emptied` and `2 abundance-search-axis emptied`, with recorded modifier examples;
...
- render `7/7 recall` and `0/3 controls` as explicit checks while warning that the frozen controls did not cover the recorded `src.redshift`-qualifier contamination documented in T1 findings;
...
The safe pass-6 correction leaves the then-current static pixels unchanged and adds 45 timed reveal states across the 10 storyboard beats. The current superseding contract permits no substantive evidence state to remain unchanged for more than three seconds; every substantive narration clause must trigger a visible metric, label, topology, comparison, or status change; standalone section-divider holds are prohibited; and the close reveals reportable-now, then pending, then holds the combined qualified summary.
...
The safe pass-7 correction leaves static-proposal pixels unchanged at that stage; visual v9 is now current. Each of 10 storyboard beats declares a matched entry/exit `state_handoff`; all nine cross-beat links validate. Full-frame scientific resets are prohibited. After construction, the 178→−21→157→T2 spine persists through the qualified close; whenever 62 is visible, its 157 parent and side-check boundary remain; T1/T2 stage identity and no-count/no-measurement/single-table scope carry into the close.
...
### Pass-7 v6 manifest-authenticated semantic chronology

Round-11 exact v3 adversarial review exposed three mechanisms partly unchanged in v5: b06 re-revealed a direct 157-to-T2 path already introduced in b05, b01's visual action contradicted its progressive structured states, and co-located approved hashes could be rotated in temporary copies because the validator did not authenticate them against the immutable manifest. V6 ends b05 on the new 157 result and reveals the T2 extension only at b06_s01, makes b01 action/state order exact, and authenticates contract/storyboard/validator bytes against `MANIFEST.json` during immutable replay. The external manifest digest remains the trust anchor. The suite now passes baseline and rejects 27/27 counterfactuals, including two approval-rotation probes. Audience science, static-v8 pixels, and candidate bytes remain unchanged. Exact v6 review is mandatory; candidate, integration, TTS, and publication gates remain closed.

### Pass-7 v23 distinct-state and custody reconciliation

Round-25 exact v19 adversarial review found that b07's `RETRIEVAL CHECK COMPLETE` and b08's `RETRIEVAL CHECK PASSED` were a synonym-only transition, leaving a declared 4.5-second semantic gap when the duplicate was excluded. V23 makes b08's first state a new retrieval-passed versus precision-not-certified comparison, then reveals the frozen-control coverage limit, each taxonomy group separately, and the not-T2-rulings boundary. All 45 declared states now add a distinct metric, label, topology, comparison, status, limitation, taxonomy group, example, or boundary under the exact approved build contract.

The same exact review exposed stale operative guidance, stale status, overcounted attack prose, filesystem-custody gaps, a preparer-only question guard, and an unstructured beat-reorder crash. V23 search-qualifies the correction target, binds current status/identifiers, discloses compound attacks, validates regular-file type plus 0444/0555 and no symlinks during sealed replay, pins the neutral-question hash independently in the validator, and returns machine-readable JSON for malformed-input exceptions. Candidate bytes and static-v9 pixels remain unchanged; exact v23 review is mandatory and all execution/release gates remain closed.
