...
if (HERE / "MANIFEST.json").is_file():
    BASE_SNAPSHOT = HERE
    BASE_SNAPSHOT_MANIFEST_SHA256 = os.environ.get("MZR_EXPECTED_MANIFEST_SHA256")
else:
    BASE_SNAPSHOT = HERE / "snapshots/pass7-single-current-local-receipt-v22"
    BASE_SNAPSHOT_MANIFEST_SHA256 = "7db814620b15785bc89f2e8d92fad73ddce2a84cd74870d066003cc59853e20e"
BASE_PAPER_PROJECTION = json.loads((HERE / "qa/PAPER_NAIVE_PACKET_PROJECTION.json").read_text())
...
def run_rotated_contract_case(name, story):
    ...
    with tempfile.TemporaryDirectory(prefix="mzr-validator-rotated-") as directory:
        ...
        shutil.copy2(HERE / "qa/PAPER_NAIVE_PACKET_PROJECTION.json", root / "qa/PAPER_NAIVE_PACKET_PROJECTION.json")
...
def run_snapshot_custody_case(name, relative, mutation):
    """Require manifest authentication to cover regular-file type and read-only modes."""
    with tempfile.TemporaryDirectory(prefix="mzr-validator-custody-") as directory:
        root = Path(directory) / "packet"
        shutil.copytree(BASE_SNAPSHOT, root, symlinks=True)
        for copied_directory in [root, *[path for path in root.rglob("*") if path.is_dir()]]:
            copied_directory.chmod(0o755)
        copied_validator = root / "validate_proposal.py"
        copied_validator.chmod(0o644)
        shutil.copy2(VALIDATOR, copied_validator)
        copied_validator.chmod(0o444)
        manifest_path = root / "MANIFEST.json"
        manifest = json.loads(manifest_path.read_text())
        for entry in manifest["files"]:
            if entry["path"] == "validate_proposal.py":
                entry["sha256"] = sha256(copied_validator)
                entry["bytes"] = copied_validator.stat().st_size
        ...
        if mutation == "symlink":
            external = Path(directory) / "external-identical-bytes"
            ...
            target.chmod(0o644)
        for copied_directory in [root, *[path for path in root.rglob("*") if path.is_dir() and not path.is_symlink()]]:
            copied_directory.chmod(0o555)
        completed = subprocess.run(
            ["python3", str(root / "validate_proposal.py"), "--check"],
            env=dict(os.environ, MZR_EXPECTED_MANIFEST_SHA256=expected_manifest_sha256),
            capture_output=True,
            text=True,
            check=False,
        )
        ...
        try:
            ...
            result = {"verdict": "UNSTRUCTURED", "errors": [completed.stderr]}
        passed = completed.returncode == 0 and result.get("verdict") == "PASS"
        diagnostic_ok = any("snapshot manifest authentication failed" in error for error in result.get("errors", []))
        normalized_errors = ["snapshot manifest authentication failed"] if diagnostic_ok else result.get("errors", [])
        return {
            "name": name,
            "expected_pass": False,
            "actual_pass": passed,
...
def run_question_projection_case(name, projection):
    """Require a validator-owned exact neutral-question contract, separate from the preparer."""
    with tempfile.TemporaryDirectory(prefix="mzr-validator-questions-") as directory:
        projection_path = Path(directory) / "PAPER_NAIVE_PACKET_PROJECTION.json"
        projection_path.write_text(json.dumps(projection, ensure_ascii=False, indent=2) + "\n")
        completed = subprocess.run(
            ["python3", str(VALIDATOR), "--check"],
            env=dict(
                os.environ,
                MZR_PAPER_PROJECTION_RECEIPT_PATH=str(projection_path),
                MZR_SEMANTIC_MUTATION_HARNESS="1",
            ),
            capture_output=True,
            text=True,
            check=False,
        ...
        try:
            ...
            result = {"verdict": "UNSTRUCTURED", "errors": [completed.stderr]}
        passed = completed.returncode == 0 and result.get("verdict") == "PASS"
        diagnostic_ok = any("paper-naive neutral-question contract failed" in error for error in result.get("errors", []))
        return {
            "name": name,
            "expected_pass": False,
            "actual_pass": passed,
...
def run_beat_reorder_structured_failure_case():
    """Malformed/reordered beats must fail with JSON, never an uncaught traceback."""
    mutant = deepcopy(BASELINE)
    mutant["beats"] = [mutant["beats"][-1], *mutant["beats"][:-1]]
    with tempfile.TemporaryDirectory(prefix="mzr-validator-reorder-") as directory:
        candidate = Path(directory) / "STORYBOARD_CANDIDATE.json"
        candidate.write_text(json.dumps(mutant, ensure_ascii=False, indent=2) + "\n")
        completed = subprocess.run(
            ["python3", str(VALIDATOR), "--check"],
            env=dict(os.environ, MZR_STORYBOARD_PATH=str(candidate), MZR_SEMANTIC_MUTATION_HARNESS="1"),
            capture_output=True,
        ...
        try:
            ...
            result = {"verdict": "UNSTRUCTURED", "errors": [completed.stderr]}
        diagnostic_ok = any("validator execution exception" in error for error in result.get("errors", []))
        return {
            "name": "reordered_beats_structured_failure",
            "expected_pass": False,
            "actual_pass": completed.returncode == 0 and result.get("verdict") == "PASS",
            "diagnostic_ok": diagnostic_ok,
            "errors": result.get("errors", []),
            "test_pass": completed.returncode != 0 and result.get("verdict") == "FAIL" and diagnostic_ok,
        }
...
cases.append(run_snapshot_custody_case("manifest_rejects_visual_symlink", "visual_proposal_v9.png", "symlink"))
cases.append(run_snapshot_custody_case("manifest_rejects_frozen_source_symlink", "frozen_sources/pass7/T1_LOG.txt", "symlink"))
cases.append(run_snapshot_custody_case("manifest_rejects_owner_writable_listed_file", "visual_proposal_v9.png", "mode"))

mutant_projection = deepcopy(BASE_PAPER_PROJECTION)
mutant_projection["review_scaffolding"]["questions_sha256"] = "0" * 64
mutant_projection["review_scaffolding"]["approved_neutral_questions_sha256"] = "0" * 64
cases.append(run_question_projection_case("validator_owned_neutral_question_hash", mutant_projection))

cases.append(run_beat_reorder_structured_failure_case())
