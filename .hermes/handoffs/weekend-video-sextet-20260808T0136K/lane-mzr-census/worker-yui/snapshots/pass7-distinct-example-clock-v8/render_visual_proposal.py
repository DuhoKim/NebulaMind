...
LANE = ROOT / ".hermes/handoffs/mzr-archive-census-20260805T1857K"
HERE = Path(__file__).resolve().parent
OUT = HERE / "visual_proposal_v8.png"
...
manifest = json.loads((LANE / "T1_MZR_MANIFEST.json").read_text())
gas = json.loads((LANE / "T1E_GASPHASE_COUNT.json").read_text())
findings = (LANE / "T1_FINDINGS.md").read_text()
...
for x1, y1, x2, y2, color, number, label in boxes:
    ...
    for line_no, line in enumerate(label.split("\n")):
        centered(d, (x1 + 12, y1 + 116 + line_no * 37, x2 - 12, y1 + 158 + line_no * 37), line, font(25, line_no == 0), FG if line_no == 0 else DIM)
for left, right in zip(boxes, boxes[1:]):
    arrow(d, left[2] + 12, fmid, right[0] - 12, DIM)

# Make the drop reasons, 62 side-check topology, and T2 boundary unavoidable.
d.text((480, 660), "19 redshift: e_Z · e_[Z/H]", font=font(16, True), fill=RED)
d.text((480, 686), "2 abundance: e_Ha/Hb · e_logOI/Ha · l_logOI/Ha", font=font(15), fill=RED)
