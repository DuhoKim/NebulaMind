...
def structured_exception_hook(exception_type, exception, traceback):
    """Keep hostile malformed-input failures machine-readable."""
    result = {
        "verdict": "FAIL",
        "errors": [f"validator execution exception: {exception_type.__name__}: {exception}"],
        "scope": "proposal validation only; no audio or encoded-candidate claim",
    }
    print(json.dumps(result, indent=2))


sys.excepthook = structured_exception_hook
...
numeric_audit_path = Path(os.environ.get("MZR_NUMERIC_SOURCE_AUDIT_PATH", HERE / "NUMERIC_SOURCE_AUDIT.json"))
numeric_audit = json.loads(numeric_audit_path.read_text())
paper_projection_path = Path(os.environ.get("MZR_PAPER_PROJECTION_RECEIPT_PATH", HERE / "qa/PAPER_NAIVE_PACKET_PROJECTION.json"))
paper_projection = json.loads(paper_projection_path.read_text())
EXPECTED_NEUTRAL_QUESTIONS_SHA256 = "eae490f5e1b4b7e7d3334e1805cf2eebf66fcfd15eddae0ddb6cf2dcc078b9ec"
EXPECTED_NUMERIC_SOURCE_AUDIT_SHA256 = "2be79738450d5314c78fb25dbad586e956f725950a68862708ad32b6687a517c"
EXPECTED_NUMERIC_AUDIT_SOURCES = [
    "frozen_sources/pass7/T1_FINDINGS.md",
    "frozen_sources/pass7/T1_FINDINGS.md",
    "frozen_sources/pass7/T1_FINDINGS.md",
    "frozen_sources/pass7/T1_MZR_MANIFEST.json:n_candidates_pre_filter",
    "frozen_sources/pass7/T1_MZR_MANIFEST.json:dropped_candidates,n_candidates",
    "frozen_sources/pass7/T1E_GASPHASE_COUNT.json:count,of_candidates",
    "frozen_sources/pass7/T1_MZR_MANIFEST.json:recall_members_returned,controls_appearing",
    "frozen_sources/pass7/T1_FINDINGS.md:25-45",
    "frozen_sources/pass7/FREEZE_RECORD_T2.md; frozen_sources/pass7/WORKFLOW_CHECKLIST.json; storyboard boundary approved by the exact contract",
]
EXPECTED_NUMERIC_AUDIT_USED_IN = [
    ["STORYBOARD_CANDIDATE.json:b03", "visual_proposal_v9.png:abundance card"],
    ["STORYBOARD_CANDIDATE.json:b03", "visual_proposal_v9.png:MASS SEARCH card"],
    ["STORYBOARD_CANDIDATE.json:b03", "visual_proposal_v9.png:redshift card"],
    ["STORYBOARD_CANDIDATE.json:b04", "visual_proposal_v9.png:direct single-table funnel"],
    ["STORYBOARD_CANDIDATE.json:b05", "visual_proposal_v9.png:funnel and grouped modifier examples"],
    ["STORYBOARD_CANDIDATE.json:b06", "visual_proposal_v9.png:62-of-157 side inset; not on the T2 input path"],
    ["STORYBOARD_CANDIDATE.json:b07-b08", "visual_proposal_v9.png:scorecards and precision boundary"],
    ["STORYBOARD_CANDIDATE.json:b09", "visual_proposal_v9.png:two-group precision taxonomy"],
    ["STORYBOARD_CANDIDATE.json:b10", "visual_proposal_v9.png:qualified T2 gate reached directly from 157"],
]
...
semantic_mutation_harness = os.environ.get("MZR_SEMANTIC_MUTATION_HARNESS") == "1"
snapshot_manifest_authentication: dict[str, object] = {
    "manifest_present": snapshot_manifest_path.exists() and not semantic_mutation_harness,
    "external_manifest_digest_required": True,
}
if semantic_mutation_harness:
    snapshot_manifest_authentication.update({
        "external_manifest_digest_required": False,
        "verdict": "NOT_APPLICABLE_SEMANTIC_MUTATION_HARNESS",
    })
elif snapshot_manifest_path.exists():
    ...
    expected_manifest_sha256 = os.environ.get("MZR_EXPECTED_MANIFEST_SHA256")
    actual_manifest_sha256 = file_sha256(snapshot_manifest_path)
    entry_checks = {
        relative: (HERE / relative).exists()
        and not (HERE / relative).is_symlink()
        and stat.S_ISREG((HERE / relative).lstat().st_mode)
        and stat.S_IMODE((HERE / relative).lstat().st_mode) == 0o444
        and file_sha256(HERE / relative) == entry["sha256"]
        and (HERE / relative).stat().st_size == entry["bytes"]
    ...
    packet_directories = [HERE, *[path for path in HERE.rglob("*") if path.is_dir() and not path.is_symlink()]]
    packet_directory_modes_exact = all(stat.S_IMODE(path.lstat().st_mode) == 0o555 for path in packet_directories)
    packet_has_no_symlinks = not any(path.is_symlink() for path in HERE.rglob("*"))
    ...
    snapshot_manifest_authentication.update({
        "expected_manifest_digest_present": bool(expected_manifest_sha256),
        "actual_manifest_sha256": actual_manifest_sha256,
        "expected_manifest_sha256": expected_manifest_sha256,
        "external_manifest_digest_matches": bool(expected_manifest_sha256)
        and actual_manifest_sha256 == expected_manifest_sha256,
        "all_manifest_entries_present_hash_size_match": bool(entry_checks) and all(entry_checks.values()),
        "all_manifest_entries_regular_read_only_files": bool(entry_checks) and all(entry_checks.values()),
        "complete_packet_inventory_matches": actual_packet_files == set(snapshot_entries),
        "packet_directory_modes_0555": packet_directory_modes_exact,
        "packet_has_no_symlinks": packet_has_no_symlinks,
    ...
    authentication_predicates = [
        snapshot_manifest_authentication["expected_manifest_digest_present"],
        snapshot_manifest_authentication["external_manifest_digest_matches"],
        snapshot_manifest_authentication["all_manifest_entries_present_hash_size_match"],
        snapshot_manifest_authentication["all_manifest_entries_regular_read_only_files"],
        snapshot_manifest_authentication["complete_packet_inventory_matches"],
        snapshot_manifest_authentication["packet_directory_modes_0555"],
        snapshot_manifest_authentication["packet_has_no_symlinks"],
        snapshot_manifest_authentication["approval_entry_present"],
        snapshot_manifest_authentication["approval_hash_matches"],
        snapshot_manifest_authentication["validator_entry_present"],
        snapshot_manifest_authentication["validator_hash_matches"],
        snapshot_manifest_authentication["storyboard_entry_present"],
        snapshot_manifest_authentication["storyboard_hash_matches"],
    ]
    snapshot_manifest_authentication["verdict"] = "PASS" if all(authentication_predicates) else "FAIL"
else:
...
errors = []
paper_scaffolding = paper_projection.get("review_scaffolding", {})
paper_question_contract_checks = {
    "validator_owned_question_hash": paper_scaffolding.get("questions_sha256") == EXPECTED_NEUTRAL_QUESTIONS_SHA256,
    "preparer_approved_hash_matches_validator": paper_scaffolding.get("approved_neutral_questions_sha256") == EXPECTED_NEUTRAL_QUESTIONS_SHA256,
    "question_count_exact": paper_projection.get("question_count") == 8,
    "exact_reviewed_questions_match": paper_scaffolding.get("exact_reviewed_questions_match") is True,
    "no_numerical_premise_hits": paper_scaffolding.get("questions_supply_numerical_premises") is False
        and all(not hits for hits in paper_scaffolding.get("numerical_premise_hits", {}).values()),
    "no_answer_premise_hits": paper_scaffolding.get("questions_supply_answers") is False
        and paper_scaffolding.get("answer_premise_guard") == "PASS"
        and all(not hits for hits in paper_scaffolding.get("answer_premise_hits", {}).values()),
    "no_avoidable_structural_premise_hits": paper_scaffolding.get("questions_supply_avoidable_structural_premises") is False
        and all(not hits for hits in paper_scaffolding.get("structural_premise_hits", {}).values()),
}
if not all(paper_question_contract_checks.values()):
    errors.append(f"paper-naive neutral-question contract failed: {[key for key, value in paper_question_contract_checks.items() if not value]}")
numeric_claims = numeric_audit.get("claims", [])
numeric_audit_checks = {
    "exact_file_sha256": file_sha256(numeric_audit_path) == EXPECTED_NUMERIC_SOURCE_AUDIT_SHA256,
    "status_pass": numeric_audit.get("status") == "PASS",
    "exactly_nine_claims": len(numeric_claims) == 9,
    "no_rejected_or_external_asset_block": "rejected_numeric_asset" not in numeric_audit,
    "sources_exactly_current_and_packet_local": [claim.get("source") for claim in numeric_claims] == EXPECTED_NUMERIC_AUDIT_SOURCES,
    "used_in_exactly_current": [claim.get("used_in") for claim in numeric_claims] == EXPECTED_NUMERIC_AUDIT_USED_IN,
    "every_claim_supported": all(str(claim.get("verdict", "")).startswith("SUPPORTED") for claim in numeric_claims),
    "no_absent_visual_names": not any(
        ".png" in json.dumps(claim) and "visual_proposal_v9.png" not in json.dumps(claim)
        for claim in numeric_claims
    ),
}
if not all(numeric_audit_checks.values()):
    errors.append(f"numeric-source audit contract failed: {[key for key, value in numeric_audit_checks.items() if not value]}")
...
if snapshot_manifest_authentication["verdict"] == "FAIL":
    errors.append("snapshot manifest authentication failed: expected external digest, complete inventory, or pinned entry bytes do not match")
...
def prohibited_token_present(token, text):
    if re.fullmatch(r"[A-Za-z ]+", token):
        return re.search(rf"\b{re.escape(token)}\b", text, re.IGNORECASE) is not None
    return token.lower() in text.lower()


found_prohibited_audience = [token for token in prohibited_audience if prohibited_token_present(token, audience_projection)]
...
approved_contract_checks = {
    "storyboard_version_exact": storyboard.get("storyboard_version") == approved_contract["storyboard_version"] == "pass7-distinct-state-custody-v23",
    ...
    "snapshot_manifest_authenticates_complete_inventory": snapshot_manifest_authentication["verdict"] in {"PASS", "NOT_APPLICABLE_UNSEALED_LIVE_LANE", "NOT_APPLICABLE_SEMANTIC_MUTATION_HARNESS"},
    "all_verification_sources_packet_local": all(
        source in {entry["file"] for entry in source_manifest["files"]}
        for beat in beats
        for source in beat["verification_sources"]
    ),
...
source_render_bindings = {
    ...
    "b08_precision_limit_exactly_source_bound": beats[7]["narration"] == "Those checks validate retrieval, not precision: the frozen controls did not cover the source-redshift qualifier contamination documented in the manifest."
        and beats[7]["on_screen_copy"] == {
            "stage": "T1 RETRIEVAL-INSTRUMENT CHECK · 3 CONTROLS",
            "retrieval_status": "RETRIEVAL CHECK PASSED",
            "precision_status": "PRECISION NOT CERTIFIED",
            "coverage_limit": "FROZEN CONTROLS DID NOT COVER THE RECORDED src.redshift-QUALIFIER CONTAMINATION",
    ...
    "b08_semantic_state_progression_exact": [state["evidence_delta"] for state in beats[7]["timed_reveal_states"]] == [
        "retrieval-passed versus precision-not-certified comparison",
        "frozen-control coverage limitation",
        "symbol-and-meaning precision taxonomy group",
        "target-domain-mismatch precision taxonomy group",
...
result = {
    ...
    "numeric_source_audit_checks": numeric_audit_checks,
    "paper_naive_question_contract_checks": paper_question_contract_checks,
...
content_receipt = {
    ...
    if key != "snapshot_manifest_authentication"
}
content_receipt["receipt_scope"] = "CONTENT_ONLY_PRESEAL; snapshot authentication is runtime output because embedding it would make MANIFEST self-referential"
if "--check" in sys.argv:
    stored_content_receipt_matches = out.is_file() and json.loads(out.read_text()) == content_receipt
    if not stored_content_receipt_matches:
        errors.append("stored content-only proposal receipt differs from current validated content")
    result["stored_content_receipt_check"] = {
        "scope": content_receipt["receipt_scope"],
        "matches": stored_content_receipt_matches,
    }
    result["errors"] = errors
    result["verdict"] = "PASS" if not errors else "FAIL"
else:
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(content_receipt, indent=2) + "\n")
print(json.dumps(result, indent=2))
