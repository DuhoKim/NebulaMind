...
for beat in beats:
    ...
    motion_checks[beat["id"]] = {
        ...
        "raw_within_beat_gap_pass": bool(gaps) and maximum_state_seconds == 4.0 and max(gaps) <= maximum_state_seconds,
...
global_motion_checks = {
    ...
    "maximum_semantic_gap_pass": bool(semantic_global_gaps) and maximum_state_seconds == 4.0 and max(semantic_global_gaps) <= maximum_state_seconds,
...
continuity_checks = {
    ...
    "main_spine_persists_after_construction": all(
...
approved_contract_checks = {
    "storyboard_version_exact": storyboard.get("storyboard_version") == approved_contract["storyboard_version"] == "pass7-self-contained-contract-v4",
...
expected_b05_narration = (
    f"Modifier columns then emptied a required axis in {dropped} tables: {redshift_drops} on redshift and "
    f"{abundance_drops} on abundance, leaving {post_filter} recorded candidates."
)
...
source_render_bindings = {
    ...
    "b05_funnel_exactly_source_bound": beats[4]["on_screen_copy"]["main"] == f"{pre_filter} − {dropped} = {post_filter} RECORDED CANDIDATE TABLES"
        and beats[4]["on_screen_copy"]["redshift_drop"].startswith(f"{redshift_drops} · REDSHIFT AXIS EMPTIED")
        and beats[4]["on_screen_copy"]["abundance_drop"].startswith(f"{abundance_drops} · ABUNDANCE AXIS EMPTIED")
    ...
    "b08_precision_limit_exactly_source_bound": beats[7]["narration"] == "Those checks validate retrieval, not precision: the frozen controls did not cover the dominant precision-contamination mode."
        and beats[7]["on_screen_copy"] == {
            "stage": "T1 RETRIEVAL-INSTRUMENT CHECK · 3 CONTROLS",
            "heading": "RETRIEVAL CHECK PASSED · PRECISION NOT CERTIFIED",
            "body": "FROZEN CONTROLS DID NOT COVER THE DOMINANT PRECISION-CONTAMINATION MODE",
        },
