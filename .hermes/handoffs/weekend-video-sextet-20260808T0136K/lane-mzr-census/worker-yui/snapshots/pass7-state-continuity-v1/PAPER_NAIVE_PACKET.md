# Paper-naive closed-book packet — MZR archive census pass-7 v1 proposal
...
Only audience-visible copy is included below. Build directions, timed reveal states, state handoffs, verification filenames, source paths, prior verdicts, and the answer key are excluded.

1. This VizieR metadata census asks which single tables intersect the abundance-search, mass-search, and redshift-search axes.
   Screen: `T1 ENUMERATION · SEARCH-AXIS METADATA REACH`; `A SINGLE-TABLE METADATA CENSUS`; `Which VizieR single tables have metadata-reachable columns on the abundance-search, mass-search, and redshift-search axes?`; `METADATA CENSUS — NOT AN MZR MEASUREMENT`; `SINGLE-TABLE METADATA INTERSECTION · CROSS-TABLE JOINS AND CROSSMATCHES NOT ASSESSED`.
...
3. UCD plus name matching reached 5,568 tables on the abundance-search axis, 6,206 on mass-search, and 6,687 on redshift-search; name matching added 175, 88, and 20.
   Screen: `T1 ENUMERATION`; `METADATA REACH BY AXIS`; `ABUNDANCE SEARCH · UCD 5,393 · UCD + NAME 5,568 · GAIN +175 TABLES`; `MASS SEARCH · UCD 6,118 · UCD + NAME 6,206 · GAIN +88 TABLES`; `REDSHIFT SEARCH · UCD 6,667 · UCD + NAME 6,687 · GAIN +20 TABLES`; `COUNTS · AREA NOT ENCODED`.
