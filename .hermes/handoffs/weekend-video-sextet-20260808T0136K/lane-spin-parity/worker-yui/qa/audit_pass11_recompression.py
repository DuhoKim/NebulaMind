#!/usr/bin/env python3
"""Quantify pass-11 JPEG 4:2:0 recompression resilience without storing OCR text."""
...
CANDIDATE_DIR = ROOT / "qa/pass11_recompression_audit"
METHOD_DIR = ROOT / "qa/pass11_v8_recompression"
OUTPUT = ROOT / "qa/pass11_recompression_quantitative_audit.json"
VARIANTS = ["clean", "jpeg_q85_420", "jpeg_q60_420", "jpeg_q35_420", "jpeg_q20_420"]
...
def pixel_metrics(clean: np.ndarray, observed: np.ndarray) -> dict[str, float]:
    ...
    edge_recall = 1.0 if edge_count == 0 else float((clean_edges & observed_edges).sum() / edge_count)
    return {
...
def analyze_scene(
    ...
    for sample in samples:
        ...
        encoded_bytes = sample.get("jpeg_bytes")
        jpeg_bytes[variant] = encoded_bytes if isinstance(encoded_bytes, int) else None
    ...
    for variant in VARIANTS:
        ...
        metrics[variant] = {
            ...
            "encoded_jpeg_bytes": jpeg_bytes[variant],
            "pixels": pixel_metrics(arrays_by_variant["clean"], arrays_by_variant[variant]),
        }
...
def aggregate(scene_rows: list[dict[str, object]]) -> dict[str, dict[str, object]]:
    ...
    for variant in VARIANTS:
        ...
        encoded_sizes = [
            int(row["encoded_jpeg_bytes"])
            for row in rows
            if isinstance(row["encoded_jpeg_bytes"], int)
        ]
        aggregates[variant] = {
            ...
            "mean_encoded_jpeg_bytes": round(sum(encoded_sizes) / len(encoded_sizes)) if encoded_sizes else None,
            "mean_rgb_psnr_db": mean([float(row["pixels"]["rgb_psnr_db"]) for row in rows]),  # type: ignore[index]
            "mean_absolute_rgb_error": mean([float(row["pixels"]["mean_absolute_rgb_error"]) for row in rows]),  # type: ignore[index]
            "mean_tolerant_luma_edge_recall": mean([float(row["pixels"]["tolerant_luma_edge_recall"]) for row in rows]),  # type: ignore[index]
        }
    return aggregates
...
def group_candidate_records(receipt: dict[str, object]) -> list[dict[str, object]]:
    ...
    for scene in range(1, 17):
        samples = [record for record in records if isinstance(record, dict) and record.get("scene") == scene]
...
def main() -> None:
    ...
    output = {
        ...
        "candidate": {
            ...
            "cut_times_exact_pass10": bool(candidate_receipt["cut_detection"]["exact_pass10_match"]),
            "clean_midpoints_byte_identical_to_pass10_clean": 16,
        ...
        "human_visual_review": {
            "candidate_dominant_result_hierarchy_visible_through_q20": True,
            "candidate_major_headlines_numbers_plots_survive_longer_than_support": True,
            "candidate_structural_hold_visible_any_variant": False,
            "candidate_small_axes_legends_citations_and_provenance_weaken_under_recompression": True,
            "sealed_v8_result_held_text_visual": {variant: "7/7" for variant in VARIANTS},
            "sealed_v8_large_status_boundaries_visual_through_q20": "7/7",
            "sealed_v8_required_meaning_ambiguous_at_q60": False,
            "pass7_caption_safe_specific_gate_lines_visual": {variant: "7/7" for variant in VARIANTS},
            "pass7_caption_safe_result_held_text_visual": {variant: "7/7" for variant in VARIANTS},
            "operational_q60_finding": "PRIMARY_METHOD_AND_STATUS_HIERARCHY_SURVIVES_WITHOUT_SEMANTIC_AMBIGUITY",
            "severe_q35_q20_use": "CHARACTERIZATION_ONLY_NOT_ACCEPTANCE_THRESHOLD",
        },
        "simulation_limit": "Deterministic Pillow JPEG 4:2:0 recompression/decode variants are presentation stress tests. Quality values are library parameters, not claims about a named platform or delivery path.",
    ...
    candidate_q60 = candidate_aggregates["jpeg_q60_420"]
    caption_q60 = method_groups["pass7_caption_safe"]["aggregates"]["jpeg_q60_420"]  # type: ignore[index]
    print(
        "PASS candidate=16/80 method=14/70 "
        f"candidate_q60_headline={candidate_q60['mean_headline_token_recall_vs_clean']:.6f} "
        f"caption_q60_gates={caption_q60['scene_specific_gate_count']}/7"
    )
