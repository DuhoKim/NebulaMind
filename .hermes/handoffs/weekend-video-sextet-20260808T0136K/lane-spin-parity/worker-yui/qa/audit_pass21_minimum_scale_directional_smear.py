#!/usr/bin/env python3
"""Quantify pass-21 native directional-smear then minimum-scale interaction."""
...
OUT = ROOT / "qa/pass21_minimum_scale_directional_smear_quantitative_audit.json"
...
def derive(native: Image.Image, width: int) -> np.ndarray:
    represented = horizontal_smear(native, width).resize((640, 360), Image.Resampling.LANCZOS)
...
def pixel_metrics(reference: np.ndarray, observed: np.ndarray) -> dict[str, float]:
    ...
    ref_x_energy = float(np.mean(ref_gx * ref_gx))
    ref_y_energy = float(np.mean(ref_gy * ref_gy))
    obs_x_energy = float(np.mean(obs_gx * obs_gx))
    obs_y_energy = float(np.mean(obs_gy * obs_gy))
    x_ratio = 1.0 if ref_x_energy == 0.0 else obs_x_energy / ref_x_energy
    y_ratio = 1.0 if ref_y_energy == 0.0 else obs_y_energy / ref_y_energy
    return {
        ...
        "x_gradient_energy_ratio_vs_lossless_360p": round(x_ratio, 6),
        "y_gradient_energy_ratio_vs_lossless_360p": round(y_ratio, 6),
        "x_to_y_gradient_retention_ratio": round(x_ratio / y_ratio, 6) if y_ratio else 1.0,
    }
...
def candidate_audit(receipt: dict[str, object]) -> dict[str, object]:
    ...
    exact_transform = {variant: 0 for variant in SMEAR}
    ...
    for record in records:
        ...
        for variant in REPRESENTED:
            ...
            if variant in SMEAR:
                exact = np.array_equal(derive(native, SMEAR[variant]), observed_pixels)
    ...
    return {
        ...
        "human_review": {
            "smear_w07_then_360p_structural_gate_scene_count": 0,
            "large_result_headlines_numbers_plots_remain_primary": True,
            "thin_vertical_edges_axes_error_bars_caveats_citations_provenance_weaken_first": True,
...
def method_audit(receipt: dict[str, object]) -> dict[str, object]:
    ...
    for group_name, group in groups.items():
        ...
        for variant in REPRESENTED:
            ...
            for scene_record in scenes:
                ...
                if variant in SMEAR:
                    ...
                    with Image.open(source) as native_open, Image.open(path) as observed_open:
                        expected = derive(native_open.convert("RGB"), SMEAR[variant])
            ...
            entry: dict[str, object] = {"exact_transform_recomputed_scenes": exact_count if variant in SMEAR else None}
            if gates:
    ...
    results["human_review"] = {
        "sealed_v8_smear_w07_then_360p_result_held_badges": "7/7",
        "sealed_v8_smear_w07_then_360p_major_status_boundaries": "7/7",
        "sealed_v8_required_meaning_ambiguous": False,
        "pass7_smear_w07_then_360p_exact_top_gates": "7/7",
        "pass7_smear_w07_then_360p_result_held_badges": "7/7",
        "pass12_smear_w07_then_360p_exact_top_gates": "7/7",
        "pass12_smear_w07_then_360p_result_held_badges": "7/7",
        "pass12_smear_w21_then_360p_top_gate_hierarchy_recognizable": "7/7_CHARACTERIZATION",
        "pass12_smear_w21_then_360p_exact_wording_acceptance_reliable": False,
        "pass12_smear_w07_then_360p_overlap_clipping_or_ambiguity": False,
        "fine_axes_citations_provenance_not_acceptance_reliable_at_w21": True,
    }
    return results
...
def main() -> None:
    ...
    operational = cast(dict[str, object], cast(dict[str, object], candidate["aggregate"])["smear_w07_then_360p"])
    proof = cast(dict[str, object], cast(dict[str, object], cast(dict[str, object], method["pass12_sharpness_safe"])["variants"])["smear_w07_then_360p"])
    output = {
        "status": "QA_ONLY_NOT_A_CANDIDATE_NOT_SCIENCE_ADJUDICATION",
        "deepening_pass": 21,
        "completed_at": "2026-08-08T12:36:41+09:00",
        "focus": "native centered horizontal directional smear followed by minimum-scale 640x360 interaction",
        "candidate_sha256": candidate_receipt["candidate_sha256"],
        "candidate": candidate,
        "method": method,
        "decision": {
            "operational_variant": "smear_w07_then_360p",
            "characterization_variants": ["smear_w13_then_360p", "smear_w21_then_360p"],
            ...
            "action": "ADD_MINIMUM_SCALE_DIRECTIONAL_SMEAR_INTEGRATION_GUARD",
            "reason": "The latest pass12 proof preserves all seven exact top gates and complete badges under the operational compound transform; the failed candidate remains structurally ungated while directional fine support weakens.",
        },
        "metric_notes": {
            ...
            "pixels": "Incremental PSNR, MAE, edge recall, and directional gradient-energy ratios are measured against the lossless 360p reference; exact transform recomputation is the representation oracle.",
    ...
    print(
        ...
        f"candidate_w07_then_360p_headline={as_float(operational['headline_recall']):.6f} "
        f"candidate_w07_then_360p_full={as_float(operational['full_recall']):.6f} "
