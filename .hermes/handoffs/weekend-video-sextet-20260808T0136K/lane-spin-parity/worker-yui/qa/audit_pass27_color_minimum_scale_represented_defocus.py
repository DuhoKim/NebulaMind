#!/usr/bin/env python3
"""Quantify color/monochrome -> 360p -> represented-pixel Gaussian r0.5 interaction."""
...
OUT = ROOT / "qa/pass27_color_minimum_scale_represented_defocus_quantitative_audit.json"
RADIUS = 0.5
...
VARIANTS = [
    "color_then_360p_then_represented_gaussian_r0_50",
    "grayscale_bt709_then_360p_then_represented_gaussian_r0_50",
    "protanopia_machado100_then_360p_then_represented_gaussian_r0_50",
    "deuteranopia_machado100_then_360p_then_represented_gaussian_r0_50",
    "tritanopia_machado100_then_360p_then_represented_gaussian_r0_50",
]
...
def pixel_metrics(reference: np.ndarray, observed: np.ndarray) -> dict:
    ...
    ref_grad = float((np.mean(np.abs(np.diff(yr, axis=1))) + np.mean(np.abs(np.diff(yr, axis=0)))) / 2)
    obs_grad = float((np.mean(np.abs(np.diff(yo, axis=1))) + np.mean(np.abs(np.diff(yo, axis=0)))) / 2)
    ...
    return {"rgb_psnr_db": psnr, "mean_absolute_rgb_error": mae, "tolerant_luma_edge_recall": edge_recall, "luma_gradient_energy_ratio": 1.0 if ref_grad == 0 else obs_grad / ref_grad}
...
def main() -> None:
    ...
    for variant in VARIANTS:
        ...
        for scene in cand["scenes"]:
            ...
            expected = np.asarray(Image.fromarray(ref_pixels).filter(ImageFilter.GaussianBlur(radius=RADIUS)), dtype=np.uint8)
            if not np.array_equal(expected, out_pixels):
                raise SystemExit(f"candidate blur mismatch S{scene['scene']} {variant}")
            pixel_rows.append(pixel_metrics(ref_pixels, out_pixels))
        candidate[variant] = {**{f"{region}_recall": round(recall(all_refs[region], all_obs[region]), 6) for region in all_refs}, **aggregate(pixel_rows), "structural_gate_scenes": gate_count, "exact_blur_frames": len(pixel_rows)}
    ...
    for name, group in method["groups"].items():
        ...
        for variant in VARIANTS:
            ...
            for scene in group["scenes"]:
                ...
                expected = np.asarray(Image.fromarray(ref_pixels).filter(ImageFilter.GaussianBlur(radius=RADIUS)), dtype=np.uint8)
                if not np.array_equal(expected, out_pixels):
                    raise SystemExit(f"method blur mismatch {name} S{scene['scene']} {variant}")
                exact += 1
            ...
            variant_results[variant] = {"exact_blur_frames": exact, "mapped_gate_scores": gate_scores, "mapped_gate_threshold_passes": sum(int(item["passes"]) for item in gate_scores), "mean_mapped_gate_similarity": round(float(np.mean([item["score"] for item in gate_scores])), 6) if gate_scores else None}
        method_groups[name] = variant_results
    human_review = {
        "candidate": {"result_hierarchy_primary": True, "structural_held_gate_scenes": "0/16_ALL_FIVE_VARIANTS", "fine_support_softens_first": ["axes", "error bars", "units", "legends", "caveats", "citations", "provenance", "small qualifiers"], "clipping_overlap_or_meaning_changing_ambiguity": False, "transform_repairs_or_authorizes_candidate": False},
    ...
    audit = {
        ...
        "deepening_pass": 27,
        "transform": "native color/monochrome -> Pillow LANCZOS 640x360 -> represented-pixel Pillow GaussianBlur radius 0.5",
