#!/usr/bin/env python3
"""Quantify color/monochrome -> 360p -> represented-pixel 512x288-to-640x360 bilinear resampling round-trip interaction."""
...
OUT = ROOT / "qa/pass30_color_minimum_scale_represented_resampling_roundtrip_quantitative_audit.json"
...
VARIANTS = [
    "color_then_360p_then_represented_bilinear_roundtrip_512x288_to_640x360",
    "grayscale_bt709_then_360p_then_represented_bilinear_roundtrip_512x288_to_640x360",
    "protanopia_machado100_then_360p_then_represented_bilinear_roundtrip_512x288_to_640x360",
    "deuteranopia_machado100_then_360p_then_represented_bilinear_roundtrip_512x288_to_640x360",
    "tritanopia_machado100_then_360p_then_represented_bilinear_roundtrip_512x288_to_640x360",
]
...
def main() -> None:
    ...
    for variant in VARIANTS:
        ...
        for scene in cand["scenes"]:
            ...
            expected = resampling_roundtrip(ref_pixels)
            if not np.array_equal(expected, out_pixels):
                raise SystemExit(f"candidate resampling round-trip mismatch S{scene['scene']} {variant}")
            pixel_rows.append(pixel_metrics(ref_pixels, out_pixels))
        candidate[variant] = {**{f"{region}_recall": round(recall(all_refs[region], all_obs[region]), 6) for region in all_refs}, **aggregate(pixel_rows), "structural_gate_scenes": gate_count, "exact_resampling_roundtrip_frames": len(pixel_rows)}
    ...
    for name, group in method["groups"].items():
        ...
        for variant in VARIANTS:
            ...
            for scene in group["scenes"]:
                ...
                expected = resampling_roundtrip(ref_pixels)
                if not np.array_equal(expected, out_pixels):
                    raise SystemExit(f"method resampling round-trip mismatch {name} S{scene['scene']} {variant}")
                exact += 1
            ...
            variant_results[variant] = {"exact_resampling_roundtrip_frames": exact, "mapped_gate_scores": gate_scores, "mapped_gate_threshold_passes": sum(int(item["passes"]) for item in gate_scores), "mean_mapped_gate_similarity": round(float(np.mean([item["score"] for item in gate_scores])), 6) if gate_scores else None}
        method_groups[name] = variant_results
    human_review = {
        "candidate": {"result_hierarchy_primary": True, "structural_held_gate_scenes": "0/16_ALL_FIVE_VARIANTS", "fine_support_softens_first": ["one-pixel rules and connectors", "small glyph counters and narrow spacing", "fine axes and grid rules", "error bars", "units", "legends", "caveats", "citations", "provenance", "small qualifiers"], "clipping_overlap_or_meaning_changing_ambiguity": False, "transform_repairs_or_authorizes_candidate": False},
    ...
    audit = {
        ...
        "deepening_pass": 30,
        "transform": "native color/monochrome -> Pillow LANCZOS 640x360 -> represented-pixel Pillow BILINEAR 512x288 -> Pillow BILINEAR 640x360",
