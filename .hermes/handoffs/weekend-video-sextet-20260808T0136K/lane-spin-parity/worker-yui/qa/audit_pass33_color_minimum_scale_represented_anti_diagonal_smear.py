#!/usr/bin/env python3
"""Quantify color/monochrome -> 360p -> represented-pixel centered anti-diagonal width-3 smear interaction."""
...
CAND_ROOT = ROOT / "qa/pass33_color_minimum_scale_represented_anti_diagonal_smear_audit"
CAND_RECEIPT = CAND_ROOT / "extraction_receipt.json"
METHOD_ROOT = ROOT / "qa/pass33_v8_color_minimum_scale_represented_anti_diagonal_smear"
METHOD_RECEIPT = METHOD_ROOT / "receipt.json"
OUT = ROOT / "qa/pass33_color_minimum_scale_represented_anti_diagonal_smear_quantitative_audit.json"
WIDTH = 3
...
VARIANTS = [
    "color_then_360p_then_represented_anti_diagonal_smear_w03",
    "grayscale_bt709_then_360p_then_represented_anti_diagonal_smear_w03",
    "protanopia_machado100_then_360p_then_represented_anti_diagonal_smear_w03",
    "deuteranopia_machado100_then_360p_then_represented_anti_diagonal_smear_w03",
    "tritanopia_machado100_then_360p_then_represented_anti_diagonal_smear_w03",
]
...
def pixel_metrics(reference: np.ndarray, observed: np.ndarray) -> dict:
    ...
    ref_grad_main = float(np.mean(np.abs(yr[1:, 1:] - yr[:-1, :-1])))
    obs_grad_main = float(np.mean(np.abs(yo[1:, 1:] - yo[:-1, :-1])))
    ref_grad_anti = float(np.mean(np.abs(yr[1:, :-1] - yr[:-1, 1:])))
    obs_grad_anti = float(np.mean(np.abs(yo[1:, :-1] - yo[:-1, 1:])))
    ...
    return {
        ...
        "main_diagonal_luma_gradient_energy_ratio": 1.0 if ref_grad_main == 0 else obs_grad_main / ref_grad_main,
        "anti_diagonal_luma_gradient_energy_ratio": 1.0 if ref_grad_anti == 0 else obs_grad_anti / ref_grad_anti,
    }
...
def main() -> None:
    ...
    human_review = {
        "candidate": {"result_hierarchy_primary": True, "structural_held_gate_scenes": "0/16_ALL_FIVE_VARIANTS", "fine_support_softens_first": ["main-diagonal and oblique strokes", "diagonal connectors and arrowheads", "small glyph joins and counters", "fine axes and grid rules", "sloped markers and error-bar caps", "units", "legends", "caveats", "citations", "provenance", "small qualifiers"], "clipping_overlap_or_meaning_changing_ambiguity": False, "transform_repairs_or_authorizes_candidate": False},
    ...
    audit = {
        ...
        "deepening_pass": 33,
        "transform": "native color/monochrome -> Pillow LANCZOS 640x360 -> represented-pixel centered anti-diagonal NW-SE width-3 box smear",
