#!/usr/bin/env python3
"""Build deterministic method proofs for 360p plus JPEG 4:2:0 interaction."""
...
OUT = ROOT / "qa/pass17_v8_minimum_scale_recompression"
OUTPUT_SIZE = (640, 360)
...
def recompress(image: Image.Image, quality: int) -> tuple[Image.Image, str, int]:
    encoded = io.BytesIO()
    image.save(
        encoded,
        format="JPEG",
...
def make_sheet(group_dir: Path, variant: str, samples: list[dict[str, object]]) -> dict[str, object]:
    ...
    for index, sample in enumerate(samples):
        ...
        draw.text((x + 8, y + 5), f"S{scene} · {variant}", fill=(242, 246, 252), font=label_font)
    ...
    for scene, source in enumerate(sources, 1):
        ...
        for variant, quality in VARIANTS.items():
            ...
            if variant == "clean":
                ...
                if quality is None:
                    raise AssertionError("quality missing")
                derived, jpeg_hash, jpeg_bytes = recompress(scaled, quality)
...
def main() -> None:
    ...
    receipt = {
        ...
        "deepening_pass": 17,
        "audit": "sealed_v8_pass7_pass12_minimum_scale_jpeg_420_interaction",
        "simulation_scope": "packet-specific native RGB to Pillow LANCZOS 640x360 then JPEG 4:2:0 recompression/decode; not a named platform or delivery path",
        ...
        "variants": {
            name: {"quality": quality, "subsampling": "4:2:0" if quality is not None else None}
