#!/usr/bin/env python3
"""Build deterministic method proofs for 360p plus linear-light black lift."""
...
OUT = ROOT / "qa/pass19_v8_minimum_scale_black_lift"
OUTPUT_SIZE = (640, 360)
...
def black_lift(image_360p: Image.Image, amount: float) -> Image.Image:
    ...
    pixels = np.rint(np.clip(linear_to_srgb(lifted_linear), 0.0, 1.0) * 255.0).astype(np.uint8)
...
def build_group(name: str, spec: dict[str, object]) -> dict[str, object]:
    ...
    for scene, source in enumerate(source_paths, 1):
        ...
        represented = native.resize(OUTPUT_SIZE, Image.Resampling.LANCZOS)
        ...
        for variant, amount in VARIANTS.items():
            ...
            if variant == "clean":
                ...
                if amount is None:
                    raise AssertionError("lift amount required")
...
def main() -> None:
    ...
    receipt = {
        ...
        "deepening_pass": 19,
        "audit": "sealed_v8_pass7_pass12_minimum_scale_linear_light_black_lift_resilience",
        ...
        "black_lift_fraction": {key: value for key, value in VARIANTS.items() if value is not None},
        "maximum_white_to_black_wcag_like_ratio": {
            "black_lift20_360p": 4.2,
            "black_lift30_360p": 3.0,
            "black_lift40_360p": 2.333333,
        },
        "transform_contract": "native RGB -> Pillow LANCZOS 640x360 -> sRGB decode to linear float64 -> L_out=a+(1-a)*L_in -> sRGB encode -> clip -> numpy rint to uint8",
