#!/usr/bin/env python3
"""Build deterministic method proofs for native directional smear then 360p."""
...
OUT = ROOT / "qa/pass21_v8_minimum_scale_directional_smear"
OUTPUT_SIZE = (640, 360)
...
def derive(native: Image.Image, variant: str) -> Image.Image:
    ...
    return horizontal_smear(native, width).resize(OUTPUT_SIZE, Image.Resampling.LANCZOS)
...
def main() -> None:
    ...
    receipt = {
        ...
        "deepening_pass": 21,
        "audit": "sealed_v8_pass7_pass12_native_directional_smear_then_minimum_scale_resilience",
        ...
        "native_horizontal_smear_widths_pixels": {key: value for key, value in VARIANTS.items() if value is not None},
        "transform_contract": "native 1920x1080 RGB -> centered horizontal box smear with edge replication, uint64 sums, integer round-half-up division -> full-canvas Pillow LANCZOS 640x360 RGB",
