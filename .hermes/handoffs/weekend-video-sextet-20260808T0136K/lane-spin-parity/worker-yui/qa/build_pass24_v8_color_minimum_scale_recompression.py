#!/usr/bin/env python3
"""Build method proofs for native color/monochrome -> 360p -> JPEG q60 4:2:0."""
...
OUT = ROOT / "qa/pass24_v8_color_minimum_scale_recompression"
...
def build_group(name: str, source_paths: list[Path], source_receipt: Path, previous_group: dict[str, object]) -> tuple[dict[str, object], int]:
    ...
    for scene, source in enumerate(source_paths, start=1):
        ...
        with Image.open(source) as opened:
            ...
            for variant in VARIANTS:
                ...
                prior_path = ROOT / "qa/pass23_v8_minimum_scale_color_vision" / name / prior_sample["frame"]
                with Image.open(prior_path) as prior_opened:
                    baseline_match = np.array_equal(np.asarray(baseline, dtype=np.uint8), np.asarray(prior_opened.convert("RGB"), dtype=np.uint8))
                baseline_matches += int(baseline_match)
                encoded = jpeg_bytes(baseline)
                jpeg_path = jpegs / f"scene_{scene:02d}_{variant}.jpg"
                jpeg_path.write_bytes(encoded)
                with Image.open(io.BytesIO(encoded)) as decoded:
                    decoded_rgb = decoded.convert("RGB")
                target = frames / f"scene_{scene:02d}_{variant}.png"
                decoded_rgb.save(target, format="PNG", optimize=False)
                sample = {
                    "variant": variant,
                    ...
                    "jpeg": jpeg_path.relative_to(group_root).as_posix(),
    ...
    return ({
        ...
        "scene_count": 7,
        "decoded_png_frame_count": 35,
        "jpeg_stream_count": 35,
...
def main() -> None:
    ...
    receipt = {
        ...
        "deepening_pass": 24,
        "audit": "method_native_monochrome_or_color_vision_then_minimum_scale_then_q60_420_recompression",
        "variant_order": VARIANTS,
        "baseline_variant_map": BASELINE_VARIANTS,
        "scene_count": 21,
        "decoded_png_frame_count": 105,
        "jpeg_stream_count": 105,
        ...
        "transform_contract": {
            ...
            "simulation_scope": "presentation stress only; not clinical diagnostic or named delivery/codec/viewing standard",
    ...
    print(f"PASS groups=3 scenes=21 png=105 jpeg=105 baseline={baseline_total}/105")
