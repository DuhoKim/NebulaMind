#!/usr/bin/env python3
"""Build method proofs for color/monochrome -> 360p -> black-lift20."""
...
OUT = ROOT / "qa/pass25_v8_color_minimum_scale_black_lift"
...
def lift(image: Image.Image) -> Image.Image:
    ...
    return Image.fromarray(np.rint(linear_to_srgb(BLACK_LIFT + (1.0 - BLACK_LIFT) * linear) * 255.0).astype(np.uint8))
    ...
    for scene, source in enumerate(source_paths, start=1):
        ...
        with Image.open(source) as opened:
            ...
            for variant in VARIANTS:
                ...
                lift(baseline).save(target, format="PNG", optimize=False)
                sample = {
                    "variant": variant,
...
def main() -> None:
    ...
    receipt = {
        ...
        "deepening_pass": 25,
        "audit": "method_native_monochrome_or_color_vision_then_minimum_scale_then_represented_pixel_black_lift20",
        ...
        "groups": groups,
        "transform_contract": {
            "order": "native RGB -> color or linear-light BT.709 grayscale/fixed Machado severity-100 matrix -> sRGB np.rint uint8 -> Pillow LANCZOS 640x360 -> represented-pixel linear-light black lift 0.20 -> sRGB np.rint uint8",
            "black_lift_fraction": BLACK_LIFT,
            "ideal_white_to_black_ratio": 4.2,
            "matrices": {key: value.tolist() for key, value in MATRICES.items()},
            "simulation_scope": "presentation stress only; not clinical diagnostic or named viewing/display/projector standard",
    ...
    print(f"PASS groups=3 scenes=21 frames=105 baseline={baseline_total}/105")
