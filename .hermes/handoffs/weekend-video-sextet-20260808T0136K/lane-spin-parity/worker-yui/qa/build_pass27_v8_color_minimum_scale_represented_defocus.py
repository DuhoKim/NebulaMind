#!/usr/bin/env python3
"""Build method proofs for color/monochrome -> 360p -> represented-pixel Gaussian r0.5."""
...
OUT = ROOT / "qa/pass27_v8_color_minimum_scale_represented_defocus"
...
def main() -> None:
        ...
        for prior_scene in prior_group["scenes"]:
            ...
            for variant, baseline_name in VARIANTS.items():
                ...
                if not np.array_equal(derived, stored):
                    raise SystemExit(f"blur mismatch {name} S{scene}")
                exact_blurs += 1
    ...
    receipt = {
        "audit": "method_proofs_native_monochrome_and_color_vision_then_minimum_scale_then_represented_pixel_gaussian_defocus",
        "deepening_pass": 27,
        ...
        "transform": {"order": ["native color/monochrome presentation", "Pillow LANCZOS 640x360", "Pillow GaussianBlur on represented pixels"], "radius_pixels_at_640x360": RADIUS, "pillow_version": pillow_version, "numpy_version": np.__version__, "png_optimize": False},
    ...
    print(f"PASS groups={len(groups)} scenes={sum(g['scene_count'] for g in groups.values())} frames={exact_blurs} baseline={baseline_matches}/105 blur={exact_blurs}/105")
