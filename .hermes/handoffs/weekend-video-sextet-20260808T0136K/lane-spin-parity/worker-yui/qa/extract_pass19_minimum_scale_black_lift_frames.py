#!/usr/bin/env python3
"""Fresh candidate midpoints plus 360p linear-light black-lift interaction variants."""
...
PASS18 = ROOT / "qa/pass18_minimum_scale_obstruction_audit"
OUT = ROOT / "qa/pass19_minimum_scale_black_lift_audit"
...
def black_lift(image_360p: Image.Image, amount: float) -> Image.Image:
    srgb = np.asarray(image_360p.convert("RGB"), dtype=np.float64) / 255.0
    linear = srgb_to_linear(srgb)
    lifted_linear = amount + (1.0 - amount) * linear
    lifted_srgb = np.clip(linear_to_srgb(lifted_linear), 0.0, 1.0)
    pixels = np.rint(lifted_srgb * 255.0).astype(np.uint8)
...
def prior_clean_map(receipt: dict[str, object]) -> dict[int, Path]:
    ...
    if not isinstance(records, list):
        raise TypeError("pass18 records must be a list")
    result: dict[int, Path] = {}
    for record in records:
        if not isinstance(record, dict):
            raise TypeError("pass18 record must be an object")
        samples = record["samples"]
        if not isinstance(samples, list):
            raise TypeError("pass18 samples must be a list")
        sample = next(item for item in samples if isinstance(item, dict) and item.get("variant") == "clean")
        result[int(record["scene"])] = PASS18 / str(sample["frame"])
    return result
...
def main() -> None:
    ...
    pass18_receipt = json.loads((PASS18 / "extraction_receipt.json").read_text())
    cuts, detector = detect_cuts()
    if cuts != EXPECTED_CUTS or cuts != pass18_receipt["cut_detection"]["cuts"]:
    ...
    prior_map = prior_clean_map(pass18_receipt)
    ...
    for scene, time_s in enumerate(midpoints, 1):
        ...
        matches.append(same)
        if not same:
            raise SystemExit(f"scene {scene} does not reproduce pass18 clean bytes")
        represented = image.resize(OUTPUT_SIZE, Image.Resampling.LANCZOS)
        ...
        for variant, amount in VARIANTS.items():
            ...
            if variant == "clean":
                ...
                represented.save(output, format="PNG", optimize=False)
            else:
                if amount is None:
                    raise AssertionError("lift variant requires amount")
                black_lift(represented, amount).save(output, format="PNG", optimize=False)
            with Image.open(output) as saved:
                width, height, mode = saved.width, saved.height, saved.mode
            samples.append({
                "variant": variant,
                "black_lift_fraction": amount,
                "maximum_white_to_black_wcag_like_ratio": (
                    round(1.05 / (amount + 0.05), 6) if amount is not None else None
                ),
        ...
        records.append({
            ...
            "prior_pass18_clean_path": str(prior.relative_to(ROOT)),
            "prior_pass18_clean_sha256": sha(prior),
    ...
    receipt = {
        ...
        "deepening_pass": 19,
        "created_at": "2026-08-08T11:47:19.442950+09:00",
        ...
        "fresh_clean_match": f"{sum(matches)}/16_BYTE_IDENTICAL_TO_PASS18",
        "variant_order": list(VARIANTS),
        "reference_variant": "downscale_360p",
        "operational_variants": ["black_lift20_360p"],
        ...
        "implementation": {
            ...
            "black_lift": "sRGB uint8 -> float64 [0,1] -> IEC-like sRGB decode -> L_out=a+(1-a)*L_in -> sRGB encode -> clip -> rint*255 -> uint8",
            "storage": "non-optimized RGB PNG",
            "transform_order": "native RGB -> LANCZOS 640x360 -> represented-pixel linear-light black lift",
        },
        "simulation_limit": "Packet-specific minimum-scale plus uniform black-lift stress only; not a claim about a named display, projector, room, player, browser, platform, service, viewer, or universal standard.",
        "tts_invoked": False,
