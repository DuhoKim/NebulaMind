#!/usr/bin/env python3
"""Fresh native color/monochrome -> 360p -> represented-pixel black-lift20 audit."""
...
OUT = ROOT / "qa/pass25_color_minimum_scale_black_lift_audit"
FRAMES = OUT / "frames"
CANDIDATE = Path("/Users/duhokim/HermesOps/cockpit/videos/spin-parity-census-narrated-20260808T0149.mp4")
PASS24 = ROOT / "qa/pass24_color_minimum_scale_recompression_audit/extraction_receipt.json"
...
PACKET_CREATED_AT = "2026-08-08T14:18:01+09:00"
NATIVE = (1920, 1080)
REPRESENTED = (640, 360)
BLACK_LIFT = 0.20
VARIANTS = [
...
BASELINE_VARIANTS = {
    "color_then_360p_then_black_lift20": "color_360p",
    "grayscale_bt709_then_360p_then_black_lift20": "grayscale_bt709_then_360p",
    "protanopia_machado100_then_360p_then_black_lift20": "protanopia_machado100_then_360p",
    "deuteranopia_machado100_then_360p_then_black_lift20": "deuteranopia_machado100_then_360p",
    "tritanopia_machado100_then_360p_then_black_lift20": "tritanopia_machado100_then_360p",
}
MATRICES = {
...
def detect() -> tuple[float, list[float]]:
    ...
    return duration, [round(float(match.group(1)), 6) for line in completed.stderr.splitlines() if (match := re.search(r"pts_time:([0-9.]+)", line))]
...
def lift(image: Image.Image) -> Image.Image:
    linear = srgb_to_linear(np.asarray(image.convert("RGB"), dtype=np.float64) / 255.0)
    output = BLACK_LIFT + (1.0 - BLACK_LIFT) * linear
    return Image.fromarray(np.rint(linear_to_srgb(output) * 255.0).astype(np.uint8))
...
def sheet(label: str, rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    for index, row in enumerate(rows):
        ...
        scene = scene_value
        draw.text((x + 8, y + 3), f"S{scene:02d} · {label}", fill=(240, 244, 250), font=face)
...
def main() -> None:
    ...
    pass24 = json.loads(PASS24.read_text())
    pass23 = json.loads(PASS23.read_text())
    duration, cuts = detect()
    if cuts != pass24["detected_cut_times_seconds"] or cuts != pass23["detected_cut_times_seconds"] or len(cuts) != 15:
        raise SystemExit(f"cut mismatch: {cuts}")
    if OUT.exists():
        shutil.rmtree(OUT)
    FRAMES.mkdir(parents=True)
    bounds = [0.0, *cuts, duration]
    prior24 = {int(row["scene"]): row for row in pass24["scenes"]}
    ...
    for number in range(1, 17):
        ...
        previous_clean = ROOT / "qa/pass24_color_minimum_scale_recompression_audit" / prior24[number]["native_clean"]
        clean_match = sha(clean) == prior24[number]["native_clean_sha256"] == sha(previous_clean)
        ...
        scenes.append({
            ...
            "previous_clean_sha256": prior24[number]["native_clean_sha256"],
            "native_clean_byte_identical_to_pass24": clean_match,
            "samples": samples,
        })
    receipt = {
        ...
        "audit": "native_monochrome_or_color_vision_then_minimum_scale_then_represented_pixel_black_lift20",
        "simulation_scope": "packet-specific presentation stress only; not a clinical diagnostic or named viewing/display/projector standard",
        ...
        "transform_contract": {
            "order": "native RGB -> color or linear-light BT.709 grayscale/fixed Machado severity-100 matrix -> sRGB np.rint uint8 -> Pillow LANCZOS 640x360 -> represented-pixel sRGB decode float64 -> linear-light black lift 0.20 -> sRGB np.rint uint8",
            "black_lift_fraction": BLACK_LIFT,
            "ideal_white_to_black_ratio": 4.2,
    ...
    print(f"PASS candidate={EXPECTED_SHA} cuts=15 scenes=16 frames=80 clean={clean_matches}/16 baseline={baseline_matches}/80")
