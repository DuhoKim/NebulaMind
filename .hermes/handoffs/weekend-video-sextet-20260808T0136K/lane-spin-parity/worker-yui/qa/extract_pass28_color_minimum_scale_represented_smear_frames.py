#!/usr/bin/env python3
"""Fresh native color/monochrome -> 360p -> represented-pixel horizontal width-3 smear audit."""
...
OUT = ROOT / "qa/pass28_color_minimum_scale_represented_smear_audit"
FRAMES = OUT / "frames"
PASS27 = ROOT / "qa/pass27_color_minimum_scale_represented_defocus_audit/extraction_receipt.json"
PASS23 = ROOT / "qa/pass23_minimum_scale_color_vision_audit/extraction_receipt.json"
PACKET_CREATED_AT = "2026-08-08T15:26:20+09:00"
...
def main() -> None:
    ...
    pass27 = json.loads(PASS27.read_text())
    pass23 = json.loads(PASS23.read_text())
    cuts = detect_cuts()
    expected = pass23["detected_cut_times_seconds"]
    if cuts != expected or cuts != pass27["detected_cut_times_seconds"] or len(cuts) != 15:
    ...
    for index in range(16):
        ...
        prior_clean = ROOT / pass27["scenes"][index]["native_clean"]
        identical = clean.read_bytes() == prior_clean.read_bytes()
        if not identical or sha(clean) != pass27["scenes"][index]["native_clean_sha256"]:
        ...
        scenes.append({"scene": number, "start_seconds": boundaries[index], "end_seconds": boundaries[index + 1], "midpoint_seconds": midpoint, "native_clean": str(clean.relative_to(ROOT)), "native_clean_sha256": sha(clean), "native_clean_byte_identical_to_pass27": identical, "samples": samples})
    sheets = {}
    ...
    receipt = {
        "audit": "native_monochrome_and_color_vision_then_minimum_scale_then_represented_pixel_horizontal_smear",
        "deepening_pass": 28,
        ...
        "transform": {"order": ["native color/monochrome presentation", "Pillow LANCZOS 640x360", "centered horizontal width-3 box smear on represented pixels"], "axis": "horizontal", "width_pixels_at_640x360": WIDTH, "edge_handling": "edge replication", "accumulator": "uint64", "rounding": "integer round half up", "pillow_version": pillow_version, "numpy_version": np.__version__, "python_version": platform.python_version(), "png_optimize": False},
        "source_receipts": {"pass27": str(PASS27.relative_to(ROOT)), "pass27_sha256": sha(PASS27), "pass23": str(PASS23.relative_to(ROOT)), "pass23_sha256": sha(PASS23)},
