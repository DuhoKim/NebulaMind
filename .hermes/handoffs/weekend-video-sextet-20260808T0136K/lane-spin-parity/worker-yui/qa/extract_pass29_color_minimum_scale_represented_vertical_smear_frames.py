#!/usr/bin/env python3
"""Fresh native color/monochrome -> 360p -> represented-pixel vertical width-3 smear audit."""
...
OUT = ROOT / "qa/pass29_color_minimum_scale_represented_vertical_smear_audit"
FRAMES = OUT / "frames"
PASS28 = ROOT / "qa/pass28_color_minimum_scale_represented_smear_audit/extraction_receipt.json"
...
def main() -> None:
    ...
    pass28 = json.loads(PASS28.read_text())
    pass23 = json.loads(PASS23.read_text())
    cuts = detect_cuts()
    expected = pass23["detected_cut_times_seconds"]
    if cuts != expected or cuts != pass28["detected_cut_times_seconds"] or len(cuts) != 15:
    ...
    for index in range(16):
        ...
        prior_clean = ROOT / pass28["scenes"][index]["native_clean"]
        identical = clean.read_bytes() == prior_clean.read_bytes()
        if not identical or sha(clean) != pass28["scenes"][index]["native_clean_sha256"]:
        ...
        scenes.append({"scene": number, "start_seconds": boundaries[index], "end_seconds": boundaries[index + 1], "midpoint_seconds": midpoint, "native_clean": str(clean.relative_to(ROOT)), "native_clean_sha256": sha(clean), "native_clean_byte_identical_to_pass28": identical, "samples": samples})
    sheets = {}
    ...
    receipt = {
        "audit": "native_monochrome_and_color_vision_then_minimum_scale_then_represented_pixel_vertical_smear",
        "deepening_pass": 29,
        ...
        "source_receipts": {"pass28": str(PASS28.relative_to(ROOT)), "pass28_sha256": sha(PASS28), "pass23": str(PASS23.relative_to(ROOT)), "pass23_sha256": sha(PASS23)},
