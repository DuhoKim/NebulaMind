#!/usr/bin/env python3
"""Fresh native color/monochrome -> 360p -> represented-pixel 512x288-to-640x360 bilinear resampling round-trip audit."""
...
OUT = ROOT / "qa/pass30_color_minimum_scale_represented_resampling_roundtrip_audit"
FRAMES = OUT / "frames"
PASS29 = ROOT / "qa/pass29_color_minimum_scale_represented_vertical_smear_audit/extraction_receipt.json"
...
def main() -> None:
    ...
    pass29 = json.loads(PASS29.read_text())
    pass23 = json.loads(PASS23.read_text())
    cuts = detect_cuts()
    expected = pass23["detected_cut_times_seconds"]
    if cuts != expected or cuts != pass29["detected_cut_times_seconds"] or len(cuts) != 15:
    ...
    for index in range(16):
        ...
        prior_clean = ROOT / pass29["scenes"][index]["native_clean"]
        identical = clean.read_bytes() == prior_clean.read_bytes()
        if not identical or sha(clean) != pass29["scenes"][index]["native_clean_sha256"]:
        ...
        for variant, baseline_name in VARIANTS.items():
            ...
            if not np.array_equal(derived, saved):
                raise SystemExit(f"resampling round-trip mismatch scene {number} {variant}")
            exact_roundtrips += 1
        ...
        scenes.append({"scene": number, "start_seconds": boundaries[index], "end_seconds": boundaries[index + 1], "midpoint_seconds": midpoint, "native_clean": str(clean.relative_to(ROOT)), "native_clean_sha256": sha(clean), "native_clean_byte_identical_to_pass29": identical, "samples": samples})
    sheets = {}
    ...
    receipt = {
        "audit": "native_monochrome_and_color_vision_then_minimum_scale_then_represented_pixel_bilinear_resampling_roundtrip",
        "deepening_pass": 30,
        ...
        "source_receipts": {"pass29": str(PASS29.relative_to(ROOT)), "pass29_sha256": sha(PASS29), "pass23": str(PASS23.relative_to(ROOT)), "pass23_sha256": sha(PASS23)},
    ...
    print(f"PASS candidate={sha(CANDIDATE)} cuts={len(cuts)} scenes=16 frames={exact_roundtrips} clean={fresh_clean}/16 baseline={baseline_matches}/80 roundtrip={exact_roundtrips}/80")
