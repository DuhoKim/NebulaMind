#!/usr/bin/env python3
"""Fresh native color/monochrome -> 360p -> represented-pixel centered main-diagonal width-3 smear audit."""
...
OUT = ROOT / "qa/pass31_color_minimum_scale_represented_diagonal_smear_audit"
FRAMES = OUT / "frames"
PASS30 = ROOT / "qa/pass30_color_minimum_scale_represented_resampling_roundtrip_audit/extraction_receipt.json"
PASS23 = ROOT / "qa/pass23_minimum_scale_color_vision_audit/extraction_receipt.json"
PACKET_CREATED_AT = "2026-08-08T16:28:24+09:00"
...
def main() -> None:
    ...
    pass30 = json.loads(PASS30.read_text())
    pass23 = json.loads(PASS23.read_text())
    cuts = detect_cuts()
    expected = pass23["detected_cut_times_seconds"]
    if cuts != expected or cuts != pass30["detected_cut_times_seconds"] or len(cuts) != 15:
    ...
    for index in range(16):
        ...
        prior_clean = ROOT / pass30["scenes"][index]["native_clean"]
        identical = clean.read_bytes() == prior_clean.read_bytes()
        if not identical or sha(clean) != pass30["scenes"][index]["native_clean_sha256"]:
        ...
        for variant, baseline_name in VARIANTS.items():
            ...
            if not np.array_equal(derived, saved):
                raise SystemExit(f"smear mismatch scene {number} {variant}")
            exact_smears += 1
        ...
        scenes.append({"scene": number, "start_seconds": boundaries[index], "end_seconds": boundaries[index + 1], "midpoint_seconds": midpoint, "native_clean": str(clean.relative_to(ROOT)), "native_clean_sha256": sha(clean), "native_clean_byte_identical_to_pass30": identical, "samples": samples})
    sheets = {}
    ...
    receipt = {
        "audit": "native_monochrome_and_color_vision_then_minimum_scale_then_represented_pixel_main_diagonal_smear",
        "deepening_pass": 31,
        ...
        "transform": {"order": ["native color/monochrome presentation", "Pillow LANCZOS 640x360", "centered main-diagonal NW-SE width-3 box smear on represented pixels"], "axis": "main diagonal NW-SE", "kernel_offsets_yx": [[-1, -1], [0, 0], [1, 1]], "width_pixels_at_640x360": WIDTH, "edge_handling": "edge replication on both axes", "accumulator": "uint64", "rounding": "integer round half up", "pillow_version": pillow_version, "numpy_version": np.__version__, "python_version": platform.python_version(), "png_optimize": False},
        "source_receipts": {"pass30": str(PASS30.relative_to(ROOT)), "pass30_sha256": sha(PASS30), "pass23": str(PASS23.relative_to(ROOT)), "pass23_sha256": sha(PASS23)},
    ...
    print(f"PASS candidate={sha(CANDIDATE)} cuts={len(cuts)} scenes=16 frames={exact_smears} clean={fresh_clean}/16 baseline={baseline_matches}/80 smear={exact_smears}/80")
