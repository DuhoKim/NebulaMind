#!/usr/bin/env python3
"""Fresh native color/monochrome -> 360p -> represented-pixel centered anti-diagonal width-3 smear audit."""
...
OUT = ROOT / "qa/pass33_color_minimum_scale_represented_anti_diagonal_smear_audit"
FRAMES = OUT / "frames"
PASS31 = ROOT / "qa/pass31_color_minimum_scale_represented_diagonal_smear_audit/extraction_receipt.json"
PASS23 = ROOT / "qa/pass23_minimum_scale_color_vision_audit/extraction_receipt.json"
PACKET_CREATED_AT = "2026-08-08T16:46:11+09:00"
EXPECTED_SHA = "02fe11f0dd9bacc9a46ca2ec8b67764bd871ce2e2dab0f59990f36df11ee8431"
DURATION = 243.3
WIDTH = 3
VARIANTS = {
    "color_then_360p_then_represented_anti_diagonal_smear_w03": "color_360p",
    "grayscale_bt709_then_360p_then_represented_anti_diagonal_smear_w03": "grayscale_bt709_then_360p",
    "protanopia_machado100_then_360p_then_represented_anti_diagonal_smear_w03": "protanopia_machado100_then_360p",
    "deuteranopia_machado100_then_360p_then_represented_anti_diagonal_smear_w03": "deuteranopia_machado100_then_360p",
    "tritanopia_machado100_then_360p_then_represented_anti_diagonal_smear_w03": "tritanopia_machado100_then_360p",
}
MATRICES = {
...
def smear(a: np.ndarray) -> np.ndarray:
    ...
    totals = padded[0:height, 2:width + 2] + padded[1:height + 1, 1:width + 1] + padded[2:height + 2, 0:width]
...
def main() -> None:
    ...
    pass31 = json.loads(PASS31.read_text())
    pass23 = json.loads(PASS23.read_text())
    cuts = detect_cuts()
    expected = pass23["detected_cut_times_seconds"]
    if cuts != expected or cuts != pass31["detected_cut_times_seconds"] or len(cuts) != 15:
    ...
    for index in range(16):
        ...
        prior_clean = ROOT / pass31["scenes"][index]["native_clean"]
        identical = clean.read_bytes() == prior_clean.read_bytes()
        if not identical or sha(clean) != pass31["scenes"][index]["native_clean_sha256"]:
        ...
        scenes.append({"scene": number, "start_seconds": boundaries[index], "end_seconds": boundaries[index + 1], "midpoint_seconds": midpoint, "native_clean": str(clean.relative_to(ROOT)), "native_clean_sha256": sha(clean), "native_clean_byte_identical_to_pass31": identical, "samples": samples})
    sheets = {}
    ...
    receipt = {
        ...
        "transform": {"order": ["native color/monochrome presentation", "Pillow LANCZOS 640x360", "centered anti-diagonal NW-SE width-3 box smear on represented pixels"], "axis": "anti diagonal NE-SW", "kernel_offsets_yx": [[-1, 1], [0, 0], [1, -1]], "width_pixels_at_640x360": WIDTH, "edge_handling": "edge replication on both axes", "accumulator": "uint64", "rounding": "integer round half up", "pillow_version": pillow_version, "numpy_version": np.__version__, "python_version": platform.python_version(), "png_optimize": False},
        "source_receipts": {"pass31": str(PASS31.relative_to(ROOT)), "pass31_sha256": sha(PASS31), "pass23": str(PASS23.relative_to(ROOT)), "pass23_sha256": sha(PASS23)},
