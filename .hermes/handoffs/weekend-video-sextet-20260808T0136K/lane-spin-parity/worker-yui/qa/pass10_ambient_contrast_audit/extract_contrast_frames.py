#!/usr/bin/env python3
"""Fresh candidate midpoints plus deterministic reflected-light wash variants."""
...
def apply_black_lift(source: Path, amount: float, destination: Path) -> None:
...
def main() -> None:
    ...
    for scene_number, (start, end, midpoint) in enumerate(ranges, start=1):
        ...
        for label in VARIANTS:
            if label == "clean":
                ...
                apply_black_lift(clean_path, BLACK_LIFT[label], OUT / frame_rel)
    ...
    receipt = {
        ...
        "deepening_pass": 10,
        "audit": "uniform_reflected_light_black_lift_and_contrast_resilience",
        ...
        "transform_contract": {
            ...
            "uniform_black_lift_10pct": "decode sRGB to linear light; output linear = 0.10 + 0.90 * input linear; encode sRGB",
            "uniform_black_lift_20pct": "decode sRGB to linear light; output linear = 0.20 + 0.80 * input linear; encode sRGB",
            "uniform_black_lift_30pct": "decode sRGB to linear light; output linear = 0.30 + 0.70 * input linear; encode sRGB",
            "uniform_black_lift_40pct": "decode sRGB to linear light; output linear = 0.40 + 0.60 * input linear; encode sRGB",
            "rounding": "numpy float64; clip to [0,1]; round-to-nearest uint8 via rint",
        },
