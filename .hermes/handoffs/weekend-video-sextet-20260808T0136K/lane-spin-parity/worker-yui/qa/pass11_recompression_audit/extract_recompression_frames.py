#!/usr/bin/env python3
"""Fresh candidate midpoints plus deterministic JPEG 4:2:0 recompression variants."""
...
PREVIOUS_CLEAN = ROOT / "qa" / "pass10_ambient_contrast_audit" / "frames"
FPS = 30
SMALL_WIDTH = 160
SMALL_HEIGHT = 90
...
def detect_cuts() -> list[float]:
    ...
    for score, index in sorted(scores, reverse=True):
        if all(abs(index - prior_index) >= MIN_CUT_SEPARATION_FRAMES for _, prior_index in selected):
...
def midpoints(cuts: list[float]) -> list[float]:
    ...
    return [round((boundaries[index] + boundaries[index + 1]) / 2.0, 6) for index in range(16)]
...
def extract_clean(timestamp: float, destination: Path) -> None:
    ...
    with Image.open(destination) as image:
        if image.mode != "RGB" or image.size != (WIDTH, HEIGHT):
            raise RuntimeError(f"unexpected clean frame {destination.name}: {image.mode} {image.size}")


def jpeg_recompress(source: Path, destination: Path, quality: int) -> tuple[str, int]:
    ...
    return jpeg_hash, len(jpeg_bytes)
...
def make_contact_sheet(frame_paths: list[Path], variant: str, destination: Path) -> None:
    ...
    for index, frame_path in enumerate(frame_paths):
        ...
        draw.text((x + 10, y + thumb_h + 5), f"S{index + 1:02d} · {variant}", fill=(235, 240, 247), font=label_font)
...
def main() -> None:
    ...
    for scene_index, timestamp in enumerate(times, start=1):
        ...
        prior_clean = PREVIOUS_CLEAN / f"scene_{scene_index:02d}_clean.png"
        if not prior_clean.exists() or sha256(clean_path) != sha256(prior_clean):
            raise SystemExit(f"clean midpoint does not reproduce pass 10: scene {scene_index}")
        ...
        for variant, quality in VARIANTS.items():
            ...
            jpeg_hash, jpeg_size = jpeg_recompress(clean_path, output_path, quality)
            with Image.open(output_path) as image:
                if image.mode != "RGB" or image.size != (WIDTH, HEIGHT):
                    raise RuntimeError(f"unexpected derivative {output_path}: {image.mode} {image.size}")
            records.append(
                {
                    "scene": scene_index,
                    "timestamp_seconds": timestamp,
    ...
    receipt = {
        ...
        "cut_detection": {
            ...
            "cuts": cuts,
            "exact_pass10_match": True,
        },
        "scene_count": 16,
        "variant_count": len(VARIANTS),
        "frame_count": len(records),
        "variants": {
            "clean": {"quality": None, "subsampling": None},
            "jpeg_q85_420": {"quality": 85, "subsampling": "4:2:0"},
            "jpeg_q60_420": {"quality": 60, "subsampling": "4:2:0"},
            "jpeg_q35_420": {"quality": 35, "subsampling": "4:2:0"},
            "jpeg_q20_420": {"quality": 20, "subsampling": "4:2:0"},
        },
        "transform_contract": {
            "encoder": "Pillow JPEG",
            "color_mode": "RGB",
            ...
            "scope_limit": "representation stress only; JPEG quality values are library parameters, not claims about any named platform or delivery path",
        },
        "clean_reproduction": "16/16 pass10 clean midpoint PNGs byte-identical",
