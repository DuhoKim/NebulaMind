#!/usr/bin/env python3
"""Extract fresh scene midpoints and deterministic caption/UI obstruction variants."""
...
def make_sheet(label: str, rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    for index, row in enumerate(rows):
        ...
        if not isinstance(scene_number, int):
            raise TypeError("contact-sheet scene number must be an integer")
        with Image.open(frame).convert("RGB") as image:
            image.thumbnail((tile_w, tile_h), Image.Resampling.LANCZOS)
            x = (index % 4) * tile_w
            y = (index // 4) * tile_h
            sheet.paste(image, (x, y))
        draw.rectangle((x, y, x + 235, y + 28), fill=(4, 8, 14))
        draw.text((x + 8, y + 3), f"S{scene_number:02d} · {label}", fill=(242, 246, 252), font=label_font)
...
def main() -> None:
    if OUT.exists():
        for pattern in ("contact_sheet_*.png", "extraction_receipt.json"):
    ...
    for scene_number, (start, end, midpoint) in enumerate(ranges, start=1):
        ...
        with Image.open(clean_path) as image:
            if image.mode != "RGB" or image.size != (WIDTH, HEIGHT):
                raise SystemExit(f"invalid clean frame {clean_rel}: {image.mode} {image.size}")
        ...
        for label, fraction in VARIANTS.items():
            if label == "clean":
                ...
                mask_y = obstruct(clean_path, fraction, OUT / frame_rel)
            sample = {
                "variant": label,
                "occluded_bottom_fraction": fraction,
                "mask_start_y": mask_y,
                "frame": frame_rel.as_posix(),
                "frame_sha256": sha256(OUT / frame_rel),
                "width": WIDTH,
                "height": HEIGHT,
            }
    ...
    receipt = {
        ...
        "deepening_pass": 7,
        "audit": "caption_and_player_ui_obstruction",
        ...
        "variants": [
            {"label": label, "occluded_bottom_fraction": fraction}
            for label, fraction in VARIANTS.items()
        ],
        "scenes": scenes,
