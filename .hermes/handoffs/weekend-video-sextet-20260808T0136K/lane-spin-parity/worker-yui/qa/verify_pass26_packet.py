#!/usr/bin/env python3
"""Verify pass-26 color/minimum-scale/bottom-quarter-obstruction packet."""
...
CAND_ROOT = ROOT / "qa/pass26_color_minimum_scale_obstruction_audit"
...
QUANT = ROOT / "qa/pass26_color_minimum_scale_obstruction_quantitative_audit.json"
SNAPSHOT = ROOT / "qa/pass26_review_snapshot_v1.json"
GUARD = ROOT / "COLOR_MINIMUM_SCALE_OBSTRUCTION_GUARD_PASS26.json"
BLOCKER = ROOT / "BLOCKER_PACKET_PASS26.json"
AUDIT = ROOT / "PASS26_ENCODED_FRAME_AUDIT.md"
MASK_TOP_Y = 270
...
def obstruct(pixels: np.ndarray) -> np.ndarray:
    output = pixels.copy()
    output[MASK_TOP_Y:, :, :] = MASK_RGB
    return output
...
def verify_candidate(receipt: dict) -> None:
    ...
    require(receipt["deepening_pass"] == 26 and receipt["scene_count"] == 16, "candidate census")
    ...
    pass25 = load(ROOT / "qa/pass25_color_minimum_scale_black_lift_audit/extraction_receipt.json")
    require(receipt["detected_cut_times_seconds"] == pass25["detected_cut_times_seconds"], "cut timestamps")
    pass25_scenes = {int(row["scene"]): row for row in pass25["scenes"]}
    exact_derivatives = 0
    for scene in receipt["scenes"]:
        number = int(scene["scene"])
        clean = CAND_ROOT / scene["native_clean"]
        previous_clean = ROOT / "qa/pass25_color_minimum_scale_black_lift_audit" / pass25_scenes[number]["native_clean"]
        ...
        with Image.open(clean) as opened:
            ...
            for sample in scene["samples"]:
                ...
                with Image.open(frame) as output:
                    ...
                    size = output.size
                require(np.array_equal(obstruct(baseline), stored), f"candidate obstruction S{number} {sample['variant']}")
                require(bool(np.array_equal(baseline[:MASK_TOP_Y], stored[:MASK_TOP_Y]) and np.all(stored[MASK_TOP_Y:] == MASK_RGB)), f"candidate mask contract S{number}")
                require(sha(frame) == sample["frame_sha256"] and size == (640, 360), f"candidate frame receipt S{number}")
                exact_derivatives += 1
...
def verify_method(receipt: dict) -> None:
    require(receipt["deepening_pass"] == 26 and receipt["variant_order"] == EXPECTED_VARIANTS, "method contract")
            ...
            with Image.open(source) as opened:
                ...
                for sample in scene["samples"]:
                    ...
                    with Image.open(frame) as output:
                        stored = np.asarray(output.convert("RGB"), dtype=np.uint8)
                        size = output.size
                    require(np.array_equal(obstruct(baseline), stored), f"method obstruction {name} S{scene['scene']} {sample['variant']}")
                    require(bool(np.array_equal(baseline[:MASK_TOP_Y], stored[:MASK_TOP_Y]) and np.all(stored[MASK_TOP_Y:] == MASK_RGB)), f"method mask contract {name}")
                    require(sha(frame) == sample["frame_sha256"] and size == (640, 360), f"method receipt {name}")
                    exact += 1
...
def verify_evidence() -> None:
    ...
    require(quant["deepening_pass"] == 26 and quant["candidate_sha256"] == EXPECTED_CANDIDATE, "quant identity")
    ...
    require(not quant["representation_review"]["candidate_obstruction_repairs_or_authorizes"], "no candidate repair")
    require(quant["representation_review"]["sealed_v8_lower_boundary_occlusion"] == "S2_S3_S4_S5_S6_ALL_FIVE_VARIANTS", "sealed v8 occlusion finding")
    for variant, data in quant["candidate"]["aggregate"].items():
        require(data["structural_gate_scenes"] == 0 and data["unobstructed_pixel_exact_scenes"] == 16 and data["masked_pixel_exact_scenes"] == 16, f"candidate aggregate {variant}")
        require(data["observed_tokens_in_mask_zone"] == 0 and data["reference_tokens_in_mask_zone"] > 0, f"candidate mask-zone OCR {variant}")
    ...
    require(snapshot["status"] == "IMMUTABLE_REVIEW_SNAPSHOT" and snapshot["deepening_pass"] == 26, "snapshot identity")
    ...
    require(blocker["candidate"]["sha256"] == EXPECTED_CANDIDATE and not blocker["pass26_representation_finding"]["candidate_repaired_or_authorized"], "blocker candidate")
    external = {
    ...
    prohibited = [path for path in ROOT.rglob("pass26*") if path.suffix.lower() in {".mp4", ".mov", ".mkv", ".mp3", ".wav", ".m4a", ".aac"}]
    require(not prohibited, "pass26 prohibited audio/video output")


def verify_handoff() -> None:
    status = load(ROOT / "STATUS.json")
    require(status["receipt_marker"] == "SPIN_WORKER_YUI_DEEPENING_PASS26_COMPLETE", "status marker")
    require(status["pass26_color_minimum_scale_obstruction_audit_completed"], "status audit flag")
    require(status["pass26_review_snapshot_id"] == "spin-worker-yui-pass26-review-v1-20260808T144908K", "status snapshot")
    ...
    for text, marker in [(lane, "SPIN_WORKER_YUI_DEEPENING_PASS26_COMPLETE"), (integrator, "## Pass-26 latest request"), (static, "## Pass-26 cumulative QA")]:
        require(marker in text, f"handoff marker {marker}")
    require("PASS26_ENCODED_FRAME_AUDIT.md" in status["allowed_next_action"], "status next-action pointer")
...
def main() -> None:
    ...
    if not args.evidence_only:
        verify_handoff()
    print("PASS pass26_packet evidence=exact transforms=185/185 gates=7/7x5 blockers=unchanged handoff=" + ("skipped" if args.evidence_only else "verified"))
