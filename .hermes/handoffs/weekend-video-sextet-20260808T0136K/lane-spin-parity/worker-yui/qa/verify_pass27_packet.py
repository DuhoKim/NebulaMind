#!/usr/bin/env python3
"""Verify pass-27 color/minimum-scale/represented-defocus packet."""
...
CAND_ROOT = ROOT / "qa/pass27_color_minimum_scale_represented_defocus_audit"
CAND_RECEIPT = CAND_ROOT / "extraction_receipt.json"
METHOD_ROOT = ROOT / "qa/pass27_v8_color_minimum_scale_represented_defocus"
METHOD_RECEIPT = METHOD_ROOT / "receipt.json"
QUANT = ROOT / "qa/pass27_color_minimum_scale_represented_defocus_quantitative_audit.json"
SNAPSHOT = ROOT / "qa/pass27_review_snapshot_v1.json"
...
def main() -> None:
    ...
    require(candidate["fresh_clean_match_count"] == 16 and candidate["pass23_baseline_match_count"] == 80 and candidate["exact_blur_recomputation_count"] == 80, "candidate custody census")

    exact_candidate = 0
    for scene in candidate["scenes"]:
        clean = ROOT / scene["native_clean"]
        require(sha(clean) == scene["native_clean_sha256"] and scene["native_clean_byte_identical_to_pass26"], f"clean S{scene['scene']}")
        ...
        for sample in scene["samples"]:
            ...
            require(np.array_equal(blur(recomputed), stored), f"candidate blur S{scene['scene']} {sample['variant']}")
    ...
    require(method["frame_count"] == 105 and method["pass23_baseline_match_count"] == 105 and method["exact_blur_recomputation_count"] == 105, "method census")
    exact_method = 0
    for name, group in method["groups"].items():
        ...
        for scene in group["scenes"]:
            ...
            for sample in scene["samples"]:
                ...
                require(np.array_equal(blur(recomputed), stored), f"method blur {name}")
    ...
    require(quant["deepening_pass"] == 27 and quant["human_review"]["candidate"]["structural_held_gate_scenes"] == "0/16_ALL_FIVE_VARIANTS", "quant identity")
    expected = {
        "color_then_360p_then_represented_gaussian_r0_50": (0.986207, 0.943368, 0.845070),
        "grayscale_bt709_then_360p_then_represented_gaussian_r0_50": (0.986301, 0.943485, 0.752475),
        "tritanopia_machado100_then_360p_then_represented_gaussian_r0_50": (0.967532, 0.911504, 0.685315),
    }
    for variant, values in expected.items():
        row = quant["candidate"][variant]
        require((row["headline_recall"], row["full_recall"], row["lower_recall"]) == values and row["structural_gate_scenes"] == 0 and row["exact_blur_frames"] == 16, f"metrics {variant}")
    for variant in VARIANTS:
        proof = quant["method_groups"]["pass12_sharpness_safe"][variant]
        require(proof["mapped_gate_threshold_passes"] == 7 and proof["exact_blur_frames"] == 7 and proof["mean_mapped_gate_similarity"] >= 0.9721, f"proof {variant}")
    ...
    guard = json.loads((ROOT / "COLOR_MINIMUM_SCALE_REPRESENTED_DEFOCUS_GUARD_PASS27.json").read_text())
    blocker = json.loads((ROOT / "BLOCKER_PACKET_PASS27.json").read_text())
    require(guard["guard_id"] == "spin-worker-yui-pass27-color-minimum-scale-represented-defocus-20260808T151015K" and not guard["disposition"]["new_pixel_or_copy_correction_requested"], "guard")
    require(blocker["packet_id"] == "spin-worker-yui-pass27-blockers-20260808T151015K" and all(item["status"] == "OPEN_UNCHANGED" for item in blocker["blockers"]), "blockers")
    require(snapshot["snapshot_id"] == "spin-worker-yui-pass27-review-v1-20260808T151015K" and snapshot["custody"]["prior_pass_snapshot"]["sha256"] == "826d418a55b315fb66a7b2c93015aed6c0eeee20f4c00950cd1ec74507075243", "snapshot")
    for label, item in snapshot["evidence"].items():
        require(sha(ROOT / item["path"]) == item["sha256"], f"snapshot evidence {label}")
    prohibited = {".mp4", ".mov", ".m4a", ".mp3", ".wav", ".aac", ".webm"}
    pass27_files = [p for p in ROOT.rglob("*pass27*") if p.is_file()]
    require(not any(path.suffix.lower() in prohibited for path in pass27_files), "prohibited media")
    ...
    if not args.evidence_only:
        ...
        require(status["phase"] == "SEALED_ISOLATED_DEEPENING_PASS27_COLOR_MINIMUM_SCALE_REPRESENTED_DEFOCUS_GUARD_V1", "status phase")
        require(status["receipt_marker"] == "SPIN_WORKER_YUI_DEEPENING_PASS27_COMPLETE" and not status["video_reportable_now"], "status marker")
        require("SPIN_WORKER_YUI_DEEPENING_PASS27_COMPLETE" in receipt and "PASS27_ENCODED_FRAME_AUDIT.md" in receipt, "lane receipt")
        require("Pass-27 latest request" in integrator and "COLOR_MINIMUM_SCALE_REPRESENTED_DEFOCUS_GUARD_PASS27.json" in integrator, "integrator handoff")
        require("Pass-27 cumulative QA" in static and "COLOR_MINIMUM_SCALE_REPRESENTED_DEFOCUS_GUARD_PASS27.json" in static, "static handoff")
        handoff = "verified"
    print(f"PASS pass27_packet evidence=exact transforms={exact_candidate + exact_method}/185 gates=7/7x5 blockers=unchanged handoff={handoff}")
