#!/usr/bin/env python3
"""Verify pass-28 color/minimum-scale/represented-smear packet."""
...
QUANT = ROOT / "qa/pass28_color_minimum_scale_represented_smear_quantitative_audit.json"
SNAPSHOT = ROOT / "qa/pass28_review_snapshot_v1.json"
EXPECTED_CANDIDATE = "02fe11f0dd9bacc9a46ca2ec8b67764bd871ce2e2dab0f59990f36df11ee8431"
WIDTH = 3
VARIANTS = {
    "color_then_360p_then_represented_horizontal_smear_w03": "color_360p",
    "grayscale_bt709_then_360p_then_represented_horizontal_smear_w03": "grayscale_bt709_then_360p",
    "protanopia_machado100_then_360p_then_represented_horizontal_smear_w03": "protanopia_machado100_then_360p",
    "deuteranopia_machado100_then_360p_then_represented_horizontal_smear_w03": "deuteranopia_machado100_then_360p",
    "tritanopia_machado100_then_360p_then_represented_horizontal_smear_w03": "tritanopia_machado100_then_360p",
}
MATRICES = {
...
def main() -> None:
    ...
    for scene in candidate["scenes"]:
        ...
        require(sha(clean) == scene["native_clean_sha256"] and scene["native_clean_byte_identical_to_pass27"], f"clean S{scene['scene']}")
    ...
    require(quant["deepening_pass"] == 28 and quant["human_review"]["candidate"]["structural_held_gate_scenes"] == "0/16_ALL_FIVE_VARIANTS", "quant identity")
    expected = {
        "color_then_360p_then_represented_horizontal_smear_w03": (0.924138, 0.736215, 0.105634, 0.536725, 0.842925),
        "grayscale_bt709_then_360p_then_represented_horizontal_smear_w03": (0.910959, 0.706436, 0.099010, 0.536574, 0.842648),
        "tritanopia_machado100_then_360p_then_represented_horizontal_smear_w03": (0.850649, 0.696165, 0.258741, 0.535701, 0.843326),
    }
    ...
    proof_expected = {
        "color_then_360p_then_represented_horizontal_smear_w03": (7, 0.911576),
        "grayscale_bt709_then_360p_then_represented_horizontal_smear_w03": (4, 0.827568),
        "protanopia_machado100_then_360p_then_represented_horizontal_smear_w03": (5, 0.864051),
        "deuteranopia_machado100_then_360p_then_represented_horizontal_smear_w03": (5, 0.843920),
        "tritanopia_machado100_then_360p_then_represented_horizontal_smear_w03": (5, 0.830960),
    }
    ...
    guard = json.loads((ROOT / "COLOR_MINIMUM_SCALE_REPRESENTED_SMEAR_GUARD_PASS28.json").read_text())
    blocker = json.loads((ROOT / "BLOCKER_PACKET_PASS28.json").read_text())
    require(guard["guard_id"] == "spin-worker-yui-pass28-color-minimum-scale-represented-smear-20260808T153230K" and guard["evidence_backed_action"] == "ADOPT_GUARD_NOT_PIXEL_OR_COPY_CORRECTION", "guard")
    require(blocker["packet_id"] == "spin-worker-yui-pass28-blockers-20260808T153230K" and all(item["status"] == "UNCHANGED_BLOCKER" for item in blocker["blocking_facts"]), "blockers")
    require(snapshot["snapshot_id"] == "spin-worker-yui-pass28-review-v1-20260808T153230K" and snapshot["baseline_receipts"]["qa/pass27_review_snapshot_v1.json"] == "c351931842f3a522bc7dbee8764a71ffadb92ac264f731c37dd9e860af592433", "snapshot")
    for path, digest in snapshot["pass28_artifacts"].items():
        require(sha(ROOT / path) == digest, f"snapshot artifact {path}")
    prohibited = {".mp4", ".mov", ".m4a", ".mp3", ".wav", ".aac", ".webm"}
    pass28_files = [p for p in ROOT.rglob("*pass28*") if p.is_file()]
    require(not any(path.suffix.lower() in prohibited for path in pass28_files), "prohibited media")
    ...
    if not args.evidence_only:
        ...
        require(status["phase"] == "SEALED_ISOLATED_DEEPENING_PASS28_COLOR_MINIMUM_SCALE_REPRESENTED_SMEAR_GUARD_V1", "status phase")
        require(status["receipt_marker"] == "SPIN_WORKER_YUI_DEEPENING_PASS28_COMPLETE" and not status["video_reportable_now"], "status marker")
        require("SPIN_WORKER_YUI_DEEPENING_PASS28_COMPLETE" in receipt and "PASS28_ENCODED_FRAME_AUDIT.md" in receipt, "lane receipt")
        require("Pass-28 latest request" in integrator and "COLOR_MINIMUM_SCALE_REPRESENTED_SMEAR_GUARD_PASS28.json" in integrator, "integrator handoff")
        require("Pass-28 cumulative QA" in static and "COLOR_MINIMUM_SCALE_REPRESENTED_SMEAR_GUARD_PASS28.json" in static, "static handoff")
        handoff = "verified"
    print(f"PASS pass28_packet evidence=exact transforms={exact_candidate + exact_method}/185 gates=human7/7x5 ocr_min=4/7 blockers=unchanged handoff={handoff}")
