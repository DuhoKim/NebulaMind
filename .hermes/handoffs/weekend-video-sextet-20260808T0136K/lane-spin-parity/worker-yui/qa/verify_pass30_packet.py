#!/usr/bin/env python3
"""Verify pass-30 color/minimum-scale/represented-bilinear-resampling-round-trip packet."""
...
CAND_ROOT = ROOT / "qa/pass30_color_minimum_scale_represented_resampling_roundtrip_audit"
CAND_RECEIPT = CAND_ROOT / "extraction_receipt.json"
METHOD_ROOT = ROOT / "qa/pass30_v8_color_minimum_scale_represented_resampling_roundtrip"
METHOD_RECEIPT = METHOD_ROOT / "receipt.json"
QUANT = ROOT / "qa/pass30_color_minimum_scale_represented_resampling_roundtrip_quantitative_audit.json"
SNAPSHOT = ROOT / "qa/pass30_review_snapshot_v1.json"
...
def main() -> None:
    ...
    require(candidate["fresh_clean_match_count"] == 16 and candidate["pass23_baseline_match_count"] == 80 and candidate["exact_resampling_roundtrip_recomputation_count"] == 80, "candidate custody census")

    exact_candidate = 0
    for scene in candidate["scenes"]:
        clean = ROOT / scene["native_clean"]
        require(sha(clean) == scene["native_clean_sha256"] and scene["native_clean_byte_identical_to_pass29"], f"clean S{scene['scene']}")
        ...
        for sample in scene["samples"]:
            ...
            require(np.array_equal(resampling_roundtrip(recomputed), stored), f"candidate resampling round trip S{scene['scene']} {sample['variant']}")
    ...
    require(method["frame_count"] == 105 and method["pass23_baseline_match_count"] == 105 and method["exact_resampling_roundtrip_recomputation_count"] == 105, "method census")
    exact_method = 0
    for name, group in method["groups"].items():
        ...
        for scene in group["scenes"]:
            ...
            for sample in scene["samples"]:
                ...
                require(np.array_equal(resampling_roundtrip(recomputed), stored), f"method resampling round trip {name}")
    ...
    require(quant["deepening_pass"] == 30 and quant["human_review"]["candidate"]["structural_held_gate_scenes"] == "0/16_ALL_FIVE_VARIANTS", "quant identity")
    expected = {
        "color_then_360p_then_represented_bilinear_roundtrip_512x288_to_640x360": (0.965517, 0.870343, 0.443662, 0.569166, 0.580139),
        "grayscale_bt709_then_360p_then_represented_bilinear_roundtrip_512x288_to_640x360": (0.952055, 0.835165, 0.267327, 0.569134, 0.579754),
        "tritanopia_machado100_then_360p_then_represented_bilinear_roundtrip_512x288_to_640x360": (0.961039, 0.856932, 0.391608, 0.568743, 0.580992),
    }
    for variant, values in expected.items():
        row = quant["candidate"][variant]
        require((row["headline_recall"], row["full_recall"], row["lower_recall"], row["horizontal_luma_gradient_energy_ratio"], row["vertical_luma_gradient_energy_ratio"]) == values and row["structural_gate_scenes"] == 0 and row["exact_resampling_roundtrip_frames"] == 16, f"metrics {variant}")
    proof_expected = {
        "color_then_360p_then_represented_bilinear_roundtrip_512x288_to_640x360": (4, 0.758973),
        "grayscale_bt709_then_360p_then_represented_bilinear_roundtrip_512x288_to_640x360": (4, 0.783481),
        "protanopia_machado100_then_360p_then_represented_bilinear_roundtrip_512x288_to_640x360": (5, 0.841171),
        "deuteranopia_machado100_then_360p_then_represented_bilinear_roundtrip_512x288_to_640x360": (4, 0.755511),
        "tritanopia_machado100_then_360p_then_represented_bilinear_roundtrip_512x288_to_640x360": (4, 0.79536),
    }
    for variant, values in proof_expected.items():
        proof = quant["method_groups"]["pass12_sharpness_safe"][variant]
        require((proof["mapped_gate_threshold_passes"], proof["mean_mapped_gate_similarity"]) == values and proof["exact_resampling_roundtrip_frames"] == 7, f"proof {variant}")
    ...
    guard = json.loads((ROOT / "COLOR_MINIMUM_SCALE_REPRESENTED_RESAMPLING_ROUNDTRIP_GUARD_PASS30.json").read_text())
    blocker = json.loads((ROOT / "BLOCKER_PACKET_PASS30.json").read_text())
    require(guard["guard_id"] == "spin-worker-yui-pass30-color-minimum-scale-represented-resampling-roundtrip-20260808T161354K" and guard["evidence_backed_action"] == "ADOPT_GUARD_ONLY_NO_NEW_PIXEL_OR_COPY_CORRECTION", "guard")
    require(blocker["packet_id"] == "spin-worker-yui-pass30-blockers-20260808T161354K" and all(item["status"] == "OPEN_UNCHANGED" for item in blocker["blocking_facts"]), "blockers")
    require(snapshot["snapshot_id"] == "spin-worker-yui-pass30-review-v1-20260808T161354K" and snapshot["baseline_receipts"]["qa/pass29_review_snapshot_v1.json"] == "d5ea947087d63b765c742467c95beb0bf7f9ec5da4cf608e7bf7957fc53df229", "snapshot")
    for path, digest in snapshot["pass30_artifacts"].items():
        require(sha(ROOT / path) == digest, f"snapshot artifact {path}")
    prohibited = {".mp4", ".mov", ".m4a", ".mp3", ".wav", ".aac", ".webm"}
    pass30_files = [p for p in ROOT.rglob("*pass30*") if p.is_file()]
    require(not any(path.suffix.lower() in prohibited for path in pass30_files), "prohibited media")
    ...
    if not args.evidence_only:
        ...
        require(status["phase"] == "SEALED_ISOLATED_DEEPENING_PASS30_COLOR_MINIMUM_SCALE_REPRESENTED_RESAMPLING_ROUNDTRIP_GUARD_V1", "status phase")
        require(status["receipt_marker"] == "SPIN_WORKER_YUI_DEEPENING_PASS30_COMPLETE" and not status["video_reportable_now"], "status marker")
        require("SPIN_WORKER_YUI_DEEPENING_PASS30_COMPLETE" in receipt and "PASS30_ENCODED_FRAME_AUDIT.md" in receipt, "lane receipt")
        require("Pass-30 latest request" in integrator and "COLOR_MINIMUM_SCALE_REPRESENTED_RESAMPLING_ROUNDTRIP_GUARD_PASS30.json" in integrator, "integrator handoff")
        require("Pass-30 cumulative QA" in static and "COLOR_MINIMUM_SCALE_REPRESENTED_RESAMPLING_ROUNDTRIP_GUARD_PASS30.json" in static, "static handoff")
        handoff = "verified"
    print(f"PASS pass30_packet evidence=exact transforms={exact_candidate + exact_method}/185 gates=human7/7x5 ocr_min=4/7 blockers=unchanged handoff={handoff}")
