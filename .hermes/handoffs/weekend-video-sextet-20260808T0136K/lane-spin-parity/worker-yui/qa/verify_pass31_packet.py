#!/usr/bin/env python3
"""Verify pass-31 color/minimum-scale/represented-main-diagonal-smear packet."""
...
CAND_ROOT = ROOT / "qa/pass31_color_minimum_scale_represented_diagonal_smear_audit"
CAND_RECEIPT = CAND_ROOT / "extraction_receipt.json"
METHOD_ROOT = ROOT / "qa/pass31_v8_color_minimum_scale_represented_diagonal_smear"
METHOD_RECEIPT = METHOD_ROOT / "receipt.json"
QUANT = ROOT / "qa/pass31_color_minimum_scale_represented_diagonal_smear_quantitative_audit.json"
SNAPSHOT = ROOT / "qa/pass31_review_snapshot_v1.json"
EXPECTED_CANDIDATE = "02fe11f0dd9bacc9a46ca2ec8b67764bd871ce2e2dab0f59990f36df11ee8431"
WIDTH = 3
VARIANTS = {
    "color_then_360p_then_represented_main_diagonal_smear_w03": "color_360p",
    "grayscale_bt709_then_360p_then_represented_main_diagonal_smear_w03": "grayscale_bt709_then_360p",
    "protanopia_machado100_then_360p_then_represented_main_diagonal_smear_w03": "protanopia_machado100_then_360p",
    "deuteranopia_machado100_then_360p_then_represented_main_diagonal_smear_w03": "deuteranopia_machado100_then_360p",
    "tritanopia_machado100_then_360p_then_represented_main_diagonal_smear_w03": "tritanopia_machado100_then_360p",
}
MATRICES = {
...
def smear(a: np.ndarray) -> np.ndarray:
    pad = WIDTH // 2
    padded = np.pad(a.astype(np.uint64), ((pad, pad), (pad, pad), (0, 0)), mode="edge")
    height, width = a.shape[:2]
    totals = padded[0:height, 0:width] + padded[1:height + 1, 1:width + 1] + padded[2:height + 2, 2:width + 2]
...
def main() -> None:
    ...
    for scene in candidate["scenes"]:
        ...
        require(sha(clean) == scene["native_clean_sha256"] and scene["native_clean_byte_identical_to_pass30"], f"clean S{scene['scene']}")
    ...
    require(quant["deepening_pass"] == 31 and quant["human_review"]["candidate"]["structural_held_gate_scenes"] == "0/16_ALL_FIVE_VARIANTS", "quant identity")
    expected = {
        "color_then_360p_then_represented_main_diagonal_smear_w03": (0.944828, 0.721311, 0.183099, 0.627436, 0.664549, 0.511647, 0.671406),
        "grayscale_bt709_then_360p_then_represented_main_diagonal_smear_w03": (0.917808, 0.667190, 0.079208, 0.627017, 0.664212, 0.511778, 0.671028),
        "tritanopia_machado100_then_360p_then_represented_main_diagonal_smear_w03": (0.902597, 0.703540, 0.209790, 0.626804, 0.664981, 0.510747, 0.670961),
    }
    for variant, values in expected.items():
        row = quant["candidate"][variant]
        require((row["headline_recall"], row["full_recall"], row["lower_recall"], row["horizontal_luma_gradient_energy_ratio"], row["vertical_luma_gradient_energy_ratio"], row["main_diagonal_luma_gradient_energy_ratio"], row["anti_diagonal_luma_gradient_energy_ratio"]) == values and row["structural_gate_scenes"] == 0 and row["exact_smear_frames"] == 16, f"metrics {variant}")
    proof_expected = {
        "color_then_360p_then_represented_main_diagonal_smear_w03": (1, 0.607436),
        "grayscale_bt709_then_360p_then_represented_main_diagonal_smear_w03": (1, 0.647995),
        "protanopia_machado100_then_360p_then_represented_main_diagonal_smear_w03": (1, 0.602912),
        "deuteranopia_machado100_then_360p_then_represented_main_diagonal_smear_w03": (1, 0.654189),
        "tritanopia_machado100_then_360p_then_represented_main_diagonal_smear_w03": (1, 0.595801),
    }
    ...
    guard = json.loads((ROOT / "COLOR_MINIMUM_SCALE_REPRESENTED_DIAGONAL_SMEAR_GUARD_PASS31.json").read_text())
    blocker = json.loads((ROOT / "BLOCKER_PACKET_PASS31.json").read_text())
    require(guard["guard_id"] == "spin-worker-yui-pass31-color-minimum-scale-represented-diagonal-smear-20260808T163239K" and guard["evidence_backed_action"] == "ADOPT_GUARD_ONLY_NO_NEW_PIXEL_OR_COPY_CORRECTION", "guard")
    require(blocker["packet_id"] == "spin-worker-yui-pass31-blockers-20260808T163239K" and all(item["status"] == "OPEN_UNCHANGED" for item in blocker["blocking_facts"]), "blockers")
    require(snapshot["snapshot_id"] == "spin-worker-yui-pass31-review-v1-20260808T163239K" and snapshot["baseline_receipts"]["qa/pass30_review_snapshot_v1.json"] == "916aecd4b143ce641303b218efebe4c398cb19d47612d90152ec7331537c1826", "snapshot")
    for path, digest in snapshot["pass31_artifacts"].items():
        require(sha(ROOT / path) == digest, f"snapshot artifact {path}")
    prohibited = {".mp4", ".mov", ".m4a", ".mp3", ".wav", ".aac", ".webm"}
    pass31_files = [p for p in ROOT.rglob("*pass31*") if p.is_file()]
    require(not any(path.suffix.lower() in prohibited for path in pass31_files), "prohibited media")
    ...
    if not args.evidence_only:
        ...
        require(status["phase"] == "SEALED_ISOLATED_DEEPENING_PASS31_COLOR_MINIMUM_SCALE_REPRESENTED_DIAGONAL_SMEAR_GUARD_V1", "status phase")
        require(status["receipt_marker"] == "SPIN_WORKER_YUI_DEEPENING_PASS31_COMPLETE" and not status["video_reportable_now"], "status marker")
        require("SPIN_WORKER_YUI_DEEPENING_PASS31_COMPLETE" in receipt and "PASS31_ENCODED_FRAME_AUDIT.md" in receipt, "lane receipt")
        require("Pass-31 latest request" in integrator and "COLOR_MINIMUM_SCALE_REPRESENTED_DIAGONAL_SMEAR_GUARD_PASS31.json" in integrator, "integrator handoff")
        require("Pass-31 cumulative QA" in static and "COLOR_MINIMUM_SCALE_REPRESENTED_DIAGONAL_SMEAR_GUARD_PASS31.json" in static, "static handoff")
        handoff = "verified"
    print(f"PASS pass31_packet evidence=exact transforms={exact_candidate + exact_method}/185 gates=human7/7x5 ocr_min=1/7 blockers=unchanged handoff={handoff}")
