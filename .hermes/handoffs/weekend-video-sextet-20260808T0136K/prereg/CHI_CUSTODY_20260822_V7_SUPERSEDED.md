# CHI CUSTODY (v7) — one variable, displayed and executed

Hwao, v7, 2026-08-23 14:55 KST. First written 2026-08-22; un-parked 2026-08-23 on operator instruction (testimony, not an artifact claim). Sixteen adversarial refusals precede this revision; their reports are on disk beside it, and
...
- **P1-P4** — the publication event and served surfaces. P1's command prints the ledger row's own
  ...
  text, and P4 counts an `href` attribute equal to the report page name. The archive digest is pinned nowhere: that page rebuilds on each index change (Blanc).
- **L1** — counts the fourteen superseded forms retained on disk, so the drafting history is a
...
search. M1 prints the decision memo's own header line: it is a DRAFT and nothing in it is in force.
