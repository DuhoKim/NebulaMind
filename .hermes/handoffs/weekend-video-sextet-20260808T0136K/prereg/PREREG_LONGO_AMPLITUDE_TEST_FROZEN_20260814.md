# PREREGISTRATION (ASSEMBLED FREEZE CANDIDATE) — LONGO-AMPLITUDE TEST
...
**Lana (science / claim-boundary seat), 2026-08-14. Status: FREEZE CANDIDATE — supersedes the
2026-08-12 draft as the working text; the 08-12 draft survives byte-for-byte as the superseded
record (`PREREG_LONGO_AMPLITUDE_TEST_20260812.md`, SHA-256 `ac43490054b159610385b8faac28dc4e…`).
This candidate becomes the preregistration only when sha-pinned after Kun's freeze-gate PASS.
Nothing is frozen, published, accepted, committed, or pushed by writing it; Duho owns acceptance.**
...
**K-8 timing statement, explicit:** no real-sky statistic has been computed anywhere in this
program — no chirality label, no sky estimand, no unblinding of any kind. This amendment is
therefore made at the only safe time, before the run. It is not, and must never be described as,
a post-hoc change. (Altering the §F-10 output boundary *after* any real-sky statistic falls under
...
TEXT REPAIRS; `KUN_AMENDMENT_GATE_20260814.md`) → **this repaired candidate (three repairs: BS-11
filled, cumulative-release policy bound at F-10.f, custody + ACCEPT semantics at F-10.f/i)** → Kun
final exact-hash confirmation → Duho.
...
## 5. Hand-check attenuation protocol (carried; HC-4 and HC-6 amended)
...
- **HC-6 Power gate (amended wording per Kun's BS-8 ruling):** the pinned harness
  ...
  re-evaluated by the same analytical method at the hand-checked a before unblinding**; failure →
...
Wilson intervals; never the 500 rows). P8 covariate battery outputs (L_C values, AUC, LR, Holm
