# PREREGISTRATION (v2 FREEZE CANDIDATE) — LONGO-AMPLITUDE TEST
...
## — and amended to the one-human hand-check protocol HC-1H

**Lana (science / claim-boundary seat), 2026-08-15. Status: v2 FREEZE CANDIDATE — the 08-14
frozen text with the accepted HC-1H amendment (§5) applied, and nothing else changed.
Supersession chain — the two predecessor hashes are embedded here; this candidate cannot carry
...
3. **This candidate** — the working text; becomes the preregistration only when sha-pinned after
   Kun's freeze-gate PASS. The applied amendment is my own line-by-line amendment A1–A5 from
   `LANA_ONE_HUMAN_ATTENUATION_20260814.md` (SHA-256
   `b2590e4213e225f9869fe782cfe0f55d8d8979dcb470752836a5cd31a58453fd`), accepted by Duho at 01:08
   KST 2026-08-15 (`HC1H_ACCEPTANCE_20260815.md`) and closed by Kun
...
**K-8 timing statement, explicit (re-affirmed for the HC-1H amendment, 2026-08-15):** no
real-sky statistic has been computed anywhere in this program — no chirality label, no sky
estimand, no unblinding of any kind. Both the output-package amendment and the HC-1H hand-check
amendment are therefore made at the only safe time, before the run. Neither is, or must ever be
described as, a post-hoc change. (Altering the §F-10 output boundary or the §5 hand-check
protocol *after* any real-sky statistic falls under K-8 and voids the run.)

**Changes in this candidate relative to the 08-14 frozen text, complete list:** (1) amendment
...
2026-08-15** → **this v2 candidate (amendment A1–A5 applied to §5)** → Kun freeze gate → Duho.
...
## 6. Pixel-path custody and negative controls (carried unchanged)

PC-1…PC-5 and NC-1…NC-6 carry verbatim from the 08-12 draft, including: delivered pixels + WCS as
measurement input; per-object parity logging; **distortion branch declared at BS-7 (filled):
FAIL_CLOSED on SIP/PV/CPDIS/DET2IM — the local-Jacobian branch is not selected**; injection
battery (≥ 1,000 sources, 100% signed recovery, silent-flip and scrambled-WCS controls must be
detected); C1 mirror-run count-swap and D̂-negation (aggregate by construction); C2–C6 as frozen.
The secondary-instrument warning of §3 I-2 applies to any C-battery use of the secondary.
