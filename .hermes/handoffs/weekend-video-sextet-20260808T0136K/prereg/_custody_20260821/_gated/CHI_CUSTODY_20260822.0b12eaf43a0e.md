...
Hwao, v6, 2026-08-23 14:55 KST. First written 2026-08-22; un-parked 2026-08-23 on operator instruction (testimony, not an artifact claim). Fifteen adversarial refusals precede this revision; their reports are on disk beside it, and
...
- **P1-P4** — the publication event and served surfaces. P1's command prints the ledger row's own
  ...
  page. P3 and P4 count the three-value string and the report href INSIDE the archive's
  `<li data-src=...231235...>` element, closed at its own `</li>`. The archive digest is pinned nowhere: that page rebuilds on each index change (Blanc).
- **L1** — counts the thirteen superseded forms retained on disk, so the drafting history is a
