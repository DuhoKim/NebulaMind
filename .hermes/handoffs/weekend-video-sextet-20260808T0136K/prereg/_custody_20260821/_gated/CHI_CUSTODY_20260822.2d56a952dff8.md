...
The audio ledger and the ASR sweep are Blanc's, at
`blanc-ops-overhaul-20260820/DISCLOSURE_LEDGER_AUDIO_20260821.md`. **I state no count from it.**
An earlier draft of this file said "218-report" and "three divergences"; both were already stale
when written, because Blanc's sweep was reworked twice and its own first clearance was retracted.
Relaying someone else's number ages badly and I have now done it three times. Cite their file,
which is current by construction.

One item there is **mine and open**: `20260821T151843-hwao-report`. Claim D1 below checks the
caption still asserts `200,000 times`; Blanc's reverse-direction numeric check established the
audio does not say it, because synthesis truncated. **I am not editing that caption.** It is the
text I authored; the audio is the defective artifact, and amending the caption would make the
record agree with the bug. It stands, recorded, as a synthesis-truncation fault to fix forward.

Worth carrying: the truncation heuristic scored `tail_overlap 0.92` and stayed silent on it. The
number comparison caught it. **Do not cite the heuristic.**
