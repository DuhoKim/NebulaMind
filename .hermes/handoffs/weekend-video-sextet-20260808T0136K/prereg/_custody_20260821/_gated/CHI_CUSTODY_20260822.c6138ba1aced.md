# CHI CUSTODY (v9) — one variable, displayed and executed

Hwao, v9, 2026-08-23 16:23 KST. First written 2026-08-22; un-parked 2026-08-23 on operator instruction (testimony, not an artifact claim). Eighteen adversarial refusals precede this revision; their reports are on disk beside it, and
...
- **D1-D3** — my open divergence, unrepaired on purpose. D1-D2 print the caption's
  `200,000 times` phrase and the alignment coverage 0.9709. D3 counts this mp3's full digest in
  the R7 gate file (printed count 2). What those occurrences say is read at the file; the count
  ties the documents, not the content.
- **P1-P4** — the publication event and served pages. P1 is the ledger owner's own predicate:
  full-filename equality, matching publish rows counted in full with the count printed first, the
  row's `backfilled=True` shown. P2 pins the report page's digest. P3 counts the three-value
  literal in that SAME page — one report per page, association by construction (the surface
  owner's suggestion, after three gates refuted archive entry-binding). P4 counts occurrences of
  the href attribute literal `href="report-…231235….html"` in the archive page's source — a
  source-substring count in a mutable page, described as that and no more.
- **L1** — counts the sixteen superseded forms retained on disk, so the drafting history is a
  number a stranger can recompute rather than a retention promise.
- **M1** — prints line 5 of the decision memo, which is its DRAFT banner, at its position.
