#!/bin/zsh
# Executable custody claims, v11 (final round by Duho's decision). THE PRINTED COMMAND IS THE EXECUTED COMMAND:
# claim() takes one string, eval-runs exactly it, and prints exactly it. Twenty gate
...
claim D3 "grep -c \"\$(shasum -a 256 $R/20260821T151843-hwao-report.mp3 | cut -d' ' -f1)\" $P/GATE_CHI_CUSTODY_R7_20260821.md" 2

echo "\n== the decision memo's own header =="
claim M1 "sed -n 5p $P/DECISION_MEMO_DECLINE_TO_PROCEED_20260821.md" "> **THIS IS A DRAFT. NOTHING IN IT IS IN FORCE.**"

echo "\n== publication event and served surfaces =="
claim P1 "python3 -c \"
import json
rows=[r for r in map(json.loads,open('$R/queue_ledger.jsonl'))
      if r.get('event')=='publish' and r.get('file')=='20260820T231235-hwao-report.mp3']
print(len(rows), rows[0].get('seq'), rows[0].get('stamp_kst'), 'backfilled='+str(rows[0].get('backfilled')))\"" "1 20 2026-08-20 23:12:51 KST backfilled=True"
claim P2 "shasum -a 256 $R/report-20260820T231235-hwao-report.html | cut -c1-16" 050a3f6245fc74f1
claim P3 "python3 -c \"print(open('$R/report-20260820T231235-hwao-report.html',encoding='utf-8',errors='replace').read().count('0.834336, 0.384410, and -0.640352'))\"" 1
claim P4 "python3 -c \"print(open('$R/archive.html',encoding='utf-8',errors='replace').read().count('href=\\\"report-20260820T231235-hwao-report.html\\\"'))\"" 1

echo "\n== superseded forms retained on disk (the drafting history, countable) =="
claim L1 "ls $P/CHI_CUSTODY_RECEIPT_20260821*.md $P/CHI_CUSTODY_20260822_V*_SUPERSEDED.md | wc -l | tr -d ' '" 18
