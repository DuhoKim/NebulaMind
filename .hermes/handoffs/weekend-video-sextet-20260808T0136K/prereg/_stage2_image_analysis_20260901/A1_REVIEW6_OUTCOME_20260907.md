...
Two consequences. The assembler was extended to require this per GOVERNING ARTEFACT (A1 and `run_path.py`) rather than once globally, and that change is real. **CORRECTION (2026-09-07 18:2x): the second consequence — a review with an access proof over A1's current bytes — was written here as "is dispatched" when it had NOT been dispatched.** I described an intention in the past tense while only the assembler change had been made; no such job existed, and the process table confirmed none was running. It is dispatched now, and this correction stays in place rather than being edited away — a record that says work happened is worth nothing if the word is chosen before the work. Readiness is TRUE and evidence-backed; presentation still waits.
