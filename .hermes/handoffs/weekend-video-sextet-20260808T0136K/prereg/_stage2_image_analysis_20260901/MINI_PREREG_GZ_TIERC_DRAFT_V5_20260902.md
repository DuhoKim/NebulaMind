...
**V5 CANONICAL, V1-V4 SUPERSEDED.**

Draft V5 for principal signature, 2026-09-02.
...
The V5 pin audit independently re-hashed every unchanged pinned identity and confirmed the §2.1, §2.2, §2.5, §2.6, §2.10, §2.11, §2.12, §2.13, §2.14, and §9.1 digests exactly as printed. It also re-hashed the unchanged live acquisition source and confirmed its §2.14 digest and byte-identity pin. No pinned file changed and no placeholder is present.
...
| F11 FATAL | CLOSED: §§7.9--7.11 define acquisition completion as the exact 17,947-brick OK-receipt set condition plus no matching fetch process, require equality of every OK receipt's computed and published SHA-256, fail any unresolved non-OK verdict, and bind the whole-journal SHA-256 and line count into both freeze record and seal receipt; verdict input `journal_head_sha256` must equal that frozen digest. |
...
| F14 FATAL | CLOSED: §7.11 removes the human-statement path entirely and substitutes the pre-launch Git blob witness, commit chronology, blob-content SHA-256 equality, and a mandatory freeze-time chained seal receipt of blob ID, live-file SHA-256, pinned-copy SHA-256, and path-specific clean-diff exit status; any absent or unequal witness refuses before pixels. §§7.9--7.10 and §16.3 now distinguish the real seven-key unchained acquisition journal from the predecessor-digest-chained seal journal. |
