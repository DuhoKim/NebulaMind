...
**V6 CANONICAL, V1-V5 SUPERSEDED.**

Draft V6 for principal signature, 2026-09-02.
...
7.9 Acquisition and verification append one JSON object per completed brick to the append-only acquisition journal `../_successor_build_20260824/acquire/tier_c_fetch_receipts.jsonl`. Every acquisition-journal object has exactly the keys `brick`, `bytes`, `computed_sha256`, `published_sha256`, `url`, `utc`, and `verdict`; this acquisition journal has no predecessor-digest or current-receipt-digest fields. Its first line, read read-only during V5 preparation, was exactly `{"brick": "0001p010", "bytes": 12058560, "computed_sha256": "28afbbabea14a286975dd36a91e6b59734a74981d4e95aa2d0c177c87a3007d6", "published_sha256": "28afbbabea14a286975dd36a91e6b59734a74981d4e95aa2d0c177c87a3007d6", "url": "https://portal.nersc.gov/cfs/cosmo/data/legacysurvey/dr10/south/coadd/000/0001p010/legacysurvey-0001p010-image-r.fits.fz", "utc": "2026-09-02T05:27:44Z", "verdict": "OK"}`.
...
7.11 Acquisition completion is the following set condition, not the exit of any one process run: the set of `brick` values having at least one receipt with `verdict == "OK"` in `tier_c_fetch_receipts.jsonl` equals the 17,947-element manifest set in `tier_c_manifest_v1.json`; no process matching `fetch_bricks[.]py --manifest` is running; and every OK receipt has `computed_sha256` equal to `published_sha256`. Any receipt with a verdict other than `OK` for a brick that lacks a later OK receipt causes refusal. The resumable journal may span multiple process runs: it had 784 receipts when stopped for the 2026-09-02 15:25 KST restart and then resumed, and a brick already present and SHA-verified is skipped. At acquisition completion and before any pixel access, the freeze record pins (a) `journal_head_sha256`, exactly the SHA-256 of the whole acquisition-journal file at that moment, and (b) `receipt_count`, exactly its line count; both values are also recorded in the seal receipt. The `journal_head_sha256` supplied to the verdict program must equal item (a), and mismatch yields `DATA-INTEGRITY-FAIL`.

The live script has three historically distinct Git blobs. Read-only Git returned `3f994f8d2112660e21d73a4f3651c71feef16bbb` at commit `888c0a2ff59bb7c4b50be3a3892ea2561ab2a125` (2026-09-01 19:39:54 KST, the first #52 launch), `8e3dedd3aa777a3e437c6963ddc73f1da8fb9472` at commit `06a242163d42588dff3aa0a40cdd86778f51ea48` (2026-09-01 19:43:59 KST, four workers), and `df704bed1c5fd872cf9dee9f4be2e88f64bb94a0` at Tier-C GO commit `ad21829fad8b805f5d32d99c74ad4d1cce92071b` (2026-09-02 14:30:32 KST); `HEAD` still returns `df704bed1c5fd872cf9dee9f4be2e88f64bb94a0`, and the live file equals that blob. The Tier-C journal's first receipt is `2026-09-02T05:27:44Z` (14:27:44 KST), 2 minutes 48 seconds before the GO commit, and 35 receipts precede the commit timestamp, so Git alone cannot witness which working-tree bytes ran during those minutes.
...
The earlier blobs would have rejected that command line, so the 35 pre-commit receipts could only have been written by the `df704bed...` content already in the working tree. The 15:25 KST resumption ran after `ad21829fa` committed that blob.

At freeze, the seal seat MUST execute a programmatic content-integrity check and append its result as a chained seal receipt under §§7.10 and 16.3. For every brick in the manifest, the check MUST establish that (i) a final OK receipt exists, (ii) the file exists in `bricks_tier_c/`, and (iii) SHA-256 recomputed from that on-disk file at freeze equals both that receipt's `computed_sha256` and its `published_sha256`. The chained receipt MUST record `manifest_count`, `files_checked`, `mismatches` (which MUST equal 0), `journal_head_sha256`, and `receipt_count`. Any missing receipt, missing file, mismatch, unequal count, or absent receipt field yields `DATA-INTEGRITY-FAIL` before any pixel access. Because this gate re-hashes every acquired brick from disk against its receipt and published digest, acquisition-script identity is supplementary rather than necessary to content integrity.
...
The V6 pin audit independently re-hashed all six unchanged §2 pinned files and confirmed the §2.10, §2.11, §2.12, §2.13, and §2.14 digests exactly as printed. It also re-hashed the unchanged live acquisition source and confirmed its §2.14 digest and byte-identity pin. No pinned file changed and no placeholder is present.
...
| F11 FATAL | CLOSED: §§7.9--7.11 retain the exact 17,947-brick completion set and journal bindings, and §7.11 now requires the seal seat to programmatically verify and chain-receipt, for every manifest brick, a final OK receipt, the on-disk file, and equality of its freshly recomputed SHA-256 to both receipt digests; zero mismatches and equal manifest/files-checked counts are mandatory before pixels. |
...
| F14 FATAL | CLOSED: §7.11 corrects the three-blob chronology, identifies `ad21829fa` as introducing `df704bed...`, binds the 15:25 KST resumption to the committed blob, and limits the freeze-time Git receipt to the resumed run and freeze state; the first 2 minutes 48 seconds are covered by the argv witness plus the mandatory content re-hash. The chained Git receipt still requires blob ID, live and pinned SHA-256 values, and a clean path-specific diff. |
| F15 MINOR | CLOSED under the coordinator's ruling: §16.12 states that the fixture receipt is the real most-recent-run output and that refreshing it each version is authorized; V6 records the fresh 13-test result at 0.082s rather than reverting real timing. |
| F16 FATAL | CLOSED: §7.11 records all three true blobs and commits, the first receipt and 35-receipt/2-minute-48-second pre-commit gap, the exact Tier-C argv, the three argparse additions unique to `df704bed...`, and the post-commit 15:25 KST resumption; it expressly says Git alone does not witness the gap and does not misattribute `df704bed...` to `888c0a2ff`. |
| F17 FATAL | CLOSED: §7.11 mandates a freeze-time, seal-seat programmatic disk re-hash of every manifest brick and a chained §16.3 receipt recording manifest count, files checked, zero mismatches, journal SHA-256, and line count; any missing OK receipt, file, field, count equality, or hash equality refuses before pixels, making script custody supplementary to content integrity. |
