# V29 CANDIDATE — COHERENT-ATTACK INSPECTION (staged, UNADOPTED) — 2026-09-07 02:54 KST

Executed: `_optionA_dev/track1/coherent_attacks_v29.py` (SHA-256 ba4cb1ba251e5e7ea9dfdd1647a6954c42019c98104f88bf01a1291a6d92a371) — driver v13, verdict v29, provenance_designs_v9; the modules named in the header are the modules imported and executed. Warning-strict; the one inherited RuntimeWarning at fourier_chirality.py:87 is the known one. Every row carries the SAME verdict as the V28 inspection (35 rows; fixture commit hashes differ per run). The row-1 note now says INCONSISTENT-INPUT for codex's wrong-repository forgery (codex V28-3).

TWO CATEGORIES, KEPT APART (Blanc 02:06): rows 1, 2, 6 and 10b–10d show the offline candidate LOADING counter-cases 1, 2, A and the inherited list trust — DISCLOSED LIMITS (column one of §3c; L-INH is a default-mode implementation limit, verify_split staged). The precedence cases codex built for V28-1/V28-2 are not rows of this table; they are the track-9 fail-first tests on the complete paths (`TRACK9_FAIL_FIRST_RECEIPT_20260907.md`). No row here is an unrepaired contradiction.

| attack | staged V29 candidate (driver v13 / verdict v29 / provenance v9) | note |
|---|---|---|
...
| 10b attack 1 again: forged approval event (coherent digest; names the pinned repo), live feed does not contain it | offline: ACCEPTED / composed: REFUSED EVENT-FORGED (the retained event is absent and the liv) | same identity, same load_identity call; composed = UNADOPTED |
| 10c attack 2 again: history rebuilt after honest publication, coherently sealed and pushed fast-forward | offline: ACCEPTED / composed: REFUSED HISTORY-CONTINUATION (HISTORY-NOT-AN-EXTENSION at fd72a4c34204) | same identity, same load_identity call; composed = UNADOPTED |
