# V31 CANDIDATE — COHERENT-ATTACK INSPECTION (staged, UNADOPTED) — 2026-09-07 04:26 KST

Executed: `_optionA_dev/track1/coherent_attacks_v31.py` (SHA-256 c9b6847417c8f8bccc32dfee9953a9f5876305c69c7d1d2b634680d0fb65d566) — driver v15 (staged finding collector), verdict v31, provenance_designs_v11; the modules named in the header are the modules imported and executed. Warning-strict; the one inherited RuntimeWarning at fourier_chirality.py:87 is the known one. Every row carries the SAME verdict as the V30 inspection (35 rows; fixture commit hashes differ per run).

TWO CATEGORIES, KEPT APART (Blanc 02:06): rows 1, 2, 6 and 10b–10d show the offline candidate LOADING counter-cases 1, 2, A and the inherited list trust — DISCLOSED LIMITS (column one of §3c). Codex's V30 constructions (the null open file over a rewrite, the failed witness fetch, the missing list id, the rewrite with an empty feed, the expired undetermined approval) are executed by track 11 on the complete path, not rows of this table. No row here is an unrepaired contradiction.

| attack | staged V31 candidate (driver v15 / verdict v31 / provenance v11) | note |
|---|---|---|
...
| 10c attack 2 again: history rebuilt after honest publication, coherently sealed and pushed fast-forward | offline: ACCEPTED / composed: REFUSED HISTORY-CONTINUATION (HISTORY-NOT-AN-EXTENSION at 9583787b8d5b) | same identity, same load_identity call; composed = UNADOPTED |
