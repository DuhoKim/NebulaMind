# V32 CANDIDATE — COHERENT-ATTACK INSPECTION (staged, UNADOPTED) — 2026-09-07 05:27 KST

Executed: `_optionA_dev/track1/coherent_attacks_v32.py` (SHA-256 9b949a93aacbe5b55311838275233174a9db6934a2dee0bfd22b236b8102149b) — driver v16 (NSD: independent contributing stages), verdict v32, provenance_designs_v12; the modules named in the header are the modules imported and executed. Warning-strict; the one inherited RuntimeWarning at fourier_chirality.py:87 is the known one. Every row carries the SAME verdict as the V31 inspection (35 rows; fixture commit hashes differ per run).

TWO CATEGORIES, KEPT APART (Blanc 02:06): rows 1, 2, 6 and 10b–10d show the offline candidate LOADING counter-cases 1, 2, A and the inherited list trust — DISCLOSED LIMITS (column one of §3c). The NSD controls are derived from the INDEPENDENCE table and executed by track 12 on the complete path, not rows of this table. No row here is an unrepaired contradiction.

| attack | staged V32 candidate (driver v16 / verdict v32 / provenance v12) | note |
|---|---|---|
...
| 10c attack 2 again: history rebuilt after honest publication, coherently sealed and pushed fast-forward | offline: ACCEPTED / composed: REFUSED HISTORY-CONTINUATION (HISTORY-NOT-AN-EXTENSION at 2d4677794564) | same identity, same load_identity call; composed = UNADOPTED |
