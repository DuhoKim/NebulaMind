...
Rule: T_pulse = first whole minute >= T_sign + 600 s. NIST is binding whenever its pulse authenticates. Before T_pulse + 24 h: an
unauthenticated or unretrievable primary is RETRY. From T_pulse + 24 h: drand round_for(T_pulse) is used ONLY if the primary is still not
authenticable at the time of the verdict (recomputed, not recorded) AND >= 2 pinned hosts agree. VOID: if NIST later serves an
...
DELAY_S = 600; FALLBACK_AFTER_H = 24
...
def verdict(rec, now, roots, fetch=None, rule_sha256=None, statement_bytes=None):
    ...
    ev = _nist_evidence(rec); nist_ok = False
    if ev is not None:
        ...
        if fetch is not None:
            try:
                ...
                out["checks"]["nist_live_equal"] = same
                if not same:
            ...
            except urllib.error.HTTPError as e: out["checks"]["nist_live_http"] = e.code
            except Exception as e: out["checks"]["nist_live_error"] = repr(e)[:120]
        nist_ok = c["accepted"]
    ...
    if now < t_pulse + timedelta(hours=FALLBACK_AFTER_H): out.update(outcome="RETRY", seed_hex=None, source=None, why="primary not authenticable/retrievable yet; fallback not permitted before T_pulse + 24 h"); return out
    # at/after 24 h: primary still not authenticable NOW — if fetch is given, re-check live (a live authenticable primary is binding -> RETRY... i.e. ACCEPT-NIST path above would have fired if evidence were live-equal; so re-collect and authenticate)
    if fetch is not None:
        try:
        ...
        except urllib.error.HTTPError as e: out["checks"]["nist_live_now_http"] = e.code
        except Exception as e: out["checks"]["nist_live_now_error"] = repr(e)[:120]
