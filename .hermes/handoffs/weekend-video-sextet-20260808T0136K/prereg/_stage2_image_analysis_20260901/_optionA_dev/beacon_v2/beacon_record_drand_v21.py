#!/usr/bin/env python3
"""THE ONE VERDICT — DRAND-ONLY, V21 (option A V21 draft, 2026-09-06; source decision by Duho via codex voice ~19:02 KST, "해"). V20 bytes preserved in beacon_record_drand.py.
...
def _parse_verify(body, rnd, url):
    """(checks, signature or None). Any failure — not JSON, not a dict, verifier exception — is a non-verifying body, never an escape."""
    try:
        resp = json.loads(body)
        if not isinstance(resp, dict): return {"error": "not a JSON object", "accepted": False}, None
        c = vd.verify(resp, rnd, url); return c, (str(resp.get("signature")) if c["accepted"] else None)
...
def verdict(rec, now, fetch=None, rule_sha256=None, statement_bytes=None):
    ...
    if fetch is not None:
        ...
        for u in ok:
            ...
            if lsig == sig: live_ok[u] = True; rep[u] = "same signature, different encoding"; continue
            return refuse("CONFLICTING-VALID-SIGNATURES", f"relay {u} serves a DIFFERENT signature for round {rnd} that also verifies under the pinned key — contradicts uniqueness under one key; investigate the key")
    ...
    out.update(outcome="ACCEPT-DRAND", seed_hex=seed, source="drand-mainnet-default", round=rnd, t_pulse=fmt(t_pulse), verified_relays=sorted(ok), signature=sig); return out
...
def main(argv=None):
    ...
    a = ap.parse_args(argv); now = datetime.now(timezone.utc); logp = Path(a.log)
    def side(entry):                                     # pre-genesis failures: outside the chain, disclosed by name
        with open(str(logp) + ".pregenesis.jsonl", "a") as fh: fh.write(json.dumps({"utc": fmt(now), **entry}, sort_keys=True) + "\n")
    def log(stage, rec_bytes, r):                       # MANDATORY: every attempt, hash-chained, in order
        H.append(logp, {"stage": stage, "record_sha256": hashlib.sha256(rec_bytes).hexdigest(), "outcome": r["outcome"], "source": r.get("source"), "seed_hex": r.get("seed_hex"), "why": r.get("why")})
    def ensure_genesis(stmt, t_sign_str):
        ...
        if not logp.is_file() or logp.stat().st_size == 0: H.genesis(logp, d, fmt(t_pulse), a.rule_sha256, rnd); return
        g = H.validate(logp)[0]
    ...
    try:
        ...
        if a.mode == "collect":
            ...
            except SystemExit as e: H.append(logp, {"stage": "collector-refusal", "refusal": str(e)}); raise
        ...
        if logp.is_file() and logp.stat().st_size > 0 and not str(e).startswith("HISTORY-"):
            try: H.append(logp, {"stage": "collector-refusal", "refusal": str(e)})
