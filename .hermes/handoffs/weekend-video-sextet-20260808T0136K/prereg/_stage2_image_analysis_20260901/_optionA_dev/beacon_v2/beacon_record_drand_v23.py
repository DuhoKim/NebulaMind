...
def main(argv=None):
    ...
    logp = Path(a.log)
    def publish(message):                            # v23 producer boundary: one entry per acknowledged commit, or refuse to proceed
        if not (a.publish_remote and a.publish_ref): return
        sys.path.insert(0, str(HERE.parent / "track2")); import provenance_designs_v3 as P, subprocess as sp
        root = sp.run(["git", "rev-parse", "--show-toplevel"], cwd=logp.parent, capture_output=True, text=True).stdout.strip()
        P.publish_entry(root, logp.resolve().relative_to(Path(root).resolve()).as_posix(), a.publish_remote, a.publish_ref, message)
