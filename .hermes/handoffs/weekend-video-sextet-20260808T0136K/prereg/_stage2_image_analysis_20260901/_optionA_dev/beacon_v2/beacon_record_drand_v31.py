#!/usr/bin/env python3
"""THE ONE VERDICT — DRAND-ONLY, V27 STAGED CANDIDATE (on provenance_designs_v7). V26 was — V26 STAGED CANDIDATE (on provenance_designs_v6). V25 was — V25 STAGED CANDIDATE (uses provenance_designs_v5: the producer refuses PUBLISH-UNRELATED-COMMITS; codex V24 N3). V24 was — V24 STAGED CANDIDATE (codex V23 M2: outer refusal/error entries are PUBLISHED too; before any operation the collector RECONCILES a pending entry with the remote — provenance_designs_v4.reconcile_pending — and stops on PUBLISH-BATCH/PENDING-PUSH). V23 was — V23 STAGED CANDIDATE (codex V22 B: the collector PUBLISHES each history entry — the genesis as the history-open commit, then every attempt — and requires the remote's acknowledgment before it proceeds, when --publish-remote/--publish-ref are given; production composed mode gives them). V22 was — V22 STAGED CANDIDATE (track 1 of Blanc's 20:45 order; NOT GATED, NOT ADOPTED). V22 = V21 + (codex V21 C6) signature
...
def main(argv=None):
    ...
    logp = Path(a.log)
    def _pub():
        sys.path.insert(0, str(HERE.parent / "track2")); import provenance_designs_v11 as P, subprocess as sp
