...
class T(unittest.TestCase):
    REMOVED_FROM_PINNED = ['"""THE ONE VERDICT (rebuilt once, 2026-09-06). A beacon record holds INPUTS (T_sign, rule digest, the retained signature statement) and', 'Rule: T_pulse = first whole minute >= T_sign + 600 s. NIST is binding whenever its pulse authenticates. Before T_pulse + 24 h: an', 'unauthenticated or unretrievable primary is RETRY. From T_pulse + 24 h: drand round_for(T_pulse) is used ONLY if the primary is still not', 'authenticable at the time of the verdict (recomputed, not recorded) AND >= 2 pinned hosts agree. VOID: if NIST later serves an', 'authenticable T_pulse pulse, any fallback identity is void (a later verdict with fetch would say ACCEPT-NIST, not ACCEPT-DRAND)."""', 'DELAY_S = 600; FALLBACK_AFTER_H = 24', '    ev = _nist_evidence(rec); nist_ok = False', '                out["checks"]["nist_live_equal"] = same', '            except urllib.error.HTTPError as e: out["checks"]["nist_live_http"] = e.code', '            except Exception as e: out["checks"]["nist_live_error"] = repr(e)[:120]', '        nist_ok = c["accepted"]', '    if now < t_pulse + timedelta(hours=FALLBACK_AFTER_H): out.update(outcome="RETRY", seed_hex=None, source=None, why="primary not authenticable/retrievable yet; fallback not permitted before T_pulse + 24 h"); return out', '    # at/after 24 h: primary still not authenticable NOW — if fetch is given, re-check live (a live authenticable primary is binding -> RETRY... i.e. ACCEPT-NIST path above would have fired if evidence were live-equal; so re-collect and authenticate)', '    if fetch is not None:', '        try:', '            live = nist_pulse.collect(fetch, t_pulse); lc = nist_pulse.authenticate(live, t_pulse, roots); out["checks"]["nist_live_now"] = lc["accepted"]', '            if lc["accepted"]: return refuse("DRAND-VOID-PRIMARY-AUTHENTICABLE", "NIST now serves an authenticable T_pulse pulse: the primary is binding")', '        except urllib.error.HTTPError as e: out["checks"]["nist_live_now_http"] = e.code', '        except Exception as e: out["checks"]["nist_live_now_error"] = repr(e)[:120]', '    c = sub.add_parser("collect"); c.add_argument("--t-sign", required=True); c.add_argument("--rule-sha256", required=True); c.add_argument("--signature-statement", required=True); c.add_argument("--out", required=True)', '    v = sub.add_parser("verify"); v.add_argument("--record", required=True); v.add_argument("--rule-sha256", required=True); v.add_argument("--signature-statement", required=True); v.add_argument("--no-live", action="store_true")', '        Path(a.out).write_text(json.dumps(rec, indent=1, sort_keys=True)); r = verdict(rec, now, roots)', '    rec = json.loads(Path(a.record).read_text()); stmt = Path(a.signature_statement).read_bytes(); r = verdict(rec, now, roots, fetch=None if a.no_live else fetch, rule_sha256=a.rule_sha256, statement_bytes=stmt)']
    ...
    def test_v19_wrong_time_pulse_is_not_fallback_evidence(self):
        # codex V18 item 2: a served, live-equal pulse whose body says T_pulse - 60 s must NOT open the fallback
        prev = pki.network(TP - timedelta(seconds=60), sign=False, drand=(RND, "c" * 64))   # an archive whose pulses are the PREVIOUS minute's
        ms_tp = int(TP.timestamp() * 1000); ms_prev = ms_tp - 60000
        def n(url, timeout=30):                                                          # serve the previous minute's pulse for our T_pulse query
            return prev(url.replace(str(ms_tp + 60000), str(ms_prev + 60000)).replace(str(ms_tp), str(ms_prev)), timeout)
        rec = X.collect(n, X.fmt(T_SIGN), D, STMT, now=SOON); self.assertIn("pulse_body_b64", rec["nist"])
        r = V(X, rec, SOON, fetch=n); self.assertEqual(r["outcome"], "RETRY"); self.assertFalse(r["checks"]["nist"]["timestamp_is_t_pulse"]); self.assertIn("not the T_pulse pulse", r["why"])
    def test_v19_property_exhibit_reproduces(self):
        import exhibit_property as ex
        res = ex.run(); self.assertTrue(all(r["identical"] for r in res)); self.assertEqual(res[0]["verdicts"][0]["outcome"], "ACCEPT-DRAND")
        self.assertEqual(ex.run(), res)
    def test_v19_collector_cli_logs_every_attempt(self):
        import tempfile, pathlib, unittest.mock as um
        d = pathlib.Path(tempfile.mkdtemp()); stmt = d / "s.txt"; stmt.write_bytes(STMT); logp = d / "collection_log.jsonl"; out = d / "rec.json"
        n = pki.network(TP, sign=False, drand=(RND, "c" * 64))
        with um.patch.object(X, "datetime") as dt:
            dt.now.return_value = SOON; dt.strptime = datetime.strptime
            with um.patch("urllib.request.urlopen") as uo: pass
            # drive main() with an injected fetch by monkeypatching nist_pulse/drand_round callers is heavier than needed: call verdict-level via main with --no-live on a record we collect here
            rec = X.collect(n, X.fmt(T_SIGN), D, STMT, now=SOON); out.write_text(json.dumps(rec))
            with um.patch.object(X.nist_pulse, "pinned_roots", pki.roots):
                rc = X.main(["verify", "--record", str(out), "--rule-sha256", D, "--signature-statement", str(stmt), "--no-live", "--log", str(logp)])
        L = [json.loads(l) for l in logp.read_text().splitlines()]; self.assertEqual(L[0]["stage"], "collector-verify"); self.assertEqual(L[0]["outcome"], "ACCEPT-DRAND"); self.assertEqual(rc, 0)
