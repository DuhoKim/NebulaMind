#!/usr/bin/env python3
"""Build the ordered, catalogue-only corpus identity and CSV templates. V17 VARIANT: build_corpus_identity.py + the expedited beacon import + the approval witness (codex V16 C5)."""
...
sys.path.insert(0, str(HERE)); import approval_witness
WITNESS_REMOTE_REF="origin/feat/paper-workflow-v2"; WITNESS_REMOTE_URL="https://github.com/DuhoKim/NebulaMind.git"; APPROVAL_GLOB="APPROVAL_RECORD_SELRULE_V17*"; WITNESS_FETCH=True   # tests override
...
def _validate_beacon_record(path, rule_sha256, signature_statement, fetch=None, now=None):
    ...
    t_pulse=beacon_record.parse_utc(B['T_pulse']); seed_round=drand_round.round_for(t_pulse)
    r['approval_witness']=approval_witness.verify(signature_statement, t_pulse, seed_round, fetch, WITNESS_REMOTE_REF, WITNESS_REMOTE_URL, beacon_record.MIN_T_SIGN, APPROVAL_GLOB, do_fetch=WITNESS_FETCH)   # V17: approval first, seed second — witnessed, not asserted
    return B, raw, r
