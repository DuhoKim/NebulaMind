#!/usr/bin/env python3
"""Build the ordered, catalogue-only corpus identity and CSV templates. V18 VARIANT: build_corpus_identity.py + the expedited beacon (V18 policy) + approval_witness_v2
...
sys.path.insert(0, str(HERE)); import approval_witness_v2 as approval_witness
WITNESS_REMOTE_REF="origin/feat/paper-workflow-v2"; WITNESS_REMOTE_URL="https://github.com/DuhoKim/NebulaMind.git"; APPROVAL_GLOB="APPROVAL_RECORD_SELRULE_V18*"; WITNESS_FETCH=True; BRANCH_REF="refs/heads/feat/paper-workflow-v2"   # tests override
EVENTS=approval_witness.github_events                       # tests inject a feed
...
def _validate_beacon_record(path, rule_sha256, signature_statement, fetch=None, now=None):
    ...
    _adoption_binding(rule_sha256)
    r['approval_witness']=approval_witness.verify(signature_statement, t_sign, t_pulse, seed_round, rule_sha256, fetch, WITNESS_REMOTE_REF, WITNESS_REMOTE_URL, beacon_record.MIN_T_SIGN, APPROVAL_GLOB, EVENTS(), BRANCH_REF, do_fetch=WITNESS_FETCH)   # V18: witnessed by the server's push time
    _collection_lock(Path(path), raw, r)
    return B, raw, r

def _adoption_binding(rule_sha256):
    """V18: the approved rule digest is a COMMITTED file, not a flag. The command-line digest must equal it; absent → refuse (fail closed)."""
    p=ADOPTION_FILE
    if not p.is_file(): _refuse('ADOPTION-MISSING', f'{p.name} absent — no approved rule digest is bound')
    lines=[l.strip() for l in p.read_text().splitlines() if l.strip()]
    if len(lines)!=1 or not re.fullmatch(r'[0-9a-f]{64}', lines[0]): _refuse('ADOPTION-MALFORMED', f'{p.name} must hold exactly one 64-hex line')
...
def _collection_lock(record_path, raw, verdict):
    """V18 source-decision lock: every accepted verdict is appended to the append-only collection log; the FIRST ACCEPT is binding — a later
    build with a different record or outcome is refused (LOCKED). The log is committed with the identity (E3 witness)."""
    import hashlib as _h; d=_h.sha256(raw).hexdigest(); entry={'utc':time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime()),'record':record_path.name,'record_sha256':d,'outcome':verdict['outcome'],'source':verdict.get('source'),'seed_hex':verdict.get('seed_hex')}
    prior=[json.loads(l) for l in COLLECTION_LOG.read_text().splitlines() if l.strip()] if COLLECTION_LOG.is_file() else []
    first=next((e for e in prior if str(e.get('outcome','')).startswith('ACCEPT')), None)
    if first is not None and (first['record_sha256']!=d or first['outcome']!=verdict['outcome'] or first.get('seed_hex')!=verdict.get('seed_hex')):
        _refuse('COLLECTION-LOCKED', f"first ACCEPT already logged at {first['utc']} ({first['outcome']}, record {first['record_sha256'][:12]}…); this build names a different one")
    if first is None:
        with open(COLLECTION_LOG,'a') as f: f.write(json.dumps(entry,sort_keys=True)+'\n')
    verdict['collection_lock']={'log':COLLECTION_LOG.name,'first_accept':first or entry}
