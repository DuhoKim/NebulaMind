...
import beacon_record_drand as beacon_record, verify_drand   # option A V20: drand-only verdict (BLS verified under the pinned key) + the APPROVAL WITNESS below
sys.path.insert(0, str(HERE)); import approval_witness_v3 as approval_witness
WITNESS_REMOTE_REF="origin/feat/paper-workflow-v2"; WITNESS_REMOTE_URL="https://github.com/DuhoKim/NebulaMind.git"; APPROVAL_GLOB="APPROVAL_RECORD_SELRULE_V20*"; WITNESS_FETCH=True; BRANCH_REF="refs/heads/feat/paper-workflow-v2"   # tests override
...
def _validate_beacon_record(path, rule_sha256, signature_statement, fetch=None, now=None):
    ...
    except Exception as e:                                                        # V20 (M2): pre-parse failures are logged, then refused
