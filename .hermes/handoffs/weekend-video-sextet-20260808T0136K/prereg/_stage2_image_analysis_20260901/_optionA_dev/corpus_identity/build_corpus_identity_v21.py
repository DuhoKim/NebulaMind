...
import beacon_record_drand_v21 as beacon_record, verify_drand_v2 as verify_drand, history as H   # option A V21: drand-only verdict (BLS verified under the pinned key) + the APPROVAL WITNESS below
sys.path.insert(0, str(HERE)); import approval_witness_v4 as approval_witness
WITNESS_REMOTE_REF="origin/feat/paper-workflow-v2"; WITNESS_REMOTE_URL="https://github.com/DuhoKim/NebulaMind.git"; APPROVAL_GLOB="APPROVAL_RECORD_SELRULE_V21*"; WITNESS_FETCH=True; BRANCH_REF="refs/heads/feat/paper-workflow-v2"   # tests override
...
COLLECTION_LOG=HERE.parent/'collection_log.jsonl'           # the AUTHENTICATED history (history.py): genesis first, hash-chained, first ACCEPT locked atomically
...
def _log_attempt(entry):
    """Hash-chained append (history.append: validates the chain under the lock, then appends). If the history is not valid the entry goes beside it."""
    try: H.append(COLLECTION_LOG, entry)
    except (ValueError, SystemExit, OSError) as e: _log_side({**entry,'history_error':repr(e)[:120]})
