#!/usr/bin/env python3
"""V25 STAGED CANDIDATE (NOT GATED, NOT ADOPTED): V24 on provenance_designs_v5 (PUBLISH-UNRELATED-COMMITS refused; codex V24 N3). V24 was: V23 + (codex V23 M2) the build RECONCILES a pending history entry with the remote before any operation (provenance_designs_v4.reconcile_pending). V23 text follows.
...
import beacon_record_drand_v25 as beacon_record, verify_drand_v2 as verify_drand, history_v2 as H   # option A V21: drand-only verdict (BLS verified under the pinned key) + the APPROVAL WITNESS below
...
def _reconcile():
    ...
    sys.path.insert(0, str(HERE.parent/'track2')); import provenance_designs_v5 as P
