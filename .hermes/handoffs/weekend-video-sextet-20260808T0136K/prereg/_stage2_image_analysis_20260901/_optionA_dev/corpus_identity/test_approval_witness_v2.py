"""approval_witness_v2 over a real git clone with a bare origin and a MOCK events feed (the server-side push timestamp)."""
...
import beacon_record_expedited as X, drand_round, test_pki as pki, approval_witness_v2 as AW
...
class T(unittest.TestCase):
    def setUp(self):
        ...
        self.stmt = self.work / "APPROVAL_RECORD_SELRULE_V18_TEST.md"
    ...
    def verify(self, commit, when=None, fetch=None, body_override=None):
        return AW.verify(self.stmt, T_SIGN, TP, RND, D, fetch or net(), "origin/main", str(self.bare), X.MIN_T_SIGN, "APPROVAL_RECORD_SELRULE_V18*", self.events(commit, when or (TP - timedelta(minutes=3))), "refs/heads/main")
    ...
    def test_no_push_event_refused(self):
        c = self.commit()
        with self.assertRaises(SystemExit) as cm: AW.verify(self.stmt, T_SIGN, TP, RND, D, net(), "origin/main", str(self.bare), X.MIN_T_SIGN, "APPROVAL_RECORD_SELRULE_V18*", [], "refs/heads/main")
    ...
    def test_unpushed_modified_second_record_refused(self):
        ...
        other = self.work / "APPROVAL_RECORD_SELRULE_V18_SECOND.md"; other.write_text("again"); git(self.work, "add", other.name); git(self.work, "commit", "-q", "-m", "again"); git(self.work, "push", "-q", "origin", "HEAD:refs/heads/main")
        with self.assertRaises(SystemExit) as cm: self.verify(c)
