"""V20 builder: DRAND-ONLY verdict on the real retained round 6441924 (BLS-verified under the pinned key; the exhibit-round exclusion lifted only here) + approval_witness_v3 + COLLECTION HISTORY (every attempt
...
import beacon_record_drand as X, verify_drand as vd
import build_corpus_identity_v20 as B19
...
TP = vd.round_time(ROUND); T_SIGN = TP - timedelta(seconds=600); D = "b" * 64; RND = ROUND; R_SIGN = vd.round_for(T_SIGN); NONCE = "e" * 64; SOON = TP + timedelta(minutes=1)
...
class T(unittest.TestCase):
    def setUp(self):
        ...
        self.stmt = self.work / "APPROVAL_RECORD_SELRULE_V20_TEST.md"; self.stmt_bytes = f"RULE_SHA256: {D}\nAPPROVAL_UTC: {X.fmt(T_SIGN)}\nDRAND_AT_APPROVAL: round {R_SIGN} randomness {NONCE}\n".encode()
        ...
        B19.WITNESS_REMOTE_REF = "origin/main"; B19.WITNESS_REMOTE_URL = str(bare); B19.APPROVAL_GLOB = "APPROVAL_RECORD_SELRULE_V20*"; B19.WITNESS_FETCH = True; B19.BRANCH_REF = "refs/heads/main"; self._ex = X.EXCLUDED_ROUNDS; X.EXCLUDED_ROUNDS = (6440756,)
