...
import beacon_record_drand_v21 as X, verify_drand_v2 as vd, history as H
import build_corpus_identity_v21 as B19
...
class T(unittest.TestCase):
    def setUp(self):
        ...
        self.stmt = self.work / "APPROVAL_RECORD_SELRULE_V21_TEST.md"; self.stmt_bytes = f"RULE_SHA256: {D}\nAPPROVAL_UTC: {X.fmt(T_SIGN)}\nDRAND_AT_APPROVAL: round {R_SIGN} randomness {NONCE}\n".encode()
        ...
        B19.WITNESS_REMOTE_REF = "origin/main"; B19.WITNESS_REMOTE_URL = str(bare); B19.APPROVAL_GLOB = "APPROVAL_RECORD_SELRULE_V21*"; B19.WITNESS_FETCH = True; B19.BRANCH_REF = "refs/heads/main"; self._ex = X.EXCLUDED_ROUNDS; X.EXCLUDED_ROUNDS = (6440756,)
    ...
    def test_history_for_another_approval_is_refused(self):
        other = self.work / "APPROVAL_RECORD_SELRULE_V21_OTHER.md"; other.write_bytes(self.stmt_bytes + b"\n")
