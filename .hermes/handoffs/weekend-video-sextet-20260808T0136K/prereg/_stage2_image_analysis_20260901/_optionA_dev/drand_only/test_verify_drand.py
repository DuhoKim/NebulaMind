...
HERE = Path(__file__).resolve().parent; sys.path.insert(0, str(HERE)); import verify_drand as vd
...
class T(unittest.TestCase):
    ...
    def test_tampered_wrong_round_unbound_path_and_wrong_key_refuse(self):
        t = dict(A); t["signature"] = A["signature"][:-2] + "00"; self.assertFalse(vd.verify(t, 6441924)["accepted"])
        self.assertFalse(vd.verify(A, 6441925)["accepted"])
        ...
        try: self.assertFalse(vd.verify(A, 6441924)["bls_verifies_under_pinned_key"])
        finally: vd.PUBLIC_KEY_HEX = saved
        r = dict(A); r["randomness"] = "0" * 64; self.assertFalse(vd.verify(r, 6441924)["accepted"])
    def test_deterministic(self):
        self.assertEqual(vd.verify(A, 6441924), vd.verify(A, 6441924))
