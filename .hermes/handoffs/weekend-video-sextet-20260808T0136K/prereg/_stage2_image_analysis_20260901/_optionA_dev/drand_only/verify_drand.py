...
def verify(resp: dict, expected_round: int, source_url: str | None = None) -> dict:
    """Recompute everything from the response bytes and the pinned key. Returns a dict of checks; 'accepted' iff all hold."""
    c = {}
    c["url_bound_to_chain_hash"] = (source_url is None) or (f"/{CHAIN_HASH}/public/{expected_round}" in source_url and any(source_url.startswith(h) for h in RELAYS))
