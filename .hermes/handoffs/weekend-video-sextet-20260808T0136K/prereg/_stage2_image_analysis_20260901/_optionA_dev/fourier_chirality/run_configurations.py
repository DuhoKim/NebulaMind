...
def load_identity(identity_path, proto):
    ...
    w = {k: v for k, v in w.items() if k != "_root"}
    ...
    if proto.rule_sha256 is not None and I["rule_sha256"] != proto.rule_sha256: raise DataIntegrityFail("IDENTITY-RULE-DIGEST: not the pinned signed rule")
...
def load_manifest(path, tensors, n_required, required_objids=None):
    ...
    return objs, {"manifest_sha256": sha_file(path), "n": len(objs), "tensor_set_sha256": hashlib.sha256("".join(sorted(r["tensor_sha256"] for r in rows)).encode()).hexdigest()}
