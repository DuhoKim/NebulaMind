#!/usr/bin/env python3
"""run_configurations_v10 — V26 STAGED CANDIDATE (codex V25 P1/P2: composed mode on provenance_designs_v6 — ancestry-aware batch classification; absence of the approval or open event is a RETRY disposition, FORGED only on contradiction). v9 bytes preserved. v9 was — V25 STAGED CANDIDATE (codex V24 N2/N3: composed mode on provenance_designs_v6 — EVIDENCE-UNAVAILABLE / EVIDENCE-INCOMPLETE are RETRY, HISTORY-PUBLICATION-BATCH is terminal). v8 bytes preserved. v8 was — V24 STAGED CANDIDATE (codex V23 M1/M2: composed mode validates the history with provenance_designs_v6 — one server-timed push event per history commit, in order). v7 bytes preserved. v7 was — V23 STAGED CANDIDATE (codex V22 A/B/C/E): composed mode uses provenance_designs_v6 — the history-open anchor is a RETAINED PushEvent (`history_open_event_file`) authenticated live and required to deliver a GENESIS-ONLY first history commit; one entry per acknowledged commit; the event predicate is W3's `delivers` (before..head included) in every mode; the render journal's render-end must be its LAST relevant record. v6 bytes preserved. v6 was — V22 STAGED CANDIDATE (track 1 of Blanc's 20:45 order; NOT GATED, NOT ADOPTED). v6 = v5 + (codex V21 C2) W4 reproduced in the
driver: exactly one approval-record path for the version's glob anywhere in history (`Protocol.approval_glob`); (C3) a `witness-closed` entry in the
history is TERMINAL (COLLECTION-CLOSED); (C9) sentinel reconciliation requires a non-empty refusal cause on every journalled refusal and a render-end
whose count equals the manifest's sentinels; (C11, UNADOPTED, default off) `Protocol.verify_split`: recompute the seeded split from the pinned
catalogue files (guarded pool, exclusion file, survey-bricks table, no-r registry — no pixel, no label read beyond the pool's own column) and require
the identity's three lists to equal it. What v6 does NOT do (track 2, UNADOPTED designs): authenticate the retained GitHub event against the live
API, or anchor the history head outside the file — a coherently forged event and a rebuilt history still pass offline; stated in the candidate text.
v5 bytes preserved in run_configurations_v5.py. v5 was (option A V21 draft): v4 plus the PRODUCTION WIRING codex's V20 gate found missing (FATAL: Protocol.rederive_seed was None, so
the production entry point refused every identity with PROTOCOL-NO-REDERIVER and only fixtures injecting a callback ever loaded one). v5 binds a REAL
module-level re-deriver into PRODUCTION (production_rederive_seed: the drand-only verdict of beacon_record_drand_v21 recomputed OFFLINE from the retained
record, BLS under the pinned key; the verifier is imported lazily and its absence is a refusal, VERIFIER-UNAVAILABLE); fixtures use PRODUCTION's own
function (paths and sizes overridden only — no callback substitution). v5 also computes the FULL CONJUNCTION from COMMITTED evidence (codex V20 M1):
the approval record is read from the repository at the witnessed approval commit (committed once, unmodified, byte-equal), its schema lines are parsed
(rule digest = adopted; APPROVAL_UTC = T_sign; the one nonce line = the witness's nonce), T_pulse/round are re-derived from T_sign and must equal the
identity's and the record's, MIN_T_SIGN and the exclusions are enforced, the push event is re-validated (type, ref, server time < T_pulse, delivery by
head / commits / before..head ancestry via git, canonical digest, provenance present), the nonce is re-verified by BLS from the bodies retained in the
witness (exact round set, scheduled bound, before the seed round), the collection history is the AUTHENTICATED chain (history.validate: genesis anchored
to the approval record digest / T_pulse / rule / round, every entry chained, known stages, count as recorded, earliest builder-accept naming this record,
outcome and seed, no semantic conflict), the beacon record is a blob at the witnessed commit binding the same T_sign/T_pulse/round/rule/statement bytes
with >= 2 distinct pinned relay bodies, and the seed is re-derived from it. ADAPTER RECONCILIATION (codex V20 S): every sentinel row in a manifest must
have a journalled REFUSED entry in the render journal and vice versa; sentinels without a journal are refused. INHERITED — PREPARED, NOT ADOPTED:
Protocol.holdout_once (default False): when True the driver refuses a second holdout invocation via a persisted marker (the one-holdout covenant of the
signed V15 is text only; adopting this flag is Duho's call). v4 bytes preserved in run_configurations_v4.py. v4 was (option A V20 draft): v3 plus the SEMANTIC conjunction the V19 audit (M1) required — the driver PARSES the committed
...
class Protocol:
    ...
    provenance_mode: str = "offline"                                            # v6, UNADOPTED: "offline" (the candidate as reviewed at V21: retained bytes only) | "composed" (track-2 v6 helpers on this call path)
    events_runner: object = None                                                # composed mode: the `gh api` runner (production: provenance_designs_v6.gh_runner; tests inject a labelled fixture runner)
...
def _drand_modules():
    ...
    try:
        ...
        import beacon_record_drand_v26 as BD, verify_drand_v2 as vd, approval_witness_v4 as AW, history_v2 as H
        return BD, vd, AW, H
...
def load_identity(identity_path, proto):
    ...
    if proto.require_beacon:                                                                               # v5: the FULL conjunction from COMMITTED evidence (codex V20 M1/M2/W3/W5)
        ...
        if not AW.delivers(ev, commit, proto.witness_branch_ref, w_root): raise DataIntegrityFail("IDENTITY-WITNESS-COMMIT: the push event does not deliver the approval commit (head, commits, or before..head ancestry)")
        ...
        if proto.provenance_mode == "composed": composed_provenance(I, proto, w_root, ev, commit, lp)   # v6, UNADOPTED: the track-2 v6 helpers ON THIS CALL PATH (default "offline")
...
def composed_provenance(I, proto, w_root, ev, commit, log_path):
    """UNADOPTED COMPOSED MODE (v10 on provenance_designs_v6; Blanc 21:14 — the recommended mode exercised on the production call path, not in isolation). After every offline
    ...
    (`validate_continuation_v6`: ls-remote on the pinned URL, push-acknowledgement boundary) from the witnessed history-open commit."""
    sys.path.insert(0, str(HERE.parents[0] / "track2")); import provenance_designs_v6 as P
    ...
    ok, hwhy, info = P.validate_continuation_v6(w_root, Path(log_path).resolve().relative_to(Path(w_root).resolve()).as_posix(), proto.witness_remote_url, proto.witness_branch_ref, open_event, runner, proto.events_repo)
...
def verify_split_from_catalogue(I, proto):
    ...
    sys.path.insert(0, str(HERE.parents[0] / "corpus_identity")); import build_corpus_identity_v26 as B22, csv as _csv
