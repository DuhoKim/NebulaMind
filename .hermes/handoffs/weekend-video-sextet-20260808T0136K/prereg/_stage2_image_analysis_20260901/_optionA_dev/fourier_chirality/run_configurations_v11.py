"""run_configurations_v11 — V27 STAGED CANDIDATE (codex V26-1/2/3): composed mode on provenance_designs_v7 — a same-event contradiction is terminal (EVENT-FORGED),
...
→ v8 (V24: composed on provenance_designs_v4) → v9 (V25: composed on provenance_designs_v5) → v10 (V26: composed on provenance_designs_v7) → v11 (this).
...
class Protocol:
    ...
    events_runner: object = None                                                # composed mode: the `gh api` runner (production: provenance_designs_v7.gh_runner; tests inject a labelled fixture runner)
...
def _drand_modules():
    ...
    try:
        ...
        import beacon_record_drand_v27 as BD, verify_drand_v2 as vd, approval_witness_v4 as AW, history_v2 as H
        return BD, vd, AW, H
...
def composed_provenance(I, proto, w_root, ev, commit, log_path):
    """UNADOPTED COMPOSED MODE (v11 on provenance_designs_v7; Blanc 21:14 — the recommended mode exercised on the production call path, not in isolation). After every offline
    ...
    sys.path.insert(0, str(HERE.parents[0] / "track2")); import provenance_designs_v7 as P
    ...
    ok, hwhy, info = P.validate_continuation_v7(w_root, Path(log_path).resolve().relative_to(Path(w_root).resolve()).as_posix(), proto.witness_remote_url, proto.witness_branch_ref, open_event, runner, proto.events_repo)
...
def verify_split_from_catalogue(I, proto):
    ...
    sys.path.insert(0, str(HERE.parents[0] / "corpus_identity")); import build_corpus_identity_v27 as B22, csv as _csv
