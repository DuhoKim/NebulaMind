...
→ v8 (V24: composed on provenance_designs_v4) → v9 (V25: composed on provenance_designs_v5) → v10 (V26: composed on provenance_designs_v6) → v11 (V27: composed on provenance_designs_v7) → v12 (V28: composed on provenance_designs_v8, tri-state witness precheck) → v13 (this; V29: composed on provenance_designs_v9, precheck defers UNDETERMINED to live authentication).
...
`import provenance_designs_vN as P` line: v6→v2, v7→v3, v8→v4, v9→v5, v10→v6, v11→v7, v12→v8, v13→v9.]
...
def _P():
    """the track-2 helpers (v9), imported lazily: used by the witness-commit precheck in every mode and by composed mode."""
    sys.path.insert(0, str(HERE.parents[0] / "track2")); import provenance_designs_v9 as P; return P
...
class Protocol:
    ...
    provenance_mode: str = "offline"                                            # v6, UNADOPTED: "offline" (the candidate as reviewed at V21: retained bytes only) | "composed" (track-2 v9 helpers on this call path)
    events_runner: object = None                                                # composed mode: the `gh api` runner (production: provenance_designs_v9.gh_runner; tests inject a labelled fixture runner)
...
def _drand_modules():
    ...
    try:
        ...
        import beacon_record_drand_v29 as BD, verify_drand_v2 as vd, approval_witness_v4 as AW, history_v2 as H
        return BD, vd, AW, H
...
def composed_provenance(I, proto, w_root, ev, commit, log_path):
    """UNADOPTED COMPOSED MODE (v13 on provenance_designs_v9; Blanc 21:14 — the recommended mode exercised on the production call path, not in isolation). After every offline
    ...
    (`validate_continuation_v9`: ls-remote on the pinned URL, push-acknowledgement boundary, tri-state open-event stage that consults the feed before any retry) from the witnessed history-open commit."""
    ...
    ok, hwhy, info = P.validate_continuation_v9(w_root, Path(log_path).resolve().relative_to(Path(w_root).resolve()).as_posix(), proto.witness_remote_url, proto.witness_branch_ref, open_event, runner, proto.events_repo)
...
def verify_split_from_catalogue(I, proto):
    ...
    sys.path.insert(0, str(HERE.parents[0] / "corpus_identity")); import build_corpus_identity_v29 as B22, csv as _csv
