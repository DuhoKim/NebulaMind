...
`import provenance_designs_vN as P` line: v6→v2, v7→v3, v8→v4, v9→v5, v10→v6, v11→v7, v12→v8, v13→v9, v14→v10.]
...
def _P():
    """the track-2 helpers (v10), imported lazily: used by the witness-commit precheck in every mode and by composed mode."""
    sys.path.insert(0, str(HERE.parents[0] / "track2")); import provenance_designs_v10 as P; return P
...
class Protocol:
    ...
    provenance_mode: str = "offline"                                            # v6, UNADOPTED: "offline" (the candidate as reviewed at V21: retained bytes only) | "composed" (track-2 v10 helpers on this call path, ONE resolver)
    events_runner: object = None                                                # composed mode: the `gh api` runner (production: provenance_designs_v10.gh_runner; tests inject a labelled fixture runner)
...
def _drand_modules():
    ...
    try:
        ...
        import beacon_record_drand_v30 as BD, verify_drand_v2 as vd, approval_witness_v4 as AW, history_v2 as H
        return BD, vd, AW, H
...
def load_identity(identity_path, proto):
    ...
    if proto.require_beacon:                                                                               # v5: the FULL conjunction from COMMITTED evidence (codex V20 M1/M2/W3/W5)
        ...
        if proto.provenance_mode == "composed": composed_resolver(I, proto, w_root, ev, commit, lp, rp, adopted, ap_bytes, rnd)   # v14 (Blanc 03:20): ONE resolver — seed re-derivation is stage C of it, contributing, not deciding
        else:
...
def composed_resolver(I, proto, w_root, ev, commit, log_path, rp, adopted, ap_bytes, rnd):
...
def verify_split_from_catalogue(I, proto):
    ...
    sys.path.insert(0, str(HERE.parents[0] / "corpus_identity")); import build_corpus_identity_v30 as B22, csv as _csv
