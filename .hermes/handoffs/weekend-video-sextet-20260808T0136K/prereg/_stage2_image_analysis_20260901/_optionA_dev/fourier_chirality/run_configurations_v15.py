...
`import provenance_designs_vN as P` line: v6→v2, v7→v3, v8→v4, v9→v5, v10→v6, v11→v7, v12→v8, v13→v9, v14→v10, v15→v11.]
...
def _P():
    """the track-2 helpers (v11), imported lazily: used by the witness-commit precheck in every mode and by composed mode."""
    sys.path.insert(0, str(HERE.parents[0] / "track2")); import provenance_designs_v11 as P; return P
...
class Protocol:
    ...
    provenance_mode: str = "offline"                                            # v6, UNADOPTED: "offline" (the candidate as reviewed at V21: retained bytes only) | "composed" (track-2 v11 helpers on this call path, ONE resolver, staged collector)
    events_runner: object = None                                                # composed mode: the `gh api` runner (production: provenance_designs_v11.gh_runner; tests inject a labelled fixture runner)
...
def _drand_modules():
    ...
    try:
        ...
        import beacon_record_drand_v31 as BD, verify_drand_v2 as vd, approval_witness_v4 as AW, history_v2 as H
        return BD, vd, AW, H
...
def load_identity_composed(identity_path, proto):
    ...
    def S0():
        ...
        if o is not None: add("LOCAL-TERMINAL", "EVENT-INCONSISTENT", ("IDENTITY-WITNESS-COMMIT: " + x) if "does not deliver" in x else x, "S0-local")   # the composed name promised since V28 for positive non-delivery: EVENT-INCONSISTENT: IDENTITY-WITNESS-COMMIT
...
def verify_split_from_catalogue(I, proto):
    ...
    sys.path.insert(0, str(HERE.parents[0] / "corpus_identity")); import build_corpus_identity_v31 as B22, csv as _csv
