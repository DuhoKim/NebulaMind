...
→ v8 (V24: composed on provenance_designs_v4) → v9 (V25: composed on provenance_designs_v5) → v10 (V26: composed on provenance_designs_v6) → v11 (V27: composed on provenance_designs_v7) → v12 (V28: composed on provenance_designs_v8, tri-state witness precheck) → v13 (V29: composed on provenance_designs_v9, precheck defers UNDETERMINED to live authentication) → v14 (V30: ONE resolver over four contributing stages, provenance_designs_v10) → v15 (V31: the whole composed path as a staged finding collector, provenance_designs_v11) → v16 (this; V32: NSD — independent stages, page-preserving snapshot, allowlist classification, provenance_designs_v12).
...
`import provenance_designs_vN as P` line: v6→v2, v7→v3, v8→v4, v9→v5, v10→v6, v11→v7, v12→v8, v13→v9, v14→v10, v15→v11, v16→v12.]
...
def _P():
    """the track-2 helpers (v12), imported lazily: used by the witness-commit precheck in every mode and by composed mode."""
    sys.path.insert(0, str(HERE.parents[0] / "track2")); import provenance_designs_v12 as P; return P
...
class Protocol:
    ...
    provenance_mode: str = "offline"                                            # v6, UNADOPTED: "offline" (the candidate as reviewed at V21: retained bytes only) | "composed" (track-2 v12 helpers on this call path, NSD: independent contributing stages, ONE resolver)
    events_runner: object = None                                                # composed mode: the `gh api` runner (production: provenance_designs_v12.gh_runner; tests inject a labelled fixture runner)
...
def _drand_modules():
    ...
    try:
        ...
        import beacon_record_drand_v32 as BD, verify_drand_v2 as vd, approval_witness_v4 as AW, history_v2 as H
        return BD, vd, AW, H
...
REQUIRED_STAGES = ("S0-local", "S0d-delivery", "S0b-lists", "S0c-integrity", "S1-conjunction", "S2-snapshot", "S3-events", "S4-seed", "S5-history")
...
def load_identity_composed(identity_path, proto):
    ...
    if "rp" in ctx: I, w_root, ev, commit, lp, rp, adopted, ap_bytes, rnd = (ctx[k] for k in ("I", "w_root", "ev", "commit", "lp", "rp", "adopted", "ap_bytes", "rnd")); degraded = False
        ...
        ap_bytes = None
        aw = I.get("approval_witness") if isinstance(I.get("approval_witness"), dict) else {}
        if w_root and re.fullmatch(r"[0-9a-f]{40}", commit or "") and aw:
            try:
                r = subprocess.run(["git", "cat-file", "-p", f"{commit}:{aw.get('record_path', '')}"], cwd=w_root, capture_output=True); ap_bytes = r.stdout if r.returncode == 0 else None
            except Exception: ap_bytes = None
        if ap_bytes is None and aw.get("record_path") and aw.get("record_sha256"):                    # no git: the RETAINED record in the working tree, accepted only if its digest is the witness's (the seed check then needs no git)
            try:
                b = (Path(proto.seal_journal).parent / str(aw["record_path"])).read_bytes(); ap_bytes = b if hashlib.sha256(b).hexdigest() == aw["record_sha256"] else None
            except Exception: ap_bytes = None
        if adopted is None:
...
def verify_split_from_catalogue(I, proto):
    ...
    sys.path.insert(0, str(HERE.parents[0] / "corpus_identity")); import build_corpus_identity_v32 as B22, csv as _csv
...
PREREQUISITES = {
    "identity-file":     {"failure_code": "IDENTITY-MISSING",           "class": "LOCAL-TERMINAL",    "how": "the identity file absent / unreadable / not an object"},
    "open-file":         {"failure_code": "HISTORY-OPEN-EVENT-INVALID", "class": "LOCAL-TERMINAL",    "how": "the history-open file absent, not JSON, null or not a PushEvent object"},
    "git-launch":        {"failure_code": "IO-UNAVAILABLE",             "class": "RETRY-UNAVAILABLE", "how": "the git executable cannot be launched (OSError)"},
    "witness-fetch":     {"failure_code": "WITNESS-FETCH-FAILED",       "class": "RETRY-UNAVAILABLE", "how": "the freeze-witness fetch of the pinned remote fails"},
    "verifier-import":   {"failure_code": "VERIFIER-UNAVAILABLE",       "class": "RETRY-UNAVAILABLE", "how": "the BLS verifier stack (py_ecc) cannot be imported"},
    "events-page1":      {"failure_code": "RETRY-EVENTS-UNAVAILABLE",   "class": "RETRY-UNAVAILABLE", "how": "the first page of the events feed fails (transport / HTTP / malformed)"},
    "events-later-page": {"failure_code": "RETRY-EVENTS-UNAVAILABLE",   "class": "RETRY-UNAVAILABLE", "how": "a later page fails after earlier pages were obtained (a PARTIAL snapshot: obtained events are kept and evaluated; presence / absence / expiry / acceptance are never established from it)"},
    "remote-head":       {"failure_code": "HISTORY-CONTINUATION",       "class": "RETRY-UNAVAILABLE", "how": "ls-remote of the pinned remote answers nothing, or the ref's objects cannot be fetched (RETRY-REMOTE-UNAVAILABLE)"},
}
INDEPENDENCE = {   # check → (stage, prerequisites it truly needs, codes it can contribute)
    "approval-local":       {"stage": "S0-local",        "needs": {"identity-file"},                                                      "codes": {"EVENT-INCONSISTENT"}},
    "approval-delivery":    {"stage": "S0-local",        "needs": {"identity-file", "git-launch"},                                        "codes": {"EVENT-INCONSISTENT"}},
    "open-local":           {"stage": "S0-local",        "needs": {"open-file"},                                                          "codes": {"HISTORY-CONTINUATION: OPEN-EVENT-INCONSISTENT-INPUT"}},
    "identity-local":       {"stage": "S0c-integrity",   "needs": {"identity-file"},                                                      "codes": {"IDENTITY-SCHEMA", "IDENTITY-FIELD-MISSING", "IDENTITY-POOL-DIGEST", "IDENTITY-EXCLUSION-DIGEST", "IDENTITY-RULE-DIGEST", "IDENTITY-NOT-BEACON-SEEDED", "IDENTITY-BEACON-NOT-ACCEPTED", "IDENTITY-SOURCE", "IDENTITY-WITNESS-MISSING", "IDENTITY-WITNESS-COMMIT", "IDENTITY-WITNESS-TIME", "IDENTITY-WITNESS-LATE", "EVENT-DIGEST"}},
    "lists":                {"stage": "S0b-lists",       "needs": {"identity-file"},                                                      "codes": {"IDENTITY-tuning_objids-SIZE-OR-TYPE", "IDENTITY-holdout_objids-SIZE-OR-TYPE", "IDENTITY-fresh_validation_objids-SIZE-OR-TYPE", "IDENTITY-OVERLAP"}},
    "conjunction":          {"stage": "S1-conjunction",  "needs": {"identity-file", "git-launch", "witness-fetch", "verifier-import"},    "codes": {"IDENTITY-LOCK-MISMATCH", "COLLECTION-CLOSED", "COLLECTION-LOG-DIGEST", "NONCE-UNAUTHENTICATED", "ADOPTION-DIGEST", "APPROVAL-NOT-FIRST", "BEACON-RECORD-BINDING"}},
    "approval-live":        {"stage": "S3-events",       "needs": {"identity-file", "events-page1"},                                      "codes": {"EVENT-FORGED", "EVENT-NOT-EARLIEST", "EVENT-EXPIRED-NO-RECEIPT-PATH", "EVENT-EXPIRED-RECEIPT-REFUSED", "RETRY-EVENTS-INCOMPLETE", "RETRY-EVENTS-UNAVAILABLE"}},
    "open-same-id":         {"stage": "S3-events",       "needs": {"open-file", "events-page1"},                                          "codes": {"HISTORY-CONTINUATION: OPEN-EVENT-FORGED"}},
    "seed":                 {"stage": "S4-seed",         "needs": {"identity-file", "verifier-import"},                                   "codes": {"REDERIVE-RETRY", "IDENTITY-SEED-NOT-REDERIVED", "REDERIVE-REFUSE"}},
    "history-remote":       {"stage": "S5-history",      "needs": {"git-launch", "remote-head"},                                          "codes": {"HISTORY-CONTINUATION"}},
    "open-delivery-auth":   {"stage": "S5-history",      "needs": {"open-file", "git-launch", "remote-head", "events-page1"},             "codes": {"HISTORY-CONTINUATION", "RETRY-HISTORY-CONTINUATION"}},
    "per-entry":            {"stage": "S5-history",      "needs": {"git-launch", "remote-head", "events-page1"},                          "codes": {"HISTORY-CONTINUATION", "RETRY-HISTORY-CONTINUATION"}},
}
