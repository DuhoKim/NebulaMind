...
def _P():
    ...
    sys.path.insert(0, str(HERE.parents[0] / "track2")); import provenance_designs_v14 as P; return P
...
class Protocol:
    ...
    events_runner: object = None                                                # composed mode: the `gh api` runner (production: provenance_designs_v14.gh_runner; tests inject a labelled fixture runner)
...
APPROVAL_OUTCOME_CODES = {"approval-live": {"UNAVAILABLE": ("RETRY-UNAVAILABLE", "RETRY-EVENTS-UNAVAILABLE"), "INCOMPLETE": ("RETRY-INCOMPLETE", "RETRY-EVENTS-INCOMPLETE"), "INCONSISTENT-INPUT": ("LOCAL-TERMINAL", "EVENT-INCONSISTENT"), "NOT-EARLIEST": ("NOT-EARLIEST", "EVENT-NOT-EARLIEST"), "FORGED": ("FORGED", "EVENT-FORGED")}}   # v18: DYNAMIC CODE MAPS as declared data, verified against the table (seat B B-2: removing EVENT-FORGED from the table left all four table tests passing)
...
LAST_OUTCOME = None
def _core():
    sys.path.insert(0, str(HERE.parents[0] / "track2")); import precedence_core_v2 as PC; return PC
def load_identity_composed(identity_path, proto):
    ...
    STAGE_CHECKS = {"S0d-delivery": ("approval-delivery",), "S0e-lists": ("lists",), "S0f-integrity": ("identity-local", "identity-time-order"), "S0g-nonce": ("nonce",), "S2-snapshot": ("snapshot",), "S3a-approval": ("approval-live",), "S3b-open-same-id": ("open-same-id",), "S4-seed": ("seed",), "S5-history": ("history-remote", "open-delivery-auth", "per-entry", "history-working-tree")}
    def stage(name, body, source, check):
        stages[name] = {"state": "attempted"}; ctx["_stage"] = name                                        # v18: the owning stage for sub-check findings
    ...
    def sub(check, source, body, own_check=None):
        # one INDEPENDENT sub-check inside a stage: its own try, its own finding; the stage continues. v18 (seat B B-2): the finding carries the OWNING
        # STAGE rather than the literal "sub", and may be filed under its own check name so the table can name it.
        try: body()
        except _Blocked as e: block(e.check, str(e))
        except Exception as e: cls, code, why, unc = PC.classify_exception(e, source=source); add(cls, code, why, ctx.get("_stage", "sub"), own_check or check, unc)
    ...
    def S0f():
        ...
        checks = [
            ...
            lambda: _retained_event_shape(ev),                                                            # v18 (seat B B-1b): the RETAINED event's own payload is swept at its own input boundary, locally, so a malformed retained value is LOCAL-TERMINAL wherever a later remote-labelled stage happens to trip over the same bytes
        ]                                                                          # v18 (seat B B-1a): created_at, T_pulse and their order are THREE INDEPENDENT INPUTS, each its own sub-check
        for c in checks: sub("identity-local", "retained", c)
        for name, c in _time_checks(I, ev): sub(name, "retained", c, own_check=name)                       # v18 (seat B B-1a): each time input is its own sub-check UNDER ITS OWN CHECK NAME, so the table can name what contributed
    def _raise(msg): raise DataIntegrityFail(msg)
    def _retained_event_shape(ev):
        """v18 (seat B B-1b): every retained push-event field the later stages will traverse, checked HERE, against the retained copy. A malformed value raises
        PC.RetainedInputError, which classify_exception files as LOCAL-TERMINAL / MALFORMED-RETAINED-INPUT regardless of the stage that later consumes it."""
        if not isinstance(ev, dict): raise PC.RetainedInputError("the retained approval event is not an object")
        pay = ev.get("payload")
        if not isinstance(pay, dict): raise PC.RetainedInputError("the retained approval event's payload is not an object")
        cs = pay.get("commits")
        if cs is not None and not isinstance(cs, list): raise PC.RetainedInputError("the retained approval event's payload.commits is not a list")
        for k, c in enumerate(cs or []):
            if not isinstance(c, dict): raise PC.RetainedInputError(f"payload.commits[{k}] is {type(c).__name__}, not an object — the delivery predicate traverses it")
            if not isinstance(c.get("sha"), str): raise PC.RetainedInputError(f"payload.commits[{k}].sha is {type(c.get('sha')).__name__}, not a string")
        for f in ("before", "head", "ref"):
            if f in pay and not isinstance(pay[f], str): raise PC.RetainedInputError(f"payload.{f} is {type(pay[f]).__name__}, not a string")
    def _utc(v): return datetime.fromisoformat(str(v).replace("Z", "+00:00")).astimezone(timezone.utc)
    def _time_checks(I, ev):
        # v18 (seat B B-1a): ONE ELEMENT PER INPUT. The order comparison NEEDS both parses; when either input is unparsable it is BLOCKED BY NAME
        # (identity-time-order) instead of being hidden behind the first failure, which is what v17 did.
        def created():
            try: _utc((ev or {}).get("created_at"))
            except Exception: raise DataIntegrityFail("IDENTITY-WITNESS-TIME: unparsable created_at")
        def t_pulse():
            try: _utc(I.get("T_pulse"))
            except Exception: raise DataIntegrityFail("IDENTITY-T-PULSE: unparsable T_pulse")
        def order():
            try: c0, p0 = _utc((ev or {}).get("created_at")), _utc(I.get("T_pulse"))
            except Exception: raise _Blocked("identity-time-order", "the approval time and T_pulse must both parse before their order can be judged")
            if c0 >= p0: raise DataIntegrityFail("IDENTITY-WITNESS-LATE: the approval push time is not before T_pulse")
        return [("identity-local", created), ("identity-local", t_pulse), ("identity-time-order", order)]
    ...
    def S0h():
        ...
        except Exception as e: ctx["adopted"] = None; block("adoption-identity", f"no adopted digest to compare the identity against ({type(e).__name__}: {str(e)[:80]})"); raise   # v18 (seat B B-1d): ANY failure blocks the comparison BY NAME; v17 caught only DataIntegrityFail      # 'adoption' contributes the file's failure (refused stage); the identity comparison is BLOCKED by name
        ctx["adopted"] = adopted
    ...
    def S3a():
        ...
        m = APPROVAL_OUTCOME_CODES["approval-live"]                                                       # v18 (seat B B-2): declared at module level so the table verification sees codes selected through m[o]
        ...
        if o == "EXPIRED":
            ...
            if ok: ctx["approval_receipt_accepted"] = rwhy; return                                            # v18 (seat B B-3): a receipt that VERIFIES completes the branch; v17 fell through to m[o] with o="EXPIRED", raised KeyError and filed the verified receipt as MALFORMED-REMOTE-EVIDENCE
    ...
    def S5():
        ...
        info = P.history_findings_v14(w_root, Path(lp).resolve().relative_to(Path(w_root).resolve()).as_posix(), proto.witness_remote_url, proto.witness_branch_ref, ctx.get("open_event"), live, prov, proto.events_repo, lambda cls, code, why, check: add(cls, code, why, "S5-history", check)); ctx["hinfo"] = info
...
INDEPENDENCE = {   # check → (stage, the prerequisites its INPUTS need — no more, the codes it can contribute)
    ...
    "identity-time-order": {"stage": "S0f-integrity",    "needs": {"identity-file"},                                                                  "codes": {"IDENTITY-WITNESS-LATE"}},                # v18 (B-1a): the order comparison NEEDS both time parses; blocked by name when either fails
    "history-working-tree": {"stage": "S5-history",      "needs": {"helpers-import", "git-launch", "remote-head"},                                    "codes": {"HISTORY-CONTINUATION", "RETRY-HISTORY-CONTINUATION"}},   # v18 (B-1c): the working-tree comparison NEEDS an established acknowledged prefix
    ...
    "history-remote":     {"stage": "S5-history",        "needs": {"helpers-import", "git-launch", "remote-head"},                                     "codes": {"RETRY-HISTORY-CONTINUATION", "HISTORY-CONTINUATION", "IO-UNAVAILABLE", "MALFORMED-REMOTE-EVIDENCE"}},
