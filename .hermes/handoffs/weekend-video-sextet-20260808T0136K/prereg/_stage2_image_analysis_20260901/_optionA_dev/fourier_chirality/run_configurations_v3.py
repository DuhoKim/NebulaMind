#!/usr/bin/env python3
"""run_configurations_v3 (option A V19 draft): v2 plus the ACCEPTANCE-PATH checks the driver had never made (codex V18 item 4): the identity
...
def load_identity(identity_path, proto):
    ...
    if proto.require_beacon:                                                                               # v3 (codex V18 item 4): the acceptance path, verified here
        if not str(I.get("beacon_outcome", "")).startswith("ACCEPT"): raise DataIntegrityFail(f"IDENTITY-BEACON-NOT-ACCEPTED {I.get('beacon_outcome')}")
        w = I.get("approval_witness") or {}; ev = (w.get("push_event") or {})
        tp = I.get("beacon_t_pulse")
        if not (tp and ev.get("created_at")): raise DataIntegrityFail("IDENTITY-WITNESS-MISSING: no approval push event or T_pulse in the identity")
        if ev["created_at"].replace("Z", "+00:00") >= str(tp).replace("Z", "+00:00"): raise DataIntegrityFail("IDENTITY-WITNESS-LATE: the approval push time is not before T_pulse")
        lk = I.get("collection_lock") or {}; fa = lk.get("first_accept") or {}
        if fa.get("record_sha256") != I["beacon_record_sha256"] or fa.get("outcome") != I["beacon_outcome"] or fa.get("seed_hex") != I["seed_hex"]:
            raise DataIntegrityFail("IDENTITY-LOCK-MISMATCH: the first logged ACCEPT does not name this identity's record, outcome and seed")
