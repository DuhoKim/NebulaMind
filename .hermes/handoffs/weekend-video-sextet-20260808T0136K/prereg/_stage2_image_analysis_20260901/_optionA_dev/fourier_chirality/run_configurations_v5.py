#!/usr/bin/env python3
"""run_configurations_v5 (option A V21 draft): v4 plus the PRODUCTION WIRING codex's V20 gate found missing (FATAL: Protocol.rederive_seed was None, so
...
def _drand_modules():
    ...
    try:
        ...
        import beacon_record_drand_v21 as BD, verify_drand_v2 as vd, approval_witness_v4 as AW, history as H
        return BD, vd, AW, H
...
def reconcile_sentinels(rows, render_journal):
    ...
    for l in Path(render_journal).read_text().splitlines():
        ...
        if str(j.get("objid")) in ids and (j.get("status") == "REFUSED" or j.get("sentinel") is True):
            if not (j.get("status") == "REFUSED" and j.get("sentinel") is True and j.get("tensor_sha256") == SENTINEL_SHA256): raise DataIntegrityFail(f"RENDER-JOURNAL-REFUSAL-SHAPE {j.get('objid')}")
    ...
    if ends and ends[-1].get("refused_sentinel") != len(sent): raise DataIntegrityFail("RENDER-JOURNAL-COUNT: render-end refused count differs from the manifest's sentinel count")
