#!/usr/bin/env python3
"""run_configurations_v6 — V22 STAGED CANDIDATE (track 1 of Blanc's 20:45 order; NOT GATED, NOT ADOPTED). v6 = v5 + (codex V21 C2) W4 reproduced in the
...
def _drand_modules():
    ...
    try:
        ...
        import beacon_record_drand_v22 as BD, verify_drand_v2 as vd, approval_witness_v4 as AW, history_v2 as H
        return BD, vd, AW, H
...
def verify_split_from_catalogue(I, proto):
    ...
    sys.path.insert(0, str(HERE.parents[0] / "corpus_identity")); import build_corpus_identity_v22 as B22, csv as _csv
...
def reconcile_sentinels(rows, render_journal):
    ...
    for l in Path(render_journal).read_text().splitlines():
        ...
        if j.get("event") == "render-end": ends.append(j)
