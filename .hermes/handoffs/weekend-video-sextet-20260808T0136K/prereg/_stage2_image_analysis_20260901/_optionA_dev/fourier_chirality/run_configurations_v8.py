...
def _drand_modules():
    ...
    try:
        ...
        import beacon_record_drand_v24 as BD, verify_drand_v2 as vd, approval_witness_v4 as AW, history_v2 as H
        return BD, vd, AW, H
...
def composed_provenance(I, proto, w_root, ev, commit, log_path):
    """UNADOPTED COMPOSED MODE (v8 on provenance_designs_v4; Blanc 21:14 — the recommended mode exercised on the production call path, not in isolation). After every offline
    ...
    sys.path.insert(0, str(HERE.parents[0] / "track2")); import provenance_designs_v4 as P
    ...
    ok, hwhy, info = P.validate_continuation_v4(w_root, Path(log_path).resolve().relative_to(Path(w_root).resolve()).as_posix(), proto.witness_remote_url, proto.witness_branch_ref, open_event, runner, proto.events_repo)
...
def verify_split_from_catalogue(I, proto):
    ...
    sys.path.insert(0, str(HERE.parents[0] / "corpus_identity")); import build_corpus_identity_v24 as B22, csv as _csv
