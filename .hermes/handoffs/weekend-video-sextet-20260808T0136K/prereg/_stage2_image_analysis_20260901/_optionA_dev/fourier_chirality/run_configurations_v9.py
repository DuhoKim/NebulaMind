#!/usr/bin/env python3
"""run_configurations_v9 — V25 STAGED CANDIDATE (codex V24 N2/N3: composed mode on provenance_designs_v5 — EVIDENCE-UNAVAILABLE / EVIDENCE-INCOMPLETE are RETRY, HISTORY-PUBLICATION-BATCH is terminal). v8 bytes preserved. v8 was — V24 STAGED CANDIDATE (codex V23 M1/M2: composed mode validates the history with provenance_designs_v5 — one server-timed push event per history commit, in order). v7 bytes preserved. v7 was — V23 STAGED CANDIDATE (codex V22 A/B/C/E): composed mode uses provenance_designs_v5 — the history-open anchor is a RETAINED PushEvent (`history_open_event_file`) authenticated live and required to deliver a GENESIS-ONLY first history commit; one entry per acknowledged commit; the event predicate is W3's `delivers` (before..head included) in every mode; the render journal's render-end must be its LAST relevant record. v6 bytes preserved. v6 was — V22 STAGED CANDIDATE (track 1 of Blanc's 20:45 order; NOT GATED, NOT ADOPTED). v6 = v5 + (codex V21 C2) W4 reproduced in the
...
def _drand_modules():
    ...
    try:
        ...
        import beacon_record_drand_v25 as BD, verify_drand_v2 as vd, approval_witness_v4 as AW, history_v2 as H
        return BD, vd, AW, H
...
def composed_provenance(I, proto, w_root, ev, commit, log_path):
    """UNADOPTED COMPOSED MODE (v9 on provenance_designs_v5; Blanc 21:14 — the recommended mode exercised on the production call path, not in isolation). After every offline
    ...
    sys.path.insert(0, str(HERE.parents[0] / "track2")); import provenance_designs_v5 as P
    ...
    ok, hwhy, info = P.validate_continuation_v5(w_root, Path(log_path).resolve().relative_to(Path(w_root).resolve()).as_posix(), proto.witness_remote_url, proto.witness_branch_ref, open_event, runner, proto.events_repo)
    if not ok: raise DataIntegrityFail(("RETRY-" if hwhy.startswith("EVIDENCE-") else "") + f"HISTORY-CONTINUATION: {hwhy}")   # v9: EVIDENCE-* dispositions are RETRY, never terminal
...
def verify_split_from_catalogue(I, proto):
    ...
    sys.path.insert(0, str(HERE.parents[0] / "corpus_identity")); import build_corpus_identity_v25 as B22, csv as _csv
