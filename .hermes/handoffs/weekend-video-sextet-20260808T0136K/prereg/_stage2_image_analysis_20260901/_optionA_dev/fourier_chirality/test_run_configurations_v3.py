...
import run_configurations_v3 as rc, fourier_chirality as fc
...
def TPJ(tmp, n_tune=6, floor_tune=5, n_hold=60, floor_hold=57, label="TEST-PROTOCOL", n_fresh=7):
    ...
    rc.ADOPTION_FILE = tmp / "ADOPTED_RULE_SHA256.txt"; rc.ADOPTION_FILE.write_text("c" * 64 + "\n")
    return rc.Protocol(n_tune=n_tune, floor_tune=floor_tune, n_hold=n_hold, floor_hold=floor_hold, n_fresh=n_fresh, label=label, seal_journal=str(work / "seal.jsonl"), collection_log=str(work / "collection_log.jsonl"), witness_remote_ref="origin/main", witness_fetch=True, witness_remote_url=str(bare),
                       pool_sha256="p" * 64, exclusion_sha256="x" * 64, require_beacon=True, rule_sha256="c" * 64)
...
def identity(tmp, tp, tune_ids, hold_ids, sealed=True, outcome="ACCEPT-DRAND", push_at="2026-09-06T03:05:00Z", t_pulse="2026-09-06T03:11:00Z", lock_ok=True, log_ok=True, witness=True):
    """PRODUCTION-SHAPED identity (CORPUS-IDENTITY-2 + the V19 acceptance-path fields), written INSIDE the test repo with its collection log so
    the seal commit carries BOTH (driver v3 requires the identity AND the log to be blobs at the witnessed commit)."""
    fresh = list(range(900000, 900000 + tp.n_fresh)); work = Path(tp.seal_journal).parent
    log = Path(tp.collection_log); rec = "b" * 64; seed = "0" * 64
    entries = [{"utc": "2026-09-06T03:11:30Z", "stage": "builder-verdict", "record_sha256": rec, "outcome": outcome, "seed_hex": seed}, {"utc": "2026-09-06T03:11:30Z", "stage": "builder-accept", "record_sha256": rec if lock_ok else "c" * 64, "outcome": outcome, "seed_hex": seed}]
    log.write_text("".join(json.dumps(e, sort_keys=True) + "\n" for e in entries)); log_sha = rc.sha_file(log)
    I = {"schema_version": "CORPUS-IDENTITY-2", "tuning_objids": [int(x) for x in tune_ids], "holdout_objids": [int(x) for x in hold_ids], "fresh_validation_objids": fresh, "pool_sha256": "p" * 64,
         "ordering_rule": "test", "seed_hex": seed, "split_mode": "beacon-seeded (test)", "renderability_inputs": {}, "detail": {}, "exclude_file_sha256": "x" * 64, "beacon_record_sha256": rec, "rule_sha256": tp.rule_sha256,
         "beacon_outcome": outcome, "beacon_t_pulse": t_pulse, "collection_lock": {"first_accept": entries[1], "log_sha256": log_sha if log_ok else "0" * 64}}
    if witness: I["approval_witness"] = {"push_event": {"created_at": push_at, "payload": {"ref": "refs/heads/main"}}}
    p = work / f"identity_{tmp.name}.json"; p.write_text(json.dumps(I))
    if sealed: seal(tp, "corpus-identity-freeze", rc.sha_file(p), files=[p, log])
    return p
...
class T(unittest.TestCase):
    ...
    def test_real_builder_output_loads_through_load_identity(self):        # END-TO-END (V19): expedited record -> V19 builder (witness v3, history, adoption) -> driver v3
        import importlib.util, subprocess, os
        from datetime import datetime, timezone, timedelta
        sys.path.insert(0, str(HERE.parents[1] / "_optionA_dev" / "beacon_v2")); sys.path.insert(0, str(HERE.parents[1] / "_optionA_dev" / "corpus_identity"))
        import beacon_record_expedited as X, drand_round, test_pki as pki, build_corpus_identity_v19 as B19
        ...
        work = Path(prod_like.seal_journal).parent; bare = prod_like.witness_remote_url
        D = "a" * 64; t_sign = datetime(2026, 9, 6, 3, 0, 7, tzinfo=timezone.utc); tp = X.pulse_time(t_sign); now = tp + timedelta(minutes=1); rnd = drand_round.round_for(tp); r_sign = drand_round.round_for(t_sign); nonce = "e" * 64
        stmt = work / "APPROVAL_RECORD_SELRULE_V19_E2E.md"; stmt.write_bytes(f"RULE_SHA256: {D}\nAPPROVAL_UTC: {X.fmt(t_sign)}\nDRAND_AT_APPROVAL: round {r_sign} randomness {nonce}\n".encode())
        B19.ADOPTION_FILE = work / "ADOPTED_RULE_SHA256.txt"; B19.ADOPTION_FILE.write_text(D + "\n"); rc.ADOPTION_FILE = B19.ADOPTION_FILE
        ...
        B19.WITNESS_REMOTE_REF = "origin/main"; B19.WITNESS_REMOTE_URL = bare; B19.APPROVAL_GLOB = "APPROVAL_RECORD_SELRULE_V19*"; B19.WITNESS_FETCH = True; B19.BRANCH_REF = "refs/heads/main"; B19.nist_pulse.pinned_roots = pki.roots; B19.COLLECTION_LOG = Path(prod_like.collection_log)
        B19.EVENTS = lambda: ([{"type": "PushEvent", "id": "1", "created_at": (tp - timedelta(minutes=3)).strftime("%Y-%m-%dT%H:%M:%SZ"), "payload": {"ref": "refs/heads/main", "head": commit, "commits": [{"sha": commit}]}}], {"endpoints": ["e"], "retrieved_utc": "x", "gh_version": "gh test"})
        ...
        rec = work / "beacon_record.json"; rec.write_text(json.dumps(X.collect(net, X.fmt(t_sign), D, stmt.read_bytes(), now=now)))
        out = work / "ident_e2e"; out.mkdir(); excl = HERE.parents[1] / "_optionA_dev" / "corpus_identity" / "dryrun_identities_to_exclude_20260905.txt"
        ok = B19.build(out_dir=out, beacon_record_path=rec, exclude_path=excl, rule_sha256=D, signature_statement=stmt, fetch=net, now=now); self.assertTrue(ok)
        ...
        ident = out / "corpus_identity.json"; I = json.loads(ident.read_text()); self.assertEqual(I["beacon_outcome"], "ACCEPT-DRAND"); self.assertEqual(I["collection_lock"]["first_accept"]["record_sha256"], I["beacon_record_sha256"])
        seal(prod_like, "corpus-identity-freeze", rc.sha_file(ident), files=[ident, Path(prod_like.collection_log)])
        Iw, d = rc.load_identity(ident, prod_like); self.assertEqual(len(Iw["tuning_objids"]), 400); self.assertIn("commit", Iw["_witness"])
    def test_witness_requires_pushed_commit_containing_record(self):            # both V6 seats: the journal alone is not a witness
        ...
        seal(self.TP, "corpus-identity-freeze", d, push=False, files=[ident, Path(self.TP.collection_log)])                                  # committed locally, NOT pushed
    ...
    def test_sentinel_tensor_is_counted_and_unscored(self):                     # v2/v3: a render-refused object stays in n, is UNSCORED, and is counted
        ...
        recs = [json.loads(l) for l in open(self.tmp / "sent" / "tuning_receipts.jsonl")]
        self.assertTrue(all(r["sentinel_count"] == 1 for r in recs)); self.assertTrue(all(r["unscored"] >= 1 and r["n"] == 6 for r in recs))
        J = [json.loads(l) for l in open(self.tmp / "sent" / "tuning_journal.jsonl")]; self.assertTrue(all(j["status"] == "UNSCORED" for j in J if j["objid"] == oid))
    def test_v3_acceptance_path_checks(self):                                 # codex V18 item 4: the driver verifies outcome, witness, lock and log
        rows = self.rows(6); m = self.tmp / "m.csv"; mk_manifest(m, rows); ids = [r[0] for r in rows]; hold = [str(500000 + i) for i in range(60)]
        for kw, tok in (({"outcome": "RETRY"}, "IDENTITY-BEACON-NOT-ACCEPTED"), ({"witness": False}, "IDENTITY-WITNESS-MISSING"), ({"push_at": "2026-09-06T03:11:00Z"}, "IDENTITY-WITNESS-LATE"), ({"lock_ok": False}, "IDENTITY-LOCK-MISMATCH"), ({"log_ok": False}, "COLLECTION-LOG-DIGEST")):
            ident = identity(self.tmp, self.TP, ids, hold, **kw)
            with self.assertRaises(rc.DataIntegrityFail) as cm: rc.tune(m, self.tens, self.tmp / ("v3" + tok), ident, self.TP)
            self.assertIn(tok, str(cm.exception), kw)
        ident = identity(self.tmp, self.TP, ids, hold); Path(self.TP.collection_log).write_text(Path(self.TP.collection_log).read_text() + "\n")   # log changed after the seal
        with self.assertRaises(rc.DataIntegrityFail) as cm: rc.tune(m, self.tens, self.tmp / "v3log", ident, self.TP)
