...
import run_configurations_v4 as rc, fourier_chirality as fc
...
def TPJ(tmp, n_tune=6, floor_tune=5, n_hold=60, floor_hold=57, label="TEST-PROTOCOL", n_fresh=7):
    ...
    return rc.Protocol(n_tune=n_tune, floor_tune=floor_tune, n_hold=n_hold, floor_hold=floor_hold, n_fresh=n_fresh, label=label, seal_journal=str(work / "seal.jsonl"), collection_log=str(work / "collection_log.jsonl"), witness_branch_ref="refs/heads/main", adoption_file=str(work / "ADOPTED_RULE_SHA256.txt"), beacon_record_path=str(work / "beacon_record.json"), rederive_seed=(lambda p: json.loads(Path(p).read_text()).get("seed_hex")), witness_remote_ref="origin/main", witness_fetch=True, witness_remote_url=str(bare),
...
def identity(tmp, tp, tune_ids, hold_ids, sealed=True, outcome="ACCEPT-DRAND", push_at="2026-09-06T03:05:00Z", t_pulse="2026-09-06T03:11:00Z", lock_ok=True, log_ok=True, witness=True, source="drand-mainnet-default", seed_ok=True, adoption_ok=True, conflict=False):
    """PRODUCTION-SHAPED identity + the V20 acceptance-path evidence: an APPROVAL COMMIT carrying the adoption file (its sha is the witness commit),
    an entire PushEvent for the protected ref delivering that commit, a beacon record file whose seed the injected rederiver returns, a
    collection log with a builder-verdict + builder-accept naming the record/outcome/seed; identity + log sealed in the same commit."""
    fresh = list(range(900000, 900000 + tp.n_fresh)); work = Path(tp.seal_journal).parent
    ap = Path(tp.adoption_file); ap.write_text(tp.rule_sha256 + ("\n" if adoption_ok else "\n\n")); git(work, "add", ap.name); git(work, "commit", "-q", "--allow-empty", "-m", "approval"); commit = git(work, "rev-parse", "HEAD"); git(work, "push", "-q", "origin", "HEAD:refs/heads/main")
    rp = Path(tp.beacon_record_path); seed = "0" * 64; rp.write_text(json.dumps({"schema": "BEACON-RECORD-4", "seed_hex": seed if seed_ok else "f" * 64})); rec = rc.sha_file(rp)
    log = Path(tp.collection_log)
    ...
    if conflict: entries.append({"utc": "2026-09-06T03:20:00Z", "stage": "builder-conflict", "record_sha256": "d" * 64, "outcome": "ACCEPT-DRAND", "seed_hex": "1" * 64})
    ...
    I = {"schema_version": "CORPUS-IDENTITY-2", "tuning_objids": [int(x) for x in tune_ids], "holdout_objids": [int(x) for x in hold_ids], "fresh_validation_objids": fresh, "pool_sha256": "p" * 64,
         ...
         "beacon_outcome": outcome, "beacon_source": source, "beacon_t_pulse": t_pulse, "adoption_sha256": rc.sha_file(ap), "collection_lock": {"first_accept": entries[1], "log_sha256": log_sha if log_ok else "0" * 64}}
    if witness: I["approval_witness"] = {"commit": commit, "nonce_round": 6441000, "nonce_randomness": "e" * 64, "push_event": {"type": "PushEvent", "id": "1", "created_at": push_at, "payload": {"ref": "refs/heads/main", "head": commit, "commits": [{"sha": commit}]}}}
    p = work / f"identity_{tmp.name}.json"; p.write_text(json.dumps(I))
    if sealed: seal(tp, "corpus-identity-freeze", rc.sha_file(p), files=[p, log, rp])
    return p
...
class T(unittest.TestCase):
    ...
    def test_real_builder_output_loads_through_load_identity(self):        # END-TO-END (V20): real public round (BLS-verified) -> V20 builder -> driver v4
        ...
        import beacon_record_drand as BD, verify_drand as vd, build_corpus_identity_v20 as B20
        ROUND = 6441924; REAL = json.loads((HERE.parents[1] / "_optionA_dev" / "drand_only" / "round_6441924_api.drand.sh.json").read_text()); BODY = json.dumps(REAL).encode()
        tp = vd.round_time(ROUND); t_sign = tp - timedelta(seconds=600); ts = BD.fmt(t_sign); now = tp + timedelta(minutes=1); D = "a" * 64
        ...
        try:
            ...
            work = Path(prod_like.seal_journal).parent; bare = prod_like.witness_remote_url; r_sign = vd.round_for(t_sign); nonce = "e" * 64
            stmt = work / "APPROVAL_RECORD_SELRULE_V20_E2E.md"; stmt.write_bytes(f"RULE_SHA256: {D}\nAPPROVAL_UTC: {ts}\nDRAND_AT_APPROVAL: round {r_sign} randomness {nonce}\n".encode())
            ...
            B20.WITNESS_REMOTE_REF = "origin/main"; B20.WITNESS_REMOTE_URL = bare; B20.APPROVAL_GLOB = "APPROVAL_RECORD_SELRULE_V20*"; B20.WITNESS_FETCH = True; B20.BRANCH_REF = "refs/heads/main"; B20.COLLECTION_LOG = Path(prod_like.collection_log)
            ...
            rec = Path(prod_like.beacon_record_path); rec.write_text(json.dumps(BD.collect(net, ts, D, stmt.read_bytes(), now=now)))
            ...
            prod_like = rc.Protocol(**{**prod_like.__dict__, "pool_sha256": rc.PRODUCTION.pool_sha256, "exclusion_sha256": rc.PRODUCTION.exclusion_sha256, "rule_sha256": D,
                                       "rederive_seed": (lambda p: BD.verdict(json.loads(Path(p).read_text()), now).get("seed_hex"))})
            ...
            Iw, d = rc.load_identity(ident, prod_like); self.assertEqual(len(Iw["tuning_objids"]), 400)
    ...
    def test_v4_semantic_conjunction(self):                                    # V19 audit M1/M2: the driver verifies evidence, not fields
        ...
        cases = (({"outcome": "RETRY"}, "IDENTITY-BEACON-NOT-ACCEPTED"), ({"outcome": "ACCEPT-INVENTED"}, "IDENTITY-BEACON-NOT-ACCEPTED"), ({"outcome": "ACCEPT-NIST"}, "IDENTITY-BEACON-NOT-ACCEPTED"),
                 ({"source": "other"}, "IDENTITY-SOURCE"), ({"witness": False}, "IDENTITY-WITNESS-MISSING"), ({"push_at": "2026-09-06T03:11:00Z"}, "IDENTITY-WITNESS-LATE"),
                 ({"lock_ok": False}, "IDENTITY-LOCK-MISMATCH"), ({"log_ok": False}, "COLLECTION-LOG-DIGEST"), ({"conflict": True}, "COLLECTION-LOG-CONFLICT"), ({"adoption_ok": False}, "ADOPTION-MALFORMED"), ({"seed_ok": False}, "IDENTITY-SEED-NOT-REDERIVED"))
        for kw, tok in cases:
            ident = identity(self.tmp, self.TP, ids, hold, **kw)
            with self.assertRaises(rc.DataIntegrityFail) as cm: rc.tune(m, self.tens, self.tmp / ("v4" + tok), ident, self.TP)
            self.assertIn(tok, str(cm.exception), kw)
        ident = identity(self.tmp, self.TP, ids, hold)                            # a complete identity tunes
