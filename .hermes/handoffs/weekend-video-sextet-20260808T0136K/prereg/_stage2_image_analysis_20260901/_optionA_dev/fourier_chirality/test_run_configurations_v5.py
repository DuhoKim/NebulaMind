...
import run_configurations_v5 as rc, fourier_chirality as fc
...
import beacon_record_drand_v21 as BD, verify_drand_v2 as vd, history as H
...
def identity(tmp, tp, tune_ids, hold_ids, sealed=True, outcome="ACCEPT-DRAND", push_at=None, t_pulse=None, lock_ok=True, log_ok=True, witness=True, source="drand-mainnet-default", seed_ok=True, adoption_ok=True, conflict=False, mutate=None, seal_files=None, nonce_ok=True, ancestry=False):
    """PRODUCTION-SHAPED identity + REAL V21 evidence (see the module docstring). Knobs fabricate ONE defect each; `mutate(I, ctx)` edits the identity
    dict before sealing; `seal_files` overrides what the freeze commit carries. The exhibit-round exclusion is lifted for the fixture only."""
    ...
    _N[0] += 1; stmt = work / f"APPROVAL_RECORD_SELRULE_V21_T{_N[0]}.md"; stmt_bytes = f"RULE_SHA256: {D}\nAPPROVAL_UTC: {TS}\nDRAND_AT_APPROVAL: round {NR} randomness {NONCE}\n".encode(); stmt.write_bytes(stmt_bytes)
    ap = Path(tp.adoption_file); ap.write_text(tp.rule_sha256 + ("\n" if adoption_ok else "\n\n")); base = git(work, "rev-parse", "HEAD")
    git(work, "add", ap.name, stmt.name); git(work, "commit", "-q", "-m", "approval"); commit = git(work, "rev-parse", "HEAD")
...
class T(unittest.TestCase):
    ...
    def test_real_builder_output_loads_through_load_identity(self):        # END-TO-END (V21): real public round (BLS-verified) -> collector CLI (genesis) -> V21 builder -> driver v5 with PRODUCTION's re-deriver
        ...
        import build_corpus_identity_v21 as B20
        ...
        try:
            ...
            stmt = work / "APPROVAL_RECORD_SELRULE_V21_E2E.md"; stmt.write_bytes(f"RULE_SHA256: {D}\nAPPROVAL_UTC: {ts}\nDRAND_AT_APPROVAL: round {r_sign} randomness {nonce}\n".encode())
            ...
            B20.WITNESS_REMOTE_REF = "origin/main"; B20.WITNESS_REMOTE_URL = bare; B20.APPROVAL_GLOB = "APPROVAL_RECORD_SELRULE_V21*"; B20.WITNESS_FETCH = True; B20.BRANCH_REF = "refs/heads/main"; B20.COLLECTION_LOG = Path(prod_like.collection_log)
    ...
    def test_sentinel_tensor_is_counted_and_unscored(self):                     # v2/v3: a render-refused object stays in n, is UNSCORED, and is counted
        ...
        RJ.write_text(json.dumps({"objid": rows[1][0], "status": "REFUSED", "sentinel": True, "tensor_sha256": rc.SENTINEL_SHA256}) + "\n")                    # journal refuses a DIFFERENT object: mismatch both ways
        ...
        RJ.write_text(json.dumps({"objid": oid, "status": "REFUSED", "sentinel": True, "tensor_sha256": rc.SENTINEL_SHA256}) + "\n" + json.dumps({"event": "render-end", "refused_sentinel": 2}) + "\n")
