...
import run_configurations_v7 as rc, fourier_chirality as fc
...
import beacon_record_drand_v23 as BD, verify_drand_v2 as vd, history_v2 as H
...
class T(unittest.TestCase):
    ...
    def test_real_builder_output_loads_through_load_identity(self):        # END-TO-END (V21): real public round (BLS-verified) -> collector CLI (genesis) -> V21 builder -> driver v5 with PRODUCTION's re-deriver
        ...
        import build_corpus_identity_v23 as B20
    ...
    def test_composed_mode_on_the_production_call_path(self):                  # UNADOPTED composed mode: track-2 v2 helpers inside load_identity (Blanc 21:14: not in isolation)
        ...
        def pev(commit, n):                                                        # a fixture PushEvent (labelled) for a published history commit
            return {"type": "PushEvent", "id": f"h{n}", "created_at": f"2026-09-06T09:{50 + n:02d}:00Z", "repo": {"name": "DuhoKim/NebulaMind"}, "payload": {"ref": "refs/heads/main", "head": commit, "commits": [{"sha": commit}]}}
        def publish(I, ctx, wk=None, of=None, evs=None):
            ...
            for n in range(len(lines)):
                log.write_bytes(b"\n".join(lines[: n + 1]) + b"\n"); git(wk, "add", log.name); git(wk, "commit", "-q", "-m", f"history entry {n}"); c = git(wk, "rev-parse", "HEAD"); git(wk, "push", "-q", "origin", "HEAD:refs/heads/main"); evs.append(pev(c, n))
        ...
        with self.assertRaises(rc.DataIntegrityFail) as cm: rc.load_identity(ident, rc.Protocol(**{**composed_TP.__dict__, "events_runner": feed([{**ev, "id": "z", "created_at": "2027-01-01T00:00:00Z"}])}))   # expired, no receipt path configured (Q1 Option C)
        ...
        self.assertIn("OPEN-EVENT", str(cm.exception))
        ...
        def late_first(I, ctx):
            ...
            e = H.append(ctx["log"], {"stage": "builder-accept", "record_sha256": I["beacon_record_sha256"], "outcome": "ACCEPT-DRAND", "seed_hex": I["seed_hex"], "source": "drand-mainnet-default"})
            I["collection_lock"] = {"first_accept": e, "log_sha256": rc.sha_file(ctx["log"]), "entries": 2}
            git(work3, "add", ctx["log"].name); git(work3, "commit", "-q", "-m", "late first publication"); c = git(work3, "rev-parse", "HEAD"); git(work3, "push", "-q", "origin", "HEAD:refs/heads/main"); EVS3.append(pev(c, 0)); open3.write_text(json.dumps(EVS3[0])); git(work3, "add", open3.name)
