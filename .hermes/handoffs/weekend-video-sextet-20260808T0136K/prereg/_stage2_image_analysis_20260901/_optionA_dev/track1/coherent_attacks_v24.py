#!/usr/bin/env python3
"""COHERENT-ATTACK INSPECTION of the STAGED V24 candidate (driver v8, verdict v24, provenance_designs_v4; supersedes the V23 inspection; coherent_attacks.py retained as pinned by V23) (Blanc 20:45: "inspect BOTH the successful production path AND the prior reviewer's coherent
...
import run_configurations_v8 as rc, test_run_configurations_v8 as TF, history_v2 as H, provenance_designs_v4 as P, verify_drand_v2 as vd
...
import beacon_record_drand_v24 as BD
...
def vc(open_event=None, events=None): ok, why, _ = P.validate_continuation_v4(w, log.name, str(bare), "refs/heads/main", open_event or EV9[0], feed9(events if events is not None else EV9), "DuhoKim/NebulaMind"); return "OK" if ok else "REFUSED " + why.split(":")[0]
rows.append(("9c track 2(b) v4 validate_continuation_v4(genuine: genesis-only open commit, authentic open event, one server-timed push event per commit)", vc(), "asks the remote; authenticates the open event; per-commit push events"))
...
ok, why, _ = P.validate_continuation_v4(w2, log2.name, str(bare2), "refs/heads/main", pev(lc, 0, w2), feed9([pev(lc, 0, w2)]), "DuhoKim/NebulaMind"); rows.append(("9k track 2(b) v4: codex V22 attack A — rebuilt history published as the FIRST commit, named history-open", "OK" if ok else "REFUSED " + why.split(":")[0], "v2 said True"))
...
ok3, why3, _ = P.validate_continuation_v3(w3, log3.name, str(bare3), "refs/heads/main", late, feed9([late]), "DuhoKim/NebulaMind"); ok4, why4, _ = P.validate_continuation_v4(w3, log3.name, str(bare3), "refs/heads/main", late, feed9([late]), "DuhoKim/NebulaMind")
rows.append(("9l codex V23 M1: four single-entry commits delivered by ONE late push", f"v3: {'OK' if ok3 else 'REFUSED'} / v4: {'OK' if ok4 else 'REFUSED ' + why4.split(':')[0]}", "v3 said True; v4 requires one server-timed push event per commit"))
