#!/usr/bin/env python3
"""COHERENT-ATTACK INSPECTION of the STAGED V26 candidate (driver v10, verdict v26, provenance_designs_v6; supersedes the V25 inspection; earlier scripts retained
as pinned). Re-runs every reviewer counter-case with adjacent digests, counts and locks kept COHERENT, against driver v10 with PRODUCTION's own re-deriver, and the
standalone v6 helpers. Prints one row per case. Deterministic; the output is filed as V26_CANDIDATE_ATTACK_INSPECTION_20260907.md."""
...
import run_configurations_v10 as rc, test_run_configurations_v10 as TF, history_v2 as H, provenance_designs_v6 as P, verify_drand_v2 as vd
...
import beacon_record_drand_v26 as BD
...
def vc(open_event=None, events=None): ok, why, _ = P.validate_continuation_v6(w, log.name, str(bare), "refs/heads/main", open_event or EV9[0], feed9(events if events is not None else EV9), "DuhoKim/NebulaMind"); return "OK" if ok else "REFUSED " + why.split(":")[0]
rows.append(("9c track 2(b) v4 validate_continuation_v6(genuine: genesis-only open commit, authentic open event, one server-timed push event per commit)", vc(), "asks the remote; authenticates the open event; per-commit push events"))
...
ok, why, _ = P.validate_continuation_v6(w2, log2.name, str(bare2), "refs/heads/main", pev(lc, 0, w2), feed9([pev(lc, 0, w2)]), "DuhoKim/NebulaMind"); rows.append(("9k track 2(b) v6: codex V22 attack A — rebuilt history published as the FIRST commit, named history-open", "OK" if ok else "REFUSED " + why.split(":")[0], "v2 said True"))
...
ok3, why3, _ = P.validate_continuation_v3(w3, log3.name, str(bare3), "refs/heads/main", late, feed9([late]), "DuhoKim/NebulaMind"); ok4, why4, _ = P.validate_continuation_v6(w3, log3.name, str(bare3), "refs/heads/main", late, feed9([late]), "DuhoKim/NebulaMind")
rows.append(("9l codex V23 M1: four single-entry commits delivered by ONE late push", f"v3: {'OK' if ok3 else 'REFUSED'} / v6: {'OK' if ok4 else 'REFUSED ' + why4.split(':')[0]}", "v3 said True; v6 requires one server-timed push event per commit (proven batch = terminal)"))
...
ok, why, _ = P.validate_continuation_v6(w, log.name, str(bare), "refs/heads/main", EV9[0], feeds9(EV9, []), "DuhoKim/NebulaMind"); rows.append(("9m codex V24 N2: complete feed for approval/open, EMPTY feed on the per-entry retrieval", "REFUSED " + why.split(":")[0], "v4 said BATCH; v5 = retry"))
ok, why, _ = P.validate_continuation_v6(w, log.name, str(bare), "refs/heads/main", EV9[0], feed9([EV9[0], EV9[2]]), "DuhoKim/NebulaMind"); rows.append(("9n codex V24 N2: one middle per-commit event missing, no evidence of batching", "REFUSED " + why.split(":")[0], "v4 said BATCH; v5 = retry within the window"))
...
ok5, why5, _ = P.validate_continuation_v5(w3, log3.name, str(bare3), "refs/heads/main", late_anc, feed9([late_anc]), "DuhoKim/NebulaMind"); ok6, why6, _ = P.validate_continuation_v6(w3, log3.name, str(bare3), "refs/heads/main", late_anc, feed9([late_anc]), "DuhoKim/NebulaMind")
...
print("| attack | staged V26 candidate (driver v10 / verdict v26 / provenance v6) | note |"); print("|---|---|---|")
