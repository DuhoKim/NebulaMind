"""THE TABLE IS A FIRST-CLASS OBJECT (Blanc 06:35, item 5): INDEPENDENCE is VERIFIED AGAINST THE CODE by this test, which fails when they diverge — not hand-maintained beside
...
import run_configurations_v17 as rc
SRC = (D / "fourier_chirality" / "run_configurations_v17.py").read_text(encoding="utf-8")
BODY = SRC[SRC.index("def load_identity_composed("):SRC.index("def verify_split_from_catalogue(")]
PSRC = (D / "track2" / "provenance_designs_v13.py").read_text(encoding="utf-8"); PBODY = PSRC[PSRC.index("def history_findings_v13("):PSRC.index("def validate_continuation_v13(")]   # checks contributed through the S5 callback live here
...
class TableVsCode(unittest.TestCase):
    ...
    def test_prerequisites_are_declared_and_all_pairs_generated(self):
        ...
        import test_track13_nsd_table as T
