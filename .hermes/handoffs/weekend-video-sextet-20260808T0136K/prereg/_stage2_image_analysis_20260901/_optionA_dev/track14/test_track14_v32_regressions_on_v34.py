"""TRACK 13 REGRESSIONS RUN AGAINST THE V34 SUCCESSORS (v18 / v14): codex's V32 constructions must stay repaired. Original header follows.
TRACK 13 — codex's V32 constructions as REGRESSIONS, one subcase per method (Blanc 05:02 item 3). Targets the SUCCESSORS (provenance_designs_v14,
run_configurations_v18); run FIRST against byte-copies of the V32 modules (import lines renamed only). The table-derived controls (v2, with identity-file pairs,
...
import provenance_designs_v14 as P
...
class V32_3_Durable(unittest.TestCase):
    ...
    def test_standalone_contributes_malformed_remote_evidence(self):
        ...
        ok, why, info = P.validate_continuation_v14(env["wk"], Path(env["cTP"].collection_log).name, env["cTP"].witness_remote_url, REF, env["evs"][0], feed(env["live"] + [bad]), REPO); self.assertFalse(ok); self.assertIn("MALFORMED-REMOTE-EVIDENCE", why + json.dumps(info.get("findings", [])))
class V32_5_Accounting(unittest.TestCase):
    ...
    def test_history_success_wording_claims_blob_equality_not_head(self):
        src = (D / "track2" / "provenance_designs_v14.py").read_text(encoding="utf-8"); i = src.index("def history_findings_v14"); body = src[i:i + 12000]; self.assertNotIn("working tree equals the live remote head", body); self.assertIn("history blob", body)
