"""TRACK 14 v4 — seat B's V33 counter-cases, conforming to the FAIL-FIRST KIT (scripts/failfirst_kit.py; Blanc 08:51 KST): ONE outcome assertion per method, and
...
v3 is retained unchanged (its 1c injection was fixture-defective against the successors — see s_unreadable_blob). Predecessors retained unchanged: v1 `test_track14_fail_first.py` (86133b80…; log a1a7fae4… 7/6F/1P) and v2 `test_track14_fail_first_v2.py` (4cc88841…; log 84f4f388… 10/7F/3P).
...
import test_track13_regressions as _R                                                                     # THE FIXTURE HELPERS HOLD THEIR OWN MODULE GLOBALS: H13.refusal calls _R.rc.load_identity and H13.build uses _R.P / _R.TF.
_R.rc, _R.P, _R.TF = rc, P, TF                                                                            # Without this rebinding an "on V34" run silently exercises V33 (found 2026-09-07: v14's blob reader was called 0 times while v13's was called 6).
def by_check(out):
    b = {}
...
class T14v4_ON_V34(unittest.TestCase):
    @classmethod
    ...
    def s_unreadable_blob(self):
        """v4 (my own defect, caught here): v3 pinned evs[1]'s head, which the rebuild fixture leaves OUTSIDE the chain the collector actually walks — the injection
        fired 0 times against V34 and its two subcases passed vacuously. The identity is now discovered from the walk itself in a first pass (still an IDENTITY, not a
        call index: the SECOND commit the collector reads), then pinned for the measured run, and the precondition FAILS the fixture if it never fires."""
        env = self.h.build(rebuild_after=True); real = P._blob_at; seen = []
        def record(root, commit, rel): seen.append(commit); return real(root, commit, rel)
        with mock.patch.object(P, "_blob_at", record): self.h.refusal(env, feed(env["live"]), ctx=self._witness_down())
        self.precondition(len(seen) >= 3, f"the collector must walk at least three committed blobs for this case; it walked {len(seen)}")
        target = seen[1]; hits = []
        def flaky(root, commit, rel):
            if commit == target: hits.append(commit); raise OSError("fixture: this blob cannot be read")
        ...
        self.precondition(len(hits) >= 1, f"the injection must actually fire at the pinned commit {target[:12]}; it fired {len(hits)} times")
        return out
    ...
    def test_2_table_verification_catches_a_removed_dynamic_code(self):
        ...
        try:
            ...
            import test_track14_table_vs_code_v18 as TV
