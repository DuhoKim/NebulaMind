...
class RetainedInputError(Exception):
    """v2 (seat B B-1b): raised where a RETAINED input is parsed. Its provenance travels with the exception, so a retained-data failure inside a remote-labelled
    stage is still classified LOCAL-TERMINAL / MALFORMED-RETAINED-INPUT instead of taking the enclosing stage's remote or git label."""
...
def classify_exception(exc, source="retained"):
    ...
    if type(exc).__name__ == "RetainedInputError": return "LOCAL-TERMINAL", "MALFORMED-RETAINED-INPUT", f"{type(exc.__cause__ or exc).__name__}: {exc}"[:300], False   # v2 (B-1b): the input decides, not the enclosing stage
