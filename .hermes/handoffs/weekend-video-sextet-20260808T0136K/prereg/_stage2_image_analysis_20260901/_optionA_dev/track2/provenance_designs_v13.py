...
arm is NOT evaluated on a partial prefix (absence from a prefix is not absence — V32-2). (3) `history_findings_v13(..., add)` CONTRIBUTES DURABLY as it derives: each
...
import precedence_core as _PC   # v13: single source — the names below are the core's (the local definitions above are retained for lineage and are shadowed)
...
def history_findings_v13(root, rel, remote_url, ref, open_event, live, prov, repo, add):
    ...
    if head is None:
        add("RETRY-UNAVAILABLE", "HISTORY-CONTINUATION", "RETRY-REMOTE-UNAVAILABLE: the remote did not answer ls-remote; nothing on the remote can be examined until it does", "history-remote"); info["blocked"].update({"open-delivery-auth": "remote-head unavailable", "per-entry": "remote-head unavailable"}); return info
    if not guard("git", "history-remote", lambda: _fetch_witness(root, remote_url, ref, head)):
        add("RETRY-UNAVAILABLE", "HISTORY-CONTINUATION", "RETRY-REMOTE-UNAVAILABLE: could not fetch the remote ref's objects", "history-remote"); info["blocked"].update({"open-delivery-auth": "remote objects not fetched", "per-entry": "remote objects not fetched"}); return info
    ...
    def extension_checks():
        ...
        for i, c in enumerate(commits):
            ...
            prev = blob
        info["entries_per_commit"] = per; info["acknowledged_entries"] = len(_lines(prev)); info["_prev"] = prev
    ...
    per_entry()
    def working_tree():
        prev = info.get("_prev", b"")
        rc_, local_head = _git(root, "rev-parse", "HEAD"); local_head = local_head.decode().strip(); rc_, unpub = _git(root, "rev-list", f"{head}..{local_head}"); info["local_commits_not_acknowledged"] = unpub.decode().split()
        cur = (Path(root) / rel).read_bytes()
        if cur != prev:
            if cur.startswith(prev): add("DERIVED-TERMINAL", "HISTORY-CONTINUATION", f"PENDING-PUSH: the working tree extends the acknowledged history by {len(cur) - len(prev)} bytes the remote has not acknowledged; publish and retry — nothing in them governs", "history-remote")
            else: add("DERIVED-TERMINAL", "HISTORY-CONTINUATION", "HISTORY-DIVERGED: the working tree does not extend the history acknowledged by the remote (local reset/rebuild); refuse", "history-remote")
        elif info["local_commits_not_acknowledged"]: add("DERIVED-TERMINAL", "HISTORY-CONTINUATION", "HISTORY-DIVERGED: local commits exist that the remote never acknowledged; refuse until local state equals the published ref", "history-remote")
    guard("local-file", "history-remote", working_tree)
    info.pop("_prev", None)
    ...
    return info
def validate_continuation_v13(root, rel, remote_url, ref, open_event, runner, repo, expected_head=None, per_page=100, max_pages=3):
    ...
    info = history_findings_v13(root, rel, remote_url, ref, open_event if o0 is None else None, live, prov, repo, add)
