"""TRACK 2 v13 — RECOMMENDED, UNADOPTED designs, repaired after the V32 review (codex V32-1..5; agy's first refusal, two FATALs on the same ground) — NSD at the granularity of
ACTUAL INPUTS. (1) The resolver, the allowlist (with DECLARED dynamic families), classification by provenance, the guarded local event checks, canonicalisation, same-id
contradictions and the delivery predicate now live in the DEPENDENCY-FREE `precedence_core.py` (this module re-exports them), so the driver runs every LOCAL check even when
this module cannot be imported (V32-1: an unavailable helper must not decide). (2) `authenticate_event(..., complete=False)`: a PARTIAL snapshot supports only predicates whose
truth survives any permitted completion — a same-id contradiction (rule ii) and a strictly earlier qualifying event beside a verbatim retained copy; the absent + same-commit
arm is NOT evaluated on a partial prefix (absence from a prefix is not absence — V32-2). (3) `history_findings_v14(..., add)` CONTRIBUTES DURABLY as it derives: each
operation is wrapped and classified at its own input boundary (remote event / local file / git / retained), a later exception erases nothing already contributed (V32-3); on a
partial snapshot the batch disposition (a negative condition — no own publication found) is not decided, unavailability is contributed instead; the success wording claims
history-BLOB equality with the acknowledged blob at the live remote head, not HEAD equality (V32-5). (4) `validate_continuation_v13` (standalone) sweeps the open event
locally first and contributes the retrieval's partial / malformed findings like the composed path. v12 bytes preserved in provenance_designs_v12.py. v12 was — RECOMMENDED, UNADOPTED designs, repaired after codex's V31 review (V31-1..5) under BLANC'S ORDER OF 05:02 KST 09-07: ONE PROPERTY, named NSD — "No Stage
...
def _retained(body):
    """v14 (seat B B-1b): evaluate a predicate over RETAINED data at its own input boundary — a malformed retained value raises RetainedInputError, which stays
    LOCAL-TERMINAL wherever the enclosing stage is labelled remote or git."""
    try: return body()
    except (ValueError, TypeError, AttributeError, KeyError, IndexError, UnicodeError) as e: raise RetainedInputError(f"{type(e).__name__}: {e}") from e
...
def local_precheck(retained, repo, branch_ref, commit, root=None):
    ...
    dv = _retained(lambda: delivery(retained, commit, branch_ref, root))                              # v14 (B-1b): a malformed retained event is a LOCAL finding, not remote evidence
...
def authenticate_event(retained, live_events, repo, branch_ref, commit, feed_reaches_back_to=None, root=None, complete=True):
    ...
    present = bool(live_events) and any(canon(e) == d for e in live_events)
    if not complete:                                                                                    # (v13, codex V32-2) a PARTIAL snapshot supports only predicates that survive every permitted completion: same-id contradictions (above) and a strictly earlier qualifying event beside a verbatim copy; NOT absence, presence, expiry or acceptance
        ...
        return "UNAVAILABLE", "the events retrieval was PARTIAL (a later page failed): absence from the obtained prefix is not absence, so the same-commit arm is not evaluated; nothing obtained contradicts the retained event by id — presence, absence, expiry and acceptance are never established from a partial snapshot; retry"
    if live_events and not present:                                                                     # (v12, codex V31-3) the ABSENT + same-commit arm — on a COMPLETE snapshot only — before any availability disposition
        ...
        if same_commit: return "FORGED", f"the retained event is absent from the COMPLETE feed and it carries a push on the pinned ref delivering the same commit with different canonical bytes (e.g. {same_commit[0].get('id')}) — an affirmative contradiction, evaluated before any retry (v7, codex V26-1; v12, codex V31-3; v13, codex V32-2)"
...
def authenticate_event_live(retained, repo, runner, branch_ref, commit, per_page=100, max_pages=3, root=None):
    ...
    o, why = authenticate_event(retained, live, repo, branch_ref, commit, feed_reaches_back_to=prov.get("oldest_created_at"), root=root, complete=prov.get("partial") is None); return o, why, prov
...
def history_findings_v12(root, rel, remote_url, ref, open_event, live, prov, repo):
...
import precedence_core_v2 as _PC   # v13: single source — the names below are the core's (the local definitions above are retained for lineage and are shadowed)
PRECEDENCE, finding, resolve, canon, same_id_contradictions, local_precheck_event, delivery, delivers = _PC.PRECEDENCE, _PC.finding, _PC.resolve, _PC.canon, _PC.same_id_contradictions, _PC.local_precheck_event, _PC.delivery, _PC.delivers
CLASS_ALLOWLIST, DYNAMIC_FAMILIES, classify_refusal, classify_exception, classify_history = _PC.CLASS_ALLOWLIST, _PC.DYNAMIC_FAMILIES, _PC.classify_refusal, _PC.classify_exception, _PC.classify_history
RetainedInputError = _PC.RetainedInputError                                                       # v14 (seat B B-1b): the retained-input boundary marker, re-exported for the driver and the fixtures
def history_findings_v14(root, rel, remote_url, ref, open_event, live, prov, repo, add):
    """THE HISTORY STAGE AS A DURABLE COLLECTOR (v13, codex V32-3). `add(cls, code, why, check)` contributes each finding THE MOMENT it is derived; every operation is
    wrapped and an exception is classified at its OWN input boundary (source) and contributed — nothing already contributed is lost, and the remaining independent
    operations continue. Checks: history-remote (remote head / fetch / genesis-only / extension / working tree vs remote), open-delivery-auth (the open event against the
    open commit), per-entry (publications on the snapshot). On a PARTIAL snapshot the negative-condition batch disposition is NOT decided. Returns info (with info['blocked'])."""
    info = {"remote_url": remote_url, "ref": ref, "asked_utc": utc(), "blocked": {}}; complete = live is not None and not (prov or {}).get("partial")
    def guard(source, check, body):
        try: return body()
        except Exception as e:
            cls, code, why, unc = classify_exception(e, source=source); add(cls, code if code not in ("MALFORMED-RETAINED-INPUT",) else code, f"{why} (during {check})", check); return None
    head = guard("git", "history-remote", lambda: remote_head(remote_url, ref)); info["remote_head"] = head
    if head is None:
        add("RETRY-UNAVAILABLE", "HISTORY-CONTINUATION", "RETRY-REMOTE-UNAVAILABLE: the remote did not answer ls-remote; nothing on the remote can be examined until it does", "history-remote"); info["blocked"].update({"open-delivery-auth": "remote-head unavailable", "per-entry": "remote-head unavailable", "history-working-tree": "remote-head unavailable — there is no acknowledged prefix to compare with"}); return info
    if not guard("git", "history-remote", lambda: _fetch_witness(root, remote_url, ref, head)):
        add("RETRY-UNAVAILABLE", "HISTORY-CONTINUATION", "RETRY-REMOTE-UNAVAILABLE: could not fetch the remote ref's objects", "history-remote"); info["blocked"].update({"open-delivery-auth": "remote objects not fetched", "per-entry": "remote objects not fetched", "history-working-tree": "remote objects not fetched — there is no acknowledged prefix to compare with"}); return info
    out = guard("git", "history-remote", lambda: _git(root, "log", "--reverse", "--format=%H", head, "--", rel)); commits = out[1].decode().split() if out and out[0] == 0 else []; info["remote_commits"] = commits
    if not commits: add("DERIVED-TERMINAL", "HISTORY-CONTINUATION", "HISTORY-NO-COMMITS on the remote ref", "history-remote"); return info
    oc = commits[0]
    def genesis_check():
        import tempfile; open_blob = _blob_at(root, oc, rel) or b""; tp = Path(tempfile.mkdtemp()) / "open.jsonl"; tp.write_bytes(open_blob)
        try: ents = H.validate(tp)
        except ValueError as e: add("DERIVED-TERMINAL", "HISTORY-CONTINUATION", f"OPEN-NOT-GENESIS-ONLY: the first history commit's blob is not a valid genesis ({e})", "history-remote"); return
        if len(ents) != 1 or ents[0].get("stage") != "genesis": add("DERIVED-TERMINAL", "HISTORY-CONTINUATION", f"OPEN-NOT-GENESIS-ONLY: the first history commit {oc[:12]} carries {len(ents)} entries; a history-open commit carries exactly the genesis — a late first publication of a rebuilt history is refused", "history-remote")
    guard("remote", "history-remote", genesis_check)
    if isinstance(open_event, dict):
        def open_checks():
            dv = delivery(open_event, oc, ref, root); info["open_event_delivery"] = dv
            if dv == "NOT-DELIVERED": add("LOCAL-TERMINAL", "HISTORY-CONTINUATION", f"OPEN-EVENT-INCONSISTENT-INPUT: the retained history-open event positively does not deliver the first history commit {oc[:12]}", "open-delivery-auth")
            o, why = authenticate_event(open_event, live, repo, ref, oc, feed_reaches_back_to=(prov or {}).get("oldest_created_at"), root=root, complete=complete); info["open_event"] = o
            m = {"UNAVAILABLE": ("RETRY-UNAVAILABLE", "RETRY-HISTORY-CONTINUATION", "EVIDENCE-UNAVAILABLE (history-open event): "), "INCOMPLETE": ("RETRY-INCOMPLETE", "RETRY-HISTORY-CONTINUATION", "EVIDENCE-INCOMPLETE (history-open event): "), "EXPIRED": ("EXPIRED", "HISTORY-CONTINUATION", "EVIDENCE-EXPIRED (history-open event): "), "FORGED": ("FORGED", "HISTORY-CONTINUATION", "OPEN-EVENT-FORGED: "), "INCONSISTENT-INPUT": ("LOCAL-TERMINAL", "HISTORY-CONTINUATION", "OPEN-EVENT-INCONSISTENT-INPUT: "), "NOT-EARLIEST": ("NOT-EARLIEST", "HISTORY-CONTINUATION", "OPEN-EVENT-NOT-EARLIEST: ")}
            if o in m: cls, code, pre = m[o]; add(cls, code, pre + why + (" — no receipt path exists for the history-open event (Q1)" if o == "EXPIRED" else ""), "open-delivery-auth")
        guard("remote", "open-delivery-auth", open_checks)
    else: info["blocked"]["open-delivery-auth"] = "no valid history-open PushEvent object (class-0 finding contributed by S0); its delivery and authentication cannot be evaluated"
    def extension_checks():
        """v14 (seat B B-1c): EACH COMMITTED BLOB IS ITS OWN INPUT. An unreadable blob is contributed and the loop continues, so every later independently readable
        adjacent relation is still checked; the commits whose blobs could not be read are recorded, and the working-tree comparison is BLOCKED rather than run against
        a defaulted empty prefix (v13 aborted the whole loop, left _prev unset, and then reported PENDING-PUSH for 708 already-published bytes)."""
        prev = b""; per = []; unread = []
        for i, c in enumerate(commits):
            try: blob = _blob_at(root, c, rel)
            except Exception as e:
                add("RETRY-UNAVAILABLE", "RETRY-HISTORY-CONTINUATION", f"EVIDENCE-UNAVAILABLE: the committed history blob at {c[:12]} (entry {i}) cannot be read ({type(e).__name__}: {str(e)[:80]}); the adjacent relations that need it are undecided; retry", "history-remote")
                unread.append(c); per.append(None); prev = None; continue
            if prev is None:                                                                              # the previous blob was unreadable: this pair's relation is undecided, later pairs are not
                per.append(None); prev = blob; continue
            if blob is None: add("DERIVED-TERMINAL", "HISTORY-CONTINUATION", f"HISTORY-DELETED-AT {c[:12]} on the remote ref", "history-remote"); per.append(None); continue
            if i > 0 and (not blob.startswith(prev) or len(blob) <= len(prev)): add("DERIVED-TERMINAL", "HISTORY-CONTINUATION", f"HISTORY-NOT-AN-EXTENSION at {c[:12]}: the remote's committed blob does not extend the previous one", "history-remote")
            added = len(_lines(blob)) - len(_lines(prev)); per.append(added)
            if added != 1 and blob.startswith(prev): add("DERIVED-TERMINAL", "HISTORY-CONTINUATION", f"HISTORY-BATCH-COMMIT at {c[:12]}: {added} entries in one commit; one acknowledged publication per entry is required", "history-remote")
            prev = blob
        info["entries_per_commit"] = per; info["unreadable_blobs"] = unread
        if unread or prev is None: info["blocked"]["history-working-tree"] = f"{len(unread)} committed history blob(s) could not be read; the acknowledged prefix is unknown"
        else: info["acknowledged_entries"] = len(_lines(prev)); info["_prev"] = prev
    guard("remote", "history-remote", extension_checks)
    def per_entry():
        if live is None: add("RETRY-UNAVAILABLE", "RETRY-HISTORY-CONTINUATION", "EVIDENCE-UNAVAILABLE: the events feed could not be retrieved this invocation — the per-entry publications cannot be checked; retry", "per-entry"); info["blocked"]["per-entry"] = "events feed unavailable"; return
        times = {}; pushes = [e for e in live if isinstance(e, dict) and e.get("type") == "PushEvent" and isinstance(e.get("payload"), dict) and e["payload"].get("ref") == ref]
        for i, c in enumerate(commits):
            def one():
                rc_, parents = _git(root, "rev-list", "--parents", "-n", "1", c); parent = (parents.decode().split() + ["", ""])[1]
                own = [e for e in pushes if e["payload"].get("head") == c and e["payload"].get("before") == parent]
                if own: times[i] = min(str(e.get("created_at")) for e in own); return
                if not complete: add("RETRY-UNAVAILABLE", "RETRY-HISTORY-CONTINUATION", f"EVIDENCE-UNAVAILABLE: history commit {c[:12]} (entry {i}) has no push event of its own in the {'PARTIAL' if live else 'EMPTY'} retrieved feed — neither its presence nor a batch can be judged from a partial snapshot; retry", "per-entry"); return
                carried = [e for e in pushes if delivers(e, c, ref, root) and not (e["payload"].get("head") == c and e["payload"].get("before") == parent)]
                if carried: add("DERIVED-TERMINAL", "HISTORY-CONTINUATION", f"HISTORY-PUBLICATION-BATCH: the feed shows history commit {c[:12]} (entry {i}) delivered inside a push that carried other commits ({carried[0].get('id')}; proven by head/commits/before..head ancestry) — not an acknowledged publication of its own; terminal", "per-entry")
                else: add("RETRY-INCOMPLETE", "RETRY-HISTORY-CONTINUATION", f"EVIDENCE-INCOMPLETE: history commit {c[:12]} (entry {i}) has no push event of its own in the retrieved feed and no evidence of batching — retry within the feed window", "per-entry")
            guard("remote", "per-entry", one)
        if not complete: add("RETRY-UNAVAILABLE", "RETRY-HISTORY-CONTINUATION", f"EVIDENCE-UNAVAILABLE: the snapshot is PARTIAL — {len(times)} of {len(commits)} per-entry publications were found affirmatively among the obtained events; absence and batching cannot be decided from a partial snapshot, so per-entry's negative half is undecided; retry", "per-entry")   # v13 (controls v3): the check NEEDS the complete snapshot for its negative half and says so under its own name
        seq = [times[i] for i in sorted(times)]
        if seq != sorted(seq): add("DERIVED-TERMINAL", "HISTORY-CONTINUATION", "HISTORY-PUBLICATION-ORDER: the server times of the per-entry pushes that were found are not in the recorded order", "per-entry")
        info["publications"] = len(times); info["publication_times"] = seq
    per_entry()
    def working_tree():
        if "_prev" not in info:                                                                       # v14 (B-1c): no acknowledged prefix was established — never compare against a defaulted empty one
            add("RETRY-UNAVAILABLE", "RETRY-HISTORY-CONTINUATION", "EVIDENCE-UNAVAILABLE: the acknowledged history prefix could not be established, so the working tree cannot be compared with it; retry", "history-working-tree")
            info["blocked"]["history-working-tree"] = "the acknowledged prefix is unknown"; return
        prev = info["_prev"]
        rc_, local_head = _git(root, "rev-parse", "HEAD"); local_head = local_head.decode().strip(); rc_, unpub = _git(root, "rev-list", f"{head}..{local_head}"); info["local_commits_not_acknowledged"] = unpub.decode().split()
        cur = (Path(root) / rel).read_bytes()
        if cur != prev:
            if cur.startswith(prev): add("DERIVED-TERMINAL", "HISTORY-CONTINUATION", f"PENDING-PUSH: the working tree extends the acknowledged history by {len(cur) - len(prev)} bytes the remote has not acknowledged; publish and retry — nothing in them governs", "history-working-tree")
            else: add("DERIVED-TERMINAL", "HISTORY-CONTINUATION", "HISTORY-DIVERGED: the working tree does not extend the history acknowledged by the remote (local reset/rebuild); refuse", "history-working-tree")
        elif info["local_commits_not_acknowledged"]: add("DERIVED-TERMINAL", "HISTORY-CONTINUATION", "HISTORY-DIVERGED: local commits exist that the remote never acknowledged; refuse until local state equals the published ref", "history-working-tree")
    guard("local-file", "history-working-tree", working_tree)                                             # v14 (B-1c): the working-tree comparison is its own CHECK — it needs an established acknowledged prefix
    info.pop("_prev", None)
    info["why"] = f"genesis-only open commit {oc[:12]}; one entry per acknowledged commit; the working tree's history blob equals the acknowledged blob at the live remote head {head[:12]} (blob equality, not HEAD equality); {info.get('publications', 0)} per-entry publications server-timed in order"
    return info
def validate_continuation_v14(root, rel, remote_url, ref, open_event, runner, repo, expected_head=None, per_page=100, max_pages=3):
    """(ok, why, info) — the standalone form under NSD: the open event swept LOCALLY first; ONE retrieval whose partial / malformed findings are contributed exactly as on
    the composed path; the durable collector; the winner by PRECEDENCE."""
    F = []
    def add(cls, code, why, check): F.append(finding(cls, code, why, "history", len(F), check))
    o0, x0 = local_precheck_event(open_event, repo, ref)
    if o0 is not None: add("LOCAL-TERMINAL", "HISTORY-CONTINUATION", f"OPEN-EVENT-INCONSISTENT-INPUT: {x0}", "open-local")
    try: live, prov = retrieve_events(repo, runner, per_page=per_page, max_pages=max_pages)
    except EventsUnavailable as e: live, prov = None, {"error": f"{e.kind}: {e.why}"}; add("RETRY-UNAVAILABLE", "RETRY-EVENTS-UNAVAILABLE", f"{e.kind}: {e.why}", "snapshot")
    if prov and prov.get("partial"): add("RETRY-UNAVAILABLE", "RETRY-EVENTS-UNAVAILABLE", f"{prov['partial']['kind']} on page {prov['partial']['page']}: {prov['partial']['why']} — a PARTIAL snapshot", "snapshot")
    for m in (prov or {}).get("malformed", []): add("RETRY-UNAVAILABLE", "MALFORMED-REMOTE-EVIDENCE", f"an event in the retrieved feed is not a PushEvent-shaped object (page {m['page']}, id {m['id']!r}) — remote evidence unusable, excluded", "snapshot")
    info = history_findings_v14(root, rel, remote_url, ref, open_event if o0 is None else None, live, prov, repo, add)
    if expected_head is not None and info.get("remote_head") not in (None, expected_head): add("DERIVED-TERMINAL", "HISTORY-CONTINUATION", f"STALE-EXPECTED-HEAD: supplied {expected_head[:12]} but the live remote head is {str(info.get('remote_head'))[:12]}", "history-remote")
    if not F and not info.get("blocked"): return True, info.get("why", "ok"), info
    if not F: add("RETRY-UNAVAILABLE", "RETRY-HISTORY-CONTINUATION", "EVIDENCE-UNAVAILABLE: checks blocked: " + "; ".join(f"{k}: {v}" for k, v in info["blocked"].items()), "history-remote")
    w = resolve(F); info["findings"] = [{k: f[k] for k in ("class", "code", "why", "check")} for f in F]
    return False, w["why"] if w["code"].endswith("HISTORY-CONTINUATION") else f"{w['code']}: {w['why']}", info
