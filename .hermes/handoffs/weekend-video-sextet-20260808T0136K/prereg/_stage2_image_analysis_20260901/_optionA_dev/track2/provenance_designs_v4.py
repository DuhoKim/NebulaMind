"""TRACK 2 v4 — RECOMMENDED, UNADOPTED designs, repaired after codex's V23 review (M1, M2, M3): (M1) PER-ENTRY PUBLICATION IS PROVEN BY THE SERVER —
...
provenance_designs.py. Wired into the production driver (v8) ONLY behind `Protocol.provenance_mode = 'composed'` (default 'offline'); UNADOPTED.
...
def publish_entry(root, rel, remote_url, ref, message):
    ...
    if head_blob == cur and lh != head and subprocess.run(["git", "merge-base", "--is-ancestor", head, lh], cwd=root, capture_output=True).returncode == 0:
