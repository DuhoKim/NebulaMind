"""TRACK 2 v6 — RECOMMENDED, UNADOPTED designs, repaired after codex's V25 review (P1, P2): (P1) batch classification uses the SAME delivery evidence as
...
provenance_designs.py. Wired into the production driver (v10) ONLY behind `Protocol.provenance_mode = 'composed'` (default 'offline'); UNADOPTED.
...
def authenticate_event(retained, live_events, repo, branch_ref, commit, feed_reaches_back_to=None, root=None):
    ...
    if not any(canon(e) == d for e in live_events):
        ...
        same_commit = [e for e in live_events if e.get("type") == "PushEvent" and (e.get("payload") or {}).get("ref") == branch_ref and delivers(e, commit, branch_ref, root) and (e.get("payload") or {}).get("head") == (retained.get("payload") or {}).get("head") and (e.get("payload") or {}).get("before") == (retained.get("payload") or {}).get("before")]
        if same_commit: return "FORGED", "the live feed carries a push delivering the same commit with DIFFERENT bytes — an affirmative contradiction of the retained event (v6, codex V25 P2)"
...
def validate_continuation_v6(root, rel, remote_url, ref, open_event, runner, repo, expected_head=None, per_page=100, max_pages=3):
    ...
    the open-event stage reports EVIDENCE-UNAVAILABLE / EVIDENCE-INCOMPLETE / EVIDENCE-EXPIRED (retry-class) and OPEN-EVENT-FORGED only on contradiction."""
