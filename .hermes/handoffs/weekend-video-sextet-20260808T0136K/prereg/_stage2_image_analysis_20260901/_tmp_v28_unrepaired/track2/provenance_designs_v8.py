"""TRACK 2 v7 — RECOMMENDED, UNADOPTED designs, repaired after codex's V26 review (V26-1, V26-2, V26-3): (V26-1) CONTRADICTION PREDICATE, stated: a
...
same commit with different canonical bytes; the open-event stage of the validator maps UNAVAILABLE/INCOMPLETE/EXPIRED to EVIDENCE-* dispositions (retry) and
...
def delivery(e, commit, branch_ref, root=None):
    """TRI-STATE delivery check (v7, codex V26-2): 'DELIVERED' (head == commit, commit in payload.commits, or before..head ancestry ESTABLISHED via git),
...
def authenticate_event(retained, live_events, repo, branch_ref, commit, feed_reaches_back_to=None, root=None):
    """The pure decision (v7): (outcome, why). Outcomes: AUTHENTIC | UNAVAILABLE (feed missing/empty, or delivery UNDETERMINED — retry) | INCOMPLETE (absent,
    nothing contradicts — retry) | EXPIRED (older than the feed reaches — a loss) | FORGED (an affirmative contradiction: same id or same delivered commit,
    different bytes) | INCONSISTENT-INPUT (the retained object is not a PushEvent for the pinned repository, or does not deliver the commit) | NOT-EARLIEST."""
    ...
    if dv == "UNDETERMINED": return "UNAVAILABLE", "the retained event's before..head delivery of the approval commit could not be established here (a git object missing locally or a failed git command) — not evidence of anything; retry with the objects fetched (v7, codex V26-2)"
    ...
    if not any(canon(e) == d for e in live_events):
        contradictions = [e for e in live_events if e.get("type") == "PushEvent" and (e.get("id") == retained.get("id") or ((e.get("payload") or {}).get("ref") == branch_ref and delivery(e, commit, branch_ref, root) == "DELIVERED")) and canon(e) != d]
        if contradictions: return "FORGED", f"the live feed carries a push with the same event id or delivering the same commit whose canonical bytes DIFFER from the retained event (e.g. {contradictions[0].get('id')}) — an affirmative contradiction (v7, codex V26-1)"
    ...
    qualifying = [e for e in live_events if e.get("type") == "PushEvent" and delivery(e, commit, branch_ref, root) == "DELIVERED"]
