#!/usr/bin/env python3
"""check_pin_consistency_v4 (Tier-C V39 draft): v3 plus ONE repair from the V38 gate — a RELATIVE aliased import (`from . import renderer_v3 as rv4`)
is now recorded as importing that name (ImportFrom with level > 0 has module None; v3 skipped it and codex bypassed the checker with exactly
that line in a scratch copy). v3 was: v2 (control pin = renderer_v4) plus three repairs from the V37 gate: (1) `from pkg import name as alias`
...
Usage: python3 check_pin_consistency_v4.py <preregistration.md>
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            ...
            for alias in node.names:                       # v3: `from pkg import name as alias` imports NAME, whatever the alias
                out.add(alias.name)                        # v4: also for RELATIVE imports (`from . import name as alias`), where module is None
                if node.module: out.add(node.module + "." + alias.name)
...
def main(argv: list[str]) -> int:
    if len(argv) != 2:
        print("usage: check_pin_consistency_v4.py <preregistration.md>")
        return 2
    doc = Path(argv[1])
