#!/usr/bin/env python3
"""Fixture tests for check_pin_consistency_v4.py; standard library only.
...
import check_pin_consistency_v4 as cpc
...
class V4RelativeImportRegression(unittest.TestCase):
    """Codex's EXACT V38 bypass: `from . import renderer_v3 as rv4` appended to a pinned production module. v4 must see it; v3 must NOT
    (the control that can fail: this test asserts the OLD checker misses it, so a checker in which the repair is absent fails here)."""
    MUTATION = "from . import renderer_v3 as rv4\n"
    def test_v4_sees_relative_alias_and_v3_did_not(self):
        import pathlib, tempfile, importlib
        d = pathlib.Path(tempfile.mkdtemp()); f = d / "m.py"; f.write_text("import numpy as np\n" + self.MUTATION)
        self.assertIn("renderer_v3", cpc.imported_modules(f))
        old = importlib.import_module("check_pin_consistency_v3")
        self.assertNotIn("renderer_v3", old.imported_modules(f))
