...
- V11 §8.12: image, maskbits, and exposure-count (`nexp-r`) planes are stitched identically. Every output pixel requires finite coverage in all three and `nexp > 0`; a missing plane, missing/non-finite value, or zero/negative exposure count refuses the whole study with `DATA-INTEGRITY-FAIL`.
...
`render_cutout(sources, target) -> Raster` accepts `(image, maskbits, nexp, astropy.wcs.WCS)` tuples and `(ra, dec)`. It never imports FITS support or touches acquisition files. Canonical image raster bytes are little-endian float64 in C order; the result also exposes the stitched maskbits and exposure-count rasters.

The three V11 inputs are image-r, maskbits, and nexp-r. Until all companions are supplied, the renderer refuses; it never invents, substitutes, or infers a plane.
