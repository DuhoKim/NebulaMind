"""THE EXECUTED RENDER CHAIN under the V37 pipeline identity (Tier-C V37 draft §8.15b consumer; option A V15 §6). One function, in the
...
import protected_region as pr

APERTURE = (slice(48, 80), slice(48, 80))      # §8.15 central 32×32
TENSOR_BYTES = 65536


def render_object(image, maskbits, nexp, wcs, ra: float, dec: float, brick_id: str, r_t: int) -> dict:
    rec = {"brick": brick_id, "r_T": int(r_t)}
    try:
        ...
        refusal = pr.refuse_on_contamination(coords, int(r_t))                                # §8.14a: F > 819 or any flag inside T
        if refusal:
