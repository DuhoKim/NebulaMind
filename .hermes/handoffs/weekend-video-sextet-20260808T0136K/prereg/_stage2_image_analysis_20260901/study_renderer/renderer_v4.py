"""renderer_v4 (Tier-C V37 draft, 2026-09-06): IDENTICAL to renderer_v3.py except that an exposure count of ZERO is CARRIED through
nearest-neighbour sampling instead of refusing the raster (three lines, all `<= 0` -> `< 0`); a NEGATIVE count still refuses. Geometry,
parity, containment, bilinear image sampling and integer-plane carriage are byte-for-byte renderer_v3. ORIGINAL DOCSTRING FOLLOWS.
...
SINGLE_BRICK_CONTAINMENT_FAIL = "SINGLE-BRICK-CONTAINMENT-FAIL"
INTEGER_PLANES = (1, 2)   # maskbits, nexp -- never interpolated
AMBIGUOUS_SOURCE_SELECTION = "AMBIGUOUS-SOURCE-SELECTION"
...
_SECTION8_GEOMETRY = {
    ...
    "interpolation": "bilinear-image-nearest-integer-planes",
    "neighbour_policy": "single-brick-containment",
    "orientation": "north-up/east-left", "parity_policy": "parity-preserve",
    "parity_refusal_token": WRONG_PARITY_REFUSAL,
}
_CONFIG = Path(__file__).resolve().parent.parent / "miniprereg_pins" / "render_config_v2.json"
try:
...
def _target_parts(target):
    ...
    return float(ra), float(dec), requested, primary_tile_id
...
def _contains_stencil(source, world):
    """True iff every output sample AND its 2x2 bilinear stencil lies inside this source."""
    plane, wcs = source[0], source[3]
    xy = wcs.all_world2pix(world, 0)
    x0 = np.floor(xy[:, 0]); y0 = np.floor(xy[:, 1])
    return bool(np.all(x0 >= 0) and np.all(y0 >= 0)
                and np.all(x0 + 1 <= plane.shape[1] - 1)
                and np.all(y0 + 1 <= plane.shape[0] - 1))


def _select_single_source(sources, world, primary_tile_id):
    """Choose exactly one source, by an order fixed HERE and not by the caller's list order.

    The rule is: with one source, use it. With more than one, the caller MUST name the
    primary tile, and that named tile is used -- never the first tile that happens to
    contain the centre point, which made the result depend on list order. The chosen
    source must then contain every sample and its bilinear stencil; a source that does
    not is a refusal, never a reason to reach into a neighbouring tile.
    """
    if len(sources) == 1:
        chosen = sources[0]
    else:
        if primary_tile_id is None:
            raise ValueError(AMBIGUOUS_SOURCE_SELECTION)
        matches = [s for i, s in enumerate(sources) if _tile_id(s[3], i) == str(primary_tile_id)]
        if len(matches) != 1:
            raise ValueError(AMBIGUOUS_SOURCE_SELECTION)
        chosen = matches[0]
    if not _contains_stencil(chosen, world):
        raise ValueError(SINGLE_BRICK_CONTAINMENT_FAIL)
    return chosen
...
def _sample_stitched(sources, world, plane_index):
    ...
    for point_index in range(world.shape[0]):
        ...
        if chosen is None:
            raise ValueError(DATA_INTEGRITY_FAIL)
        base, x, y = chosen
        if plane_index in INTEGER_PLANES:
            # maskbits and nexp carry BIT and COUNT semantics. Interpolating them destroys
            # both: a bilinear blend of bit 3 and bit 0 is not a bitmask, and a fractional
            # exposure count is not a count. They are sampled by nearest neighbour, which
            # is an index selection and not a resampling, so no value is ever invented.
            plane = sources[base][plane_index]
            ix, iy = int(np.rint(x)), int(np.rint(y))
            if not (0 <= iy < plane.shape[0] and 0 <= ix < plane.shape[1]):
                raise ValueError(DATA_INTEGRITY_FAIL)
            value = plane[iy, ix]
            if not np.isfinite(value):
                raise ValueError(DATA_INTEGRITY_FAIL)
            if plane_index == 2 and value < 0:          # v4: a NEGATIVE count is corrupt; ZERO exposure is CARRIED (V37 §8.12)
                raise ValueError(DATA_INTEGRITY_FAIL)
            result[point_index] = np.float64(value)
            continue
...
def render_cutout(sources: Sequence[tuple[np.ndarray, np.ndarray, np.ndarray, WCS]], target) -> Raster:
    ...
    ra, dec, requested, primary_tile_id = _target_parts(target)
    if not sources:
        raise ValueError(DATA_INTEGRITY_FAIL)
    normalized = []
    for source in sources:
    ...
    selected = _select_single_source(normalized, world, primary_tile_id)
    outputs = [_sample_stitched([selected], world, i).reshape(HEIGHT, WIDTH) for i in range(3)]
    # restore integer dtype to the planes that carry bit and count semantics
    for i in INTEGER_PLANES:
        integral = np.rint(outputs[i])
        if not np.array_equal(integral, outputs[i]):
            raise ValueError(DATA_INTEGRITY_FAIL)
        outputs[i] = integral.astype(np.int32)
    if np.any(outputs[2] < 0):                      # v4: zero-exposure output pixels reach §8.9d/§8.14a instead of refusing here
        raise ValueError(DATA_INTEGRITY_FAIL)
...
__all__ = ["Raster", "RenderTarget", "render_cutout", "PINNED_GEOMETRY", "PROHIBITED_TRANSFORMS",
    ...
    "SINGLE_BRICK_CONTAINMENT_FAIL", "AMBIGUOUS_SOURCE_SELECTION"]
