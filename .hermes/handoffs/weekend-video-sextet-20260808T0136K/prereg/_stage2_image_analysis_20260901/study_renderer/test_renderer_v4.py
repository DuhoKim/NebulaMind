...
from study_renderer.renderer_v4 import (CD, DATA_INTEGRITY_FAIL, PINNED_GEOMETRY, RenderTarget,
...
class StudyRendererTests(unittest.TestCase):
    ...
    def test_raster_spanning_two_tiles_now_refuses(self):
        """A raster that no single tile contains is a refusal, not a stitch.

        The previous contract stitched across tiles. That only ever worked because
        these synthetic tiles share a pixel lattice; real Legacy Surveys bricks are
        separate TAN projections and never do, so the stitch path could not run on
        real data. It is removed rather than left as unreachable code.
        """
        ...
        with self.assertRaisesRegex(ValueError, "^(SINGLE-BRICK-CONTAINMENT-FAIL|AMBIGUOUS-SOURCE-SELECTION)$"):
            render_cutout([tile(left,lw),tile(right,rw)],
                          RenderTarget(40.,10.,None,"left"))

    def test_multiple_sources_without_a_named_primary_refuse(self):
        """Selection must never depend on the caller's list order."""
        w=make_wcs(tile_id="a"); v=make_wcs(tile_id="b")
        s=[tile(sky_field(w,(180,180)),w), tile(sky_field(v,(180,180)),v)]
        with self.assertRaisesRegex(ValueError, "^AMBIGUOUS-SOURCE-SELECTION$"):
            render_cutout(s,(40.,10.))

    def test_named_primary_is_order_independent(self):
        """The same named primary gives byte-identical output whatever the list order."""
        w=make_wcs(tile_id="a"); v=make_wcs(tile_id="b")
        ta,tb=tile(sky_field(w,(180,180)),w), tile(sky_field(v,(180,180)),v)
        r1=render_cutout([ta,tb],RenderTarget(40.,10.,None,"a"))
        r2=render_cutout([tb,ta],RenderTarget(40.,10.,None,"a"))
        self.assertEqual(r1.digest,r2.digest)

    def test_single_source_is_unchanged_and_needs_no_primary(self):
        w=make_wcs(tile_id="solo")
        s=[tile(sky_field(w,(180,180)),w)]
        r=render_cutout(s,(40.,10.))
    ...
    def test_zero_nexp_pixel_is_carried_not_refused(self):
        # V37 §8.12: zero exposure is CARRIED to §8.9d/§8.14a (renderer_v3 refused here; its fixture asserted the refusal)
        w=make_wcs(); image=np.ones((180,180)); nexp=np.ones((180,180),dtype=np.int16); nexp[90,90]=0
        r=render_cutout([(image,np.zeros_like(image),nexp,w)],RenderTarget(ra=40.0,dec=10.0))
        self.assertEqual(int((np.asarray(r.nexp)==0).sum()),1); self.assertTrue(np.issubdtype(np.asarray(r.nexp).dtype,np.integer))
    def test_negative_nexp_pixel_refuses(self):
        w=make_wcs(); image=np.ones((180,180)); nexp=np.ones((180,180),dtype=np.int16); nexp[90,90]=-1
