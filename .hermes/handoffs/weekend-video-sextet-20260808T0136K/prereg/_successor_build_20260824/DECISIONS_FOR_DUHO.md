...
| 2 | **The BS-3g parameter sitting — five class-P values and one shape choice, one sitting.** `n_draws` (the gate's strictness, §3 below) · `draw_master_seed` · a named `draw_generator_id` · `Δγ` (the grid spacing that bounds what `HELD` means) · `k_γ` (the bound multiplier) · **and the bound's SHAPE: measurement-derived `|γ̂|+k_γ·σ_γ` (tracks the instrument, inherits its failure modes) versus an a-priori frozen γ range (immune to estimator pathology, may sweep a range the instrument never occupies).** **and the draw-variate semantics: does draw `i` reuse ONE random stream across every γ (common random variates — a flip is the gradient's doing) or redraw independently per γ (a flip may be noise)? The within-draw comparison was built assuming the first; assuming is not preregistering.** All block BS-3g emission until set — **and as of V73 the draw-discipline TEXT is frozen UNSTABLE-PENDING-RULING after five consecutive rounds of findings against it: the seats keep winning because they are refereeing text built over the parameters this row is waiting on. The patch loop is stopped until this sitting lands.** | §3 below + §11 |
