**STATUS: GO-LIVE IS RULED — "Go-live when staging is done" (2026-08-31 late evening; `GO_LIVE_ACCELERATION_RULING_20260831.md`, direction #22).** The BS-1→start dependency edge is expressly superseded: the chain goes live the moment staging-v2 completes and verifies — days before the Sep-5 default. Unchanged and disclosed: the frozen v9 date-gate means the BS-1 receipt itself still lands Sep-5 (no truthful earlier stamp exists); the run opens with BS-1 resolved-by-ruling/receipt-pending, and the receipt joins the chain at its date. NOTHING IS PENDING ON YOU tonight. Sequence: staging-v2 (morning, as paced) → codex/agy verify → GO-LIVE → P1+ on the Branch-B data. Any human waypoint at go-live comes to you via Blanc in plain words.
