...
- **The V65 schedule sentence is recut**: *"re-viewing the current object is unrestricted"* remains
  ...
  **BUFFER LIFETIME AND CACHE, because G5 and G6 composed into two contradictions in one round.**
  ...
  image.** **MULTIPLICITY IS A CONSTANT, BY PADDING (V93 — GPT56-V92 F4 and CODEX-V92 F2 broke V92's single-pass rule against the restore requirement two sentences up: an interrupted member had NO route to a terminal label, and "no re-render operation" contradicted "must produce a second committed render event"): the interface delivers EXACTLY `R_max` committed render events per allocated object per member — `R_max` a BS-2k design constant ≥ 2 — where interruptions consume replays from the fixed allowance and, at the member's session-set end, the interface issues every unconsumed replay as a PADDING commit (a real conveyance that opens no session; the no-session buffer path above destroys it at request end). Every object's per-member render multiplicity therefore equals `R_max` regardless of member behaviour, and the log's counts carry nothing — the χ-blind multiplicity the traversal rule demands, WITH a replay route for interruption. A member exhausting `R_max` without submitting a label: that object's label from that member is ABSENT, entering the existing absent-label path. V92's single-pass (R_max = 1, no replay) is superseded — it satisfied the counting rule by forbidding what this section requires.**
