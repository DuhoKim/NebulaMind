...
- **The V65 schedule sentence is recut**: *"re-viewing the current object is unrestricted"* remains
  ...
  **BUFFER LIFETIME AND CACHE, because G5 and G6 composed into two contradictions in one round.**
  ...
  image.** **MULTIPLICITY IS A CONSTANT, BY PADDING (V93 — GPT56-V92 F4 and CODEX-V92 F2 broke V92's single-pass rule against the restore requirement two sentences up: an interrupted member had NO route to a terminal label, and "no re-render operation" contradicted "must produce a second committed render event"): the interface delivers EXACTLY `R_max` committed render events per allocated object per member — `R_max` a BS-2k design constant ≥ 2 — where interruptions consume replays from the fixed allowance, **and the padding is PER-OBJECT AND PRE-LABEL, because end-clustered padding timed the very count it hides (CODEX-V93 F4) and "session-set end" was undefined and evadable (GPT56-V93 F5, CODEX-V93 F3): when the member proceeds to label an object, the interface FIRST issues that object's unconsumed replays as PADDING commits (real conveyances opening no session; the no-session buffer path above destroys each at request end) and only then accepts label entry — no label is held anywhere (Row H's no-intermediate-persistence rule is untouched), and every object's chain pattern is the SAME: exactly `R_max` render commits, then its label write. The member-level backstop is the BS-2k LABELING DEADLINE: an allocation neither fully labeled nor exhausted by it is closed by the deadline machinery like any other request.** Every object's per-member render multiplicity therefore equals `R_max` regardless of member behaviour, and neither counts nor padding timing carry anything. **EXHAUSTION HAS A NAMED TERMINAL, because the previously-cited absent-label path did not exist (GPT56-V93 F6): a member exhausting `R_max` on an object without a label HALTS THE RUN pre-BS-8f — the same conservative treatment Row I already gives an allocated object lacking a usable instrument output — recorded with the object and member identities in the halt record; `R_max ≥ 2` makes this the rare storm case, the cost is stated, and softening it (an ABSTAIN label, a member-replacement rule) would change the labeling protocol and so belongs to the principal, flagged in the decisions file.** V92's single-pass (R_max = 1, no replay) is superseded — it satisfied the counting rule by forbidding what this section requires.**
