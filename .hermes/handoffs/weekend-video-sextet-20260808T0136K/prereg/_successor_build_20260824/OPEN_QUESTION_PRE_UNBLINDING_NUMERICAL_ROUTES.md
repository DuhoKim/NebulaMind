...
# 16:5x — the reading is now a per-site record, and two of my prose counts were off

`ref/RAISE_SITE_CLASSIFICATION.md` — **all 112 raise sites, one row each**, with line, function,
exception type, class, and the soft-edge flag. Generated by `ref/gen_raise_classification.py`:
**enumeration by AST, classification as explicit line-number data**, so the whole thing regenerates
and a seat can check any row against the source. This is what V50 §11 asks for — *record the verdict
per site* — and it replaces a count nobody could verify with a table anybody can.

    CALLER 20 · INTEGRITY 61 · NUMERICAL 22 · NUMERICAL-PLANNING 3 · WRAPPER 3 · TYPED-OUTCOME 3
    = 112, with zero unassigned.

**Two of the numbers in my prose above were wrong, and the table is why I know.** I wrote "~24 caller
errors" and "~17 integrity" among the 70; counting them gives **20** and **22**. I estimated those
two from the shape of the list instead of counting, in a note whose entire subject was that I keep
getting counts wrong. **The table is authoritative from here; the prose figures above stand only as
the record of how they were reached.**

**What did not move: the numerical class is 22**, and the soft edge is unchanged — if the four
domain checks read as CALLER instead, it is 18.

**What a seat should check**, in order of consequence: the 22 NUMERICAL rows (is any of them actually
a caller error?); the 3 NUMERICAL-PLANNING rows (is a planning failure a run outcome at all?); and
whether any of the 61 INTEGRITY rows is genuinely unclaimed by a VOID antecedent — that last one is
the direction where a mistake leaves a real failure unterminated.
