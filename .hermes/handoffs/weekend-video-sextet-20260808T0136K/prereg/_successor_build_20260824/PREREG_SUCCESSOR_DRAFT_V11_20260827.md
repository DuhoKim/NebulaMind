# PREREGISTRATION DRAFT V11 — LONGO-AMPLITUDE TEST ON A LEVERAGE-CHOSEN FOOTPRINT

> **V11 repairs the six BLOCKING findings of the first text review (2026-08-27).** Three seats —
...
> **WHICH STAGE-P TEST THIS TEXT PROMISES (V11, repairing GPT56 F3).** V10 named two: §4
...
4. **The ledger is the producer, and the code recomputes from it.** BS-2f is derived from an
   append-only ledger carrying one terminal status and one machine-checkable reason per object
   ID, with digests for both the accepted and excluded sets. `run_production_verdict()` must
   **recompute** the accepted set from that ledger rather than accept caller-supplied flags, and
   `require_complete_sample()` must verify the partition rather than compare two integers.
5. **The thresholds in (d) are pinned before any image byte**, in BS-3, with the same force as
...
1. **The primary lock is defined here, not at execution.** It is the moment at which every
   class-P slot holds a receipt, BS-5f's confirmatory power receipt exists, and the decision
   function's inputs are digest-fixed. It is sealed by a signed BS-V receipt naming the digests
   of the accepted mask, the calibration record and those inputs, and may be opened only by
   Duho's signature. What proves it held is the log in (3): **a lock with no log is a lock that
   failed.**
2. **Who may hold read access.** Only named key holders, recorded in BS-V's schema before any
   image byte. **No person or process able to alter this text, fill or adjudicate a class-P
   slot, construct the accepted mask, or operate the lock may decrypt, query, render, summarise
   or inspect any χ-bearing object or derivative** until the lock, unblinding and BS-5f have
   occurred in that fixed order. Blind automation is permitted only where named here and only
   where it cannot emit a χ-derived value outside the store.
3. **Access is logged or it did not happen.** Every decryption, query and read against the
   sealed store — successful or refused — appends to a log whose digest is receipted at BS-2f
   and again at BS-V. The checkable sentence is: **no χ-derived artifact exists outside the
   sealed store before the primary lock.** The log is what makes a violation visible rather
   than merely forbidden.
4. **Unauthorised access voids the run**, whether or not anything was disclosed and whether or
   not the accessor believed it harmless.
5. **What establishes that the redesign was blind.** The successor footprint was chosen from
