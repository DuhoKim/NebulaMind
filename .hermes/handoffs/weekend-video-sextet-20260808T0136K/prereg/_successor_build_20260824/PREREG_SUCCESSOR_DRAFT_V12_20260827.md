# PREREGISTRATION DRAFT V12 — LONGO-AMPLITUDE TEST ON A LEVERAGE-CHOSEN FOOTPRINT

> **V12 repairs three of the four BLOCKING findings of the second text review (2026-08-27).**
...
2. **Who may hold read access.** Only named key holders, recorded in BS-V's schema before any
   ...
   or inspect any χ-bearing object or derivative** until **BS-5f's confirmatory power receipt
   exists, then the primary lock is sealed, then unblinding occurs — in that order**.
   ...
   > reach §6. The order above is the only one consistent with §4, §5, §6.1(1) and §7. Blind automation is permitted only where named here and only
...
| BS-2a ⚠ **DESIGN, NEW IN V12** | Hwao | **acceptance design**: the numeric confidence threshold, retry/failure semantics, the evidence schema for exclusion reasons (a)–(d), the ledger schema, the recomputation code and its fixtures. Gated as text AND code **before any image byte**. Repairs CODEX-V11 3 / GPT56-V11 F3 | `run_production_verdict`, pre-verdict validator | BS-2f, BS-6 |
