...
6. **The confidence quantity is defined, not merely thresholded.** Reason (d) is the
...
7. **Acceptance design is its own class-P slot, and it closes before BS-6.** The numeric
...
2. **Who may hold read access.** Only named key holders, recorded in BS-V's schema before any
   ...
   exists, then BS-L seals the primary lock, then unblinding occurs — in that order**. BS-V, the
   verdict receipt, is written after unblinding and is not the lock.
