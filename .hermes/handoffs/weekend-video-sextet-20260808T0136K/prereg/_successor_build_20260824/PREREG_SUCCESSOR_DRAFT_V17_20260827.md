# PREREGISTRATION DRAFT V17 — LONGO-AMPLITUDE TEST ON A LEVERAGE-CHOSEN FOOTPRINT

> **V17 is a repair of V16.** It repairs `PREREG_SUCCESSOR_DRAFT_V16_20260827.md`, sha256
...
Scalar path: `Â_L = β̂/(2â−1)`. Profile path (frozen fallback, §6): `Â_L = β̂/ŵ` with `w_profile()` under **unit weight per accepted object**. The branch predicate (after BS-8f, before any real statistic, explicitly tied to `adjudicate_path()`) selects the scalar path if `max_b |â_b - â| <= 0.03`, and selects the profile path otherwise. This is a profile selection, not a run failure. 
...
`PROD-REFUSES` close that.) It emits exactly one outcome from the canonical registry (V3-pred F-6 thresholds,
applied to Â_L):
...
**V15 → V16.** Recorded the complete replacement of §6 from R15 Part 1, and applied its conforming edits to §2.5, §2.7, §4, §5, and §7, as required by Clause 10 and V15 §6.3/§10.
