# PREREGISTRATION DRAFT V18 — LONGO-AMPLITUDE TEST ON A LEVERAGE-CHOSEN FOOTPRINT

> **V18 is a repair of V17.** It repairs `PREREG_SUCCESSOR_DRAFT_V17_20260827.md`, sha256
> `1a0a259a91f5a73a80fc864148e5fb6b0a2014dbf2494d243484e3948c16fce5` — independently verified.
...
`require_sealed()`, requires and verifies the canonical BS-L artifact and the one-use unblinding receipt. The guard must also verify the exact final-mask binding and post-unblinding ledger recomputation before forming any statistic. The production runner must refuse before forming any statistic if the adequacy tree emits an `INCONCLUSIVE` result. It derives
...
`PROD-REFUSES` close that.) The canonical registry is explicitly split by cardinality. `run_production_verdict()` emits **exactly one run-level outcome** (V3-pred F-6 thresholds, applied to Â_L):

- **Numeric verdicts:** **REPRODUCED-LONGO** (p < **0.001** AND Longo's sign AND |Â_L − 0.0408| ≤ 3·σ_comb AND Â_L ≥ the evaluated floor), **REJECTED-AT-LONGO-AMPLITUDE** (p > **0.05** AND (|Â_L| + 3·σ_ours(â)) < **0.0408**), **INCONCLUSIVE** (any other numeric outcome).
- **Pre-statistic inconclusive halts:** **INCONCLUSIVE-BY-POWER** (§4), **INCONCLUSIVE-BY-CALIBRATION** (§6, pre-unblinding, post-unblinding removal, or non-finite/degenerate decision inputs), and **INCONCLUSIVE-BY-MISSING-ALLOCATED-OUTPUT** (Row I pre-BS-8f abort).
...
| finding | change |
|---|---|
| §6.3 operative bodies | Restored the operative bodies with normative verbs to the §6.3 titles. |
| §4/Row-J conformance | Added the Stage C calibration gate and pre-attrition-only text to §4. |
| §7 count | Repaired the Class P and Class E counts in §7 to match the table. |
| Two smaller overclaims | Narrowed the candidate evidence claim in §2.6 and added the 0.03 spread threshold in §3. |
| Partial repairs | Chronology split applied to banner only; outcome registry gathered but not explicitly partitioned by cardinality; reason (d) deletion was left incomplete. |
...
| Blocker 4 & 6 (§10) | Removed the false historical claim, added the missing V16→V17 and V17→V18 repair traces, and made the trace part of every future revision. |
