# PREREGISTRATION DRAFT V21 — LONGO-AMPLITUDE TEST ON A LEVERAGE-CHOSEN FOOTPRINT

> **V21 is a repair of V20.** It repairs `PREREG_SUCCESSOR_DRAFT_V20_20260827.md`, sha256
> `607df3dd5b022a299162dac501b9c5766dda87bac8b3ba1cea11a105efa00261` — independently verified.
...
`run_production_verdict()` returns exactly: the numeric outcomes and its two `INCONCLUSIVE-BY-POWER` branches (Stage-C failure and `N_eq` floor). **Unresolved required implementation:** BS-L verification, authenticated one-use unblinding-receipt verification, accounting, post-unblinding calibration return, Row-I emission, the Row-J calibration guard, per-attempt emission, and `VOID` conversion.
...
> and a fresh text gate, not a receipt insertion**. On today's count: **BS-2a and BS-2k are DESIGN slots.** One of fourteen class-P slots is filled (BS-2m). There are 8 class-E slots. (Lint assertion: the prose count equals the parsed table count and the DESIGN inventory matches the VALUE/DESIGN classification.)
...
| `VOID` converter ⚠ **DESIGN, CLASS P — UNFILLED** | — | **`VOID` conversion**: handle every enumerated void antecedent into the `VOID` category, plus branch-complete fixtures. | — | BS-6 |
...
| §7 count | Repaired the Class E count in §7 from 7 to 8; the already-correct Class P count remained 14. |
...
- **`VOID` conversion:** Implement a converter that handles every enumerated void antecedent into the `VOID` category, and add its branch-complete fixtures. This is a pre-BS-6 dependency.
