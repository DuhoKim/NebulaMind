# PREREGISTRATION DRAFT V22 — LONGO-AMPLITUDE TEST ON A LEVERAGE-CHOSEN FOOTPRINT

> **V22 is a repair of V21.** It repairs `PREREG_SUCCESSOR_DRAFT_V21_20260827.md`, sha256
> `8386d5f0b3cdc8ed4161545dbcf2f8e4898c9c68942ddfc117b3103ef6ea10e5` — independently verified.
> Both seats verified the capability inventory by parsing the pinned source. The guard sentence at lines 458-461 and VOID reachability are repaired here.
...
2. **Exclusion reasons are enumerated here and nowhere else.** An object may be excluded pre-lock only
   ...
   incomplete at the frozen tensor shape. Instrument absence/non-finiteness and confidence threshold
...
3. **Every exclusion predicate must be sign-blind by construction.** None of (a)–(b) may read,
...
> and a fresh text gate, not a receipt insertion**. On today's count: **BS-2a, BS-2k, and the `BS-2v` VOID converter are DESIGN slots.** One of fifteen class-P slots is filled (BS-2m). There are 7 class-E slots. (Lint assertion: the prose count equals the parsed table count and the DESIGN inventory matches the VALUE/DESIGN classification.)
...
| BS-2a ⚠ **DESIGN, CLASS P — REFUSED / UNFILLED** | Hwao | **acceptance design**: the numeric confidence threshold **and the named authority that sets it**, retry/failure semantics, the evidence schema for exclusion reasons (a)–(b), the ledger schema, the recomputation code and its fixtures. Gated as text AND code **before any image byte**. V12 placed this in Class E while §2.7 called it a class-P prerequisite. **BS-6 is blocked until a new BS-2a design passes gates that removes the confidence/amplitude dependency and implements the hermetic integrity verifier.** | `run_production_verdict`, pre-verdict validator | BS-2f, BS-6 |
| BS-2k ⚠ **DESIGN** | Duho | **custody provisioner**: creates stores, escrows keys, installs mediator, records archive seal state | — | BS-6 |
| BS-2v ⚠ **DESIGN, CLASS P — UNFILLED** | Hwao | **`VOID` conversion**: handle every enumerated void antecedent. The converter must define a canonical closed antecedent registry with stable IDs and exact source/phase/failure-effect for each `VOID` branch in §5 and §6. It requires a fixture coverage receipt that verifies exact set equality between manifest IDs and exercised IDs (`set(fixture.antecedent_id) == set(converter.branch_id)`). Missing, duplicate, extra, or non-`VOID` conversion for any ID fails the gate and leaves BS-6 blocked. | `VOID_converter` | BS-6 |
...
| BS-5f | Hwao | Stage-C confirmatory power receipt | BS-L |
...
blind double, BS-9's input-function schema, the BS-L primary lock, and that the frozen
...
**V15 → V16.** Recorded the complete replacement of §6 from R15 Part 1. (The previous historical claim about other edits was removed as a V17 finding confirmed they were incomplete.)

**V16 → V17.** Applied the V16 review findings to the document:
| finding | change |
|---|---|
| §6.3 operative bodies | Restored the operative bodies with normative verbs to the §6.3 titles, and replaced Row P's superseded V15 citation with a citation to the restored §6.3. |
| §4/Row-J conformance | Added the Stage C calibration gate and pre-attrition-only text to §4. |
| §7 count | V17 changed the class-E prose from 7 to 8 while the table held 7, introducing a count error that V18–V21 inherited, and V22 corrects the prose to 7. |
| Two smaller overclaims | Narrowed the candidate evidence claim in §2.6 and added the 0.03 spread threshold in §3. |
| Partial repairs | Chronology split applied to banner only; outcome registry gathered but not explicitly partitioned by cardinality; reason (d) deletion was left incomplete; and the historically false §10 claim ("applied §4") was left unrepaired. |

**V17 → V18.** Applied the V17 review findings:
| finding | change |
|---|---|
| Blocker 1 (§2.7) | Removed the disjunctive threshold ownership and the stale reason (d); Row P applies the single BS-2a authority. |
| Blocker 2 (§5) | Split the canonical registry into explicitly disjoint run-level outcomes and per-attempt terminal states; added missing Row I abort. |
| Blocker 3 (§3) | Enforced the calibration gate's precedence over the 0.03 scalar/profile spread. |
| Blocker 4 & Repair 6 (§10) | Removed the false historical claim, added the missing V16→V17 and V17→V18 repair traces, and made the trace part of every future revision. |
| Repair 5 (Chronology) | Applied the three-moment chronology correctly to the fold record to match the banner. |

**V18 → V19.** Applied the V18 review findings:
| finding | change |
|---|---|
| Blocker 1 (§5) | Defined the run-level registry as the study-run lifecycle outcome registry, named the producing phase/process for each category, narrowed `run_production_verdict()`'s emitter claim, and split non-finite failures by cause (listing the unhandled permutation/statistic failures as unresolved required work). |
| Blocker 2 (§10 trace) | Expanded the V16→V17 §6.3 row to include the Row P citation replacement, added the unrepaired §10 claim to the partial repairs row, and fixed the "Blocker 4 & Repair 6" mislabeling in V17→V18. |

**V19 → V20.** Applied the V19 review findings:
| finding | change |
|---|---|
...
| E. Non-finite split | Defined the calibration-input non-finite/degenerate failures to exclude the Row-I case, and named the required aggregate validator and its authenticated aggregate outcome in §5 and §11. (Compared the V19→V20 diff against §5/§11 wording: the text called it a validator and outcome, not a producer). |
| Trace repair | Corrected the V16→V17 trace row for the §7 count. |


**V20 → V21.** Applied the V20 review findings:
| finding | change |
|---|---|
| Blocker 1 (§5) | Replaced the present-tense guard claim with an explicit required-but-unimplemented guard. Added BS-L verification and authenticated one-use unblinding-receipt verification to the unresolved implementation list. Kept the exact return-value sentence. |
| Blocker 2 (Clause 10 / §7 / §11) | Added a direct clause-10 status sentence: `VOID` reverse reachability is unresolved; therefore clause 10 is not yet executable, and BS-6 and the first image byte remain blocked until a pinned producer or conversion handles every enumerated void antecedent. Added the converter and branch-complete fixtures to §11, and made it a pre-BS-6 dependency in §7 without inventing a producer name. |
| Repair 3 (§10) | Corrected the V19→V20 trace aggregate-validation row to what actually changed, stating the diff versus wording comparison. |

**V21 → V22.** Applied the V21 review findings:
| finding | change |
|---|---|
| Blocker 1 (§7) | Repaired the §7 count: updated the class-P prose to 15 filled/unfilled slots including the new converter, added it to the DESIGN inventory, and corrected the class-E count from 8 down to 7 to match the table. Gave the converter a `BS-2v` slot ID. |
| Blocker 2 (§7/§11) | Upgraded the `BS-2v` dependency to an enforceable gate by specifying a canonical antecedent-manifest schema, exact stable IDs, set-equality coverage requirement, and a defined failure effect for missing/duplicate/non-`VOID` coverage. |
| Blocker 3 (§5/§11) | Expanded the unresolved implementation inventory in §5 and §11 to explicitly list exact final-mask binding verification, post-unblinding ledger recomputation, and the adequacy-tree pre-statistic refusal. |
| Blocker 2 (trace) | Corrected the V16→V17 trace row to state that V17 introduced the class-E count error rather than repairing it. |

**Future revisions:** Every future gated revision must include its finding→change mapping in this section.
