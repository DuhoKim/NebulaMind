# PREREGISTRATION DRAFT V23 — LONGO-AMPLITUDE TEST ON A LEVERAGE-CHOSEN FOOTPRINT

> **V23 is a repair of V22.** It repairs `PREREG_SUCCESSOR_DRAFT_V22_20260827.md`, sha256
...
| `VOID-2.7-THRESHOLD-MOVED` | §2.7 | Post-first-real-χ | VOID |
...
| §7 count | V16's class-E prose of 7 was wrong; **V17 corrected it to 8**; V18–V21 carried 8 correctly; **V22 changed it to 7 and introduced the present mismatch, on a wrong instruction from the coordinator**; V23 restores 8. |
...
| finding | change |
|---|---|
| Blocker 1 (§7) | Updated the class-P prose to 15 filled/unfilled slots, added the converter to the DESIGN inventory, and changed the class-E count to 7, introducing the present mismatch on a wrong instruction from the coordinator. |
| Blocker 2 (§7/§11) | `BS-2v` was **given a slot ID and a stated intent, not yet an enforceable gate.** |
...
**V22 → V23.** Applied the V22 repair brief:
| finding | change |
|---|---|
| Blocker 1 (§7) | Restored the class-E count to 8. |
| Blocker 2 (trace) | Corrected the materially inaccurate trace rows for V16→V17 and V21→V22 to state the truth about the class-E count error and `BS-2v` intent. |
| Blocker 3 (§7) | Gave `BS-2v` an independent canonical antecedent registry to establish a reference set for equality. |
| Blocker 4 (§6.1/§11) | Added `BS-2v` to §6.1's closed non-χ-bearing list and §11's `SLOT_SCHEMA` additions with its authenticated fields. |
