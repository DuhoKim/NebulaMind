# PREREGISTRATION DRAFT V37 — LONGO-AMPLITUDE TEST ON A LEVERAGE-CHOSEN FOOTPRINT
...
**One reconciliation is deliberately left open.** `VOID-2.7-THRESHOLD-CHOSEN-OR-MOVED` carries phase `Post-first-real-χ` **inherited unchanged** from its predecessor ID. §2.7's own trigger is *"after inference exists"*, and whether that names the same instant has **not** been decided. The authorisation extended the ID's *coverage*; it did not settle the *phase*. **Until that is settled this row must not be read as phase-verified, and the registry must not be pinned as final.**
