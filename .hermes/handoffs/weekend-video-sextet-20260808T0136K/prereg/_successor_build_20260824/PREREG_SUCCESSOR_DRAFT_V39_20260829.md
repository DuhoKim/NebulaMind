# PREREGISTRATION DRAFT V39 — LONGO-AMPLITUDE TEST ON A LEVERAGE-CHOSEN FOOTPRINT
...
**OPEN, raised by CODEX at V38 (F2, HIGH), and deliberately not resolved here.** §5 line 493 states the VOID trigger for *"permutation/statistic/protocol non-finite/degenerate failures"* **without a phase qualifier**, while `VOID-5-NONFINITE` and `VOID-5-DEGENERATE` are scoped `Post-unblinding`. Line 491's `INCONCLUSIVE-BY-CALIBRATION` covers only **aggregate** non-finite/degenerate failures, so a *permutation* or *statistic* failure arising **pre-unblinding** — reachable, since Stage C runs permutations pre-unblinding — is voided by §5's prose and matched by no antecedent phase. **Every available repair changes what voids a run** (broaden the phases, add pre-unblinding antecedents, or qualify §5's prose), and the authorship record does not determine which was meant. Referred to the principal; see `OPEN_QUESTION_VOID_5_PHASE_SCOPE.md`. **Until it is answered the registry is complete only as to the gaps closed below.**
