# PREREGISTRATION DRAFT V40 — LONGO-AMPLITUDE TEST ON A LEVERAGE-CHOSEN FOOTPRINT
...
- **Pre-statistic inconclusive halts:** **INCONCLUSIVE-BY-POWER** (produced by Row J, and the production runner's `N_eq` and Stage-C power guards), **INCONCLUSIVE-BY-CALIBRATION** (produced by Row J pre-unblinding, pre-verdict validator post-unblinding removal, or aggregate non-finite/degenerate failures excluding Row-I's missing allocated outputs — validated by `validate_calibration_aggregates` before the < 0.85 comparison, emitting the authenticated aggregate outcome), **INCONCLUSIVE-BY-MISSING-ALLOCATED-OUTPUT** (produced by Row I pre-BS-8f abort), and **INCONCLUSIVE-BY-COMPUTATION** (new; see the procedure below).
- **`INCONCLUSIVE-BY-COMPUTATION` — the pre-unblinding numerical route, and what the operator does.** Emitted when a **permutation, statistic or protocol** computation returns a **non-finite or degenerate** result **before unblinding**. This is a halt, not a void. The procedure is binding, because an inconclusive code that does not say what to do is a label rather than a rule:
  1. **Halt pre-unblinding and emit the code.** No real χ is read, and no verdict path opens.
  2. **A rerun of Stage C is permitted** under the **same frozen implementation and protocol digests** — an unchanged mechanism is what makes a rerun a repeat rather than a new experiment.
  3. **Every attempt is recorded** — attempt index, the triggering condition, and the outcome — appended to the §6 log on the same receipted path as any other conduct record. **A silent rerun is a forbidden act** and voids the run under the unmoved clause above.
  4. **A rerun may be triggered only by a recorded non-finite or degenerate result, never by a finite result the operator dislikes.** Rerunning on a finite outcome is threshold-shopping by another name and voids the run.
  5. **The maximum attempt count is NOT pinned by this document and must be bound before BS-6.** An unbounded rerun allowance is itself a researcher degree of freedom, and naming a number here would be inventing policy rather than recording it. **This is a known unbound parameter, stated rather than hidden.**
...
**The §2.7 phase is settled from the authorship record, and the cell is unchanged at `Post-first-real-χ`.** The clause entered at V11 (commit `4d99d1d93`, 2026-08-27), written to answer GPT56 F2 and CODEX 1. **Authorship evidence, stated at its real strength (CODEX-V38 F4):** every commit in this repository carries the principal's git identity, so **commit metadata does not prove which agent wrote the words.** What supports lane authorship is the commit body itself — written in the first person, adjudicating referee findings by name and admitting its own errors (*"KIMI F3, and it was mine"*) — together with its `Co-Authored-By` model trailer. That is strong evidence, **not proof**, and the principal declined the question on the same basis. V11's own §2.7 preamble states what "inference" means: the acceptance freedom is *"the largest remaining researcher degree of freedom **because it is exercised after image inference exists** and it moves both the signs and the geometry."* **"Inference exists" is image inference having produced real output — the first real χ.** The document's ordering confirms real χ exists *before* unblinding, not after: the §6 conduct table's Row J (line 552) *"never reads a real χ"* yet can halt the run **pre-unblinding**, and §5's disclosure clause (line 518) forbids χ-derived disclosure *"before the primary lock"* — both presuppose real χ sitting in the sealed stores pre-lock. (**Both citations were wrong in V38**, which named §6.1 and §6.2; corrected per CODEX-V38 F4.) `Post-first-real-χ` is therefore **earlier and broader** than `Post-unblinding`, and is the instant the clause names. §6.3's void rule is anchored at the same instant, so the registry and §6.3 agree rather than compete.
