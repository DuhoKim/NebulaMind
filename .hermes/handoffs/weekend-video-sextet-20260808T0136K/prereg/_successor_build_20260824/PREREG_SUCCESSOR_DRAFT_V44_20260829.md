# PREREGISTRATION DRAFT V44 — LONGO-AMPLITUDE TEST ON A LEVERAGE-CHOSEN FOOTPRINT
...
- **`BS-3g` sensitivity-gradient receipt:** Add the `SLOT_SCHEMA` entry, producer and independent verifier for the sensitivity-gradient control's slot receipt. **Without this the slot's `blocks BS-6` edge is declared but not receiptable** — the class is listed in §6.1 as of V41, and nothing yet produces or verifies an artifact conforming to it. Its content depends on the completeness semantics still open in `OPEN_QUESTION_GAIN_SIGN_MAPPING.md`, so this item is named, not specified.
