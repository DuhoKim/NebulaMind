# PREREGISTRATION DRAFT V45 — LONGO-AMPLITUDE TEST ON A LEVERAGE-CHOSEN FOOTPRINT
...
- **Pre-statistic inconclusive halts:** **INCONCLUSIVE-BY-POWER** (produced by Row J, and the production runner's `N_eq` and Stage-C power guards), **INCONCLUSIVE-BY-CALIBRATION** (produced by Row J pre-unblinding, pre-verdict validator post-unblinding removal, or aggregate non-finite/degenerate failures excluding Row-I's missing allocated outputs — validated by `validate_calibration_aggregates` before the < 0.85 comparison, emitting the authenticated aggregate outcome), **INCONCLUSIVE-BY-MISSING-ALLOCATED-OUTPUT** (produced by Row I pre-BS-8f abort), and **INCONCLUSIVE-BY-COMPUTATION** (**produced by the same Stage-C runner (Row J) and production-runner guards that produce the two codes above**; see below — it is **residual**, firing only where neither `INCONCLUSIVE-BY-POWER` nor `INCONCLUSIVE-BY-CALIBRATION` applies).
- **`INCONCLUSIVE-BY-COMPUTATION` — the pre-unblinding numerical route, and what the operator does.** Emitted when a **permutation, statistic or protocol** computation returns a **non-finite or degenerate** result **before unblinding**, **and only where no other pre-statistic code applies.** `INCONCLUSIVE-BY-COMPUTATION` is **residual by construction**: an aggregate calibration failure emits `INCONCLUSIVE-BY-CALIBRATION`, and a Stage-C or `N_eq` power-guard failure emits `INCONCLUSIVE-BY-POWER`; this code fires only on a numerical failure neither of those guards claims (GPT56-V40 F5; GPT56-V43 F1, CODEX-V43 F1). **Producer:** the Stage-C runner (Row J) and the production runner, the same producers as the codes it defers to — so every branch of Row J terminates in exactly one named outcome, as §6.3(10) requires. **The halt is terminal** (principal ruling of 2026-08-29, option A). The run ends with this outcome and no verdict path opens; no real χ is read. **There is no rerun inside this run.** The operator's recourse is a **new run under a new preregistration**, which is a rule that can be followed without a further decision. **A re-execution of Stage C inside this run is a forbidden act** and voids the run under the unmoved clause below.
...
**RESOLVED at V40 by principal ruling of 2026-08-29 (CODEX-V38 F2, HIGH; option C).** §5 line 493 previously stated the VOID trigger for *"permutation/statistic/protocol non-finite/degenerate failures"* with **no phase qualifier**, while these antecedents are scoped `Post-unblinding` — leaving a pre-unblinding permutation failure voided by the prose and matched by no antecedent, reachable because Stage C permutes pre-unblinding on synthetic signs. **The ruling qualified the prose rather than the phases:** the numerical conditions read post-unblinding, and pre-unblinding numerical failures route to `INCONCLUSIVE-BY-COMPUTATION`, which is a **terminal halt** — the V40 rerun allowance was deleted at V43 under the principal's option A ruling. **The misconduct conditions were deliberately left untouched at `Any`.** **No registry row changed, so `registry_digest` is unmoved** — the prose was brought to the registry, not the registry to the prose.
...
- **`INCONCLUSIVE-BY-COMPUTATION` producer:** Implement the residual numerical-failure outcome in the Stage-C runner and the production runner, **evaluated after** the power and calibration guards so its residual status is enforced by order of evaluation rather than by assertion. It must emit the code and halt pre-unblinding; it must not be reachable when either prior guard has fired.
