# PREREGISTRATION DRAFT V52 — LONGO-AMPLITUDE TEST ON A LEVERAGE-CHOSEN FOOTPRINT
...
- **`INCONCLUSIVE-BY-NUMERICAL-FAILURE` — the class rule (principal ruling, 2026-08-29, option B).**
  ...
  **The live example, and it is (iii).** `allocate_handcheck` carries eight feasibility guards; **five did not fire across 60,000 generated cell-count tables** at nine density scales with dead strata and dead bins injected, at the frozen constants — **and** the function's own docstring states that *feasibility is DECIDED before allocating*, so the later guards are subsumed by the earlier decision. Measurement and structure agree.
  ...
  **Known extent, reported as a range and not a number.** Counted by AST rather than by `grep` after GPT56-V49 F4 showed the grep wrong: the pinned reference carries **112 `Raise` nodes — 68 `RuntimeError`, 39 `ManifestClosureError`, 2 `InconclusiveByPower`, 1 `ValueError`, 1 `InconclusiveByCalibration`, 1 bare re-raise.** The earlier figure of 111 keyed on `RuntimeError|ValueError` and **missed all 39 `ManifestClosureError` sites**, a third of the total. Classified by the boundary above: **29 caller guards, 31 reachable run-time failures, 48 not yet read** — so the class is **at least 31 and at most 79 — and that range is a heuristic candidate partition which has NOT had the sites claimed by VOID or by a more specific outcome subtracted from it (CODEX-V49 F3). It is an upper bound on candidates, not a count of members**. Four branches are *demonstrated* reachable by execution (`calibration_bins`' degenerate bins, and three of `allocate_handcheck`'s eight feasibility failures under the frozen constants). **The rule is written to cover the 48 unread sites without amendment**; if it needed a list, it would be the patch-per-instance habit wearing a rule's clothing.
