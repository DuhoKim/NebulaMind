# PREREGISTRATION DRAFT V53 — LONGO-AMPLITUDE TEST ON A LEVERAGE-CHOSEN FOOTPRINT
...
> **STAGE P REMAINS DUAL-VALUED, AND THIS TEXT CANNOT FIX IT (V12, KIMI-V11 F7 / GPT56-V11 F4,
...
- **`INCONCLUSIVE-BY-NUMERICAL-FAILURE` — the class rule (principal ruling, 2026-08-29, option B).**
  ...
  **The live example, named per site — because the previous statement of it broke this very rule (GPT56-V52 F1, CODEX-V52 F1).** V52 claimed five of `allocate_handcheck`'s eight guards were unreachable **without naming which five**, while the supporting ledger marked all eight `NUMERICAL`. A rule requiring per-site evidence cannot be illustrated by an unnamed count.
  **Harness:** 60,000 generated cell-count tables, nine density scales, dead strata and dead bins injected, at the frozen constants (`N_CAL_BINS=3`, `N_HC_STRATA=9`, `HC_MIN_PER_CELL=10`, `HC_MIN_PER_STRATUM=30`, `HC_REAL_LABELS=500`). **Positive control: three guards did fire** — L1397, L1403, L1442 — so the harness can reach this family.
  **Promoted on (iii), measurement AND structure — four sites:** L1411 (floors exceed budget after the stratum lift), L1435 (no headroom remains), L1437 (allocation ≠ budget), L1439 (allocation exceeds available in a cell). Each is a **post-allocation** guard, and the function's docstring states feasibility is *DECIDED before allocating*, so each is subsumed by the earlier decision.
  **Promoted on (i), measurement ONLY — one site:** **L1401** (inherited floors need N, budget B). **The structural argument does not cover it and V52 wrongly implied it did**: L1401 is a branch of the feasibility decision *itself*, evaluated beside L1403 which fired. Its promotion rests on 60,000 executions and nothing else, which by this section's own wording is the weaker basis and is recorded as such.
