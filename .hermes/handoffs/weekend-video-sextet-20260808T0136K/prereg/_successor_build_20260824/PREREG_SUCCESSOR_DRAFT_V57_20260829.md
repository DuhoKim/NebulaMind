# PREREGISTRATION DRAFT V57 — LONGO-AMPLITUDE TEST ON A LEVERAGE-CHOSEN FOOTPRINT
...
- **`BS-3g` sensitivity-gradient receipt:** Add the `SLOT_SCHEMA` entry, producer and independent verifier for the sensitivity-gradient control's slot receipt. **Without this the slot's `blocks BS-6` edge is declared but not receiptable** — the class is listed in §6.1 as of V41, and nothing yet produces or verifies an artifact conforming to it. **The completeness semantics are settled**, not open: the principal ruled this a real gate on 2026-08-29 and selected the executable joint counterfactual path (`ref/gain_counterfactual_path.py`). What the receipt's content still waits on is **one input only — the mapping from a gain gradient γ to the counterfactual accepted-sign vector and calibration** (`OPEN_QUESTION_GAIN_SIGN_MAPPING.md`), which is a modelling assumption requiring preregistration. So this item is named, not specified.
