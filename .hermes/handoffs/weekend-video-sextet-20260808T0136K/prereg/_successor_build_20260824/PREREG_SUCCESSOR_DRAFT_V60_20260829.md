# PREREGISTRATION DRAFT V60 — LONGO-AMPLITUDE TEST ON A LEVERAGE-CHOSEN FOOTPRINT
...
- **`INCONCLUSIVE-BY-NUMERICAL-FAILURE` — the class rule (principal ruling, 2026-08-29, option B).**
  ...
  **PRECEDENCE, stated so this code cannot become unreachable the way its predecessor did.** Where a more specific outcome already claims a failure, that outcome governs and this one does not fire: `INCONCLUSIVE-BY-POWER` for the Stage-C and `N_eq` power guards, `INCONCLUSIVE-BY-CALIBRATION` for aggregate validation, `INCONCLUSIVE-BY-MISSING-ALLOCATED-OUTPUT` for Row I's abort, **and — this is the one that was missing and that both seats caught (GPT56-V49 F2, CODEX-V49 F3) — every `VOID` antecedent in §7.1.** In particular the VOID clause below claims **permutation/statistic/protocol non-finite/degenerate failures post-unblinding**, and integrity conditions such as a digest mismatch, a planner change during the plan or an unclosed manifest are claimed by `VOID-5-DIGEST-DEVIATION`, `VOID-5-PROTOCOL-DEVIATION` and `VOID-5-FORBIDDEN-ACT`. **`INCONCLUSIVE-BY-NUMERICAL-FAILURE` never fires where a VOID antecedent applies.** Without this sentence the rule and the VOID clause claimed the same post-unblinding failures, which would have converted voiding conditions into inconclusive halts — the option-C concern arriving from the opposite side. **Being silent where a specific code already fires is correct behaviour here, not a defect** — this rule exists to terminate branches that terminate nowhere, not to compete for branches already terminated. That distinction is what the V46 deletion was about and what makes this addition consistent with it.
  ...
  **What promotes a guard to this status is evidence, and the evidence must be named per site — not a judgement that it looks unreachable.** Exactly one of:
  (i) **an execution count over a stated harness** — the harness, its input generator and the count recorded, so the claim is reproducible and its strength is visible; or
  (ii) **a structural argument** — a preceding guard that provably subsumes it, stated as the specific earlier condition; or
  (iii) **both**, which is the only combination that should be treated as strong.
...
— it is a fact about the harness.**
  **The four remaining guards (L1411, L1435, L1437, L1439) still did not fire across 80,000
executions with `budget` varied**, so their *measurement* survives. **They are nonetheless withdrawn**,
because CODEX-V53 F2 is right that a generic appeal to *"feasibility is decided before allocating"* is
not the **per-site predecessor condition** this section requires. Surviving a better harness is not the
same as meeting the bar.
  **What a promotion must carry, stated so the third attempt is not a third failure:** (a) a harness
that **varies every argument in the callable's documented surface**, not only the values a particular
...
- **VOID:** triggered, **at any phase**, by **forbidden acts** and by **protocol/digest deviation**; and, **post-unblinding only**, by **permutation/statistic/protocol non-finite/degenerate failures**. **This category is not yet executable.**
...
(ii) the access log under its BS-2k event schema (timestamp, actor, table row, operation, object identity, success/refusal, refusal reason, running chain digest — identities and flags, never payload bytes). **The refusal-reason field carries exactly one code from the closed set below and nothing else: no free text, no appended detail, no formatted values** (principal ruling of 2026-08-29, option A, on GPT56-V49 F1).
  ...
  **The closed set.** `REFUSED-ROW-NOT-AUTHORISED` (the requesting row has no stated surface covering this operation); `REFUSED-OUTSIDE-STATED-SURFACE` (authorised row, act outside its declared surface); `REFUSED-PRECONDITION-UNVERIFIED` (a required prior artifact is absent or fails its verifier); `REFUSED-PHASE-NOT-REACHED`; `REFUSED-LOCK-NOT-OPEN` (no verified BS-L, or no canonical opening authorization); `REFUSED-IDENTITY-OUTSIDE-PERMITTED-SET`; `REFUSED-SCHEMA-NONCONFORMING`; `REFUSED-CEREMONY-CONSUMED`.
  **NO CATCH-ALL, and the cost is accepted knowingly.** A refusal outside this set cannot be logged, and §6.1 makes an unlogged refusal void the run. **That is the correct outcome, not a harsh one:** if the set is genuinely derived from a closed row table, a refusal outside it means the table changed and the run's premises changed with it. **Options B (a catch-all code, proposed under the name REFUSED-UNCLASSIFIED — written here without code formatting deliberately, so it is not mistaken for a member of the set) and C (a second, out-of-band incident surface) were put to the principal and REFUSED.** They are recorded here so neither is rediscovered later as a fresh idea.
  **THE DERIVATION IS WITHDRAWN AND THE SET IS SUSPENDED (principal ruling, 2026-08-29 20:30).** This paragraph previously argued the set is closed because §6.1's row table is closed. **Both seats broke that argument in one round** (GPT56-V56 F1/F2, CODEX-V56 F1/F3): an authorised read of a permitted but **missing or unreadable cutout** is a refusal Row B must make that **none of the eight codes covers**, and since an unlogged refusal voids the run, a routine I/O failure would void the study. **The enumeration missed a CLASS, not a member** — it was derived from *authorisation* structure alone and ignored that a mediator also fails for reasons that are nobody's fault.
  **The principal has ruled the derivation be REDONE FROM SCRATCH, not extended by a ninth code**, because a set patched once will be patched again. **Option A is SUSPENDED — neither reaffirmed nor reversed.** The no-catch-all decision was made on this closure argument; the argument failed, so **the catch-all question returns to the principal only after a derivation exists that has been derived rather than asserted.** Nothing here may be read as carrying no-catch-all forward, or as abandoning it.
  **`REFUSED-IDENTITY-OUTSIDE-PERMITTED-SET` does not survive as written**: it describes the **object**, which the principle in this same paragraph forbids. CODEX's generalisation is the sharper warning and governs the rebuild — **request-shaped codes can still reveal object membership or bounds**, and that test applies to every candidate code, not only this one.
  **The new derivation must cover both axes and argue their joint exhaustiveness:** refusals arising from **authorisation**, and refusals arising from **availability and mediator behaviour** (missing, unreadable, truncated, timed out, checksum mismatch). **That exhaustiveness argument is a claim to be tested, not assumed — the first one failed.** The eight codes below are retained **only as the record of the superseded attempt**; they are not in force: Row B is the only path to sealed bytes, and Row R's default is forbidden. Every refusal is therefore one of — requester is not a row with a surface here; the surface does not cover this act; a stated precondition is unmet; the phase is not reached; the lock is not open; the identity is out of set; the write does not conform; the ceremony is spent. **The set is DERIVED, not constant. It stops being closed the moment §6.1 gains a row, a surface or a precondition** — and on that day it is **regenerated and re-pinned, never extended by hand.**
  refusal-vocabulary-derivation: **DELIBERATELY NOT RE-PINNED while the set is suspended.** The stale value `1d0b3e48…` was computed by a fingerprint that **omitted the `may touch` column** — the surface the whole derivation depends on — so rewriting Row B's surface to unrestricted access left it unchanged. The tool is fixed (`fd6d6d7e…` is the corrected digest). **Re-pinning now would turn the checker green while the set is still open**, which is a control reporting health about something it no longer describes. **`tools/refusal_vocabulary_check.py` fails when the table moves away from it**, so the derivation is a check rather than a claim; 
...
- **`BS-3g` sensitivity-gradient receipt — SPECIFIED (principal instruction, 2026-08-29 20:35).**
  **`SLOT_SCHEMA['BS-3g']` is exactly these twelve fields, no more and no fewer:**
  ...
  `gamma_bound` · `invariance_outcome` · `n_perturbations`.
  ...
  **Field definitions, written so a verifier can be implemented by someone who did not write this
  ...
  since a count alone cannot distinguish a full sweep from a favourable handful. `kernel_sha256`, `estimator_sha256`, `verifier_sha256` — 64-character lowercase hex
  ...
  count of perturbations evaluated.
  ...
  **Independent verifier:** a separately pinned checker that (a0) **refuses unless `mask_sha256`
  ...
  and (d) refuses `mapping_id` values not naming a preregistered mapping. **It must not accept the
