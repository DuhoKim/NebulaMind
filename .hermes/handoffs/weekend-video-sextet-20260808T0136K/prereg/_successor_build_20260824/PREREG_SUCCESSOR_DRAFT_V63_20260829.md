# PREREGISTRATION DRAFT V63 — LONGO-AMPLITUDE TEST ON A LEVERAGE-CHOSEN FOOTPRINT
...
- **`BS-3g` sensitivity-gradient receipt — SPECIFIED (principal instruction, 2026-08-29 20:35).**
  **`SLOT_SCHEMA['BS-3g']` is exactly these sixteen fields, no more and no fewer:**
  ...
  `draw_master_seed` · `draw_verdict_digest`.
  ...
  **Independent verifier:** a separately pinned checker that (a0) **refuses unless `mask_sha256`
  ...
  `n_draws` in either direction, and refuses unless the reported outcome is the worst of that exact
  sequence.** **The stopping rule is exactly `n_draws` draws, all evaluated: not "until it fails",
