# PREREGISTRATION DRAFT V67 — LONGO-AMPLITUDE TEST ON A LEVERAGE-CHOSEN FOOTPRINT
...
**REQUEST LIFECYCLE — Row B decides permission durably, and every state has one terminal treatment (repairing CODEX-VOCAB F1 and GPT56-VOCAB F1, both HIGH).** The derivation above assumed permission is *"evaluated before the attempt."* **The covenant nowhere imposed that ordering, and it is false in two ways** — a permission check can die before deciding, and a write's conformance cannot be judged before its payload is read. **Asserting the ordering is what failed; stating the machine is the repair.**
  ...
  **RESOLVE, THEN APPEND, THEN RELEASE — and the middle step is the repair (GPT56-V66 F2, CODEX-V66 F1, both HIGH).** V66 appended **before** the transfer, which kept the log ahead of delivery but made the event's own outcome field a claim Row B could not yet have: **one pre-transfer event cannot truthfully record a later `COMPLETED` or `FAILED`.** Both seats found it, and it was a defect introduced by the previous repair rather than an inherited one.
  **The three steps, in order.** (1) Row B **resolves the touch into storage it alone controls** — the bytes are read, or the write is validated and staged — so the outcome is **known**. (2) Row B **appends the single event carrying that true outcome.** (3) Only then does Row B **release the bytes to the requester, or commit the write to the store.**
  **What each crash point leaves.** Before (2): **no event and no delivery** — the touch did not happen, and re-processing is safe. After (2), before (3): **a true event and possibly no delivery** — the log records a touch the requester never received. **That residue is unchanged and still stated: the log can over-report a touch and can never under-report one**, which is the only safe direction for a custody log. **What is repaired is that the event is no longer false in the failure case** — it is complete-but-possibly-generous rather than premature-and-possibly-wrong.
  **This keeps `exactly one event per touch`**, so nothing here changes what the access log records.
...
- **`BS-3g` sensitivity-gradient receipt — SPECIFIED (principal instruction, 2026-08-29 20:35).**
  **`SLOT_SCHEMA['BS-3g']` is exactly these seventeen fields, no more and no fewer:**
  ...
  `draw_master_seed` · `draw_verdict_digest` · `baseline_verdict`.
  ...
  **THE MANIFEST MUST SPAN THE BOUND, because digesting a list does not make the list adequate
  ...
  singleton `{0}` by name.** **An invariance test that never perturbs anything is not a weak test; it
  is not a test.**
  ...
  **Independent verifier:** a separately pinned checker that (a0) **refuses unless `mask_sha256`
  ...
  encoding falls outside its stated bound.** **The stopping rule is exactly `n_draws` draws, all evaluated: not "until it fails",
