# PREREGISTRATION DRAFT V68 — LONGO-AMPLITUDE TEST ON A LEVERAGE-CHOSEN FOOTPRINT
...
- **`INCONCLUSIVE-BY-NUMERICAL-FAILURE` — the class rule (principal ruling, 2026-08-29, option B).**
  ...
  **THE PARTITION AGAINST `VOID` — this replaces a precedence rule, and the replacement is the repair (principal ruling, 2026-08-29; GPT56-V54 F1, HIGH).** V50 added the VOID antecedents to this rule's precedence to stop the two clauses claiming the same post-unblinding failures, **and the defect recurred at V54**, because precedence decides *which code wins* while both clauses still **describe the same events in the same phase**. A third ordering patch would have been the third attempt at the wrong kind of fix. **The two clauses are therefore made disjoint by what they are about, and the partition is stated here rather than the edit**, so a reader can take any post-unblinding failure and see which clause owns it **without consulting any rule of precedence**:
...
(ii) the access log under its BS-2k event schema (timestamp, actor, table row, operation, object identity, success/refusal, refusal reason, running chain digest — identities and flags, never payload bytes). **The refusal-reason field carries exactly one code from the set below and nothing else: no free text, no appended detail, no formatted values** (principal ruling of 2026-08-29 on GPT56-V49 F1, as revised at 22:18 that day — see the non-closure ruling below).
  **THE PRINCIPLE, which is the test — the list is only its consequence.** A refusal reason may describe the **REQUEST** and the **AUTHORISATION STATE**. It may **never describe the OBJECT**. That is what makes closure safe rather than merely tidy: object identity is already its own field, so the reason adds no per-object information, and no code below can encode a measured property because none refers to one. A reason such as *"instrument confidence below threshold"* is χ-derived and is excluded **by construction, not by discipline**. **An editor proposing a ninth code must satisfy this test, not merely match the style of the eight.**
  ...
  **THE BLOCKING INVARIANT, stated as a consequence rather than an intention: `BS-L` MAY NOT BE ISSUED while any `REFUSED-UNCLASSIFIED` event in the access-log chain is unenumerated.** The enumeration is part of the lock checkpoint: **one entry per emission**, each naming the request's row, operation and lifecycle state, and **each named or explained by a person**.
  ...
  **THE CLASS KEY IS COMPUTED, NOT ASSIGNED — because a class you may name is a class you may rename (GPT56-V67 F4, CODEX-V67 F4, both HIGH).** Two emissions are the same class **iff their `class_key`s are equal**, and `class_key` = **(table row, operation, lifecycle state at failure)** — three fields the event already carries. **Relabelling cannot split a class because the label is not in the key**; the enumeration verifier **recomputes the key from the event** and refuses an entry whose stored key mismatches, and **refuses a second `EXPLAINED` disposition for the same key** — the second occurrence demands `NAMED-AS-DEFECT` and the vocabulary re-derivation that naming requires. **The key is facts about the request, not judgements about it, which is what makes the recurrence rule enforceable rather than aspirational.**
  ...
  **THE ENTRY, ITS PRODUCER, ITS JOIN AND ITS WIRING — because a verifier with no object to verify is a name (GPT56-V67 F3, CODEX-V67 F3, both HIGH).** An **ENUMERATION ENTRY** is an authenticated record with exactly these fields: `chain_position` (the event's index in the access-log chain) · `event_digest` (the chain's running digest at that position) · `class_key` (**computed, defined below**) · `disposition` (exactly one of `NAMED-AS-DEFECT` · `EXPLAINED`) · `explanation_ref` (the identifier of the signed human explanation — **no free text in the entry itself**) · the enumerator's signature. **The JOIN is `(chain_position, event_digest)`** — both already exist in the chain, so **nothing new enters the access log**; entries live in the **lock-checkpoint materials**, produced under Row B's existing checkpoint duty. **THE WIRING:** §5's BS-L verification guard extends — **`BS-L` may not issue without an enumeration-verifier PASS over the chain at issuance, and the opening authorization's verifier requires a FRESH PASS at opening**; §11 carries the build item for `gates/enumeration_verifier.py`, separately pinned, which **recomputes the emission set and every `class_key` from the chain itself and never accepts a producer's summary of its own emissions**. **AND THE LIMIT OF THE LINT, stated because three consecutive revisions phrase-matched here:** `tools/refusal_vocabulary_check.py` verifies that **this text states** the mechanism. **It cannot verify the mechanism exists.** Existence is established by the §11 item and its gate round, and a green lint says nothing about it. That is what makes the guard executable: **it is not a promise to review, it is a gate that will not open.**
  ...
  **`REFUSED-INTEGRITY-MISMATCH` IS FLAGGED AND UNRESOLVED, deliberately.** A digest mismatch on a sealed object may be a storage fault or it may be tampering, and **both seats find nothing observable at emission distinguishes them.** CODEX adds that the same observable event is already claimed by the **phase-Any digest-deviation VOID antecedent**, so a refusal and a void compete for one event. **Left open rather than resolved:** getting it wrong toward VOID is how the earlier over-strict concern arose, and getting it wrong the other way logs tampering as a fault.
...
**REQUEST LIFECYCLE — Row B decides permission durably, and every state has one terminal treatment (repairing CODEX-VOCAB F1 and GPT56-VOCAB F1, both HIGH).** The derivation above assumed permission is *"evaluated before the attempt."* **The covenant nowhere imposed that ordering, and it is false in two ways** — a permission check can die before deciding, and a write's conformance cannot be judged before its payload is read. **Asserting the ordering is what failed; stating the machine is the repair.**
  **The states, and every request is in exactly one:** `RECEIVED` → `PENDING-AUTHORISATION` → for reads `AUTHORISED` or `REFUSED`; **for writes `PENDING-SURFACE-CHECK` first**, because whether a field-constrained write on Rows C2 or H is within the stated surface **cannot be known until Row B decodes the payload** — then `AUTHORISED` or `REFUSED` → `TRANSFER` → `COMPLETED` or `FAILED`.
  **THE DURABLE LOG BOUNDARY — restated, because V64's version asserted a boundary without making it atomic (GPT56-V64 F1, CODEX-V64 F1, both HIGH).** V64 said the event is appended *"at the instant the verdict is reached."* **Reaching a verdict and appending it are two operations, and a crash between them loses the event** — the same shape as the ordering assertion this clause was written to repair, committed inside the repair.
  **THE APPEND IS THE VERDICT.** No verdict exists until its event is durably in the access-log chain. Row B does not decide and then record; **the record is the decision**, and a request whose event is not in the chain was never decided.
  **THREE ORDERINGS HAVE FAILED, AND THE THIRD FAILURE IS STRUCTURAL (GPT56-V67 F1, CODEX-V67 F1, both HIGH).** V64 appended at the verdict and a crash between deciding and appending lost the event. V66 appended before release and the event asserted an outcome Row B could not yet know. V67 resolved first, then appended — and **for a READ, resolving IS the touch**: the sealed bytes leave the store into Row B's buffer at step (1), so a crash before step (2) leaves **a completed sealed-store read with no event**, violating the one property the log must never lose. For a WRITE, staging success cannot establish the later commit's outcome, so the "true outcome" claim was false there too. **One event, two fallible operations — the store effect and the log append — and no ordering of two fallible operations can make one of them always-true about the other.** Three arrangements failing the same way is the partition's evidence-shape again: **the axis was wrong, not the arrangements.** The repair is not a fourth ordering.
  **THE ATOMIC TOUCH CONTRACT — a named BS-2k design obligation, so it is buildable rather than assumed.** The three sealed stores and the access-log chain are provisioned in **one transactional commit domain**, and a **TOUCH is a single atomic commit** comprising three things: **the store effect** (bytes leaving the store, or a write landing in it), **its one event**, and **Row B's recovery-state update** — the request-identifier binding. **It commits entirely or not at all.**
  **Crash semantics become exact, and now they are true for reads as well.** An uncommitted touch leaves **no effect, no event, no binding** — including no bytes having left the sealed store, because the bytes' leaving is part of the commit — so re-processing is safe and the log has under-reported nothing. A committed touch carries its **true** outcome by construction: the event does not predict the outcome, **it records it inside the same commit that produced it**.
  **What this closes and what it deliberately does not.** It closes the under-logged read, the falsely-reported write, and bytes-preceding-event — all three of this round's ordering failures. **It also closes the recovery ambiguity (GPT56-V67 F2, CODEX-V67 F2):** the identifier→event binding commits atomically with the event, so recovery consults its own committed bindings — a request with a binding is **never re-decided**, a request without one **never happened**, and a legal retry is a new request with no binding and proceeds. **Nothing new enters the access log**: the recovery record is Row B's own state, committed alongside the log, not written into it. **Still exactly one event per touch.** **The residue that remains is the referred one**: a request that dies before its touch commits is indistinguishable from one that never arrived, and making it visible needs the second event class that is not authorised.
  **The contract is a requirement on the BS-2k design artifacts** (§7 Row A) **and §11 carries its implementation item.** Until BS-2k is filled with a design meeting it, the lifecycle above is specified and not yet operative — **which is the honest state of every mechanism in this section, and the reason BS-2k blocks what it blocks.**
  ...
  **ONE DECISION PER REQUEST, AND ROW B OWNS RECOVERY.** Row B is a **single serialised writer** with **at most one decision in flight per request**. On recovery: a request **with no appended event** never happened and may be re-processed; a request **with an appended event is never re-decided**, which is what forbids the two-terminal-treatments case. **A worker that has lost its lease may not append** — the lease is Row B's, and losing it means the work is abandoned, not resumed.
  **WHAT THIS DOES NOT FIX, and it is not mine to fix.** A **durable pre-verdict state** — a record that a request was in flight when Row B died — **requires a second event class**, and Row B's cell commits to *"exactly one event per touch."* **Adding one changes what the access log records, which is explicitly not authorised**, so the gap is recorded and referred rather than closed. **What the rules above do achieve without it:** no request is both undecided and delivered, and none is decided twice. **What remains:** a crash between decide-and-append is indistinguishable from a request that never arrived. **That is a real residue and it is stated as one.**
  **TERMINAL TREATMENT, fixed per state, so no request can end undecided or unlogged.** A request that dies in `RECEIVED`, `PENDING-AUTHORISATION` or `PENDING-SURFACE-CHECK` — timeout, deadlock, lost verifier, crash — **has no permission verdict and is logged as a refusal carrying `REFUSED-UNCLASSIFIED`.** A request that dies in `TRANSFER` **has a verdict already durably logged** and is completed as `FAILED` under the availability codes. **`REFUSED-UNCLASSIFIED` is where undecided permission lands, and the continuous enumeration above — verified at `BS-L` issuance and again at the opening — is what keeps that visible instead of routine.**
...
| F | **Calibration-bin sealing** | **for BIN CONSTRUCTION: the accepted partition's positions and acceptance flags only (χ-free)** on the genuinely 49,211-row mask; **for the HAND-CHECK ALLOCATION ONLY: additionally the per-object HC stratum index, WHICH IS χ-BEARING** (widened by principal ruling, 2026-08-29 — see §6.3) → writes sealed boundaries, bin labels, and the hand-check allocation | P3, at BS-2f | BS-8p and the realised partition | the sealed boundary and allocation record | any χ-bearing input to bin construction |
...
- **Calibration.** Bin-construction algorithm and the 3 × 9 joint allocation with V3-pred's
  ...
  **AND THE BIN/ALLOCATION SEPARATION IS ENFORCED BY RECOMPUTATION, NOT BY PROMISE.** Both seats
  ...
  sealed boundaries EQUAL that recomputation.** Boundaries constructed from any other input —
  stratum-filtered, stratum-augmented, truncated — differ from the full-set recomputation and are
  refused. **The barrier is verification, not typing, because v9 is frozen; §11 carries the build
  item.**
...
> and a fresh text gate, not a receipt insertion**. On today's count: **BS-2a, BS-2k, `BS-3g`, and the `BS-2v` VOID converter are DESIGN slots.** One of sixteen class-P slots is filled (BS-2m). There are 8 class-E slots. (These class counts are emitted from the table by `tools/prereg_counts.py` and are not to be hand-edited.)
...
- **`BS-3g` sensitivity-gradient receipt — SPECIFIED (principal instruction, 2026-08-29 20:35).**
  **`SLOT_SCHEMA['BS-3g']` is exactly these eighteen fields, no more and no fewer:**
  ...
  **Field definitions, written so a verifier can be implemented by someone who did not write this
  ...
  of γ values evaluated; **this is what makes `n_perturbations` checkable rather than asserted**,
  since a count alone cannot distinguish a full sweep from a favourable handful.
  **THE MANIFEST MUST SPAN THE BOUND, because digesting a list does not make the list adequate
  ...
  verifier **refuses any manifest whose largest `|γ|` is less than `gamma_bound`, and refuses the
  ...
  **The resolution is preregistered, not chosen afterwards.** The manifest is a grid of **stated maximum spacing `Δγ`**, a class-P parameter **CURRENTLY UNSET** and therefore **a further blocker on BS-3g emission**, alongside `n_draws`, `draw_master_seed` and the empty generator set. **`Δγ` is where the strength of the `HELD` claim is set, so it is a preregistered choice and not an implementation detail** — the same reasoning that made `n_draws` the gate's strictness. `kernel_sha256`, `estimator_sha256`, `verifier_sha256` — 64-character lowercase hex
  ...
  and a receipt carrying it **cannot discharge the BS-6 edge**. `gamma_hat`, `sigma_gamma`,
  `gamma_bound` — finite IEEE-754 doubles, decimal, the estimated gradient, its standard error, and
  the bound the invariance test was evaluated against. `invariance_outcome` — exactly one token from
  the closed set `HELD` · `FAILED` · `NOT-EVALUATED`. `n_perturbations` — a non-negative integer, the
  ...
  **what it means**: the maximum of `n_draws` exchangeable draws sits at the `D/(D+1)` expected
  ...
  seed-derivation rule, so draw `i` is reproducible from the master seed alone. `draw_master_seed` —
  ...
  closed token set the run outcomes use; **"worst" is meaningless without the thing it is worse than.** `delta_gamma_max` — a finite positive
  ...
  **(3) THE CATEGORICAL WORST-CASE RULE (GPT56-V63 F5).** Verdicts are **categorical**, so "worst" needs
  ...
  reduction. **There is no ordering: `invariance_outcome` is `HELD` if and only if EVERY cell equals
  `baseline_verdict`, and `FAILED` otherwise.** That is what the control is for — whether any allowed
  ...
  **Independent verifier:** a separately pinned checker that (a0) **refuses unless `mask_sha256`
  ...
  is the subset case this repair exists to catch; (a) recomputes all three module digests from the
  files on disk, (b) recomputes `gamma_hat` and `sigma_gamma` from the frozen kernel and refuses
  ...
  `n_draws` in either direction, and refuses unless `invariance_outcome` is `HELD` exactly when every
  cell of that sequence equals `baseline_verdict`; **it independently refuses unless `n_draws` and
  ...
  receipt discharges the BS-6 edge only if `invariance_outcome` is `HELD`; `FAILED` and
  `NOT-EVALUATED` receipts are valid records that do not fill the slot** (CODEX-V67 F5).** **The stopping rule is exactly `n_draws` draws, all evaluated: not "until it fails",
