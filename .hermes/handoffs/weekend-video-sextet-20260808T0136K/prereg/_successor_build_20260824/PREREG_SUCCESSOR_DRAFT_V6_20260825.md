# PREREGISTRATION DRAFT V6 — LONGO-AMPLITUDE TEST ON A LEVERAGE-CHOSEN FOOTPRINT
...
Hwao, 2026-08-25 13:52 KST. Predecessor status: **DECLINED BY SIGNATURE 2026-08-25**
...
- **`ref/successor_ref_v2.py`, sha256 `dda4436cf0b10710ad9f8a6bb3dff6581c293df31ca8d577b4a2423d33d2dcfd`**
- fixture output **`ref/FIXTURES_V2_20260825.out`, sha256 `4ceb6f94dbebffebdabc18738e156bf4f5db058c3b3c4290df8afc648437e74b`** (ALL FIXTURES PASS)
...
environment. Supersedes V5 (sha `1c283bbf…`) and its ref (sha `67bc4876…`), both retained.
...
**The check (BS-2m, class P, pre-freeze).** `manifest_closure()` computes the required set
from the frozen selection via the frozen cutout planner (named in BS-2m, not reimplemented);
`require_manifest_closure()` compares it against the candidate manifest and **REFUSES on a
difference of even one brick in either direction**, emitting `manifest_count`,
`required_count`, and the missing/extra bricknames into the receipt — so a future gate reads
numbers rather than an assurance. Fixtures `CLOSURE-CATCHES`, `CLOSURE-PASSES` and
`CLOSURE-EXTRA` demonstrate it on the predecessor's exact defect shape (see §8).
...
**Admissible input.** Stage C and the production record accept **only** a `Mask` built by the
typed constructor (validates lengths, uniqueness of (brickid, objid), finite |c| ≤ 1, bin
labels in range, signs exactly ±1; sorts canonically by (brickid, objid); binds a digest and
a provenance `kind`). Bare vectors, parent positions, uniform-sphere inputs, non-sign labels
and duplicate keys are refused — fixture `MASK-REFUSALS` (6/6) and `MASK-CANON-ORDER`.
...
**The power null and its equality contract.** Running 1,000 × 100,000 full permutations per
prefix is not executable at production scale (a gate measured the nested kernel and
extrapolated ≈ 9 hours per prefix). Stage P therefore uses `perm_p_analytic()`: the
permutation null of β̂ has **exact mean 0 and exact variance** `perm_sigma_exact()²`; only
normality is approximated, and the finite-population CLT applies at production N. The
contract is checked, not asserted: fixture **PWR-EQ** compares the analytic decision quantile
against a 20,000-permutation Monte-Carlo quantile and requires agreement within 5%
(measured 0.66%). **Production decisions never use the analytic null** — `decide()` calls the
full `perm_record()`.
...
`decide()` is the **only** producer of a verdict and emits exactly one of four outcomes
(V3-pred F-6 thresholds, applied to Â_L):
...
**Validation battery, carried verbatim in intent from the lapsed build spec** and executed as
fixtures: A = 0 must never return REPRODUCED (`BATTERY-A0`); A = −0.0408 must not return
REPRODUCED (`BATTERY-SIGN`); an injected positive sky at adequate N runs the full path
(`BATTERY-POS-RUNS`); a failed power gate returns INCONCLUSIVE-BY-POWER (`BATTERY-POWER`).
...
- **Calibration.** Bin-construction algorithm and the 3 × 9 joint allocation with V3-pred's
  ...
  `allocate_handcheck()` — proportional, minimum 10 per non-empty cell, largest remainder,
  explicit tie rule, FAIL if minima exceed budget). Numeric boundaries are instantiated and
  sealed at **BS-2f** from positions and flags only. **BS-8f** reports â, σ_a, a_LB, per-bin
  â_b, σ_ab, a_LB_b and the full Cov_a. **Admissibility (`adjudicate_path()`):**
...
Trace V5 → V6, one change per finding: gpt56-V5 F1 & codex-V5 F1 → `inject_signs` per-bin +
Stage-C profile path; gpt56-V5 F2 → `build_plan` raw/retained orchestrator + `RAW-RET`
fixture; gpt56-V5 F3 & codex-V5 F5 → the typed `Mask` + `require_mask`; gpt56-V5 F4 &
codex-V5 F8 → the calibration producer suite + full `Cov_a` + allocation; gpt56-V5 F5 &
codex-V5 F2/F4 → domain-separated `receipt()` envelopes + `require_environment()`;
gpt56-V5 F6 & codex-V5 F3 → §6's honest blind-double scoping; gpt56-V5 F7 → `decide()` +
BS-V; gpt56-V5 F8 & codex-V5 F5 → the analytic power null with the PWR-EQ equality contract;
codex-V5 F6 → fail-closed sigmas + the no-BLAS scalar quadratic form; codex-V5 F7 →
`validate_count_oracle` dual anti-join. Changes NOT traceable to a V5 finding, with their
hypotheses stated: (a) **§2.4 + BS-2m manifest closure** — Duho's direction 2026-08-25 after
the Trio report; hypothesis: a closure check that reports counts would have caught the
60,308-vs-60,310 gap at freeze rather than at the end of the run; (b) **§2.1 bound
choice-point** — Duho's direction; hypothesis: specifying both branches in frozen text lets
the Sep-5 data decision resolve without reopening the text; (c) **§1 citation anchors and
published sign** — external-practice adoption; hypothesis: a verified bibcode/DOI and an
explicit published-sign statement prevent a directional inversion by a later reader.

Next: adversarial gates on THIS text plus the pinned code (gpt-5.6-sol + codex, fresh
dispatch, all three shas pinned in the brief). Then freeze candidate → Duho's signature →
444 + git → class E. **Undecided and untouched by this draft:** the methods-note question
and the strata question.
