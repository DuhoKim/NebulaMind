# PREREGISTRATION DRAFT V70 — LONGO-AMPLITUDE TEST ON A LEVERAGE-CHOSEN FOOTPRINT
...
- **`BS-3g` sensitivity-gradient receipt — SPECIFIED (principal instruction, 2026-08-29 20:35).**
  ...
  **THE SHAPE CHOICE UNDERNEATH IS THE PRINCIPAL'S, flagged before it becomes a third-round object:**
  ...
  BS-3g parameter sitting** (`DECISIONS_FOR_DUHO.md`). `invariance_outcome` — exactly one token from
  ...
  **And the baseline has the same outside-origin obligation as the bound (CODEX-V69 F5): the manifest
  MUST contain the point `γ = 0`, and the verifier RECOMPUTES `baseline_verdict` by replaying the
  γ = 0 column of the draw × perturbation matrix and refuses any receipt whose stated baseline
  differs.** A producer-chosen baseline lets `HELD` be measured against a verdict nothing produced —
  `require_authorization`'s shape at the reference point instead of the bound. `delta_gamma_max` — a finite positive
