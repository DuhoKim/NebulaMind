# PREREGISTRATION DRAFT V72 — LONGO-AMPLITUDE TEST ON A LEVERAGE-CHOSEN FOOTPRINT
...
**REQUEST LIFECYCLE — Row B decides permission durably, and every state has one terminal treatment (repairing CODEX-VOCAB F1 and GPT56-VOCAB F1, both HIGH).** The derivation above assumed permission is *"evaluated before the attempt."* **The covenant nowhere imposed that ordering, and it is false in two ways** — a permission check can die before deciding, and a write's conformance cannot be judged before its payload is read. **Asserting the ordering is what failed; stating the machine is the repair.**
  ...
  **THE GUARANTEE — QUOTED FROM THE SPEC BY DIGEST, NOT RESTATED (GPT56-V71 F1, CODEX-V71 F1).** V71 amended the spec's G2/G3 and left this block carrying the V70-broken wording — the exact one-round death the derivation claim was warned about, arrived in mirror form: the warning said do not fix the draft and leave the spec behind; V71 fixed the spec and left the draft behind. **The derivation is now a CHECK, not a claim**: `tools/lifecycle_derivation_check.py` fails when the pin below goes stale or any quoted invariant body diverges from the spec's bytes — a spec edit breaks the pin until the draft re-pins, which forces the re-derivation instead of permitting the drift. **This class of finding is now impossible rather than unlikely, which is the difference between a predicate and a name.**
  lifecycle-spec: sha256 `1c499dbcb9be30f959722dc76b84379da25c6842e640321ca9e1e1adf2a8df3c`
  ...
  **G3 — **One event per touch** — and **every event is either exactly one touch's event or a refusal's event; no event is both, and no event is neither** (V70's wording said "one touch per event", which contradicted refusal events outright — GPT56-V70 F1, CODEX-V70 F1, the round's first finding against this spec)**
  ...
  **N2 — **A request that dies before any commit is invisible****
...
**THE STRING RULE, global because the seats have been finding this surface by surface (GPT56-V71 F4,
CODEX-V71 F4 — the explanation prose was the refusal-reason leak, one surface over): every string
field in every non-χ-bearing artifact either carries a value from a CLOSED VOCABULARY declared where
the field is defined, or is a BOUNDED ENCODING (hex digest, decimal integer in stated range,
identifier from a declared set). THERE IS NO THIRD KIND pre-unblinding.** Free prose is not a field
type on any surface a pre-unblinding verifier reads, because a sentence-constraint on free prose is a
guard the seats have beaten all night, while a closed vocabulary is a property. **One field existed
in violation when this rule was written — the explanation artifact's prose, three revisions old and
this lane's own construction, never ruled — and it is redesigned below rather than filed, since
tightening an unruled artifact of mine to obey the rule changes nothing the principal decided. Any
FUTURE field the rule would constrain is listed and filed before it is built.**
...
- **`BS-3g` sensitivity-gradient receipt — SPECIFIED (principal instruction, 2026-08-29 20:35).**
  **`SLOT_SCHEMA['BS-3g']` is exactly these nineteen fields, no more and no fewer:**
  ...
  **`counterfactual_path_sha256` pins the module; every verdict cell must be produced under the
  PRODUCTION permutation contract — `n_perm = 100,000`, the frozen stage/trial/address parameters §3
  ...
  **Independent verifier:** a separately pinned checker that (a0) **refuses unless `mask_sha256`
  ...
  is the subset case this repair exists to catch; (a) recomputes **all four** module digests from the
  files on disk — kernel, estimator, verifier, **and `counterfactual_path_sha256`, added at V71 while this clause still said three (GPT56-V71 F6)** —, (b) recomputes `gamma_hat` and `sigma_gamma` from the frozen kernel and refuses
  ...
  must carry `HELD` if every cell `(i, j)` equals its draw's `(i, 0)` and `FAILED` otherwise — a fully
...
- **Enumeration verifier (`gates/enumeration_verifier.py`) — REQUIRED, DOES NOT EXIST (CODEX-V68 F5
  ...
  emission, dangling reference, or second `EXPLAINED`. Consulted at **five gates, each over the chain as it then stands: `BS-L` issuance, the lock
