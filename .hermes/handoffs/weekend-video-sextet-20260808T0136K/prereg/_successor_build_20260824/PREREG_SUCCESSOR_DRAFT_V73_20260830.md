# PREREGISTRATION DRAFT V73 — LONGO-AMPLITUDE TEST ON A LEVERAGE-CHOSEN FOOTPRINT
...
blocks themselves** — the generator extracts every field token from the declared field lists, so it
...
- **`BS-3g` sensitivity-gradient receipt — SPECIFIED (principal instruction, 2026-08-29 20:35).**
  ...
  **`counterfactual_path_sha256` pins the wrapper — and the wrapper is not what computes (GPT56-V72
  F5): the verifier must ALSO assert, in-process at replay time, that the `successor_ref_v9` module
  object actually imported hashes to the frozen `6a9abbbd…`, because pinning the caller of a
  computation while the computation's module is resolved at import time pins a name. Every verdict
