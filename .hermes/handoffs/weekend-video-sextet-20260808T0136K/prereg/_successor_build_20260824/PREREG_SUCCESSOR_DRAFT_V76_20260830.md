# PREREGISTRATION DRAFT V76 — LONGO-AMPLITUDE TEST ON A LEVERAGE-CHOSEN FOOTPRINT
...
**REQUEST LIFECYCLE — Row B decides permission durably, and every state has one terminal treatment (repairing CODEX-VOCAB F1 and GPT56-VOCAB F1, both HIGH).** The derivation above assumed permission is *"evaluated before the attempt."* **The covenant nowhere imposed that ordering, and it is false in two ways** — a permission check can die before deciding, and a write's conformance cannot be judged before its payload is read. **Asserting the ordering is what failed; stating the machine is the repair.**
  ...
  **THE GUARANTEE — QUOTED FROM THE SPEC BY DIGEST, NOT RESTATED (GPT56-V71 F1, CODEX-V71 F1).** V71 amended the spec's G2/G3 and left this block carrying the V70-broken wording — the exact one-round death the derivation claim was warned about, arrived in mirror form: the warning said do not fix the draft and leave the spec behind; V71 fixed the spec and left the draft behind. **The derivation is now a CHECK, not a claim**: `tools/lifecycle_derivation_check.py` fails when the pin below goes stale or any quoted invariant body diverges from the spec's bytes — a spec edit breaks the pin until the draft re-pins, which forces the re-derivation instead of permitting the drift. **This class of finding is now impossible rather than unlikely, which is the difference between a predicate and a name.**
  lifecycle-spec: sha256 `22c65dcfe4272b8e2e69d30746275c05b75c06a855157b2db0e5b2c8498c2c27`
...
blocks themselves AND from frozen v9's `SLOT_SCHEMA`, its receipt ENVELOPE constructor, the enumerator signature, the nine non-slot artifact classes and **the environment's six leaf fields — 145 fields** (V74's 123 omitted the envelope/environment fields v9 wraps every receipt in, the signature itself, and the non-slot classes — GPT56-V74 F1, CODEX-V74 F1; and the signature is constrained to a **deterministic scheme mandated at BS-2k**, because a valid ECDSA signature still carries a nonce — a covert channel wearing a validity proof). **V75's registry collapsed the environment into one falsely-closed row — the six leaves are now classified honestly, and three of them (`python`, `platform`, `machine`) are RECORDED-UNPINNED: interpreter-supplied text refused by nothing, a real residual surface named rather than dressed as closed (GPT56-V75 F1, CODEX-V75 F2). And the nine non-slot rows stop being pseudo-fields (GPT56-V75 F2, CODEX-V75 F1): three classes are inventoried field-by-field in the registry itself, six are honest `SCHEMA-PENDING` stubs whose fields are unenumerable until their defining slots fill — a stub that says so, not a constraint it does not have.** The registry records its generating draft and **the lint blocks when that is not the draft under lint** (CODEX-V75 F3): the first registry
...
- **`BS-3g` sensitivity-gradient receipt — SPECIFIED (principal instruction, 2026-08-29 20:35).**
  ...
  **⛔ THE DRAW DISCIPLINE IS UNSTABLE-PENDING-RULING (declared 2026-08-30 01:5x, after FIVE
  consecutive rounds with a finding against this object — V68 F7, V69 F6, V70 F5/F8, V71 F5/F6, V72
  F1 — past the three-round threshold).** The reason is structural, not carelessness to be patched
  ...
  **`counterfactual_path_sha256` pins the wrapper — and the wrapper is not what computes (GPT56-V72
  ...
  `PYTHONSTARTUP`, path-injected import hooks all execute first). The replay process is launched under **the interpreter's ABSOLUTE PATH as recorded in BS-7p's
  `environment` field — a bare `python` is PATH-rebindable before isolation begins (CODEX-V75 F4)** —
  ...
  The dependency roots and their contents are part of the frozen environment `require_environment`
  checks** — the same record that pins `numpy 1.26.4` now names where it loads from. It hashes the
  pinned file, imports it, computes. **And the trust boundary is stated
