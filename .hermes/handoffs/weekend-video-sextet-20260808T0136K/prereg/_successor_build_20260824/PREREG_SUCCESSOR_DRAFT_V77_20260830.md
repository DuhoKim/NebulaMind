# PREREGISTRATION DRAFT V77 — LONGO-AMPLITUDE TEST ON A LEVERAGE-CHOSEN FOOTPRINT
...
- **`BS-3g` sensitivity-gradient receipt — SPECIFIED (principal instruction, 2026-08-29 20:35).**
  ...
  **`counterfactual_path_sha256` pins the wrapper — and the wrapper is not what computes (GPT56-V72
  ...
  as an ordered list of (path, digest) pairs — under the canonical field-order encoding, and a
  separately pinned verifier recomputes the interpreter and root digests from disk and refuses
  mismatch.** The roots join the frozen environment `require_environment` checks — the record that
