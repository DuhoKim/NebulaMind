# PREREGISTRATION DRAFT V79 — LONGO-AMPLITUDE TEST ON A LEVERAGE-CHOSEN FOOTPRINT
...
- **`BS-3g` sensitivity-gradient receipt — SPECIFIED (principal instruction, 2026-08-29 20:35).**
  ...
  **`counterfactual_path_sha256` pins the wrapper — and the wrapper is not what computes (GPT56-V72
  ...
  native objects — and REFUSES its own result if any loaded object is outside the manifest.** The
  frozen reference and the counterfactual path contain **no dynamic-load constructs — no `ctypes`,
  no `dlopen`, no `importlib` dynamic use — auditable once by AST over the frozen bytes**, so a
  late load is a violation, never a need. **What remains above the manifest is the linker ITSELF
