# PREREGISTRATION DRAFT V81 — LONGO-AMPLITUDE TEST ON A LEVERAGE-CHOSEN FOOTPRINT
...
- **`BS-3g` sensitivity-gradient receipt — SPECIFIED (principal instruction, 2026-08-29 20:35).**
  ...
  **`counterfactual_path_sha256` pins the wrapper — and the wrapper is not what computes (GPT56-V72
  ...
  `PYTHONSTARTUP`, path-injected import hooks all execute first). **OPTIMIZATION IS FORBIDDEN AND CHECKED (GPT56-V80 F5, CODEX-V80 F2): `-O` survives `-I -S` and
  strips `assert` statements — and v9's verdict path carries a LOAD-BEARING assert, the
  calibration-stability check at L1622, which `-O` would remove silently. The launch contract
  forbids `-O`/`-OO`, the cleared environment kills `PYTHONOPTIMIZE`, and the harness verifies
  `sys.flags.optimize == 0` in-process before computing — a recorded v9 limit, since a frozen file
  cannot trade its assert for a raise.** The replay process is launched under **the interpreter's
