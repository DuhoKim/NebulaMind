# PREREGISTRATION DRAFT V91 — LONGO-AMPLITUDE TEST ON A LEVERAGE-CHOSEN FOOTPRINT
...
**REQUEST LIFECYCLE — Row B decides permission durably, and every state has one terminal treatment (repairing CODEX-VOCAB F1 and GPT56-VOCAB F1, both HIGH).** The derivation above assumed permission is *"evaluated before the attempt."* **The covenant nowhere imposed that ordering, and it is false in two ways** — a permission check can die before deciding, and a write's conformance cannot be judged before its payload is read. **Asserting the ordering is what failed; stating the machine is the repair.**
  ...
  **THE GUARANTEE — QUOTED FROM THE SPEC BY DIGEST, NOT RESTATED (GPT56-V71 F1, CODEX-V71 F1).** V71 amended the spec's G2/G3 and left this block carrying the V70-broken wording — the exact one-round death the derivation claim was warned about, arrived in mirror form: the warning said do not fix the draft and leave the spec behind; V71 fixed the spec and left the draft behind. **The derivation is now a CHECK, not a claim**: `tools/lifecycle_derivation_check.py` fails when the pin below goes stale or any quoted invariant body diverges from the spec's bytes — a spec edit breaks the pin until the draft re-pins, which forces the re-derivation instead of permitting the drift. **This class of finding is now impossible rather than unlikely, which is the difference between a predicate and a name.**
  lifecycle-spec: sha256 `7e59096d78e3597c5bfed9097d17042fb54baf30fe245a14e7bc75c5e39eccb6`
...
(ii-b) **the ARRIVAL event under its own authenticated closed schema — `(kind=ARRIVAL, timestamp, boot_epoch, monotonic_reading, row, operation, object identity, request_key, running chain digest)`** — identities, the same CLOSED operation set as item (ii), an ISO-8601 timestamp (human-facing — NEVER the deadline basis), **the authenticated clock pair `(boot_epoch, monotonic_reading)` — Row B's own monotonic reading at append, the durable start §3b's deadline is measured from (GPT56-V89 F1, CODEX-V89 F2); `boot_epoch` a decimal integer in [0, 10^6] — the BS-2k restart counter, a sequence number owing nothing to any clock — and `monotonic_reading` a decimal integer nanosecond count in [0, 2^63 − 1], both under §3b's chain-order monotonicity invariants (GPT56-V90 F2/F3, CODEX-V90 F1: the pair was authenticated but unbounded, and signatures prove authorship, not monotonicity)** — and a bounded-decimal chain position; **never payload bytes, never a payload digest, never an outcome or refusal field** — value domains bound to BS-2k exactly as item (ii)'s are (GPT56-V88 F2, CODEX-V88 F1: the ruled second event class was defined at Row B, BELOW this exhaustive list, and so defaulted χ-bearing by the default rule — the list is extended AT the list, which is where exhaustiveness lives; the registry's `arrival.*` rows now cite THIS item as their source, not v9's slot schema, which never contained an arrival); 
(iii) the producer checksum list (§2.5), exclusively for source images; 
(iv) fixtures and their transcripts, synthetic by construction; **(iv-b) the enumeration entries, their continuation segment, and their signed explanation artifacts (CODEX-V70 F3 — the pre-unblinding enumeration verifier must read these, and the closed-list default would have made them χ-bearing and unreadable): entries carry only positions, digests, closed tokens and signatures; explanations are bound by the same REBUILT principle as refusal reasons — storage state allowed, **nothing content-derived ever**, and an explanation describing measured properties is itself a violation to be refused;** **(iv-c) the binding→key map — sealed segment and continuation entries alike — under its closed schema `(request_key, decision chain_position, decision event_digest, signature)`: a bounded-decimal key, chain positions, digests and a provisioned-keypair signature, never a payload byte, never an outcome field (CODEX-V90 F2: the pre-opening verifier consumes this map, and an artifact off this exhaustive list is χ-bearing by default — the consumer was unlawful by the covenant's own rule until the map was DECLARED. This declares what the map already carried; it does not widen what it may carry, so the ruled refusal vocabulary is untouched);** 
...
- **`BS-3g` sensitivity-gradient receipt — SPECIFIED (principal instruction, 2026-08-29 20:35).**
  ...
  **The resolution is preregistered, not chosen afterwards.** The manifest is a grid committed as a STEP COUNT — **`n_steps = 50`, even** (`ref/DRAW_MECHANICS_COMMIT_20260830.md`, AMENDMENT 2) — over the ratified symmetric endpoints ±Γ, with **Δγ DERIVED = 2Γ/n_steps** (0.01 at the proposed Γ = 0.25), `n_perturbations = n_steps + 1 = 51` including the γ=0 baseline at `j₀ = 25`, verified exactly zero. (This sentence said Δγ was "CURRENTLY UNSET" beside an "empty generator set" for one revision after the sitting committed every draw value — GPT56-V89 F4, CODEX-V89 F3: live pre-sitting prose whose paraphrase the literal sweep's registry-scoped tokens never searched; the tokens now cover the draft, and the superseded phrases are quoted here dead.) **The grid's fineness is where the strength of the `HELD` claim is set, so it is a preregistered commitment and not an implementation detail** — the same reasoning that made `n_draws` the gate's strictness. `kernel_sha256`, `estimator_sha256`, `verifier_sha256` — 64-character lowercase hex
