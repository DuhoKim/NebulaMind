# PREREGISTRATION DRAFT V9 — LONGO-AMPLITUDE TEST ON A LEVERAGE-CHOSEN FOOTPRINT
...
Hwao, 2026-08-25 16:50 KST. Supersedes V8 (sha `faea9047…`, REFUSED; kept).
...
- **`ref/successor_ref_v4.py`, sha256 `ffea5b6c58956c1f6c2e44939113f5170e459e566d132e8e3f69d117344e657b`**
- fixture output **`ref/FIXTURES_V4_20260825.out`, sha256 `c5a4b95b554e16a7aea99213b06f21b18868e701c5a9682e8d3b325a18b10e72`** (34 checks, ALL FIXTURES PASS)
...
environment. Supersedes V5–V8 and refs v1–v3, all retained for provenance.
...
- Count oracle: universe **366,912** bricks, **270,577** with objects, **96,335** zero rows
  ...
  0.4452.
- Selection **through the frozen reduction pass**: **6,445 bricks**, **65,060 raw objects**,
...
**V8 → V9.** Round 7 returned one verdict (codex, REFUSED); the second seat was declined by a
provider content classifier, so V9 has had only one engine's review and does not pretend
otherwise. Codex reproduced the real Stage-P result exactly (997/1000, 77/77, PASS) and then
found two defects in how the geometry was produced. Both were verified here and both are
repaired:

| finding | change |
|---|---|
| the shipped planner is not the frozen planner, and on the real brick table it returns only the home brick for both historical objects | reimplementation RETIRED (raises); BS-2m binds to the frozen `plan_candidate_bricks` with its pinned adapter; closure fixtures moved onto the real brick table and the real historical objects |
| the reported real selection is a greedy prefix, not `local_pass()` output (brick 155487 was removable) | selection re-run through the frozen reduction: 6,446 → **6,445** bricks; the fast reduction is proven equal to `local_pass` on 30 random cases |

Open findings from that report, **not** claimed closed here: the closure caller-trust check
still compares a caller-supplied digest string rather than recomputing it from the artifact;
Stage-C mask-digest chronology; the receipt layer's round-trip into its consumers; HC-1H
nine-stratum population weights and the boundary-tie statement; the release availability probe;
and the integrated power battery. These are the next round's work.
...
`greedy_ledger()`/`local_pass()` do not scale to the production universe.

Next: both gate seats on THIS text, the corrected code, and the real-geometry receipt.
