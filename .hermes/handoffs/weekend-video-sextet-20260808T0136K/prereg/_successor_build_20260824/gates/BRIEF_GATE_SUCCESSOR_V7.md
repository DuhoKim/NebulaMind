# ADVERSARIAL GATE BRIEF — successor preregistration V7 (round 6)

You are an adversarial gate. REFUSE if you can. Rounds 1–5 refused V2–V6; V7 claims to
...
- `../PREREG_SUCCESSOR_DRAFT_V7_20260825.md` =
  `f15b0b4dad9d5c565969e18aa8dee200cc8a80c9b7f817e68c8b3e239f800286`
- `../ref/successor_ref_v3.py` =
  `b89c21288935a026f882d2f417c68d82e12934beae1b00e72474186f03d74e90`
- `../ref/FIXTURES_V3_20260825.out` =
  `445e32c8c573423729a196cfd35f70faf26dc5073ca3295d89219c756142b33a`
Any mismatch: STOP, report, review nothing.

## Context (read-only)

`GATE_GPT56_SUCCESSOR_V6.md`, `GATE_CODEX_SUCCESSOR_V6.md` (this directory — V7 §10 traces
...
1. **Run the code.** `python3 ../ref/successor_ref_v3.py --fixtures` and compare to the pinned
...
`GATE_<YOURSEAT>_SUCCESSOR_V7.md`: pinned shas as computed; your environment; numbered
...
## Round-6 specific instructions

1. **Re-run your own V6 attacks.** Your V6 report's executed attacks (lazy planner maps,
   FIXTURE-mask-to-production, monkeypatched guards, `_perm`/`n_perm` bypass, reduced-N
   verdicts, float/negative count tables, the 17-raw/16-retained branch, the HC estimator and
   stratum floor, the tie rule) are the first thing to try again. If any still succeeds, that
   is a REFUSAL and worth more than a new finding.
2. **Attack the measured power null specifically.** V7 replaced a normal-tail critical value
   after its own fixture measured it anti-conservative on polar geometry. Check: is one
   measured null per prefix legitimate across 1,000 trials; does the deflation actually cover
   the measured multiset dependence (PWR-Z-STABLE reports ~35% relative tail-mass spread — is
   a 1% deflation adequate against that, and is tail mass the right statistic to bound it?);
   can a Stage-P pass still overstate true power at the 962/1000 decision boundary?
3. **§10 names three things as NOT closed** (clean-room spec, BS-9 input schema, BS-V lock).
   Do not count those as new findings — check instead whether anything ELSE is claimed closed
   that is not.
