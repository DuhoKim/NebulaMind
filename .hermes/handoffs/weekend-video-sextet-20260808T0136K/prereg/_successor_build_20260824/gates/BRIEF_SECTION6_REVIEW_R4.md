# REFEREE BRIEF — §6, fourth pass. Test the mechanisms, not the verdict labels

`SECTION6_DRAFT_AGY_R4.md` is a proposed replacement for §6 of the preregistration, the blinding
...
## What changed about this round

Last round you found that three of the author's nine "Accepted" verdicts had **renamed** a
problem rather than repaired it — an opening authorization with no canonical body, a final
checkpoint that was not a named receipt, archive checkpoints with no transition rule. That was
the most useful thing either of you did, because a nominal acceptance reads as settled and a
refusal stays visible.

So this pass was drafted under a rule: **renaming counts as refusing.** Every finding had to be
either REPAIR — naming the artifact, its fields, its producer, its verifier, and what fails if it
is absent — or REFUSE, with what would have to exist first. A bare "Accepted" was banned.

**All six findings came back REPAIR. None was refused.** I check that mechanically and it is
clean of bare acceptances, but a mechanism can be named and still be wrong, incomplete, or
inconsistent with a clause elsewhere. **Test the mechanisms, not the labels.** The rule I imposed
makes a nominal repair harder to write; it does not make one impossible, and an author under a
no-renaming rule has an incentive to invent a mechanism rather than refuse.

The repair I would look at first is finding 3, because it was a contradiction rather than
plumbing: the acceptance recompute was declared sign-blind while authorised to read receipts
carrying signs. The draft splits row D's output so a separate authenticated projection carries
attempt identity, checksums, shape, completion status and the confidence quantity, with no sign
or amplitude, and the recompute reads only that. **Is that projection actually incapable of
carrying an outcome, or only declared to be?**
...
Write `SECTION6_REVIEW_R4_<YOURSEAT>.md` here. Numbered findings with severity, the clause or table
