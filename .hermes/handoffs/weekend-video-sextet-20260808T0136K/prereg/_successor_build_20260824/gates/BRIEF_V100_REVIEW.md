# BRIEF — V100 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V100_20260830.md`
**sha256 `8cdc89cc495ca3803cc594a6314c5c54bf3318a21953a8c5f042b82f877f6b43`**
...
**Last refereed draft: V99** `55062e8ae7696353` — **NOT CLEAR from both seats** (GPT56 8,
...
## What V100 changed — the terminal signature filed, the preimages canonical

**V99 was NOT CLEAR ×2** (GPT56 8, CODEX 6).

- **The successor-as-waypoint answer did not survive and the text says so** (your F1 pair). The
  honest resolution — a TERMINAL HUMAN SIGNATURE over the disclosure pass record's digest — is
  FILED as the recommended ruling; until ruled, the suffix stands as machine testimony with NO
  closing waypoint, in exactly those words.
- **The spec's reader row is scoped** (GPT56 F2): custody from committed bindings, never the
  chain; ARRIVAL DISCOVERY from the chain as designed. (The draft was already scoped.)
- **The root schema's nested preimages are canonical** (F3/F4 + CODEX F4): clock_record under
  field framing; both sealed-set digests count-prefixed, sorted, cut-bounded.
- **My V99 stub edit stamped the lockcp schema onto all seven classes sharing one declaration**
  (F5 pair) — split; six keep their honest stub. **The cut is contiguity plus unrepeatability**
  (F6/F3 pair). **Same-chain resumption is semantics, not a barrier** (GPT56 F7): the halt
  receipt remains in force forever; a continuing resumption is a NEW chain.
- **The decoder is a pinned build item** (GPT56 F8): `gates/canonical_decoder.py`,
  REQUIRED-DOES-NOT-EXIST, provenance fixtures including dtype=object refused.
- **`require_authorization`'s raises are CALLER** (CODEX F5; 56/32). **The activation list
  gains the demonstrated phrasing AND the guard's claim is demoted in its own comment** (CODEX
  F6): a named-shape tripwire, not a semantic guarantee; 40 controls.
...
- **ATTACK THE FILED-STATE BOUNDARY. Highest value here.** The suffix now SAYS it has no
  closing waypoint pending the principal's ruling. **Break the interim**: name what a
  compromised enumerator key does in the unruled window that the eventual terminal signature
  cannot retroactively catch (the signature signs the digest the key PRESENTS — does the
  principal's act verify anything, or notarize testimony?); if it merely notarizes, say what
  MORE the ruling should require so the filed recommendation is complete rather than
  comforting.
- **Attack the canonical preimages.** Count-prefixed, sorted, cut-bounded. **Break them**: an
  entry at the signed cut position exactly (≤ vs < — which side?); two entries sharing a
  chain_position; a bindmap request_key repeating across epochs after a declared gap.
- **Attack contiguity-plus-unrepeatability.** **Break it**: the issuance commit aborts mid-run
  and retries — if aborts leave no events, is the first episode invisible and the rule vacuous;
  if they leave events, the retry is 'malformed' by the rule's own words. Which?
- **Attack the pinned decoder item.** **Break the spec-of-a-future-thing**: which named fixture
  is underspecified enough that a passing decoder still emits attacker-behaviored objects
  (huge-int DoS, crafted strides, str subclass)?
- **Attack the demoted guard's residue.** Find the draft sentence still citing the lint as
  'protecting' or 'enforcing' the vocabulary rather than stating it.
...
Write to `gates/V100_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
