# BRIEF — V101 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V101_20260830.md`
**sha256 `9a5c612ebb02ad4f3427b756eddf753d24cd3b2efe2600a598ff4644dd602907`**
...
**Last refereed draft: V100** `8cdc89cc495ca380` — **NOT CLEAR from both seats** (GPT56 5,
...
## What V101 changed — recomputed not notarized, grammar not fixtures, one prefix rule

**V100 was NOT CLEAR ×2** (GPT56 5, CODEX 7 — severity softening; every finding on the newest
machinery).

- **The terminal-signature filing is strengthened** (your F1 pair): the ceremony RECOMPUTES the
  terminal head from chain bytes under a pinned verifier; the principal signs (recomputed head,
  transcript digest). Signing the presented digest would notarize testimony.
- **The decoder's definition is the STATED GRAMMAR with reject-by-default** (F5/F4 pair):
  unknown fields, out-of-grammar bytes, trailing bytes, size bounds — refused; fixtures demoted
  to corroboration.
- **Domain separation, corpus-wide in one pass** (F4/F5 pair): every canonical body begins
  `NMPR1:<kind>` + newline, fourteen-kind closed set — cross-kind collisions dead.
- **Termination is a RUN state checked AT RECEIPT** (CODEX F2): Row B refuses every subsequent
  request once any TERMINATED-family receipt exists — the relabel escape dies because identity
  is never consulted.
- **The passrec schema divergence is fixed and CHECKED** (GPT56 F2): L08 — shared closed-schema
  tuples must match across the files.
- **BS-SI/Row D2 has its build item** (GPT56 F3): runner + independent verifier,
  REQUIRED-DO-NOT-EXIST, three fixtures. **R01 membership requires the canonical backticked
  spelling** (CODEX F6). **The V99→V100 trace row is in** (CODEX F7).
...
- **ATTACK THE RECOMPUTATION CEREMONY. Highest value here.** The principal signs a recomputed
  head. **Break the independence**: the pinned verifier binary — pinned BY WHOM and launched
  from WHOSE environment at signing time (if the enumerator's environment launches it, the
  recomputation inherits the compromise); the transcript digest — who checks the transcript
  says what the recomputation did?
- **Attack the domain-tag set.** Fourteen kinds, closed. **Break it**: a preimage that DIGESTS
  today without a tag (the running chain digest itself?); the boundary where pre-rule bytes
  meet the new rule mid-corpus.
- **Attack reject-by-default.** **Break it**: an input the grammar ACCEPTS whose decoded object
  is still attacker-shaped — the size bounds are named but not valued; where do the values
  live?
- **Attack the receipt-time stop rule.** **Break it**: WHERE does Row B read the TERMINATED
  receipt from, pre-gate; a receipt landing while a request is mid-processing; the check's cost
  on the hot path and whether THAT read is logged.
- **Attack L08.** Only `(gate, …)` shapes match. **Say which other shared schema deserves the
  same check and whether the general rule is writable.**
...
Write to `gates/V101_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
