# BRIEF — V102 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V102_20260830.md`
**sha256 `a990cbc7d494cf3d6ca1596771b83e15a03ada325b37ee84e21ef6f165e2f85a`**
...
**Last refereed draft: V101** `9a5c612ebb02ad4f` — **NOT CLEAR from both seats** (GPT56 6,
...
## What V102 changed — one construction for termination, generated kinds, the ledger

**V101 was NOT CLEAR ×2** (GPT56 6, CODEX 8) — and the round surfaced a PROCESS defect this
build owns: V101 repaired from the coordinator's digest without enumerating the raw blocks,
missing three V100 findings while claiming completeness. `gates/gen_repair_ledger.py` now makes
the repair list regenerable — findings in, dispositions out, `--check` diffable, undisposed
fatal — and this brief QUOTES it instead of asserting.

- **THE TERMINATION CHECKPOINT — one construction** (GPT56 F3 + CODEX F2 + the V100 new-chain
  escape): termination is a Row-B state transition appended to the log as a checkpoint-family
  record; Row B DRAINS every PENDING arrival (full-drain bounded) then appends, so every request
  is strictly before or after — no race, no in-flight survivor, no hot-path store read. A new
  chain under a freeze holding a TERMINATED receipt is malformed at opening. The review window
  is an event interval on the log.
- **The terminal ceremony is ROOTED** (both F1): verifier digest printed in this preregistration
  and BS-2k; the launcher is the principal's own environment per a ceremony-script design
  artifact; he signs the domain-tagged terminal-review body after reading its transcript.
- **The kind set is GENERATOR OUTPUT** (GPT56 F2 + CODEX F3): `ref/gen_domain_kinds.py`,
  declared kind→probe pairs, strangers and declared-only fatal, count as output, `--check`;
  frozen v9's envelope preimages EXCLUDED BY NAME with the reason. No hand-written exhaustive
  claims remain in this corpus's new text.
- **The decimal alternation closes per branch** (`-0.5.6` no longer matches — GPT56 F4).
  **Bounds are VALUED as productions** including the closed ndarray contract (GPT56 F5 + CODEX
  F6). **L08 is generalized** to all shared backticked tuples keyed by first two fields, with a
  control — and on this build's own battery it immediately found the terminal-review tuple
  divergence and a keying bug, both fixed before you read this (GPT56 F6 + CODEX F7).
- **R01 strips HTML comments for membership** (CODEX F8). **Sealed-set canonicalization is
  COMPLETE** (CODEX F4 + the V100 miss): decimal-ASCII count prefix + newline after the tag,
  duplicates REFUSED not sorted, membership bound by the receipt's own `chain_head_position`.
  **Row B's key-uniqueness claim is scoped as-of-the-anchors** (CODEX F5, twice found while
  absolute).
...
- **ATTACK THE TERMINATION CHECKPOINT. Highest value here.** Drain-then-append by one writer.
  **Break it**: the drain itself emits refusal commits — can a drain refusal's processing spawn
  work (an enumeration entry, a map entry) that lands AFTER the checkpoint?; Row B dies
  MID-DRAIN (restart opens a new epoch — does the half-drained termination survive, repeat, or
  vanish?); the checkpoint-family framing (is a termination checkpoint clock-BEARING, and do
  the clock invariants now quantify over it?).
- **Attack the ceremony script.** The principal launches on his own environment. **Break it**:
  what installs the pinned verifier THERE (the download/copy path is the new supply chain);
  the transcript he reads — generated by the binary he's verifying, self-reporting; say what
  the script must make him CHECK rather than read.
- **Attack the generated kinds.** Probe phrases prove definition sites exist. **Break it**: a
  kind whose probe phrase survives while its BODY definition changed shape (probe alive,
  schema dead); the frozen exclusions — is the running chain digest really kind-safe by
  position alone against a crafted cross-kind preimage?
- **Attack the repair ledger.** Dispositions are hand-declared per round. **Break it**: a
  disposition that says REPAIRED while the repair text misses the finding's point — the ledger
  checks presence, not adequacy; say whether adequacy is checkable or the round remains its
  only control (and if so, whether the ledger should say that on its face).
- **Attack the drained refusals' codes.** Every PENDING arrival gets a refusal at drain —
  under WHICH code (deadline refusals for unexpired arrivals are false; catch-alls flood the
  enumeration machinery AT termination, when nobody will re-derive the vocabulary). Which is
  it, and does the choice leak?
...
Write to `gates/V102_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
