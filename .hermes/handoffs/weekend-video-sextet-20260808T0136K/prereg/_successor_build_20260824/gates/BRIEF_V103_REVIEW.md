# BRIEF — V103 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V103_20260830.md`
**sha256 `a9506e37e85e593da13e8aa84a4a64fcc5a6a269509748445b5c1627da407814`**
...
**Last refereed draft: V102** `a990cbc7d494cf3d` — **NOT CLEAR from both seats** (GPT56 7,
...
## What V103 changed — the drain derived from properties, the generators made real

**V102 was NOT CLEAR ×2** (GPT56 7, CODEX 8) — the drain attacked from every side, both new
generators found hollow, and a STANDING RULE issued: no generator's output is citable without a
seeded positive control and a deletion probe.

- **Spec §3c states the termination PROPERTIES as T-rows and derives the construction** (your
  F1–F4/F6 set): **T1** admission closes at DRAIN-START (the receipt transition itself closes;
  post-drain frames are wire residue); **T2** DRAIN-OPEN is a chain state and recovery opens
  DRAINING epochs — lawful, constrained to `REFUSED-LOCK-OR-CEREMONY-STATE` refusals of the
  remaining PENDING arrivals, idempotent, never ordinary admission (the V102 deadlock — the
  terminal receipt making every recovery opening malformed — dissolves); **T3** the terminal
  checkpoint is a first-class artifact (closed schema, value domains, exhaustive-list item
  (ii-f) with the drain-start record, in the kind set, under the clock pass) and **THE LOG IS
  THE ROOT**, said in the artifact set. The draft quotes T1–T3 label-bound; **the derivation
  checker binds the T family**, so its scope no longer lags new spec sections.
- **The new-chain sentence that authorised the escape is dead** (GPT56 F4 + CODEX F1); the rule
  is §3c's, stated once.
- **Both generators rebuilt to the standing rule**: `gen_domain_kinds` v2 enumerates the
  registry's 61 digest-ref rows as REAL sites, each covered TAGGED/FROZEN/RAW or fatal — 18
  genuinely unmapped sites surfaced on its first run and were curated; `gen_repair_ledger` v2
  discovers rounds from the directory, checks block CONTRACTS (COUNT, contiguity), and states
  its presence-not-adequacy limit on its own face. Both carry seeded positives + deletion
  probes in `--self-test`, run in the battery.
- **The ceremony's verifier is an honest pin** (GPT56 F1 + CODEX F4): REQUIRED-DOES-NOT-EXIST,
  digest set AND printed when built, an acquisition path, and a CHECK-not-read script contract
  (chain head vs an independent chain copy; receipt digest vs the store; verifier digest vs
  the pin).
- **R08 requires BS-V** with a deletion control (CODEX F7). **Bool bytes are exactly
  0x00/0x01** (CODEX F8). Generalized L08 caught a real two-field-prefix collision between the
  terminal checkpoint and the review body on this build's own battery — the review tuple
  reordered. Registry 250; kinds 17 over 61 sites.
...
- **ATTACK T1–T3 AS A SYSTEM. Highest value here.** Three properties, one construction.
  **Break the composition**: a TERMINATED condition firing DURING an existing drain (two
  drain-starts?); the draining epoch's own crash mid-refusal (does T2's idempotence survive a
  half-committed refusal?); a drain-start with no reachable terminal checkpoint (PENDING
  arrival whose refusal commit itself fails under the bounded-commit rule — abort loops
  forever?); and the wire-residue scoping of T1 (a request RECEIVED but not yet arrival-
  committed when drain-start lands — which side?).
- **Attack the DRAINING epoch against the clock invariants.** It opens like any epoch. **Break
  it**: does its opening declare gaps normally; do drained refusals join the FIFO/decide-
  within-D predicates (their arrivals may be long past D — which acceptance predicate do drain
  refusals satisfy, and is REFUSED-LOCK-OR-CEREMONY-STATE truthful for a request that expired
  BEFORE the drain began?).
- **Attack the site coverage map.** 61 sites, three categories, hand-curated regexes. **Break
  it**: a site whose category is WRONG (a RAW that is actually a canonical body; the MIXED
  lockbody rule hiding a taggable constituent); the seeded control plants an unknown NAME —
  plant an unknown CATEGORY instead and say whether the probe still bites.
- **Attack the ledger v2's floor.** Coverage begins at V100 by rule. **Say whether the
  V88–V99 rounds' findings have any completeness control at all**, and if not, whether the
  FINDINGS_MAP prose is now the only witness for twelve rounds.
- **Attack the reordered review tuple.** Field order chosen to dodge the L08 prefix key.
  **Say whether prefix-keying is the right identity for schemas at all** — two records could
  legitimately share a prefix; is L08's key a heuristic wearing an invariant's name?
...
Write to `gates/V103_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
