# BRIEF — V104 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V104_20260830.md`
**sha256 `7414666df54d2757e412cc541c0de37cb13667487d3a3a76a8a3221284f02d2f`**
...
**Last refereed draft: V103** `a9506e37e85e593d` — **NOT CLEAR from both seats** (GPT56 7,
...
## What V104 changed — the drain set defined once, the controls shown failing first

**V103 was NOT CLEAR ×2** (GPT56 7, CODEX 5) — drain edge windows plus instrument QA.

- **T2 defines THE DRAIN SET once, at drain-start, from the log**: every zero-terminal arrival
  at or before the drain-start event — deadline-expired INCLUDED (they get forced terminals at
  drain) — plus the in-hand decoded frames T1 commits ahead of the drain-start. Nothing joins
  later, nothing leaves. Drain refusals satisfy a DRAIN-SCOPED acceptance predicate; the
  precondition chain is the only ordering (receipt durable → drain-start binding it →
  per-member terminals → checkpoint on empty set); at most ONE drain-start per chain, later
  TERMINATED receipts store-only.
- **The strict control sequence ran visibly** (the coordinator's order after two hollow
  generators): the seeded CATEGORY control was built FIRST and SHOWN FAILING on the live kinds
  tool — it caught the undeclared MIXED exactly as you did, then caught my incomplete first
  repair (a second stale MIXED rule) before the battery ran. Categories are a closed validated
  set; the lockbody blanket is split per-constituent; canonical-under-v9 sites moved
  RAW→FROZEN.
- **L08 v3 is rebuilt against a WRITTEN SPEC**: schema identity = the canonical schema digest
  over the NFC-normalized, sorted, count-prefixed field set. Reordering cannot evade (same
  identity, ordered mismatch = divergence). The legitimate-collision residual is stated on its
  face. Control replanted as a reorder.
- **R08's BS-V check was SHOWN HOLLOW on draft-shaped text** (BS-V mentioned elsewhere, deleted
  from the clause — it passed), then scoped to the fresh-pass clause with a shaped control.
- **The ledger covers V88–V99**: all 161 historical findings verified LITERALLY CITED in the
  findings map by mechanical scan; an uncited historical finding is fatal; V88 is the block
  format's own floor, stated. And R01 caught the coordinator's sketched code quoted live in my
  own T2 draft text — reworded tokenless.
...
- **ATTACK THE DRAIN SET'S EDGES. Highest value here.** Defined once at drain-start. **Break
  the definition**: an arrival whose position is AFTER drain-start because T1's in-hand commit
  pushed it there (in the set by T1's clause, after the boundary by position — which reading
  wins, and is the set still log-derivable?); a drain-set member whose forced refusal ABORTS
  under the bounded-commit rule repeatedly (does idempotent resumption make progress or loop?);
  the drain interval's own clock predicate when the drain spans epochs (crash mid-drain, new
  epoch — "inside the drain interval" measured how, across the epoch seam?).
- **Attack the drain-scoped predicate's honesty.** REFUSED-LOCK-OR-CEREMONY-STATE for every
  member. **Break it**: the enumeration machinery — drain refusals are ordinary refusals on the
  chain; do they need enumeration entries, do they flood any gate's completeness check, and is
  the ceremony-state code truthful for an arrival that expired BEFORE any ceremony state
  existed?
- **Attack the schema-digest identity.** Field SETS identify schemas. **Break it**: two
  legitimate schemas sharing an exact field set TODAY in these files (find the instance, not
  the possibility); a field renamed in one file only (new identity both sides — silently two
  schemas, no flag; is rename-divergence L08's job or L03's, and does either catch it?).
- **Attack the citation-scan floor.** "SEAT Fn" substring matching over one big map. **Break
  it**: a citation for V93 F4 that actually refers to V83 F4 by ellipsis (the scan's patterns
  cross round boundaries — construct the false positive); the V88 floor claim (do any V1–V87
  reports carry parseable blocks after all?).
- **Attack the shown-hollow method itself.** Twice this build, a control was demonstrated
  failing before repair. **Say what that demonstration does NOT establish** — a control that
  fails on the known defect can still be blind to the unknown one; is there a stated discipline
  for choosing the seed, or does the seed always chase the last finding?
...
Write to `gates/V104_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
