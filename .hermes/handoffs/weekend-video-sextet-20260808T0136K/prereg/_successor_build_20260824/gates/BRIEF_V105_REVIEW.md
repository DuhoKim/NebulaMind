# BRIEF — V105 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V105_20260830.md`
**sha256 `bbf3dd45e22e818d130bf29c3a3540bcb6239b51e79dd65221d86070f9583735`**
...
**Last refereed draft: V104** `7414666df54d2757` — **NOT CLEAR from both seats** (GPT56 6,
...
## What V105 changed — controls-as-spec, the full floor, the store as projection

**V104 was NOT CLEAR ×2** (GPT56 6, CODEX 5).

- **L08 v4, controls before code** (your F2/F3 pair; the coordinator's strike-three order):
  FOUR seeded controls were run against v3 first, colors recorded — rename BLIND, type-change
  BLIND, reorder caught, collapsed-duplicate red-for-the-wrong-reason — then identity rebuilt:
  digest over the ORDERED (field, registry-constraint) serialization at EVERY occurrence,
  name-set grouping, L09 as the rename fingerprint. All four post-fix colors correct. The live
  run then caught matrix notation posing as a tuple, a prose join-key pair, and a GENUINE gap —
  the terminal-review body's six fields were never registered (now REVBODY).
- **Ledger v4** (F1 pair + CODEX F5): the citation key binds (round, seat, finding) with
  wrong-round and wrong-seat decoy controls; the false V88 floor is gone — every parseable
  block is covered, 355 pre-convention findings COUNTED per round as audit debt, 21
  convention-era gaps cured by a citation addendum, and the parser takes the report's FINAL
  block (CODEX-V104's own report quoted the marker mid-prose and the first-match parse refused
  its reviewer — the citation checker got the same column-0 rule).
- **THE STORE IS A PROJECTION OF THE LOG** (GPT56 F3): later TERMINATED receipts get
  RECEIPT-NOTE chain records. **The drain predicate is grounded in log facts** (CODEX F2 +
  GPT56 F6): membership fixed at drain-start; timeliness is a chain-position interval,
  epoch-independent. **Aborts cap at `A_max`** (GPT56 F5): cap-exhausted members are listed in
  the terminal checkpoint's `failed_members` — platform family per member, never a loop.
- **Row I's history join derives the store per OBJECT CLASS** (GPT56 F4). **Comment stripping
  is battery-wide** in the refusal checker, raw text reserved for the tombstone scan (CODEX
  F4; commented-decoy control added; 43 controls). Registry 261; kinds 18 over 66 sites.
...
- **ATTACK THE PROJECTION PRINCIPLE. Highest value here.** Every terminal fact is a log event
  first. **Break it**: enumerate the STORE-FIRST writes that remain anywhere (slot receipts?
  the halt receipt itself is store-first by T1's own ordering — receipt durable, THEN
  drain-start: is the principle violated by the construction that cites it?); a receipt-note
  whose receipt was never written (note-first inversion); the gates' refusal rule reading the
  STORE ("any receipt in the store") — is the store still load-bearing where the principle
  says the log leads?
- **Attack failed_members.** Cap-exhausted members are listed and skipped. **Break it**: the
  checkpoint lands with failed members — the run ends TERMINATED, but those arrivals are
  zero-terminal forever; T2's own after-the-checkpoint rule says any zero-terminal arrival is
  malformed history. Reconcile the checkpoint's own list with the rule it violates, or find
  the wording that already does.
- **Attack the L09 registry dependency.** The rename fingerprint needs the registry loaded;
  absent registry = no L09 (the fallback). **Break it**: which invocation paths run without
  the registry present, and does the battery guarantee the live path always has it?
- **Attack the pre-convention debt.** 355 findings counted, not audited. **Say what the debt
  means adversarially**: if one-tenth were silently unrepaired, what in TODAY'S draft would
  show it — or is the debt's only witness the era prose the strict key already distrusts?
- **Attack the column-0 marker rule.** Quoted markers are prose. **Break it**: a hostile
  report placing a decoy block AT column 0 before the real one (two column-0 blocks = C05
  UNVERIFIABLE — is refusing the whole report the right failure mode, or does it let a seat
  DoS its own audit?).
...
Write to `gates/V105_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
