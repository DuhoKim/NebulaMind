# BRIEF — V106 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V106_20260830.md`
**sha256 `b0bfded176fdfb171581b2cb700ece4385dbeb95da3331f969f24e3f33ac48c9`**
...
**Last refereed draft: V105** `bbf3dd45e22e818d` — **NOT CLEAR from both seats** (GPT56 4,
...
## What V106 changed — authority not chronology, terminal-by-listing, the copy deleted

**V105 was NOT CLEAR ×2** (GPT56 4, CODEX 5 — the smallest round yet; both self-flagged
attacks confirmed).

- **The projection principle is refined to AUTHORITY, not write order** (your F1 pair): the
  store write may precede in time, but a fact becomes OPERATIVE only at its log record; an
  orphan store receipt carries no force. The write-order reading contradicted T1's own
  receipt-first sequence — the seats were right and the principle now says what it meant.
- **`failed_members` composes with the after-rule** (F2 pair): a listed member's terminal fact
  is CARRIED BY THE CHECKPOINT — terminal-by-listing; malformed = neither drained NOR listed.
- **The seven-vs-eight-field divergence** (F3 pair): T3's spec tuple synced, and the structural
  fix — (ii-f)'s restated tuples are DELETED; the item references the label-bound T-quotes as
  the single surface. L08's name-set grouping is blind to pure addition BY DESIGN (supersets
  are legitimate between records), so single-sourcing replaces another heuristic.
- **L09 runs on the live path** (CODEX F4): prereg_lint now passes `spec_path` — the rename
  fingerprint was silently disabled in the one invocation that mattered.
- **`n_perturbations` is POSITIVE at both sites** (CODEX F5). **The debt number is quoted from
  the ledger** (GPT56 F4): my 355 was a hand count drifting from the generator's 334 within
  the hour — the standing rule applied to its own author's prose.
...
- **ATTACK TERMINAL-BY-LISTING. Highest value here.** A listed member's terminal fact lives
  in the checkpoint. **Break it**: the join verifier's exactly-one-terminal predicate — does a
  listing COUNT as the one terminal (binding→key has no entry for it; the bidirectional join
  finds an arrival with no terminal EVENT and no map entry — which check fires?); a listed
  member's requester (N1 says delivery is outside custody, but what does the LOG claim this
  request's outcome was, and under which acceptance predicate?).
- **Attack authority-not-chronology.** An orphan store receipt carries no force. **Break it**:
  WHO detects the orphan and WHEN (the gates refuse on receipts — do they refuse on
  receipt-WITHOUT-log-record, and is there a bound on how long staging may dangle?); a hostile
  store write that never gets its log record — is it invisible forever or a named refusal?
- **Attack the single-surface move.** (ii-f) now references quotes instead of restating.
  **Say whether the exhaustive list still functions**: the list's job is closed schemas ON the
  list — does a reference-by-label satisfy "an artifact is non-χ only if it conforms to one of
  these authenticated schemas", or did the move trade divergence-immunity for definitional
  indirection a seat can attack?
- **Attack the live-path L09.** It now runs under lint. **Break it**: the registry loads from
  spec_path.parent/ref — is that path right for every invocation geometry (round scripts run
  from gates/; the battery from repo root); a registry regenerated AFTER lint in the build
  order.
- **Attack the smallest-round signal.** Four and five findings, all on latest-round text.
  **Say what the corpus's oldest UNREVISITED normative claim is** — the sections nobody has
  attacked in twenty rounds — and whether quiet means sound or merely unread.
...
Write to `gates/V106_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
