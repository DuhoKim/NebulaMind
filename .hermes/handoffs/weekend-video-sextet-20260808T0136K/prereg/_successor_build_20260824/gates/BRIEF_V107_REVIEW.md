# BRIEF — V107 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V107_20260830.md`
**sha256 `d7b57cdaa24fc51f2b8135dd00960377fb3186891fa8c7cb6e83ac4645204401`**
...
**Last refereed draft: V106** `b0bfded176fdfb17` — **NOT CLEAR from both seats** (GPT56 3,
...
## What V107 changed — the listing joins the join, the lost schema restored

**V106 was NOT CLEAR ×2** (GPT56 3, CODEX 4 — smallest yet).

- **The listing is the join's THIRD TERMINAL FORM** (your F1 pair — terminal-by-listing failed
  G4 and the bidirectional join because a listed member has no decision event and no binding):
  exactly-one-terminal now reads as ONE DECISION EVENT XOR ONE `failed_members` LISTING;
  both-or-neither is malformed; three fixtures (listed accepted; listed-and-decided refused;
  a listing in a checkpoint the chain does not carry refused). G4 counts decisions — zero is
  not two.
- **T2's precondition chain synced** (CODEX F2): the checkpoint requires every member DRAINED
  OR LISTED — the chain clause had demanded the empty set one clause after listing existed.
- **The DRAIN-START schema is back on the single surface** (F2/F3 pair): the single-surface
  deletion had removed the only place its tuple lived; T1's row now carries `(kind,
  receipt_digest, boot_epoch, monotonic_reading)`.
- **Orphan refusal has machinery** (F3/F4 pair): the STORE↔LOG PASS at all five gates
  enumerates the TERMINATED receipt store via Row B and refuses any receipt without its log
  record after `B_stage` chain positions — a BS-2k staging bound; fixtures include the
...
- **ATTACK THE XOR FORM. Highest value here.** One decision event XOR one listing. **Break
  it**: a member drained in the FIRST drain attempt whose refusal committed, then Row B crashed
  pre-checkpoint and the resumed drain LISTS it after A_max aborts of a retry it never needed
  (decided AND listed — whose bug?); the requester-facing meaning of a listing (what outcome
  does N1 let the requester infer, and under which code does the auditor file it?).
- **Attack B_stage.** A staging bound in chain positions. **Break it**: a quiet chain (no
  positions advance — the bound never expires; is an orphan on a stalled chain invisible
  forever?); the bound's interaction with the drain itself (drain-start is the receipt's log
  record — can a receipt's staging window outlive the entire drain?).
- **Attack the restored drain-start tuple.** It now lives in T1's row. **Say whether L08/L09
  actually cover T-row tuples** — the tuple extractor reads backticked parenthesized forms
  anywhere; the receipt-note and drain-start tuples now appear ONCE each (spec-only after the
  V106 deletion) — a spec-side edit to either has no draft counterpart to diverge from; is
  single-occurrence a blind spot the single-surface move created?
- **Attack the store↔log pass's read path.** The verifier enumerates the receipt store via Row
  B. **Break it**: the verifier runs at gates — is its store read a logged touch (every touch
  logged!), and do those reads flood the chain the pass then re-reads (self-feeding)?
- **Attack the oldest-quiet question again, concretely**: pick ONE §2-era selection/photometry
  clause untouched since the V30s and re-derive it against the current machinery — does it
  still compose with the arrival class, the drain, and the acceptance predicates?
...
Write to `gates/V107_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
