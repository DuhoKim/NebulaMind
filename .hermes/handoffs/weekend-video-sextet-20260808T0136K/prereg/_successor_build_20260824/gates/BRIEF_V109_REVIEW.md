# BRIEF — V109 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V109_20260830.md`
**sha256 `69e726ffef1f1468ade0bd46fcaa94f8cc840c4e5b1e50e54f2d9f66031f9fe5`**
...
**Last refereed draft: V108** `ae54ecdf9118d4c3` — **NOT CLEAR from both seats** (GPT56 4,
...
## What V109 changed — the ordering law, the double-read, the export, the third token

**V108 was NOT CLEAR ×2** (GPT56 4, CODEX 5).

- **The drain-scoped ORDERING LAW** (your F1 pair): ordinary FIFO ends at drain-start; members
  are ATTEMPTED in arrival order, individual commits land in attempt order, and the deferred
  in-commit events are arrival-ordered among themselves — no cross-set order is claimed
  because none exists to enforce. And the drained-or-listed language is swept from all three
  surviving clauses (one survived my first sweep; the re-grep caught it) — the disjunction is
  gone because both branches are the same thing: an event.
- **The DOUBLE-READ orphan discipline** (F3/F2 pair): the receipt-first interval is real, so
  the orphan verdict requires a receipt present in two reads with no record while the head
  ADVANCED between them; no-movement intervals defer to the next pass, which always exists
  before anything the orphan could corrupt.
- **The SUCCESSOR EXPORT exists** (F4 pair): at the run's last gate pass the verifier emits
  the canonical successor-bound artifact — `(kind, sealed_enumeration_digest,
  continuation_segment_digest, terminal_head, freeze_signature_digest)` under the enumerator
  keypair, receipted, on the exhaustive list. The duty now points at bytes. Registry 266,
  kind 19.
- **The REVIEWED disposition joins the entry schema** (CODEX F5), valid exactly for the
  mismatch class — the ruled logged-and-continued treatment was excluded by the only exact
  schema; the catch-all keeps its two and the verifier enforces the pairing. **The tuple
  probes require unique anchors** (CODEX F3).
...
- **ATTACK THE DOUBLE-READ. Highest value here.** Orphan iff present-twice + head-advanced +
  no record. **Break it**: Row B appends the drain-start BETWEEN the two reads (head advanced,
  record now exists — does the second read see it, and is read-order store-then-chain or
  chain-then-store on each pass?); a pass whose second read races a crash; the "next pass
  always exists" claim at the disclosure gate (what pass follows the last one?).
- **Attack the ordering law's verifier.** Attempt-order from individual commits, arrival-order
  within the checkpoint set. **Break it**: attempts are not chain events — what byte sequence
  does the verifier read attempt-order FROM (individual commits show commit order; an aborted
  attempt leaves nothing); say whether attempt-order is checkable or testimony.
- **Attack the successor export.** One artifact, enumerator-signed, at the last pass. **Break
  it**: the run that dies BEFORE its last pass (terminated mid-drain — which pass emits the
  export, and does a crashed run export anything?); the export's own verification by the
  successor (the enumerator key again — what human-signed byte does the successor check the
  export against?).
- **Attack REVIEWED.** Class-paired disposition. **Break it**: a mismatch entry disposed
  EXPLAINED (legal under the old text — still legal? which wins?); the recurrence rule's
  interaction (do REVIEWED entries count toward class recurrence, and can a recurring mismatch
  class be REVIEWED forever — the same explained-forever shape one token over?).
- **The oldest-quiet question, third asking**: one §2-era clause, re-derived against the
  current machinery.
...
Write to `gates/V109_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
