# BRIEF — V44 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V44_20260829.md`
**sha256 `4faa2564ba093ae4eccbd3f868f62782833b2a09c56ec8282945b5fd46d65aa2`**
Verify this digest before reading. If it does not match, stop and report the mismatch.

**Last refereed draft: V43** `7b2e9a701c38c570` — **NOT CLEAR from both seats**, 13:33 (GPT56 3
...
**On V44 the lint exits 0 — 96 advisory findings, 0 blocking.** The 96 are citations whose reports
...
Other checkers on V44: counts **16 class P / 8 class E** prose-matched; `prereg_trace` 43 transitions,
0 problems; `void_registry` self-test 6 controls, 0 failures.

## What V44 changed — all five V43 findings

- **GPT56-V43 F1 / CODEX-V43 F1 (HIGH, both seats).** A pre-unblinding Stage-C numerical failure
  mapped to **both** `INCONCLUSIVE-BY-COMPUTATION` and Row J's POWER route, and the new code had **no
  named producer**. `INCONCLUSIVE-BY-COMPUTATION` is now **residual by construction** — it fires only
  where neither `INCONCLUSIVE-BY-POWER` nor `INCONCLUSIVE-BY-CALIBRATION` claims the failure — with
  its producers named (Row J and the production runner) and a **§11 item requiring it be evaluated
  AFTER both guards**, so residual status is enforced by order of evaluation rather than asserted.
- **GPT56-V43 F2 / CODEX-V43 F2 (both seats).** The `BS-3g` row still presented the three-way
  completeness fork **the principal ruled on at 10:15**. It now records the ruling — the executable
  joint counterfactual path is selected, holding observed `p` fixed is rejected — and names the **one**
  thing still open: the γ → sign-vector/calibration mapping.
- **GPT56-V43 F3.** V43 wrote its **own** V42→V43 transition in-band with `(this revision)` in place
  of a result digest, which §10's own contract forbids (a current transition is sidecar-only, and
  every written row must carry its own digest). The row now appears with V43's real digest, and V44
  writes **no** row for the transition that created it.
...
- **Is `INCONCLUSIVE-BY-COMPUTATION` genuinely residual**, or only declared so? Test whether any
  pre-unblinding numerical failure can still reach two codes, and whether §6.3(10)'s one-outcome
  requirement is now actually satisfied for every Row J branch.
...
Write to `gates/V44_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
