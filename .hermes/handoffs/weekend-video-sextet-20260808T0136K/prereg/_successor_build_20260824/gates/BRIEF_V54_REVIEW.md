# BRIEF — V54 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V54_20260829.md`
**sha256 `b0ccbecc46e216777867ef1e219b15cb991781d1455c6bb8e8e0af7d0c204190`**
Verify this digest before reading. If it does not match, stop and report the mismatch.

**Last refereed draft: V53** `cc4e289578b129e4` — **NOT CLEAR from both seats**, 19:32 (GPT56 3,
...
**On V54 the lint exits 0 — 96 advisory findings, 0 blocking.** The 96 are citations whose reports
...
Other checkers on V54: counts **16 class P / 8 class E** prose-matched; `prereg_trace` 53 transitions,
0 problems; `void_registry` self-test 6 controls, 0 failures.

## What V54 changed — and the headline is a withdrawal, not a repair

**GPT56-V53 F1 / CODEX-V53 F1 (both seats, HIGH) — my evidence was an artefact of my harness.**
`allocate_handcheck(cell_counts, budget)` takes **`budget` as an argument**. My harness held it at the
frozen `HC_REAL_LABELS = 500` and varied only cell counts, so it **could not reach the guards `budget`
controls**. At `budget = 200` with abundant objects, **L1401 fires immediately.** Non-observation under
that harness was a fact about the harness, not about the code.

Re-run with `budget` varied over **80,000 executions**: L1401 fires; L1411/L1435/L1437/L1439 still do
not.

**All five promotions are nevertheless WITHDRAWN, and no site now holds
`UNREACHABLE-BY-CONSTRUCTION`.** The four that survived the better harness are withdrawn because
**CODEX-V53 F2** is right that a generic appeal to *"feasibility is decided before allocating"* is not
the **per-site predecessor condition** §5 demands. Surviving a better harness is not meeting the bar.

**This is the second consecutive round the promotion failed**, so V54 withdraws rather than re-argues,
per the standing rule. §5 restates the bar so a third attempt would not be a third failure: a harness
varying **every argument in the callable's documented surface**; a **named per-site** subsuming
condition for a structural claim; a positive control.

**GPT56-V53 F3 / CODEX-V53 F3.** §11 still carried the **48-unread** status after §5 had superseded
it. Withdrawn. **§5 and the ledger are both at `NUMERICAL 22`**, ledger regenerated in the same commit
— they had disagreed in both directions across V52 and V53.
...
Write to `gates/V54_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
