# BRIEF — V59 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V59_20260829.md`
**sha256 `9257411511b39de6c32b8b5b52a2f4ad45dec287a9150332dadafdd6253c6105`**
...
**On V59 the lint exits 0 — 96 advisory findings, 0 blocking.** The 96 are citations whose reports
...
Other checkers on V59: counts **16 class P / 8 class E** prose-matched; `prereg_trace` 58 transitions,
0 problems; `void_registry` self-test 6 controls, 0 failures.

## What V57, V58 and V59 changed

**V57 — a withdrawal.** You broke both things the V56 brief asked you to break, and the principal
ruled on it. **The closure argument is withdrawn and the eight-code refusal set is SUSPENDED** —
neither reaffirmed nor reversed. Your counterexample stands: an authorised read of a permitted but
**missing or unreadable cutout** is a refusal Row B must make that none of the eight codes covers, and
an unlogged refusal voids the run. **The enumeration missed a class, not a member.** The derivation is
to be **redone from scratch**, covering authorisation *and* availability/mediator behaviour with a
**tested** joint-exhaustiveness argument. `REFUSED-IDENTITY-OUTSIDE-PERMITTED-SET` does not survive.
**The fingerprint is deliberately NOT re-pinned** — `refusal_vocabulary_check.py` reports **R05 on
this draft, and that is the designed state, not a defect.**

**V58 — BS-3g's schema, specified** (CODEX-V56 F2 half one). Nine fields:
`kernel_sha256` · `estimator_sha256` · `verifier_sha256` · `mapping_id` · `gamma_hat` ·
`sigma_gamma` · `gamma_bound` · `invariance_outcome` · `n_perturbations`, with types written so an
implementer who did not write the schema can verify against them; plus producer, independent verifier
and failure behaviour. **`mapping_id` admits only `MAPPING-NOT-PREREGISTERED` until a mapping is
preregistered, and such a receipt cannot discharge the BS-6 edge.**

**V59 — the producer binding** (half two). `receipt()` enforces field sets **only** for slots in
`SLOT_SCHEMA`; executed against the pinned bytes, `{'per_object_chi': b'+1'}` is **ACCEPTED** for
every absent slot, and **five are absent**. V59 binds every named producer to a `receipt_strict()`
that refuses unknown slots, and adds a verifier making a permissive-path receipt **detectable** rather
than merely discouraged. **v9 is untouched at `6a9abbbd`.**

**Four `SLOT_SCHEMA` entries are deliberately still absent** (BS-2a, BS-2k, BS-L, BS-2v). No receipt
may be emitted for a slot lacking an entry, which makes each a **blocker on filling that slot** rather
than a hole — BS-2a is DESIGN/UNFILLED and BS-2v UNRESOLVED, so a schema written now would pin
undecided content.
...
Write to `gates/V59_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
