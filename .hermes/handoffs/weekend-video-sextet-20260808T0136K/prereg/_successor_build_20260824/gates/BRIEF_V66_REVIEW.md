# BRIEF — V66 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V66_20260829.md`
**sha256 `92b589e635228be87120585dfe04ec691923ac0cd8ae970634da57d0e49ca79d`**
...
**On V66 the lint exits 0 — 97 advisory findings, 0 blocking.** The 96 are citations whose reports
...
Other checkers on V66: counts **16 class P / 8 class E** prose-matched; `prereg_trace` 65 transitions,
0 problems; `void_registry` self-test 6 controls, 0 failures.

## What V65 and V66 changed — a new covenant clause, and six of your V64 findings

**V64 was NOT CLEAR ×2** (GPT56 5, CODEX 7) and **five of your findings converged.** All but one are
repaired below; the exception is named in the parked list and is with the principal.

**V65 — the access schedule is precommitted and χ-blind** (principal ruling, on your converging
findings: GPT56 via Row G, CODEX via Row D). §2.7(3) constrains the **exclusion predicates** and does
not reach the **access schedule**, so the χ-blindness this covenant relied on was inherited from
something that does not cover it. Every row that may read a χ-bearing object now has a precommitted
traversal — order, multiplicity, retries, stopping — and **Row B refuses any request that is not the
next unserved position.** **The clause forbids rather than observes:** nothing built today is adaptive,
but the covenant permitted it and **permission is what leaks**. `flag → discard → replace` is the one
exemption, recorded **as a finding with its dependency bound to the Row G interface digest**, not as a
permission.

**V66 — your V64 findings.**
- **F1 / F1, the lifecycle.** You were right that it asserted a boundary without making it atomic.
  **The append is now the verdict** — none exists until its event is durably in the chain — and Row B
  **appends before releasing any byte**, so a crash before the append leaves no event *and* no
  delivery. The cost is stated: **the log can over-report a touch and can never under-report one.**
  Single serialised writer, one decision per request, Row B owns recovery, a lease-losing worker may
  not append. **The durable PRE-verdict state is NOT fixed** — it needs a second event class and
  Row B's cell commits to one event per touch, so it changes what the log records and is referred.
- **F2 / F3, the catch-all guard.** You were right that freeze-time review cannot police run-time
  emissions — **the freeze is P0 and every refusal is P1 or later.** The enumeration moves to the
  **pre-unblinding lock checkpoint** and gains a **blocking invariant**: `BS-L` may not be issued while
  any `REFUSED-UNCLASSIFIED` is unenumerated. **And I found the follow-on myself rather than shipping
  it to you:** the lock is P6 while Row B runs through unblinding, so a single checkpoint leaves a
  P6 → P7 after — the obligation is now **continuous**, blocking both `BS-L` and the opening of the
  lock. **The checker verifies the mechanism now** (R06 anchored-at-freeze, R07 no-blocking-invariant)
  rather than phrase-matching the guard.
- **F3 / F5, the draw pre-commitment.** Correct: it claimed the freeze witnesses values **nobody had
  written**. `n_draws` and `draw_master_seed` are class-P and **UNSET**, and their absence **blocks
  BS-3g emission**; `mapping_id`'s closed set has one member and `draw_generator_id`'s is **empty**,
  which blocks too.
- **CODEX F4, the manifest.** A singleton `γ = 0` satisfied every check while perturbing nothing. It
  must now span the bound — both endpoints, three distinct values, singleton refused by name.
- **F4 / F6, L986** — the generator was fixed, not just the prose; the ledger reads CALLER 25,
  PLANNING-INTERNAL 1.
- **F5 / F7, the checker digest** — recomputed, **and it went stale a second time inside the repair**
  when the checker gained R06/R07. Recorded with the only rule that survives: compute it last.
...
- **The durable pre-verdict state** — needs a second event class; changing what the log records is not
  authorised. Referred.
- **Row F's read surface and the nine HC strata** — `FINDING_ROW_F_STRATA.md`, **with the principal**.
...
- **BREAK THE ATOMICITY. Highest value in this round.** The claim: under append-before-release, no
...
Write to `gates/V66_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
