# BRIEF — V67 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V67_20260829.md`
**sha256 `3dbf4af7fab34e1f58477fffb92fcc4af40a4ceb9ed21d61f8063469f9e7c0e8`**
...
**On V67 the lint exits 0 — 97 advisory findings, 0 blocking.** The 96 are citations whose reports
...
Other checkers on V67: counts **16 class P / 8 class E** prose-matched; `prereg_trace` 66 transitions,
0 problems; `void_registry` self-test 6 controls, 0 failures.

## What V67 changed — six of your V66 findings, four of which my V66 repairs caused

**V66 was NOT CLEAR ×2** (GPT56 5, CODEX 6), and **four of those findings were defects the V66 repairs
introduced.** That is the round working, and it is worth being explicit that the previous repair is
where they came from.

- **F2 / F1 — the event could not carry an outcome it did not have.** Append-before-release kept the
  log ahead of delivery and made the success/refusal field a claim Row B could not yet make. Now
  **resolve, then append, then release**: the touch is resolved into storage Row B alone controls, the
  single event carries the **true** outcome, and only then are bytes released. **The residue is
  unchanged and still stated** — the log can over-report a touch and never under-report one — but the
  event is no longer false in the failure case. **Still exactly one event per touch.**
- **CODEX F2 — request identity.** *"Never re-decided"* was unenforceable without saying what a
  request is. Row B assigns an internal identifier, **held in its recovery state and not written to the
  log**. **A legal retry is a NEW request with its own event**, because a retry is a second touch;
  what is forbidden is **one request producing two events**.
- **F3 / F3 — the continuous obligation had no verifier**, and its control matched the phrase `BS-L`
  rather than the mechanism — **the phrase-matching defect reproduced inside the control written to
  remove it.** A separately pinned **enumeration verifier** now recomputes emissions from the chain and
  is **consulted twice**: at `BS-L` issuance and again **at the opening of the lock, over the chain as
  it then stands.** Checker gains **R08**.
- **CODEX F4 — a recurring class could be explained forever** and stay formally enumerated. **Explanation
  now stops discharging a recurring class**; the vocabulary must be re-derived to name it. Checker
  gains **R09**.
- **F4 / F5 — spanning is not resolving, and this is NOT engineered away.** No finite manifest can
  prove no interior flip exists. **The asymmetry is stated: a flip found anywhere is decisive; `HELD`
  means only that none was found on the evaluated grid.** The grid spacing `Δγ` is class-P, **UNSET**,
  and a further blocker.
- **F5 / F6 — §5 still said all three planning sites are `CALLER`**, three revisions after L986 moved.
  **The drift, in the sentence written to record the drift.**

**Also applied, by principal ruling: Row F's read surface is WIDENED to what its emission requires.**
The input is the **per-object HC stratum index**, and **both its axes are χ-derived** — the |χ| tertile
by definition, the machine-committee state because it is two classifiers judging handedness. **The
expected escape does not exist.** Widening puts the χ-bearing input inside the covenant **now** rather
than at freeze, and makes Row F's void clause **load-bearing instead of vacuous**: the stratum index
may reach the allocation and **may never reach `calibration_bins()`**.
...
- **Whether this study's strata should BE χ-derived** — `FINDING_ROW_F_STRATA.md`, **with the principal**. Row F's *surface* is ruled and applied; only the strata decision is open.
...
- **BREAK RESOLVE-APPEND-RELEASE. Highest value in this round.** The claim: the event always
...
Write to `gates/V67_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
