# BRIEF — V69 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V69_20260829.md`
**sha256 `d52844620fbda2e561f8904d5cbffc62e88a5e57a03c6024edd59cc0671e5f88`**
...
**On V69 the lint exits 0 — 97 advisory findings, 0 blocking.** The 96 are citations whose reports
...
Other checkers on V69: counts **16 class P / 8 class E** prose-matched; `prereg_trace` 68 transitions,
0 problems; `void_registry` self-test 6 controls, 0 failures.

## What V69 changed — the guarantee is stated first, and the corners enumerate themselves

**V68 was NOT CLEAR ×2** (GPT56 5, CODEX 8). **The lifecycle had then failed two consecutive rounds on
different corners — one short of this lane's three-failures rule — so V69 does what that rule demands:
state what the thing is FOR, then derive it.** §6.1 now opens the lifecycle with the guarantee — **G1
no unlogged touch, G2 no false event, G3 one event per touch, G4 no double decision — and with equal
weight what is NOT guaranteed: N1 delivery is outside the custody claim (the requester is external to
any commit domain), N2 a pre-commit death is invisible (referred).** The construction derives from it:
a **touch commit** is atomic {store effect, true-outcome event, binding}; a **refusal commit** is
{event, binding}; **delivery is a separate act from the committed buffer** — re-sending is the same
conveyance, never a second event; **bytes cannot escape an aborted commit** because the requester
receives only from a committed buffer; and **the `TRANSFER` state is DELETED** — after the commit, no
failure alters anything the log claims, which dissolves your F2 pair rather than patching it. Terminal
treatment now distinguishes **who died**: a worker failing under a live Row B gets a refusal commit;
Row B dying pre-commit is N2, said plainly.

**Your other findings:**
- **class_key = (row, operation)** — the fields the schema actually carries (your F3/F4 pair); adding
  a field is not authorised, and coarser merges more, so the recurrence rule fires sooner.
- **Dispositions bind to artifacts** (GPT56 F4): `NAMED-AS-DEFECT` carries the re-derivation digest,
  `EXPLAINED` must resolve to a signed artifact, dangling refs refused.
- **Recurrence is WITHIN-RUN, stated as a limit** (GPT56 F5), with the cross-run duty placed on any
  successor preregistration's freeze review.
- **The ledger violated the planning ruling by auto-typing** (CODEX F3): `_plan` L1331/L1341 raise
  `InconclusiveByPower` **at planning** — moved to `PLANNING-INTERNAL` (now 3; TYPED-OUTCOME 1). The
  typed exception is the operator-stop mechanism; the outcome class is produced only by run-time
  guards. The AST type counts stay true.
- **The three build items EXIST now** (CODEX F5): `gates/enumeration_verifier.py` with its refusal
  set, the atomic-touch commit domain as a BS-2k design requirement with fixtures, the BS-2f boundary
  recomputation verifier.
- **The recomputation predicate is stated at its true strength** (CODEX F6): it certifies the
  **artifact**, not the process — what it cannot do is move a boundary undetected; influence that
  changed nothing stays a conduct matter under the void clause.
- **`gamma_bound` has an origin outside the receipt** (CODEX F7): the verifier recomputes it as
  `|gamma_hat| + k_gamma·sigma_gamma`, `k_gamma` frozen class-P, **UNSET — a further blocker**.
- **The outcome is a total function of the evaluation** (CODEX F8): `NOT-EVALUATED` only at zero
  evaluated cells; an evaluated matrix is `HELD` iff every cell equals baseline, else `FAILED`.
...
- **BREAK THE GUARANTEE, NOT THE CONSTRUCTION. Highest value in this round.** G1–G4 and N1–N2
...
Write to `gates/V69_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
