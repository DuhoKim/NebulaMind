# BRIEF — V71 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V71_20260830.md`
**sha256 `7a8e7151e4063e5e77f0910835686ba6fba0aececa6e645429bcd1afda8ea238`**
...
**On V71 the lint exits 0 — 97 advisory findings, 0 blocking.** The 96 are citations whose reports
...
Other checkers on V71: counts **16 class P / 8 class E** prose-matched; `prereg_trace` 70 transitions,
0 problems; `void_registry` self-test 6 controls, 0 failures.

## What V71 changed — the spec survived its first round and gained two invariant fixes

**V70 was NOT CLEAR ×2** (GPT56 5, CODEX 8). **The spec worked as designed: the seats attacked it
directly and scored one real hit on its invariants** — everything else was mechanism downstream.

- **Your F1 pair, the spec hit**: refusal events have no store effect, so G3's *one-touch-per-event*
  contradicted them and G2 said nothing about their truth. **G2 and G3 are restated**: a refusal
  event truthfully records that NO effect occurred; every event is exactly one touch's or one
  refusal's — never both, never neither.
- **CODEX F2**: a **VIEW is now defined — the display session of one render commit.** Within it,
  dwell and magnification are the same logged view; displaying again after the session ends is a new
  view and a new commit. If the principal means per-glance, the spec says that is his to impose.
- **Your F2/F4 pair**: post-`BS-L` explanations **follow their entries into the continuation** — V70
  sent entries there and left explanations pointing at a sealed surface, the same defect one
  reference deeper.
- **CODEX F3**: the entries, continuation and explanations join the **closed non-χ list as (iv-b)**
  — the pre-unblinding verifier must read them, and explanations are bound by the same
  never-the-object principle as refusal reasons.
- **Your F3/F5 pair**: the §11 item now names **all five gates** — `BS-L`, the lock opening, `BS-7f`,
  `BS-V`, disclosure — and the checker's R08 requires the post-opening passes.
- **CODEX F6**: the key's `operation` component is a token from the **closed BS-2k operation set**.
- **GPT56 F5**: the comparison is **WITHIN-DRAW** — `HELD` iff `verdict(i,j) = verdict(i,0)` for
  every draw and perturbation — because one scalar baseline confounds draw noise with gradient
  sensitivity. `baseline_verdict` is informational: the token when the γ=0 column is constant,
  `PER-DRAW` otherwise; the verifier's replay governs.
- **CODEX F7**: **the verdict-producing computation is bound, not just its output.**
  `counterfactual_path_sha256` enters the schema (**nineteen fields**), and every verdict cell must
  be produced under the **production permutation contract** — `n_perm = 100,000`, the frozen
  stage/trial/address parameters, verifier-replayed. The path had been defaulting to 2,000 with
  caller overrides. A cheaper resolution is a parameter for the principal's sitting, not a default.
- **CODEX F8**: the verifier **refuses any manifest point with `|γ| > gamma_bound`**.
- **GPT56 F4**: the checker's member-parse is **decoration-independent** (with a retirement-line rule
  and an identifier-prefix guard), an affirmative *may-describe-the-object* line is refused even when
  the principle also appears, and the draft's claim about the checker is softened to a lint's true
  strength.
...
- **BREAK THE SPEC — round two. Highest value here.** G2/G3 were restated on your finding; the
...
Write to `gates/V71_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
