# BRIEF — V72 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V72_20260830.md`
**sha256 `66fcc42c6de59cfd8b19397f5bc482f80391fc04f75926cf04b41281ea928979`**
...
**On V72 the lint exits 0 — 97 advisory findings, 0 blocking.** The 96 are citations whose reports
...
Other checkers on V72: counts **16 class P / 8 class E** prose-matched; `prereg_trace` 71 transitions,
0 problems; `void_registry` self-test 6 controls, 0 failures.

## What V72 changed — the derivation became a predicate, and the string rule went global

**V71 was NOT CLEAR ×2** (GPT56 7, CODEX 6), and both F1s were the same sentence: the draft did not
inherit the spec's G2/G3 repair — the mirror of the one-round death the derivation claim was warned
about. **The repair is the mechanism, not the copy:**

- **The derivation is now a CHECK.** `tools/lifecycle_derivation_check.py` (4-control self-test)
  fails when the draft's `lifecycle-spec: sha256` pin goes stale or any quoted `G…/N… —` invariant
  body diverges from the spec's bytes. The draft's duplicated guarantee block is **replaced by
  digest-pinned verbatim quotation**. A spec edit now breaks the pin until the draft re-pins.
  **The control caught a real prose paraphrase of N2 on its first run.**
- **Your F2 (GPT56)**: the spec's G2 gains the **reason-token truth condition** — a request with no
  completed verdict may carry only `REFUSED-UNCLASSIFIED`; a specific code asserts its condition was
  actually established.
- **Your session findings (GPT56 F3 / CODEX F2)**: the session is bounded by **continuous
  uninterrupted display** — position advance, interface clear, or ANY interruption ends it; duration
  alone does not multiply views; nothing displayed after an interruption is the same view.
- **Your explanation findings (GPT56 F4/F5, CODEX F4) — repaired by a GLOBAL rule, not per field:**
  **THE STRING RULE**: every string field in every non-χ artifact is **closed-vocabulary or
  bounded-encoding — no third kind pre-unblinding.** The explanation artifact is redesigned under
  it: a `cause` token from a five-member set plus bounded numerics, **no free prose**; a failure
  inexpressible in the vocabulary **cannot be `EXPLAINED` and must be `NAMED-AS-DEFECT`.** What the
  verifier checks is consistency; what it cannot check is whether the cause is what happened —
  **testimony, signed and named as such.**
- **CODEX F3**: signatures bind a **named signer** — detached over the canonical entry body, under
  the **BS-2k-provisioned enumerator keypair** with roster identity.
- **GPT56 F6**: verifier clause (a) recomputes **all four** module digests.
- **CODEX F5**: the two surviving scalar-baseline clauses are now **within-draw**.
- **CODEX F6 — escalated, not absorbed**: what "the same draw" means across γ (common random
  variates vs independent redraw) is a **preregistration semantics choice** — CLASS-P, **UNSET, a
  blocker**, filed beside `n_draws` because it decides what a draw IS.
- **GPT56 F7**: delete/truncate/metadata mutation land in **no crash-window cell because they are
  FORBIDDEN, not unmodelled** — via Row B a refusal commit; outside Row B a bypass already claimed
  by `VOID-5-FORBIDDEN-ACT`. A wall, not a hole.
- **And the gain gate's FOR is composed in one place**: `HELD` asserts the systematic that actually
  exists cannot have flipped this draw set's verdicts, up to grid resolution, conditional on the
  model — the acceptance region is implied by the bound statement, and neither stands without the
  other's conditions.
...
- **BREAK THE DERIVATION CHECK. Highest value here.** It pins the spec by digest and requires
...
Write to `gates/V72_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
