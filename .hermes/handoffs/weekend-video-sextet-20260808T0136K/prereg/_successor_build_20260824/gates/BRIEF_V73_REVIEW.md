# BRIEF — V73 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V73_20260830.md`
**sha256 `d48c3000aa50d804841f3c170cd660791dc5f3355d7aa682ed33147f6aa3a8ae`**
...
**On V73 the lint exits 0 — 97 advisory findings, 0 blocking.** The 96 are citations whose reports
...
Other checkers on V73: counts **16 class P / 8 class E** prose-matched; `prereg_trace` 72 transitions,
0 problems; `void_registry` self-test 6 controls, 0 failures.

## What V73 changed — the checker went from name to wired predicate, and the rule became a registry

**V72 was NOT CLEAR ×2** (GPT56 5, CODEX 8). V73 was **banked overnight rather than dispatched**, on
instruction; you are its first round.

**1. The derivation checker now does its job** (GPT56-V72 F1/F2, CODEX-V72 F3). It is **wired into
`prereg_lint` as blocking** — it was demonstrated failing on V72's own bytes (stale pin, G6 unquoted)
**before** any repair. It **binds labels** (a quote must match its own spec row; swapped labels fail)
and **detects deletion in both directions** (L04/L05). The session boundary is now **spec row G6**,
and the unlabelled paraphrase you found became a labelled, bound quote.

**2. The universal string rule is an ENUMERATED REGISTRY** (CODEX-V72 F2, GPT56-V72 F3):
`ref/STRING_FIELD_REGISTRY.md`, generated from the schema blocks by
`ref/gen_string_field_registry.py` — 37 fields, each with a constraint, a field with no row
**forbidden by default**, generator nonzero on omission. `explanation_ref` is a **digest-ref** now.
Value-domain enforcement for slot receipts is a written §11 obligation on `receipt_strict()`. The
honest limit is stated: bounded numerics carry bits; the registry bounds capacity and removes free
prose. (The first extractor was digit-blind and dropped all seven sha256 fields — Row F's omission,
committed by the tool built to prevent it, caught by its own stale-row report. It is recorded in the
module.)

**3. ⛔ THE DRAW DISCIPLINE IS FROZEN — DO NOT ATTACK IT THIS ROUND.** Five consecutive rounds of
findings (V68 F7 → V72 F1) put it past the three-round threshold, and the cause is structural: its
defining parameters (`n_draws`, seed, generator, `Δγ`, `k_γ`, bound shape, draw semantics) are
**awaiting the principal**. Until that sitting lands, findings against it are recorded, not patched.
The re-derivable parts were fixed: the third scalar-baseline survivor is within-draw, and
**`NOT-EVALUATED` is deleted as unreachable-by-conformance** (CODEX-V72 F7) — zero-cell receipts are
non-conformant under the schema's own bounds, so the outcome set is `HELD` · `FAILED`.

**4. Normal repairs:** the entry↔emission relation is a **bijection checked both directions**
(GPT56-V72 F4); **existence is not naming** — the re-derivation revision must contain the entry's
`class_key`, adequacy being testimony (CODEX-V72 F4); the replay **asserts in-process** that the
imported `successor_ref_v9` hashes to `6a9abbbd` (GPT56-V72 F5); the retirement exemption is
**negation-guarded** (CODEX-V72 F6); the ledger's "each is a setup error" paragraph is fixed
(CODEX-V72 F8).
...
- **BREAK THE WIRED CHECKER. Highest value here.** It is label-bound, deletion-detecting in both
...
Write to `gates/V73_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
