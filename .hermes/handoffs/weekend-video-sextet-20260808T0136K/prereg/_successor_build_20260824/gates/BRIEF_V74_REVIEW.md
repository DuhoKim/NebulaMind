# BRIEF — V74 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V74_20260830.md`
**sha256 `d229952d5046e9cc3827e81e371b49ef7bcb887daae22c1cef58208f3b243835`**
...
**On V74 the lint exits 0 — 97 advisory findings, 0 blocking.** The 96 are citations whose reports
...
Other checkers on V74: counts **16 class P / 8 class E** prose-matched; `prereg_trace` 73 transitions,
0 problems; `void_registry` self-test 6 controls, 0 failures.

## What V74 changed — your V73 findings, all eleven

**V73 was NOT CLEAR ×2** (GPT56 4, CODEX 7).

- **The registry covers the operative corpus now** (GPT56 F2 / CODEX F1): the generator extracts
  frozen v9's `SLOT_SCHEMA` by AST — **123 fields**, all classified; `BS-1b.provenance` (the
  free-prose field you named) is a **digest-ref**. **Stale rows EXIT NONZERO** (CODEX F4) — a stale
  row is extractor/schema drift, the very signal that caught the digit-blind bug.
- **The derivation checker refuses truncation and double pins** (GPT56 F3 / CODEX F2): quoted bodies
  must equal the FULL spec row (L06 — a true substring can reverse meaning by omission), exactly one
  pin (L07), 9-control self-test. **And the lint no longer silently skips a missing companion**
  (CODEX F5): absence is a blocking `lifecycle-derivation-L00` — unchecked is not passed.
- **The retirement exemption is per-sentence-fragment** (GPT56 F4 / CODEX F6): one token's
  retirement no longer exempts a second active token on the same line. Em-dash is deliberately not a
  splitter — it orphaned a legitimate retired mention from its own retirement two asides later.
- **The delivery-retry allowance is CONVEYANCE-ONLY, stated where the allowance lives** (GPT56 F1 /
  CODEX F7): the unscoped sentence re-authorised a no-new-event render contrary to G5. Render
  buffers do not outlive their commit.
- **The replay's v9 assert runs in a FRESH INTERPRETER** (CODEX F3): a new process hashes the pinned
  file, imports it, computes — no moment in which rebinding code can have run. An in-process assert
  left the file hash true while `perm_record` pointed elsewhere: the V6 monkeypatch seam wearing a
  digest.
...
- **BREAK THE 123-FIELD REGISTRY. Highest value here.** It now claims the operative corpus. **Find
...
Write to `gates/V74_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
