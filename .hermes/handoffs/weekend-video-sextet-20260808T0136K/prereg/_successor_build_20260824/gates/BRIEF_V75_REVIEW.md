# BRIEF — V75 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V75_20260830.md`
**sha256 `781b7f3f065ff20dc2cbee1ec4bf5bde944cfe3a85ffe75f5df2a83fe0e69054`**
...
**On V75 the lint exits 0 — 97 advisory findings, 0 blocking.** The 96 are citations whose reports
...
Other checkers on V75: counts **16 class P / 8 class E** prose-matched; `prereg_trace` 74 transitions,
0 problems; `void_registry` self-test 6 controls, 0 failures.

## What V75 changed — your nine, and one recurring class killed by mechanism

**V74 was NOT CLEAR ×2** (GPT56 4, CODEX 5).

- **The registry reaches 138 fields** (your F1 pair): v9's receipt **envelope and environment**
  fields extracted from the constructor itself, the **nine declared non-slot artifact classes** as
  digest-refs to their per-class schemas, and **the enumerator signature itself — constrained to a
  deterministic scheme mandated at BS-2k**, because a valid ECDSA signature still carries a nonce: a
  covert channel wearing a validity proof.
- **The replay interpreter is ISOLATED, not merely fresh** (GPT56 F2 / CODEX F4): `python -I -S`,
  cleared environment, pinned cwd — and **the trust boundary is stated**: the interpreter binary and
  OS are trusted by declaration, exactly as for v9's own runtime. Isolation removes every rebinding
  vector that rides configuration rather than ownership.
- **G5/G6 composition repaired both ways** (GPT56 F3 / CODEX F2): a render buffer lives **until its
  view session ends** (dying with its commit made post-commit delivery impossible), and **the sealed
  interface holds no redisplayable surface beyond the live session** — occlude-and-restore must
  produce a second committed render event or no image, a BS-2k fixture.
- **A retiring fragment may retire at most one token** (CODEX F3) — the em-dash mirror of V73's fix.
- **The stale-digest class is DEAD BY MECHANISM** (your F4/F5 pair): after the third manual
  violation of compute-it-last, `prereg_lint` now recomputes the live checker's sha256 and blocks on
  mismatch with the draft's claim.
...
- **BREAK THE 138-FIELD REGISTRY. Highest value here.** Envelope, environment, signatures and
...
Write to `gates/V75_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
