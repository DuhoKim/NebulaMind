# BRIEF — V76 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V76_20260830.md`
**sha256 `2aa58d40bfedfc701f7e951eec16c6e9c0753b889cced73d905e9821407469b9`**
...
**On V76 the lint exits 0 — 97 advisory findings, 0 blocking.** The 96 are citations whose reports
...
Other checkers on V76: counts **16 class P / 8 class E** prose-matched; `prereg_trace` 75 transitions,
0 problems; `void_registry` self-test 6 controls, 0 failures.

## What V76 changed — your ten, with two honesty conversions

**V75 was NOT CLEAR ×2** (GPT56 4, CODEX 6).

- **The environment's six leaves are in the registry, honestly split** (GPT56 F1 / CODEX F2):
  three pinned by `require_environment` (a closed vocabulary of one frozen value each), and three —
  `python`, `platform`, `machine` — **RECORDED-UNPINNED: interpreter text refused by nothing, a real
  residual surface named rather than dressed as closed.** The container row is a digest-ref over the
  leaves.
- **The nine non-slot rows stop being pseudo-fields** (GPT56 F2 / CODEX F1): three classes are
  **inventoried field-by-field** in the registry itself (the event rows, the enumeration surface, the
  projection bits); six are **honest `SCHEMA-PENDING` stubs** — fields unenumerable until their
  defining slots fill, and the row says so instead of wearing a constraint it does not have.
- **The replay bootstrap is pinned end-to-end** (GPT56 F3 / CODEX F4): interpreter by **absolute
  path recorded in BS-7p's environment** (a bare `python` is PATH-rebindable before isolation
  begins), explicit `sys.path` to pinned dependency roots (`-S` skips site and NumPy does not import
  from nowhere), the roots joining the frozen environment `require_environment` checks.
- **G6 gains one-to-one commit↔session ownership** (CODEX F5): each render commit opens at most one
  session; every session is opened by exactly one commit.
- **The registry records its generating draft and the lint blocks on mismatch** (CODEX F3).
- **Retirement exempts only KNOWN retired tokens** (GPT56 F4): a novel token cannot buy legality by
  standing next to the word "deleted" — retirement is a fact about this vocabulary's history, not a
  property of nearby prose.
- **Ledger: `require_complete_sample`'s raise moved INTEGRITY → CALLER** (CODEX F6) — §5's own
  recorded limit had said "two caller-supplied integers" for four revisions while the ledger
  contradicted it.
...
- **BREAK THE HONESTY CONVERSIONS. Highest value here.** Three environment leaves are declared
...
Write to `gates/V76_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
