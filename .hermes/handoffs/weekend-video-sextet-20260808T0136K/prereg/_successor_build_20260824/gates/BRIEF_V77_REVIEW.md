# BRIEF — V77 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V77_20260830.md`
**sha256 `d2d61a274c8c0739b4cd1b597265f7d7a1580d19a150b420235af9ca7901cfee`**
...
**On V77 the lint exits 0 — 97 advisory findings, 0 blocking.** The 96 are citations whose reports
...
Other checkers on V77: counts **16 class P / 8 class E** prose-matched; `prereg_trace` 76 transitions,
0 problems; `void_registry` self-test 6 controls, 0 failures.

## What V77 changed — your seven, the narrowest round yet

**V76 was NOT CLEAR ×2 at SEVEN findings total (GPT56 4, CODEX 3) — the lowest of the run.**

- **The bound exists now** (GPT56 F1 / CODEX F2): the three value-unpinned environment leaves carry
  **printable ASCII ≤ 64 bytes, refused by the successor-layer envelope verifier** — unpinned in
  VALUE, bounded in ENCODING; the constraint column's claim and the note's admission are both true.
- **The counts are the generator's** (GPT56 F3 / CODEX F1): V76 hand-wrote "nine classes, six
  pending"; the generator says **ten and seven** — the hand-copied-count drift on the surface built
  while writing about that drift. Quoted from `ref/_registry_counts.txt` now. **And the declared
  canonical bodies join the registry**: freeze-signature body, lock body, opening authorization,
  entry body, explanation body, provenance record — **151 fields.**
- **The bootstrap pins are RECEIPTABLE** (GPT56 F2): BS-7p's `environment` carries a canonical
  sub-schema — `interpreter_path`, `interpreter_sha256`, `dependency_roots` as ordered (path,
  digest) pairs — with a separately pinned verifier recomputing from disk.
- **Reactivation defeats the exemption** (GPT56 F4 / CODEX F3): *"REFUSED-X was deleted but is
  hereby reinstated"* rode the retirement word; an activation word anywhere in the fragment now
  makes the token illegal. 25 controls.
...
- **BREAK THE ENCODING BOUND. Highest value here.** Printable ASCII ≤ 64 bytes on three
...
Write to `gates/V77_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
