# BRIEF — V78 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V78_20260830.md`
**sha256 `b4a9c69d9389e66202bdfa3fe41468dc7af14ca993ce3f3fcbb64f92cc70cb5d`**
...
**On V78 the lint exits 0 — 97 advisory findings, 0 blocking.** The 96 are citations whose reports
...
Other checkers on V78: counts **16 class P / 8 class E** prose-matched; `prereg_trace` 77 transitions,
0 problems; `void_registry` self-test 6 controls, 0 failures.

## What V78 changed — all sixteen, with the closure boundary finally said out loud

**V77 was NOT CLEAR ×2** (GPT56 11, CODEX 5 — GPT56 went a level deeper on the newest mechanisms).

- **BS-7p's sub-schema fields join the registry** (GPT56 F1 / CODEX F2): `interpreter_path` bounded
  (absolute POSIX path, ≤256 printable-ASCII bytes, no traversal), digests as digest-refs, and
  `BS-7p.environment`'s stale closed-vocab label corrected to digest-ref.
- **The `dynamic_load_manifest` joins BS-7p** (GPT56 F2 / CODEX F3): the linker-resolved transitive
  closure of shared objects, ordered (path, digest) pairs at freeze, verifier-recomputed. **And the
  boundary is said out loud: above the manifest sit the linker itself and the kernel — each closure
  level names the next as trusted, and this one is where the naming stops.**
- **`canonical.provenance_record` is an honest SCHEMA-PENDING row** (GPT56 F3 / CODEX F1) — it had
  been force-added as digest-ref with no written encoding, the SCHEMA-PENDING defect wearing a
  canonical name. The **explanation body's field order is now written**, so that digest-ref has a
  real preimage.
- **`timestamp` has a bound** (GPT56 F4): ISO-8601 UTC, exactly 24 bytes.
- **Reason-token truth says what can check it** (GPT56 F5): chain-recomputable conditions are
  audit-checkable; availability codes are **testimony under Row B's signature, named as such** —
  detectable in one class, attributable in the other, silent in neither.
- **A no-session render buffer dies at request end** (GPT56 F6). **A digest-equal re-derivation
  discharges nothing** (GPT56 F7). **The cause set is an ordered precedence, not a menu** (GPT56
  F8) — mandatory highest-applicable kills the five-way selection channel. **The manifest's byte
  serialization is written** (GPT56 F10). **The last hand counts are stripped** (F11/F5).
- **The activation guard scans a 160-char window after the token** (GPT56 F9 / CODEX F4):
  "now mandatory" rode a pronoun across a fragment; whole-line scanning false-fired on a legitimate
  mention. Member tombstones are divergences. **The finite word list's limit is stated in the
  checker rather than papered over.** 27 controls.
...
- **BREAK THE CLOSURE BOUNDARY STATEMENT. Highest value here.** The dynamic-load manifest stops at
...
Write to `gates/V78_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
