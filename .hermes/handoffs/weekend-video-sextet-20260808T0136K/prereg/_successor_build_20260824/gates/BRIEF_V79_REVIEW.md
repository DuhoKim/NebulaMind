# BRIEF — V79 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V79_20260830.md`
**sha256 `01d3877a2973fff901b16d0668bb0438e7cc42ca6ae0ea35429f65c474972561`**
...
**On V79 the lint exits 0 — 97 advisory findings, 0 blocking.** The 96 are citations whose reports
...
Other checkers on V79: counts **16 class P / 8 class E** prose-matched; `prereg_trace` 78 transitions,
0 problems; `void_registry` self-test 6 controls, 0 failures.

## What V79 changed — your eleven, and one honest residual named

**V78 was NOT CLEAR ×2** (GPT56 5, CODEX 6).

- **The manifest is enforced at replay END** (your F1 pair): after the verdict computes, the process
  enumerates its actually-loaded objects and **refuses its own result** if any is outside the
  manifest; the frozen reference and path contain **no dynamic-load constructs, auditable once by
  AST** — so a late load is a violation, never a need.
- **The container entries join the registry** (CODEX F2): `roots_entry`/`dlm_entry` paths bounded to
  the 256-byte POSIX form — unbounded leaves had been hiding behind bounded wrappers.
- **Per-cause CLOSED parameter schemas** (GPT56 F2): five parameter names total, arity exactly as
  declared per cause — names and arity had been free strings, the string rule's own surface one
  level down.
- **The applicability channel is a NAMED RESIDUAL** (CODEX F3): which causes apply can correlate
  with what the process touched — at most log₂5 bits per emission, bounded by the
  enumerated-defect review that already exists. **The price of the token having diagnostic content
  at all, like N3 — stated, not claimed away.**
- **The checkability split is per-code with its consumer named** (GPT56 F4 / CODEX F5):
  authorisation codes recomputable; **`OBJECT-ABSENT` falsifiable-from-history** (a prior committed
  touch contradicts it); UNREADABLE/INCOMPLETE/INTEGRITY-MISMATCH testimony; **the §11 audit pass
  recomputes every checkable condition for EVERY specific refusal at the same five gates.**
- **ONE canonical byte encoding for every canonical body** (GPT56 F3 / CODEX F6), defined by frozen
  v9's `field()` constructor in each body's declared order; the opening authorization's order is
  declared, closing its digest-without-preimage.
- **The naming is a checkable TEMPLATE** (CODEX F4): `NAMES-CLASS: <key> AS <token>` with membership
  checked — a parse instead of a hope.
- **Future-tense reactivation forms join the guard** (GPT56 F5). 28 controls.
...
- **BREAK THE REPLAY-END ENFORCEMENT. Highest value here.** The process refuses its own result if
...
Write to `gates/V79_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
