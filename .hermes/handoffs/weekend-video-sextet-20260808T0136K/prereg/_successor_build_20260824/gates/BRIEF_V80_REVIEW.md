# BRIEF — V80 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V80_20260830.md`
**sha256 `a9d5d0a2214fe4b16d15a4821d2463209b2105424eb2ce9b84933769e2656edc`**
...
**On V80 the lint exits 0 — 97 advisory findings, 0 blocking.** The 96 are citations whose reports
...
Other checkers on V80: counts **16 class P / 8 class E** prose-matched; `prereg_trace` 79 transitions,
0 problems; `void_registry` self-test 6 controls, 0 failures.

## What V80 changed — two concessions checked and made, and your eleven closed

**V79 was NOT CLEAR ×2** (GPT56 4, CODEX 7), with your first four findings verbatim-convergent — and
**the two biggest were you catching me in false claims, both verified against the bytes before
repair:**

- **The no-importlib claim was FALSE** (your F1 pair): v9's `_frozen_planner()` loads the frozen
  cutout planner via `importlib.util` at L269–277. The corrected statement is the **checked** one:
  exactly one dynamic-load site, loading a module that pins its own adapter digest, **unreachable
  from the verdict path** — the call graph from `run_production_verdict` reaches neither
  `_frozen_planner` nor `frozen_plan_object` (AST-verified at V80), and the counterfactual path
  imports neither `importlib` nor `ctypes`. The load-then-unload gap closes **by composition**:
  under `-I -S` with a cleared environment, the only code running before the end-snapshot is the
  pinned code the AST audit covers — an unload needs an unloader, and there is nowhere for one to
  live.
- **V79 invented a second opening-authorization body** (your F2 pair) for an object Clause 6 had
  already bound. **One object, one body: the V79 declaration is WITHDRAWN**; the canonical order is
  Clause 6's eight-field enumeration.
- **Presence is monotone, so the history rule is a DISJUNCTION** (your F3 pair): a prior touch +
  later `OBJECT-ABSENT` proves *either the code is false or a forbidden removal occurred* — both
  audit-findings; the audit surfaces the contradiction rather than adjudicating it.
- **The canonical-body leaves join the registry** (your F4 pair): Clause 6's eight fields, the
  freeze body's five — **177 fields.**
- **`NAMES-CLASS` binds the token's own definition to the key** (CODEX F5). **Payload encodings
  under `field()` are declared** (CODEX F6) — raw/UTF-8/decimal/hex/canonical-JSON, v9's own
  envelope convention. **Three more activation forms** (CODEX F7), 29 controls.
...
- **BREAK THE COMPOSITION ARGUMENT. Highest value here.** The claim: one dynamic-load site,
...
Write to `gates/V80_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
