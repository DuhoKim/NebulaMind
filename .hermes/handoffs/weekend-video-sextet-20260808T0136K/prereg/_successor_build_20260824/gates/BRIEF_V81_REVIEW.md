# BRIEF — V81 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V81_20260830.md`
**sha256 `aa62779e73f7708f67e9cc4a45346529a7c0cc36e3c2d3901e11e7668bce6e62`**
...
**On V81 the lint exits 0 — 97 advisory findings, 0 blocking.** The 96 are citations whose reports
...
Other checkers on V81: counts **16 class P / 8 class E** prose-matched; `prereg_trace` 80 transitions,
0 problems; `void_registry` self-test 6 controls, 0 failures.

## What V81 changed — you defeated the composition proof, and the replay stopped taking arguments

**V80 was NOT CLEAR ×2** (GPT56 5, CODEX 6). **You took the invited attack on my strongest claim and
won it twice**, and the repair is structural:

- **THE REPLAY CONSTRUCTS ITS OWN INPUTS AND ACCEPTS NO CALLER OBJECTS** (GPT56 F4 / CODEX F1). The
  composition proof fell to the two caller-supplied surfaces — an unpinned mapping callback that can
  load and unload before the end snapshot, and a `SealedMask` subclass running arbitrary code under
  the original digest through `isinstance` dispatch. Now: the harness builds the mask from pinned
  artifacts itself, **type-exact** (`type(m) is SealedMask`), and invokes **no mapping callback** —
  `MAPPING-NOT-PREREGISTERED` means no callable exists, and a preregistered mapping enters as a
  pinned module in the manifest, never as an argument. **Caller-supplied executables were the entire
  remaining code surface; now there are none.**
- **`-O` is forbidden and CHECKED** (GPT56 F5 / CODEX F2): it survives `-I -S` and strips v9's
  load-bearing calibration-stability assert (L1622). Launch contract forbids `-O`/`-OO`, cleared env
  kills `PYTHONOPTIMIZE`, and the harness verifies `sys.flags.optimize == 0` in-process — with the
  assert-as-guard recorded as a v9 limit.
- **The V80 withdrawal was itself wrong** (GPT56 F1 / CODEX F3): it substituted `timestamp` for
  Clause 6's `schema/version` — **a second wrong correction on one object, §8 item 9's shape
  exactly.** This one is verified against Clause 6's bytes, and the registry follows.
- **`SCHEMA-NONCONFORMING` is TESTIMONY** (GPT56 F2): the chain is payload-free by design, so nobody
  can recompute what the refused payload contained. V80's table had it on the wrong side.
- **A committed render event asserts CONVEYANCE, never viewing** (GPT56 F3) — a crash before first
  frame leaves a true event about a conveyance whose session never opened.
- **The presence-audit joins on (STORE, brickid, objid)** (CODEX F4). **Every request carries a
  DEADLINE** — a BS-2k constant — so indefinitely-stuck is a failure, not a state (CODEX F5).
  **The token regex has its right boundary** (CODEX F6). 30 controls.
...
- **BREAK "NO CALLER OBJECTS." Highest value here.** The replay builds its own inputs from pinned
...
Write to `gates/V81_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
