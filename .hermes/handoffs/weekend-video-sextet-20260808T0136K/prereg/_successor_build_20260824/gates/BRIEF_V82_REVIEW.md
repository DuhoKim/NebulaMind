# BRIEF — V82 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V82_20260830.md`
**sha256 `12d54356b4fde6b0dec0919a13f7af65f34a1927a9c1984e427e3401a93ed5ad`**
...
**On V82 the lint exits 0 — 97 advisory findings, 0 blocking.** The 96 are citations whose reports
...
Other checkers on V82: counts **16 class P / 8 class E** prose-matched; `prereg_trace` 81 transitions,
0 problems; `void_registry` self-test 6 controls, 0 failures.

## What V82 changed — one mechanism kills three attacks, and one collision goes up

**V81 was NOT CLEAR ×2** (8 + 8). **One finding was a genuine collision between two principal
rulings and is FILED, not repaired** (CODEX F6): naming a recurring catch-all class requires
re-deriving the vocabulary, and §6.3's void rule voids any post-χ change to a binding rule — the
only exit keeping both rulings is that a post-χ recurring class **terminates the run**, and that is
his to decide (`OPEN_QUESTION_VOCAB_REDERIVATION_VS_VOID.md`). **Do not re-derive it; attack the
options if you can.**

The fifteen mine:
- **COMPILE-FROM-VERIFIED-BUFFER — one mechanism, three kills** (GPT56 F1, CODEX F2, CODEX F3): the
  harness reads v9 and the path once into memory, hashes those buffers, and
  `compile(buffer, optimize=0)`-executes into fresh namespaces. No import machinery → `-X
  pycache_prefix` bytecode substitution dies; no second read → the verified/consumed swap window
  dies; optimize is an argument we set → a stripped-assert code object dies. Roots still travel
  imports, so their digests are **re-verified after computation**.
- **`replay_harness_sha256` enters the schema — twenty fields** (CODEX F1): the harness carrying
  every replay obligation was the one executable left outside the receipt.
- **The deadline moved into the spec** (GPT56 F2 / CODEX F4) — §3b, monotonic clock, fixed at
  receipt, never reset. The single-home rule had been violated by the very repair extending the
  lifecycle.
- **Render is a distinct kind, one event of one kind** (GPT56 F3); **the operation vocabulary is
  STORE-QUALIFIED** so (row, operation) determines the store (GPT56 F4 / CODEX F5); **the detached
  signature fields join the registry** (GPT56 F5); **canonical JSON is made unique** — NFC, minimal
  escapes, integers only, floats forbidden in structured payloads (F7 pair); **required/
  authoritative join the activation list** (GPT56 F6), 31 controls; **the historical count sentence
  says it is historical** (GPT56 F8); **the registry header states its two provenances plainly** —
  extracted sets and declared sets (CODEX F8).
...
- **BREAK COMPILE-FROM-BUFFER. Highest value here.** The two pinned modules never touch the import
...
Write to `gates/V82_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
