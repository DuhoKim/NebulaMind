# BRIEF — V83 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V83_20260830.md`
**sha256 `7bc71ce4787f1d81aa06b3537260969aad3084aa9a7a4c94d733dfed690f1d30`**
...
**On V83 the lint exits 0 — 97 advisory findings, 0 blocking.** The 96 are citations whose reports
...
Other checkers on V83: counts **16 class P / 8 class E** prose-matched; `prereg_trace` 82 transitions,
0 problems; `void_registry` self-test 6 controls, 0 failures.

## What V83 changed — your ten, led by the buffer's own first line

**V82 was NOT CLEAR ×2** (GPT56 3 — the lowest single-seat count of the run — CODEX 7).

- **The path buffer's first line defeated the mechanism built around it** (your convergent F1):
  `import successor_ref_v9` re-entered the import machinery. **PRE-BINDING is the repair**: the
  harness executes the verified v9 buffer first, registers it as
  `sys.modules['successor_ref_v9']`, and the path's import binds to the already-verified in-memory
  module, touching no disk.
- **The harness pin has an INDEPENDENT expected value** frozen in the preregistration (CODEX F2) —
  the who-checks-the-checker regress stops at the freeze, where every pin already stops. Clause (a)
  recomputes **five** digests (the fifth joined while the clause said four — the one-revision lag a
  third time, and it is what the round is for).
- **The enumeration surfaces have a TEMPORAL PARTITION** (CODEX F3): a continuation entry may join
  only post-`BS-L` events; a claimed pre-`BS-L` `chain_position` is refused — no retroactive healing
  of a lock-time failure.
- **The COMMIT is bounded in the spec** (CODEX F7): the domain aborts a commit exceeding its BS-2k
  bound, and the aborted request receives the deadline refusal — abort-then-refusal, never a wait.
- **Cross-run recurrence has teeth** (GPT56 F3): a class explained in ANY prior run is RECURRING at
  the successor's freeze review — once-per-run pacing cannot explain a class forever.
- **The five parameters have real ranges** (GPT56 F2); **lowercase escape hex and post-NFC key
  collisions refused** (CODEX F4); **`lockbody.bound_digests` points at clause 3(a)'s enumeration —
  one enumeration, not two** (CODEX F5); **R08 matches negated FORMS** after two proximity attempts
  false-fired on neighbouring clauses (CODEX F6), 32 controls.
...
- **BREAK PRE-BINDING. Highest value here.** v9 executes first and is registered under its name;
...
Write to `gates/V83_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
