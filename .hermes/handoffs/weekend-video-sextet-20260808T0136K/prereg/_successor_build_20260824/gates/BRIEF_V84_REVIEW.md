# BRIEF — V84 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V84_20260830.md`
**sha256 `6ec2bc2bdabcd12c4d292d33fce867dcc482a29e4bf874c16545f7105a1c95f6`**
...
**On V84 the lint exits 0 — 97 advisory findings, 0 blocking.** The 96 are citations whose reports
...
Other checkers on V84: counts **16 class P / 8 class E** prose-matched; `prereg_trace` 83 transitions,
0 problems; `void_registry` self-test 6 controls, 0 failures.

## What V84 changed — your seven convergent pairs, one of which caught the lesson-row lying

**V83 was NOT CLEAR ×2 at 7+7, fully verbatim-convergent.**

- **The lock-body pointer said 3(a); the body lives in 3(b)** (F6) — the unverified-pointer defect
  inside the row written as that lesson. Read this time: **twelve leaves plus `signer_identity`,
  enumerated in the registry and CROSS-CHECKED against the clause text at generation** — and the
  cross-check caught my hand list missing `signer_identity` before it shipped.
- **Checked declarations** (F7): the generator now extracts Clause 6's and clause 3(b)'s own phrases
  and fails on a missing or stranger member. Declared sets that remain unchecked say so.
- **The harness is a NAMED artifact** (F1): `gates/replay_harness.py`, REQUIRED in §11, its sha256 a
  class-P value **set when built, frozen at freeze — UNSET now, one more blocker.** A pin without an
  artifact was a name; a named artifact with an unset pin is a blocker.
- **The loader's read window joins the trust declaration by name** (F2): no userland check closes a
  race against the loader it runs on; an adversary who can race the loader owns the machine — which
  the declaration already concedes for linker and kernel.
- **The issuance boundary is a point, not a window** (F3): checkpoint-seal-through-issuance is ONE
  serialised step of the single writer; no event can be appended during it by construction.
- **The recurrence domain is the LISTED predecessors** (F4): a run that died before recording its
  failures contributes nothing — a named limit, because such a run cannot teach.
- **The lifecycle is total over the states the platform serves** (F5): storage that never returns is
  the platform failing — N2's sibling, ended by operator observation.
...
- **BREAK THE CHECKED DECLARATIONS. Highest value here.** The cross-check extracts clause phrases
...
Write to `gates/V84_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
