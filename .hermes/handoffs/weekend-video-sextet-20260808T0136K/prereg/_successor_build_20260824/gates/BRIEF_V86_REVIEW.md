# BRIEF — V86 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V86_20260830.md`
**sha256 `25cfb64cd69d0d86915ad1b9635bcdc55ea288c3e4144589669842412829457e`**
...
**On V86 the lint exits 0 — 97 advisory findings, 0 blocking.** The 96 are citations whose reports
...
Other checkers on V86: counts **16 class P / 8 class E** prose-matched; `prereg_trace` 85 transitions,
0 problems; `void_registry` self-test 6 controls, 0 failures.

## What changed since your last round — SEVEN PRINCIPAL RULINGS, then your twelve

**Read this section carefully: the ground moved.** Between your V84 round and this draft, the
principal ruled on every open item (2026-08-30 10:46), and V85 applied all seven:

1. **The VOID/numerical overlap is REAL** (option c, superseding make-them-disjoint): a
   computed-then-sealed artefact carries two failure meanings, and an **open ORDERING rule** decides
   by what the failure demonstrates — impeached record → VOID; unresolved arithmetic → inconclusive;
   both → VOID.
2. **BS-3g is fully parameterised**: `n_draws = 99`; **a-priori frozen bound** (endpoints ±0.25
   proposed, awaiting ratification; `k_γ` moot); **common random variates**; seed/generator/Δγ
   committed blind in `ref/DRAW_MECHANICS_COMMIT_20260830.md`. **The draw discipline is UNFROZEN —
   it is attackable again, against a complete spec.**
3. **`REFUSED-INTEGRITY-MISMATCH`: the refusal owns it** — log and continue; every mismatch
   enumerated at freeze; an unexplained one BLOCKS the freeze; the digest-deviation VOID antecedent
   scoped by the ordering rule.
4. **The principle is REBUILT**: storage state allowed, content-derived forbidden, safe only under
   the precommitted χ-blind schedule.
5. **The WRITE-AHEAD ARRIVAL RECEIPT exists** — a second event class, authorised; no real request
   can vanish; **N2 is RETIRED** (the spec table keeps the record).
6. **A post-χ recurring catch-all class TERMINATES the run** as
   `TERMINATED-UNNAMEABLE-REFUSAL-CLASS` — a third ending, neither VOID nor INCONCLUSIVE.
7. **Strata option A**: **Row D2** (may read χ) produces the sealed stratum-index artifact — **slot
   BS-SI, class counts 16/8 → 17/8** — Row F consumes it, its void clause amended; the typed barrier
   keeps stratum data out of `calibration_bins()`.

**And your twelve V84 findings**: two were photographs of the mid-application state (the
half-migrated spec, the stale digest) — resolved by V85's committed form; the live ten are repaired:
issuance is a **commit** (a half-issued lock cannot exist); the predecessor listing is **rule-bound
over the shared `PINNED_PARENT_SHA256`** with its enforcement limit named; the freeze-body
crosscheck exists and Clause 6 has **stranger detection**; `baseline_verdict` carries **production
verdict tokens** (the matrix's cells are run verdicts — `HELD`/`FAILED` is the comparison's outcome,
not a cell's); the generator takes the draft path as an argument; `calibration_sha256`'s byte
encoding is stated; the four pipe-broken registry rows are sanitized; *it-controls* joined the
activation guard.
...
- **ATTACK THE RULING APPLICATIONS. Highest value here.** Seven rulings entered the text in one
...
Write to `gates/V86_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
