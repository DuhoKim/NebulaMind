# BRIEF — V87 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V87_20260830.md`
**sha256 `700fd8eb810c46c6403af44b6ce799c2923bf73aab69367df6569b1adc7b645c`**
...
## What V87 changed — the application gaps you found, closed; one count corrected

**V86 was NOT CLEAR ×2** (GPT56 6, CODEX 10) — you did what the brief asked and found the ruling
applications that fell short.

- **Row D2 has its VOID antecedents** (`VOID-6.1D2-OUTSIDE-STORE`, `-LATE-WRITE`, `-REACHES-BINS`)
  and **`void_registry.py`, which REFUSED V86, is green and back in the battery** (your F1 pair).
- **BS-SI is CLASS E, and the count correction is reported: 16/9, not 17/8** (CODEX F2) — my class-P
  label violated §0's own definition (a class-P slot holds its receipt BEFORE the freeze; a
  χ-derived artifact cannot exist until P2). My implementation's error, not the ruling's.
- **N2's retirement propagated** (GPT56 F2 / CODEX F3): the spec's W1 row shows the arrival event,
  the §6 leftover and the terminal-treatment sentence updated. The quote checker stayed green
  through this because the stale text was unlabelled prose — its stated blind spot, exercised.
- **The arrival event has an authenticated schema and an ON-CHAIN `request_key`** joining each
  arrival to exactly one terminal event (GPT56 F3 / CODEX F4), the binding→key map committed in the
  domain and exposed at checkpoint.
- **The stale never-describe-the-object text is gone from the (iv-b) site and the checker
  docstring** (GPT56 F4 / CODEX F6).
- **The draw block no longer contradicts itself** (GPT56 F5 / CODEX F8): UNSET/EMPTY claims updated
  to the ruled/committed values; the superseded measurement-derived-bound passages marked HISTORY
  with the shape choice recorded as decided.
- **`TERMINATED-UNNAMEABLE-REFUSAL-CLASS` has its producer** (GPT56 F6): the enumeration verifier at
  whichever gate detects the recurrence — the refusal IS the production.
- **The issuance commit's own writes are sealed-side by construction** (CODEX F5). **The vocabulary
  label reads RESOLVED** (CODEX F7). **The manifest is the GRID BY CONSTRUCTION** — recomputed from
  the ratified endpoints and committed Δγ, nothing left for a producer to choose (CODEX F9). **The
  committed seed rule is zero-based, amended in the commit artifact per its own post-hoc rule**
  (CODEX F10).
...
- **ATTACK THE PROPAGATION SWEEP. Highest value here.** Three rulings needed second-site sweeps
  this round (N2, the principle, the draw block). **Find the fourth stale site** — any sentence
  still describing a pre-ruling state as live: the old bound formula quoted as operative, N2 as a
  residue, flagged-mismatch language, a class count in prose.
...
- **Attack the count correction.** 16/9 now. **Verify every prose statement of the counts agrees**,
  including the §10 rows' row-count column. Seven rulings entered the text in one
