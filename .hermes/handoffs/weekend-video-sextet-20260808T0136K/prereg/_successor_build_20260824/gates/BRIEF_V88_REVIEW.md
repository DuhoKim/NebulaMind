# BRIEF — V88 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V88_20260830.md`
**sha256 `1a7810442b6d774301c840487ec271b095512b85311745d93c2fdc0fd3cc235a`**
...
**Last refereed draft: V87** `700fd8eb810c46c6` — **NOT CLEAR from both seats** (GPT56 7,
...
## What V88 changed — THE SUPERSESSION SWEEP, one event repaired as a set

**V87 was NOT CLEAR ×2** (GPT56 7, CODEX 6) and six of the findings were pairs: A MORNING RULING
LANDED AND ITS PREDECESSOR TEXT DID NOT DIE. Per the principal's relay these were not repaired as
point fixes. For each ruling of 2026-08-29/30 the old regime was **grep-enumerated across the
draft, the spec, the registry generator AND THE CONTROLS**, then killed or re-derived with the
ruling cited on each site:

- **G4 restated over the arrival class** (your F1 pair): the spec row now counts DECISION events —
  a touch commit or a refusal commit — with arrival its own class; the draft's label-bound quote is
  **derived from the swept row by the checker's own extraction regex, not retyped**.
- **The old principle tail is dead** (F2 pair): the PRINCIPLE-STILL-BINDS site now states the
  rebuilt storage-state/content-derived form.
- **The two live-N2 corners are dead** (F3 pair): the death-before-commit corners now cite the
  arrival event; the spec's N2 row is a retirement record.
- **The registry generator's four old-state notes are re-derived** (F4 pair): n_draws RULED = 99,
  generator one-member committed blind, seed COMMITTED = 20260830, γ bound a-priori with k_γ moot;
  an ARRIVAL field set is declared, `request_key` = the arrival event's own chain position.
- **F5/F6 (stale count prose; arrival fields): the sweep's grep found ZERO live sites** — V87's
  correction had already propagated. Recorded as verified-clean, not silently skipped. **Attack
  this claim if you can find the site the grep pattern missed.**
- **The freeze-time mismatch guard is re-anchored** (GPT56 F7): to the five-gate machinery and the
  successor-facing freeze review — a freeze-time enumeration cannot police run-time emissions, and
  the text no longer pretends it can.
- **THE SWEEP'S OWN CATCH — the eighth stale site was a CONTROL.** `refusal_vocabulary_check.py`'s
  R03 still *required* the pre-ruling never-the-OBJECT sentence and had stayed green off the dead
  draft tail: the stale control and the dead text were masking each other. R03 is rebuilt to three
  prongs (storage-state permission, content-derived ban, χ-blind-schedule dependency), the fixture
  rebuilt with it, 34 controls green. **The §10 row for V86 → V87 now carries the count move
  (class-P 17 → 16, class-E 8 → 9) in the generated row-count column.**
...
- **ATTACK THE SWEEP'S COMPLETENESS. Highest value here.** The sweep grep-enumerated the old
  regime and found an eighth site (a stale CONTROL) its own re-grep surfaced. **Find the ninth** —
  any sentence, table row, note, fixture or REGEX in the draft, the spec, the registry generator or
  the tools that still encodes a pre-ruling state as live: an old-form phrase a paraphrase kept
  alive past the grep patterns, a control that passes for the wrong reason, a fixture asserting the
  dead regime.
...
Write to `gates/V87_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
