# BRIEF — V89 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V89_20260830.md`
**sha256 `c608ca9a798e157151c869653089ca0dc844a361456f81a07ceecc92ba1f50e6`**
...
**Last refereed draft: V88** `1a7810442b6d7743` — **NOT CLEAR from both seats** (GPT56 8,
...
## What V89 changed — ARRIVAL integrated as ONE pass, the sweep now a computed receipt

**V88 was NOT CLEAR ×2** (GPT56 8, CODEX 4). The event existed; the exhaustive partitions did not
admit it. V89 is the integration pass, plus the two real design items:

- **G3 is a THREE-WAY partition** (your F1): every committed event is exactly one of ARRIVAL, a
  touch's, a refusal's — no event two, no event none; G2 now claims the arrival's truth too. Both
  draft quotes re-derived label-bound from the swept rows.
- **The ARRIVAL schema is item (ii-b) of the exhaustive non-χ list** (F2 pair) — at the list, value
  domains bound to BS-2k; the registry's six `arrival.*` rows cite item (ii-b) as source (they
  shipped citing v9's slot schema, which never held an arrival); `access_log_chain`'s inventory
  names both event classes; Row B's void column now sanctions an arrival left unappended.
- **The request/recovery block is REPLACED, not tail-patched** (F3 pair): visible-not-vacuous
  death corner, two-identities rule, residue re-drawn at the pre-arrival wire, the what-changed
  ledger naming both authorised log changes; spec N2's why-cell no longer refuses what its body
  retired; W1 resumes-to-one-decision.
- **Exactly-one-terminal has its EXISTENCE half** (F4 pair): spec §3b names PENDING (arrival
  committed, no decision, deadline unexpired); recovery drives open arrivals to their one decision
  via the normal path or the deadline refusal; the verifier refuses past-deadline zero-terminal
  arrivals at ANY gate and any zero-terminal arrival at disclosure; §11's item carries the full
  contract and six fixtures (GPT56 F5).
- **BS-L has TWO CUTS, named once** (F6/CODEX F4): the signed-checkpoint cut (clause 4's, before
  canonicalization — nothing after it in the signed body) and the partition cut (issuance-commit
  end). Issuance's own writes lie between; no enumerable event can exist there — a refused
  issuance is no issuance. The extends-through-issuance sentence is dead. Row L's phase set
  P0/P6/P7 untouched.
- **THE SWEEP IS NOW A RECEIPT, NOT A CLAIM.** `ref/gen_sweep_receipt.py`: 15 superseded tokens ×
  4 files, every hit printed with a DEAD/LIVE verdict and line number, LIVE exits nonzero, the
  token list append-only, in the battery, emitting `ref/SUPERSESSION_SWEEP_RECEIPT.md`. Its two
  stated limits: tokens are literal (paraphrase is yours to catch) and the marker test is
  line-local (every hit is printed for re-judging).
- **Minors:** the BS-SI narrative and the map QUOTE the generated count move (17/8 → 16/9, with
  17/8 named as my erroneous intermediate — GPT56 F7); L1462/L1468 resolved CALLER under the
  table's own boundary, counts 28/18, §5 quotes 18 (GPT56 F8).
...
- **ATTACK THE INTEGRATION'S COMPLETENESS. Highest value here.** Every list claiming
  exhaustiveness over events or artifacts was to admit ARRIVAL or state why not. **Find the list
  that still excludes it** — a partition, closed schema set, void column, corner enumeration,
  verifier contract or registry row where an arrival event is still impossible, χ-bearing by
  default, or unnamed. Then **attack the sweep receipt itself**: its tokens are literal — find the
  PARAPHRASE of a retired regime that slips every token, or a LIVE sentence whose marker word
  makes it read DEAD.
- **Attack the PENDING/deadline closure.** A past-deadline zero-terminal arrival refuses any gate;
  disclosure refuses any zero-terminal arrival. **Construct the request that evades both** — one
  that is never PENDING, never past-deadline, never terminal; or show a gate ordering where a
  PENDING arrival's decision lands where no verifier pass ever reads it.
- **Attack the two cuts.** No enumerable event between them rests on "a refused issuance is no
  issuance". **Break that** — an event in the seam that is neither the issuance commit's own write
  nor post-partition, or an opening-pass exactness check that cannot actually decide "exactly the
  issuance commit's writes".
...
Write to `gates/V89_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
