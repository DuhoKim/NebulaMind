# BRIEF — V90 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V90_20260830.md`
**sha256 `4ab89668382e291cf65e508bcb588314e24d2d20d11d990180fd4d14e4ff6a8b`**
...
**Last refereed draft: V89** `c608ca9a798e1571` — **NOT CLEAR from both seats** (GPT56 7,
...
## What V90 changed — the clock, the wire, and the generators

**V89 was NOT CLEAR ×2** (GPT56 7, CODEX 6). The existence-half repair needed a clock the verifier
could actually read; the write-ahead rule needed a boundary; and two checked-in artifacts had
drifted from their generators.

- **The deadline has an AUTHENTICATED CLOCK BASIS** (your F1/F2 pair): ARRIVAL carries
  `(boot_epoch, monotonic_reading)` — Row B's own reading at append; every checkpoint's materials
  carry a CLOCK RECORD of their production; overdue is computed from those bytes only (same epoch:
  reading difference vs the BS-2k constant; cross-epoch: overdue by definition at the first
  later-epoch pass). The wall timestamp is human-facing and never the basis. Spec §3b carries the
  comparison rule; the registry gains four clock fields (207 total).
- **The RECEIPT TRANSITION is defined** (F2/F5 pair): a request exists exactly when its complete
  framed unit — length-prefixed, digest-closed — is read and decoded AND its arrival's framed unit
  is durably committed. Frame decode is receipt, not processing. Truncated/malformed frames are by
  definition not requests; a crash mid-arrival-append fails digest closure and is no arrival. Five
  framing fixtures added.
- **The binding→key map has its continuation** (CODEX F1): sealed map for pre-partition decisions,
  authenticated continuation map entries for post-partition ones — same temporal cut as the
  enumeration entries; a PENDING arrival crossing BS-L is decidable without touching sealed bytes.
- **The two live pre-sitting draw survivors are dead** (F4/F3 pair), killed from the committed
  mechanics and quoting their dead forms; the sweep gained four draw/grid tokens with DRAFT scope,
  an honest per-token-scope header (no implied cross-product), and `--check` byte-equality.
- **The raise ledger's resolutions live in its GENERATOR** (CODEX F4): `RESOLVED={1462,1468}` with
  annotations is encoded in `gen_raise_classification.py`, the ledger regenerates byte-equal, and
  both generators now take `--check`. One pattern, two instances, both closed.
- **The grid is a STEP COUNT** (GPT56 F3+F5, DRAW_MECHANICS AMENDMENT 2): `n_steps = 50` (even)
  committed; endpoints constrained symmetric ±Γ; Δγ DERIVED = 2Γ/n_steps (0.01 at the proposed
  0.25 — value unchanged, form now total over every symmetric ratification); `j` zero-based over
  the baseline-inclusive manifest, `n_perturbations = 51`, baseline `j₀ = 25` verified exactly
  γ=0; every `(i, 0)` comparison is now `(i, j₀)`. `PROPOSAL_GAMMA_RANGE.md` v2 asks one number.
- **Row B appends one DECISION event per completed request** — a touch's event or a refusal's
  event, never a refusal-as-touch (GPT56 F6). **The title says V90** (both seats' last finding).
...
- **ATTACK THE CLOCK BASIS. Highest value here.** Overdue is computed from `(boot_epoch,
  monotonic_reading)` pairs Row B itself writes. **Break it**: an epoch rule a crashing Row B can
  violate (same epoch surviving a restart, or an epoch that regresses); a reading Row B can
  truthfully write that defeats the comparison (readings are Row B's testimony — say what checks
  them); a checkpoint clock record absent exactly when the comparison needs it; or a PENDING
  arrival made immortal by never producing a later checkpoint in any epoch.
- **Attack the receipt transition.** A request exists at complete-frame-decode + durable arrival
  append. **Find the byte state that is neither**: a frame complete but undecodable in a way the
  wire schema does not name; a decode that must begin authorisation to finish (framing that needs
  row facts); an arrival append durably committed whose request frame was NOT complete.
- **Attack the map continuation.** Same cut as the enumeration entries. **Find the divergence** —
  a decision at the partition boundary whose map entry is required on both sides or neither, or a
  verifier ordering that reads the sealed map before a continuation entry it needs exists.
- **Attack the step-count grid.** `n_steps=50`, symmetric ±Γ, `j₀=25` verified zero. **Break the
  totality**: a ratification the constraint permits that still collides; a verifier that cannot
  decide "exactly γ=0" under the stated decimal encodings; the count closure 99 × 51 anywhere it
  is still stated as another number.
- **Attack the generator discipline.** Both `--check` modes claim byte-equality catches drift.
  **Defeat one**: an edit to a committed artifact the check cannot see, or a generator input that
  changes under regeneration (timestamps, orderings) making --check fail spuriously or pass
  vacuously.
...
Write to `gates/V90_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
