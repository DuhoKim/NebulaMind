# BRIEF — V91 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V91_20260830.md`
**sha256 `38fe971a0e0fd5869b4231e8a5d48f412453736a7fc736e15955e5d74d53d3be`**
...
**Last refereed draft: V90** `4ab89668382e291c` — **NOT CLEAR from both seats** (GPT56 8,
...
## What V91 changed — semantics the signatures could not carry, seams closed by rule

**V90 was NOT CLEAR ×2** (GPT56 8, CODEX 4).

- **The clock has SEMANTICS now** (CODEX F1 / GPT56 F2+F3): `boot_epoch` is DEFINED — the BS-2k
  restart counter, a sequence number owing nothing to any clock, incremented once per Row B start
  and committed before any event of its epoch. Monotonicity is an INVARIANT OVER THE SIGNED EVENT
  ORDER: epochs non-decreasing along the chain; readings non-decreasing within an epoch; an
  event's epoch equals the committed counter. Violations are MALFORMED HISTORY and refuse the
  CHAIN. Bounds stated where the fields live (epoch ∈ [0,10⁶]; reading ∈ [0,2⁶³−1] ns).
- **The map seam is closed BY RULE, bindings unmoved** (CODEX F3 / GPT56 F5): the binding→key
  map's two segments are cut at the SIGNED-CHECKPOINT cut — sealed for decisions at or before it,
  continuation for everything after, the issuance commit's own bindings the continuation's first
  members by definition. The asymmetry with enumeration entries is stated: no refusal can exist
  between the cuts, but decisions do. Seam fixtures added both ways.
- **The map is on the exhaustive list** (CODEX F2): item (iv-c), closed schema `(request_key,
  decision chain_position, decision event_digest, signature)` — declares what it already carried,
  widens nothing. Registry 211 fields.
- **The strata ruling propagated** (GPT56 F1): the no-producer paragraph is re-derived — Row D2 IS
  the producer; the open question is CLOSED BY RULING with its stale header corrected; the
  undecided list drops the strata question. Two sweep tokens added.
- **The verifier contract follows AMENDMENT 2** (GPT56 F6): the manifest is recomputed from
  `(Γ, n_steps = 50)` with Δγ DERIVED; the adjacent-gap and `delta_gamma_max` refusals compare the
  derived value; the committed-value list names the step count.
- **Exact-zero is TOTAL** (GPT56 F7): manifest values are canonical DECIMAL STRINGS built by exact
  decimal arithmetic (Δγ = Γ·4/100, a digit shift), compared as strings; `j₀`'s value is
  string-equal to canonical `0`; `delta_gamma_max` re-typed decimal-string. Mechanics
  clarification appended, values untouched.
- **The receipt control went red and was HONOURED** (GPT56 F8 / CODEX F4): five line numbers went
  stale because V90 inserted its §10 row after generating the receipt. Receipt regeneration is now
  the LAST build step, like the pins; regenerated against final V91 bytes and committed.
- **The crash-window table has the window the arrival created** (GPT56 F4): W0 pre-arrival wire /
  W1 the PENDING span, spec first; W2 covers both commits.
...
- **ATTACK THE CLOCK INVARIANTS. Highest value here.** Monotonicity now rests on chain-order
  invariants, not signatures. **Break the invariants**: a Row B crash path where the epoch counter
  commits but the restart dies before any event (is the next epoch's first event checkable?); two
  events in one atomic commit with readings the invariant cannot order; a chain where every
  invariant holds and an old arrival is STILL made non-overdue (counter rollback with the domain's
  own recovery); or show the invariant is stated but NO verifier pass is named to check it.
- **Attack the seam rule.** Sealed iff at-or-before the signed cut. **Find the decision that
  defeats the rule**: one whose chain position is at the cut exactly (boundary semantics), or a
  sealed-segment claim the canonicalized body cannot actually contain (the cut is
  pre-canonicalization — what fixes the sealed segment's CONTENT?).
- **Attack (iv-c).** The map's schema says never-payloads-never-outcomes. **Break it**: a
  request_key or position pattern that leaks χ through the map's own join structure, or a
  signature envelope field outside the declared four.
- **Attack the decimal-string grid.** Δγ = Γ·4/100 as a digit shift. **Break the totality**: a
  permitted Γ whose canonical decimal form makes Γ·4/100 or some γ_j non-canonical (trailing
  zeros, sign of zero, precision growth under repeated addition); or a string-comparison rule the
  canonical encoding leaves ambiguous.
- **Attack the strata propagation.** Row D2 is the producer and the question is closed. **Find the
  next stale site**: any sentence still treating the allocation as unconstructible, the producer
  as absent, or BS-SI as class P.
...
Write to `gates/V91_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
