# BRIEF — V92 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V92_20260830.md`
**sha256 `1607525cf9e52a9f9ae18fd17129a8debe644087045c184dbf106fcc75379dd9`**
...
**Last refereed draft: V91** `38fe971a0e0fd586` — **NOT CLEAR from both seats** (GPT56 9,
...
## What V92 changed — the clock verified, the interface fixed, one grammar

**V91 was NOT CLEAR ×2** (GPT56 9, CODEX 6).

- **The clock invariants are SCOPED and CONSUMED** (your F1 pair): they quantify over
  CLOCK-BEARING records by design (decision events carry no pair and compare nowhere); every epoch
  OPENS with a recovery checkpoint whose clock record is the epoch's first clock-bearing record —
  the chain evidence the restart counter lacked; arrivals' epochs must equal the latest opening's.
  The enumeration verifier's five gates run a CLOCK PASS with seven fixtures (rollback, reuse,
  epoch mismatch, regression, missing opening, the decision-scoping acceptance, FIFO violation).
- **Re-view multiplicity is the constant 1** (GPT56 F3): the sealed interface is single-pass,
  forward-only — no re-render operation exists, so nothing content-driven reaches the log's
  multiplicities. The labeling-ergonomics cost is FLAGGED in the decisions file, reversible until
  BS-2k fills.
- **Decisions commit FIFO in arrival order** (GPT56 F7): the binding→key join permutation is the
  identity; the position-gap residual is NAMED like the cause-token channel; out-of-order
  decisions are malformed history.
- **The lock-checkpoint preimage requires the sealed segments** (GPT56 F2) — enumeration entries
  AND the sealed binding-map half, whatever else the pending schema enumerates.
- **One canonical decimal grammar** for every γ-grid byte (GPT56 F4 / CODEX F2):
  `-?(0|[1-9][0-9]*)(\.[0-9]*[1-9])?`, exact decimal arithmetic, never a binary float;
  shortest-round-trip language is dead; the registry's `delta_gamma_max` row is re-typed DERIVED.
- **Stale prose killed**: the BS-3g row's draw set is DERIVED (its dead `NOT-EVALUATED` quoted
  dead), the summary says six windows and G1–G6, the stay-16/8 sentence quotes 16/9, the strata
  sidecar header says class E (GPT56 F5/F6/F9, CODEX F6).
- **Generators own their outputs harder**: v9's two choice-point raises moved INTEGRITY → CALLER
  in the generator (30/57, byte-equal); the registry generator now EXTRACTS the (ii-b) and (iv-c)
  tuples from the draft and crosschecks the declared sets (CODEX F4); `bindmap.signature` is
  on-enum with no envelope leaves (CODEX F3); R01's completeness is BLOCK-SCOPED to the operative
  vocabulary — a stray mention elsewhere no longer counts (CODEX F5; 35 controls).
...
- **ATTACK THE EPOCH OPENING. Highest value here.** Every epoch opens with a recovery checkpoint
  before any event of that epoch. **Break it**: Row B dies AFTER the domain commits the counter but
  BEFORE the opening checkpoint lands (what epoch does the opening of the NEXT restart carry, and
  does invariant (a) survive a skipped number?); two restarts with no intervening arrivals (two
  consecutive openings — legal or malformed?); or the FIRST epoch of the run (what opens it, and
  against what prior state does the verifier check it?).
- **Attack single-pass rendering.** No re-render operation exists. **Break the constant-1 claim**:
  the crash-window replay (a render commit whose view session never opened — does the no-session
  destroy path re-deliver?); occlusion-restore under G5/G6 (a SECOND committed render event was
  previously REQUIRED — reconcile with exactly-once-per-member or find the contradiction); a
  member whose session ends mid-view (advance or lose the label?).
- **Attack FIFO.** Decisions in arrival order, deadline-bounded heads. **Break it**: a
  within-deadline head whose §3b abort-then-refusal machinery does NOT fire (it is not stalled,
  just slow) while every later request's deadline expires behind it — construct the cascade and
  say what the verifier sees; or two decisions in one atomic multi-request commit (does FIFO order
  exist INSIDE a commit?).
- **Attack the extraction crosschecks.** The generator now parses the (ii-b)/(iv-c) tuples from
  the draft. **Defeat the parser**: a draft wording change that keeps the schema but breaks the
  regex silently, or a tuple the extractor maps wrongly (the normalization table is hand-written).
- **Attack the decimal-string grid still.** Exact decimal addition across 51 steps: show precision
  growth producing a non-canonical string for a permitted Γ, or an ambiguity the grammar leaves.
...
Write to `gates/V92_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
