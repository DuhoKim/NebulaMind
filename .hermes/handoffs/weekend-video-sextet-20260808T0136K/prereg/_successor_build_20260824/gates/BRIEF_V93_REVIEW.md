# BRIEF — V93 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V93_20260830.md`
**sha256 `fbafd1c47abf145b3311a4f0d7af54125dd40ff13bb9a7a5569f4dbe7913e63c`**
...
**Last refereed draft: V92** `1607525cf9e52a9f` — **NOT CLEAR from both seats** (GPT56 6,
...
## What V93 changed — anchored epochs, padded multiplicity, the laws come home

**V92 was NOT CLEAR ×2** (GPT56 6, CODEX 6; three exact pairs).

- **Epoch openings are ANCHORED AND LINKED** (your F1/F3 pair): each opening names its
  predecessor epoch and declares crash-consumed gaps; undeclared gaps, predecessor mismatches and
  double openings are malformed history; the first epoch anchors at the BS-2k provisioning
  constant. The provable claim is restated honestly: one opening per chain-visible epoch with
  every gap owned — not one-increment-per-start, which crashed epochs make unprovable.
- **Single-pass is SUPERSEDED by constant-multiplicity padding** (F4/F2 pair — you broke V92's
  rule against §5's own occlude-and-restore requirement): exactly `R_max` renders per object per
  member; interruptions consume replays; unconsumed replays are issued as padding commits;
  exhaustion enters the absent-label path. The rule lives in spec §5; the draft derives.
- **The ORDER AND LATENCY LAWS live in §3b** (F3/F1 pair — the V81 single-home lesson repeating):
  FIFO; DECIDE-WITHIN-D with the decision's clock evidence carried by the BINDING MAP (the ruled
  log schema untouched — the map is WIDENED by the decision clock pair, and the widening is FILED
  with the coordinator); the HEAD BUDGET (an overrun head takes the catch-all at its turn, so a
  slow head cannot silently cascade; BS-2k carries D ≥ budget × queue-bound as a stated design
  obligation). Verifier fixtures for decide-within-D and the cascade.
- **The checkpoint clock record is exhaustive-list item (ii-c)** (GPT56 F2) — the third artifact
  to repeat the arrival-schema defect, admitted at the list the same way.
- **The arrival extraction is SCOPED** (F5/F6 pair): anchored on the (ii-b) label itself AND
  requiring Row B's duplicate to exist and match — deleting the item now fails.
- **The grammar forbids `-0` in the pattern, not the prose** (F6/F5 pair); **the BS-3g row no
  longer contradicts itself** (CODEX F4: the generator and stopping rule are FROZEN BY
  COMMITMENT). Registry 213 fields.
...
- **ATTACK THE GAP DECLARATIONS. Highest value here.** A successor opening declares
  crash-consumed epochs. **Break the declaration**: Row B truthfully declares a gap that was NOT
  crash-consumed (a rollback dressed as a crash — what distinguishes them in bytes?); a
  declaration racing recovery (the opening commits, then Row B dies — is the NEXT opening's
  predecessor the dead epoch or its predecessor?); or the padding between what the counter did
  and what the chain can prove — say exactly what an auditor concludes from a declared gap.
- **Attack padding.** Every object gets exactly `R_max` render commits. **Break the constant**:
  the padding commits are issued at "session-set end" — define it adversarially (a member who
  never ends); padding events are conveyances that open no session — reconcile with G5's
  render-=-touch and Row G's any-unlogged-view void; a member interrupted during PADDING; or show
  the timing of padding emission itself carries the count it exists to hide.
- **Attack DECIDE-WITHIN-D.** The decision clock lives in the map entry Row B writes. **Break
  it**: Row B truthfully writes a reading taken BEFORE the commit it stamps (read-then-stall);
  the head-budget refusal path racing the deadline (which wins at exactly D?); or a queue-bound
  the BS-2k obligation cannot actually enforce.
- **Attack the widened map.** Two new fields, FILED not ruled. **Say what the widening leaks**:
  decision readings correlate with processing content — is the position-gap residual argument
  still sufficient when readings are explicit? If not, that is a finding against the widening,
  not a repair of it.
- **Attack the scoped extractor still.** The (ii-b) anchor is a 400-char window regex. **Defeat
  it**: reword within the window so the anchor matches a DIFFERENT tuple, or push the tuple past
  the window.
...
Write to `gates/V93_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
