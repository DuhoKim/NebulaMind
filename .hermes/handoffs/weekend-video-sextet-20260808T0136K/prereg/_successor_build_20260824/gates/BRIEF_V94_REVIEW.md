# BRIEF — V94 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V94_20260830.md`
**sha256 `74efc959dc8c9b9f8f72789bf0d5820c830429d87771c7cceee245dc0c911c25`**
...
**Last refereed draft: V93** `fbafd1c47abf145b` — **NOT CLEAR from both seats** (GPT56 9,
...
## What V94 changed — the gap proof, the bound reading, padding v2

**V93 was NOT CLEAR ×2** (GPT56 9, CODEX 6).

- **The opening has its schema and the gap declaration needs NO TRUST** (F1/F2 pairs): the
  opening record is `(boot_epoch, monotonic_reading, predecessor_epoch, gap_declaration)` — and
  what a declared gap could hide is an epoch with events, but events extend the running digest,
  so the verifier checks no chain event carries a declared-gap epoch and CONTINUITY does the
  rest. A rollback that appended nothing is byte-identical to a crash that appended nothing, and
  equally harmless. The declaration is a naming convenience over a fact the chain proves.
- **The reading is bound to the commit by the ordering domain** (F3/F2 pair): map entries join
  invariant (b), so a read-then-stall reading is smaller than any record committed during the
  stall and violates non-decreasing order. The no-intervening-record residual is NAMED — an empty
  queue has no cascade victim.
- **All recorded readings are QUANTIZED** up to the BS-2k granularity `g` (GPT56 F7): order
  preserved, decide-within-D conservative, the processing-time channel shrunk to log₂(D/g) bits.
- **Padding v2 — per-object and pre-label** (F5/F6 + F3/F4): unconsumed replays are issued when
  the member proceeds to label, BEFORE entry — no label held anywhere, every object shows the
  identical `R_max`-renders-then-label chain pattern, so neither counts nor timing carry
  anything. Session-set end is replaced by the BS-2k labeling deadline. **Exhaustion HALTS the
  run pre-BS-8f** like Row I's missing-output rule — the absent-label path V93 cited did not
  exist; softening is the principal's, flagged.
- **`request_key` equality is CHECKED** (GPT56 F4): the verifier refuses a key unequal to its
  arrival's chain position — the definitional identity, asserted since V86, never checked.
- **The delivery-retry corner is CONVEYANCE-ONLY** (GPT56 F8). **R03/R04 gain polarity scans**
  (GPT56 F9; +2 controls). **R01 refuses decoy vocabulary blocks** (CODEX F6; +1 control; 38
  green). **The extractor anchors on the (ii-b) ITEM-DEFINITION form, exactly one tuple in its
  segment** (CODEX F5 — the first cut counted bare cross-references and the battery caught it).
  Registry 215.
...
- **ATTACK THE EMPTINESS PROOF. Highest value here.** "No chain event carries a declared-gap
  epoch" + continuity = the gap is empty. **Break the proof**: an event class that carries NO
  epoch field (decision events!) appended during a doomed epoch — what binds IT to the gap?; a
  gap declared over the CURRENT epoch; continuity across the crash boundary itself (what digest
  does the first post-crash record extend, and can a prefix-truncation masquerade as a crash?).
- **Attack padding v2.** Per-object, pre-label, `R_max` renders then the label write. **Break
  it**: a member interrupted BETWEEN padding start and label entry (pattern broken — what does
  the chain show?); an exhausted object (R_max consumed, no label — the halt record's own chain
  pattern differs from labeled objects, and the halt is post-hoc: what did the chain leak before
  the halt?); the labeling-deadline backstop closing an allocation mid-padding.
- **Attack the quantization.** Readings rounded UP to `g`. **Break it**: two records in one
  quantum making non-decreasing vacuous (order inside `g` invisible — does any law need it?);
  `g` vs the commit-bound race (a stall shorter than `g` hides entirely — compose with
  read-then-stall's named residual and say whether the two blind spots overlap into something
  bigger); the arrival deadline D not divisible by `g`.
- **Attack the exhaustion halt.** The run halts pre-BS-8f on one member's exhaustion. **Break
  it**: an adversarial MEMBER who forces a halt at will (occlude R_max times) — is a
  deliberate-halt channel acceptable in a blinding covenant?; the halt record's identities
  (member, object) — χ-adjacent?
- **Attack the request_key identity check and the item-form anchor** — both are one revision
  old: find the wording change that defeats the item-form uniqueness, or a chain where the
  identity check and the bidirectional join disagree.
...
Write to `gates/V94_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
