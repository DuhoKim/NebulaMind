# BRIEF — V95 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V95_20260830.md`
**sha256 `1262c2767cf1cfe6c27bf3f179e6ce81f939ff43e402358622dacb4eeda31a83`**
...
**Last refereed draft: V94** `74efc959dc8c9b9f` — **NOT CLEAR from both seats** (GPT56 10,
...
## What V95 changed — anchors, directions, and the machinery the terminal lacked

**V94 was NOT CLEAR ×2** (GPT56 10, CODEX 7).

- **The emptiness proof holds AS OF THE EXTERNAL ANCHORS** (your F1 pair — prefix truncation
  survives internal continuity): chain heads are bound outside the chain at BS-2f's checkpoint
  receipt, BS-L's sealed materials, and every verifier gate pass record; truncation is caught at
  the first anchor after it; the inter-anchor window is NAMED.
- **Quantization directions are fixed per role** (F3 pair): arrivals floor, decisions/openings/
  checkpoints ceil — the recorded gap bounds the true gap from above, decide-within-D
  conservative BY CONSTRUCTION. The ordering invariant runs with slack `g` (F4); the channel
  bound is `log₂(⌈D/g⌉+1)`, never negative (F10). Predecessor NONE is the reserved 0 (F2 pair).
- **The V94 ordering argument is declared DEAD in the text** (CODEX F2 killed it: a stalled
  single writer appends nothing to order against). The binding is the domain's own commit bound:
  reading at commit start is a BS-2k implementation obligation with a stall fixture; bounded
  commits cap staleness; decide-within-D checks against `D − commit_bound`. Fixture-not-proof is
  SAID.
- **Padding v3 is Row-H-true** (GPT56 F6): Row H persists ONE label set as its row states — the
  per-object label write V94 presumed does not exist; the pattern claim is now `R_max` contiguous
  renders per object with no per-object label event. Inter-object PACE is the named leftover
  (CODEX F5).
- **Exhaustion has its full machinery** (CODEX F4 + GPT56 F7): `TERMINATED-BY-LABEL-EXHAUSTION`,
  second member of the terminated family; produced by the detecting interface via Row B as a
  RECEIPTED artifact outside the chain (no third event class); two-field non-χ face, identities
  sealed committee-side; gates refuse after a halt receipt. The member-controlled channel is
  DECLARED: denial, not disclosure.
- **The join checks terminal identity equality** (GPT56 F5); **the enumeration machinery's
  domain includes the mismatch class with the ruling's review disposition** (GPT56 F8); **the
  ledger enumerates the corpus's one production `assert`** (v9:1622, INTEGRITY — 113 failure
  sites; GPT56 F9); **R01 requires the operative block's four-header structure**, with the
  which-block-is-normative limit stated (CODEX F7). Registry 217.
...
- **ATTACK THE ANCHORS. Highest value here.** Truncation is caught at the first external anchor.
  **Break the anchor set**: a gate pass record is itself gate-materials — SEALED at BS-L (can a
  pre-BS-L pass record anchor anything after the sealing?); the window between the LAST anchor
  and the run's end (what anchors the final suffix — disclosure's pass record, and what anchors
  THAT?); an adversary who truncates and then FORCES a crash so the next opening's prior-head is
  the truncated head.
- **Attack the commit-bound staleness argument.** Reading at commit start is
  fixture-not-proof, staleness ≤ commit_bound. **Break it**: a commit that legitimately retries
  after abort (does the re-read happen, and what does the map entry show for the aborted
  attempt?); compose the slack-g inversion window with commit_bound and say whether the two
  bounds stack into something a late decision can hide inside.
- **Attack padding v3.** `R_max` contiguous renders, no per-object label event, ONE label-set
  write. **Break the contiguity**: interleaved objects (a member flips between two objects —
  does the interface serialize object blocks, and where is THAT rule?); the exhaustion halt
  firing mid-padding; the labeling deadline closing an allocation whose padding never ran.
- **Attack the exhaustion machinery.** A receipted artifact outside the chain, gates refuse
  after it. **Break it**: WHO verifies the halt receipt's authenticity (producer is the sealed
  interface via Row B — under whose key?); a forged halt receipt as a denial attack from outside
  the committee; the two-field face's chain_head — is a truncated chain's halt receipt an anchor
  or a lie?
- **Attack the assert enumeration.** One production assert joined the ledger. **Find the next
  blind node kind**: implicit raises (KeyError on a dict access, ZeroDivision) reachable on the
  verdict path that no ast.Raise or ast.Assert row covers — is the every-failure-path claim
  true of UNRAISED exceptions?
...
Write to `gates/V95_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
