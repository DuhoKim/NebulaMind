# BRIEF — V96 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V96_20260830.md`
**sha256 `ca49c2a2842f4dc01412658db3c8e1743c065a743edfe803bc84f1449d2cd25d`**
...
**Last refereed draft: V95** `1262c2767cf1cfe6` — **NOT CLEAR from both seats** (GPT56 6,
...
## What V96 changed — the anchors built, the window owned, the claims scoped

**V95 was NOT CLEAR ×2** (GPT56 6, CODEX 7).

- **The pass record is BUILT** (your F2/F4 pair): closed schema `(gate, head_position,
  head_digest, verifier_digest, signature)` under the provisioned enumerator keypair, custody in
  each gate's receipted materials, each record verifying its predecessors — the anchors chain.
  The disclosure pass record is the run's TERMINAL HEAD, closing the final suffix.
- **The inter-anchor window JOINS THE RESIDUE** (GPT56 F1): no-vanish, deadline and
  key-uniqueness hold AS OF THE ANCHORS; a rollback inside the window is platform-level failure
  in the storage clause's family; the residue sentence names the window beside the pre-arrival
  wire. The corner's totality claim carries §3b's own quantifier (GPT56 F6).
- **The deadline law is TWO-TIER** (CODEX F3): substantive decisions by `D − commit_bound`; the
  deadline refusal — which necessarily commits after D — within `(D, D + enforcement_lag]`.
  **Every retry RE-READS at its own commit start** (CODEX F5).
- **The halt receipt is authenticated and registered** (F3/F2 pair): interface keypair, roster
  check, head match; `TERMINATED-BY-LABEL-EXHAUSTION` joins the canonical run-outcome
  enumeration; the receipt's face is exhaustive-list item (ii-d). Registry 222 with the PASSREC
  fields.
- **The draft's padding sentence caught up with the spec** (F4/F1 pair — it kept the retired
  renders-then-label pattern for one revision).
- **The every-failure-path claim is SCOPED to explicit sites** (F5/F7 pair): implicit exceptions
  classify by the catching boundary, an uncaught one is platform failure — stated as the limit;
  the three cited implicit paths are hand-enumerated in the ledger's append-only addendum.
- **Canonical JSON: NFC first, THEN bytewise sort** (CODEX F6).
...
- **ATTACK THE PASS-RECORD CHAIN. Highest value here.** Anchors now chain by predecessor
  verification under one keypair. **Break the chain**: the FIRST pass record (what predecessor
  does it verify — BS-2f's receipt, and is that checkable from the record's schema?); a pass
  record for a gate that REFUSED (does a refusing pass anchor the head it refused at?); the
  enumerator key compromised AFTER BS-L (every post-seal anchor is one key's testimony — say
  what limits that); two pass records for one gate (retry after refusal — which anchors?).
- **Attack the two-tier deadline.** `(D, D + enforcement_lag]` for deadline refusals. **Break
  it**: a substantive decision landing inside the refusal tier (late but pre-enforcement — which
  tier's check fires, and can both pass?); enforcement_lag vs the FIFO head budget (a queued
  deadline refusal behind a budget-consuming head — does the lag bound survive?).
- **Attack the halt-receipt authentication.** Signature + roster + head match. **Break it**: a
  REPLAYED receipt from an earlier legitimate halt attempt (aborted run resumed — head matches
  an old chain state?); the interface signing under coercion of a hostile member (the denial
  channel now has an authenticated artifact — is that better or worse?).
- **Attack the scoped failure-path claim.** Implicit exceptions classify by catching boundary.
  **Break the boundaries**: name a reachable implicit exception whose catching boundary assigns
  it a class its semantics contradict (a CALLER-boundary catch swallowing what is really state
  corruption); or show the addendum's three rows misclassify their own examples.
- **Attack the residue composition.** Pre-arrival wire + inter-anchor window + fsync-hang +
  empty-queue stall + within-quantum inversions + pace + log₂ channels — SEVEN named residuals
  now coexist. **Compose two into something unnamed**: pick any pair and show their overlap
  admits an event history none of the seven descriptions covers.
...
Write to `gates/V96_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
