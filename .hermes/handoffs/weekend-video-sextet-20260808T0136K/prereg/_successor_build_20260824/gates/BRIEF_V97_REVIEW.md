# BRIEF — V97 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V97_20260830.md`
**sha256 `2a469a2f2adb239e96dd2ef27ccbf9f20721cd0e2741c532185f3a7dc74b3f3b`**
...
**Last refereed draft: V96** `ca49c2a2842f4dc0` — **NOT CLEAR from both seats** (GPT56 6,
...
## What V97 changed — the anchors chain by construction, the tiers derive

**V96 was NOT CLEAR ×2** (GPT56 6, CODEX 5).

- **`predecessor_record_digest` is INSIDE the signed pass-record body** (your F2 pair): the
  anchors chain by construction — exactly one record per gate in the five-gate order, first
  predecessor is BS-2f's receipt digest, a refusing pass emits no record, forks and duplicates
  are malformed. **And the pass record is registered**: exhaustive-list item (ii-e), plus §11 as
  the enumeration verifier's OWN emission item with producer, custody, chain-rule verification
  and four fixtures (F1 pair).
- **The deadline tier DERIVES from clock evidence** (CODEX F3 — deadline and substantive
  refusals share event bytes): substantive iff map-entry reading ≤ arrival + D − commit_bound;
  deadline-tier within the lag window; beyond it malformed. **`enforcement_lag` is sized to the
  backlog** (GPT56 F3): ≥ detection + Q·commit_bound, Q the provisioned queue bound.
- **Both TERMINATED records are built and registered**: exhaustion in §5's exact outcome
  registry with its authenticator restated as a detached envelope (CODEX F4); the
  unnameable-class terminated-verdict record gains canonical body `(kind, class_key, gate,
  chain_head)`, enumerator signature, custody, and its face beside the exhaustion receipt's in
  item (ii-d) (GPT56 F6). Registry 227.
- **The implicit addendum is corrected FROM THE BYTES** (GPT56 F4 + CODEX F5 — two of three
  rows had invented semantics): 1493–96 `adjudicate_path` implicits are UNCAUGHT on the verdict
  path, and the disposition is stated — process death with NO verdict record, operator-observed
  platform family, loud never silent; 1609 is the propagation site; 1647–48 `int()` casts are
  CALLER. The draft's scoping sentence now says the verdict path HAS no catching boundary.
- **R01 parses digit-bearing tokens** (GPT56 F5): `REFUSED-X2` no longer evades; 39 controls.
...
- **ATTACK THE ANCHOR CHAIN'S ROOT AND KEY. Highest value here.** The chain now binds
  predecessors, but everything hangs from BS-2f's receipt digest and ONE enumerator keypair.
  **Break the root**: is BS-2f's receipt itself bound to a chain head signed by anyone, or is
  the first link testimony?; the enumerator key signs entries, pass records AND the terminated
  record — enumerate what a single compromised key can now forge END-TO-END and whether any
  cross-check survives it.
- **Attack the derived tier.** Substantive iff reading ≤ arrival + D − commit_bound. **Break
  it**: a substantive refusal legitimately committing inside the lag window (slow but real —
  misclassified as deadline-tier, and does anything downstream care?); a deadline refusal whose
  map entry is MISSING (nothing derives — which check fires first?).
- **Attack the backlog bound.** enforcement_lag ≥ detection + Q·commit_bound. **Break it**: Q
  bounds the queue, but arrivals keep landing during the drain — is the bound closed under
  arrivals-during-enforcement, or does the backlog regenerate faster than it drains?
- **Attack the terminated-family records.** Two siblings, two keypairs, one list item. **Break
  the symmetry**: the exhaustion receipt binds no class_key/gate — can two halts be told apart,
  and does replay-across-runs bite the two-field body harder than the four-field one?
- **Attack the uncaught-verdict-path disposition.** Process death with no record is called
  loud. **Break it**: an implicit exception AFTER the verdict record is written but before the
  process exits (is the record valid? emitted? whose custody?); or partial output flushed
  before death that downstream tooling reads as a verdict.
...
Write to `gates/V97_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
