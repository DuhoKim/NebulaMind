# BRIEF — V98 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V98_20260830.md`
**sha256 `1bf168c0d31e19a3f400bb9cde6079ce35e55dac68d8835973073ddd13f945da`**
...
**Last refereed draft: V97** `2a469a2f2adb239e` — **NOT CLEAR from both seats** (GPT56 7,
...
## What V98 changed — the trust boundary at the human waypoints, predicates not tiers

**V97 was NOT CLEAR ×2** (GPT56 7, CODEX 5).

- **The single-key question is answered honestly, not laundered** (your F1/F4 pair): what a
  compromised enumerator key forges is EVERYTHING BETWEEN HUMAN SIGNATURES. The anchors'
  independence is the three human waypoints — freeze (P0), BS-L (P6, whose Duho-signed body
  binds BS-2f's receipt digest and so roots the chain), opening authorization (P7). Between
  waypoints, anchors are provisioned-key testimony confined to their interval. A second machine
  key from the same provisioning would be co-located trust, not independence — NOT taken; the
  repair is the stated boundary.
- **Tiers are ABOLISHED for two acceptance predicates** (F6/F3 pair): touches timely by
  `D − commit_bound`; refusals by `D + enforcement_lag` whatever their reason. The
  misclassification dissolves because nothing downstream needed the distinction.
- **The partition cut is RECORDED** (GPT56 F3): `passrec.partition_cut_position`, 0 pre-BS-L,
  read by the opening pass's exactness check from its own predecessor-bound record.
- **Both terminated bodies are run-bound** (`freeze_signature_digest` — replay across runs
  fails) **and both signatures join the registry** (F5/F4 pair; registry 232).
- **The V71 recap's two-way revival is quoted as history** with the three-way beside it
  (CODEX F2). **The dynamic-load claim is rescoped at its real site** (GPT56 F2): unreachable in
  v9's OWN control flow; a hostile dict subclass's `.get()` is the CALLER's capability; the
  harness obligation is plain dicts, `type(x) is dict`, with fixture. **NumPy reduction
  ValueError joins the addendum** (GPT56 F7). **LOUD is a contract** (CODEX F5): atomic verdict
  writes, and Row S via BS-V — the only consumer — requires a verified complete record.
...
- **ATTACK THE WAYPOINT ARGUMENT. Highest value here.** Compromise is claimed confined between
  human signatures. **Break the confinement**: BS-L binds BS-2f's digest — but what binds the
  OPENING AUTHORIZATION to the post-BS-L anchor chain (it binds bsl_digest; do the post-P6 pass
  records enter ANY human-signed body, or is the entire P6→P9 suffix one key's testimony with no
  closing waypoint?); the freeze waypoint covers class-P receipts — does any clock/anchor
  artifact actually enter the freeze manifest?
- **Attack the acceptance predicates.** Touches by D−commit_bound, refusals by
  D+enforcement_lag. **Break them**: a TOUCH legitimately delayed past its predicate by the
  backlog the refusal-lag was sized for (the drain serializes touches too — do timely touches
  exist behind a storm?); a refusal-flood as a channel (every refusal gets the long allowance —
  can a hostile row stretch every response to D+lag and encode in the stretch?).
- **Attack the partition_cut_position field.** Written by the verifier into its own record.
  **Break it**: the opening pass writes the cut it then checks — self-attestation one field
  deep; what independent byte corroborates the issuance commit's last write position?
- **Attack the run-binding.** freeze_signature_digest in both terminated bodies. **Break it**:
  two runs sharing a freeze (a resumed run after operator halt — same freeze, is a first-run
  halt receipt valid in the resumption?); the freeze signature exists at P0 but the exhaustion
  halt can fire in a run that never reached a valid freeze (pre-freeze storms — what binds
  then?).
- **Attack the plain-dict harness rule.** `type(x) is dict` at the harness. **Break it**: v9
  itself calls `.get()` on nested values of the receipt (sub-dicts a subclass can hide in);
  name a nested access the top-level type check does not cover.
...
Write to `gates/V98_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
