# BRIEF — V99 whole-document referee round

**Subject:** `PREREG_SUCCESSOR_DRAFT_V99_20260830.md`
**sha256 `55062e8ae7696353315004a2711979041c80bd7f90a29b4a6518d424e68d06cd`**
...
**Last refereed draft: V98** `1bf168c0d31e19a3` — **NOT CLEAR from both seats** (GPT56 8,
...
## What V99 changed — the successor as the closing eye, the root schema closed

**V98 was NOT CLEAR ×2** (GPT56 8, CODEX 7).

- **The endgame suffix's closing waypoint is NAMED, honestly** (your F1 pair): within this run,
  everything after the opening authorization is machine testimony — said so; the closing human
  eye is THE SUCCESSOR PREREGISTRATION'S FREEZE REVIEW, which now explicitly owes this run the
  terminal head (the disclosure pass record's digest). Extending Clause 6 with the issuance
  pass-record digest would close P6→P7 under the principal's own P7 signature — FILED, not
  taken, because it changes what he signs.
- **The lock checkpoint receipt's schema is CLOSED** (GPT56 F2): five fields, nothing else —
  BS-L no longer human-signs an undefined digest; the registry stub is retired, `lockcp.*` live.
- **`partition_cut_position` is corroborated by the chain itself** (F3/F4 pair): the verifier
  recomputes the cut as the last BS-L-writing event's position and refuses a differing record.
- **The harness rule is DECODER PROVENANCE** (F4/F5 pair): every node of a production input is
  constructed by the canonical envelope decoder itself — no foreign object at any depth; nested
  array-protocol fixture added.
- **Run binding is CHAIN binding** (F5/F2 pair): both terminated bodies gain
  `first_opening_digest` — one freeze can govern a resumed run; one first opening cannot. The
  (ii-d) faces are synced (F6/F3 pair). Registry 239.
- **The queue obligation is the FULL-DRAIN inequality** (GPT56 F7):
  `D ≥ Q·(budget + commit_bound) + commit_bound`. **The refusal stretch channel is bounded by
  the BUDGET, not the lag** (CODEX F6): `log₂(⌈budget/g⌉+1)` bits, named.
- **My changelog tripped the checker it cites** (F8/F7 pair): the map entry named the
  digit-probe token bare, the generated §10 row carried it into the draft AFTER the battery's
  vocab pass. The entry is reworded tokenless and the BUILD ORDER is corrected — the full
  battery now runs after the §10 insertion.
...
- **ATTACK THE SUCCESSOR-AS-WAYPOINT. Highest value here.** The closing eye is a future
  document's freeze review. **Break it**: the successor may never exist (a run that ends the
  programme — who reads the terminal head then?); the successor's review reads what the
  enumerator key wrote (is the cross-run duty itself rooted in anything human from THIS run, or
  does the compromise argument just move one run right?); say precisely what a compromised key
  can do in the P7→P9 suffix that the successor's review CANNOT detect from the artifacts it
  reads.
- **Attack the closed root schema.** Five fields, nothing else. **Break it**: the clock_record
  field packs an (epoch, reading) pair into one bounded-decimal slot — under what encoding, and
  is it the canonical one?; the sealed_entry_set_digest covers entries that continue to accrue
  until the SIGNED cut — what fixes the set's boundary instant?
- **Attack the chain-corroborated cut.** The last BS-L-writing event. **Break it**: an issuance
  commit that writes BS-L in multiple events interleaved with a checkpoint write (which is
  last?); a post-issuance REPAIR write whose object identity is also BS-L.
- **Attack decoder provenance.** Every node decoder-constructed. **Break it**: the decoder
  itself consumes attacker bytes — name a decoder output that preserves attacker-controlled
  BEHAVIOR (a huge int, a crafted ndarray dtype=object?); the fixture set covers subclass and
  __array__ — name the third protocol.
- **Attack the chain-identity binding.** first_opening_digest. **Break it**: a resumed run that
  REUSES the old chain (same first opening — the binding admits it; is a resumption on the same
  chain a replay the binding should refuse or a continuation it should allow, and who decides?).
...
Write to `gates/V99_WHOLE_REVIEW_<SEAT>.md`, `<SEAT>` being `GPT56` or `CODEX`. End with exactly:
