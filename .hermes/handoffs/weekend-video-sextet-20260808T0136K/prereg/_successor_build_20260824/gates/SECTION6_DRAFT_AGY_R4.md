# §6 DRAFT — AGY SEAT, FOURTH PASS (R4), 2026-08-27

Status: DRAFT FOR REFEREE. Written under `BRIEF_DRAFT_SECTION6_R4.md`, revising
`SECTION6_DRAFT_AGY_R3.md` against the two third-pass referee reports —
`SECTION6_REVIEW_R3_GPT56.md` and `SECTION6_REVIEW_R3_CODEX.md`.
...
sample, never a per-object value), and **the BS-5f Stage-C receipt (PASS/FAIL and the permitted Stage-C scalar output, never a per-object calibration label)**. **Non-χ-bearing receipt and log classes — a closed list,
...
fixtures and their transcripts, synthetic by construction; (v) the authenticated **acceptance-evidence projection** containing only attempt identity, expected/actual checksum, shape, execution completion/non-finite status, and the frozen confidence quantity (no sign or amplitude). **Everything else is χ-bearing
...
| D | **Instrument runner** — the BS-9 gated runner executing BS-3's pinned instrument identity (weights `83008c1c…`, τ = 4.4006456017494235, antisymmetry) | reads cutouts **and the χ-bearing cutout-completion receipt** in the store → writes per-object χ-bearing measurement receipts (χ, sign, amplitude) and **a separate authenticated acceptance-evidence projection containing only attempt identity, expected/actual checksum, shape, execution completion/non-finite status, and the frozen confidence quantity (no sign or amplitude)** into the store only | P2, after row C, before BS-2f | BS-3 (instrument identity), BS-9 (runner pin), and the cutout-completion receipt | the per-object measurement receipts and acceptance-evidence projections, one per parent object, in the store | any χ-derived value emitted outside the store |
| E | **Acceptance-ledger recompute** — the callable `recompute_acceptance_ledger` whose symbol and digest BS-2a's receipt must pin (future work) | reads **only the separate authenticated acceptance-evidence projections** in the main store (expected/actual cutout checksums, shapes, statuses, confidence) and fixed parent lists — and **computes every §2.7(2)(a)–(d) predicate and status from it**; never receives the measurement receipt bytes, thus sign-blind by construction (the BS-2a gate proves sign/amplitude are structurally unreachable) → atomically writes both the append-only evidence ledger and the realised partition | P2–P3, after complete inference, before BS-2f | BS-2a (the frozen acceptance design), the cutout-completion receipt, and the complete set of acceptance-evidence projections | the realised-partition record, bound by BS-2f | reading any field outside the evidence schema; any exclusion reason outside §2.7(2)(a)–(d); allowing operator-supplied statuses |
...
| O | **Unsealing service** — the unsealing callable whose symbol BS-2k pins (future work) | reconstructs key use for this ceremony only and decrypts both sealed stores into the declared post-unblinding working location; every step a logged event. Requires and verifies the canonical opening authorization, ensuring its unique ceremony identifier has not been consumed | P7 only; runs exactly once | a passing `verify_lock()` and Duho's **canonical opening authorization** | the **unblinding receipt**, carrying the authorization digest, verification result, consumed ceremony identifier, and the genuinely final access-log checkpoint | any invocation before a verified BS-L; any replay of the opening authorization; any decrypted byte outside the declared working location |
| P | **Verdict path** — `run_production_verdict()` (pinned symbol; guard extended per clause 3(d)) | **post-unblinding only**: reads the real χ vector joined to the accepted mask; first re-invokes `recompute_acceptance_ledger` against the sealed ledger and refuses on a mask-digest disagreement with BS-2f; requires a verified BS-L bound to that mask digest exactly as it requires BS-5f's → writes the permutation record and the verdict | P8, after unblinding | the unblinding receipt and a verified BS-L | BS-7f, then BS-V | any execution before unblinding; any verdict produced outside this symbol |
...
3. **§7 counts.** Update §7's text to state "One of fourteen class-P slots is filled", updating the parsed count from 12 to 14. Add BS-2a and BS-2k to the "today's count" of DESIGN slots. Add a lint assertion that the prose count equals the parsed table count. Add the unblinding receipt as a named post-unblinding artifact.
...
5. **§2.7(4) and §5:** `recompute_acceptance_ledger` computes statuses and reasons, atomically writing the evidence ledger and realised partition. Post-unblinding it is re-invoked. Modify `run_production_verdict()` to require and verify the canonical BS-L artifact, the one-use unblinding receipt, the exact accepted-mask binding, and the post-unblinding ledger recomputation before forming any statistic. Name the corresponding new arguments/guards in §5.
6. **§10.** Record this §6 replacement.
7. **Code-side items in the same atomic revision**: `SLOT_SCHEMA` updates, `verify_lock()`, `recompute_acceptance_ledger`, enforceable mediation gate checks, opening authorization verifier, acceptance-evidence projection boundary.
...
- **C3 — Label-set receipt stays in committee store.** The finding required making it χ-bearing but readable by Row I. Alternative: create a non-bearing attestation digest for export. Rejected because keeping the single χ-bearing receipt in the store is simpler and requires no new schema type.
- **C4 — Keeping the cutout-completion receipt in the main store.** Alternative: create a non-bearing completion attestation. Rejected because keeping it in the main store and authorizing Row D and E to read it explicitly maintains a simpler surface without opaque digest loopholes.
...
- **R2 — Executability rides the next code revision.** Many required schemas and checks (BS-L manifest, enforceable mediation gates, opening authorization, evidence projection) do not exist yet. Clause 3(d) makes the replacement an atomic candidate revision condition.
...
1. **Canonical opening authorization has no canonical body or verifier.** REPAIR. Clause 6 defines the exact canonical body and signature envelope binding the BS-L digest, both store identities, destination, unique ceremony ID, phase P7, signer identity, and schema. Row O verifies it and emits the unblinding receipt carrying the consumption result.
2. **Final post-unblinding access-log checkpoint is not a named receipt.** REPAIR. The unblinding receipt is named as the artifact carrying the genuinely final post-unblinding checkpoint, which must extend BS-L's checkpoint. Added to row B, row O, and Part 2's §7 updates.
3. **Row E's recompute is declared sign-blind while reading full execution receipts.** REPAIR. Row D's output is split. It writes per-object χ-bearing measurement receipts and a separate authenticated acceptance-evidence projection containing only attempt identity, expected/actual checksum, shape, execution completion/non-finite status, and frozen confidence quantity (no sign or amplitude). Row E is authorized to read only this separate projection, which structurally ensures sign-blindness.
4. **Archive seal-state checkpoints have no transition rule.** REPAIR. Clause 7 defines the canonical seal-state schema and invariant (identity and intact-state equality from BS-2k → BS-2f → lock checkpoint). `verify_lock()` checks it and refuses any mismatch.
5. **Cutout completion verification path & BS-5f disclosure boundary.** REPAIR. The cutout-completion receipt remains χ-bearing in the main store, and its authenticated read is added to row D and row E's exact surface. BS-5f is explicitly added to the permitted aggregate surface in the Disclosure rule and Scope paragraph, bounding its χ-derived export legally.
6. **Void consequence ends at lock while access ban continues to unblinding.** REPAIR. Clause 5 and rows K, L, M now explicitly state that any "pre-unblinding" access outside the table voids the run, extending the void consequence up to the unblinding phase.
