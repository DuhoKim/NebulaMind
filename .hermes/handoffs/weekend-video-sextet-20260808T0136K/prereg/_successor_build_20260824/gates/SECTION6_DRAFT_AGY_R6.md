# §6 DRAFT — AGY SEAT, SIXTH PASS (R6), 2026-08-27

Status: DRAFT FOR REFEREE. Written under `BRIEF_DRAFT_SECTION6_R6.md`, revising `SECTION6_DRAFT_AGY_R5.md` against the two fifth-pass referee reports (`SECTION6_REVIEW_R5_GPT56.md` and `SECTION6_REVIEW_R5_CODEX.md`).
This file modifies nothing; `../PREREG_SUCCESSOR_DRAFT_V15_20260827.md` is untouched. Part 1 is the complete replacement §6, written to drop in. Part 2 lists the conforming edits outside §6 that dropping it in requires. Part 3 states every choice the findings did not force, with its alternative. Part 4 names the residual risks this draft itself carries. Part 5 maps every referee finding to REPAIR or REFUSE.
...
(i) a slot receipt under the pinned `SLOT_SCHEMA` as conformed by this revision's code items — namely BS-1, BS-1b, BS-2a, BS-2c, BS-2k, BS-2m, BS-2o, BS-3, BS-4, BS-5p, BS-2s, BS-6, BS-7p, BS-8p, BS-9, BS-2f, BS-8f, BS-5f, and BS-L (BS-7f and BS-V are explicitly post-unblinding χ-bearing receipts and are removed from this list); 
...
**The phase line.** P0 freeze → P1 BS-6, first image byte → P2 cutout production, pre-inference integrity projection, exact-parent C2 stage-completion, and instrument inference → P3 BS-2f → P4 BS-8f → P5 BS-5f → P6 BS-L, **the primary lock** → P7 unblinding → P8 BS-7f and BS-V → P9 disclosure. 
...
| I | **Calibration computation** | reads the sealed label set, the corresponding instrument outputs, AND the **χ-bearing label-set receipt**, inside the stores → writes **only** the BS-8f aggregate fields | P4 | the completed label-set receipt in the store | BS-8f | any per-object label, sign or agreement leaving the stores; any field beyond the BS-8f schema |
| J | **Stage-C runner** | reads the sealed BS-2f mask (χ-free) and the BS-8f aggregates; injects synthetic signs only — **never reads a real χ** → writes the Stage-C receipt | P5, before BS-L | BS-2f and BS-8f | BS-5f | reading any real χ; continuing the run after a Stage-C FAIL |
...
| P | **Verdict path** | **post-unblinding only**: reads the real χ vector joined to the accepted mask; first re-invokes `recompute_acceptance_ledger`. **Applies confidence-based exclusion threshold**. **If this exclusion removes any object, Row P must deterministically recompute the final mask digest, calibration applicability, and Stage-C power under a pinned protocol, and must refuse the scientific verdict if any locked adequacy condition no longer holds.** | P8, after unblinding | the unblinding receipt and a verified BS-L | BS-7f, then BS-V | any execution before unblinding; any verdict produced outside this symbol |
...
4. **Access is logged and mediated.** Enforceable mediation is a **BS-2k gate condition**.
5. **The void rule.** 
...
(Carried unchanged from R5: archived, not read, governed by seal-state and enforceable mediation in BS-2k, re-receipted at BS-2f and pre-unblinding lock checkpoint).

### §6.3 General conduct clauses (carried unchanged)
- No strata in the estimator, Calibration, Void rule for changes, One change per iteration, No claim stronger than its check, Custody, Blind double.
...
2. **§2.7(4) and §5:** Remove reason (c) (instrument absence/non-finiteness) from the pre-lock structural exclusion predicates, deferring any non-finite instrument outputs to post-unblinding handling. Confidence threshold exclusion is applied post-unblinding.
3. **Verdict Path (Row P) Post-Unblinding Consequence:** Define the deterministic rule: if the post-unblinding confidence cut removes any object, `run_production_verdict()` must recompute the final mask digest, calibration applicability, and Stage-C power under a pinned post-unblinding protocol, and refuse the verdict if adequacy conditions fail. 
4. **§7 class-P table:** Remove BS-L. Add BS-2k (class P, DESIGN; blocks BS-6).
5. **§7 class-E table:** Add BS-L row. Keep BS-V verdict only. 
6. **Code-side items in the next atomic revision:** Implement Row B's hard block on Row D prior to C2's exact-parent stage-completion receipt verification. Implement the hermetic worker profile allowlist and adversarial producer fixtures for C2.

---

# PART 3 — CHOICES THE FINDINGS DID NOT FORCE, AND THE ALTERNATIVE TO EACH

- **C1 — Refusing to build a pre-lock execution supervisor for reason (c).** The brief allowed either building a mechanically sound execution supervisor for instrument presence/finiteness, or refusing/deferring reason (c) entirely. Alternative: Build the execution supervisor. We chose to REFUSE reason (c) pre-lock because building a sealed supervisor transaction is a massive expansion of scope and mechanism. The cost is that we lock a mask that may contain objects whose instrument outputs are non-finite, relying on the post-unblinding confidence cut consequence (recomputing power and potentially failing the verdict) instead of structurally excluding them early.
- **C2 — Exporting no checksums and relying on predicate bits.** The brief allowed closing the checksum channel by exporting no checksums. Alternative: Giving attempt identity a closed form while exporting checksums. We chose to export only recomputable predicate bits (`byte_integrity_pass`) and keep all cutout digests inside the sealed boundary. The cost is the loss of the capability to externally verify exact image byte hashes against a public manifest without re-running the sealed verifier.
...
- **R3 — Post-Unblinding Attrition.** By deferring both confidence and reason (c) exclusions to post-unblinding, we risk locking a calibration and Stage-C power that falls apart after unblinding when objects are excluded, which would void the run. 

---

# PART 5 — THE FINDINGS, WHERE EACH LANDS

1. **Defect 1 — The impossibility (Row C2 cannot produce execution facts before D).** REPAIR (by REFUSAL). We refused to build the pre-lock execution supervisor for instrument presence/finiteness. We dropped reason (c) from the pre-lock structural exclusion predicates. E now E never trusts a D-authored boolean; it only reads C2's pre-inference cutout facts.
2. **Defect 2 — The C2 checksum channel.** REPAIR. C2 exports no checksum at all before the lock. All cutout digests stay inside the sealed boundary. 
3. **Defect 2b — Export predicate bits, not values.** REPAIR. C2's acceptance-evidence projection exports only narrowly enumerated authenticated predicate bits (`parent_attempt_present`, `byte_integrity_pass`, `canonical_shape_pass`). Every legal value is mechanically fixed. Attempt identity must be derived from a canonical source and serialization. Exact-parent closure is enforced by the stage-completion artifact.
4. **Defect 3 — "Never invokes the classifier" is a promise, not a boundary.** REPAIR. C2 is required to run under a hermetic worker profile with a strict allowlist. Row B enforces that D cannot touch the store until C2's exact-parent stage-completion artifact verifies. Clause 9 requires the BS-2a gate to run adversarial producer fixtures that actively attempt to encode signs through all channels (rerunning the real producer under transformed cutouts) and must fail.
5. **Defect 4 — The post-unblinding confidence cut is unbound.** REPAIR. Row P is now required to deterministically recompute the final mask digest, calibration applicability, and Stage-C power if the post-unblinding exclusion removes any object. It must refuse the scientific verdict if any locked adequacy condition no longer holds.
