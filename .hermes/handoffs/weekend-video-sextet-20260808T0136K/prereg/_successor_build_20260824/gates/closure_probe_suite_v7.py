...
So the production entry point is now `closure_receipt()`, which runs `ref/closure_worker_v7.py` in
...
SUBJECT = (HERE / ".." / "ref" / "successor_ref_v7.py").resolve()
FIXTURES = (HERE / ".." / "ref" / "FIXTURES_V7_20260826.out").resolve()
WORKER = (HERE / ".." / "ref" / "closure_worker_v7.py").resolve()
...
def load_subject():
    spec = importlib.util.spec_from_file_location("closure_probe_subject_v7", SUBJECT)
...
@probe("B07", "a JSON object rather than an array as the worker's manifest",
       ...
       mentions="must be a JSON array", direct="closure_worker_v7.py subprocess")
def b07(c):
...
@probe("B04", "the worker started without -I",
       ...
       mentions="isolated", direct="closure_worker_v7.py subprocess")
def b04(c):
...
@probe("B05", "a worker whose subject bytes do not match its pin",
       ...
       mentions="SUBJECT DIGEST MISMATCH", direct="closure_worker_v7.py subprocess")
def b05(c):
...
@probe("G01", "the digests and provenance this run actually observed, through the boundary",
       ...
       direct="closure_worker_v7.py --self-check plus one boundary call")
def g01(c):
...
def main(argv=None):
    ...
    stable = {
        "receipt_version": "2",
        "subject": {"path": "../ref/successor_ref_v7.py", "sha256": sha256_of(SUBJECT)},
        "suite": {"path": "closure_probe_suite_v7.py", "sha256": sha256_of(Path(__file__))},
        "fixtures": {"path": "../ref/FIXTURES_V7_20260826.out",
