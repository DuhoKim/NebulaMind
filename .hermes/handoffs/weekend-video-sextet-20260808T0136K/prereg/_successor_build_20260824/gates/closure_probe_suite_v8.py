...
So the production entry point is now `closure_receipt()`, which runs `ref/closure_worker_v8.py` in
...
SUBJECT = (HERE / ".." / "ref" / "successor_ref_v8.py").resolve()
FIXTURES = (HERE / ".." / "ref" / "FIXTURES_V8_20260826.out").resolve()
WORKER = (HERE / ".." / "ref" / "closure_worker_v8.py").resolve()
...
NOT_COVERED = [
    ...
    "are pinned; its parser is not. Every receipt records the full sys.path so this is visible.",
...
def load_subject():
    spec = importlib.util.spec_from_file_location("closure_probe_subject_v8", SUBJECT)
...
@probe("B07", "a JSON object rather than an array as the worker's manifest",
       ...
       mentions="must be a JSON array", direct="closure_worker_v8.py subprocess")
def b07(c):
...
@probe("B04", "the worker started without -I",
       ...
       mentions="-I", direct="closure_worker_v8.py subprocess",
       verify=lambda o, r: (r.get("isolated") is False,
                            "payload records isolated=False, not merely a phrase in the message"))
def b04(c):
...
@probe("B05", "a worker whose subject bytes do not match its pin",
       ...
       mentions="SUBJECT DIGEST MISMATCH", direct="closure_worker_v8.py subprocess")
def b05(c):
...
@probe("W01", "the worker started without -I, in an environment carrying a poisoned import",
       ...
       direct="closure_worker_v8.py subprocess",
...
@probe("W02", "the subject file counted, once when only hashed and once through a full closure",
       ...
       direct="closure_worker_v8.py under sys.addaudithook",
...
@probe("G01", "the digests and provenance this run actually observed, through the boundary",
       ...
       direct="closure_worker_v8.py --self-check plus one boundary call")
def g01(c):
...
def main(argv=None):
    ...
    stable = {
        "receipt_version": "2",
        "subject": {"path": "../ref/successor_ref_v8.py", "sha256": sha256_of(SUBJECT)},
        "suite": {"path": "closure_probe_suite_v8.py", "sha256": sha256_of(Path(__file__))},
        "fixtures": {"path": "../ref/FIXTURES_V8_20260826.out",
