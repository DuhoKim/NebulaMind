...
with open("SECTION6_DRAFT_AGY_R10B.md", "r") as f:
    text = f.read()
...
if old_status not in text and new_status not in text:
    print("Failed to replace Status")
...
if old_row_p not in text and new_row_p not in text:
    print("Failed to replace Row P")
...
if old_p2 not in text and new_p2 not in text:
    print("Failed to replace Part 2 Item 4")

# 4. Update Part 3
old_p3 = "The Stage C power failure threshold (< 962 passing trials out of 1,000) is carried from `../ref/successor_ref_v9.py` lines 77–78. Alternative: Invent decision thresholds for the missing gaps (like post-attrition Stage C power or calibration applicability). We chose to cite the frozen record and assign an ordered tree of fail-closed terminal consequences (`INCONCLUSIVE-BY-CALIBRATION` and `INCONCLUSIVE-BY-POWER`) where the frozen criteria cannot be applied, because V15 lines 570–573 void any post-first-real-χ change to a decision threshold, and inventing numbers renames a finding."
new_p3 = "The Stage C power failure threshold (< 962 passing trials out of 1,000) is carried from `../ref/successor_ref_v9.py` lines 77–78. Alternative: Maintain an `INCONCLUSIVE-BY-POWER` branch for cases where the power criterion is inapplicable (e.g. if the trial count changes). We chose to delete the inapplicability branch entirely and apply the pinned `< 962` rule alone, because lines 1275–1277 of the frozen code only admit a trial count exactly equal to `N_TRIALS = 1000`. No lawful state exists where the count differs without a protocol breach. Any deviation from the pinned 1,000-trial protocol or the frozen Stage-C implementation is a departure from the frozen protocol, and V15 lines 570–573 make any post-first-real-χ deviation from a binding rule `VOID`, not inconclusive. Emitting a softer terminal state for a protocol breach would be the document excusing its own violation. For calibration applicability, we cite the frozen record and assign `INCONCLUSIVE-BY-CALIBRATION` where the frozen criteria cannot be applied, because inventing numbers renames a finding."
text = text.replace(old_p3, new_p3)
if old_p3 not in text and new_p3 not in text:
    print("Failed to replace Part 3")
...
if old_p5_8 not in text and new_p5_8 not in text:
    print("Failed to replace Part 5 Item 8")
