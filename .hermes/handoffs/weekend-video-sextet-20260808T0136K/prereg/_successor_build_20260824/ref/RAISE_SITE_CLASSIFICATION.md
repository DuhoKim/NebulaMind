...
**ADDENDUM — known IMPLICIT exception paths, hand-enumerated as found (GPT56-V95 F5, CODEX-V95 F7; append-only; CORRECTED AT V97 from the actual bytes after GPT56-V96 F4 showed two of three rows misread — the lesson this corpus keeps teaching, applied to its own ledger):** L1493–1496 `adjudicate_path` — dict subscripts `cal[...]` and numpy reductions: implicit KeyError/TypeError — and ValueError from `np.min`/`np.max` on an empty array (GPT56-V97 F7) — UNCAUGHT on the verdict path → process death, no verdict record (the operator-observed platform family; loud, never a silent verdict); L1609 `run_production_verdict` — the `adjudicate_path(cal)` call, the propagation site of the same, same disposition; L1647–1648 `require_complete_sample` — `int()` casts of supplied counts: implicit ValueError/TypeError, CALLER by the as-supplied boundary (its explicit raise at 1649 is already a row).**
...
| 986 | `local_pass` | `RuntimeError` | **PLANNING-INTERNAL** | moved | MOVE_CAP reached — FAIL |
...
| 1641 | `require_authorization` | `RuntimeError` | **CALLER** |  | authorization unreadable: {} (tests the supplied authorization pair; unreadable path = argument admissibility — moved CALLER, CODEX-V99 F5) |
| 1643 | `require_authorization` | `RuntimeError` | **CALLER** |  | authorization digest mismatch: {} (tests the supplied authorization pair; digest mismatch of supplied inputs — moved CALLER, CODEX-V99 F5) |
...
| 1677 | `resolve_branch` | `RuntimeError` | **CALLER** |  | the choice-point cannot close for Branch B before {}: DR11 photo-z may (tests the supplied choice-point pair pre-run; no VOID antecedent owns it — moved CALLER, GPT56-V91 F8) |
| 1681 | `resolve_branch` | `RuntimeError` | **CALLER** |  | after {} the choice-point is closed on Branch B; selecting A requires  (tests the supplied choice-point pair pre-run; no VOID antecedent owns it — moved CALLER, GPT56-V91 F8) |
