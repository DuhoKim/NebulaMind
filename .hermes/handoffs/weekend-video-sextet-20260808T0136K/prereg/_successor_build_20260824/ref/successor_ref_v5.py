...
PINNED_PLANNER_DIGEST = "10cea7a6a16458b9f0bcb3534f7afaeb1173ba98a89ac4c7cd709db98e4bb8d1"
...
def frozen_planner_digest() -> str:
    ...
    # V5 (GPT56-V5 F4): the bytes on disk are not the code that runs. That seat rebound the
    # already-imported adapter's plan_object in memory; the file digests were untouched, this
    # function still returned the pinned value, and the replacement then produced the answer.
    # So the LIVE callables are hashed too: their bytecode, constants, names and defaults.
    for label, fn in (("plan_candidate_bricks", m.plan_candidate_bricks),
        ...
        parts.append(f"live:{label}:".encode() + _code_fingerprint(code)
                     + _stable_repr(getattr(fn, "__defaults__", None)).encode())
...
def load_pinned_counts():
    """Loads the count oracle from its PINNED path and verifies the file's digest against the
    pinned constant, exactly as load_pinned_geometry does for the sidecar.

    V4 took the oracle as a caller-supplied path. Both referee seats defeated that: an edited
    table whose grand total still equalled the release total passed the completeness proof
    (C01), and so did the same bytes reached through a symlink (C03). A digest computed over a
    file the caller nominated is custody metadata, not an independent witness.

    Everything this returns is derived from the pinned file: the per-brick eligible counts, the
    row count and the grand total. Schema violations are refused here rather than surviving as
    silently coerced values (CODEX-V5 F7: duplicate keys, negative or non-integral counts).
    """
    from pathlib import Path as _P
    table = _P(__file__).resolve().parents[2] / PINNED_COUNTS_REL
    if not table.is_file():
        raise ManifestClosureError(f"pinned count table missing: {table}", {"counts": str(table)})
    got = sha256_file(table)
    if got != PINNED_COUNTS_SHA256:
        raise ManifestClosureError(
            f"COUNT TABLE DIGEST MISMATCH: {got} != pinned {PINNED_COUNTS_SHA256}",
            {"counts_sha256": got})
    import csv as _csv
    counts, total = {}, 0
...
def load_pinned_selection():
    """Loads the authorized selection from its PINNED path with its digest verified.

    V4 accepted any non-empty unique `selected_brickid` array from the caller, so a selection
    reduced to match a parent missing rows passed (C02). The set of bricks the study analyses is
    not something the closure call may be told; it is fixed before the call exists.
    """
    from pathlib import Path as _P
    path = _P(__file__).resolve().parents[2] / PINNED_SELECTION_REL
    if not path.is_file():
        raise ManifestClosureError(f"pinned selection missing: {path}", {"selection": str(path)})
    got = sha256_file(path)
    if got != PINNED_SELECTION_SHA256:
        raise ManifestClosureError(
            f"SELECTION DIGEST MISMATCH: {got} != pinned {PINNED_SELECTION_SHA256}",
            {"selection_sha256": got})
    z = np.load(path)
...
def load_pinned_parent():
    """Loads the authorized parent from its PINNED path, verified against BOTH the constant
    above and the digest the fetch recorded in its own receipts.

    V4 hashed a caller-supplied parent and then validated only columns, selection membership and
    per-brick row counts, so a row carrying an unused ls_id and another object's coordinates
    passed while understating the required brick set (C04). Row counts prove cardinality, not
    identity. The producer receipt is what supplies identity here: positions_receipts.json was
    written by the fetch, against TAP job URLs, before any closure code existed.
    """
    import csv as _csv, json as _json
    from pathlib import Path as _P
    root = _P(__file__).resolve().parents[2]
    path, rec_path = root / PINNED_PARENT_REL, root / PINNED_PARENT_RECEIPTS_REL
    for pth in (path, rec_path):
        if not pth.is_file():
            raise ManifestClosureError(f"pinned parent input missing: {pth}", {"missing": str(pth)})
    got = sha256_file(path)
    if got != PINNED_PARENT_SHA256:
        raise ManifestClosureError(
            f"PARENT DIGEST MISMATCH: {got} != pinned {PINNED_PARENT_SHA256}",
            {"parent_sha256": got})
    rec = _json.loads(rec_path.read_text())
    if rec.get("output_sha256") != got:
        raise ManifestClosureError(
            f"PARENT NOT THE FETCHED ARTIFACT: producer receipt records "
            f"{rec.get('output_sha256')}, file is {got}",
            {"receipt_sha256": rec.get("output_sha256"), "parent_sha256": got})
    chunk_rows = sum(int(c["rows"]) for c in rec.get("chunks", []))
    if int(rec.get("total_rows", -1)) != chunk_rows:
        raise ManifestClosureError(
            f"producer receipt is internally inconsistent: total_rows {rec.get('total_rows')} "
            f"!= sum over chunks {chunk_rows}", {"chunk_rows": chunk_rows})
    rows = []
    with path.open() as f:
        rd = _csv.DictReader(f)
        ...
        if not need.issubset(set(rd.fieldnames or [])):
            raise ManifestClosureError(f"parent columns {rd.fieldnames} lack {sorted(need)}",
                                       {"columns": rd.fieldnames})
        for r in rd:
            rows.append((int(r["ls_id"]), int(r["brickid"]), float(r["ra"]), float(r["dec"])))
...
def close_manifest(manifest_bricknames) -> dict:
    ...
    bindings:
      ...
      4. the parent's COMPLETENESS, proved against the count table loaded in (3): the parent's
         per-brick row counts must equal the pinned eligible counts for exactly the selected
         bricks. A parent missing rows fails here and cannot repair itself by editing the
         table, because the table is not one of its inputs.

    V5 closes the seam both seats walked through in the V4 review: `oracle_npz` is gone. The
    count table is read from its pinned path with its digest verified (F1 on both reports), and
    every selected brick must resolve in the pinned geometry universe (CODEX-V5 F7).

    V5 closes all three seams the V4 review found. The signature now carries ONE argument: the
    candidate manifest being judged. Geometry, planner, count table, selection and parent are
    all loaded from pinned paths with pinned digests, and the parent is additionally required to
    match the digest its own fetch receipts recorded. There is no path by which the caller can
    nominate the artifact that judges it.

    What remains weaker than the rest, stated rather than hidden: the selection's digest is a
    code pin only. The parent has a producer receipt; the selection does not yet.
    ...
    try:
        ...
        parent_rows, par_sha = load_pinned_parent()
        ...
        rows, per_brick, incoherent = [], {}, []
        for (lsid, b, ra, dec) in parent_rows:
            if b not in selset:
                raise ManifestClosureError(
                    f"parent row in brick {b}, which is not in the selection",
                    {"stray_brick": b})
            # V5 (probe U02): row counts prove cardinality, not that a row sits where it says.
            # A row whose coordinates fall outside the brick it declares understates the plan,
            # because the plan is derived from the coordinates while the count proof is keyed on
            # the brickid. Both must describe the same object.
            g = by_id[b]
        ...
        if incoherent:
            raise ManifestClosureError(
                f"PARENT ROW INCOHERENT: {len(incoherent)} row(s) carry coordinates outside the "
                f"brick they declare, first {incoherent[:4]}",
                {"incoherent_rows": len(incoherent), "examples": [list(x) for x in incoherent[:4]]})
        if not rows:
        ...
        result = {"sidecar_sha256": sidecar_sha, "planner_digest": planner_sha,
                  ...
                  "counts_sha256": counts_sha, "plan_digest": digest(plan_payload),
...
def run_fixtures():
    ...
    # ---- closure: every artifact that judges the manifest is pinned, not passed in.
    # V4 took four arguments and both referee seats walked through three of them: the count
    # oracle, the selection and the parent were all nominated by the caller. V5 takes ONE --
    # the candidate manifest -- so those inputs no longer exist. The end-to-end closure moved
    # to gates/closure_probe_suite.py, which runs it on the real 65,060-object parent; keeping
    # it here would mean a toy parent, and a toy parent cannot satisfy a pinned count table.
    try:
        import inspect as _insp
        _cm = _insp.getsource(close_manifest)
        sig = list(_insp.signature(close_manifest).parameters)
        ok &= _fx("CLOSURE-ONE-ARGUMENT", sig == ["manifest_bricknames"], lines,
                  f"close_manifest{tuple(sig)} — no oracle, selection, parent, digest, geometry "
                  f"or receipt argument")
        ...
        ok &= _fx("CLOSURE-PINNED-COUNTS", csha == PINNED_COUNTS_SHA256
                  ...
                  f"count table read from its pinned path, digest verified ({csha[:12]}…), "
                  f"{len(counts):,} bricks totalling {sum(counts.values()):,}")
        ...
        ok &= _fx("CLOSURE-PINNED-SELECTION", ssha == PINNED_SELECTION_SHA256
                  ...
                  f"selection read from its pinned path, digest verified ({ssha[:12]}…), "
                  f"{len(selset):,} bricks")
        prows, psha = load_pinned_parent()
        ok &= _fx("CLOSURE-PINNED-PARENT", psha == PINNED_PARENT_SHA256
                  and len(prows) == PINNED_PARENT_ROWS, lines,
                  f"parent read from its pinned path, digest verified ({psha[:12]}…) against "
                  f"both the constant and the fetch receipt, {len(prows):,} rows")
