...
def verified_bytes(path, expected_sha256, what):
    ...
    The file must be a regular file and must not be reached through a symlink; both are checked
    on the open descriptor, not on the path, so the answer cannot change underneath.
    """
    import os as _os
    from pathlib import Path as _P
    path = _P(path)
    if path.is_symlink():
        raise ManifestClosureError(f"{what} is a symlink: {path}", {"symlink": str(path)})
    try:
        # O_NONBLOCK so that a FIFO standing where a regular file should be is refused rather
        # than blocking this process forever waiting for a writer. For a regular file the flag
        # changes nothing about the read.
        fd = _os.open(str(path), _os.O_RDONLY | _os.O_NONBLOCK)
...
def closure_receipt(manifest_bricknames, python_executable=None) -> dict:
    ...
    from pathlib import Path as _P
    worker = _P(__file__).resolve().parent / "closure_worker.py"
    ...
    with _tf.TemporaryDirectory(prefix="closure_run_") as td:
        proc = _sp.run([python_executable or sys.executable, "-I", str(worker), "--work-dir", td],
...
def run_fixtures():
    ...
    try:
        ...
        ok &= _fx("CLOSURE-SINGLE-OPEN",
                  "fstat" in _vb and "S_ISREG" in _vb and "is_symlink" in _vb, lines,
                  "artifacts are read once, hashed as read, and parsed from that snapshot "
                  "(CODEX-V5 F3)")
