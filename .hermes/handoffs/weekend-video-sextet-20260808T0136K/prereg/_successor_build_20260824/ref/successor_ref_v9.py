...
def closure_receipt(manifest_bricknames) -> dict:
    ...
    from pathlib import Path as _P
    worker = _P(__file__).resolve().parent / "closure_worker_v9.py"
    ...
    # v9 (KIMI-V8 F3): the worker's provenance was dropped here for PASS and REFUSE alike, so
    # no receipt carried the sys.path that two written claims said it carried. It travels with
    # the result now, which makes those claims true rather than narrowed.
    prov = out.get("provenance") or {}
    if out.get("outcome") == "REFUSE":
        result = dict(out.get("result") or {})
        result["worker_provenance"] = prov
        raise ManifestClosureError(out.get("message", "closure refused"), result)
