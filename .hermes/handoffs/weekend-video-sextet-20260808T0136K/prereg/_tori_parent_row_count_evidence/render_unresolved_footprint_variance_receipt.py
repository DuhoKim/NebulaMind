...
EXECUTED_RUNNER = RUN / "execution_runner.py.txt"
DISABLED_RUNNER = ROOT / "run_authorized_footprint_variance.py"
EXPECTED = {
    "query": "5d4c7812331419eff0ec7dca4e40f690203cb94cc71b6309d7b8694299249ff1",
    "submission": "b8a11ab632131d2daebc08d48a0c92d0d63a9695bee5b5e540bd722baca79912",
    "abort": "c49d43f81c3f245bb349b7b69c77a7d319a6849c273866933a17a243fc73483b",
    "job_xml": "7f762a6ca94152b8d2026c7c2e3af755431bcff6fe554ae64bb8833126bd2b80",
    "lifecycle": "af8f71f5dcb1a4a965a175b828919c4751f390cc2a70e27f15d8666ec6bd22ab",
    "ordinary_guard": "228a045a9c896ca7bef6dc199e5988bbd0d222e5c027cdee3c1d6d23842a1a51",
    "executed_runner": "0ffeb0d79f0b70faa37b0e0ef17db52988adba9516163b409c663e4c349bd826",
    "disabled_runner": "5ff98618c6d8dd8ed1f19d2ba7843fe94ce78af503247212df2ce8a6d1d91de9",
    "route": "3f41b6d925c0b540120f94636e4d78a045bebd1ed579293e4ec6f6d9163d3a87",
    "cut6": "ed6b6e5e957903473c7692d5973f3b2d05a991916ce3aa247365938b0f414651",
    "full": "9d62960718b4f7aa1bb2eb67a9fddb83d6712698e1bc323fb1d21d1f4965e020",
}
...
def main() -> None:
    paths = {
        ...
        "abort": RUN / "abort_receipt.json",
        "job_xml": RUN / "aborted_job.xml",
        "lifecycle": RUN / "guard_lifecycle.json",
        "ordinary_guard": ORDINARY_GUARD,
        "executed_runner": EXECUTED_RUNNER,
        "disabled_runner": DISABLED_RUNNER,
        "route": PREREG / "TORI_SURVEY_ROUTE_BINDING_20260812.md",
        "cut6": PREREG / "TORI_CUT6_INCLINATION_COUNT_20260812.md",
        "full": PREREG / "TORI_FULL_KEYSPACE_SWEEP_20260813.md",
    }
    for label, path in paths.items():
        if not path.exists() or sha(path) != EXPECTED[label]:
            raise RuntimeError(f"custody drift: {label}: {path}")
    if list(SCOPE.rglob("result.csv")):
        raise RuntimeError("variance scope unexpectedly contains a result")
    if (RUN / "receipt.json").exists():
        raise RuntimeError("variance scope unexpectedly contains a completed machine receipt")
    if (SCOPE / "partitioned").exists():
        raise RuntimeError("unapproved partition scope exists")
    ...
    if submission["submission_attempts"] != submission["submission_limit"] or submission["submission_limit"] != 1:
        raise RuntimeError("one-query authorization violated")
    if submission["query_sha256"] != EXPECTED["query"] or abort["query_sha256"] != EXPECTED["query"]:
        raise RuntimeError("query custody mismatch")
    if submission["job_url"] != abort["job_url"]:
        raise RuntimeError("job custody mismatch")
    if abort["phase_before_abort"] != "EXECUTING" or abort["phase_after_abort"] != "ABORTED":
        raise RuntimeError("abort phase custody mismatch")
    if abort["result_retrieved"] or abort["aggregate_result_rows_exported"] != 0:
        raise RuntimeError("unexpected result retrieval")
    for key, expected in {
        ...
        if abort[key] != expected:
            raise RuntimeError(f"boundary violation: {key}")
    if lifecycle["exception_state"] != "CLOSED" or not lifecycle["ordinary_guard_unchanged"]:
        raise RuntimeError("guard lifecycle not closed")
    if not lifecycle["ordinary_guard_verified_rejects_query_after"] or not lifecycle["exception_runner_disabled"]:
        raise RuntimeError("guard/exception path not restored")
    if lifecycle["ordinary_guard_sha256_before"] != lifecycle["ordinary_guard_sha256_after"] != EXPECTED["ordinary_guard"]:
        raise RuntimeError("ordinary guard hash mismatch")

    query_text = QUERY.read_text().rstrip()
    md = f"""# TORI — Longo-axis footprint-variance attempt receipt
...
The one narrow query authorized by Duho was submitted exactly once. It remained in UWS phase `EXECUTING` for `03:09:56`, from `{submission['recorded_utc']}` until the abort request at `{abort['abort_requested_utc']}`. It was then explicitly aborted and reached terminal phase `ABORTED` at `{abort['phase_observations_after_abort'][0]['timestamp_utc']}`.
...
- `TORI_SURVEY_ROUTE_BINDING_20260812.md` — SHA-256 `{EXPECTED['route']}`;
- `TORI_CUT6_INCLINATION_COUNT_20260812.md` — SHA-256 `{EXPECTED['cut6']}`;
- `TORI_FULL_KEYSPACE_SWEEP_20260813.md` — SHA-256 `{EXPECTED['full']}`; exact dered Cut-6 count `832,393` and direct zero-tail closure.
...
- ordinary guard path: `{ORDINARY_GUARD}`;
- ordinary guard SHA-256 before: `{lifecycle['ordinary_guard_sha256_before']}`;
- ordinary guard verified to reject query before: **YES**;
- one-time exception opened: `{lifecycle['opened_utc']}`;
- exact authorized query SHA-256: `{EXPECTED['query']}`;
- submissions made / limit: `1/1`;
- executed one-time runner custody SHA-256: `{EXPECTED['executed_runner']}`;
- one-time submission path closed: `{lifecycle['closed_utc']}`;
...
- disabled stub SHA-256: `{EXPECTED['disabled_runner']}`;
...
- job: `{submission['job_url']}`;
- submitted UTC: `{submission['recorded_utc']}`;
- phase before abort: `{abort['phase_before_abort']}`;
- abort requested UTC: `{abort['abort_requested_utc']}`;
- terminal phase: `{abort['phase_after_abort']}`;
- query SHA-256: `{EXPECTED['query']}`;
- submission record SHA-256: `{EXPECTED['submission']}`;
- abort receipt SHA-256: `{EXPECTED['abort']}`;
- terminal job XML SHA-256: `{EXPECTED['job_xml']}`;
- guard lifecycle SHA-256: `{EXPECTED['lifecycle']}`;
...
"""
    ...
    print(f"rendered={OUTPUT} status=UNRESOLVED job={submission['job_url']}")
