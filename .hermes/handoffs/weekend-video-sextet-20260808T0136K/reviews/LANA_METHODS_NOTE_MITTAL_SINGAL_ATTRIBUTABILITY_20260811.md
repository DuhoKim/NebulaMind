...
> **Revision 2 (2026-08-11 13:31 KST) — repairs per Kun's gate
> `KUN_METHODS_NOTE_MITTAL_SINGAL_OVERCLAIM_GATE_20260811T1314K.md` (BLOCK_METHODS_NOTE_OVERCLAIM_AND_
> PROVENANCE_MISSTATEMENT). Four corrections, carried openly, not quietly fixed:** (1) *"identical data /
> verified identical"* downgraded to *"strongly supported as the same v0.1.0 release; Mittal does not
> self-bind exact input bytes; a release mismatch is not supported as the explanation"* — Rev 1 contradicted
> Tori's exact custody finding; (2) §3 *"second-order choices are ruled out"* → *"not sufficient by
> themselves to explain the disagreement"* — Rev 1 performed the causal narrowing this note refuses; (3) the
> kinematic-null statement was stale and materially wrong (it claimed the two adopt "essentially the same
> null") — corrected to the actual, **different** constructions; (4) provenance made exact, nothing resting
> on Hwao's non-authoritative artifact. Rev 1 text is superseded by the corrected sentences below.

> **Revision 3 (2026-08-11) — repairs per Kun's re-gate
> `KUN_METHODS_NOTE_MITTAL_SINGAL_REPAIR_REGATE_20260811T1331K.md` (SHA-256 `5515e927…`,
> BLOCK_REPAIR_STILL_OVERCLAIMS_AND_CARRIES_STALE_NULL). What passed in Rev 2 is untouched (identity
> downgrade, causal-narrowing repair, §3 coupled-choices framing). Three fixes, each recorded verbatim:**
> **(1) §3 stale null values.** Rev 2 announced the kinematic-null correction and then printed Mittal's
> superseded original-analysis amplitudes — *"yielding an expected `D̄≈0.0080` (Quaia low) / `0.0068` (high)
> and `v=369.82 km s⁻¹`"* — and inferred *"These expected amplitudes are numerically comparable"*. Kun: worse
> than vagueness, because it tells the reader a correction was made while showing the uncorrected numbers.
> Replaced with the corrected expected amplitudes **`0.0048` (Quaia low) / `0.0043` (high)** per Tori's
> custody receipt (`TORI_TO_HWAO_MITTAL_SINGAL_CUSTODY_ASSESSMENT_RECEIPT_20260811T1125K.md`, the
> spectral-index-correction bullet); the old pair is retained only under an explicit **superseded** label, and
> the "numerically comparable" inference is deleted, not restated with new numbers.
> **(2) Goru removed from provenance.** §4 said *"The factual base for this note is **Tori's custody record
> and Goru's numbers**"*; §6 said *"or supplied by Goru's read of the public products"*; the provenance table
> said *"**Goru** numbers + **Lana** primary read"*. Goru's only quasar-dipole artifact in the lane
> (`GORU_QUASAR_DIPOLE_DESIGN_BRIEF_20260811T0959K.md`) is a design brief containing none of the numbers
> attributed to him, and Tori's receipt records that the required distinct Goru facts artifact was absent at
> gate time. All three attributions re-sourced to **Tori custody record + Lana primary read**. No Goru path
> invented to satisfy the table.
> **(3) §5 overhardening narrowed.** *"We separately established (Lana novelty judgment; Tori exact-artifact
> gate) that this cannot be custody-frozen on public data: the selection treatment cannot be pre-declared to
> custody grade without seeing data"* — stronger than anything established — replaced with Kun's narrower
> formulation: **not available from the published record as-is; a separate reconstruction scope would first
> need Tori to verify exact recoverable artifacts or record each missing implementation choice.** The same
> overhard phrase echoed in the Summary (*"which we separately established cannot be custody-frozen on public
> data"*) is narrowed identically.
> **(4) Sourcing re-check (ordered).** Hwao's `HWAO_LOOSENED_BAR_MITTAL_SINGAL_ASSESSMENT_20260811T1110K.md`
> (SHA-256 `1bdf7d66…`) is NON-AUTHORITATIVE. Re-checked every factual claim: none traces to it — the
> estimator description here (Bayesian model comparison over pixel counts, not spherical-harmonic), the stated
> magnitude cuts, and the 35°/40° latitude cuts all contradict that artifact and trace to Lana's primary read
> and Tori's receipt. Nothing needed re-sourcing beyond fix (2).
...
## Disposition
Methods note **repaired to Revision 3** per Kun's re-gate (`BLOCK_REPAIR_STILL_OVERCLAIMS_AND_CARRIES_STALE_
NULL`): the §3 kinematic-null now carries Mittal's corrected expected amplitudes `0.0048/0.0043` (old pair
labelled superseded, the "numerically comparable" inference deleted), Goru is removed from §4, §6 and the
provenance table (no Goru facts packet exists in the lane; attributions re-sourced to Tori custody record +
Lana primary read), and §5 plus its Summary echo are narrowed to "not available from the published record
as-is" with the reconstruction-scope precondition. Rev 2's accepted repairs are untouched. Ordered sourcing
re-check: no factual claim traces to Hwao's non-authoritative assessment artifact. Finding unchanged: **the
disagreement is not attributable from the published record.** Not published, uploaded, or accepted — Duho
decides after reading. **Back to Kun to re-gate; Tori to bind sources.** Relay submitted.
