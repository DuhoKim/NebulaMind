...
**Draft methods note, 2026-08-11. Not published.** Every factual claim below is traced (§Provenance) to a
published source or to a check this note itself performed and states. **We did not re-run either analysis and
do not claim to.**
...
**not available from the published record as-is** (§4). **This note reports only what can and cannot
be established from the published record; it adjudicates nothing and makes no cosmological claim.**

## 1. The inputs are strongly supported to be the same release — but not byte-verified on Mittal's side
What the published record and this note's own product check establish, stated exactly:
...
- **For this note, the v0.1.0 catalogues and selection maps were downloaded and checked against the published
  MD5 checksums and byte counts; they match** — i.e. the v0.1.0 *products* are verified, and Singal's binding
  to them is verified.
...
> *The product check above is this note's own. Checking the public products against their published checksums
> verifies the products and Singal's binding to them — it does not, and cannot, upgrade the identity of
> Mittal's inputs beyond supported, not byte-verified on Mittal's side.*

## 2. The principal method fork is openly stated, not hidden — in the papers themselves
...
The fork is the *stated difference* between the papers; the **magnitude** of its effect is not attributable
from them, because the choices are coupled, Singal's side carries its own unstated order-unity correction, and
the two papers estimate different quantities (§3). The selection-function fork therefore remains the leading
stated difference, **not** the demonstrated explanation of the amplitude gap.

## 3. Why the factor of three cannot be attributed from the record
This note's reading of the two papers finds the published record does not permit isolating the cause, for
...
**The kinematic-null constructions also differ.** The two do **not** adopt the same null:
...
- **Mittal** does **not** use that closed form: he **forward-models Doppler boosting of the actual source
  ...
  the observed `α` distribution, at `v=369.82 km s⁻¹`. **With the spectral-index correction published in
  Mittal's current paper, the expected amplitudes are `D̄≈0.0048` (Quaia low) / `0.0043` (high)** (that
  published correction also increases prior sensitivity and makes amplitude consistency less decisive;
  **Singal uses the uncorrected input**). The original-analysis values `0.0080/0.0068` are **superseded by
  that published correction** and are quoted here only so the historical numbers carry their superseded label
  rather than circulating unlabelled.
The two null constructions are different, **and this note draws no inference — of attribution or exclusion —
from any numerical relation between their expected amplitudes.** The differing null constructions are one
more reason the two numbers are not the same estimand.

## 4. What would settle it — and what that would require
...
reconstruct (§3). **A separate reconstruction effort would first need to verify which exact artifacts are
recoverable, or record each missing implementation choice as an explicit replacement convention.**
This note does not attempt it.

## 5. Limitation and claim boundary (binding)
- **We did not re-run either analysis.** No statistic here is ours; all measured values are quoted from the
  papers themselves or from this note's own check of the public data products.
...
## Provenance (every factual claim → published source, or this note's own stated check)
| Claim | Source |
|---|---|
| Singal self-binds Quaia v0.1.0 (DOI 10.5281/zenodo.8060755) and states it is Mittal's release; Mittal self-binds no record and predates public v1.0.0 | **Singal 2024** (MNRAS 532, L1); **Mittal et al. 2024** (MNRAS 527, 8497) — absence of a self-bound record in the latter |
| The v0.1.0 products match the published MD5 checksums and byte counts; **Mittal's exact input bytes are NOT byte-verified** — "strongly supported same release," not "verified identical" | **This note's own check**: v0.1.0 catalogues and selection maps downloaded and compared against the checksums and byte counts published with DOI 10.5281/zenodo.8060755 |
| Selection-function fork; Mittal `λᵢ·sᵢ`, `1/sᵢ`; Singal verbatim omission quotes | **Mittal et al. 2024** (likelihood construction); **Singal 2024** (quoted verbatim) |
| Coupled masking+selection; unstated O(1) mask correction; different estimands (Bayesian model comparison vs direct count dipole) | **This note's own reading** of both papers (§3) — analysis, not citation |
| Singal cuts (`mG<20.5/20.0/20<mG<20.5`; `|b|>30/35/40`); Mittal `Nside=64`, `|b|<40°`+`30∗` | **Singal 2024**; **Mittal et al. 2024** (stated in the papers) |
| Kinematic-null constructions **differ**: Singal closed-form `[2+x(1+α)]`, `x=1.3, α≈2.4`→≈6.4, `v≈370`; Mittal forward Doppler-boost of actual counts, per-source `αᵢ`, `v=369.82` | **Mittal et al. 2024**; **Singal 2024** |
| Corrected Mittal expected amplitudes `D̄≈0.0048/0.0043` (published spectral-index correction; original `0.0080/0.0068` superseded; Singal uses the uncorrected input) | **Mittal et al. 2024, current version** (the correction is published there) |
| Controlled re-derivation **not available from the published record as-is**; a reconstruction effort needs verified recoverable artifacts or recorded replacement conventions | **This note's own analysis** (§4), from what both published packages omit: analysis code, exact mask memberships, the exact O(1) correction implementation |
