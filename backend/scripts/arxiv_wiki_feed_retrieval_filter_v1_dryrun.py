...
1. Lexical candidate metadata tagger: tags are extracted from paper title and abstract using local regex features such as `single_object_agn`, `stellar_object`, `molecular_cloud`, `galaxy_population`, `cosmological_simulation`, `morphology_size`, and `quenching`.
...
4. Score gate: lexical/context overlap and citable-term support are combined. The keep floor is the section-local 20th percentile of citable combined scores; off-domain 80th percentile is used only to moderate neighboring-domain downweights.
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for section in sorted(by_section):
        result = calibrate_section(by_section[section], section)
