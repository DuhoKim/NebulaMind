#!/usr/bin/env python3
"""
AstroSage-70B quality boost for 28 restored pages.
Queries the 10 lowest-health pages from the restored set (excluding page_id=57),
finds the weakest non-reference ## section in each, rewrites it with AstroSage-70B,
and submits the improvement via POST /api/edits (agent_id=55).
"""
import json
import logging
import sys
...
def get_lowest_health_pages(slugs: list[str], top_n: int) -> list[dict]:
    """Query DB for health scores, return top_n lowest (excluding page_id=57)."""
    db = SessionLocal()
    try:
        pages = (
...
def pick_weakest_section(content: str) -> tuple[str, str]:
    """Return (section_header_line, full_section_text) of shortest non-reference ## section."""
    ...
    candidates = [
        ...
        if hdr.lstrip("# ").strip().lower() not in SKIP_SECTIONS and len(lines) > 1
    ]

    if not candidates:
        if sections:
            hdr, lines = sections[0]
            return hdr, "\n".join(lines)
        return "", ""

    hdr, lines = min(candidates, key=lambda x: sum(len(l.split()) for l in x[1]))
    return hdr, "\n".join(lines)
...
def replace_section(content: str, old_section: str) -> tuple[str, bool]:
    """Replace old_section block in content. Returns (new_content, replaced)."""
    if old_section and old_section in content:
        return content, False  # will be replaced below
...
def main():
    log.info("=== AstroSage Restored Pages Boost ===")
    ...
    for entry in targets:
        ...
        section_header, section_text = pick_weakest_section(content)
        if not section_text:
            log.warning("No rewritable section found in %s — skipping", slug)
            failed += 1
            continue

        word_count = sum(len(l.split()) for l in section_text.split("\n"))
        log.info("Weakest section: '%s' (%d words)", section_header, word_count)
        ...
        summary = (
            f"AstroSage-70B rewrite of weakest section '{section_header.lstrip('# ').strip()}' "
            f"[restored_pages_boost, health={health:.1f}]"
        )
    ...
    log.info("")
    log.info("=== Done. submitted=%d failed=%d ===", submitted, failed)
