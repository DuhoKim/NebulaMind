...
assert.match(panelSource, /const closeOnEscape = \(event: KeyboardEvent\) => \{[\s\S]*?if \(event\.key === "Escape"\) closePanel\(\);[\s\S]*?\};/, "Escape should use the same closePanel path as the close button.");
