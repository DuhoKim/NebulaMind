...
export function getPlotBEmptyMessage(surveyCount: number): string {
  return surveyCount > 0
    ? `${surveyCount} matching survey${surveyCount === 1 ? "" : "s"}, but none have positive source counts and limiting magnitude values.`
    : "No surveys match the active search, status, and operator filters."
}

export function surveyHasPlotBData(
  survey: Pick<Survey, "num_sources_count" | "limiting_magnitude">
): boolean {
  return survey.num_sources_count != null
    && survey.num_sources_count > 0
    && survey.limiting_magnitude != null
}
...
export default function PlotB({
  ...
  const missingSurveys = useMemo(
    () => surveys.filter(s => !surveyHasPlotBData(s)),
    [surveys]
  )
      ...
      {(!xScaleFn || pts.length === 0) ? (
        <>
          <div style={{
            ...
            {getPlotBEmptyMessage(surveys.length)}
          </div>
        ...
        <>
          ...
          >
            ...
            <desc id={descId}>
              {`${pts.length} plotted of ${surveys.length} matching surveys. Rows outside the selected band are dimmed, not hidden. ${missingSurveys.length} survey${missingSurveys.length === 1 ? "" : "s"} not shown because source count is missing or non-positive, or limiting magnitude is missing.`}
            </desc>
              ...
              {pts.map(({ s, x, y }) => {
                ...
                return (
                  <g
                    ...
                    style={{ cursor: "pointer", opacity: active ? 1 : 0.15 }}
                  >
