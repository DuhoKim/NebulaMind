...
export default function PlotB({
  ...
  const missingSurveys = useMemo(
    () => surveys.filter(s => s.num_sources_count == null || s.limiting_magnitude == null),
    [surveys]
  )
  ...
  const { pts, xScaleFn, yScaleFn, xTicks, yTicks } = useMemo(() => {
    ...
    const pts = surveys
      .filter(s => s.num_sources_count != null && s.limiting_magnitude != null)
        ...
        <>
          ...
          {missingSurveys.length > 0 && (
            <div style={{ paddingLeft: ML, marginTop: "0.5rem" }}>
              ...
              >
                <span>+{missingSurveys.length} survey{missingSurveys.length !== 1 ? "s" : ""} not shown (missing data)</span>
              ...
              {missingExpanded && (
                <div style={{
                  ...
                  <div style={{ fontSize: "0.7" , color: "#475569", marginBottom: "0.35rem" }}>
                    Missing num_sources_count or limiting_magnitude:
                  </div>
