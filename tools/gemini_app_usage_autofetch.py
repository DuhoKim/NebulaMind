...
def build_reading(extract: dict, now: datetime) -> dict:
    ...
    weekly_label = str(extract.get('weekly_reset_label') or '')
    ...
    if extract.get('weekly_used_pct') is not None:
        reading.update({
            'weekly_used_pct': extract.get('weekly_used_pct'),
            'weekly_reset_label': weekly_label or None,
            'weekly_reset_at_utc': parse_reset(weekly_label, now) if weekly_label else None,
        })
    return reading
