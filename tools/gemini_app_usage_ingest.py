...
def parse_absolute_reset(label: str, captured_at: datetime) -> str | None:
    ...
    'Resets Jul 14 at 9:59 AM' -> absolute UTC, parsing the month/day as well."""
    import calendar
    match = re.search(r'(?:([A-Z]{3})\s+(\d{1,2})\s+)?at\s+(\d{1,2}):(\d{2})\s*([AaPp])\.?[Mm]\.?', label, re.IGNORECASE)
    if not match:
        return None
    ...
    if month_str and day_str:
        month_abbrs = {v.lower(): k for k, v in enumerate(calendar.month_abbr) if k}
        month = month_abbrs.get(month_str.lower())
        if month:
            reset = reset.replace(month=month, day=int(day_str))
            if reset < local - timedelta(days=14):
