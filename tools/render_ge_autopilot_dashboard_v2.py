...
OVERNIGHT_REPORT_MARKER = "GE_AUTOPILOT_C41_STEP2_FULLTEXT_20260804_COMPLETE"
LAST_INCIDENT = {
...
KIMI_BALANCE_CACHE_PATH = Path(os.environ.get(
    "GE_KIMI_BALANCE_CACHE_JSON",
    "/Users/duhokim/HermesOps/private-state/kimi-direct-balance.json",
))
...
YOUTUBE_QUOTA_STATUS_PATH = Path(os.environ.get(
    "GE_YOUTUBE_QUOTA_STATUS_JSON",
    "/Users/duhokim/HermesOps/scripts/clips/subnav_flow_female_voice_v8/youtube_review_checkpoint.json",
))
YOUTUBE_QUOTA_DOC_URL = "https://developers.google.com/youtube/v3/determine_quota_cost"
...
# Remapped 2026-08-05: the paper-to-wiki method lanes (Method 1/2/3 = mesh-ge-m{1,2,3}-*) were
...
def group_for(pane: Dict[str, Any]) -> str:
    """Bucket a pane by the surface it belongs to (2026-08-05 remap).

    The old buckets were the three paper-to-wiki method lanes, keyed off `m1/m2/m3` in the
    tmux target. Those sessions are gone. Directors = the mastermind window; Review = the
    reviewer seats (Kun, Miru) wherever they run; Lanes = task sessions the crew spawns
    (goru-agy and any future per-task session); Other = everything else, incl. renderers.
    """
...
def youtube_api_quota_card(
    checkpoint: Dict[str, Any] | None = None,
    observed_at: dt.datetime | None = None,
) -> Dict[str, Any]:
    """Build an official-limit card without inventing remaining quota."""
    if checkpoint is None:
        checkpoint = read_small_json(YOUTUBE_QUOTA_STATUS_PATH) or {}
    ...
    checkpoint_status = str(checkpoint.get("status") or "not recorded")
    reset_target_raw = str(checkpoint.get("quota_reset_target") or "")
    reset_target = parse_ts(reset_target_raw)
    rollback = checkpoint.get("rollback") if isinstance(checkpoint.get("rollback"), dict) else {}
    rollback_required = bool(rollback.get("required"))

    if "UPLOADS_UNLISTED" in checkpoint_status or "UPLOADS_COMPLETE" in checkpoint_status:
        status = "V8 review uploads complete"
        state_detail = "Rejected V7 rollback and corrected unlisted review uploads are recorded complete."
        tone = "ok"
    elif rollback_required and reset_target and observed_at < reset_target:
        status = "quota exhausted · reset pending"
        state_detail = f"Last videos.update returned quotaExceeded; local checkpoint expects reset at {reset_target_raw}."
        tone = "danger"
    elif reset_target and observed_at >= reset_target:
        status = "reset expected · not API-verified"
        state_detail = "The reset time has passed, but remaining quota is not exposed and no fresh API verification is recorded."
        tone = "warn"
    else:
        status = "daily limits · current balance not exposed"
        state_detail = "No safe API field reports exact remaining quota; the card shows official limits and the latest local checkpoint only."
        tone = "warn"

    return {
        "name": "YouTube Data API",
        "kind": "Google Cloud project daily quota",
        "status": status,
        "percent": None,
        "big": "10,000",
        "percent_label": "shared units/day · exact remaining amount not exposed by YouTube API",
        "activity": "100 video uploads/day · 100 searches/day · resets midnight Pacific",
        "detail": (
            f"{state_detail} Costs: videos.list 1 · videos.update 50 · captions.list 50 · "
            "captions.insert 400. Prepared V8 review packet: 4 upload calls + about 63 shared units; "
            "four manual captions later add 1,600 shared units."
        ),
        "source": f"Official YouTube quota calculator (updated 2026-06-01) + local checkpoint {YOUTUBE_QUOTA_STATUS_PATH.name}",
        "tone": tone,
        "sub_gauges": [
            {"label": "Shared API pool", "percent": None, "value_label": "10,000 units/day"},
            {"label": "Video upload calls", "percent": None, "value_label": "100/day"},
            {"label": "Search calls", "percent": None, "value_label": "100/day"},
            {"label": "V8 review packet", "percent": None, "value_label": "4 uploads · ~63 shared"},
            {"label": "Four manual captions", "percent": None, "value_label": "+1,600 shared"},
        ],
        "official_url": YOUTUBE_QUOTA_DOC_URL,
...
def build_overnight_report(usage_snapshot: Dict[str, Any]) -> Dict[str, Any]:
    usage_cards = usage_snapshot.get("cards") or []
    cards_count = len(usage_cards)
    ...
    flow_usage = next(
        (card for card in usage_cards if card.get("name") == "Flow / Veo credits (Ultra)"),
        {},
    )
    flow_status = flow_usage.get("status") or "STALE - needs capture"
    flow_big = flow_usage.get("big") or flow_usage.get("percent_label") or "Not captured"
    flow_detail = " ".join(
        text for text in (
            flow_usage.get("percent_label"),
            flow_usage.get("detail"),
        ) if text
    ) or "No fresh Flow balance is available; no live percentage is shown."
    return {
        "marker": OVERNIGHT_REPORT_MARKER,
        "reported_at_utc": now_utc(),
        "headline": "C41 Step 2 complete: full text acquired and extracted for all 180 sealed papers.",
        "next_action": "The sealed 180 now has a verified full-text manifest and source-strength labels. Hwao decides the next Baseline step; no later-stage execution is active.",
        ...
        "cards": [
            ...
            {"title": "Sealed corpus", "status": "EXACT SET PRESERVED", "big": "180 / 180",
             "detail": "Processed exactly the sealed C41 selection. No excluded records were readmitted and no new science-selection decision was made.",
             "tone": "healthy"},
            {"title": "Full-text access", "status": "COMPLETE", "big": "180 full text",
             "detail": "All 180 records extracted successfully. Abstract-only: 0. Metadata-only: 0. Failed: 0.",
             "tone": "healthy"},
            {"title": "Cache-first acquisition", "status": "POLITE / VERIFIED", "big": "42 hits · 138 fetched",
             "detail": "23.33% cache-hit rate. New arXiv PDF requests used the lane's polite interval and completed without a three-failure hard stop.",
             "tone": "healthy"},
            {"title": "Independent verification", "status": "PASS", "big": "180 paths checked",
             "detail": "Input and module hashes matched; record identities, cache paths, byte counts, access labels, and extraction thresholds all passed verification. Runtime: 691.565 seconds.",
             "tone": "healthy"},
            {"title": "Flow / Veo credits", "status": flow_status, "big": flow_big,
             "detail": flow_detail,
             "tone": "needs-review"},
            {"title": "Safety boundary", "status": "PRESERVED", "big": "0 risky actions",
             "detail": "No DB or SQL writes, product or wiki changes, deploys, restarts, git actions, Deep Research runs, browser actions, cron jobs, or credit-spending actions.",
             "tone": "healthy"},
        ]
    }
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .quota-glance-wrap { margin-top:18px; padding-top:14px; box-shadow:inset 0 1px rgba(118,215,255,.18); }
    .quota-glance-head { display:flex; justify-content:space-between; gap:12px; align-items:baseline; margin-bottom:9px; }
    .quota-glance-head strong { color:var(--soft); }
    .quota-glance-head a { color:#ace4ff; font-size:12px; text-decoration:none; }
    .quota-glance { display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); gap:8px; }
    .quota-glance-card { border:1px solid rgba(118,215,255,.20); border-radius:14px; padding:10px; background:rgba(6,18,31,.66); min-width:0; }
    .quota-glance-card b { display:block; color:var(--soft); font-size:22px; letter-spacing:-.04em; overflow-wrap:anywhere; }
    .quota-glance-card span { display:block; color:var(--muted); font-size:10px; text-transform:uppercase; letter-spacing:.08em; margin-bottom:4px; }
    .quota-glance-card small { color:var(--muted); font-size:10px; line-height:1.3; }
    ...
    @media (max-width:720px) { header { position:static; } .metrics,.summaries,.flow,.gate-list,.usage-grid,.overnight-grid,.quota-glance { grid-template-columns:1fr; } .state-word { font-size:44px; } }
  </style>
</head>
<body>
  <header>
    ...
    <section class="grid hero">
      <div class="panel room">
        <div>
          ...
          <div class="quota-glance-wrap">
            <div class="quota-glance-head"><strong>Live quota snapshot</strong><a href="#usage-monitor-panel">See all quota cards ↓</a></div>
            <div id="quota-glance" class="quota-glance"><div class="empty">Loading quota…</div></div>
          </div>
        </div>
        <div class="topline">
...
const GROUP_ORDER = ["Directors", "Review", "Lanes", "Other"];
...
function quotaPercentText(value) {
  if (value > 0 && value < 0.1) return `${value.toFixed(2)}%`;
  if (value > 0 && value < 1) return `${value.toFixed(1)}%`;
  return `${value.toFixed(0)}%`;
}
function quotaGlanceCard(name, value, detail) { return `<div class="quota-glance-card"><span>${esc(name)}</span><b>${esc(value)}</b><small>${esc(detail)}</small></div>`; }
function renderQuotaGlance(u) {
  ...
  const rendered = primary.map(card => {
    ...
    const value = card.big || (pct === null ? 'Unknown' : quotaPercentText(pct));
    return quotaGlanceCard(card.name, value, card.percent_label || card.status || 'not observed');
  });
...
function usageCard(u) {
  ...
  const big = u.big || (hasPct ? quotaPercentText(pct) : '—%');
