...
class TestGaugeProvenance:
    def test_chrome_auto_reads_as_unattended_not_operator_confirmed(self):
        ...
        assert gauge['kind'] == 'high-confidence unattended DOM capture of gemini.google.com/usage'
        assert 'extractor stores only trusted page-scoped signals' in gauge['detail']
        assert 'only ever an operator-confirmed reading' not in gauge['detail']
    ...
    def test_manual_still_reads_as_operator_confirmed(self):
        ...
        assert gauge['kind'] == 'human-confirmed capture of gemini.google.com/usage'
        assert 'captured and confirmed by the operator' in gauge['detail']
