...
import gemini_app_usage_autofetch as autofetch  # noqa: E402
...
class TestValidate:
    ...
    def test_preserves_distinct_current_window_and_weekly_limits(self):
        checked = gau.validate(
            reading(
                used_pct=2.0,
                reset_label='Resets at 6:59 PM',
                reset_at_utc='2026-07-10T09:59:00Z',
                weekly_used_pct=0.0,
                weekly_reset_label='Resets Jul 16 at 9:59 AM',
                weekly_reset_at_utc='2026-07-16T00:59:00Z',
            )
        )

        assert checked['used_pct'] == 2.0
        assert checked['weekly_used_pct'] == 0.0
        assert checked['weekly_reset_label'] == 'Resets Jul 16 at 9:59 AM'
        assert checked['weekly_reset_at_utc'] == '2026-07-16T00:59:00Z'

    def test_accepts_authenticated_accessibility_capture(self):
        checked = gau.validate(reading(capture_method='accessibility-verified'))
        assert checked['capture_method'] == 'accessibility-verified'
...
class TestRouteLine:
    ...
    def test_unknown_when_stale(self):
        old = gau.validate(reading(captured_at_utc=gau.format_utc(NOW - timedelta(hours=25))))
...
class TestStaleness:
    ...
    def test_reading_past_the_window_is_stale(self):
        assert gau.is_stale(reading(captured_at_utc=gau.format_utc(NOW - timedelta(hours=24, minutes=1))), NOW)
...
class TestGauge:
    def test_fresh_reading_shows_the_number(self):
        ...
        assert g['tone'] == 'ok'
        assert '30% current-window used' in g['value_label']
    ...
    def test_stale_reading_withholds_the_number(self):
        old = gau.validate(reading(used_pct=30.0, captured_at_utc=gau.format_utc(NOW - timedelta(hours=25))))
    ...
    def test_current_window_and_weekly_limits_render_as_separate_gauges(self):
        dual = gau.validate(
            reading(
                used_pct=2.0,
                reset_label='in 2 hr 20 min',
                reset_at_utc=gau.format_utc(NOW + timedelta(hours=2, minutes=20)),
                weekly_used_pct=0.0,
                weekly_reset_label='Resets Jul 16 at 9:59 AM',
                weekly_reset_at_utc=gau.format_utc(NOW + timedelta(days=5, hours=17)),
            )
        )

        gauge = gau.build_gauge(dual, NOW, 'obs')
        by_label = {item['label']: item for item in gauge['sub_gauges']}

        assert gauge['fill_pct'] == 2.0
        assert '2% current-window used' in gauge['value_label']
        assert by_label['Current-window used']['fill_pct'] == 2.0
        assert by_label['Weekly used']['fill_pct'] == 0.0
        assert '0% used' in by_label['Weekly used']['value_label']
        assert '2% current-window' in gauge['detail']
        assert '0% weekly' in gauge['detail']

    def test_accessibility_capture_has_distinct_provenance(self):
        capture = gau.validate(reading(capture_method='accessibility-verified'))
        gauge = gau.build_gauge(capture, NOW, 'obs')

        assert gauge['kind'] == 'authenticated accessibility capture of gemini.google.com/usage'
        assert gauge['status'].startswith('Authenticated accessibility capture')
        assert 'operator-confirmed' not in gauge['detail'].lower()
        assert 'unattended dom' not in gauge['detail'].lower()


class TestAutofetchDualMeter:
    def test_build_reading_carries_current_and_weekly_values(self):
        got = autofetch.build_reading(
            {
                'used_pct': 2.0,
                'reset_label': 'Resets at 6:59 PM',
                'weekly_used_pct': 0.0,
        ...
        assert got['used_pct'] == 2.0
        assert got['weekly_used_pct'] == 0.0
        assert got['weekly_reset_label'] == 'Resets Jul 16 at 9:59 AM'
        assert got['weekly_reset_at_utc'] is not None
